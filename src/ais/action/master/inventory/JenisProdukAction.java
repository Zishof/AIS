...
/**
 * Controller/action ZK untuk jenis produk. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * paging}, {@code MyGrid grid}, {@code Textbox searchnama}, {@code Textbox nama}, {@code MyDoublebox
 * maksimalHarian}, {@code Textbox keterangan}, {@code ais.action.master.akunting.helper.AmbilDataAkunBanbox
 * akunPendapatan}; inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code doAfterCompose()}, {@code init()},
 * {@code init()}, {@code initCriteria()}); pembacaan/pencarian ({@code onSearchDefault()}, {@code
 * tampilkanIntroDashboardInventoryV1()}); validasi/perhitungan ({@code checkNamaJenisProduk()}); mutasi data
 * ({@code onSave()}); operasi domain lain ({@code onAdd()}, {@code escapeDashboardHtmlInventoryV1()}). Bagian
...
public class JenisProdukAction extends GenericAutowireComposer
	...
	/** Kontra-pendapatan saat retur penjualan dijurnal; kosong = pakai akun pendapatan. */
	private ais.action.master.akunting.helper.AmbilDataAkunBanbox akunReturPenjualan;
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link JenisProdukAction}. Kelas ini menerjemahkan satu item data
	 * menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link JenisProdukAction} dan dapat mengakses state
	...
	private void init(JenisProduk jenisProduk) {
		...
		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Akun Retur Penjualan"));
		row.appendChild(akunReturPenjualan = new ais.action.master.akunting.helper.AmbilDataAkunBanbox(false));
		akunReturPenjualan.setWidth("90%");
		if (jenisProduk.getAkunReturPenjualan() != null) {
			akunReturPenjualan.setAttribute("akun", jenisProduk.getAkunReturPenjualan());
			akunReturPenjualan.setValue(jenisProduk.getAkunReturPenjualan().toString());
		}
	...
	public boolean onSave(Event event) throws Exception {
		...
		jenisProduk.setAkunReturPenjualan((ais.database.model.akunting.Akun) (akunReturPenjualan == null
				? null : akunReturPenjualan.getAttribute("akun")));
