...
/**
 * Penyusun/penyaji laporan untuk laporan lulus mahasiswa baru. Kelas ini mengubah data domain
 ...
 * Combobox searchjurusan}, {@code Combobox searchtahunAngkatan}, {@code Combobox searchprogram}, {@code Combobox
 * searchjenjang}, {@code Combobox searchjenisseleksi}, {@code Spreadsheet spreadsheet}, {@code Center center};
 ...
 * @see MyWindow
 */
public class LaporanLulusMahasiswaBaru extends MyWindow {
	...
	public LaporanLulusMahasiswaBaru() {
		super();
		try {
			init();
			initFakultas();
	...
	public LaporanLulusMahasiswaBaru(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		print.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				bout.close();
				Filedownload.save(bout.toByteArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Laporan_lulus_mahasiswa_baru.xlsx");
			}
		});
		print.setParent(toolbar);
	}
	...
	private void initSpreadsheet() {
		...
		biodataCalonMahasiswas = session.createCriteria(BiodataCalonMahasiswa.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))).addOrder(Order.asc("noUjian"))
		...
		// .add(Restrictions.eq("prodi_lulus", searchjurusan.getSelectedItem()
		// .getValue()))
		// .createCriteria("prodi_lulus")
		// .add(Restrictions.eq("fakultas", searchfakultas
		// .getSelectedItem().getValue())).list();
		...
		spreadsheet.setMaxrows(biodataCalonMahasiswas.size() + 8);
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 6, "Asal Sekolah");
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 9, "Jalur");

		Utils.setColumnWidth(sheet, 9, 100);
		...
		try {
			...
			e1.printStackTrace(); ais.common.ErrorAuditUtil.record(e1, "auto-audit src/ais/action/master/pmb/statistik/LaporanLulusMahasiswaBaru.java:346");
		}
		...
		for (BiodataCalonMahasiswa biodataCalonMahasiswa : biodataCalonMahasiswas) {
			...
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, colIndex, biodataCalonMahasiswa.getNoRegistrasi());
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, colIndex + 1, biodataCalonMahasiswa.getNoUjian());
			...
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, colIndex + 4, biodataCalonMahasiswa.getAsalSma());
			...
			try {
				ais.ui.util.EcampusUtil.setBorder(sheet,
						...
						BorderStyle.THIN, color);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/pmb/statistik/LaporanLulusMahasiswaBaru.java:375");
			}
			index++;
			rowIndex++;
		}
