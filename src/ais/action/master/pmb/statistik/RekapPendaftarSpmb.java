...
/**
 * Tipe khusus untuk rekap pendaftar spmb. Kelas ini memberi nama dan batas tanggung jawab yang
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code MyGrid grid}, {@code Tabbox
 * tabboxPendaftarSpmbInduk}, {@code Tabbox tabboxDashboardPmb}, {@code Tabbox tabboxRekapPmb}, {@code Tabbox
 * tabboxStatistikPmb}, {@code int tabAktifInduk}, {@code int tabAktifDashboard}, {@code int tabAktifRekap};
 * inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code doAfterCompose()}); pembacaan/pencarian ({@code
 * onDasborTarget()}, {@code getFilterTa()}, {@code getFilterSem()}, {@code onSearchDefault()}, {@code
 * tampilandata()}); pelaporan/ekspor ({@code onReportPendaftarSPMB()}, {@code onReportPeminat()}, {@code
 * onReportLulus()}, {@code onReportDapatNim()}); operasi domain lain ({@code gantiTabboxNativeKeButton()},
 * {@code onTabDashboardPmb()}, {@code onTabRekapPmb()}, {@code onTabStatistikPmb()}, {@code onStatistik()},
 * {@code onDasbor()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di
 * atas.</p>
