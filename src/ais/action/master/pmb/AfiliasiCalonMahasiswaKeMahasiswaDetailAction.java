...
/**
 * Controller/action ZK untuk afiliasi calon mahasiswa ke mahasiswa detail. Tipe ini merupakan
 ...
 * tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code Mahasiswa afiliasiMahasiswa}, {@code
 * MyGrid grid}, {@code Textbox pencarian}, {@code Combobox searchTahunAjaran}; inisialisasi/lifecycle ({@code
 ...
 * @see MyDetail
 */
public class AfiliasiCalonMahasiswaKeMahasiswaDetailAction extends MyDetail implements DataCriteria {
	...
	public AfiliasiCalonMahasiswaKeMahasiswaDetailAction(Mahasiswa afiliasiMahasiswa, Combobox searchTahunAjaran) {
		super();
		...
		this.addEventListener(Events.ON_OPEN, new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				Common.clear(AfiliasiCalonMahasiswaKeMahasiswaDetailAction.this);
				if (isOpen()) {
					display();
				}
			}
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link AfiliasiCalonMahasiswaKeMahasiswaDetailAction}. Kelas ini
	 * menerjemahkan satu item data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik
	 * kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link AfiliasiCalonMahasiswaKeMahasiswaDetailAction}
	 ...
	 *
	 * @see AfiliasiCalonMahasiswaKeMahasiswaDetailAction
	 */
	...
	public void display() {
		...
		groupbox.appendChild(
				new MyCaptionStyled("Daftar calon mahasiswa yang masuk afiliasi " + afiliasiMahasiswa.getNama()));
							...
							public void onEvent(Event arg0) throws Exception {
								...
								if (biodataCalonMahasiswas != null) {
									...
									for (BiodataCalonMahasiswa biodataCalonMahasiswa : biodataCalonMahasiswas) {
										biodataCalonMahasiswa.setAfiliasiMahasiswa(afiliasiMahasiswa);
	...
	public Criteria initCriteria(boolean order) {
		...
		return session.createCriteria(BiodataCalonMahasiswa.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
				...
				.addOrder(Order.desc("id")).add(Restrictions.eq("afiliasiMahasiswa", afiliasiMahasiswa));
	}
