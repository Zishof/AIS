...
/**
 * Controller/action ZK untuk cari data peserta ujian. Tipe ini merupakan titik masuk UI yang
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code Textbox noRegistrasi}, {@code MyGrid
 * grid}, {@code Textbox pinPassword}, {@code Row tampilanPin}, {@code BiodataCalonMahasiswa
 * biodataCalonMahasiswa}, {@code North uploadMenu}, {@code North menuLogin}, {@code Combobox tahun};
 * inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code doAfterCompose()}, {@code afterLogin()});
 * pembacaan/pencarian ({@code onSearchDefault()}); mutasi data ({@code onReset()}); operasi domain lain ({@code
 * onLogin()}, {@code genInfo()}, {@code genInfo()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau
...
//		row.appendChild(new ais.ui.util.MyLabelBold(biodataCalonMahasiswa.getNama()));
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link CariDataPesertaUjianAction}. Kelas ini menerjemahkan satu item
	 * data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link CariDataPesertaUjianAction} dan dapat
	 ...
	 *
	 * @see CariDataPesertaUjianAction
	 */
