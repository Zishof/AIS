...
/**
 * Controller/action ZK untuk pengumuman akademis. Tipe ini merupakan titik masuk UI yang
 ...
 * paging}, {@code MyWindow addWindowAttachment}, {@code MyGrid grid}, {@code Textbox judul}, {@code MyCkEditor
 * catatan}, {@code Combobox diperuntukkan}, {@code MyDatebox tanggal}; inisialisasi/lifecycle ({@code
 * doBeforeCompose()}, {@code doAfterCompose()}, {@code init()}, {@code init()}, {@code initCriteria()}, {@code
 * initIsiPolling()}); pembacaan/pencarian ({@code tampilkanButton()}, {@code reloadDataGambar()}, {@code
 * tampilkanPolling()}, {@code tandaiKehadiranHomeDitampilkan()}, {@code isKehadiranHomeDitampilkan()}, {@code
 * tampilkanKehadiranDosen()}); mutasi data ({@code onSave()}); operasi domain lain ({@code
 * onPengumumanPerkuliahan()}, {@code onPenumumanWebsite()}, {@code onKategoriPengumuman()}, {@code
 * onTeksBerjalan()}, {@code diperuntukkan()}, {@code onAdd()}). Bagian lain dari kontrak tetap mengikuti kelas
...
public class PengumumanAkademisAction extends GenericAutowireComposer implements DataInitDefault {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link PengumumanAkademisAction}. Kelas ini menerjemahkan satu item data
	 * menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link PengumumanAkademisAction} dan dapat mengakses
