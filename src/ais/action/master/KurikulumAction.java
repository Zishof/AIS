...
/**
 * Controller/action ZK untuk kurikulum. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * paging}, {@code MyGrid grid}, {@code Intbox tahun}, {@code Textbox keterangan}, {@code Combobox
 * searchfakultas}, {@code Combobox searchjurusan}, {@code Combobox searchjenjang}; inisialisasi/lifecycle
 * ({@code doBeforeCompose()}, {@code doAfterCompose()}, {@code init()}, {@code initCriteria()});
 * pembacaan/pencarian ({@code onTampilKurikulum()}, {@code onTampilKurikulumMahasiswa()}, {@code
 * onTampilRekapKurikulum()}, {@code onSearchDefault()}); mutasi data ({@code prosesUlangTagihan()}, {@code
 * onSave()}, {@code onSaveCopy()}); operasi domain lain ({@code onPilihTabKurikulum()}, {@code onKurikulum()},
 * {@code onRps()}, {@code onStatistik()}, {@code onPikobe()}, {@code onRekapPertemuanPerkuliahan()}). Bagian
...
public class KurikulumAction extends GenericAutowireComposer implements DataCriteria {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link KurikulumAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link KurikulumAction} dan dapat mengakses state
