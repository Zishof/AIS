...
public final class SocialSmartlinkCredentialService {
 ...
 public void verifyCallback(String remoteAddress,String signature,String raw){Credential c=forCallback(raw);if(empty(c.getCallbackSecret())||c.getCallbackSecret().length()<32||empty(c.getAllowedIps()))throw new SecurityException("Keamanan callback SosialChannel belum lengkap.");boolean allowed=false;for(String ip:c.getAllowedIps().split(","))if(remoteAddress!=null&&remoteAddress.equals(ip.trim())){allowed=true;break;}if(!allowed)throw new SecurityException("Source callback ditolak.");if(signature==null||!signature.trim().matches("(?i)[a-f0-9]{64}")||!SocialSecurity.constantEquals(SocialSecurity.hmacSha256(c.getCallbackSecret(),raw),signature.trim().toLowerCase()))throw new SecurityException("Signature callback ditolak.");}
