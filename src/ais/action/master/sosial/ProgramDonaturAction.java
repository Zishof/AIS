...
/**
 * Controller/action ZK untuk program donatur. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * {@code boolean edit}, {@code boolean delete}, {@code ProgramDonatur programDonatur}; inisialisasi/lifecycle
 * ({@code doBeforeCompose()}, {@code doAfterCompose()}, {@code init()}, {@code init()}, {@code initCriteria()});
 * pembacaan/pencarian ({@code tampilkanButton()}, {@code reloadDataGambar()}, {@code onSearchDefault()}, {@code
 * ambil()}, {@code ambilClass()}); mutasi data ({@code onSave()}, {@code setPersetujuan()}); pelaporan/ekspor
 * ({@code cetakData()}); operasi domain lain ({@code onAdd()}, {@code form()}, {@code istilah()}). Bagian lain
...
//		add.getParent().appendChild(upload);
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link ProgramDonaturAction}. Kelas ini menerjemahkan satu item data
	 * menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link ProgramDonaturAction} dan dapat mengakses
