...
/**
 * Controller/action ZK untuk item kpi tree. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code Window addWindow}, {@code MyLabelConfig
 * searchLabelFormatKpi}, {@code AmbilDataFormatKpiDetailBanbox searchFormatKpi}, {@code Tree tree}, {@code Vbox
 * kpi}, {@code MyIntbox nomorUrut}, {@code Textbox keterangan}, {@code boolean edit}; inisialisasi/lifecycle
 * ({@code doAfterCompose()}, {@code initTree()}, {@code init()}); pembacaan/pencarian ({@code tampilkanKunci()},
 * {@code reloadDataFormula()}, {@code reloadFormula()}, {@code onReloadTree()}, {@code reloadTreeitem()}, {@code
 * reloadTreeitem()}); mutasi data ({@code onSave()}); pelaporan/ekspor ({@code onCetak()}); operasi domain lain
 * ({@code onAdd()}, {@code openChilds()}, {@code closeChilds()}, {@code hasSomeChilds()}). Bagian lain dari
