...
/**
 * Controller/action ZK untuk pengajuan kpi. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * {@code MyTextbox searchnama}, {@code MyTextbox searchnamaPegawai}, {@code Combobox searchtahun}, {@code
 * boolean persetujuan}, {@code MyWindow addWindow}, {@code PengajuanKpi pengajuanKpi}; inisialisasi/lifecycle
 * ({@code doAfterCompose()}, {@code initCriteria()}, {@code init()}); pembacaan/pencarian ({@code
 * onSearchDefault()}, {@code ambil()}, {@code ambilClass()}, {@code getindex()}); mutasi data ({@code onSave()},
 * {@code setPersetujuan()}); pelaporan/ekspor ({@code cetakData()}); operasi domain lain ({@code onAdd()},
 * {@code form()}, {@code istilah()}, {@code generateCode()}). Bagian lain dari kontrak tetap mengikuti kelas
...
public class PengajuanKpiAction extends GenericAutowireComposer implements DataInitDefault, FormSop {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link PengajuanKpiAction}. Kelas ini menerjemahkan satu item data
	 * menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link PengajuanKpiAction} dan dapat mengakses state
