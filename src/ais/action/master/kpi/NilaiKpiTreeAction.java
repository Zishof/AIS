...
/**
 * Controller/action ZK untuk nilai kpi tree. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code Tree tree}, {@code
 * AmbilDataPegawaiFormatKPIBanbox pegawaiBanbox}, {@code Combobox ta}, {@code ItemKpiTreeModel
 * itemKpiTreeModel}, {@code TreeMap treecellMap}, {@code Date sekarang}, {@code MyLabelConfig
 * pegawaiBanboxLabel}, {@code MyLabelBold totalTarget}; inisialisasi/lifecycle ({@code doAfterCompose()}, {@code
 * initTree()}); pembacaan/pencarian ({@code tampilkanKunci()}, {@code onReloadTreeRefresh()}, {@code
 * onReloadTree()}, {@code reload()}, {@code reloadTreeitem()}, {@code reloadTreeitem()}); pelaporan/ekspor
 * ({@code onCetak()}); operasi domain lain ({@code openChilds()}, {@code closeChilds()}, {@code
