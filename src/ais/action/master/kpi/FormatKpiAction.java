...
/**
 * Controller/action ZK untuk format kpi. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * searchaktif}, {@code Textbox searchpegawai}, {@code AmbilDataSatuanKerjaBanbox searchparent};
 * inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code doAfterCompose()}, {@code init()}, {@code init()},
 * {@code initCriteria()}); pembacaan/pencarian ({@code onSearchDefault()}); validasi/perhitungan ({@code
 * checkNamaFormatKpi()}); mutasi data ({@code onSave()}); operasi domain lain ({@code onManajemenMasaKpi()},
 * {@code onNilaiDefaultKpi()}, {@code onAdd()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau
...
public class FormatKpiAction extends GenericAutowireComposer
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link FormatKpiAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link FormatKpiAction} dan dapat mengakses state
