...
/**
 * Controller/action ZK untuk kpi. Tipe ini merupakan titik masuk UI yang menghubungkan event layar
 ...
 * searchkategori}, {@code Checkbox searchaktif}, {@code Checkbox fontBesar}; inisialisasi/lifecycle ({@code
 * doBeforeCompose()}, {@code doAfterCompose()}, {@code init()}, {@code init()}, {@code initCriteria()});
 * pembacaan/pencarian ({@code reloadDataFormula()}, {@code reloadFormula()}, {@code onSearchDefault()});
 * validasi/perhitungan ({@code checkNamaKpi()}); mutasi data ({@code onSave()}); operasi domain lain ({@code
 * onManajemenParameter()}, {@code onKonstanta()}, {@code onAdd()}). Bagian lain dari kontrak tetap mengikuti
...
//				onSearchDefault(null);
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link KpiAction}. Kelas ini menerjemahkan satu item data menjadi baris
	 * atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link KpiAction} dan dapat mengakses state kelas
