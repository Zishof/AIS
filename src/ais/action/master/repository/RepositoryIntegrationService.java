...
public class RepositoryIntegrationService {
    ...
    public Result authenticateOrcid(Long authorityId,String code,Tbmuser actor,String requestId){requireAdmin(actor);String client=property("ais.repository.orcidClientId","");String secret=property("ais.repository.orcidClientSecret","");String redirect=property("ais.repository.orcidRedirectUri","");if(client.length()==0||secret.length()==0||redirect.length()==0)return auditResult(null,"ORCID","AUTHENTICATE","NOT_CONFIGURED","","","ORCID OAuth belum lengkap.",actor,requestId,false,false);try{String form="client_id="+URLEncoder.encode(client,"UTF-8")+"&client_secret="+URLEncoder.encode(secret,"UTF-8")+"&grant_type=authorization_code&code="+URLEncoder.encode(clean(code),"UTF-8")+"&redirect_uri="+URLEncoder.encode(redirect,"UTF-8");HttpResult response=httpForm(property("ais.repository.orcidTokenUrl","https://orcid.org/oauth/token"),form);JSONObject body=new JSONObject(response.body);String orcid=clean(body.optString("orcid"));if(response.code<200||response.code>=300||orcid.length()==0)return auditResult(null,"ORCID","AUTHENTICATE","FAILED","",safeOrcidResponse(body),"HTTP "+response.code,actor,requestId,false,true);Session session=HibernateUtil.openSession();Transaction tx=null;try{tx=session.beginTransaction();RepoAuthorAuthority authority=(RepoAuthorAuthority)session.get(RepoAuthorAuthority.class,authorityId);if(authority==null||!RepositoryTenantScope.currentKey().equals(authority.getTenantKey()))throw new IllegalArgumentException("Authority penulis tidak ditemukan.");authority.setOrcid(orcid);authority.setVerified(Boolean.TRUE);authority.setUpdatedAt(new Date());session.update(authority);tx.commit();}catch(Exception e){rollback(tx);throw e;}finally{HibernateUtil.closeSessionQuietly(session);}return auditResult(null,"ORCID","AUTHENTICATE","SUCCESS","",safeOrcidResponse(body),"",actor,requestId,true,true);}catch(Exception e){return auditResult(null,"ORCID","AUTHENTICATE","FAILED","","",e.getMessage(),actor,requestId,false,true);}}
    public Result matchRor(Long authorityId,String query,Tbmuser actor,String requestId){requireAdmin(actor);String endpoint=property("ais.repository.rorUrl","https://api.ror.org");try{HttpResult response=httpGet(endpoint+"/organizations?query="+URLEncoder.encode(clean(query),"UTF-8"),"");JSONObject body=new JSONObject(response.body);JSONArray items=body.optJSONArray("items");if(items==null)items=body.optJSONArray("organizations");if(response.code<200||response.code>=300||items==null||items.length()==0)return auditResult(null,"ROR","MATCH","FAILED",clean(query),limit(response.body,5000),"Afiliasi ROR tidak ditemukan.",actor,requestId,false,true);JSONObject first=items.getJSONObject(0);JSONObject organization=first.optJSONObject("organization");if(organization!=null)first=organization;String id=clean(first.optString("id"));String name=clean(first.optString("name"));if(name.length()==0&&first.optJSONArray("names")!=null&&first.optJSONArray("names").length()>0)name=clean(first.optJSONArray("names").getJSONObject(0).optString("value"));Session session=HibernateUtil.openSession();Transaction tx=null;try{tx=session.beginTransaction();RepoAuthorAuthority authority=(RepoAuthorAuthority)session.get(RepoAuthorAuthority.class,authorityId);if(authority==null||!RepositoryTenantScope.currentKey().equals(authority.getTenantKey()))throw new IllegalArgumentException("Authority penulis tidak ditemukan.");authority.setRorId(id);if(name.length()>0)authority.setAffiliation(name);authority.setUpdatedAt(new Date());session.update(authority);tx.commit();}catch(Exception e){rollback(tx);throw e;}finally{HibernateUtil.closeSessionQuietly(session);}return auditResult(null,"ROR","MATCH","SUCCESS",clean(query),new JSONObject().put("id",id).put("name",name).toString(),"",actor,requestId,true,true);}catch(Exception e){return auditResult(null,"ROR","MATCH","FAILED",clean(query),"",e.getMessage(),actor,requestId,false,true);}}
    ...
    public boolean orcidConfigured(){return property("ais.repository.orcidClientId","").length()>0&&property("ais.repository.orcidClientSecret","").length()>0&&property("ais.repository.orcidRedirectUri","").length()>0;}
    ...
    public Map<String,String> deploymentChecks(){
        Map<String,String> checks=new LinkedHashMap<String,String>();
        checks.put("Hibernate authority",classAvailable("ais.database.model.repository.RepoAuthorAuthority")?"SIAP":"GAGAL");
        checks.put("Hibernate feedback",classAvailable("ais.database.model.repository.RepoHelpFeedback")?"SIAP":"GAGAL");
        checks.put("Hibernate integration audit",classAvailable("ais.database.model.repository.RepoIntegrationEvent")?"SIAP":"GAGAL");
        String storage=property("ais.repository.storage",System.getenv("AIS_REPOSITORY_STORAGE"));File storageDir=storage.length()==0?null:new File(storage);
        checks.put("Storage",storageDir!=null&&storageDir.isDirectory()&&storageDir.canWrite()?"WRITABLE: "+storage:"BELUM WRITABLE");
        String scanner=property("ais.repository.virusScanner","");File scannerFile=scanner.length()==0?null:new File(scanner);
        checks.put("Antivirus",scannerFile!=null&&scannerFile.isFile()&&scannerFile.canExecute()?"SIAP":"BELUM SIAP");
        String salt=property("ais.repository.analyticsSalt","");checks.put("Analytics salt",salt.length()>=24&&!"AIS-REPOSITORY".equals(salt)?"SIAP":"WAJIB DIGANTI");
        checks.put("OAI token secret",property("ais.repository.oaiTokenSecret","").length()>=32?"SIAP":"WAJIB >= 32 KARAKTER");
        checks.put("Public base URL",validHttpBase(property("ais.repository.publicBaseUrl",""))?"SIAP":"WAJIB URL HTTP(S) VALID");
        checks.put("Scanner timeout",property("ais.repository.virusScannerTimeoutSeconds","120")+" detik");
        checks.put("Search alert",config("repository_search_alerts","Aktif"));
        checks.put("Semantic candidate limit",property("ais.repository.semanticCandidateLimit","750"));
        checks.put("DataCite",dataCiteConfigured()?"TERKONFIGURASI":"NONAKTIF");checks.put("ORCID",orcidConfigured()?"TERKONFIGURASI":"NONAKTIF");checks.put("ROR",rorConfigured()?"TERKONFIGURASI":"DEFAULT API");checks.put("COAR Notify",coarConfigured()?"TERKONFIGURASI":"NONAKTIF");checks.put("AI endpoint",aiConfigured()?"TERKONFIGURASI":"FALLBACK LOKAL");checks.put("OAI-PMH","/oai?verb=Identify");return checks;
    }
    private boolean classAvailable(String name){try{Class.forName(name);return true;}catch(Throwable e){return false;}}
    private boolean validHttpBase(String value){try{URL url=new URL(clean(value));String path=clean(url.getPath());return ("http".equalsIgnoreCase(url.getProtocol())||"https".equalsIgnoreCase(url.getProtocol()))&&clean(url.getHost()).length()>0&&url.getUserInfo()==null&&url.getQuery()==null&&url.getRef()==null&&(path.length()==0||"/".equals(path));}catch(Exception e){return false;}}
    ...
    private HttpResult httpForm(String url,String form)throws Exception{HttpURLConnection c=(HttpURLConnection)new URL(url).openConnection();c.setConnectTimeout(10000);c.setReadTimeout(20000);c.setRequestMethod("POST");c.setRequestProperty("Accept","application/json");c.setRequestProperty("Content-Type","application/x-www-form-urlencoded");c.setDoOutput(true);OutputStream out=c.getOutputStream();try{out.write(form.getBytes("UTF-8"));}finally{out.close();}HttpResult r=new HttpResult();r.code=c.getResponseCode();r.body=read(r.code>=400?c.getErrorStream():c.getInputStream(),200000);c.disconnect();return r;}
    private HttpResult httpGet(String url,String authorization)throws Exception{HttpURLConnection c=(HttpURLConnection)new URL(url).openConnection();c.setConnectTimeout(10000);c.setReadTimeout(20000);c.setRequestMethod("GET");c.setRequestProperty("Accept","application/json");if(clean(authorization).length()>0)c.setRequestProperty("Authorization",authorization);HttpResult r=new HttpResult();r.code=c.getResponseCode();r.body=read(r.code>=400?c.getErrorStream():c.getInputStream(),200000);c.disconnect();return r;}
    private String safeOrcidResponse(JSONObject body){try{return new JSONObject().put("orcid",body.optString("orcid")).put("name",body.optString("name")).toString();}catch(Exception e){return "";}}
