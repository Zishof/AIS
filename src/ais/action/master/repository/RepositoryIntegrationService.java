...
public class RepositoryIntegrationService {
    ...
    public Map<String,String> deploymentChecks(){Map<String,String> checks=new LinkedHashMap<String,String>();checks.put("Hibernate authority",classAvailable("ais.database.model.repository.RepoAuthorAuthority")?"SIAP":"GAGAL");checks.put("Hibernate integration audit",classAvailable("ais.database.model.repository.RepoIntegrationEvent")?"SIAP":"GAGAL");String storage=property("ais.repository.storage",System.getenv("AIS_REPOSITORY_STORAGE"));checks.put("Storage",storage.length()>0?storage:"DEFAULT");checks.put("Antivirus",property("ais.repository.virusScanner","").length()>0?"TERKONFIGURASI":"PENDING (unggahan tetap divalidasi signature/checksum)");checks.put("DataCite",dataCiteConfigured()?"TERKONFIGURASI":"NONAKTIF");checks.put("ORCID",orcidConfigured()?"TERKONFIGURASI":"NONAKTIF");checks.put("ROR",rorConfigured()?"TERKONFIGURASI":"DEFAULT API");checks.put("COAR Notify",coarConfigured()?"TERKONFIGURASI":"NONAKTIF");checks.put("AI endpoint",aiConfigured()?"TERKONFIGURASI":"FALLBACK LOKAL");checks.put("OAI-PMH","/oai?verb=Identify");return checks;}
