...
/**
 * Controller/action ZK untuk capaian lulusan vs capaian pembelajaran lulusan. Tipe ini merupakan
 ...
 * jurusan}, {@code Center center}, {@code java.util.List loadedCpl}, {@code java.util.List loadedCpmk}, {@code
 * PerguruanTinggi perguruanTinggi}, {@code Textbox nama}, {@code Textbox nama1}; inisialisasi/lifecycle ({@code
 * initKHS()}, {@code init()}); operasi domain lain ({@code onKHS()}, {@code showMatriks()}, {@code
 * createTransferAdapter()}, {@code esc()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface
...
public class CapaianLulusanVsCapaianPembelajaranLulusanAction extends Div {
	...
	private void init() throws Exception {
		...
		Div obeNorth = ObePageHelpHelper.pasangPadaNorth(west, "CPL vs CPMK",
				"Hubungkan CPL dengan CPMK yang berkontribusi mencapainya. Centang CPMK yang sesuai untuk membentuk pemetaan dan evaluasi ketercapaian pembelajaran.");
		...
		Common.createDefaultTimer(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				try {
					...
					Jurusan jurusan = (Jurusan) CapaianLulusanVsCapaianPembelajaranLulusanAction.this.jurusan
							.getSelectedItem().getValue();
					List<CapaianPembelajaranLulusan> bapaianPembelajaranLulusans = ConstantValues.simpleList(
							HibernateUtil.currentSession().createCriteria(CapaianPembelajaranLulusan.class)
									...
									.add(nama1.getValue().trim().isEmpty() ? Restrictions.sqlRestriction("true")
											: Restrictions.or(
													Restrictions.ilike("kode", nama1.getValue().trim(),
															MatchMode.ANYWHERE),
													Restrictions.ilike("nama", nama1.getValue().trim(),
															MatchMode.ANYWHERE)))
					...
					for (CapaianPembelajaranLulusan bapaianPembelajaranLulusan : bapaianPembelajaranLulusans) {
						column = new MyColumnConfig(bapaianPembelajaranLulusan.getKode());
						column.setParent(columns);
					...
					loadedCpmk = bapaianPembelajaranLulusans;
					...
					for (final CapaianLulusan capaianLulusan : capaianLulusans) {
						...
						for (final CapaianPembelajaranLulusan bapaianPembelajaranLulusan : bapaianPembelajaranLulusans) {
	...
	private void showMatriks() {
		if (loadedCpl == null || loadedCpmk == null || loadedCpl.isEmpty()) {
			try {
				MyMessageboxConfig.show("Klik Refresh dahulu untuk memuat data.", "Info",
						MyMessageboxConfig.OK, MyMessageboxConfig.INFORMATION);
			} catch (Exception ex) { ais.common.ErrorAuditUtil.record(ex, "auto-audit(empty-catch) src/ais/action/master/obe/CapaianLulusanVsCapaianPembelajaranLulusanAction.java:322"); /* ignore */ }
			return;
		}
		...
		sb.append("<h3>Matriks CPL &times; CPMK</h3>");
		sb.append("<table class='mxt'><thead><tr>");
		sb.append("<th class='corner lkode'>CPL</th><th class='corner lnama'>Deskripsi CPL</th>");
		for (CapaianPembelajaranLulusan cpmk : loadedCpmk) {
			sb.append("<th class='hcol' title='").append(esc(cpmk.getNama())).append("'>")
			  .append(esc(cpmk.getKode())).append("</th>");
		}
		sb.append("<th class='corner tot'>#</th></tr></thead><tbody>");
		int[] colTotals = new int[loadedCpmk.size()];
		int grand = 0;
		...
		int max = loadedCpl.size() * loadedCpmk.size();
		...
		sb.append("<div class='summ'>Cakupan: <b>").append(grand).append("/").append(max)
		  ...
		  .append(loadedCpl.size()).append(" CPL &times; ").append(loadedCpmk.size()).append(" CPMK</div>");
		sb.append("</div>");
		MyWindow win = new MyWindow("Matriks CPL × CPMK", "normal", true);
		win.setWidth("90%");
		win.setHeight("88%");
