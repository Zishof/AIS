...
/**
 * Controller/action ZK untuk hasil spmi. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code Combobox searchjenis}, {@code Combobox
 * searchstatus}, {@code Combobox searchfakultas}, {@code Combobox searchjurusan}, {@code MyDatebox start},
 * {@code MyDatebox end}, {@code Textbox nama}, {@code Textbox keterangan}; inisialisasi/lifecycle ({@code
 * doAfterCompose()}, {@code init()}, {@code initCriteria()}); pembacaan/pencarian ({@code tampilRinci()}, {@code
 * onSearchDefault()}, {@code ambil()}, {@code ambilClass()}); mutasi data ({@code onSave()}, {@code
 * setPersetujuan()}); pelaporan/ekspor ({@code cetakData()}, {@code cetak()}); operasi domain lain ({@code
 * onDasbor()}, {@code buildLaporan()}, {@code onAdd()}, {@code form()}, {@code buildFormWindow()}, {@code
 * istilah()}); konfigurasi constructor: {@code tbmuser}. Bagian lain dari kontrak tetap mengikuti kelas induk
...
public class HasilSPMIAction extends BaseSPMIAction implements FormSop {
    ...
    /**
     * Renderer lokal untuk layar/komponen {@link HasilSPMIAction}. Kelas ini menerjemahkan satu item data menjadi
     * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
     *
     * <p><b>Scope:</b> setiap instance terikat pada instance {@link HasilSPMIAction} dan dapat mengakses state
