...
import ais.action.report.format1.library.LaporanKartuAnggota;
...
public class AnggotaAction extends GenericAutowireComposer implements DataCriteria, DataSearchDefault {
	...
	private Combobox searchTipeAnggota;
	private Combobox searchJenisAnggota;
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		Common.insertComboDanSemua(searchTipeAnggota, "nama", TipeAnggota.class);
		Common.insertComboDanSemua(searchJenisAnggota, "nama", JenisAnggota.class);
		Common.insertComboDanSemua(searchProdi, "nama", Jurusan.class, Restrictions.eq("aktif", true));
		...
		MyToolbarbuttonConfig barcode = new MyToolbarbuttonConfig("Cetak Kartu Anggota", "/img/album.png");
		if (barcode != null) { barcode.setParent(add.getParent()); }
		barcode.addEventListener("onClick", new EventListener() {

			@Override
			public void onEvent(Event arg0) throws Exception {

				LaporanKartuAnggota laporanKartuAnggota = new LaporanKartuAnggota();
				laporanKartuAnggota.setTitle("Cetak Kartu Anggota");
				laporanKartuAnggota.setClosable(true);
				page.getFirstRoot().appendChild(laporanKartuAnggota);
				laporanKartuAnggota.setHeight("95%");
				laporanKartuAnggota.setWidth("85%");
				laporanKartuAnggota.onModal();
			}
		});

		String[] contents = new String[] { "id", "kodeIdentitas", "jenisIdentitas", "kode", "nama", "alamat",
				"perpustakaan", "mahasiswa", "dosen", "pegawai", "tbmuser", "jenisAnggota", "tipe", "keterangan",
				"telp", "hp", "email", "jenisIdentitasAnggota", "tipeAnggota", "aktif", "tanggal", "" };

		MyToolbarbuttonConfig cetakToolbarbutton = Common.cetakData(this, contents);
		Common.appendKeToolbar(cetakToolbarbutton, add, comp);

		MyToolbarbuttonConfig upload = Common.uploadData(this, Anggota.class, contents);
		...
		singkron.setVisible(
				Common.bolehKonfigurasi("singkronkan_mahasiswa_dengan_anggota_perpustakaan"));
		singkron.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				final Intbox tahunAngkatanMulai = new Intbox(ais.ui.util.WaktuUtil.getCalendar().get(Calendar.YEAR));
				final Intbox tahunAngkatanSampai = new Intbox(ais.ui.util.WaktuUtil.getCalendar().get(Calendar.YEAR));
				...
				row.appendChild(tahunAngkatanMulai);
				tahunAngkatanMulai.setWidth("90%");

				row = new MyFormRow();
				row.setValign("top");
				row.setParent(rows);
				row.appendChild(new ais.ui.util.MyLabelConfig("Tahun Angkatan Sampai *"));
				row.appendChild(tahunAngkatanSampai);
				tahunAngkatanSampai.setWidth("90%");
				...
				save.addEventListener("onClick", new EventListener() {
					...
					public void onEvent(Event event) throws Exception {
						...
						if (tahunAngkatanMulai.getValue() == null || tahunAngkatanSampai.getValue() == null) {
							MyMessageboxConfig.show("Tahun angkatan mulai dan sampai harus diisi", "Peringatan", MyMessageboxConfig.OK,
									MyMessageboxConfig.INFORMATION);
							return;
						}
						if (tahunAngkatanMulai.getValue().intValue() > tahunAngkatanSampai.getValue().intValue()) {
							MyMessageboxConfig.show("Tahun angkatan mulai tidak boleh lebih besar dari tahun angkatan sampai",
									"Peringatan", MyMessageboxConfig.OK, MyMessageboxConfig.INFORMATION);
							return;
						}

						final ais.common.LaporanUpload laporan = new ais.common.LaporanUpload("Sinkronisasi Anggota Perpustakaan Mahasiswa");
						...
						new Thread(new Runnable() {
							...
							public void run() {
								...
								List<String> ids = session.createCriteria(Mahasiswa.class)
										...
										.add(Restrictions.between("tahunangkatan", tahunAngkatanMulai.getValue(),
												tahunAngkatanSampai.getValue()))
								...
								int i = 1;
								for (String nim : ids) {
									label.setValue("Singkronkan anggota perpustakaan dengan nim " + nim + " ("
											+ Common.numberFormat.get().format((i * 100.0 / ids.size())) + "%)");
									try {
										Common.checkApakahMahasiswaOtomatisMenjadiAnggotaPerpustakaan(nim);
		...
		Common.appendKeToolbar(singkron, add, comp);
		singkron.setVisible(Common.bolehKonfigurasi("singkronkan_dosen_dengan_anggota_perpustakaan"));
		singkron.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				save.addEventListener("onClick", new EventListener() {
					...
					public void onEvent(Event event) throws Exception {
						...
						final ais.common.LaporanUpload laporan = new ais.common.LaporanUpload("Sinkronisasi Anggota Perpustakaan Dosen");
		...
		Common.appendKeToolbar(singkron, add, comp);
		singkron.setVisible(Common.bolehKonfigurasi("singkronkan_pegawai_dengan_anggota_perpustakaan"));
		singkron.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				save.addEventListener("onClick", new EventListener() {
					...
					public void onEvent(Event event) throws Exception {
						...
						final ais.common.LaporanUpload laporan = new ais.common.LaporanUpload("Sinkronisasi Anggota Perpustakaan Pegawai");
						...
						new Thread(new Runnable() {
							...
							public void run() {
								...
								int i = 1;
								for (String nidn : ids) {
									label.setValue("Singkronkan anggota perpustakaan dengan NIK " + nidn + " ("
											+ Common.numberFormat.get().format((i * 100.0 / ids.size())) + "%)");
									try {
										Common.checkApakahPegawaiOtomatisMenjadiAnggotaPerpustakaan(nidn);
										laporan.catatBerhasil(i - 1, nidn, "Sinkronisasi berhasil");
									} catch (Exception ePerItem) {
										Common.tampilErrorJikaAdmin(ePerItem);
										laporan.catatGagalDetail(i - 1, nidn, ePerItem);
		...
		Common.appendKeToolbar(singkron, add, comp);
		singkron.setVisible(Common.bolehKonfigurasi("singkronkan_siswa_dengan_anggota_perpustakaan"));
		singkron.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				save.addEventListener("onClick", new EventListener() {
					...
					public void onEvent(Event event) throws Exception {
						...
						final ais.common.LaporanUpload laporan = new ais.common.LaporanUpload("Sinkronisasi Anggota Perpustakaan Siswa");
						...
						new Thread(new Runnable() {
							...
							public void run() {
								...
								List<Siswa> ids = ConstantValues.simpleList(session.createCriteria(Siswa.class)
										...
										.add(Restrictions.between("tahunMasuk", tahunAngkatanMulai.getValue(),
												tahunAngkatanSampai.getValue()))
										.add(f == null ? Restrictions.sqlRestriction("true")
								...
								int i = 1;
								for (Siswa siswa : ids) {
									label.setValue("Singkronkan anggota perpustakaan dengan " + siswa + " ("
											+ Common.numberFormat.get().format((i * 100.0 / ids.size())) + "%)");
									String kunciSiswa = String.valueOf(siswa);
									try {
										Common.checkApakahSiswaOtomatisMenjadiAnggotaPerpustakaan(siswa);
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public static Map siapkanParemeter(Anggota anggota) throws Exception {

		final File myfilebarcode = new File(Common.ambilREAL_PATH_REPORT() + "/barcode_" + anggota.getKode() + ".png");
		Barcode mybarcode = BarcodeFactory.createCode128B(anggota.getKode());
		BarcodeImageHandler.savePNG(mybarcode, myfilebarcode);
		String kodeBarcode = myfilebarcode.getAbsolutePath();

		final Map parameters = ais.common.HashMapGenerator.getRand();
		parameters.put("id", anggota.getId());
		parameters.put("kode_barcode", kodeBarcode);

		parameters.put("kode", anggota.getKode());
		parameters.put("nama", anggota.getNama());
		parameters.put("alamat", anggota.getAlamat());
		parameters.put("telp", anggota.getTelp());
		Calendar calendar = ais.ui.util.WaktuUtil.getCalendar();
		if (anggota.getMahasiswa() != null) {
			calendar.setTime(anggota.getMahasiswa().getTanggalMasuk());
		}
		...
		if (anggota.getMahasiswa() != null) {
			anggota.getMahasiswa().putPhoto(parameters);

			parameters.put("nama_fakultas", anggota.getMahasiswa().getJurusan().getFakultas().getNama());
			parameters.put("nama_perguruan_tinggi",
					anggota.getMahasiswa().getJurusan().getFakultas().getPerguruanTinggi() == null ? ""
							: anggota.getMahasiswa().getJurusan().getFakultas().getPerguruanTinggi().getNama());
			parameters.put("ttl",
					anggota.getMahasiswa().getTempatlahir().toUpperCase() + " / "
							+ (anggota.getMahasiswa().getTanggallahir() == null ? ""
									: Common.dateFormat2.get().format(anggota.getMahasiswa().getTanggallahir())));

			parameters.put("email", anggota.getMahasiswa().getEmail());
			parameters.put("nama", anggota.getMahasiswa().getNama());
			parameters.put("angkatan", anggota.getMahasiswa().getTahunangkatan());
			parameters.put("alamat_data", anggota.getMahasiswa().getAlamat());
			parameters.put("tempatlahir", anggota.getMahasiswa().getTempatlahir());
			parameters.put("tanggallahir", anggota.getMahasiswa().getTanggallahir());
			parameters.put("jurusan", anggota.getMahasiswa().getJurusan().getNama());
			parameters.put("jenjang", anggota.getMahasiswa().getJurusan().getJenjang().getNama());

		} else if (anggota.getDosen() != null) {
			anggota.getDosen().putPhoto(parameters);

			parameters.put("nama_fakultas", anggota.getDosen().getJurusan() == null ? ""
					: anggota.getDosen().getJurusan().getFakultas().getNama());
			parameters.put("nama_perguruan_tinggi",
					anggota.getDosen().getJurusan() == null ? ""
							: anggota.getDosen().getJurusan().getFakultas().getPerguruanTinggi() == null ? ""
									: anggota.getDosen().getJurusan().getFakultas().getPerguruanTinggi().getNama());
			parameters.put("ttl",
					anggota.getDosen().getTempatlahir().toUpperCase() + " / "
							+ (anggota.getDosen().getTanggallahir() == null ? ""
									: Common.dateFormat2.get().format(anggota.getDosen().getTanggallahir())));

			parameters.put("email", anggota.getDosen().getEmail());
			parameters.put("nama", anggota.getDosen().getNama());
			parameters.put("angkatan", 0);
			parameters.put("alamat_data", anggota.getDosen().getAlamat());
			parameters.put("tempatlahir", anggota.getDosen().getTempatlahir());
			parameters.put("tanggallahir", anggota.getDosen().getTanggallahir());
			parameters.put("jurusan",
					anggota.getDosen().getJurusan() == null ? "" : anggota.getDosen().getJurusan().getNama());
			parameters.put("jenjang", anggota.getDosen().getJurusan() == null ? ""
					: anggota.getDosen().getJurusan().getJenjang().getNama());

		} else if (anggota.getGuru() != null) {
			anggota.getGuru().putPhoto(parameters);

			parameters.put("nama_fakultas",
					anggota.getGuru().getSekolah() == null ? "" : anggota.getGuru().getSekolah().getNama());
			parameters.put("nama_perguruan_tinggi",
					anggota.getGuru().getSekolah() == null ? "" : anggota.getGuru().getSekolah().getNama());
			parameters.put("ttl",
					anggota.getGuru().getTempatLahir().toUpperCase() + " / "
							+ (anggota.getGuru().getTanggalLahir() == null ? ""
									: Common.dateFormat2.get().format(anggota.getGuru().getTanggalLahir())));

			parameters.put("email", anggota.getGuru().getAlamatEmail());
			parameters.put("nama", anggota.getGuru().getNama());
			parameters.put("angkatan", 0);
			parameters.put("alamat_data", anggota.getGuru().getAlamatGuru());
			parameters.put("tempatlahir", anggota.getGuru().getTempatLahir());
			parameters.put("tanggallahir", anggota.getGuru().getTanggalLahir());
			parameters.put("jurusan",
					anggota.getGuru().getSekolah() == null ? "" : anggota.getGuru().getSekolah().getNama());
			parameters.put("jenjang",
					anggota.getGuru().getSekolah() == null || anggota.getGuru().getSekolah().getJenisSekolah() == null
							? ""
							: anggota.getGuru().getSekolah().getJenisSekolah().getNama());

		} else if (anggota.getPegawai() != null) {
			anggota.getPegawai().putPhoto(parameters);

			parameters.put("ttl",
					anggota.getPegawai().getTempatlahir().toUpperCase() + " / "
							+ (anggota.getPegawai().getTanggallahir() == null ? ""
									: Common.dateFormat2.get().format(anggota.getPegawai().getTanggallahir())));

			parameters.put("email", anggota.getPegawai().getEmail());
			parameters.put("nama", anggota.getPegawai().getNama());
			parameters.put("angkatan", 0);
			parameters.put("alamat_data", anggota.getPegawai().getAlamat());
			parameters.put("tempatlahir", anggota.getPegawai().getTempatlahir());
			parameters.put("tanggallahir", anggota.getPegawai().getTanggallahir());
			parameters.put("jurusan",
					anggota.getPegawai().getJurusan() == null ? "" : anggota.getPegawai().getJurusan().getNama());
			parameters.put("jenjang", anggota.getPegawai().getJurusan() == null ? ""
					: anggota.getPegawai().getJurusan().getJenjang().getNama());
		}
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link AnggotaAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link AnggotaAction} dan dapat mengakses state kelas
	...
	class AnggotaRenderer extends ais.ui.util.MyRowRenderer {
		...
		public void render(final Row arg0, Object arg1) throws Exception {
			...
			if (anggota.getMahasiswa() != null) {
				...
				CommonMedia.tampilkanGambarKecil(anggota.getSiswa()).setParent(arg0);
			} else if (anggota.getGuru() != null) {
				CommonMedia.tampilkanGambarKecil(anggota.getGuru()).setParent(arg0);
			} else if (anggota.getDosen() != null) {
			...
			Vbox a;
			(a = RevisiHelper.createNewRevisi(Anggota.class, anggota, anggota.getNama())).setParent(arg0);

			if (anggota.getMahasiswa() != null && anggota.getMahasiswa().getJurusan() != null) {
				new Label(anggota.getMahasiswa().getJurusan().getNama()).setParent(a);
			}
			if (anggota.getMahasiswa() != null && anggota.getMahasiswa().getJurusan() != null
					&& anggota.getMahasiswa().getJurusan().getFakultas() != null) {
				new Label(anggota.getMahasiswa().getJurusan().getFakultas().getNama()).setParent(a);
			}

			if (anggota.getDosen() != null && anggota.getDosen().getJurusan() != null) {
				new Label(anggota.getDosen().getJurusan().getNama()).setParent(a);
			}
			if (anggota.getDosen() != null && anggota.getDosen().getFakultas() != null) {
				new Label(anggota.getDosen().getFakultas().getNama()).setParent(a);
			}

			new Label(anggota.getJenisIdentitas()).setParent(arg0);
			new Label(anggota.getKodeIdentitas()).setParent(arg0);

			new Label(anggota.getJenisAnggota() == null ? "" : anggota.getJenisAnggota().getNama()).setParent(arg0);
			new Label(anggota.getTipe()).setParent(arg0);
			new Label(anggota.getTelp()).setParent(arg0);
			new Label(anggota.getHp()).setParent(arg0);
			new Label(anggota.getEmail()).setParent(arg0);
			new Label(anggota.getAlamat()).setParent(arg0);
			new Label(anggota.getAktif() ? "Aktif" : "Tidak Aktif").setParent(arg0);
			new Label(Common.dateFormat1.get().format(anggota.getTanggal())).setParent(arg0);

			Hbox toolbar = new Hbox();

			MyToolbarbuttonConfig button = new MyToolbarbuttonConfig("", "/img/print.png");
			button.setTooltiptext("Cetak Kartu Anggota");
			button.addEventListener("onClick", new EventListener() {
				...
				public void onEvent(Event event) throws Exception {
					...
					if (!anggota.getAktif()) {
						MyMessageboxConfig.show("Anggota ini tidak aktif", "Peringatan", MyMessageboxConfig.OK,
								MyMessageboxConfig.INFORMATION);
						return;
					}
					List list = new ArrayList();
					list.add(siapkanParemeter(anggota));
					Map parameters = ais.common.HashMapGenerator.getRand();
					parameters = AnggotaAction.siapkanParemeterGambar(parameters);
					parameters.put("maps", list);

					Report.generatePDFReport(Report.PDF, parameters, "library/kartu_anggota",
							anggota.getTanggal_dirubah());
				}
	...
	private void init(Anggota anggota) throws Exception {
		this.anggota = anggota;
		addWindow.setTitle(anggota.getId() == null ? "Tambah Anggota" : "Ubah Anggota");
		...
		myPerpustakaan = Common.getCurrentPerpustakaan();
		Common.insertCombo(perpustakaan, "nama", Perpustakaan.class);

		if (myPerpustakaan != null) {
			perpustakaan.setDisabled(true);
			Common.selectComboItem(perpustakaan, myPerpustakaan);
		}

		if (anggota.getPerpustakaan() != null) {
			Common.selectComboItem(perpustakaan, anggota.getPerpustakaan());
		}
		...
		String mykode = anggota.getKode();
		row.appendChild(kode = new Textbox(anggota.getKode() == null ? mykode : anggota.getKode()));
		kode.setWidth("90%");
		...
		// if (myPerpustakaan != null) {
		// String mykode = LibraryUtil.generateCode(Anggota.class, 8,
		// "AGT", myPerpustakaan);
		...
		Common.insertCombo(tipe, "nama", TipeAnggota.class);
		Common.selectComboItem(tipe, anggota.getTipeAnggota());
		tipe.setWidth("90%");
		tipe.setReadonly(true);
		// List<TipeAnggota> tipeAnggotas = HibernateUtil.currentSession()
		// .createCriteria(TipeAnggota.class).list();
		// for (TipeAnggota tipeAnggota : tipeAnggotas) {
		// MyRadioConfig radio = new MyRadioConfig(tipeAnggota.getNama());
		// radio.setChecked(anggota.getTipeAnggota() != null
		// && anggota.getTipeAnggota().getId() != null
		// && anggota.getTipeAnggota().getId()
		// .equals(tipeAnggota.getId()));
		// radio.setAttribute("tipeAnggota", tipeAnggota);
		// radio.addEventListener("onCheck", jenisEventListener);
		// tipe.appendChild(radio);
		// }

		// tipe.setDisabled(anggota.getId() != null);

		Tbmuser tbmuser = anggota.getTbmuser();

		rowUsername.setStyle("border:0px;background: transparent;");
		rowUsername.setParent(rows);
		rowUsername.appendChild(new Label(ais.common.Common.getBahasaConfig("Username Anggota")));
		...
		if (anggota.getId() != null && tbmuser != null) {
		...
		mahasiswa.setAttribute("mahasiswa", anggota.getMahasiswa());
		mahasiswa.setValue(anggota.getMahasiswa() == null ? ""
				: anggota.getMahasiswa().getNim() + " - " + anggota.getMahasiswa().getNama());
		...
		dosen.setAttribute("dosen", anggota.getDosen());
		dosen.setValue(
				anggota.getDosen() == null ? "" : anggota.getDosen().getCode() + " - " + anggota.getDosen().getNama());
		dosen.setWidth("90%");
		...
		guru.setAttribute("guru", anggota.getGuru());
		guru.setValue(
				anggota.getGuru() == null ? "" : anggota.getGuru().getKode() + " - " + anggota.getGuru().getNama());
		guru.setWidth("90%");
		...
		pegawai.setAttribute("pegawai", anggota.getPegawai());
		pegawai.setValue(anggota.getPegawai() == null ? ""
				: anggota.getPegawai().getCode() + " - " + anggota.getPegawai().getNama());
		pegawai.setWidth("90%");
		...
		siswa.setAttribute("siswa", anggota.getSiswa());
		siswa.setValue(
				anggota.getSiswa() == null ? "" : anggota.getSiswa().getNim() + " - " + anggota.getSiswa().getNama());
		siswa.setWidth("90%");
		...
		Common.insertCombo(jenisAnggota, "nama", JenisAnggota.class);
		Common.selectComboItem(jenisAnggota, anggota.getJenisAnggota());
		jenisAnggota.setWidth("90%");
		jenisAnggota.setReadonly(true);

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Jenis Identitas Anggota"));
		row.appendChild(jenisIdentitas = new Combobox());
		Common.insertCombo(jenisIdentitas, "nama", JenisIdentitasAnggota.class,
				Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)));
		Common.selectComboItem(jenisIdentitas, anggota.getJenisIdentitasAnggota());
		jenisIdentitas.setWidth("90%");
		jenisIdentitas.setReadonly(true);

		// List<JenisIdentitasAnggota> jenisIdentitasAnggotas = HibernateUtil
		// .currentSession().createCriteria(JenisIdentitasAnggota.class)
		// .list();
		// for (JenisIdentitasAnggota jenisIdentitasAnggota :
		// jenisIdentitasAnggotas) {
		// MyRadioConfig radio = new
		// MyRadioConfig(jenisIdentitasAnggota.getNama());
		// radio.setChecked(anggota.getJenisIdentitasAnggota() != null
		// && anggota.getJenisIdentitasAnggota().getId() != null
		// && anggota.getJenisIdentitasAnggota().getId()
		// .equals(jenisIdentitasAnggota.getId()));
		// radio.setAttribute("jenisIdentitasAnggota", jenisIdentitasAnggota);
		// jenisIdentitas.appendChild(radio);
		// }

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Kode Identitas Anggota"));
		row.appendChild(kodeIdentitas = new Textbox(anggota.getKodeIdentitas()));
		kodeIdentitas.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Nama Anggota"));
		row.appendChild(nama = new Textbox(anggota.getNama() == null ? "" : anggota.getNama()));
		nama.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Telp"));
		row.appendChild(telp = new Textbox(anggota.getTelp()));
		telp.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("HP"));
		row.appendChild(hp = new Textbox(anggota.getHp()));
		hp.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Email"));
		row.appendChild(email = new Textbox(anggota.getEmail()));
		email.setWidth("90%");
		...
		aktif.setChecked(anggota.getAktif());

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Alamat"));
		row.appendChild(alamat = new Textbox(anggota.getAlamat()));
		alamat.setWidth("90%");
		alamat.setRows(3);
		...
		row.appendChild(keterangan = new Textbox(anggota.getKeterangan() == null ? "" : anggota.getKeterangan()));
	...
	public boolean onSave(Event event) throws Exception {
		...
		myPerpustakaan = (Perpustakaan) (perpustakaan.getSelectedItem() == null ? null
				: perpustakaan.getSelectedItem().getValue());
		...
		if (tipe.getSelectedItem() == null) {
			MyMessageboxConfig.show("Tipe Anggota harus diisi", "Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION);
			return false;
		}
		if (kode.getValue().trim().equals("")) {
		...
		boolean i = checkKode();
		if (i) {
			MyMessageboxConfig.show("Kode Anggota sudah ada di database", "Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION);
			return false;
		}

		if (nama.getValue().trim().equals("")) {
			MyMessageboxConfig.show("Nama Anggota harus diisi", "Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION);
			return false;
		}
		// if (email.getValue().trim().equals("")) {
		// MyMessageboxConfig.show("Email Anggota harus diisi", "Peringatan",
		// MyMessageboxConfig.OK, MyMessageboxConfig.INFORMATION);
		// return false;
		// }
		if (jenisAnggota.getSelectedItem() == null) {
			MyMessageboxConfig.show("Jenis Anggota harus diisi", "Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION);
			return false;
		}
		...
		if (kodeIdentitas.getValue().trim().equals("")) {
			MyMessageboxConfig.show("Identitas Anggota harus diisi", "Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION);
			return false;
		}
		...
		if (rowUsername.isVisible()) {
			...
			if (userid.getValue().trim().isEmpty()) {
				MyMessageboxConfig.show("Username Anggota harus diisi", "Peringatan", MyMessageboxConfig.OK,
						MyMessageboxConfig.INFORMATION);
				return false;
			}
			...
			if (tbmuser != null && tbmuser.ambilDosen() != null
				...
				MyMessageboxConfig.show("Username ini merupakan pengguna dosen, Anda harus memilih tipe anggota Dosen",
						"Peringatan", MyMessageboxConfig.OK, MyMessageboxConfig.INFORMATION);
				return false;
			}
			...
			if (tbmuser != null && tbmuser.getMahasiswa() != null) {
				MyMessageboxConfig.show(
						"Username ini merupakan pengguna mahasiswa, Anda harus memilih tipe anggota Mahasiswa",
						"Peringatan", MyMessageboxConfig.OK, MyMessageboxConfig.INFORMATION);
				return false;
			}
			...
			if (tbmuser != null && tbmuser.ambilPegawai() != null) {
				MyMessageboxConfig.show(
						"Username ini merupakan pengguna pegawai, Anda harus memilih tipe anggota Pegawai",
						"Peringatan", MyMessageboxConfig.OK, MyMessageboxConfig.INFORMATION);
				return false;
			}
			...
			if (tbmuser == null || tbmuser.getUserId() == null) {
				...
				tbmuser.setUserRole(ConstantValues.roleAnggotaPerpustakaan);
		...
		if (anggota.getId() != null) {
			anggota = (Anggota) session.load(Anggota.class, anggota.getId());
		}

		anggota.setSiswa((Siswa) siswa.getAttribute("siswa"));
		anggota.setGuru((Guru) guru.getAttribute("guru"));
		anggota.setAktif(aktif.isChecked());
		anggota.setTipeAnggota((TipeAnggota) tipe.getSelectedItem().getValue());
		anggota.setJenisIdentitasAnggota((JenisIdentitasAnggota) jenisIdentitas.getSelectedItem().getValue());
		anggota.setHp(hp.getValue());
		anggota.setTelp(telp.getValue());
		anggota.setEmail(email.getValue());
		anggota.setJenisIdentitas(jenisIdentitas.getSelectedItem().getLabel());
		anggota.setKodeIdentitas(kodeIdentitas.getValue().trim());
		anggota.setAlamat(alamat.getValue());
		anggota.setTipe(tipe.getSelectedItem().getLabel());
		anggota.setMahasiswa(null);
		anggota.setDosen(null);
		anggota.setGuru(null);
		anggota.setPegawai(null);
		anggota.setKode(kode.getValue().trim());
		anggota.setNama(nama.getValue());
		anggota.setKeterangan(keterangan.getValue());
		anggota.setJenisAnggota((JenisAnggota) (jenisAnggota.getSelectedItem() == null ? null
				: jenisAnggota.getSelectedItem().getValue()));
		anggota.setDibuatOleh(Common.getCurrentUser());
		if (((Comboitem) tipe.getSelectedItem()).getLabel().equals("Mahasiswa")) {
			anggota.setMahasiswa((Mahasiswa) mahasiswa.getAttribute("mahasiswa"));
		}
		if (((Comboitem) tipe.getSelectedItem()).getLabel().equals("Dosen")) {
			anggota.setDosen((Dosen) dosen.getAttribute("dosen"));
		}
		if (((Comboitem) tipe.getSelectedItem()).getLabel().equals("Guru")) {
			anggota.setGuru((Guru) guru.getAttribute("guru"));
		}
		if (((Comboitem) tipe.getSelectedItem()).getLabel().equals("Pegawai")) {
			anggota.setPegawai((Pegawai) pegawai.getAttribute("pegawai"));
		}

		anggota.setTbmuser(tbmuser);
		anggota.setPerpustakaan(myPerpustakaan);
	...
	public Criteria initCriteria(boolean order) {
		...
		criteria
				...
				.add(searchTipeAnggota.getSelectedItem() == null
						|| searchTipeAnggota.getSelectedItem().getValue() == null ? Restrictions.sqlRestriction("1=1")
								: Restrictions.eq("tipeAnggota", searchTipeAnggota.getSelectedItem().getValue()))

				.add(searchJenisAnggota.getSelectedItem() == null
						|| searchJenisAnggota.getSelectedItem().getValue() == null ? Restrictions.sqlRestriction("1=1")
								: Restrictions.eq("jenisAnggota", searchJenisAnggota.getSelectedItem().getValue()))

				.add(searchProdi.getSelectedItem() == null || searchProdi.getSelectedItem().getValue() == null
						? Restrictions.sqlRestriction("1=1")
						: Restrictions.or(
								CommonSearchFilterHelper.eqSelectedWithId("mahasiswa.jurusan", searchProdi, false),
								CommonSearchFilterHelper.eqSelectedWithId("dosen.jurusan", searchProdi, false)))
	...
	public Boolean checkKode() {
		...
		Session session = HibernateUtil.currentSession();
		kotaCount = ((Number) session.createCriteria(Anggota.class).setProjection(Projections.rowCount())
				.add(Restrictions.eq("kode", kode.getValue().trim()))
				.add(this.anggota.getId() == null ? Restrictions.sqlRestriction("1=1")
						: Restrictions.ne("id", this.anggota.getId()))
