...
/**
 * Controller/action ZK untuk ddc item tree. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code MyWindow addWindow}, {@code Combobox
 * searchversi}, {@code Tree tree}, {@code Textbox kode}, {@code Textbox nama}, {@code Textbox keterangan},
 * {@code Combobox versiDdcItem}, {@code MyCheckboxConfig defaultItem}; inisialisasi/lifecycle ({@code
 ...
 */
public class DdcItemTreeAction extends GenericAutowireComposer {
	...
	private DdcItemTreeModel ddcItemTreeModel;

	private TreeMap<DdcItem, Treecell[]> treecellMap = new TreeMap<DdcItem, Treecell[]>();
	...
	private void initTree() throws Exception {
		...
		Treecol treecol = new Treecol("Ddc");
	...
	public void onAdd(Event event) throws Exception {
		...
		DdcItem myddcItem = new DdcItem();
		myddcItem.setParent(null);

		init(myddcItem, new EventListener() {
	...
	private void init(DdcItem ddcItem, final EventListener eventListener) {
		this.ddcItem = ddcItem;
		addWindow.setTitle(ddcItem.getId() == null ? "Tambah Standar Biaya" : "Ubah Standar Biaya");
		...
		defaultItem.setChecked(ddcItem.getDefaultItem() != null && ddcItem.getDefaultItem());
		...
		save.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				if (onSave(event)) {
					// onReloadTree(null);
					eventListener.onEvent(new Event("", null, DdcItemTreeAction.this.ddcItem));
					addWindow.setVisible(false);
				}
			}
		});
	...
	public void onReloadTree(Event event) throws Exception {
		...
		ddcItemTreeModel = new DdcItemTreeModel(true, (VersiDdcItem) (searchversi.getSelectedItem() == null ? null
				: searchversi.getSelectedItem().getValue()));
				...
				try {
					...
					button.addEventListener("onClick", new EventListener() {
						...
						public void onEvent(Event event) throws Exception {
							...
							DdcItem myddcItem = (DdcItem) ddcItem.clone();
							myddcItem.setParent(ddcItem);
							myddcItem.setId(null);
							init(myddcItem, new EventListener() {

								@Override
								public void onEvent(Event arg0) throws Exception {
									render(treeitem, ddcItem);
										...
										public void onEvent(Event arg0) throws Exception {
											...
											timer.addEventListener("onTimer", new EventListener() {
												...
												public void onEvent(Event arg0) throws Exception {
													...
													try {
														Treeitem myTreeitem = (Treeitem) treecellMap.get(ddcItem)[0]
																.getParent().getParent();

														render(myTreeitem, ddcItem);
					...
					button.addEventListener("onClick", new EventListener() {
						...
						public void onEvent(Event event) throws Exception {
							...
							DdcItem myddcItem = (DdcItem) ddcItem.clone();
							myddcItem.setParent(ddcItem.getParent());
							myddcItem.setId(null);
							init(myddcItem, new EventListener() {
					...
					button.addEventListener("onClick", new EventListener() {
						@Override
						public void onEvent(Event event) throws Exception {
							init(ddcItem, new EventListener() {
										...
										public void onEvent(Event event) throws Exception {
											...
											if (i == MyMessageboxConfig.OK) {
												try {
													...
													ddcItemTreeModel.deleteChilds(ddcItem);
	...
	private void hasSomeChilds(Treerow treerow, final DdcItem ddcItem) {

		Treecell treecell = new Treecell(ddcItem.toString());
		treecell.setTooltiptext(ddcItem.toString());
		treecell.setStyle("font-size:x-small;text-align: left;");
		treecell.setParent(treerow);

		Treecell treecellAktif = new Treecell(
				ddcItem.getDefaultItem() == null || !ddcItem.getDefaultItem() ? "Tidak" : "Aktif");
		...
		treecellMap.put(ddcItem, new Treecell[] { treecellAktif });
