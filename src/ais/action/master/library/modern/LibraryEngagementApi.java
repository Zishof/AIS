...
public final class LibraryEngagementApi {
    ...
    @SuppressWarnings("unchecked") private static JSONObject requests(Tbmuser user)throws Exception{
        Session s=null;try{s=HibernateUtil.openSession();JSONArray data=new JSONArray();for(PermintaanPengadaanItem x:(List<PermintaanPengadaanItem>)s.createCriteria(PermintaanPengadaanItem.class).add(Restrictions.eq("olehId",user.getUserId())).add(Restrictions.like("keterangan","[USULAN ANGGOTA]%")).addOrder(Order.desc("id")).setMaxResults(30).list()){String detail=safe(x.getKeterangan());String status=x.getTanggalPersetujuan()==null?"Sedang ditinjau":detail.contains("[REJECTED ")?"Ditolak":"Disetujui";data.put(new JSONObject().put("type","Usulan koleksi").put("code",safe(x.getKode())).put("description",detail.replaceFirst("^\\[USULAN ANGGOTA\\]\\s*","")).put("status",status));}for(Pesan x:(List<Pesan>)s.createCriteria(Pesan.class).add(Restrictions.eq("olehId",user.getUserId())).add(Restrictions.or(Restrictions.like("isi","[ASK_LIBRARIAN]%"),Restrictions.like("isi","[INTERLIBRARY_LOAN]%"))).addOrder(Order.desc("id")).setMaxResults(30).list()){String detail=safe(x.getIsi());String status=Boolean.FALSE.equals(x.getAktif())?(detail.contains("[REJECTED ")?"Ditolak":"Selesai"):"Diajukan";data.put(new JSONObject().put("type",detail.startsWith("[ASK_LIBRARIAN]")?"Tanya pustakawan":"Pinjam antarperpustakaan").put("code","MSG-"+x.getId()).put("description",detail.replaceFirst("^(\\[[^]]+\\])+\\s*","")).put("status",status));}return ok().put("data",data);}finally{HibernateUtil.closeSessionQuietly(s);}}

    private static JSONObject message(Tbmuser user,String tag,HttpServletRequest r)throws Exception{String subject=text(r.getParameter("subject"),120),body=text(r.getParameter("message"),1200);Perpustakaan library=Common.getCurrentPerpustakaan();if(subject==null||body==null)return error("Subjek dan uraian wajib diisi.");if(library==null||library.getId()==null)return error("Perpustakaan aktif belum dipilih.");Session s=null;Transaction tx=null;try{s=HibernateUtil.openSession();Pesan x=new Pesan();x.setTbmuser((Tbmuser)s.get(Tbmuser.class,user.getUserId()));x.setIsi("["+tag+"][LIBRARY_ID="+library.getId()+"] "+subject+" — "+body);x.setWaktu(new Date());x.setAktif(true);x.setSemua(false);x.setOlehId(user.getUserId());x.setOleh(user.getUserNama());tx=s.beginTransaction();s.save(x);tx.commit();return ok().put("message",tag.equals("ASK_LIBRARIAN")?"Pertanyaan dikirim ke antrean pustakawan.":"Permintaan pinjam antarperpustakaan diajukan.").put("id",x.getId());}catch(Exception e){rollback(tx);throw e;}finally{HibernateUtil.closeSessionQuietly(s);}}
    ...
    private static JSONObject booking(Tbmuser user,HttpServletRequest r)throws Exception{
        Long roomId=positiveLong(r.getParameter("roomId"));Date start=date(r.getParameter("start")),end=date(r.getParameter("end"));String purpose=text(r.getParameter("purpose"),240);
        if(roomId==null||start==null||end==null||!end.after(start)||purpose==null)return error("Ruang, waktu mulai/selesai, dan tujuan wajib diisi dengan benar.");
        if(start.before(new Date()))return error("Waktu mulai tidak boleh berada di masa lalu.");
        Session s=null;Transaction tx=null;
        try{
            s=HibernateUtil.openSession();Ruang room=(Ruang)s.get(Ruang.class,roomId);
            if(room==null||Boolean.FALSE.equals(room.getAktif())||!roomBelongsToCurrentLibrary(room))return error("Ruang tidak tersedia.");
            tx=s.beginTransaction();s.lock(room,LockMode.UPGRADE);
            long conflict=((Number)s.createQuery("select count(id) from PesanRuangan where ruang.id=:room and mulai<:end and sampai>:start").setLong("room",roomId).setTimestamp("start",start).setTimestamp("end",end).uniqueResult()).longValue();
            if(conflict>0){rollback(tx);return error("Ruang sudah dipesan pada rentang waktu tersebut.");}
            ...
            return ok().put("message","Fasilitas berhasil dipesan.").put("id",x.getId());
        }catch(Exception e){rollback(tx);throw e;}finally{HibernateUtil.closeSessionQuietly(s);}
    }

    private static boolean roomBelongsToCurrentLibrary(Ruang room){Perpustakaan p=Common.getCurrentPerpustakaan();if(p==null)return false;if(p.getYayasan()!=null&&(room.getYayasan()==null||!p.getYayasan().getId().equals(room.getYayasan().getId())))return false;if(p.getSekolah()!=null&&(room.getSekolah()==null||!p.getSekolah().getId().equals(room.getSekolah().getId())))return false;if(p.getFakultas()!=null&&(room.getFakultas()==null||!p.getFakultas().getId().equals(room.getFakultas().getId())))return false;if(p.getJurusan()!=null&&(room.getJurusan()==null||!p.getJurusan().getId().equals(room.getJurusan().getId())))return false;return true;}
