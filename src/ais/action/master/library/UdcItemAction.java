...
/**
 * Controller/action ZK untuk udc item. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * paging}, {@code MyGrid grid}, {@code Textbox searchkode}, {@code Textbox searchnama}, {@code MyCheckboxConfig
 * aktif}, {@code Textbox kode}, {@code Textbox nama}; inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code
 * doAfterCompose()}, {@code init()}, {@code initCriteria()}); pembacaan/pencarian ({@code onSearchDefault()});
 * validasi/perhitungan ({@code checkNamaUdcItem()}); mutasi data ({@code onSave()}); operasi domain lain ({@code
...
public class UdcItemAction extends GenericAutowireComposer {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link UdcItemAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link UdcItemAction} dan dapat mengakses state kelas
