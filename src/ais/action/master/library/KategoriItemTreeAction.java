...
/**
 * Controller/action ZK untuk kategori item tree. Tipe ini merupakan titik masuk UI yang
 ...
 * {@code Textbox nama}, {@code Textbox keterangan}, {@code MyCheckboxConfig defaultItem}, {@code boolean edit},
 * {@code boolean delete}, {@code KategoriItem kategoriItem}; inisialisasi/lifecycle ({@code doBeforeCompose()},
 * {@code doAfterCompose()}, {@code initTree()}, {@code init()}); pembacaan/pencarian ({@code onReloadTree()},
 * {@code reloadTreeitem()}, {@code reloadTreeitem()}, {@code reloadTotal()}); mutasi data ({@code onSave()});
 * operasi domain lain ({@code onAdd()}, {@code openChilds()}, {@code closeChilds()}, {@code hasSomeChilds()}).
 ...
 */
public class KategoriItemTreeAction extends GenericAutowireComposer {
	...
	private KategoriItemTreeModel kategoriItemTreeModel;

	private TreeMap<KategoriItem, Treecell[]> treecellMap = new TreeMap<KategoriItem, Treecell[]>();
	...
	public void onAdd(Event event) throws Exception {
		...
		KategoriItem mykategoriItem = new KategoriItem();
	...
	private void init(KategoriItem kategoriItem,
		...
		defaultItem.setChecked(kategoriItem.getDefaultItem() != null
			...
			public void onEvent(Event event) throws Exception {
				if (onSave(event)) {
					...
					eventListener.onEvent(new Event("", null,
							KategoriItemTreeAction.this.kategoriItem));
					addWindow.setVisible(false);
				}
			}
		});
	...
	public void onReloadTree(Event event) throws Exception {
		...
		kategoriItemTreeModel = new KategoriItemTreeModel(true);
				...
				try {
					...
					button.addEventListener("onClick", new EventListener() {
						...
						public void onEvent(Event event) throws Exception {
							...
							KategoriItem mykategoriItem = (KategoriItem) kategoriItem
									.clone();
	...
	private void hasSomeChilds(Treerow treerow, final KategoriItem kategoriItem) {
		...
		Treecell treecellAktif = new Treecell(
				kategoriItem.getDefaultItem() == null
						|| !kategoriItem.getDefaultItem() ? "Tidak" : "Aktif");
