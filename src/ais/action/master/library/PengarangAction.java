...
/**
 * Controller/action ZK untuk pengarang. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * AmbilDataTbmuserBanbox tbmuser}, {@code MyCheckboxConfig aktif}; inisialisasi/lifecycle ({@code
 * doBeforeCompose()}, {@code doAfterCompose()}, {@code init()}, {@code initCriteria()}); pembacaan/pencarian
 * ({@code onSearchDefault()}); validasi/perhitungan ({@code checkNamaPengarang()}); mutasi data ({@code
 * onSave()}); operasi domain lain ({@code onAddExternal()}, {@code onAdd()}). Bagian lain dari kontrak tetap
...
public class PengarangAction extends GenericAutowireComposer implements DataCriteria, DataSearchDefault {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link PengarangAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link PengarangAction} dan dapat mengakses state
