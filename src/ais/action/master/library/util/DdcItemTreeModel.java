...
/**
 * Tipe khusus untuk ddc item tree model. Kelas ini memberi nama dan batas tanggung jawab yang
 ...
 * VersiDdcItem versiDdcItem}; pembacaan/pencarian ({@code getChildren()}, {@code getChildren()}, {@code
 * getChild()}, {@code getChildCount()}, {@code getParentCount()}, {@code getParentSet()});
 * penghapusan/pembatalan ({@code deleteChilds()}); operasi domain lain ({@code generateAllChildren()}, {@code
 * isLeaf()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di atas.</p>
