...
public class InformasiPerpustakaanPunyaKomentarHelper {
	...
	private void loadDataDetail(
		...
		List<InformasiPerpustakaanKomentar> informasiPerpustakaanKomentars = informasiPerpustakaan == null
				|| informasiPerpustakaan.getId() == null ? new ArrayList<InformasiPerpustakaanKomentar>()
				: HibernateUtil
						.currentSession()
						.createCriteria(InformasiPerpustakaanKomentar.class)
						.add(Restrictions.eq("informasiPerpustakaan",
								informasiPerpustakaan)).list();

		Rows rows = gridKomentar.getRows() == null ? new Rows() : gridKomentar
				.getRows();
		rows.setParent(gridKomentar);

		for (InformasiPerpustakaanKomentar informasiPerpustakaanKomentar : informasiPerpustakaanKomentars) {
			Row row = new Row();row.setValign("top");
			row.setParent(rows);
			initRow(row, informasiPerpustakaanKomentar);
		}
	}
	...
	public void initRow(final Row row,
		...
		row.setValign("top");row.setAttribute("informasiPerpustakaanKomentar",
				informasiPerpustakaanKomentar);

		new Label(informasiPerpustakaanKomentar.getNama()).setParent(row);
		new Label(informasiPerpustakaanKomentar.getKontak()).setParent(row);
		new Label(informasiPerpustakaanKomentar.getEmail()).setParent(row);
						...
						MyMessageboxConfig.QUESTION, new EventListener() {
							...
							public void onEvent(Event event) throws Exception {
								...
								if (i == MyMessageboxConfig.OK) {
									if (informasiPerpustakaanKomentar.getId() != null) {
										Session session = HibernateUtil
												.currentSession();
										session.delete(informasiPerpustakaanKomentar);
									}
