...
/**
 * Tipe khusus untuk ambil data permintaan pengadaan per penyedia banbox. Kelas ini memberi nama
 ...
 * kodePermintaanPengadaanIteman}, {@code MyTextbox nama}, {@code AmbilDataPerpustakaanBanbox
 * searchperpustakaan}; pembacaan/pencarian ({@code onSearchDefault()}, {@code setEventListener()}, {@code
 ...
 * @see Bandbox
 */
public class AmbilDataPermintaanPengadaanPerPenyediaBanbox extends Bandbox {
	...
	public AmbilDataPermintaanPengadaanPerPenyediaBanbox() throws Exception {
		super();
		display();
	...
	private MyTextbox kodePermintaanPengadaanIteman;
	private MyTextbox nama;
	private AmbilDataPerpustakaanBanbox searchperpustakaan;

	/**
	 * Renderer lokal untuk layar/komponen {@link AmbilDataPermintaanPengadaanPerPenyediaBanbox}. Kelas ini
	 * menerjemahkan satu item data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik
	 * kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link AmbilDataPermintaanPengadaanPerPenyediaBanbox}
	 ...
	 *
	 * @see AmbilDataPermintaanPengadaanPerPenyediaBanbox
	 */
	class PermintaanPengadaanItemRenderer extends ais.ui.util.MyRowRenderer {
		...
		public void render(Row arg0, Object arg1) throws Exception {arg0.setValign("top");
			...
			final PermintaanPengadaanItem permintaanPengadaanItem = (PermintaanPengadaanItem) objects[0];
			final Penyedia penyedia = (Penyedia) objects[1];
			arg0.addEventListener("onClick", new EventListener() {
				@Override
				public void onEvent(Event event) throws Exception {
					if (permintaanPengadaanItem.getDisetujuiOleh() == null) {
						MyMessageboxConfig.show("Permintaan Pengadaan "
								+ permintaanPengadaanItem.getKode()
								+ " belum disetujui", "Peringatan",
								MyMessageboxConfig.OK, MyMessageboxConfig.EXCLAMATION);
						return;
					}
					AmbilDataPermintaanPengadaanPerPenyediaBanbox.this
							.setOpen(false);
					AmbilDataPermintaanPengadaanPerPenyediaBanbox.this
							.setAttribute("permintaanPengadaanItem",
									permintaanPengadaanItem);
					AmbilDataPermintaanPengadaanPerPenyediaBanbox.this
							.setAttribute("penyedia", penyedia);
					AmbilDataPermintaanPengadaanPerPenyediaBanbox.this
							.setValue(permintaanPengadaanItem.toString());
			...
			new Label(permintaanPengadaanItem.getKode()).setParent(arg0);
			...
			new Label(
					permintaanPengadaanItem.getTanggalPersetujuan() == null ? ""
							: Common.dateFormat.get().format(permintaanPengadaanItem
									.getTanggalPersetujuan())).setParent(arg0);
			new Label(permintaanPengadaanItem.getDisetujuiOleh() == null ? ""
					: permintaanPengadaanItem.getDisetujuiOleh().getUserId())
					.setParent(arg0);
		}
	...
	public void display() throws Exception {
		...
		panel.setWidth("100%");
		panel.setHeight("100%");
		panel.setTitle("Daftar Permintaan Pengadaan Item");
		...
		row.appendChild(kodePermintaanPengadaanIteman = new MyTextbox());
		kodePermintaanPengadaanIteman.setWidth("90%");
		kodePermintaanPengadaanIteman.addEventListener(Events.ON_OK,
				new EventListener() {
					public void onEvent(Event event) throws Exception {
	...
	public void onSearchDefault(Event event) {
		...
		List<PermintaanPengadaanItem> permintaanPengadaanItems = session
				.createCriteria(PermintaanPengadaanItem.class)
				...
				.add(Restrictions.ilike("keterangan", nama.getValue().trim(),
						MatchMode.ANYWHERE))
				.add(kodePermintaanPengadaanIteman.getValue().trim().equals("") ? Restrictions
						.sqlRestriction("1=1") : Restrictions.ilike("kode",
						kodePermintaanPengadaanIteman.getValue().trim(),
						MatchMode.ANYWHERE)).setMaxResults(Common.MAX_RESULT)
				.list();

		List<Object[]> objects = new ArrayList<Object[]>();
		for (PermintaanPengadaanItem item : permintaanPengadaanItems) {
			List<Penyedia> penyedias = session
					.createCriteria(PermintaanPengadaanItemDetail.class)
					.add(Restrictions.eq("permintaanPengadaanItem", item))
