...
public class PenerbitPunyaPemeriksaHelper {
	...
	public Borderlayout initDetail(final Penerbit penerbit) {
		...
		if (penerbit.getId() != null) {
			...
			domainPenelitians = session.createCriteria(DomainPenelitian.class)
					.add(Restrictions.eq("penerbit", penerbit)).list();
		}
		...
		MyToolbarbuttonConfig add = new MyToolbarbuttonConfig("Tambah Pemeriksa",
				"/img/new.gif");
		add.setVisible(PenerbitPunyaPemeriksaHelper.this.add);
		...
		column = new MyColumnConfig("Domain Penelitian");
		column.setParent(columns);
		column.setWidth(penerbit.getId() != null ? "20%" : "0%");
	...
	private void loadDataDetail(final Penerbit penerbit) {
		...
		List<PenerbitPunyaPemeriksa> penerbitPunyaPemeriksas = penerbit == null
				|| penerbit.getId() == null ? new ArrayList<PenerbitPunyaPemeriksa>()
	...
	public void initRow(final Row row,
		...
		final Combobox domain = new Combobox();
		Common.insertComboItems(domain, "nama", domainPenelitians);
		Common.selectComboItem(domain,
				penerbitPunyaPemeriksa.getDomainPenelitian());
		domain.setWidth("90%");
		domain.setParent(row);
		domain.addEventListener("onChange", new EventListener() {

			@Override
			public void onEvent(Event arg0) throws Exception {
				penerbitPunyaPemeriksa
						.setDomainPenelitian((DomainPenelitian) (domain
								.getSelectedItem() == null ? null : domain
