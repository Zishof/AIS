...
/**
 * Tipe khusus untuk ambil data domain penelitian banyak. Kelas ini memberi nama dan batas tanggung
 ...
 * domainPenelitians}, {@code List domainPenelitiansHanyaDitampilkan}, {@code Set ids}, {@code MyTextbox nama};
 ...
 * @see MyWindow
 */
public class AmbilDataDomainPenelitianBanyak extends MyWindow {
	...
	private List<DomainPenelitian> domainPenelitians;
	private List<DomainPenelitian> domainPenelitiansHanyaDitampilkan;

	private Set<Long> ids = new HashSet<Long>();

	public AmbilDataDomainPenelitianBanyak(List<DomainPenelitian> domainPenelitians) {
		super();
		this.domainPenelitians = domainPenelitians;
		display();
		onSearchDefault(null);
	}

	public AmbilDataDomainPenelitianBanyak(List<DomainPenelitian> domainPenelitians,
			List<DomainPenelitian> domainPenelitiansHanyaDitampilkan) {
		super();
		this.domainPenelitians = domainPenelitians;
		this.domainPenelitiansHanyaDitampilkan = domainPenelitiansHanyaDitampilkan;
		display();
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link AmbilDataDomainPenelitianBanyak}. Kelas ini menerjemahkan satu
	 * item data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link AmbilDataDomainPenelitianBanyak} dan dapat
	 ...
	 *
	 * @see AmbilDataDomainPenelitianBanyak
	 */
	class DomainPenelitianRenderer extends ais.ui.util.MyRowRenderer {
		...
		public void render(Row arg0, Object arg1) throws Exception {arg0.setValign("top");
			...
			arg0.setAttribute("domainPenelitian", domainPenelitian);
			final MyCheckboxConfig checkbox = new MyCheckboxConfig();
			checkbox.setParent(arg0);arg0.setAttribute("checkbox", checkbox);
			for (DomainPenelitian myDomainPenelitian : domainPenelitians) {
				if (myDomainPenelitian.getId().equals(domainPenelitian.getId())) {
			...
			checkbox.setChecked(ids.contains(domainPenelitian.getId()));
			...
			checkbox.addEventListener("onCheck", new EventListener() {
				...
				public void onEvent(Event arg0) throws Exception {
					if (checkbox.isChecked()) {
						ids.add(domainPenelitian.getId());
					} else {
						ids.remove(domainPenelitian.getId());
					}
				}
			});
	...
	public void display() {
		...
		panel.setWidth("100%");
		panel.setHeight("100%");
		panel.setTitle("Daftar DomainPenelitian");
		...
		cancel.addEventListener("onClick", new EventListener() {
			@Override
			public void onEvent(Event event) throws Exception {
				AmbilDataDomainPenelitianBanyak.this.detach();
			}
		});
		...
		button.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				if (eventListener != null && grid.getRows() != null
					...
					List<DomainPenelitian> domainPenelitians = new ArrayList<DomainPenelitian>();
					List<Row> rows = grid.getRows().getChildren();
					for (Row row : rows) {
						MyCheckboxConfig checkbox = (MyCheckboxConfig) row.getAttribute("checkbox");
						if (checkbox.isChecked() && !checkbox.isDisabled()) {
							DomainPenelitian myDomainPenelitian = (DomainPenelitian) row
	...
	public void onSearchDefault(Event event) {
		...
		if (domainPenelitiansHanyaDitampilkan != null) {
			for (DomainPenelitian domainPenelitian : domainPenelitiansHanyaDitampilkan) {
				values.add(domainPenelitian.getId());
			}
		}

		List<DomainPenelitian> domainPenelitian = session
				.createCriteria(DomainPenelitian.class)
				.addOrder(Order.asc("nama"))
				.add(ids.size() == 0 ? Restrictions.sqlRestriction("1!=1")
						: Restrictions.in("id", ids)).list();

		List<DomainPenelitian> myDomainPenelitian = session
				.createCriteria(DomainPenelitian.class)
				.add(Restrictions.eq("defaultItem", true))
				.addOrder(Order.asc("nama"))
				.add(ids.size() == 0 ? Restrictions.sqlRestriction("1=1")
						: Restrictions.not(Restrictions.in("id", ids)))
				.add(domainPenelitiansHanyaDitampilkan == null
		...
		domainPenelitian.addAll(myDomainPenelitian);
