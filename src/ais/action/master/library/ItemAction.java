...
/**
 * Controller/action ZK untuk item. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code boolean kolomTeksItemSudahDiperiksa},
 * {@code Textbox searchPublikasiNama}, {@code Textbox searchPublikasiPengarang}, {@code Combobox
 * searchPublikasiStatus}, {@code Combobox searchPublikasiTersedia}, {@code MyGrid gridPublikasi}, {@code Paging
 * pagingPublikasi}, {@code boolean comboStatusPublikasiSiap}; inisialisasi/lifecycle ({@code doBeforeCompose()},
 * {@code doAfterCompose()}, {@code initDetail()}, {@code initAbstrackDanKeyword()}, {@code
 * initAbstrackDanKeywordIndonesia()}, {@code initAbstrackDanKeywordEnglish()}); pembacaan/pencarian ({@code
 * tampilkanKutipan()}, {@code tryDownloadDiagnostic()}, {@code uploadDataItem()}, {@code
 * getDspaceItemTahunPT()}, {@code getDspaceItemTahunJurusan()}, {@code getDspaceItemJurusan()});
 * validasi/perhitungan ({@code checkForTerbit()}, {@code checkNamaItem()}); mutasi data ({@code onSave()},
 * {@code ubahStatusSemuaHasilFilter()}, {@code ubahStatusTerpilih()}); operasi domain lain ({@code
 * buildSenayanProgressHtml()}, {@code generateCSLItemData()}, {@code createDaemonThread()}, {@code
 * processSenayanZip()}, {@code appendLocalSenayanZipButtons()}, {@code showSenayanDiagnosticWindow()}). Bagian
...
//	private boolean padaSaatpendataanItemPerpustakaanTampilkanPilihanFakultasDanProgramStudi = false;
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link ItemAction}. Kelas ini menerjemahkan satu item data menjadi baris
	 * atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link ItemAction} dan dapat mengakses state kelas
