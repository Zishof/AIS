...
/**
 * Controller/action ZK untuk berkas. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * grid}, {@code Tree tree}, {@code Textbox searchnama}, {@code Textbox searchkode}, {@code Textbox nama}, {@code
 * Textbox keterangan}, {@code AmbilDataBerkasBanbox parent}; inisialisasi/lifecycle ({@code doBeforeCompose()},
 * {@code doAfterCompose()}, {@code init()}); pembacaan/pencarian ({@code onReloadTree()}, {@code
 * reloadTreeitem()}, {@code onSearchDefault()}); mutasi data ({@code onSave()}); operasi domain lain ({@code
 * onAddBerkas()}, {@code onAdd()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang
 * disebut di atas.</p>
...
public class BerkasAction extends GenericAutowireComposer {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link BerkasAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link BerkasAction} dan dapat mengakses state kelas
