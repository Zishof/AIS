...
/**
 * Controller/action ZK untuk new krs. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * {@code Integer semesterPendek}, {@code boolean remedial}, {@code Tabpanel krsSp}; inisialisasi/lifecycle
 * ({@code doBeforeCompose()}, {@code doAfterCompose()}); pembacaan/pencarian ({@code
 * onSearchDefaultKeDatabase()}, {@code onSearchDefault()}, {@code load()}); operasi domain lain ({@code
 * onKrsSp()}, {@code onMkRemedial()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang
 * disebut di atas.</p>
