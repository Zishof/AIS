...
/**
 * Controller/action ZK untuk posting biaya payment gateway pembayaran mahasiswa. Tipe ini
 ...
 * {@code Textbox searchnama}, {@code Combobox searchfakultas}, {@code MyCheckboxConfig searchtampil}, {@code
 * MyCheckboxConfig searchtelahtampil}, {@code Combobox jenissemester}, {@code Combobox searchjurusan};
 ...
 * {@code kriteriaPostingStatic()}, {@code batalkanPostingSemua()}, {@code postingSemua()}); operasi domain lain
 * ({@code populateAkun()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di
 * atas.</p>
 ...
 */
public class PostingBiayaPaymentGatewayPembayaranMahasiswaAction extends GenericAutowireComposer {
	...
	public void onBatalkanPostingSemua(Event event) throws Exception {
		...
		MyMessageboxConfig.show("Apakah yakin ingin membatalkan posting semua biaya payment gateway pembayaran ?",
				...
				public void onEvent(Event event) throws Exception {
					...
					if (i == MyMessageboxConfig.OK) {
						List<LogPembayaran> logPembayarans = initCriteria(true)
							.add(Restrictions.isNotNull("postingHistoryPaymentGateway")).list();
						for (LogPembayaran logPembayaran : logPembayarans) {
							logPembayaran.setPostingHistoryPaymentGateway(null);
	...
	public void onPostingSemua(Event event) throws Exception {
		...
		save.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				MyMessageboxConfig.show("Apakah yakin ingin memposting biaya payment gateway pembayaran ?",
						"Pertanyaan", MyMessageboxConfig.OK | MyMessageboxConfig.CANCEL, MyMessageboxConfig.QUESTION,
						new EventListener() {
								...
								if (i == MyMessageboxConfig.OK) {
									...
									final Label label = Common.displayLoadBar(new EventListener() {
										...
										public void onEvent(Event arg0) throws Exception {
											MyMessageboxConfig.show(
													"Posting biaya payment gateway pembayaran mahasiswa berhasil dilakukan",
									...
									new Thread(new Runnable() {
										...
										public void run() {
											...
											PostingHistory postingHistoryPaymentGateway = new PostingHistory(
													PostingHistory.JENIS_MAHASISWA);
											postingHistoryPaymentGateway.setTbmuser(tbmuser);
											postingHistoryPaymentGateway.setTanggal(tgl);
											postingHistoryPaymentGateway.setKeterangan(keterangan.getValue().trim()
													+ " \nTgl:" + Common.dateFormat.get().format(tglMulai.getValue())
													+ " s.d " + Common.dateFormat.get().format(tglSampai.getValue()));
											session.getTransaction().begin();
											session.save(postingHistoryPaymentGateway);
											session.getTransaction().commit();

											List<LogPembayaran> logPembayarans = initCriteria(true)
													.add(Restrictions.isNull("postingHistoryPaymentGateway")).list();
											int rowIndex = 1;
											for (LogPembayaran logPembayaran : logPembayarans) {
												if (logPembayaran != null) {

													Akun[] akuns = populateAkun(session, logPembayaran);
													Akun akunDebet = akuns[1];
													Akun akunKredit = akuns[0];

													if (akunDebet != null && akunKredit != null) {
														Boolean apakahUangMasuk = true;
														String ket = "Biaya payment gateway "
														...
														Double nilai = logPembayaran.getBiayaPaymentGateway();
														try {
															session.getTransaction().begin();
															CommonAkunting
															...
															} catch (Exception ex) { ais.common.ErrorAuditUtil.record(ex, "auto-audit(empty-catch) src/ais/action/master/PostingBiayaPaymentGatewayPembayaranMahasiswaAction.java:383");
															}
															continue;
														}

														session.getTransaction().begin();
														logPembayaran.setPostingHistoryPaymentGateway(
																postingHistoryPaymentGateway);
	...
	static Akun[] populateAkun(Session session, LogPembayaran logPembayaran) {
		...
		if (logPembayaran.getFaspayRequest() != null) {
			...
			String kodeAkunKredit = Common.getKonfigurasi("kode_akun_faspay_biaya_payment_gateway", "").getNilai();
			...
			String kodeAkunKredit = Common.getKonfigurasi("kode_akun_bni_biaya_payment_gateway", "").getNilai();
			JenisPembayaran jenisPembayaran = JenisPembayaran.ambilJenisPembayaranBerdasarkanKodeAkun(session,
					kodeAkun);
			akunDebet = jenisPembayaran.getAkun();
			akunKredit = kodeAkunKredit.trim().isEmpty() ? null
			...
			String kodeAkunKredit = Common.getKonfigurasi("kode_akun_jatelindo_biaya_payment_gateway", "").getNilai();
			JenisPembayaran jenisPembayaran = JenisPembayaran.ambilJenisPembayaranBerdasarkanKodeAkun(session,
					kodeAkun);
			akunDebet = jenisPembayaran.getAkun();
			akunKredit = kodeAkunKredit.trim().isEmpty() ? null
			...
			String kodeAkunKredit = Common.getKonfigurasi("kode_akun_manual_biaya_payment_gateway", "").getNilai();
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link PostingBiayaPaymentGatewayPembayaranMahasiswaAction}. Kelas ini
	 ...
	 * PostingBiayaPaymentGatewayPembayaranMahasiswaAction} dan dapat mengakses state kelas induk. Jangan menyimpan
	 ...
	 *
	 * @see PostingBiayaPaymentGatewayPembayaranMahasiswaAction
	 */
	class LogPembayaranRenderer extends ais.ui.util.MyRowRenderer {
		...
		public void render(final Row arg0, Object arg1) throws Exception {
			...
			if (kegiatan.getMahasiswa() != null) {
				...
				new Label(logPembayaran.getBiayaPaymentGateway() == null ? "0"
						: Common.numberFormat.get().format(logPembayaran.getBiayaPaymentGateway())).setParent(arg0);
			...
			if (akunDebet != null && akunKredit != null) {
				...
				// (populateAkun) — BUKAN pemetaan ItemBiaya. Perhatikan indeks akuns[] DITUKAR
				// dibanding Biaya Administrasi: akunDebet=akuns[1] (kode akun biaya payment
				// gateway), akunKredit=akuns[0] (akun Jenis Pembayaran).
				String kodeParamDebet = logPembayaran.getFaspayRequest() != null
			...
			new Label(logPembayaran.getPostingHistoryPaymentGateway() == null ? Common.getBahasaConfig("Belum diposting")
					: logPembayaran.getPostingHistoryPaymentGateway().toString()).setParent(arg0);
			...
			if (akunDebet != null && akunKredit != null) {
				...
				button.setVisible(edit && adminLain && logPembayaran.getPostingHistoryPaymentGateway() != null);
				...
				button.setVisible(edit && logPembayaran.getPostingHistoryPaymentGateway() == null && tbmuser != null);
				button.addEventListener("onClick", new EventListener() {
					@Override
					public void onEvent(Event event) throws Exception {
						...
						Common.createDefaultTimer(new EventListener() {
							...
							public void onEvent(Event arg0) throws Exception {
								...
								PostingHistory postingHistoryPaymentGateway = new PostingHistory(
										PostingHistory.JENIS_MAHASISWA);
								postingHistoryPaymentGateway.setTbmuser(Common.getCurrentUser());
								postingHistoryPaymentGateway.setTanggal(ais.ui.util.WaktuUtil.getDate());
								postingHistoryPaymentGateway.setKeterangan(
								...
								if (akunDebet != null && akunKredit != null) {
									...
									Double nilai = logPembayaran.getBiayaPaymentGateway();
									CommonAkunting.saveTransaksi(akunDebet, akunKredit, null, null,
											postingHistoryPaymentGateway, apakahUangMasuk, ket,
											logPembayaran.getTanggal(), nilai, 0.0, logPembayaran,
											tbmuser.ambilSatuanKerja(), session);

									logPembayaran.setPostingHistoryPaymentGateway(postingHistoryPaymentGateway);
									session.update(logPembayaran);
								}
	...
	public Criteria initCriteria(boolean order) {
		...
		Criteria criteria = session.createCriteria(LogPembayaran.class).add(ais.action.master.helper.PostingJurnalHelper.restriksiPosting("postingHistoryPaymentGateway", sudahPostingDasbor))

				.add(Restrictions.gt("biayaPaymentGateway", 0.1))
				...
				.add(searchtelahtampil.isChecked() ? Restrictions.isNotNull("postingHistoryPaymentGateway")
						: !searchtampil.isChecked() ? Restrictions.sqlRestriction("1=1")
						: Restrictions.isNull("postingHistoryPaymentGateway"))
	...
	/**
	 * Kriteria log pembayaran ber-biaya payment gateway pembayaran mahasiswa pada rentang tanggal -- sama dengan penghitung
	 * baris "Mahasiswa - Biaya Payment Gateway" pada dasbor
	...
	/**
	 * Batalkan posting SEMUA biaya payment gateway pembayaran mahasiswa dalam rentang.
	 *
		...
		try {
			...
			for (LogPembayaran log : daftar) {
				try {
					...
					// (biaya payment gateway di sini, biaya administrasi di action sebelahnya).
					...
					// ini, membatalkan kaki payment gateway ikut menghapus jurnal kaki
					// administrasi padahal capnya tetap terpasang -- dokumen tampak terposting
					// sementara jurnalnya sudah lenyap.
					if (log.getPostingHistoryPaymentGateway() == null
							|| log.getPostingHistoryPaymentGateway().getId() == null) {
						continue;
					}
					String syarat = "log_pembayaran=" + log.getId() + " and posting_history="
							+ log.getPostingHistoryPaymentGateway().getId() + " and closing is null";
					...
					session.createSQLQuery("delete from akunting.grup_transaksi where " + syarat)
							.executeUpdate();
					log.setPostingHistoryPaymentGateway(null);
					session.update(log);
					session.getTransaction().commit();
					n++;
				} catch (Exception e) {
					...
					ais.common.ErrorAuditUtil.record(e, "PostingBiayaPaymentGatewayPembayaranMahasiswaAction jalur API");
				}
			}
		} finally {
			try {
	...
	/**
	 * Posting SEMUA biaya payment gateway pembayaran mahasiswa yang belum dijurnal dalam rentang.
	 *
	...
	public static int postingSemua(java.util.Date mulai, java.util.Date sampai, Tbmuser oleh,
		...
		try {
			List<Long> ids = kriteriaPostingStatic(session, mulai, sampai)
					.add(Restrictions.isNull("postingHistoryPaymentGateway"))
			...
			postingHistory.setKeterangan("Posting massal biaya payment gateway pembayaran mahasiswa dari dasbor jurnal"
			...
			for (Long id : ids) {
				try {
					...
					Double nilai = log.getBiayaPaymentGateway();
					...
					if (tersimpan) {
						log.setPostingHistoryPaymentGateway(postingHistory);
						session.getTransaction().begin();
						session.update(log);
						session.getTransaction().commit();
						n++;
