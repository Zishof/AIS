...
/**
 * <h3>MonitorKasBesarDashboard — Dashboard Pemantauan Kas Besar &amp; Pembayaran (DPC)</h3>
 *
 * <p><b>Untuk apa dashboard ini.</b> Memantau seluruh pengeluaran kas besar organisasi dalam satu
 ...
 * menu Kas Besar, bersifat hanya-baca untuk analisis.</p>
 *
 ...
 * mudah dirawat.</p>
 */
public class MonitorKasBesarDashboard extends Vbox {

	private static final long serialVersionUID = 8843017762210114559L;
	...
	/**
	 * Tipe implementasi bersarang {@link Baris} milik {@link MonitorKasBesarDashboard}. Kelas ini memberi nama
	 * pada state atau perilaku lokal agar tanggung jawabnya tidak tersebar sebagai blok anonim.
	 *
	 * <p><b>Scope:</b> tipe bersifat {@code static}; instance tidak menangkap object {@link
	 * MonitorKasBesarDashboard}. Dependensi yang diperlukan harus diberikan secara eksplisit agar aman digunakan
	...
	public MonitorKasBesarDashboard() {
		setWidth("100%");
		setHeight("100%");
	...
	private void renderRingkasan(List<Baris> data) {
		...
		Panelchildren isi = DashboardCardHelper.panel(body, "Ringkasan Kas Besar",
				"Kondisi kas besar sekarang dalam angka besar: jumlah dokumen, total nilai, yang sudah "
						+ "dibayar lewat DPC, dan yang sudah dipertanggungjawabkan.");
		String baris = DashboardCardHelper.barisKartu(
				DashboardCardHelper.kartu("Jumlah Kas Besar", fmtInt(total), "dokumen kas besar", "#1d4ed8"),
	...
	private void renderDpc(List<Baris> data) {
		...
		String donut = HtmlChartHelper.donut("Status Pembayaran (DPC)",
				"Sudah sampai mana pengajuan transfer dana kas besar: dari belum diajukan sampai sudah dibayar (cair).",
		...
		String bar = HtmlChartHelper.barHorizontal("Dana Dibayar per Bank",
				"Total dana kas besar yang sudah cair (DPC) dikelompokkan menurut bank pembayarnya.",
				bankLabels, bankVals, "#0f766e");
		Panelchildren isi = DashboardCardHelper.panel(body, "Pembayaran & Transfer (DPC)",
				"Memantau apakah dana kas besar sudah dibayar lewat DPC, lewat bank apa, dan sudah sampai mana prosesnya.");
	...
	private void renderSatkerDanPtj(List<Baris> data) {
		...
		String bar = HtmlChartHelper.barHorizontal("Nilai per Satuan Kerja",
				"Unit kerja mana yang paling besar pengeluaran kas besarnya.", labels, vals, "#4338ca");
		String donut = HtmlChartHelper.donut("Status Pertanggungjawaban",
				"Perbandingan kas besar yang sudah dan belum dibuat laporan pertanggungjawabannya (SPJ).",
	...
	private void renderGrid(List<Baris> data) {
		Panelchildren isi = DashboardCardHelper.panel(body, "Rincian Kas Besar",
				"Daftar lengkap tiap kas besar beserta nilai, status transfer, bank, pembayaran DPC, dan SPJ.");
		...
		if (data.isEmpty()) {
			...
			row.appendChild(new Label("Tidak ada kas besar pada rentang/filter ini."));
		}
	}
	...
	private List<Baris> loadData() {
		...
		try {
			...
			Criteria c = session.createCriteria(KasBesar.class)
					...
					.createAlias("jenisKasBesar", "jenisKasBesar", Criteria.LEFT_JOIN);
			...
			List<KasBesar> list = c.list();
			for (KasBesar k : list) {
				if (k == null) {
					continue;
				}
				Baris b = new Baris();
				try { b.kode = k.getKode(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/akunting/helper/MonitorKasBesarDashboard.java:359"); }
				try { b.nama = k.getNama(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/akunting/helper/MonitorKasBesarDashboard.java:360"); }
				try { b.satker = k.getSatuanKerja() == null ? null : k.getSatuanKerja().getNama(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/akunting/helper/MonitorKasBesarDashboard.java:361"); }
				try { b.jenis = k.getJenisKasBesar() == null ? null : k.getJenisKasBesar().getNama(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/akunting/helper/MonitorKasBesarDashboard.java:362"); }
				try { b.nilai = k.getNilai() == null ? 0.0 : k.getNilai().doubleValue(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/akunting/helper/MonitorKasBesarDashboard.java:363"); }
				try { b.tanggal = k.getTanggalPembuatan(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/akunting/helper/MonitorKasBesarDashboard.java:364"); }
				try { b.disetujui = k.getDisetujuiOleh() != null; } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/akunting/helper/MonitorKasBesarDashboard.java:365"); }
				try { b.sudahPtj = k.getPertangungjawabanKasBesar() != null; } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/akunting/helper/MonitorKasBesarDashboard.java:366"); }
				DaftarPengajuanTransfer dpt = null;
				try { dpt = k.getDaftarPengajuanTransfer(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/akunting/helper/MonitorKasBesarDashboard.java:368"); }
			...
			if (session != null) {
				try { session.clear(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/akunting/helper/MonitorKasBesarDashboard.java:386"); }
				try { session.close(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/akunting/helper/MonitorKasBesarDashboard.java:387"); }
			}
		}
		return hasil;
	}
	...
	private DashboardReportKit.SumberLaporan buatSumberLaporan() {
		return new DashboardReportKit.SumberLaporan() {
			@Override
			public String judul() {
				return "Monitor Kas Besar";
			}

			@Override
			public String subjudul() {
				return "Pemantauan Kas Besar & Pembayaran DPC";
			}

			@Override
			public String deskripsi() {
				return "Melihat nilai, status transfer/pembayaran DPC, bank, dan pertanggungjawaban kas besar.";
			}
			...
			public List<DashboardReportKit.Bagian> bagian() {
				...
				b.add(DashboardReportKit.kpi("Ringkasan",
						"Jumlah kas besar, total nilai, yang sudah dibayar DPC, dan yang sudah dipertanggungjawabkan.",
						new DashboardReportKit.PenyediaBaris() {
							...
							public List<Object[]> ambil() {
								...
								o.add(new Object[] { "Jumlah Kas Besar", DashboardReportKit.fmt(tot), "dokumen" });
				...
				b.add(DashboardReportKit.donut("Status Pembayaran (DPC)",
						"Sudah sampai mana pengajuan transfer dana kas besar.", new String[] { "Status", "Jumlah" },
						new DashboardReportKit.PenyediaBaris() {
							@Override
							public List<Object[]> ambil() {
								String[] urut = { DpcTransferStatusHelper.BELUM_AJU, DpcTransferStatusHelper.DIAJUKAN,
				...
				b.add(DashboardReportKit.tabel("Rincian Kas Besar", "Daftar kas besar & status pembayaran.",
						new String[] { "Kode", "Nama", "Satuan Kerja", "Jenis", "Nilai", "Status Transfer", "Via Bank",
