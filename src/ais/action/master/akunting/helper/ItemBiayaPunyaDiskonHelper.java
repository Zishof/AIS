...
public class ItemBiayaPunyaDiskonHelper {
	...
	public Borderlayout initDetail(final ItemBiaya itemBiaya) {
		...
		add.setVisible(ItemBiayaPunyaDiskonHelper.this.add);
					...
					public void onEvent(Event arg0) throws Exception {
						...
						AmbilDataBanyakAkun ambilDataAkunBanyak = new AmbilDataBanyakAkun(akuns, new EventListener() {
							...
							public void onEvent(Event arg0) throws Exception {
								...
								for (Akun akun : akuns) {
									ItemBiayaPunyaDiskon itemBiayaPunyaDiskon = new ItemBiayaPunyaDiskon();
									itemBiayaPunyaDiskon.setItemBiaya(itemBiaya);
									itemBiayaPunyaDiskon.setAkun(akun);

									Rows rows = gridDiskon.getRows() == null ? new Rows() : gridDiskon.getRows();
									rows.setParent(gridDiskon);
									Row row = new Row();row.setValign("top");
									row.setParent(rows);
									initRow(row, itemBiayaPunyaDiskon);
								}
							}
						});
		...
		Common.clear(gridDiskon);
		gridDiskon.setParent(center);
		gridDiskon.setWidth("100%");
		gridDiskon.setHeight("100%");
		Columns columns = new Columns();
		columns.setParent(gridDiskon);
	...
	private void loadDataDetail(final ItemBiaya itemBiaya) {
		...
		List<ItemBiayaPunyaDiskon> itemBiayaPunyaDiskons = itemBiaya == null || itemBiaya.getId() == null
				? new ArrayList<ItemBiayaPunyaDiskon>()
				: HibernateUtil.currentSession().createCriteria(ItemBiayaPunyaDiskon.class)
						.add(Restrictions.eq("itemBiaya", itemBiaya)).list();

		Rows rows = gridDiskon.getRows() == null ? new Rows() : gridDiskon.getRows();
		rows.setParent(gridDiskon);

		for (ItemBiayaPunyaDiskon itemBiayaPunyaDiskon : itemBiayaPunyaDiskons) {
			Row row = new Row();row.setValign("top");
			row.setParent(rows);
			initRow(row, itemBiayaPunyaDiskon);
		}
	...
	public void initRow(final Row row, final ItemBiayaPunyaDiskon itemBiayaPunyaDiskon) {
		...
		new Label(itemBiayaPunyaDiskon.getAkun() == null ? "" : itemBiayaPunyaDiskon.getAkun().getKode())
				.setParent(row);

		try {
			RevisiHelper
					.createNewRevisi(ItemBiayaPunyaDiskon.class, itemBiayaPunyaDiskon,
							itemBiayaPunyaDiskon.getAkun() == null ? "" : itemBiayaPunyaDiskon.getAkun().getNama())
					.setParent(row);
		} catch (Exception e) {
			new Label(itemBiayaPunyaDiskon.getAkun() == null ? "" : itemBiayaPunyaDiskon.getAkun().getNama())
					.setParent(row);
		...
		final Textbox angkatan = new Textbox(itemBiayaPunyaDiskon.getAngkatan());
		...
		Common.selectComboItem(fakultas, itemBiayaPunyaDiskon.getFakultas());
		...
		Common.insertComboDanSemua(jurusan, new String[] { "nama", "kodeEpsbed" }, "jenjang", Jurusan.class,
				Restrictions.and(
						itemBiayaPunyaDiskon.getFakultas() == null ? Restrictions.sqlRestriction("true")
								: Restrictions.eq("fakultas", itemBiayaPunyaDiskon.getFakultas()),
						Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))));
		Common.pilihJurusan(jurusan, itemBiayaPunyaDiskon.getJurusan());
		jurusan.setWidth("90%");
		jurusan.setReadonly(true);
		jurusan.setParent(row);

		Common.selectComboItem(program, itemBiayaPunyaDiskon.getProgram());
		program.setWidth("90%");
						...
						new EventListener() {
							...
							public void onEvent(Event event) throws Exception {
								...
								if (i == MyMessageboxConfig.OK) {
									if (itemBiayaPunyaDiskon.getId() != null) {
										Session session = HibernateUtil.currentSession();
										session.delete(itemBiayaPunyaDiskon);
									}
