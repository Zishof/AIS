...
/**
 * Helper terfokus untuk transaksi jurnal pengeluaran. Tipe ini membungkus satu variasi kecil dari
 ...
 * boolean edit}, {@code boolean delete}, {@code String parentCode}, {@code Transaksi transaksiUtama}, {@code
 * AmbilDataAkunKreditBanbox kodeAkun}, {@code Textbox keteranganAkun}; inisialisasi/lifecycle ({@code
 * initWindow()}); pembacaan/pencarian ({@code onSearchDefault()}, {@code setEventListener()}, {@code
 * getEventListener()}); validasi/perhitungan ({@code validasiSimpan()}, {@code hitungTotal()}); mutasi data
 * ({@code onSaveTransaksiUtama()}); operasi domain lain ({@code onAdd()}). Bagian lain dari kontrak tetap
 ...
 * @see MyWindow
 */
public class TransaksiJurnalPengeluaranHelper extends MyWindow {
	...
	private AmbilDataAkunKreditBanbox kodeAkun;
	private Textbox keteranganAkun;
	private Label noref;
	// private Textbox kelompokArusKas;
	...
	public TransaksiJurnalPengeluaranHelper(Transaksi transaksiUtama) throws Exception {
		super();
		this.transaksiUtama = transaksiUtama;
		initWindow();
	}

	public TransaksiJurnalPengeluaranHelper(Transaksi transaksiUtama, String title, String border, boolean closable)
			throws Exception {
	...
	public void initWindow() throws Exception {
		...
		transaksiUtama.setJenisJurnal(Transaksi.JURNAL_KAS_KELUAR);
		setWidth("80%");
		setHeight("90%");
		setClosable(false);
		...
		panel.setWidth("100%");
		panel.setHeight("100%");
		panel.setTitle("Jurnal Pengeluaran Kas / Bank");
		...
		row.appendChild(tanggal = new MyDatebox(
				transaksiUtama.getTanggalTransaksi() == null ? ais.ui.util.WaktuUtil.getDate() : transaksiUtama.getTanggalTransaksi()));
		...
		row.appendChild(noref = new Label(
				grupTransaksi.getKode() == null ? CommonAkunting.generateNoJurnal(grupTransaksi.getJenisTransaksi(), false) : grupTransaksi.getKode()));

		row = new MyFormRow();
		row.setParent(rows);

		row.appendChild(new ais.ui.util.MyLabelConfig("Akun Pengeluaran"));
		row.appendChild(kodeAkun = new AmbilDataAkunKreditBanbox());
		...
		satuanKerja.setAttribute("satuanKerja", grupTransaksi.getSatuanKerja() == null
				? Common.getCurrentUser().ambilSatuanKerja() : grupTransaksi.getSatuanKerja());
		...
		row.appendChild(jumlah = new MyDoublebox(transaksiUtama.getKredit()));

		row = new MyFormRow();
		row.setParent(rows);

		row.appendChild(new ais.ui.util.MyLabelConfig("Keterangan Pengeluaran"));
		...
		batal.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				TransaksiJurnalPengeluaranHelper.this.detach();
			}
		});
			...
			if (grupTransaksi != null && grupTransaksi.getPostingHistory() != null) {
				...
				timer.addEventListener("onTimer", new EventListener() {
					...
					public void onEvent(Event arg0) throws Exception {
						...
						Common.freeze(TransaksiJurnalPengeluaranHelper.this, true);
						batal.setDisabled(false);
					}
				});
				timer.start();
	...
	/**
	 * Tipe implementasi bersarang {@link TransaksiJurnalPengeluaranEditor} milik {@link
	 * TransaksiJurnalPengeluaranHelper}. Kelas ini memberi nama pada state atau perilaku lokal agar tanggung
	 * jawabnya tidak tersebar sebagai blok anonim.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link TransaksiJurnalPengeluaranHelper} dan dapat
	 ...
	 *
	 * @see TransaksiJurnalPengeluaranHelper
	 */
	private class TransaksiJurnalPengeluaranEditor {

		private Transaksi transaksi;

		public TransaksiJurnalPengeluaranEditor(Transaksi transaksi) {
		...
		public boolean onSave(Event event) throws Exception {
			...
			if (transaksi.getDebet() == null) {
				MyMessageboxConfig.show("Mohon maaf, Nilai Debet belum diisi. Langkah yang dapat dilakukan: (1) Isikan kolom Nilai Debet dengan nominal yang valid; (2) Pastikan nilai tidak kosong dan lebih dari nol; (3) ulangi proses simpan. Jika masih mengalami kendala, hubungi Administrator atau tim teknis.", "Peringatan", MyMessageboxConfig.OK,
						MyMessageboxConfig.EXCLAMATION);
				return false;
			}
			if (transaksi.getKredit() == null) {
				MyMessageboxConfig.show("Mohon maaf, Nilai Kredit belum diisi. Langkah yang dapat dilakukan: (1) Isikan kolom Nilai Kredit dengan nominal yang valid; (2) Pastikan nilai tidak kosong; (3) ulangi proses simpan. Jika masih mengalami kendala, hubungi Administrator atau tim teknis.", "Peringatan", MyMessageboxConfig.OK,
						MyMessageboxConfig.EXCLAMATION);
				return false;
			}
			...
			transaksi.setMerupakanDebet(true);
			transaksi.setGrupTransaksi(grupTransaksi);

			Common.refreshSaveOrUpdate(transaksi);
			hitungTotal();
			return true;
		}
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link TransaksiJurnalPengeluaranHelper}. Kelas ini menerjemahkan satu
	...
	class TransaksiRenderer extends ais.ui.util.MyRowRenderer {
		...
		private void initEdit(final Row arg0, final Object arg1) throws Exception {
			Common.clear(arg0);
			final Transaksi transaksi = (Transaksi) arg1;
			final TransaksiJurnalPengeluaranEditor transaksiJurnalPengeluaranEditor = new TransaksiJurnalPengeluaranEditor(
					transaksi);
			transaksi.setParentCode(parentCode);
			transaksi.setJenisJurnal(Transaksi.JURNAL_KAS_KELUAR);
			transaksi.setTanggalTransaksi(tanggal.getValue());

			final AmbilDataAkunDebetBanbox akunBanbox = new AmbilDataAkunDebetBanbox();
			...
			pegawaiBanbox
					.setValue(
							transaksi.getPegawai() == null ? ""
									: (transaksi.getPegawai().getKode() == null ? ""
											: transaksi.getPegawai().getKode() + " - ")
			...
			save.addEventListener("onClick", new EventListener() {
				@Override
				public void onEvent(Event event) throws Exception {
					if (transaksiJurnalPengeluaranEditor.onSave(event)) {
	...
	public void onAdd(Event event) throws Exception {
		...
		if (jumlah.getValue() == null || jumlah.getValue() < 1.0) {
			MyMessageboxConfig.show("Mohon maaf, Nilai Kredit tidak boleh nol. Langkah yang dapat dilakukan: (1) Isikan kolom Nilai Kredit dengan nominal yang lebih dari nol; (2) Pastikan nilai transaksi pengeluaran sudah benar; (3) ulangi proses simpan. Jika masih mengalami kendala, hubungi Administrator atau tim teknis.", "Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.EXCLAMATION);
			jumlah.focus();
			return;
		}
		if (keteranganAkun.getValue().trim().equals("")) {
			MyMessageboxConfig.show("Mohon maaf, Keterangan belum diisi. Langkah yang dapat dilakukan: (1) Isikan kolom Keterangan dengan deskripsi transaksi pengeluaran yang jelas; (2) Pastikan keterangan tidak kosong atau hanya terdiri dari spasi; (3) ulangi proses simpan. Jika masih mengalami kendala, hubungi Administrator atau tim teknis.", "Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.EXCLAMATION);
			return;
		}
	...
	public boolean onSaveTransaksiUtama(Event event) throws Exception {
		...
		transaksiUtama.setMerupakanDebet(false);
	...
	public void onSearchDefault(Event event) {
		...
		System.out.println("transaksi = " + transaksi.size());
	...
	private void hitungTotal() {
		...
		if (grid != null && grid.getRows() != null && grid.getRows().getChildren() != null) {
			...
			for (Row row : rows) {
				...
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/akunting/helper/TransaksiJurnalPengeluaranHelper.java:948");

				}
			}
		}
		// grupTransaksi.setTotalDebet(total);
		// grupTransaksi.setTotalKredit(jumlah.getValue());
		grupTransaksi.setTotalKredit(total);
		grupTransaksi.setTotalDebet(jumlah.getValue());
