...
public final class DraftJurnalRingkasanUtil {
    ...
    public static final String[] KUNCI = { "jurnal_umum", "uang_muka", "pj_uang_muka", "pj_pengembalian", "kas_kecil",
            ...
            "pembagian_shu", "biaya_sesi_sales", "pembatalan_kantin", "mahasiswa", "siswa", "penyusutan", "penghapusan_aset", "pengajuan_transfer", "transitori", "closing",
    ...
    public static String kategoriUntukKunci(String kunci) {
        ...
        // Biaya sesi sales bagian modul Inventory & Sales, bukan dana anggota.
        if ("biaya_sesi_sales".equals(kunci)) return "posting_penjualan";
    ...
    public static List<Baris> hitungModul(Session session, String kunci, Date mulai, Date sampai) {
        ...
        if ("jurnal_umum".equals(kunci)) {
        ...
        } else if ("penyesuaian_saldo_anggota".equals(kunci)) {
            out.add(new Baris(kunci, "Penyesuaian Saldo Anggota",
                    "Hasil opname saldo e-wallet anggota dipastikan menjadi jurnal koreksi kewajiban"
                            + " (jurnal memorial, tanpa kas).",
                    hitungPostingHistory(session, kriteriaPenyesuaianSaldo(session, mulai, sampai),
                            false),
                    hitungPostingHistory(session, kriteriaPenyesuaianSaldo(session, mulai, sampai),
                            true),
                    hitungClosing(session, "penyesuaianSaldoAnggota", null, null, mulai, sampai)));
        } else if ("modal_penyertaan".equals(kunci)) {
            out.add(new Baris(kunci, "Modal Penyertaan Masuk",
                    "Setoran modal penyertaan dipastikan menjadi jurnal kas masuk dan modal"
                            + " penyertaan.",
            ...
            out.add(new Baris(kunci, "Pengembalian Modal Penyertaan",
                    ...
                    hitungClosing(session, "modalPenyertaanKoperasi", null, null, mulai, sampai)));
        } else if ("biaya_sesi_sales".equals(kunci)) {
            out.add(new Baris(kunci, "Biaya Sesi Sales",
                    "Biaya sesi sales lapangan yang masih aktif dipastikan menjadi jurnal beban"
                            + " memakai akun kategori biayanya.",
                    hitungPostingHistory(session, kriteriaBiayaSales(session, mulai, sampai), false),
                    hitungPostingHistory(session, kriteriaBiayaSales(session, mulai, sampai), true),
                    hitungClosing(session, "notaSalesBiaya", null, null, mulai, sampai)));
    ...
    private static Criteria kriteriaPenyesuaianSaldo(Session session, Date mulai, Date sampai) {
        return ais.action.master.koperasi.helper.PostingDanaAnggotaUtil.kriteriaPenyesuaianStatic(
                session, mulai, sampai);
    }
    ...
    private static Criteria kriteriaBiayaSales(Session session, Date mulai, Date sampai) {
        return ais.action.master.koperasi.helper.PostingBiayaSalesUtil.kriteriaBiayaStatic(session,
                mulai, sampai);
    }
    ...
    */
    ...
    public static Criteria kriteriaDokumen(Session session, String namaBaris, Date mulai, Date sampai) {
        ...
        if ("Penyesuaian Saldo Anggota".equals(namaBaris)) {
            return kriteriaPenyesuaianSaldo(session, mulai, sampai);
        }
        ...
        if ("Biaya Sesi Sales".equals(namaBaris)) {
            return kriteriaBiayaSales(session, mulai, sampai);
        }
