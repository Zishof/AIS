...
/**
 * Controller/action ZK untuk pembelian anggota koperasi. Tipe ini merupakan titik masuk UI yang
 ...
 * searchmr}, {@code MyTextbox kode}, {@code AmbilDataAnggotaKoperasiBanbox anggotaKoperasi};
 * inisialisasi/lifecycle ({@code doAfterCompose()}, {@code mainInit()}, {@code initDetailTransaksi()}, {@code
 * init()}, {@code initCriteria()}); pembacaan/pencarian ({@code onSearchDefault()}); mutasi data ({@code
 * onSave()}, {@code simpanDetailTransaksi()}); penghapusan/pembatalan ({@code onDelete()}); pelaporan/ekspor
 * ({@code onCetakPembelianAnggotaKoperasi()}); operasi domain lain ({@code onAdd()}). Bagian lain dari kontrak
