...
public class PostingHppKantinAction extends GenericAutowireComposer {
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		// "Posting Semua yang Siap" -- memakai mode PER barang (pola Posting Cicilan
		...
		btnPostingSiap.addEventListener("onClick", new EventListener() {
			@Override
			public void onEvent(Event e) throws Exception {
				JSONObject ringkas = postingPerBarang(java.util.Collections.<Long>emptyList(), dpMulai.getValue(), dpSampai.getValue());
				MyMessageboxConfig.show(
						"Diposting: " + ringkas.optInt("diposting", 0) + " barang; dilewati (belum siap): "
	...
	private boolean bolehMemposting() {
		...
		return ais.common.EbisnisMenuKatalog.bolehAksiAkuntansi(peran.getEbisnisMenu(),
				peran.getRoleId(), "posting_hpp", "approve");
	}

	private void onPosting() throws Exception {
		if (!bolehMemposting()) {
			ais.ui.util.MyMessageboxConfig.show("Grup pengguna Anda tidak memiliki hak memposting pada menu ini.", "Posting HPP",
