...
public class PostingPenjualanKantinAction extends GenericAutowireComposer {
	...
	/**
	 * Membatalkan batch posting penjualan yang TANGGAL riwayatnya jatuh pada rentang --
	 * mundur-saja (LIFO), cermin dari aturan maju-saja postingnya (lihat
	 * {@code PostingHppKantinAction.batalkanPeriode}). Tanda {@code posting_history} pada header
	 * penjualan ({@code koperasi.pembelian_anggota_koperasi}) DILEPAS sebelum riwayatnya dihapus
	 * sehingga dokumennya kembali terhitung draf dan dapat diposting ulang. Jurnal yang sudah
	 * closing tidak disentuh: batch yang grupnya terlanjur closing dilewati utuh.
	 */
	public static int batalkanPeriode(Date mulai, Date sampai) {
		if (mulai == null || sampai == null || mulai.after(sampai)) {
			throw new IllegalArgumentException("Periode pembatalan penjualan tidak valid.");
		}
		int n = 0;
		...
		try {
			...
			if (lebihBaru != null && lebihBaru.intValue() > 0) {
				throw new IllegalStateException("Masih ada " + lebihBaru.intValue() + " batch \"" + JENIS
						...
						+ ". Batalkan periode yang lebih baru dulu -- pembatalan penjualan mundur-saja.");
			}
			...
			for (Object o : daftar) {
				...
				try {
					...
					session.createSQLQuery(
							...
							.executeUpdate();
					// Lepas tanda header SEBELUM riwayatnya dihapus (FK header -> posting_history).
					session.createSQLQuery("UPDATE koperasi.pembelian_anggota_koperasi"
							+ " SET posting_history = NULL WHERE posting_history = " + idPh)
							.executeUpdate();
					...
					ais.common.ErrorAuditUtil.record(e, "PostingPenjualanKantinAction.batalkanPeriode");
				}
			}
		} finally {
			try {
