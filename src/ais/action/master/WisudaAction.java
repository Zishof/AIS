...
/**
 * Controller/action ZK untuk wisuda. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * paging}, {@code MyGrid grid}, {@code Textbox searchWisudaKe}, {@code Textbox wisudaKe}, {@code Intbox
 * maksimalQuota}, {@code MyDatebox tanggal}, {@code Textbox moto}; inisialisasi/lifecycle ({@code
 * doBeforeCompose()}, {@code doAfterCompose()}, {@code init()}, {@code initCriteria()}); pembacaan/pencarian
 * ({@code onSearchDefault()}); mutasi data ({@code onSave()}); operasi domain lain ({@code onStatistik()},
 * {@code onGenerateNoKursi()}, {@code onPesertaWisuda()}, {@code onAlbumWisuda()}, {@code onAlbumProfile()},
 * {@code onTranskipWisuda()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut
 * di atas.</p>
