...
/**
 * Controller/action ZK untuk jenis tugas tree. Tipe ini merupakan titik masuk UI yang
 ...
 * checkKodeJenisTugas()}); mutasi data ({@code onSave()}); operasi domain lain ({@code onAdd()}, {@code
 ...
 */
public class JenisTugasTreeAction extends GenericAutowireComposer {
	...
	private JenisTugasTreeModel jenisTugasTreeModel;

	private TreeMap<JenisTugas, Treecell[]> treecellMap = new TreeMap<JenisTugas, Treecell[]>();
	...
	private void initTree() throws Exception {
		...
		Treecol treecol = new Treecol("Standar Biaya");
		treecol.setParent(treecols);

		treecol = new Treecol("Biaya");
		treecol.setWidth("10%");
		treecol.setParent(treecols);
	...
	public void onAdd(Event event) throws Exception {
		...
		JenisTugas myjenisTugas = new JenisTugas();
		myjenisTugas.setParent(null);

		init(myjenisTugas, new EventListener() {
	...
	private void init(JenisTugas jenisTugas, final EventListener eventListener) {
		this.jenisTugas = jenisTugas;
		addWindow.setTitle(jenisTugas.getId() == null ? "Tambah Standar Biaya" : "Ubah Standar Biaya");
		...
		row.appendChild(kode = new Textbox(jenisTugas.getKode()));
		kode.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Nama"));
		row.appendChild(nama = new Textbox(jenisTugas.getNama() == null ? "" : jenisTugas.getNama()));
		nama.setWidth("90%");
		...
		defaultItem.setChecked(jenisTugas.getDefaultItem() != null && jenisTugas.getDefaultItem());

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Keterangan"));
		row.appendChild(keterangan = new Textbox(jenisTugas.getKeterangan() == null ? "" : jenisTugas.getKeterangan()));
		...
		save.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				if (onSave(event)) {
					// onReloadTree(null);
					eventListener.onEvent(new Event("", null, JenisTugasTreeAction.this.jenisTugas));
					addWindow.setVisible(false);
				}
			}
		});
	...
	public boolean onSave(Event event) throws Exception {
		...
		// boolean i = checkKodeJenisTugas();
		// if (i) {
		// MyMessageboxConfig.show("Kode sudah ada di database", "Peringatan",
		// MyMessageboxConfig.OK, MyMessageboxConfig.INFORMATION);
		// return false;
		// }
		JenisTugasDao jenisTugasDao = DaoFactory.getInstance().getJenisTugasDao();
		if (jenisTugas.getId() != null) {
			jenisTugas = jenisTugasDao.load(jenisTugas.getId());
		}

		jenisTugas.setDefaultItem(defaultItem.isChecked());
		jenisTugas.setKode(kode.getValue());
		jenisTugas.setNama(nama.getValue());
		jenisTugas.setKeterangan(keterangan.getValue());

		if (jenisTugas.getId() != null) {
			jenisTugasDao.update(jenisTugas);
		} else {
			jenisTugasDao.save(jenisTugas);
		}

		return true;
	}

	public Boolean checkKodeJenisTugas() {

		Integer kotaCount = null;
		Session session = HibernateUtil.currentSession();
		kotaCount = ((Number) session.createCriteria(JenisTugas.class).setProjection(Projections.rowCount())
				.add(Restrictions.eq("kode", kode.getValue().trim())).add(this.jenisTugas.getId() == null
						? Restrictions.sqlRestriction("1=1") : Restrictions.ne("id", this.jenisTugas.getId()))
	...
	public void onReloadTree(Event event) throws Exception {
		...
		jenisTugasTreeModel = new JenisTugasTreeModel(true);
		...
		tree.setItemRenderer(new ais.ui.util.MyTreeitemRenderer() {
			...
			public void render(final Treeitem treeitem, Object arg1) throws Exception {
				...
				try {
					...
					hasSomeChilds(treerow, jenisTugas);
					...
					button.addEventListener("onClick", new EventListener() {
						...
						public void onEvent(Event event) throws Exception {
							...
							JenisTugas myjenisTugas = (JenisTugas) jenisTugas.clone();
							myjenisTugas.setParent(jenisTugas);
							myjenisTugas.setId(null);
							init(myjenisTugas, new EventListener() {

								@Override
								public void onEvent(Event arg0) throws Exception {
									render(treeitem, jenisTugas);
												...
												public void onEvent(Event arg0) throws Exception {
													...
													try {
														Treeitem myTreeitem = (Treeitem) treecellMap
																.get(jenisTugas)[0].getParent().getParent();

														render(myTreeitem, jenisTugas);
					...
					button.addEventListener("onClick", new EventListener() {
						...
						public void onEvent(Event event) throws Exception {
							...
							JenisTugas myjenisTugas = (JenisTugas) jenisTugas.clone();
							myjenisTugas.setParent(jenisTugas.getParent());
							myjenisTugas.setId(null);
							init(myjenisTugas, new EventListener() {
					...
					button.addEventListener("onClick", new EventListener() {
						@Override
						public void onEvent(Event event) throws Exception {
							init(jenisTugas, new EventListener() {
								...
								public void onEvent(Event event) throws Exception {
									...
									if (i == MyMessageboxConfig.OK) {
										try {
											...
											jenisTugasTreeModel.deleteChilds(jenisTugas);

											Common.refreshDelete((jenisTugas));
	...
	private void hasSomeChilds(Treerow treerow, final JenisTugas jenisTugas) {

		Treecell treecell = new Treecell(jenisTugas.toString());
		treecell.setTooltiptext(jenisTugas.toString());
		treecell.setStyle("font-size:x-small;text-align: left;");
		treecell.setParent(treerow);

		treecell = new Treecell(Common.numberFormat.get().format(jenisTugas.getDefaultBiaya()));
		treecell.setTooltiptext(jenisTugas.getDefaultBiaya() + "");
		treecell.setStyle("font-size:x-small;text-align: left;");
		treecell.setParent(treerow);

		Treecell treecellAktif = new Treecell(
				jenisTugas.getDefaultItem() == null || !jenisTugas.getDefaultItem() ? "Tidak" : "Aktif");
		...
		treecellMap.put(jenisTugas, new Treecell[] { treecellAktif });
