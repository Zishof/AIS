...
/**
 * Controller/action ZK untuk kalender. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code String CALENDAR_EVENT}, {@code String
 * CTRL_EVENT}, {@code ThreadLocal DefaultDateFormat}, {@code ThreadLocal DefaultMonthFormat}, {@code
 * SimpleDateFormat timeFormat}, {@code MyToolbarbuttonConfig currentDate}, {@code Textbox filter_text}, {@code
 * Menuitem prevPage}; inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code doAfterCompose()}, {@code
 * initTimeDropdown()}); pembacaan/pencarian ({@code getPopupPosition()}); mutasi data ({@code
 * onEventUpdate$calendars()}, {@code updatePopup()}, {@code setCalendar()}); penghapusan/pembatalan ({@code
 * onClick$cancelBtn$EventPop()}, {@code onClick$deleteBtn$EventPop()}); operasi domain lain ({@code
 * onClick$tabJurnalPengeluaran$EventPop()}, {@code onEventFilter()}, {@code onMoveDate$calendars()}, {@code
 * onClick$prevPage()}, {@code onClick$nextPage()}, {@code onEventCreate$calendars()}). Bagian lain dari kontrak
