...
/**
 * Controller/action ZK untuk chart perencanaan. Tipe ini merupakan titik masuk UI yang
 ...
 * {@code NumberFormat numberFormat}, {@code Combobox tahunWorkspace}, {@code MyChart mychart}, {@code
 * SimpleCategoryModel categoryModel}, {@code SimplePieModel simplePieModel}, {@code Hbox hirarchy};
 * inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code doAfterCompose()}); pembacaan/pencarian ({@code
 * onRefresh()}, {@code onTampil()}); mutasi data ({@code updateBulan()}, {@code updateChart()});
 * pelaporan/ekspor ({@code onReport()}); operasi domain lain ({@code onPilihKegiatan()}, {@code
