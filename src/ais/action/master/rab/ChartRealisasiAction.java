...
/**
 * Controller/action ZK untuk chart realisasi. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code List workspaces}, {@code Long index},
 * {@code Combobox tahunWorkspace}, {@code MyChart mychart}, {@code SimpleCategoryModel categoryModel}, {@code
 * Hbox hirarchy}, {@code Workspace currentWorkspace}, {@code AmbilDataSatuanKerjaBanbox satuanKerja};
 * inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code doAfterCompose()}); pembacaan/pencarian ({@code
 * onRefresh()}, {@code onTampil()}, {@code getRealisasi()}); mutasi data ({@code updateBulan()}, {@code
 * updateChart()}); pelaporan/ekspor ({@code onReport()}); operasi domain lain ({@code onPilihKegiatan()}, {@code
 * generateHirarcy()}, {@code createModel()}, {@code createCustomModel()}, {@code onKembali()}, {@code
 * onClickChart()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di
 * atas.</p>
 ...
 */
public class ChartRealisasiAction extends GenericAutowireComposer implements Pemilih {
	...
	private void generateHirarcy(Long uniqueId, List<A> as) throws Exception {
		...
		final Workspace workspace = (Workspace) HibernateUtil.currentSession().createCriteria(Workspace.class)
		...
		if (workspace != null) {
			...
			mychart.setTitle("RENCANA ANGGARAN DAN REALISASI"
					+ (bulan == null ? "" : " BULAN " + Common.BULAN[bulan - 1].toUpperCase()) + " TAHUN "
					+ (tahunWorkspace.getSelectedItem() == null ? "" : tahunWorkspace.getSelectedItem().getValue()));
		}
	}
	...
	private void createModel(Long parentId) throws Exception {
		...
		WorkspaceTreeModel workspaceTreeModel = new WorkspaceTreeModel(tahun, rabReportHelper.getMaxrevisi(),
		...
		workspaces = session.createCriteria(Workspace.class)
				...
				.add(bulan == null ? Restrictions.sqlRestriction("1=1")
						: Restrictions.sqlRestriction("date(mulai) between date('" + tanggal_mulai + "') and date('"
								+ tanggal_selesai + "')"))
				.add(Restrictions.or(
				...
				.add(sumberDana == null ? Restrictions.sqlRestriction("1=1")
						: Restrictions.eq("sumberDana", sumberDana))
				.add(Restrictions.eq("parentId", parentId)).addOrder(Order.asc("kode")).addOrder(Order.asc("nama"))
		...
		for (Workspace workspace : workspaces) {
			...
			if (bulan != null) {
				...
				childsId.add(workspace.getId());
				workspaceTreeModel.generateChildsByIds(workspace.getId(), childsId);
				realisasi = getRealisasi(childsId, dateFormat.parse(tanggal_mulai), dateFormat.parse(tanggal_selesai));
			}

			Double persen = ((realisasi) * 100)
					/ ((workspace.getHargaTotal() == null || workspace.getHargaTotal().equals(0.0)) ? 0.00001
							: workspace.getHargaTotal());

			String nama = workspace.getNama() + " - " + Common.numberFormat.get().format(workspace.getHargaTotal()) + " - "
					+ Common.numberFormat.get().format(realisasi) + " - " + Common.numberFormat.get().format(persen) + "%";

			categoryModel.setValue(nama, "Perencanaan", workspace.getHargaTotal());
			categoryModel.setValue(nama, "Realisasi", realisasi);

		}

	}

	public Double getRealisasi(Set<Long> workspaces, Date mulai, Date sampai) {
		...
		Number count = ((Number) session.createCriteria(Transaksi.class).createAlias("grupTransaksi", "grupTransaksi")
				...
				.add(Restrictions.in("grupTransaksi.workspace.id", workspaces)).setProjection(Projections.sum("kredit"))
				.uniqueResult());
	...
	private void createCustomModel() {
		if (/*
			 * this.jenisChart.getSelectedItem() == null ||
			 */customeWorkspaces == null || customjudul == null)
			return;
		// String jenisChart = (String) this.jenisChart.getSelectedItem()
		// .getValue();
		...
		for (Workspace workspace : customeWorkspaces) {
			categoryModel.setValue(workspace.getNama() + " - " + Common.numberFormat.get().format(workspace.getHargaTotal()),
		...
		// for (Workspace workspace : customeWorkspaces) {
		// simplePieModel.setValue(workspace.getNama() + " - "
		// + numberFormat.get().format(workspace.getHargaTotal()),
		// workspace.getHargaTotal());
		// }
		// }
	}
	...
	public void onKembali(Event event) {
		try {
			...
			currentWorkspace = (Workspace) HibernateUtil.currentSession().createCriteria(Workspace.class)
					.add(Restrictions.or(Restrictions.eq("carryOver", true),
							Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))))
			...
			try {
				...
				e1.printStackTrace(); ais.common.ErrorAuditUtil.record(e1, "auto-audit src/ais/action/master/rab/ChartRealisasiAction.java:414");
			}
		}
	}
		...
		try {
			...
			if (area != null) {
				...
				if (strs.length >= 1) {
					...
					Workspace myWorkspace = (Workspace) HibernateUtil.currentSession().createCriteria(Workspace.class)
							.add(Restrictions.or(Restrictions.eq("carryOver", true),
									Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))))
							.add(Restrictions.or(
							...
							// .add(Restrictions.eq("tahunWorkspace", tahun))
							.add(Restrictions.eq("parentId", parentId)).uniqueResult();

					if (myWorkspace == null)
						return;
					updateBulan(tahun);
					Integer count = ((Number) HibernateUtil.currentSession().createCriteria(Workspace.class)
							.add(Restrictions.or(Restrictions.eq("carryOver", true),
									Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))))
							.add(bulan == null ? Restrictions.sqlRestriction("1=1")
							...
							.add(sumberDana == null ? Restrictions.sqlRestriction("1=1")
									: Restrictions.eq("sumberDana", sumberDana))
							.add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
							.setProjection(Projections.rowCount()).add(Restrictions.eq("parentId", myWorkspace.getId()))
