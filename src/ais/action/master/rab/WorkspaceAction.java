...
/**
 * Halaman utama <b>Anggaran / RAB</b> (Rencana Anggaran dan Belanja) tingkat perencanaan.
 *
 * <p>
 * Layar ini adalah pintu masuk pengelolaan anggaran satu satuan kerja: pengguna memilih
 * <b>Tahun Anggaran</b>, <b>Satuan Kerja</b>, dan (bila tersedia lebih dari satu) <b>Sumber Dana</b>,
 * lalu sistem menampilkan daftar item anggaran per <b>Revisi</b> dalam bentuk tab. Tiap tab "Revisi N"
 * memuat halaman rincian <code>workspace_revisi.zul</code> (dirender via {@link MyInclude}) yang berisi
 * pohon item anggaran beserta volume, satuan, harga satuan, dan total. Dari sini pengguna juga dapat
 * membuat <b>Revisi baru</b> (menyalin seluruh anggaran dari revisi sebelumnya) serta melihat
 * <b>History</b> perubahan revisi.
 * </p>
 *
 * <h3>Alur kerja</h3>
 * <ol>
 * <li><b>{@link #doAfterCompose(Component)}</b> — menentukan hak akses (tambah/ubah/hapus), menyiapkan
 * model pohon satuan kerja, memasang tombol "History", lalu memuat tab awal lewat timer agar UI tampil
 * dahulu (responsif).</li>
 * <li><b>{@link #onReloadTab(Event)}</b> — dipanggil saat Satuan Kerja berubah: mengisi ulang pilihan
 * Sumber Dana sesuai tahun &amp; satuan kerja, menyembunyikan pemilih Sumber Dana bila hanya ada satu
 * (sekaligus membuat Sumber Dana default), menentukan apakah tombol "Revisi baru" boleh tampil
 * (hanya pada satuan kerja paling bawah/leaf), lalu memuat tab revisi.</li>
 * <li><b>{@link #loadTabRevisi()}</b> — membaca daftar nomor revisi yang ada di basis data untuk
 * kombinasi (tahun, satuan kerja, sumber dana), lalu membuat satu tab per revisi; bila belum ada
 * revisi, menampilkan satu tab "Revisi 1" kosong.</li>
 * <li><b>{@link #onCreateNewRevisi(Event)}</b> — menghitung nomor revisi berikutnya (maks + 1) lalu,
 * setelah konfirmasi, menyalin anggaran revisi terakhir menjadi revisi baru melalui
 * {@link RabUtil#createNewRevisi}.</li>
 * </ol>
 *
 * <h3>Higiene session basis data</h3>
 * Pembacaan biasa memakai {@link HibernateUtil#currentSession()} (session ber-{@code ThreadLocal} yang
 * ditutup otomatis oleh kerangka kerja — TIDAK boleh ditutup manual). Operasi tulis pembuatan Sumber
 * Dana default memakai {@link HibernateUtil#currentNativeSession()} yang <b>wajib</b> ditutup; di sini
 * penutupannya dipindahkan ke blok {@code finally} via {@link HibernateUtil#closeSessionQuietly} agar
 * koneksi tidak menggantung ("idle in transaction") meski terjadi kesalahan saat menyimpan.
 *
 * <h3>Catatan teknis</h3>
 * Kompatibel Java 1.7 dan ZK 5.5. Penanganan {@code try/catch} ditulis gaya Java 1.6 (tanpa multi-catch
 * maupun try-with-resources). Pembuatan komponen UI memaksimalkan komponen pembungkus yang sudah ada
 * ({@code MyTabConfig}, {@code MyToolbarbuttonConfig}, {@code MyInclude}) demi konsistensi tampilan dan
 * kemudahan perawatan. Pemuatan berat (tab + include) sengaja dijalankan lewat timer singkat agar
 * kerangka halaman tampil lebih dahulu sehingga terasa ringan di perangkat mobile maupun desktop.
 */
public class WorkspaceAction extends GenericAutowireComposer {
	...
	public void onReloadTab(Event event) throws Exception {
		...
		if (mySatuanKerja != null && sumberDana.getChildren().size() > 2) {
			...
			try {
				...
				} catch (Exception ig) { ais.common.ErrorAuditUtil.record(ig, "auto-audit(empty-catch) src/ais/action/master/rab/WorkspaceAction.java:279");
				}
				throw eSimpan;
			} finally {
		...
		if (!revisis.isEmpty()) {
			...
			for (Integer revisi : revisis) {
				if (revisi > 0) {
					...
					MyInclude iframe = new MyInclude("/pages/master/rab/workspace_revisi.zul?revisi=" + revisi
			...
			tabs.setHeight("0px");
			Tabpanel tabpanel = new ais.ui.util.MyTabpanel();
			MyInclude iframe = new MyInclude("/pages/master/rab/workspace_revisi.zul?revisi=" + 1 + "&satuanKerja="
					+ satuanKerja.getId() + "&sumberDana=" + (sumberDana == null || sumberDana.getId() == null ? -1L : sumberDana.getId())
					+ "&selectedTahun=" + selectedTahun + "&edit=" + edit + "&delete=" + delete + "&add=" + add);
			tabpanel.setParent(tabpanels);
			tabpanel.appendChild(iframe);
		}
