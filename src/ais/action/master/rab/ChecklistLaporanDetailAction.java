...
/**
 * Controller/action ZK untuk checklist laporan detail. Tipe ini merupakan titik masuk UI yang
 ...
 * {@code AmbilDataChecklistLaporanBanbox checklistLaporan}, {@code Textbox kode}, {@code Textbox nama}, {@code
 * Textbox keterangan}, {@code MyCheckboxConfig diperlukan}, {@code boolean edit}; inisialisasi/lifecycle ({@code
 * doBeforeCompose()}, {@code doAfterCompose()}, {@code initTree()}, {@code init()}); pembacaan/pencarian ({@code
 * checkKodeChecklistLaporanDetail()}, {@code onReloadTree()}, {@code reloadTreeitem()}, {@code
 * reloadTreeitem()}, {@code reloadTotal()}); mutasi data ({@code onSave()}); pelaporan/ekspor ({@code
 * onCetak()}); operasi domain lain ({@code onAdd()}, {@code openChilds()}, {@code closeChilds()}, {@code
