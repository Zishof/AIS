...
/**
 * Tipe khusus untuk satuan kerja tree model. Kelas ini memberi nama dan batas tanggung jawab yang
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code Boolean tampilkanSemua}, {@code Yayasan
 * yayasan}, {@code String satuanKerjaPengguna}, {@code String kodeSatuanKerjas}; inisialisasi/lifecycle ({@code
 * initKodeSatuanKerja()}); pembacaan/pencarian ({@code ambilYayasan()}, {@code getChildren()}, {@code
 * getChildren()}, {@code getChildrenByIds()}, {@code getChildrenByIds()}, {@code getChild()});
 * penghapusan/pembatalan ({@code deleteChilds()}, {@code deleteChildsRecursive()}); operasi domain lain ({@code
 * closeSessionSafe()}, {@code applyBaseRestrictions()}, {@code generateAllChildren()}, {@code isLeaf()}, {@code
 * generateChildsByIds()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di
 * atas.</p>
