...
/**
 * Controller/action ZK untuk kegiatan temporary. Tipe ini merupakan titik masuk UI yang
 ...
 * doBeforeCompose()}, {@code doAfterCompose()}, {@code initTombolPembayaranKeranjang()}, {@code
 * setupKeranjangGateway()}, {@code setupKeranjangBankOnline()}, {@code setupKeranjangSmartlink()});
 * pembacaan/pencarian ({@code getSelectedKegiatanTemporary()}, {@code onSearchDefault()}); validasi/perhitungan
 * ({@code validateSelectedKegiatanTemporary()}); operasi domain lain ({@code htmlAman()}, {@code
 * formatRingkas()}, {@code perbaruiRingkasanKeranjangPembayaran()}). Bagian lain dari kontrak tetap mengikuti
