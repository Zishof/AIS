...
/**
 * Controller/action ZK untuk biodata dosen. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * String DATA}, {@code String DATA_DESC}, {@code TreeMap MAPPING_DATA}, {@code AktifitasTugasAkhirHelper
 * aktifitasTugasAkhirHelper}, {@code AktifitasSkripsiHelper aktifitasSkripsiHelper}, {@code
 * CommonOnSearchdefault commonOnSearchdefault}, {@code ManagingBiodataDosen managingBiodataDosen};
 * inisialisasi/lifecycle ({@code init()}); pembacaan/pencarian ({@code setCommonOnSearchdefault()}, {@code
 * getCommonOnSearchdefault()}, {@code reloadDosen()}, {@code reloadDosenBimbingan()}, {@code
 * reloadDosenPenguji()}, {@code reloadDosenKkn()}); validasi/perhitungan ({@code checkEmailDosen()}, {@code
 * checkBiodataDosen()}); operasi domain lain ({@code displayBiodataWindow()}). Bagian lain dari kontrak tetap
