...
import java.security.MessageDigest;import java.sql.*;import java.util.*;
...
public final class OjsImportPreflightService{
 ...
 private String readVersion(Connection c,Config cfg,Result r){if(!r.fieldsByTable.containsKey("versions"))return r.foundTables<100?"LEGACY":"UNKNOWN";String prefix=blank(cfg.schema)?"":quote(cfg.schema)+".";String sql="select major,minor,revision,build from "+prefix+quote("versions")+" order by date_installed desc";Statement st=null;ResultSet rs=null;try{st=c.createStatement();st.setQueryTimeout(Math.max(1,Math.min(300,cfg.queryTimeoutSeconds)));st.setMaxRows(1);rs=st.executeQuery(sql);if(rs.next())return rs.getInt(1)+"."+rs.getInt(2)+"."+rs.getInt(3)+"-"+rs.getInt(4);}catch(Exception ignored){}finally{try{if(rs!=null)rs.close();}catch(Exception ignored){}try{if(st!=null)st.close();}catch(Exception ignored){}}return "UNKNOWN";}
 private static String signature(Result r)throws Exception{StringBuilder b=new StringBuilder();for(Map.Entry<String,Integer>e:r.fieldsByTable.entrySet())b.append(e.getKey()).append(':').append(e.getValue()).append(';');MessageDigest d=MessageDigest.getInstance("SHA-256");byte[] h=d.digest(b.toString().getBytes("UTF-8"));StringBuilder o=new StringBuilder();for(byte x:h)o.append(String.format("%02x",x&255));return o.toString();}
 private static void validate(Config c){if(c==null||blank(c.jdbcUrl)||blank(c.user)||c.password==null)throw new IllegalArgumentException("Konfigurasi sumber OJS belum lengkap.");String u=c.jdbcUrl.toLowerCase();if(!(u.startsWith("jdbc:postgresql:")||u.startsWith("jdbc:mysql:")||u.startsWith("jdbc:mariadb:")))throw new IllegalArgumentException("Driver sumber OJS tidak diizinkan.");if(c.jdbcUrl.indexOf('\n')>=0||c.jdbcUrl.indexOf('\r')>=0)throw new IllegalArgumentException("JDBC URL tidak valid.");}
 private static String dialect(String u){String x=u.toLowerCase();return x.startsWith("jdbc:postgresql:")?"POSTGRESQL":x.startsWith("jdbc:mariadb:")?"MARIADB":"MYSQL";}private static String quote(String v){return '"'+v.replace("\"","\"\"")+'"';}private static boolean blank(String v){return v==null||v.trim().length()==0;}
}
