...
public final class JurnalNativeImportService {
    ...
    public RepoItem importDraft(Long collectionId,String xml,String idempotencyKey,Tbmuser actor){
        auth.requireCrud(actor,"integrations","create");if(xml==null||xml.length()==0||xml.length()>1048576)throw new IllegalArgumentException("Ukuran XML native tidak valid.");String key=token(idempotencyKey);
