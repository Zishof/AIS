...
public final class JurnalEmailDeliveryService {
    ...
    @SuppressWarnings("unchecked") public int releaseDueDigests(Date now,int limit,Tbmuser actor){
        if(actor==null)throw new SecurityException("Login diperlukan.");auth.requireCrud(actor,"emailNotifikasi","update");if(now==null||limit<1||limit>1000)throw new IllegalArgumentException("Batas scheduler digest tidak valid.");
