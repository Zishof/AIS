...
public final class JurnalDemoDataSelfTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            boolean demoProtected=false;try{service.removeSample(key,JurnalDemoDataService.DELETE_SAMPLE_CONFIRMATION,actor);}catch(IllegalArgumentException expected){demoProtected=true;}check(demoProtected,"Dataset demo non-sample dapat terhapus melalui cleanup sample.");
            String sampleKey="sample-selftest-50x100";JurnalDemoDataService.Result sample=service.generate(50,100,Long.valueOf(245L),JurnalDemoDataService.DEFAULT_AUTHOR,sampleKey,JurnalDemoDataService.SAMPLE_CONFIRMATION,actor);JurnalDemoDataService.RemoveResult removed=service.removeSample(sampleKey,JurnalDemoDataService.DELETE_SAMPLE_CONFIRMATION,actor);
            check(sample.journalsRequested==50&&sample.articlesPerJournal==100,"Batas minimal sample gagal.");check(removed.journalsRemoved==50&&removed.articlesRemoved==5000&&removed.contributorsRemoved==5000,"Cleanup sample tidak lengkap.");
            Number sampleLeft=(Number)s.createQuery("select count(*) from RepoItem where sourceClass=:s").setString("s","AIS_JOURNAL_DEMO:"+sampleKey).uniqueResult();check(sampleLeft.longValue()==0L,"Artikel sample masih tersisa.");
            System.out.println("JurnalDemoDataSelfTest OK journals=500 articles=50000 published="+published+" drafts="+drafts+" sample=50x100 cleanup=PASS author="+first.authorName+" firstMs="+first.elapsedMillis+" idempotentMs="+second.elapsedMillis);
