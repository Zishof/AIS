...
/**
 * Controller/action ZK untuk checklist penilaian umum grup oleh peserta. Tipe ini merupakan titik
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code MyWindow addWindow}, {@code List
 * pertanyaanFilter}, {@code MyCheckboxConfig tampilBelumDiisi}, {@code Paging paging}, {@code MyGrid grid},
 * {@code Mahasiswa mahasiswa}, {@code Tbmuser tbmuser}, {@code South southTutup}; inisialisasi/lifecycle ({@code
 * doBeforeCompose()}, {@code doAfterCompose()}, {@code init()}, {@code initData()}, {@code initData()});
 * pembacaan/pencarian ({@code onSearchDefault()}, {@code dataChecklist()}); mutasi data ({@code onSave()});
 * operasi domain lain ({@code onIsiAngketDosenSelesai()}, {@code createSingleScrollContainer()}, {@code
 * applyQuestionCardStyle()}, {@code createQuestionLabel()}, {@code styleKeteranganTextbox()}, {@code
 * styleRadioGroup()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di
 * atas.</p>
...
public class ChecklistPenilaianUmumGrupOlehPesertaAction extends GenericAutowireComposer {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link ChecklistPenilaianUmumGrupOlehPesertaAction}. Kelas ini
	 * menerjemahkan satu item data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik
	 * kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link ChecklistPenilaianUmumGrupOlehPesertaAction}
	 * dan dapat mengakses state kelas induk. Jangan menyimpan atau membagikannya lintas desktop/session.</p>
	 * <p>Kontrak yang tampak dari deklarasi ini meliputi state utama: {@code int index}; operasi lokal: {@code
	 ...
	 *
	 * @see ChecklistPenilaianUmumGrupOlehPesertaAction
	 */
