...
/**
 * Komponen dashboard khusus untuk dashboard statistik guru masuk. Kelas ini memilih variasi data
 ...
 * @see MyWindow
 */
public class DashboardStatistikGuruMasuk extends MyWindow {
	...
	public DashboardStatistikGuruMasuk() {
		super();

	}

	public DashboardStatistikGuruMasuk(int width, int height) throws Exception {
		super();
		reinit(width, height);
	}
	...
	public DashboardStatistikGuruMasuk(String title, String border, boolean closable) {
	...
	public void init() {
		...
		center = (Div) ais.ui.util.DasborResponsifHelper.isiTunggal(this,
				"Statistik Guru Masuk",
				"Jumlah guru baru per periode, beserta grafiknya.");
	...
	public class MyEventListener implements EventListener {
		...
		public MyEventListener(Long sekolahId) {
		...
		public void onEvent(Event arg0) throws Exception {
			...
			EventListener eventListener = (EventListener) Common
					.cetakDataCustomButton(Guru.class, new DataCriteriaWithColumn() {
						...
						public Object[] initCriteria(boolean order) {
							...
							try {
								...
								Criteria criteria = session.createCriteria(Guru.class)
								...
								return new Object[] { criteria, GuruAction.DATA };
	...
	public void initChart(Component center, boolean tampilkanChart) {
		...
		column = new MyColumnConfig("Jumlah Guru");
		column.setParent(columns);
		column.setWidth("20%");
		...
		String sql = "select b.id as sekolah_id, b.nama as sekolah,count(*) as jumlah_guru from sekolah.guru a\r\n"
				+ "	inner join sekolah.sekolah b on (a.sekolah_id=b.id) where a.aktif " + sqlSekolah + " " + sqlYayasan
		...
		for (Object[] objects : jurusans) {
			...
			Integer jumlahGuru = ((Number) (objects[2] == null ? -1 : objects[2])).intValue();
		...
		row.setValign("top");
			...
			A a = new A(Common.numberFormat.get().format(jumlahGuru));
			a.addEventListener("onClick", new MyEventListener(sekolahId));

			row.appendChild(a);

			model.setValue(sekolah, "", jumlahGuru);
		}
		...
		a.addEventListener("onClick", new MyEventListener(-1L));
		row.appendChild(a);
		...
		if (tampilkanChart) {
			...
			mychart = null;
        mychart = DashboardModernHtmlUtil.createAnyChart(model, "Dasbor Statistik Guru Masuk", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", "pie");
