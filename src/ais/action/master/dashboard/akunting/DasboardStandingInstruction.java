...
/**
 * Komponen dashboard khusus untuk dasboard standing instruction. Kelas ini memilih variasi data
 ...
 * inisialisasi/lifecycle ({@code init()}); pembacaan/pencarian ({@code reload()}); mutasi data ({@code
 * standingInstructionBelumDiprosesCriterion()}); operasi domain lain ({@code transferViaSqlText()}, {@code
 * standingInstructionDiajukanCriterion()}, {@code standingInstructionTransferCriterion()}). Bagian lain dari
 ...
 * @see MyWindow
 */
public class DasboardStandingInstruction extends MyWindow {
	...
	public DasboardStandingInstruction() {
		super();
		try {
	...
	public DasboardStandingInstruction(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DasboardStandingInstruction.this.grid);
			}
		});
	}
	...
	private void reload() {
		...
		treeMap.put(1, "Belum Diproses");
		treeMap.put(2, "Diajukan");
		treeMap.put(3, "Transfer");

		final TreeMap<Integer, Criterion> treeMapCriterion = new TreeMap<Integer, Criterion>();

		Criterion criterion = standingInstructionBelumDiprosesCriterion();
		treeMapCriterion.put(1, criterion);
		criterion = standingInstructionDiajukanCriterion();
		treeMapCriterion.put(2, criterion);
		criterion = standingInstructionTransferCriterion();
										...
										public Object[] initCriteria(boolean order) {
											...
											try {
												...
												return new Object[] { criteria,
														StandingInstructionAction.contents };
		...
		row.appendChild(DashboardModernHtmlUtil.createAnyChart(categoryModel, "Dasbor Standing Instruction", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
}
}
