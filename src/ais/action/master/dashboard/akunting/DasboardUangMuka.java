...
/**
 * Komponen dashboard khusus untuk dasboard uang muka. Kelas ini memilih variasi data atau tampilan
 ...
 * @see MyWindow
 */
public class DasboardUangMuka extends MyWindow {
	...
	public DasboardUangMuka(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DasboardUangMuka.this.grid);
			}
		});
	}
	...
	private void reload() {
		...
		Criterion criterion = Restrictions.and(Restrictions.isNull("pertangungjawaban"), Restrictions.and(
				Restrictions.isNull("prosesTransfer.realisasikanOleh"), Restrictions.eq("status", UangMuka.PENGAJUAN)));
		treeMapCriterion.put(1, criterion);
		criterion = Restrictions.and(Restrictions.isNull("pertangungjawaban"), Restrictions.and(
				Restrictions.isNull("prosesTransfer.realisasikanOleh"), Restrictions.eq("status", UangMuka.DISETUJU)));
		treeMapCriterion.put(2, criterion);
		criterion = Restrictions.and(Restrictions.isNull("pertangungjawaban"), Restrictions.and(
				Restrictions.isNull("prosesTransfer.realisasikanOleh"), Restrictions.eq("status", UangMuka.DITOLAK)));
		treeMapCriterion.put(3, criterion);
		criterion = Restrictions.and(Restrictions.isNull("pertangungjawaban"),
		...
		criterion = Restrictions.and(Restrictions.isNotNull("prosesTransfer.realisasikanOleh"),
				Restrictions.isNotNull("pertangungjawaban"));
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			for (Integer status : treeMap.keySet()) {
				...
				Criteria criteria = HibernateUtil.currentSession().createCriteria(UangMuka.class)
					...
					a.addEventListener("onClick", new EventListener() {
						...
						public void onEvent(Event arg0) throws Exception {
							EventListener eventListener = (EventListener) Common
									.cetakDataCustomButton(UangMuka.class, new DataCriteriaWithColumn() {
										...
										public Object[] initCriteria(boolean order) {
											...
											try {
												...
												return new Object[] { criteria, UangMukaAction.contents };
		...
		row.appendChild(DashboardModernHtmlUtil.createAnyChart(categoryModel, "Dasbor Uang Muka", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
}
}
