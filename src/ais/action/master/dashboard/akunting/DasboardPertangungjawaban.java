...
/**
 * Komponen dashboard khusus untuk dasboard pertangungjawaban. Kelas ini memilih variasi data atau
 ...
 * @see MyWindow
 */
public class DasboardPertangungjawaban extends MyWindow {
	...
	public DasboardPertangungjawaban() {
		super();
		try {
	...
	public DasboardPertangungjawaban(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DasboardPertangungjawaban.this.grid);
			}
		});
	}
	...
	private void reload() {
		...
		Criterion criterion = Restrictions.and(Restrictions.isNull("prosesTransfer.realisasikanOleh"),
				Restrictions.eq("status", Pertangungjawaban.PENGAJUAN));
		treeMapCriterion.put(1, criterion);
		criterion = Restrictions.and(Restrictions.isNull("prosesTransfer.realisasikanOleh"),
				Restrictions.eq("status", Pertangungjawaban.DISETUJU));
		treeMapCriterion.put(2, criterion);
		criterion = Restrictions.and(Restrictions.isNull("prosesTransfer.realisasikanOleh"),
				Restrictions.eq("status", Pertangungjawaban.DITOLAK));
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			for (Integer status : treeMap.keySet()) {
				...
				Criteria criteria = HibernateUtil.currentSession().createCriteria(Pertangungjawaban.class)
					...
					a.addEventListener("onClick", new EventListener() {
						...
						public void onEvent(Event arg0) throws Exception {
							EventListener eventListener = (EventListener) Common
									.cetakDataCustomButton(Pertangungjawaban.class, new DataCriteriaWithColumn() {
										...
										public Object[] initCriteria(boolean order) {
											...
											try {
												...
												Criteria criteria = HibernateUtil.currentSession()
														.createCriteria(Pertangungjawaban.class)
												...
												return new Object[] { criteria, PertangungjawabanAction.contents };
		...
		row.appendChild(DashboardModernHtmlUtil.createAnyChart(categoryModel, "Dasbor Pertangungjawaban", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
}
}
