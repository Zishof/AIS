...
/**
 * Komponen dashboard khusus untuk dashboard status pekerjaan setelah lulus mahasiswa. Kelas ini
 ...
 * @see MyWindow
 */
public class DashboardStatusPekerjaanSetelahLulusMahasiswa extends MyWindow {
	...
	public DashboardStatusPekerjaanSetelahLulusMahasiswa() {
		super();
		try {
	...
	public DashboardStatusPekerjaanSetelahLulusMahasiswa(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		DashboardGridExportHelper.pasang(this, "Status Pekerjaan Setelah Lulus Mahasiswa");
		setHeight("100%");
		setWidth("100%");
		setBorder("none");
		setClosable(false);
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Sebaran status pekerjaan alumni setelah lulus, beserta grafiknya.");
	...
	private void reload() {
		...
		final List<StatusPekerjaanSetelahLulus> statusPekerjaanSetelahLuluss = HibernateUtil.currentSession()
				.createCriteria(Mahasiswa.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))).setProjection(Projections.groupProperty("statusPekerjaanSetelahLulus"))
				...
				.addOrder(Order.asc("statusPekerjaanSetelahLulus"))
				.add(Restrictions.isNotNull("statusPekerjaanSetelahLulus"))
				.add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))).list();
		...
		for (StatusPekerjaanSetelahLulus sInit : statusPekerjaanSetelahLuluss) {
			if (sInit != null) { try { sInit.getNama(); } catch (Exception eInit) { ais.common.ErrorAuditUtil.record(eInit, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardStatusPekerjaanSetelahLulusMahasiswa.java:251");} }
		}
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				Map<StatusPekerjaanSetelahLulus, SimpleCategoryModel> mapsChart = new HashMap<StatusPekerjaanSetelahLulus, SimpleCategoryModel>();
				for (StatusPekerjaanSetelahLulus statusPekerjaanSetelahLulus : statusPekerjaanSetelahLuluss) {
					auxheader = new Auxheader(statusPekerjaanSetelahLulus.getNama());
					...
					mapsChart.put(statusPekerjaanSetelahLulus, categoryModelTelahAmbil);
				}
				...
				StatusPekerjaanSetelahLulus statusPekerjaanSetelahLulus : statusPekerjaanSetelahLuluss) {
		...
		row.setValign("top");
					...
					for (final StatusPekerjaanSetelahLulus statusPekerjaanSetelahLulus : statusPekerjaanSetelahLuluss) {
						...
						for (int tahun = mul; tahun <= sam; tahun++) {
							...
							for (Object[] o : dataJumlahMhs) {
								...
								StatusPekerjaanSetelahLulus statusPekerjaanSetelahLulusData = (StatusPekerjaanSetelahLulus) o[2];
								if (statusPekerjaanSetelahLulusData != null
										&& statusPekerjaanSetelahLulusData.getId()
												.equals(statusPekerjaanSetelahLulus.getId())
						...
						for (final Integer tahun : data.keySet()) {
								...
								public void onEvent(Event arg0) throws Exception {
									...
									Common.displayWindow(
											"/common/dashboard/mahasiswa.zul?jurusan=" + jurusan.getId()
													...
													+ "&statusPekerjaanSetelahLulus="
													+ statusPekerjaanSetelahLulus.getId() + "&tahunLulus=" + tahun
													+ (program == null ? ""
							...
							mapsChart.get(statusPekerjaanSetelahLulus).setValue(jurusan.getNama(), tahun, d);
		...
		row.setValign("top");
					row.setParent(rows);
					row.setSpans((((sam - mul) * statusPekerjaanSetelahLuluss.size()) + 2) + "");
					row.setAlign("center");

					row.appendChild(DashboardModernHtmlUtil.createAnyChart(mapsChart.get(statusPekerjaanSetelahLulus), String.valueOf(statusPekerjaanSetelahLulus.getNama()), "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
				}
			}
		});
				...
				for (final Jurusan jurusan : jurusans) {
					...
					List<Object[]> dataJumlahMhs = session.createCriteria(Mahasiswa.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
							...
							.setProjection(Projections.projectionList().add(Projections.rowCount())
									...
									.add(Projections.groupProperty("statusPekerjaanSetelahLulus")))
