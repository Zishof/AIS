...
/**
 * Komponen dashboard khusus untuk dashboard masa studi mahasiswa. Kelas ini memilih variasi data
 ...
 * @see MyWindow
 */
public class DashboardMasaStudiMahasiswa extends MyWindow {
	...
	public DashboardMasaStudiMahasiswa() {
		super();
		try {
	...
	public DashboardMasaStudiMahasiswa(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		DashboardGridExportHelper.pasang(this, "Masa Studi Mahasiswa");
		setHeight("100%");
		setWidth("100%");
		setBorder("none");
		setClosable(false);
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Sebaran lama masa studi mahasiswa, beserta grafiknya.");
	...
	private void reload() {
		...
		final List<Integer> masaPerkuliahans = HibernateUtil.currentSession().createCriteria(Mahasiswa.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
				.setProjection(
						...
						Projections.sqlGroupProjection("(tahunlulus-tahunangkatan) as masaPerkuliahan",
								"(tahunlulus-tahunangkatan)", new String[] { "masaPerkuliahan" },
								new Type[] { org.hibernate.type.StandardBasicTypes.INTEGER })
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				for (Integer masa : masaPerkuliahans) {
					auxheader = new Auxheader((masa - 1) + " s.d " + masa + " thn");
		...
		row.setValign("top");
					...
					row.appendChild(DashboardModernHtmlUtil.createAnyChart(mapsChart.get(masa), String.valueOf((masa - 1) + " s.d " + masa + " thn"), "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
				}
			}
		});
							...
							.setProjection(Projections.projectionList().add(Projections.rowCount())
									.add(Projections.groupProperty("tahunLulus")).add(
											...
											Projections.sqlGroupProjection(
													"(tahunlulus-tahunangkatan) as masaPerkuliahan",
													"(tahunlulus-tahunangkatan)", new String[] { "masaPerkuliahan" },
													new Type[] { org.hibernate.type.StandardBasicTypes.INTEGER })
