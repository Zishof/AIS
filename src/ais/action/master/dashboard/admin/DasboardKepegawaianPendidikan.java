...
/**
 * Komponen dashboard khusus untuk dasboard kepegawaian pendidikan. Kelas ini memilih variasi data
 ...
 * @see MyWindow
 */
public class DasboardKepegawaianPendidikan extends MyWindow {
	...
	public DasboardKepegawaianPendidikan() {
		super();
		try {
	...
	public DasboardKepegawaianPendidikan(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Sebaran Pegawai per Pendidikan",
				"Jumlah pegawai menurut jenjang pendidikan, beserta grafiknya.");
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DasboardKepegawaianPendidikan.this.grid);
			}
		});
	}
	...
	private void reload() {
		...
		List<Pendidikan> pendidikans = ConstantValues.simpleList(
				HibernateUtil.currentSession().createCriteria(Pendidikan.class).addOrder(Order.asc("nama")),
				Pendidikan.class);

		pendidikans.add(null);
		Map<Long, MyColumnConfig> listCols = new HashMap<Long, MyColumnConfig>();
		for (Pendidikan pendidikan : pendidikans) {
			column = new MyColumnConfig(pendidikan == null ? "Tidak ditentukan" : pendidikan.getNama());
			column.setWidth("5%");
			column.setAlign("right");
			listCols.put(pendidikan == null || pendidikan.getId() == null ? -1L : pendidikan.getId(), column);
		}
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			for (Pendidikan pendidikan : pendidikans) {
				Integer count = ((Number) HibernateUtil.currentSession().createCriteria(Pegawai.class)
						...
						.add(pendidikan == null ? Restrictions.isNull("pendidikan")
								: Restrictions.eq("pendidikan", pendidikan))
				...
				data.add(count);
				Integer colCount = listTotals.get(pendidikan == null || pendidikan.getId() == null ? -1L : pendidikan.getId());
				if (colCount == null) {
					colCount = 0;
				}
				colCount += count;
				listTotals.put(pendidikan == null || pendidikan.getId() == null ? -1L : pendidikan.getId(), colCount);
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			int jml = 0;
			int i = 0;
			for (final Pendidikan pendidikan : pendidikans) {
				Integer colCount = listTotals.get(pendidikan == null || pendidikan.getId() == null ? -1L : pendidikan.getId());
				if (colCount == null) {
					colCount = 0;
				}
				if (colCount > 0) {
					...
					if (count > 0) {
						categoryModel.setValue(satuanKerja == null ? "Tidak Ditentukan" : satuanKerja.getNama(),
								pendidikan == null ? "Tidak ditentukan" : pendidikan.getNama(), count);
					}
					A a = new A(count + "");
		...
		for (final Pendidikan pendidikan : pendidikans) {
			...
			if (colCount > 0) {
				countCol++;
				listCols.get(pendidikan == null || pendidikan.getId() == null ? -1L : pendidikan.getId()).setParent(columns);
		...
		row.appendChild(DashboardModernHtmlUtil.createAnyChart(categoryModel, "Dasbor Kepegawaian Pendidikan", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
}
}
