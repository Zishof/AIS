...
/**
 * Komponen dashboard khusus untuk dashboard predikat kelulusan mahasiswa. Kelas ini memilih
 ...
 * @see MyWindow
 */
public class DashboardPredikatKelulusanMahasiswa extends MyWindow {
	...
	public DashboardPredikatKelulusanMahasiswa() {
		super();
		try {
	...
	public DashboardPredikatKelulusanMahasiswa(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		DashboardGridExportHelper.pasang(this, "Predikat Kelulusan Mahasiswa");
		setHeight("100%");
		setWidth("100%");
		setBorder("none");
		setClosable(false);
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Predikat Kelulusan",
				"Sebaran predikat kelulusan mahasiswa, beserta grafiknya.");
	...
	private void reload() {
		...
		final List<Judisium> judisiums = HibernateUtil.currentSession().createCriteria(Mahasiswa.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
				.setProjection(Projections.groupProperty("predikatKelulusan"))
				...
				.addOrder(Order.asc("predikatKelulusan")).add(Restrictions.isNotNull("predikatKelulusan"))
				.add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))).list();
		...
		for (Judisium sInit : judisiums) {
			if (sInit != null) { try { sInit.getNama(); } catch (Exception eInit) { ais.common.ErrorAuditUtil.record(eInit, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardPredikatKelulusanMahasiswa.java:251");} }
		}
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				Map<Judisium, SimpleCategoryModel> mapsChart = new HashMap<Judisium, SimpleCategoryModel>();
				for (Judisium judisium : judisiums) {
					GeneralValueObject.check(judisium);
					auxheader = new Auxheader(judisium.getNama());
					...
					mapsChart.put(judisium, categoryModelTelahAmbil);
				}
		...
		row.setValign("top");
					...
					for (final Judisium judisium : judisiums) {
						...
						for (int tahun = mul; tahun <= sam; tahun++) {
							...
							for (Object[] o : dataJumlahMhs) {
								...
								Judisium judisiumData = (Judisium) o[2];
								if (judisiumData != null && judisiumData.getId().equals(judisium.getId())
						...
						for (final Integer tahun : data.keySet()) {
								...
								public void onEvent(Event arg0) throws Exception {
									...
									Common.displayWindow(
											"/common/dashboard/mahasiswa.zul?jurusan=" + jurusan.getId()
													...
													+ "&predikatKelulusan=" + judisium.getId() + "&tahunLulus=" + tahun
													+ (program == null ? ""
							...
							mapsChart.get(judisium).setValue(jurusan.getNama(), tahun, d);
		...
		row.setValign("top");
					row.setParent(rows);
					row.setSpans((((sam - mul) * judisiums.size()) + 2) + "");
					row.setAlign("center");

					row.appendChild(DashboardModernHtmlUtil.createAnyChart(mapsChart.get(judisium), String.valueOf(judisium.getNama()), "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
				}
			}
		});
				...
				for (final Jurusan jurusan : jurusans) {
					...
					List<Object[]> dataJumlahMhs = session.createCriteria(Mahasiswa.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
							...
							.setProjection(Projections.projectionList().add(Projections.rowCount())
									...
									.add(Projections.groupProperty("predikatKelulusan")))
