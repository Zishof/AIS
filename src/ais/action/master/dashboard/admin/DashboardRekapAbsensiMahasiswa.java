...
/**
 * Komponen dashboard khusus untuk dashboard rekap absensi mahasiswa. Kelas ini memilih variasi
 ...
 * pembacaan/pencarian ({@code tampilkanPopupPreviewExcel()}, {@code tampilkanLoading()}, {@code
 * getSelectedLimit()}); validasi/perhitungan ({@code hitungSummary()}); pelaporan/ekspor ({@code
 * renderRekapGrid()}); operasi domain lain ({@code generateWhere()}, {@code generateWhere()}, {@code sqlText()},
 * {@code sqlLike()}, {@code buildDashboardHtml()}, {@code cardHtml()}). Bagian lain dari kontrak tetap mengikuti
