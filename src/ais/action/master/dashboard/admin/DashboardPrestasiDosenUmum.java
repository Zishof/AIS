...
/**
 * Komponen dashboard khusus untuk dashboard prestasi dosen umum. Kelas ini memilih variasi data
 ...
 * @see MyWindow
 */
public class DashboardPrestasiDosenUmum extends MyWindow {
	...
	public DashboardPrestasiDosenUmum() {
		super();
		try {
	...
	public DashboardPrestasiDosenUmum(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		DashboardGridExportHelper.pasang(this, "Prestasi Dosen Umum");
		setHeight("100%");
		setWidth("100%");
		setBorder("none");
		setClosable(false);
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Prestasi Dosen",
				"Rekap prestasi dosen, beserta grafiknya.");
	...
	private void reload() {
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				for (Object[] d : datas) {
					...
					final CabangPrestasiDosen cabangPrestasiDosen = (CabangPrestasiDosen) d[2];
					final KategoriPrestasiDosen kategoriPrestasiDosen = (KategoriPrestasiDosen) d[3];

					GeneralValueObject.check(fakultas);
					GeneralValueObject.check(jurusan);
					GeneralValueObject.check(cabangPrestasiDosen);
					GeneralValueObject.check(kategoriPrestasiDosen);
		...
		row.setValign("top");
					...
					row.appendChild(
							new MyLabelBoldAja(cabangPrestasiDosen == null ? "" : cabangPrestasiDosen.getNama()));
					row.appendChild(
							new MyLabelBoldAja(kategoriPrestasiDosen == null ? "" : kategoriPrestasiDosen.getNama()));

					String nama = fakultas.getNama() + "-" + jurusan.getNama();
					String jabatan = (cabangPrestasiDosen == null ? "" : cabangPrestasiDosen.getNama()) + "-"
							+ (kategoriPrestasiDosen == null ? "" : kategoriPrestasiDosen.getNama());
						...
						a.addEventListener("onClick", new EventListener() {
							...
							public void onEvent(Event arg0) throws Exception {
								...
								Common.displayWindow(
										"/common/dashboard/prestasi_dosen.zul?jurusan=" + jurusan.getId()
												+ "&cabangPrestasiDosen=" + cabangPrestasiDosen.getId()
												+ "&kategoriPrestasiDosen=" + kategoriPrestasiDosen.getId()
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				List<Object[]> data = session.createCriteria(PrestasiDosen.class)
						...
						.setProjection(Projections.projectionList().add(Projections.groupProperty("dosen.fakultas"))
								...
								.add(Projections.groupProperty("cabangPrestasiDosen"))
								.add(Projections.groupProperty("kategoriPrestasiDosen"))
				...
				for (Object[] d : data) {
					...
					CabangPrestasiDosen cabangPrestasiDosen = (CabangPrestasiDosen) d[2];
					KategoriPrestasiDosen kategoriPrestasiDosen = (KategoriPrestasiDosen) d[3];
					
					fakultas = GeneralValueObject.check(fakultas);
					jurusan = GeneralValueObject.check(jurusan);
					cabangPrestasiDosen = GeneralValueObject.check(cabangPrestasiDosen);
					kategoriPrestasiDosen = GeneralValueObject.check(kategoriPrestasiDosen);
					

					String kodeUnik = fakultas.getId() + "-" + jurusan.getId() + "-" + kategoriPrestasiDosen.getId()
							+ "-" + (cabangPrestasiDosen == null ? "" : cabangPrestasiDosen.getId());

					if (!kodes.contains(kodeUnik)) {
						nilais = new TreeMap<String, Number>();
						datas.add(
								new Object[] { fakultas, jurusan, cabangPrestasiDosen, kategoriPrestasiDosen, nilais });
						kodes.add(kodeUnik);
					}
