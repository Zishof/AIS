...
/**
 * Komponen dashboard khusus untuk dashboard rekap absensi per mahasiswa. Kelas ini memilih variasi
 ...
 * searchsemester}, {@code Combobox searchprogram}, {@code Combobox angkatanMhs}, {@code AmbilDataDosenBanbox
 ...
 * @see MyWindow
 */
public class DashboardRekapAbsensiPerMahasiswa extends MyWindow {
	...
	private Perkuliahan selectedperkuliahan;

	private Combobox comboTampilkan;

	public DashboardRekapAbsensiPerMahasiswa() {
		super();
		try {
			init();
			initFakultas();
	...
	public DashboardRekapAbsensiPerMahasiswa(Mahasiswa mahasiswa) {
		super();
	...
	public DashboardRekapAbsensiPerMahasiswa(Perkuliahan perkuliahan) {
		super();
	...
	public DashboardRekapAbsensiPerMahasiswa(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		print.addEventListener("onClick", new EventListener() {
			@Override
			public void onEvent(Event event) throws Exception {
				// FIX NullPointerException: tombol "Download" bisa diklik SEBELUM "Proses" (yang mengisi
				// `spreadsheet` via initSpreadsheet()) -- spreadsheet masih null saat itu. Beri tahu user
				// dengan pesan jelas, bukan crash mentah.
				if (spreadsheet == null || spreadsheet.getBook() == null) {
					ais.ui.util.MyMessageboxConfig.show(
							"Belum ada data untuk diunduh. Klik tombol \"Proses\" terlebih dahulu untuk menyiapkan rekap data.",
	...
	private void initSpreadsheet() {
		...
		Common.createDefaultTimer(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				Common.clear(center);
				String tahunAkademik = (String) (DashboardRekapAbsensiPerMahasiswa.this.tahunAkademik
						.getSelectedItem() == null ? null
								: DashboardRekapAbsensiPerMahasiswa.this.tahunAkademik.getSelectedItem().getValue());
				...
				String sql = "select\n(a.nim||' '||a.nama) as mahasiswa,"
						...
						+ (selectedmahasiswa == null ? "" : " and a.id = " + selectedmahasiswa.getId())
						+ " \ngroup by d.id,a.id\norder by (a.nim||' '||a.nama)";

				sql = "select max(mahasiswa) mahasiswa, \n"
						+ "sum(hadir) hadir, sum(alpa) alpa, sum(sakit) sakit, sum(izin) izin, kode_perkuliahan\n"
						+ "from (" + sql
						+ ") aaa group by kode_perkuliahan  \n having sum(hadir+alpa+sakit+izin)>0 limit "
						+ comboTampilkan.getSelectedItem().getValue();
				...
				ais.ui.util.EcampusUtil.setCellValue(sheet, 1, 0, selectedperkuliahan != null
						...
						: "REKAPITULASI ABSENSI PER MAHASISWA \n " + Common.getBahasaConfig("Fakultas") + " "
				...
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 5, Common.getBahasaConfig("%Hadir"));
				Utils.setColumnWidth(sheet, 5, 80);
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 6, Common.getBahasaConfig("%Hadir dr Alpa"));
				Utils.setColumnWidth(sheet, 6, 80);
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 7, Common.getBahasaConfig("Jml Pertemuan"));
				Utils.setColumnWidth(sheet, 7, 80);
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 8, "Total tidak hadir");
				...
				for (Object[] objects : jurusans) {
					...
					Integer totalP = nilai0 + nilai1 + nilai2 + nilai3;
					...
					Double persenHadir = (nilai0 * 100.0) / (nilai0 + nilai1 + nilai2 + nilai3);

					Double persenHadirDrAlpa = ((nilai0 + nilai2 + nilai3) * 100.0)
							/ (nilai0 + nilai1 + nilai2 + nilai3);
					...
					ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 7, totalP);
					...
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapAbsensiPerMahasiswa.java:498");
					}
				...
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 5, "");
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 6, "");
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 7, totalPSemua);
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 8, totalSemua);
				...
				try {
					ais.ui.util.EcampusUtil.setBold(sheet,
							...
							true);
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapAbsensiPerMahasiswa.java:524");
				}
			}
		});
