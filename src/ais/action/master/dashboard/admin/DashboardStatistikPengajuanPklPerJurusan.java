...
/**
 * Komponen dashboard khusus untuk dashboard statistik pengajuan pkl per jurusan. Kelas ini memilih
 ...
 * @see MyWindow
 */
public class DashboardStatistikPengajuanPklPerJurusan extends MyWindow {
	...
	public static String[] contents = new String[] { "mahasiswa.nim", "mahasiswa.nama", "mahasiswa.jurusan.nama",
			"kelompokPkl.nama_kelompok", "kelompokPkl.alamat", "kelompokPkl.tanggal_mulai",
			"kelompokPkl.tanggal_selesai", "hasil", "keterangan", "kelompokPkl.dosen_pembimbing1.nama",
			"kelompokPkl.dosen_pembimbing2.nama", "kelompokPkl.dosen_pembimbing3.nama",
			"kelompokPkl.dosen_pembimbing4.nama", "kelompokPkl.dosen_pembimbing5.nama", "totalNilai", "nilaiHuruf",
			"totalIP", "diterima" };

	public DashboardStatistikPengajuanPklPerJurusan() {
		super();
		initFakultas();
		init();
	...
	public DashboardStatistikPengajuanPklPerJurusan(int width, int height) throws Exception {
		super();
		tampilRinci = true;
		reinit(width, height);
	}
	...
	public DashboardStatistikPengajuanPklPerJurusan(String title, String border, boolean closable) {
	...
	/**
	 * Event listener lokal milik {@link DashboardStatistikPengajuanPklPerJurusan}. Kelas ini menangani event untuk
	 * komponen induk dan meneruskan pekerjaan domain ke method/service yang sudah tersedia.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link DashboardStatistikPengajuanPklPerJurusan} dan
	 * dapat mengakses state kelas induk. Jangan menyimpan atau membagikannya lintas desktop/session.</p>
	 * <p>Kontrak yang tampak dari deklarasi ini meliputi state utama: {@code Boolean diterima}, {@code Long
	 ...
	 *
	 * @see DashboardStatistikPengajuanPklPerJurusan
	 */
	public class MyEventListener implements EventListener {
		...
		public void onEvent(Event arg0) throws Exception {
			...
			EventListener eventListener = (EventListener) Common
					.cetakDataCustomButton(MahasiswaDapatKelompokPkl.class, new DataCriteriaWithColumn() {
						...
						public Object[] initCriteria(boolean order) {
							...
							try {
								...
								Criteria criteria = session.createCriteria(MahasiswaDapatKelompokPkl.class)
										...
										.createAlias("kelompokPkl", "kelompokPkl").createAlias("kelompokPkl.pkl", "pkl")
										...
										.add(Restrictions.eq("pkl.semester",
	...
	private void init() {
		...
		Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Statistik Pengajuan PKL per Program Studi",
				"Jumlah pengajuan PKL yang sudah dan belum disetujui di tiap program studi, beserta grafiknya.");
		...
		EventListener taEventListener = new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				List<Pkl> pkls = HibernateUtil.currentSession().createCriteria(Pkl.class)
				...
				Common.insertComboDanSemua(searchjenis, new String[] { "nama" }, "nama", KelompokPkl.class,
						"=Nama Kelompok=",
						pkls.isEmpty() ? Restrictions.sqlRestriction("false") : Restrictions.in("pkl", pkls));
			}
		};
		...
		try {
			...
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/dashboard/admin/DashboardStatistikPengajuanPklPerJurusan.java:317");
		}
		...
		if (tampilRinci) {
			...
			myButtonConfig.addEventListener("onClick", new EventListener() {
				...
				@Override
				public void onEvent(Event arg0) throws Exception {
					DashboardPklMahasiswa laporan = new DashboardPklMahasiswa();
					laporan.setHeight("99%");
					laporan.setWidth("99%");
					laporan.setTitle("Rekap PKL");
			...
			myButtonConfig.addEventListener("onClick", new EventListener() {
				...
				public void onEvent(Event arg0) throws Exception {
					...
					center.appendChild(new Iframe("/pages/master/pkl/kelompok_pkl.zul"));
					ExecutionsCtrl.getCurrentCtrl().getCurrentPage().getFirstRoot().appendChild(laporan);
					laporan.onModal();
				}
			});
	...
	private void initChart() {
		...
		KelompokPkl kelompokPkl = (KelompokPkl) (searchjenis.getSelectedItem() == null ? null
		...
		String sql = "select sum(case when a1.diterima then 1 else 0 end) as telah_sidang,  "
				...
				+ " from mahasiswa a inner join mahasiswa_dapat_kelompok_kelompok_pkl a1 on (a.id = a1.mahasiswa)  "
				+ " inner join jurusan b on (a.jurusan = b.id  )  left join fakultas c on (c.id = b.fakultas) "
				+ " inner join kelompok_pkl d on (a1.kelompok_pkl = d.id  ) " + " inner join pkl e on (d.pkl = e.id  ) "
				...
				+ (kelompokPkl == null ? "" : " and a1.kelompok_pkl=" + kelompokPkl.getId())
		...
		mychart = null;
        mychart = DashboardModernHtmlUtil.createAnyChart(model, "Dasbor Statistik Pengajuan Pkl Per Jurusan", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", "pie");
