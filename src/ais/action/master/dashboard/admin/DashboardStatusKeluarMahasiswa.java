...
/**
 * Komponen dashboard khusus untuk dashboard status keluar mahasiswa. Kelas ini memilih variasi
 ...
 * @see MyWindow
 */
public class DashboardStatusKeluarMahasiswa extends MyWindow {
	...
	public DashboardStatusKeluarMahasiswa() {
		super();
		try {
	...
	public DashboardStatusKeluarMahasiswa(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		DashboardGridExportHelper.pasang(this, "Status Keluar Mahasiswa");
		setHeight("100%");
		setWidth("100%");
		setBorder("none");
		setClosable(false);
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Status Keluar Mahasiswa",
				"Sebaran status keluar mahasiswa, beserta grafiknya.");
	...
	private void reload() {
		...
		final List<StatusKeluar> statusKeluars = HibernateUtil.currentSession().createCriteria(Mahasiswa.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
				.setProjection(Projections.groupProperty("statusKeluar"))
				...
				.addOrder(Order.asc("statusKeluar")).add(Restrictions.isNotNull("statusKeluar"))
				.add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))).list();
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				Map<StatusKeluar, SimpleCategoryModel> mapsChart = new HashMap<StatusKeluar, SimpleCategoryModel>();
				for (StatusKeluar statusKeluar : statusKeluars) {
					auxheader = new Auxheader(statusKeluar.getNama());
					...
					mapsChart.put(statusKeluar, categoryModelTelahAmbil);
				}
				...
				StatusKeluar statusKeluar : statusKeluars) {
		...
		row.setValign("top");
					...
					for (final StatusKeluar statusKeluar : statusKeluars) {
						...
						for (int tahun = mul; tahun <= sam; tahun++) {
							...
							for (Object[] o : dataJumlahMhs) {
								...
								StatusKeluar statusKeluarData = (StatusKeluar) o[2];
								if (statusKeluarData != null && statusKeluarData.getId().equals(statusKeluar.getId())
						...
						for (final Integer tahun : data.keySet()) {
								...
								public void onEvent(Event arg0) throws Exception {
									...
									Common.displayWindow(
											"/common/dashboard/mahasiswa.zul?jurusan=" + jurusan.getId()
													...
													+ "&statusKeluar=" + statusKeluar.getId() + "&tahunLulus=" + tahun
													+ (program == null ? ""
							...
							mapsChart.get(statusKeluar).setValue(jurusan.getNama(), tahun, d);
		...
		row.setValign("top");
					row.setParent(rows);
					row.setSpans((((sam - mul) * statusKeluars.size()) + 2) + "");
					row.setAlign("center");

					row.appendChild(DashboardModernHtmlUtil.createAnyChart(mapsChart.get(statusKeluar), String.valueOf(statusKeluar.getNama()), "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
				}
			}
		});
				...
				for (final Jurusan jurusan : jurusans) {
					...
					List<Object[]> dataJumlahMhs = session.createCriteria(Mahasiswa.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
							...
							.setProjection(Projections.projectionList().add(Projections.rowCount())
									...
									.add(Projections.groupProperty("statusKeluar")))
