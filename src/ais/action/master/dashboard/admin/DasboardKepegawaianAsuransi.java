...
/**
 * Komponen dashboard khusus untuk dasboard kepegawaian asuransi. Kelas ini memilih variasi data
 ...
 * @see MyWindow
 */
public class DasboardKepegawaianAsuransi extends MyWindow {
	...
	public DasboardKepegawaianAsuransi() {
		super();
		try {
	...
	public DasboardKepegawaianAsuransi(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Sebaran Pegawai per Jenis Asuransi",
				"Jumlah pegawai menurut jenis asuransi, beserta grafiknya.");
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DasboardKepegawaianAsuransi.this.grid);
			}
		});
	}
	...
	private void reload() {
		...
		List<AsuransiPegawai> asuransiPegawais = ConstantValues
				.simpleList(
						HibernateUtil.currentSession().createCriteria(AsuransiPegawai.class)
								.add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))).addOrder(Order.asc("nama")),
						AsuransiPegawai.class);

		asuransiPegawais.add(null);
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			for (AsuransiPegawai asuransiPegawai : asuransiPegawais) {
				Integer count = ((Number) HibernateUtil.currentSession().createCriteria(Pegawai.class)
						...
						.add(asuransiPegawai == null
								? Restrictions.and(Restrictions.isNull("asuransiPegawai1"),
										Restrictions.and(Restrictions.isNull("asuransiPegawai4"),
												Restrictions.and(Restrictions.isNull("asuransiPegawai3"),
														Restrictions.isNull("asuransiPegawai2"))))

								: Restrictions.or(Restrictions.eq("asuransiPegawai1", asuransiPegawai),
										Restrictions.or(Restrictions.eq("asuransiPegawai4", asuransiPegawai),
												Restrictions.or(Restrictions.eq("asuransiPegawai3", asuransiPegawai),
														Restrictions.eq("asuransiPegawai2", asuransiPegawai)))))
				...
				data.add(count);
				Integer colCount = listTotals.get(asuransiPegawai == null || asuransiPegawai.getId() == null ? -1L : asuransiPegawai.getId());
				if (colCount == null) {
					colCount = 0;
				}
				colCount += count;
				listTotals.put(asuransiPegawai == null || asuransiPegawai.getId() == null ? -1L : asuransiPegawai.getId(), colCount);
		...
		for (AsuransiPegawai asuransiPegawai : asuransiPegawais) {
			...
			if (colCount > 0) {
				column = new MyColumnConfig(asuransiPegawai == null ? "Tidak ditentukan" : asuransiPegawai.getNama());
				column.setWidth("5%");
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			int jml = 0;
			int i = 0;
			for (final AsuransiPegawai asuransiPegawai : asuransiPegawais) {
				Integer colCount = listTotals.get(asuransiPegawai == null || asuransiPegawai.getId() == null ? -1L : asuransiPegawai.getId());
				if (colCount == null) {
					colCount = 0;
				}
				if (colCount > 0) {
					...
					if (count > 0) {
						categoryModel.setValue(satuanKerja == null ? "Tidak Ditentukan" : satuanKerja.getNama(),
								asuransiPegawai == null ? "Tidak ditentukan" : asuransiPegawai.getNama(), count);
					}
					A a = new A(count + "");
										...
										public Object[] initCriteria(boolean order) {
											...
											try {
												...
												Criteria criteria = HibernateUtil.currentSession()
														...
														.add(asuransiPegawai == null ? Restrictions.and(
																Restrictions.isNull("asuransiPegawai1"),
																Restrictions.and(
																		Restrictions.isNull("asuransiPegawai4"),
																		Restrictions.and(
																				Restrictions.isNull("asuransiPegawai3"),
																				Restrictions
																						.isNull("asuransiPegawai2"))))

																: Restrictions.or(
																		Restrictions.eq("asuransiPegawai1",
																				asuransiPegawai),
																		Restrictions.or(
																				Restrictions.eq("asuransiPegawai4",
																						asuransiPegawai),
																				Restrictions.or(
																						Restrictions.eq(
																								"asuransiPegawai3",
															...
															: Restrictions.or(
																	...
																	Restrictions.or(
																			Restrictions.eq("asuransiPegawai4",
																					asuransiPegawai),
																			Restrictions.or(
																					Restrictions.eq("asuransiPegawai3",
																							asuransiPegawai),
																					Restrictions.eq("asuransiPegawai2",
																							asuransiPegawai)))))
		...
		row.appendChild(DashboardModernHtmlUtil.createAnyChart(categoryModel, "Dasbor Kepegawaian Asuransi", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
}
}
