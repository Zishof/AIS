...
/**
 * Komponen dashboard khusus untuk dashboard mahasiswa pendidikan ortu. Kelas ini memilih variasi
 ...
 * @see MyWindow
 */
public class DashboardMahasiswaPendidikanOrtu extends MyWindow {
	...
	public DashboardMahasiswaPendidikanOrtu() {
		super();
		try {
	...
	public DashboardMahasiswaPendidikanOrtu(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Mahasiswa per Pendidikan Orang Tua",
				"Sebaran mahasiswa menurut pendidikan orang tua, beserta grafiknya.");
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DashboardMahasiswaPendidikanOrtu.this.grid);
			}
		});
	...
	private void reload() {
		...
		final List<Jenjang> jenjangs = HibernateUtil.currentSession().createCriteria(Jenjang.class).list();
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				column.setParent(columns);
				column = new MyColumnConfig("Pendidikan");
				column.setParent(columns);
				...
				/* Donat DISTRIBUSI pendidikan orang tua (ayah/ibu/wali) lintas prodi dari data
				 * yang SUDAH dimuat — tanpa query tambahan; via DashboardUiKit.donut (HTML/CSS). */
				java.util.LinkedHashMap<String, Double> distPendidikan = new java.util.LinkedHashMap<String, Double>();
				for (Jurusan jurAgg : jurusans) {
					List<BiodataMahasiswa> dataAgg = datas.get(jurAgg.getId());
					if (dataAgg == null) {
						continue;
					}
					for (Jenjang jjAgg : jenjangs) {
						int n = 0;
						for (BiodataMahasiswa o : dataAgg) {
							if (o != null && ((o.getJenjangPendidikanAyah() != null
									&& o.getJenjangPendidikanAyah().getId().equals(jjAgg.getId()))
									|| (o.getJenjangPendidikanIbu() != null
											&& o.getJenjangPendidikanIbu().getId().equals(jjAgg.getId()))
									|| (o.getJenjangPendidikanWali() != null
											&& o.getJenjangPendidikanWali().getId().equals(jjAgg.getId())))) {
								n++;
							}
						}
						if (n > 0) {
							Double pv = distPendidikan.get(jjAgg.getNama());
							distPendidikan.put(jjAgg.getNama(), (pv == null ? 0.0 : pv) + n);
						}
					}
				}
				if (!distPendidikan.isEmpty()) {
					center.appendChild(new org.zkoss.zul.Html(ais.ui.util.DashboardUiKit.donut(
							"Distribusi Pendidikan Orang Tua",
							"Sebaran jenjang pendidikan orang tua (ayah/ibu/wali) dari seluruh program studi sesuai saringan.",
							distPendidikan, false, "Belum ada data untuk ditampilkan.")));
				}
		...
		row.setValign("top");
					...
					for (final Jenjang jenjang : jenjangs) {

						int jumlah = 0;
						for (BiodataMahasiswa o : data) {

							if (o != null && ((o.getJenjangPendidikanAyah() != null
									&& o.getJenjangPendidikanAyah().getId().equals(jenjang.getId())) ||

									(o.getJenjangPendidikanIbu() != null
											&& o.getJenjangPendidikanIbu().getId().equals(jenjang.getId()))

									||

									(o.getJenjangPendidikanWali() != null
											&& o.getJenjangPendidikanWali().getId().equals(jenjang.getId()))
						...
						if (jumlah > 0) {
							...
							A a = new A(jenjang.getNama() + ":" + Common.numberFormat.get().format(jumlah) + ", ");
												...
												public Object[] initCriteria(boolean order) {
													...
													try {
														...
														Criteria criteria = HibernateUtil.currentSession()
																...
																.add(Restrictions.or(
																		Restrictions.eq("jenjangPendidikanWali",
																				jenjang),
																		Restrictions.or(
																				Restrictions.eq("jenjangPendidikanAyah",
																						jenjang),
																				Restrictions.eq("jenjangPendidikanIbu",
																						jenjang))))
