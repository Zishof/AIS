...
/**
 * Komponen dashboard khusus untuk dasboard kepegawaian jenis jabatan. Kelas ini memilih variasi
 ...
 * @see MyWindow
 */
public class DasboardKepegawaianJenisJabatan extends MyWindow {
	...
	public DasboardKepegawaianJenisJabatan() {
		super();
		try {
	...
	public DasboardKepegawaianJenisJabatan(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Sebaran Pegawai per Jenis Jabatan",
				"Jumlah pegawai menurut jenis jabatan, beserta grafiknya.");
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DasboardKepegawaianJenisJabatan.this.grid);
			}
		});
	}
	...
	private void reload() {
		...
		List<JenisJabatan> atasans = ConstantValues.simpleList(HibernateUtil.currentSession()
				.createCriteria(JenisJabatan.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))).addOrder(Order.asc("nama")),
				JenisJabatan.class);

		atasans.add(null);
		Map<Long, MyColumnConfig> listCols = new HashMap<Long, MyColumnConfig>();
		for (JenisJabatan atasan : atasans) {
			column = new MyColumnConfig(atasan == null ? "Tidak ditentukan" : atasan.getNama());
			column.setWidth("5%");
			column.setAlign("right");
			listCols.put(atasan == null || atasan.getId() == null ? -1L : atasan.getId(), column);
		}
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			for (JenisJabatan atasan : atasans) {
				Integer count = ((Number) HibernateUtil.currentSession().createCriteria(Pegawai.class)
						...
						.add(atasan == null ? Restrictions.isNull("atasan") : Restrictions.or(Restrictions.eq("atasan", atasan), Restrictions.eq("atasanPendukung", atasan)) )
				...
				data.add(count);
				Integer colCount = listTotals.get(atasan == null || atasan.getId() == null ? -1L : atasan.getId());
				if (colCount == null) {
					colCount = 0;
				}
				colCount += count;
				listTotals.put(atasan == null || atasan.getId() == null ? -1L : atasan.getId(), colCount);
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			int jml = 0;
			int i = 0;
			for (final JenisJabatan atasan : atasans) {
				Integer colCount = listTotals.get(atasan == null || atasan.getId() == null ? -1L : atasan.getId());
				if (colCount == null) {
					colCount = 0;
				}
				if (colCount > 0) {
					...
					if (count > 0) {
						categoryModel.setValue(satuanKerja == null ? "Tidak Ditentukan" : satuanKerja.getNama(),
								atasan == null ? "Tidak ditentukan" : atasan.getNama(), count);
					}
					A a = new A(count + "");
										...
										public Object[] initCriteria(boolean order) {
											...
											try {
												...
												Criteria criteria = HibernateUtil.currentSession()
														...
														.add(satuanKerja == null ? Restrictions.isNull("satuanKerja")
																: Restrictions.eq("satuanKerja", satuanKerja))
														.add(atasan == null ? Restrictions.isNull("atasan")
																:  Restrictions.or(Restrictions.eq("atasan", atasan), Restrictions.eq("atasanPendukung", atasan))  )
														.add(tipeMasaKerja.getSelectedItem() == null
																|| tipeMasaKerja.getSelectedItem().getValue() == null
																		? Restrictions.sqlRestriction("true")
		...
		for (final JenisJabatan atasan : atasans) {
			...
			if (colCount > 0) {
				countCol++;
				listCols.get(atasan == null || atasan.getId() == null ? -1L : atasan.getId()).setParent(columns);
		...
		row.appendChild(DashboardModernHtmlUtil.createAnyChart(categoryModel, "Dasbor Kepegawaian Jenis Jabatan", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
}
}
