...
/**
 * Komponen dashboard khusus untuk dashboard wisuda mahasiswa. Kelas ini memilih variasi data atau
 ...
 * @see MyWindow
 */
public class DashboardWisudaMahasiswa extends MyWindow {
	...
	public DashboardWisudaMahasiswa() {
		super();
		try {
	...
	public DashboardWisudaMahasiswa(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Wisuda Mahasiswa",
				"Rekap wisuda per program studi, beserta grafiknya.");
		...
		row.appendChild(new MyLabelConfig("Wisuda"));
		row.appendChild(formatNilai);
		Common.insertComboDanSemua(formatNilai, "nama", Wisuda.class);
	...
	private void reload() {
		...
		final Wisuda wisuda = (Wisuda) (formatNilai.getSelectedItem() == null
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
												...
												try {
													...
													Criteria criteria = HibernateUtil.currentSession()
															...
															.add(status == null ? Restrictions.sqlRestriction("1=1")
																	: Restrictions.eq("persetujuanWisuda", status))
															...
															.add(Restrictions.eq("skripsi.tahunAkademik", tahunAjaran));

													return new Object[] { criteria, DetailwisudaHelper.contents };
				...
				row.appendChild(DashboardModernHtmlUtil.createAnyChart(categoryModel, "Dasbor Wisuda Mahasiswa", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
}
		});
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				for (final Jurusan jurusan : jurusans) {
					...
					List<Object[]> data = session.createCriteria(PendaftaranWisuda.class)
							.add(status == null ? Restrictions.sqlRestriction("1=1")
									: Restrictions.eq("persetujuanWisuda", status))
							.createAlias("mahasiswa", "mahasiswa").createAlias("skripsi", "skripsi")
							.add(wisuda == null ? Restrictions.sqlRestriction("true")
									: Restrictions.eq("wisuda", wisuda))

							.add(Restrictions.eq("mahasiswa.jurusan", jurusan))
							.add(Restrictions.in("skripsi.tahunAkademik", tas))

							.setProjection(Projections.projectionList().add(Projections.rowCount())
									.add(Projections.groupProperty("skripsi.tahunAkademik")))
							.list();
