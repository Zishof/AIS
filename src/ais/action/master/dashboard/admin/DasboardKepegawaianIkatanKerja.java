...
/**
 * Komponen dashboard khusus untuk dasboard kepegawaian ikatan kerja. Kelas ini memilih variasi
 ...
 * @see MyWindow
 */
public class DasboardKepegawaianIkatanKerja extends MyWindow {
	...
	public DasboardKepegawaianIkatanKerja() {
		super();
		try {
	...
	public DasboardKepegawaianIkatanKerja(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Sebaran Pegawai per Ikatan Kerja",
				"Jumlah pegawai menurut ikatan kerja, beserta grafiknya.");
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DasboardKepegawaianIkatanKerja.this.grid);
			}
		});
	}
	...
	private void reload() {
		...
		List<IkatanKerjaDosen> ikatanKerjaDosens = ConstantValues
				.simpleList(
						HibernateUtil.currentSession().createCriteria(IkatanKerjaDosen.class)
								.add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))).addOrder(Order.asc("nama")),
						IkatanKerjaDosen.class);

		ikatanKerjaDosens.add(null);
		Map<Long, MyColumnConfig> listCols = new HashMap<Long, MyColumnConfig>();
		for (IkatanKerjaDosen ikatanKerjaDosen : ikatanKerjaDosens) {
			column = new MyColumnConfig(ikatanKerjaDosen == null ? "Tidak ditentukan" : ikatanKerjaDosen.getNama());
			column.setWidth("5%");
			column.setAlign("right");
			listCols.put(ikatanKerjaDosen == null || ikatanKerjaDosen.getId() == null ? -1L : ikatanKerjaDosen.getId(), column);
		}
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			for (IkatanKerjaDosen ikatanKerjaDosen : ikatanKerjaDosens) {
				Integer count = ((Number) HibernateUtil.currentSession().createCriteria(Pegawai.class)
						...
						.add(ikatanKerjaDosen == null ? Restrictions.isNull("ikatanKerjaDosen")
								: Restrictions.eq("ikatanKerjaDosen", ikatanKerjaDosen))
				...
				data.add(count);
				Integer colCount = listTotals.get(ikatanKerjaDosen == null || ikatanKerjaDosen.getId() == null ? -1L : ikatanKerjaDosen.getId());
				if (colCount == null) {
					colCount = 0;
				}
				colCount += count;
				listTotals.put(ikatanKerjaDosen == null || ikatanKerjaDosen.getId() == null ? -1L : ikatanKerjaDosen.getId(), colCount);
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			int jml = 0;
			int i = 0;
			for (final IkatanKerjaDosen ikatanKerjaDosen : ikatanKerjaDosens) {
				Integer colCount = listTotals.get(ikatanKerjaDosen == null || ikatanKerjaDosen.getId() == null ? -1L : ikatanKerjaDosen.getId());
				if (colCount == null) {
					colCount = 0;
				}
				if (colCount > 0) {
					...
					if (count > 0) {
						categoryModel.setValue(satuanKerja == null ? "Tidak Ditentukan" : satuanKerja.getNama(),
								ikatanKerjaDosen == null ? "Tidak ditentukan" : ikatanKerjaDosen.getNama(), count);
					}
					A a = new A(count + "");
										...
										public Object[] initCriteria(boolean order) {
											...
											try {
												...
												Criteria criteria = HibernateUtil.currentSession()
														...
														.add(satuanKerja == null ? Restrictions.isNull("satuanKerja")
																: Restrictions.eq("satuanKerja", satuanKerja))
														.add(ikatanKerjaDosen == null
																? Restrictions.isNull("ikatanKerjaDosen")
																: Restrictions.eq("ikatanKerjaDosen", ikatanKerjaDosen))
														.add(tipeMasaKerja.getSelectedItem() == null
																|| tipeMasaKerja.getSelectedItem().getValue() == null
																		? Restrictions.sqlRestriction("true")
		...
		for (final IkatanKerjaDosen ikatanKerjaDosen : ikatanKerjaDosens) {
			...
			if (colCount > 0) {
				countCol++;
				listCols.get(ikatanKerjaDosen == null || ikatanKerjaDosen.getId() == null ? -1L : ikatanKerjaDosen.getId()).setParent(columns);
		...
		row.appendChild(DashboardModernHtmlUtil.createAnyChart(categoryModel, "Dasbor Kepegawaian Ikatan Kerja", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
}
}
