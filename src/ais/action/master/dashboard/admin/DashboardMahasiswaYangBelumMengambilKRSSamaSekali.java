...
/**
 * Komponen dashboard khusus untuk dashboard mahasiswa yang belum mengambil krs sama sekali. Kelas
 ...
 * searchsemester}, {@code Combobox searchprogram}, {@code Combobox searchstatus}, {@code Combobox
 * searchStatusAwalMahasiswa}; inisialisasi/lifecycle ({@code initFakultas()}, {@code init()}, {@code
 ...
 * @see MyWindow
 */
public class DashboardMahasiswaYangBelumMengambilKRSSamaSekali extends MyWindow {
	...
	public DashboardMahasiswaYangBelumMengambilKRSSamaSekali() {
		super();
		try {
	...
	public DashboardMahasiswaYangBelumMengambilKRSSamaSekali(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		vbox.appendChild(semesterAbsensi);
		...
		vbox.appendChild(searchsemester);
		searchsemester.setCols(2);

		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Status Keluar"));
		row.appendChild(searchStatusKeluar = new Combobox());
		Common.insertComboDanSemua(searchStatusKeluar, new String[] { "nama", "feeder" }, "keterangan",
				StatusKeluar.class, "== Mahasiswa Belum Keluar / Masih Aktif ==", Restrictions.sqlRestriction("true"));
			...
			public void onEvent(Event event) throws Exception {
				try {
					Filedownload.save(new FileInputStream(file),
							...
							"KRS_BELUM_AMBIL_SEMUA.xlsx");
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardMahasiswaYangBelumMengambilKRSSamaSekali.java:333");
	...
	private void initSpreadsheet() throws Exception {
		...
		Common.createDefaultTimer(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				Common.clear(center);
				final String tahunAkademik = (String) (DashboardMahasiswaYangBelumMengambilKRSSamaSekali.this.tahunAkademik
						.getSelectedItem() == null ? null
								: DashboardMahasiswaYangBelumMengambilKRSSamaSekali.this.tahunAkademik.getSelectedItem()
										.getValue());
				...
				final StatusKeluar statusKeluar = (StatusKeluar) (searchStatusKeluar.getSelectedItem() == null ? null
				...
				final Label label = Common.displayLoadBar(DashboardMahasiswaYangBelumMengambilKRSSamaSekali.this, file,
						center, sizedata);
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						XSSFSheet sheet = workbook.createSheet("BELUM_AMBIL_KRS");
						...
						rowhead.createCell(5).setCellValue("PROGRAM STUDI");
						rowhead.createCell(6).setCellValue("ANGKATAN");
						rowhead.createCell(7).setCellValue("TAHUN AKADEMIK");
						rowhead.createCell(8).setCellValue("SEMESTER");
						rowhead.createCell(9).setCellValue("KELAS");
						rowhead.createCell(10).setCellValue("DOSEN PA");
						rowhead.createCell(11).setCellValue("STATUS");
						rowhead.createCell(12).setCellValue("STATUS KELUAR");
						rowhead.createCell(13).setCellValue("IPK");
						rowhead.createCell(14).setCellValue("SKSK");

						Session session = HibernateUtil.currentNativeSession();

						String sql = " (select a.mahasiswa from "
								+ "(select aa.mahasiswa,max(aa.persetujuan) as persetujuan "
						...
						List<Mahasiswa> mahasiswas = ConstantValues.simpleList(session.createCriteria(Mahasiswa.class)
								...
								.add(Restrictions.sqlRestriction("this_.id not in " + sql))
