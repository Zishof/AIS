...
/**
 * Komponen dashboard khusus untuk dashboard mahasiswa kota. Kelas ini memilih variasi data atau
 ...
 * @see MyWindow
 */
public class DashboardMahasiswaKota extends MyWindow {
	...
	public DashboardMahasiswaKota(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Mahasiswa per Kota/Kabupaten",
				"Sebaran mahasiswa menurut kota/kabupaten asal, beserta grafiknya.");
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DashboardMahasiswaKota.this.grid);
			}
		});
	...
	private void reload() {
		...
		final List<Long> kotasId = HibernateUtil.currentSession().createCriteria(BiodataMahasiswa.class)
				.add(criteriaStatus).add(Restrictions.isNotNull("kota")).createAlias("mahasiswa", "mahasiswa")
				...
				.setProjection(Projections.groupProperty("kota.id"))
				...
				.addOrder(Order.asc("kota.id")).list();
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				column.setParent(columns);
				column = new MyColumnConfig("Kota/Kabupaten");
				column.setParent(columns);
		...
		row.setValign("top");
					...
					for (Long kotaid : kotasId) {

						final Kota kota = (Kota) ConstantValues.ambil(Kota.class.getName(), kotaid);
						Number jumlah = 0;
						for (Object[] o : data) {
							Long kec = (Long) o[1];
							if (kec != null && kec.equals(kota.getId())) {
								jumlah = (Number) o[0];
								break;
							}
						}
						...
						if (jumlah.intValue() > 0) {
							...
							A a = new A(kota.getNama() + ":" + Common.numberFormat.get().format(jumlah) + ", ");
												...
												public Object[] initCriteria(boolean order) {
													...
													try {
														...
														Criteria criteria = HibernateUtil.currentSession()
																...
																.add(criteriaStatus).add(Restrictions.isNotNull("kota"))
			...
			public void run() {
				...
				for (final Jurusan jurusan : jurusans) {
					...
					List<Object[]> data = session.createCriteria(BiodataMahasiswa.class).add(criteriaStatus)
							.add(Restrictions.isNotNull("kota")).createAlias("mahasiswa", "mahasiswa")
							...
							.setProjection(Projections.projectionList().add(Projections.rowCount())
									.add(Projections.groupProperty("kota.id")))
