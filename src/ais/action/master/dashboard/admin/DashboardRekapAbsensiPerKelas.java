...
/**
 * Komponen dashboard khusus untuk dashboard rekap absensi per kelas. Kelas ini memilih variasi
 ...
 * @see MyWindow
 */
public class DashboardRekapAbsensiPerKelas extends MyWindow {
	...
	public DashboardRekapAbsensiPerKelas() {
		super();
		try {
			init();
			initFakultas();
	...
	public DashboardRekapAbsensiPerKelas(String title, String border, boolean closable) {
	...
	private void initSpreadsheet() {
		...
		Common.createDefaultTimer(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				Common.clear(center);
				String tahunAkademik = (String) (DashboardRekapAbsensiPerKelas.this.tahunAkademik
						.getSelectedItem() == null ? null
								: DashboardRekapAbsensiPerKelas.this.tahunAkademik.getSelectedItem().getValue());
				...
				sql = "select kelas,\n" + "sum(hadir) hadir, sum(alpa) alpa, sum(sakit) sakit, sum(izin) izin \n"
						+ "from (" + sql + ") a group by kelas \n having sum(hadir+alpa+sakit+izin)>0";
				...
				for (Object[] objects : jurusans) {
					...
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapAbsensiPerKelas.java:433");
					}
				...
				try {
					ais.ui.util.EcampusUtil.setBold(sheet,
							...
							true);
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapAbsensiPerKelas.java:456");
				}
			}
		});
