...
/**
 * Komponen dashboard khusus untuk dasboard kepegawaian status kepegawaian. Kelas ini memilih
 ...
 * @see MyWindow
 */
public class DasboardKepegawaianStatusKepegawaian extends MyWindow {
	...
	public DasboardKepegawaianStatusKepegawaian() {
		super();
		try {
	...
	public DasboardKepegawaianStatusKepegawaian(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Sebaran Pegawai per Status Kepegawaian",
				"Jumlah pegawai menurut status kepegawaian, beserta grafiknya.");
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DasboardKepegawaianStatusKepegawaian.this.grid);
			}
		});
	}
	...
	private void reload() {
		...
		List<StatusKepegawaian> statusKepegawaians = ConstantValues.simpleList(
				HibernateUtil.currentSession().createCriteria(StatusKepegawaian.class).addOrder(Order.asc("nama")),
				StatusKepegawaian.class);
		statusKepegawaians.add(null);
		...
		for (StatusKepegawaian statusKepegawaian : statusKepegawaians) {
			column = new MyColumnConfig(statusKepegawaian == null ? "Tidak ditentukan" : statusKepegawaian.getNama());
			column.setWidth("5%");
			column.setAlign("right");
			listCols.put(statusKepegawaian == null || statusKepegawaian.getId() == null ? -1L : statusKepegawaian.getId(), column);
		}
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			for (StatusKepegawaian statusKepegawaian : statusKepegawaians) {
				Integer count = ((Number) HibernateUtil.currentSession().createCriteria(Pegawai.class)
						...
						.add(statusKepegawaian == null ? Restrictions.isNull("statusKepegawaian")
								: Restrictions.eq("statusKepegawaian", statusKepegawaian))
				...
				Integer colCount = listTotals.get(statusKepegawaian == null || statusKepegawaian.getId() == null ? -1L : statusKepegawaian.getId());
				if (colCount == null) {
					colCount = 0;
				}
				colCount += count;
				listTotals.put(statusKepegawaian == null || statusKepegawaian.getId() == null ? -1L : statusKepegawaian.getId(), colCount);
			}
		}
				...
				if (colCount > 0) {
					...
					if (count > 0) {
						categoryModel.setValue(satuanKerja == null ? "Tidak Ditentukan" : satuanKerja.getNama(),
								statusKepegawaian == null ? "Tidak ditentukan" : statusKepegawaian.getNama(), count);
					}
					A a = new A(count + "");
										...
										public Object[] initCriteria(boolean order) {
											...
											try {
												...
												Criteria criteria = HibernateUtil.currentSession()
														...
														.add(satuanKerja == null ? Restrictions.isNull("satuanKerja")
																: Restrictions.eq("satuanKerja", satuanKerja))
														.add(statusKepegawaian == null
																? Restrictions.isNull("statusKepegawaian")
																: Restrictions.eq("statusKepegawaian",
																		statusKepegawaian))
														.add(tipeMasaKerja.getSelectedItem() == null
																|| tipeMasaKerja.getSelectedItem().getValue() == null
																		? Restrictions.sqlRestriction("true")
		...
		for (final StatusKepegawaian statusKepegawaian : statusKepegawaians) {
			...
			if (colCount > 0) {
				countCol++;
				listCols.get(statusKepegawaian == null || statusKepegawaian.getId() == null ? -1L : statusKepegawaian.getId()).setParent(columns);
		...
		row.appendChild(DashboardModernHtmlUtil.createAnyChart(categoryModel, "Dasbor Kepegawaian Status Kepegawaian", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
}
}
