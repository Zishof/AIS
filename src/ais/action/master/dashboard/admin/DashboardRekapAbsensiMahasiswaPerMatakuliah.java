...
/**
 * Komponen dashboard khusus untuk dashboard rekap absensi mahasiswa per matakuliah. Kelas ini
 ...
 * searchfakultas}, {@code Combobox searchjurusan}, {@code Combobox tahunAkademik}, {@code Combobox
 * semesterAbsensi}, {@code Combobox searchsemester}, {@code Combobox searchprogram}, {@code Combobox
 * angkatanMhs}; inisialisasi/lifecycle ({@code initFakultas()}, {@code init()}, {@code initSpreadsheet()});
 * pembacaan/pencarian ({@code tampilkanPopupPreviewExcel()}, {@code tampilkanLoading()}); validasi/perhitungan
 * ({@code hitungSummary()}); pelaporan/ekspor ({@code renderRekapGrid()}); operasi domain lain ({@code
 * buildDashboardHtml()}, {@code cardHtml()}, {@code barHtml()}, {@code buildEmptyHtml()}, {@code toInteger()},
 * {@code nvl()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di atas.</p>
 ...
 * @see MyWindow
 */
public class DashboardRekapAbsensiMahasiswaPerMatakuliah extends MyWindow {
	...
	public DashboardRekapAbsensiMahasiswaPerMatakuliah() {
		super();
		try {
			init();
			initFakultas();
	...
	public DashboardRekapAbsensiMahasiswaPerMatakuliah(String title, String border, boolean closable) {
	...
	private void initSpreadsheet() {
		Common.clear(center);
		tampilkanLoading("Mengambil dan menghitung rekap absensi per mata kuliah...");

		Common.createDefaultTimer(new EventListener() {

			@Override
			public void onEvent(Event arg0) throws Exception {
				String tahunAkademik = (String) (DashboardRekapAbsensiMahasiswaPerMatakuliah.this.tahunAkademik
						.getSelectedItem() == null ? null
								: DashboardRekapAbsensiMahasiswaPerMatakuliah.this.tahunAkademik.getSelectedItem()
										.getValue());
				...
				String sql = "select max(e.kode||' '||e.nama) as matakuliah,\n"
				...
				sql = "select max(matakuliah) matakuliah,max(perkuliahan) perkuliahan,\n"
				...
				AttendanceSummary summary = hitungSummary(jurusans);
				Common.clear(center);
				...
				new Html(buildDashboardHtml("Dashboard Rekap Absensi per Mata Kuliah",
						...
						summary,
						"Membantu operator melihat kelas atau mata kuliah mana yang memiliki tingkat kehadiran baik dan mana yang perlu perhatian. Rekap ditampilkan per mata kuliah agar mudah dibahas dengan dosen pengampu."))
				...
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 0, "Matakuliah");
				Utils.setColumnWidth(sheet, 0, 200);
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 1, "Jadwal Perkuliahan/Dosen");
				...
				for (Object[] objects : jurusans) {
					...
					Integer nilai0 = toInteger(objects[2]);
					Integer nilai1 = toInteger(objects[3]);
					Integer nilai2 = toInteger(objects[4]);
					Integer nilai3 = toInteger(objects[5]);
					...
					try {
						...
						if (debug) {
							e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/dashboard/admin/DashboardRekapAbsensiMahasiswaPerMatakuliah.java:475");
						}
					}
				...
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 2, Integer.valueOf(summary.hadir));
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 3, Integer.valueOf(summary.alpa));
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 4, Integer.valueOf(summary.sakit));
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 5, Integer.valueOf(summary.izin));
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 6, Integer.valueOf(summary.totalTidakHadir));
				...
				try {
					...
					if (debug) {
						e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/dashboard/admin/DashboardRekapAbsensiMahasiswaPerMatakuliah.java:500");
					}
				}
			}
		});
	...
	private void renderRekapGrid(Vbox parent, List<Object[]> data) {
		new Html("<div style='background:#ffffff;border:1px solid #e2e8f0;border-radius:16px;padding:14px;"
				...
				+ "<div style='font-size:16px;font-weight:900;color:#0f172a;'>Tabel Rekap Absensi per Mata Kuliah</div>"
				+ "<div style='font-size:12px;color:#64748b;line-height:1.55;margin-top:5px;'>"
				+ "Data ditampilkan lebih dulu sebagai tabel agar pengguna dapat menilai kondisi kehadiran setiap mata kuliah tanpa membuka Excel. Gunakan tombol Download untuk melihat preview Excel, lalu unduh jika datanya sudah sesuai."
		...
		if (data == null || data.isEmpty()) {
			...
			ais.ui.util.ZkCompat.setSpans(empty, "9");
			empty.setParent(rows);
			new Label(ais.common.Common.getBahasaConfig("Tidak ada data absensi per mata kuliah yang sesuai dengan filter.")).setParent(empty);
			return;
		}

		AttendanceSummary summary = hitungSummary(data);
		for (Object[] objects : data) {
			if (objects == null || objects.length == 0 || objects[0] == null) {
				continue;
			}
			Integer hadir = toInteger(objects[2]);
			Integer alpa = toInteger(objects[3]);
			Integer sakit = toInteger(objects[4]);
			Integer izin = toInteger(objects[5]);
	...
	private void tampilkanPopupPreviewExcel() {
		try {
			...
			popup.setTitle("Preview Excel Rekap Absensi per Mata Kuliah");
			...
			download.addEventListener("onClick", new EventListener() {
				...
				public void onEvent(Event event) throws Exception {
					...
					Filedownload.save(bout.toByteArray(),
							"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Rekap_Data_Per_Matakuliah.xlsx");
				}
			});
	...
	private static AttendanceSummary hitungSummary(List<Object[]> rows) {
		...
		for (Object[] row : rows) {
			...
			summary.hadir += toInteger(row[2]).intValue();
			summary.alpa += toInteger(row[3]).intValue();
			summary.sakit += toInteger(row[4]).intValue();
			summary.izin += toInteger(row[5]).intValue();
			summary.jumlahBaris++;
		}
	...
	private static String buildDashboardHtml(String title, String subtitle, AttendanceSummary summary, String description) {
		...
		sb.append(cardHtml("Mata Kuliah", String.valueOf(summary.jumlahBaris), "Jumlah kelas/mata kuliah", "#2563eb"));
	...
	/**
	 * Pembawa data/helper lokal milik {@link DashboardRekapAbsensiMahasiswaPerMatakuliah} untuk attendance
	 * summary. Tipe ini mengelompokkan nilai antara agar perhitungan atau rendering tidak memakai array/map tanpa
	 * kontrak yang jelas.
	 *
	 * <p><b>Scope:</b> tipe bersifat {@code static}; instance tidak menangkap object {@link
	 * DashboardRekapAbsensiMahasiswaPerMatakuliah}. Dependensi yang diperlukan harus diberikan secara eksplisit
	 * agar aman digunakan dan diuji.</p> Tipe ini merupakan detail implementasi privat; pemanggil luar harus
	 * memakai API kelas induk.
	 ...
	 * yang dipanggilnya.</p>
	 *
	 * @see DashboardRekapAbsensiMahasiswaPerMatakuliah
	 */
