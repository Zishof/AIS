...
/**
 * Komponen dashboard khusus untuk dashboard mahasiswa agama. Kelas ini memilih variasi data atau
 ...
 * @see MyWindow
 */
public class DashboardMahasiswaAgama extends MyWindow {
	...
	public DashboardMahasiswaAgama() {
		super();
		try {
	...
	public DashboardMahasiswaAgama(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Mahasiswa per Agama",
				"Sebaran mahasiswa menurut agama, beserta grafiknya.");
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DashboardMahasiswaAgama.this.grid);
			}
		});
	...
	private void reload() {
		...
		final List<Agama> agamas = HibernateUtil.currentSession().createCriteria(Agama.class)
				.add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))).list();
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				column.setParent(columns);
				column = new MyColumnConfig("Agama");
				column.setParent(columns);
				...
				/* Donat komposisi total mahasiswa per agama (lintas prodi) dari data yang
				 * SUDAH dimuat — agregasi ringan tanpa query tambahan, lalu dirender via
				 * DashboardUiKit.donut (HTML/CSS, tanpa JFreeChart). */
				java.util.LinkedHashMap<String, Double> komposisiAgama = new java.util.LinkedHashMap<String, Double>();
				for (Jurusan jurAgg : jurusans) {
					List<BiodataMahasiswa> dataAgg = datas.get(jurAgg.getId());
					if (dataAgg == null) {
						continue;
					}
					for (Agama agAgg : agamas) {
						int n = 0;
						for (BiodataMahasiswa o : dataAgg) {
							if (o != null && o.getAgama() != null && o.getAgama().getId().equals(agAgg.getId())) {
								n++;
							}
						}
						if (n > 0) {
							Double pv = komposisiAgama.get(agAgg.getNama());
							komposisiAgama.put(agAgg.getNama(), (pv == null ? 0.0 : pv) + n);
						}
					}
				}
				if (!komposisiAgama.isEmpty()) {
					center.appendChild(new org.zkoss.zul.Html(ais.ui.util.DashboardUiKit.donut(
							"Komposisi Mahasiswa per Agama",
							"Porsi mahasiswa pada tiap agama dari seluruh program studi sesuai saringan.",
							komposisiAgama, false, "Belum ada data untuk ditampilkan.")));
				}
		...
		row.setValign("top");
					...
					for (final Agama agama : agamas) {

						int jumlah = 0;
						for (BiodataMahasiswa o : data) {

							if (o != null && ((o.getAgama() != null && o.getAgama().getId().equals(agama.getId()))
						...
						if (jumlah > 0) {
							...
							A a = new A(agama.getNama() + ":" + Common.numberFormat.get().format(jumlah) + ", ");
												...
												public Object[] initCriteria(boolean order) {
													...
													try {
														...
														Criteria criteria = HibernateUtil.currentSession()
																...
																.add(Restrictions.eq("agama", agama))
