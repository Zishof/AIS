...
/**
 * Komponen dashboard khusus untuk dashboard rekap pendaftar mahasiswa baru. Kelas ini memilih
 ...
 * @see MyWindow
 */
public class DashboardRekapPendaftarMahasiswaBaru extends MyWindow {
	...
	public DashboardRekapPendaftarMahasiswaBaru() {
		super();
		try {
			init();
			initFakultas();
	...
	private void initSpreadsheet() {
		...
		List<String> generalValueObjects = new ArrayList<String>();
		generalValueObjects.add("Peminat");
		generalValueObjects.add("Lulus");
		generalValueObjects.add("Dapat NIM");

		// Collections.sort(generalValueObjects);

		String sql = "select max(y.nama) as fakultas, max(x.nama) as jurusan, ";

		for (String s : generalValueObjects) {
			if (s.equals("Peminat")) {
				sql += "sum(case when aaa.jenis_kelamin='Laki-laki' then 1 else 0 end) as \"" + s + " Laki-laki\", ";
				sql += "sum(case when aaa.jenis_kelamin='Perempuan' then 1 else 0 end) as \"" + s + " Perempuan\", ";
				sql += "sum(case when aaa.jenis_kelamin in ('Laki-laki','Perempuan') then 1 else 0 end) as \"" + s
						+ "\", ";
			} else if (s.equals("Lulus")) {
				sql += "sum(case when aaa.jenis_kelamin='Laki-laki' and prodi_lulus is not null then 1 else 0 end) as \""
						+ s + " Laki-laki\", ";
				sql += "sum(case when aaa.jenis_kelamin='Perempuan' and prodi_lulus is not null then 1 else 0 end) as \""
						+ s + " Perempuan\", ";
				sql += "sum(case when aaa.jenis_kelamin in ('Laki-laki','Perempuan') and prodi_lulus is not null then 1 else 0 end) as \""
						+ s + "\", ";
			} else if (s.equals("Dapat NIM")) {
				sql += "sum(case when aaa.jenis_kelamin='Laki-laki' and mahasiswa is not null then 1 else 0 end) as \""
						+ s + " Laki-laki\", ";
				sql += "sum(case when aaa.jenis_kelamin='Perempuan' and mahasiswa is not null then 1 else 0 end) as \""
						+ s + " Perempuan\", ";
				sql += "sum(case when aaa.jenis_kelamin in ('Laki-laki','Perempuan') and mahasiswa is not null then 1 else 0 end) as \""
						+ s + "\", ";
			}
		}
		...
		sql += " 1 as total from biodata_calon_mahasiswa aaa   inner join jurusan x on (aaa." + pilihan
				+ " = x.id  )     inner join fakultas y on (y.id = x.fakultas)      where 1=1 and aaa.semester_mulai='"
		...
		spreadsheet.setMaxcolumns((generalValueObjects.size() * 3) + 2);
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, 1, 0,
				"REKAPITULASI MAHASISWA BARU " + searchpilihan.getValue().toUpperCase() + "\n"
						...
						+ tahunAkademik + " " + searchJenisSemester.getSelectedItem().getValue() + "\nPROGRAM "
		...
		int colIndex = 2;
		for (String s : generalValueObjects) {
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, colIndex, s);
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, colIndex + 1, s);
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, colIndex + 2, s);
		...
		for (Object[] objects : jurusans) {
			...
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapPendaftarMahasiswaBaru.java:437");
			}
		...
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapPendaftarMahasiswaBaru.java:469");
		}
		...
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapPendaftarMahasiswaBaru.java:480");
		}

		// ── Panel visual: DONUT komposisi gender pendaftar maba (HTML/CSS, BUKAN JFreeChart) ──
		try {
			...
			chartPanel.setContent("<div style='padding:10px 12px 2px;'>"
					+ ais.ui.util.HtmlChartHelper.donut(
							"Komposisi Pendaftar Mahasiswa Baru (Laki-laki vs Perempuan)",
							"Melihat sekilas perbandingan jumlah pendaftar mahasiswa baru laki-laki dan perempuan.",
							gLabels, gValues, gColors, total + " pendaftar")
					+ "</div>");
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapPendaftarMahasiswaBaru.java:494");
