...
/**
 * Komponen dashboard khusus untuk dashboard kehadiran mahasiswa. Kelas ini memilih variasi data
 ...
 * @see MyWindow
 */
public class DashboardKehadiranMahasiswa extends MyWindow {
	...
	public DashboardKehadiranMahasiswa() {
		super();
		try {
	...
	public DashboardKehadiranMahasiswa(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Kehadiran Mahasiswa",
				"Rekap kehadiran mahasiswa di perkuliahan per kelas, beserta grafiknya.");
		...
		Rows rows = new Rows();
		rows.setParent(grid);
		appendDashboardIntroRow(rows, "Dashboard Kehadiran Mahasiswa", "Menyajikan tingkat kehadiran mahasiswa per mata kuliah, kelas, dosen, program, fakultas, prodi, tahun ajaran, dan semester. Informasi ini membantu prodi menemukan kelas dengan partisipasi rendah, memeriksa kebutuhan pembinaan akademik, dan memastikan proses perkuliahan berjalan sesuai rencana.");
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DashboardKehadiranMahasiswa.this.grid);
			}
		});
	...
	private void reload() {
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				appendDashboardSummary(dashboardBody, "Dashboard Kehadiran Mahasiswa", "Menyajikan tingkat kehadiran mahasiswa per mata kuliah, kelas, dosen, program, fakultas, prodi, tahun ajaran, dan semester. Informasi ini membantu prodi menemukan kelas dengan partisipasi rendah, memeriksa kebutuhan pembinaan akademik, dan memastikan proses perkuliahan berjalan sesuai rencana.", "Kehadiran Mhs");
				grid.setWidth("100%");
				...
				column.setParent(columns);
				column = new MyColumnConfig("Kehadiran Mhs");
				column.setParent(columns);
				column.setWidth("8%");

				column.setParent(columns);
				column = new MyColumnConfig("% Mhs");
				column.setParent(columns);
				column.setWidth("8%");
				...
				for (final Dosen dosen : dosensMap.keySet()) {
					...
					for (final Perkuliahan perkuliahan : t1) {
						...
						Map<String, Integer> semuaStatusesMahasiswa = new HashMap<String, Integer>();
						for (Pertemuan pertemuan : pertemuans) {
							if (pertemuan.getAktif()) {
								...
								Map<String, Integer> statusesMahasiswa = pertemuan.hitungStatus(null);
								for (String key : statusesMahasiswa.keySet()) {
									if (semuaStatusesMahasiswa.containsKey(key)) {
										semuaStatusesMahasiswa.put(key,
												semuaStatusesMahasiswa.get(key) + statusesMahasiswa.get(key));
									} else {
										semuaStatusesMahasiswa.put(key, statusesMahasiswa.get(key));
									}
								}
							}
						}
						...
						Integer kehadiran = semuaStatusesMahasiswa.get("M") == null ? 0
								: semuaStatusesMahasiswa.get("M");
						...
						a.addEventListener("onClick", new EventListener() {
							...
							public void onEvent(Event arg0) throws Exception {
								...
								CommonReportHelper.onLaporanAbsensi(perkuliahan, true, 6);

							}
						});

						row.appendChild(
								a = new A(Common.numberFormat.get()
										.format(countSudahDisetujui.equals(0) ? 0
												: (kehadiran * 100.0) / (countSudahDisetujui * pertemuans.size()))
										+ "%"));
						a.setStyle("font-size:12px;");
						a.setParent(row);
						a.addEventListener("onClick", new EventListener() {
							...
							public void onEvent(Event arg0) throws Exception {
								...
								CommonReportHelper.onLaporanAbsensi(perkuliahan, true, 8);
	...
	private void appendDashboardIntroRow(Rows rows, String title, String description) {
		...
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardKehadiranMahasiswa.java:573");
		}
