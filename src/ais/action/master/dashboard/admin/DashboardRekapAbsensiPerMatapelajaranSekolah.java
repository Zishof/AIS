...
/**
 * Rekap absensi <b>PER MATA PELAJARAN</b> versi <b>SEKOLAH</b> (berbasis
 * JadwalPelajaran / Guru / MataPelajaran). Padanan sekolah dari versi
 * perkuliahan (Per Matakuliah).
 *
 * <p>
 * Memakai ulang filter &amp; join {@link DashboardRekapAbsensiSiswa}
 * ({@code generateWhere(...)}). Kehadiran dihitung per JADWAL lalu DIAGREGASI
 * PER MATA PELAJARAN (gabungan seluruh kelas/siswa), sehingga setiap baris
 * ringkasan = satu mata pelajaran.
 * </p>
 */
public class DashboardRekapAbsensiPerMatapelajaranSekolah extends MyWindow {

	private static final long serialVersionUID = 790038368339375117L;
	...
	public DashboardRekapAbsensiPerMatapelajaranSekolah() {
		super();
		try {
			init();
			initYayasan();
	...
	public DashboardRekapAbsensiPerMatapelajaranSekolah(String title, String border, boolean closable) {
	...
	private void initSpreadsheet() {
		...
		Common.createDefaultTimer(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				Common.clear(center);
				String tahunAjaran = (String) (DashboardRekapAbsensiPerMatapelajaranSekolah.this.tahunAjaran
						.getSelectedItem() == null ? null
								: DashboardRekapAbsensiPerMatapelajaranSekolah.this.tahunAjaran.getSelectedItem()
										.getValue());
				...
				sql = "select max(matapelajaran) matapelajaran,\n"
						+ "sum(hadir) hadir, sum(alpa) alpa, sum(sakit) sakit, sum(izin) izin, kode_mapel\n"
						+ "from (" + sql
						+ ") aaa group by kode_mapel \n having sum(hadir+alpa+sakit+izin)>0\norder by max(matapelajaran)";
				...
				new org.zkoss.zul.Html(
						ais.action.master.dashboard.helper.DashboardVisualHelper.kehadiran(sekolahs, 0, 1, 2, 3, 4))
				...
				ais.ui.util.EcampusUtil.setCellValue(sheet, 1, 0,
						"REKAPITULASI ABSENSI PER MATA PELAJARAN \n " + "Yayasan" + " "
				...
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 0, "Matapelajaran");
				Utils.setColumnWidth(sheet, 0, 260);
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 1, Common.getBahasaConfig("Hadir"));
				Utils.setColumnWidth(sheet, 1, 90);
				...
				Utils.setColumnWidth(sheet, 5, 110);
				...
				for (Object[] objects : sekolahs) {
					...
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapAbsensiPerMatapelajaranSekolah.java:445");
					}
				...
				try {
					ais.ui.util.EcampusUtil.setBold(sheet,
							...
							true);
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapAbsensiPerMatapelajaranSekolah.java:468");
				}
			}
		});
