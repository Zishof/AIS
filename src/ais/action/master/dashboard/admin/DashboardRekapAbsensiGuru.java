...
/**
 * Komponen dashboard khusus untuk dashboard rekap absensi guru. Kelas ini memilih variasi data
 ...
 * center}, {@code JadwalPelajaran selectedjadwalPelajaran}; inisialisasi/lifecycle ({@code init()}, {@code
 ...
 * @see MyWindow
 */
public class DashboardRekapAbsensiGuru extends MyWindow {
	...
	public DashboardRekapAbsensiGuru(JadwalPelajaran jadwalPelajaran) {
		super();
	...
	public DashboardRekapAbsensiGuru(String title, String border, boolean closable) {
	...
	private void initSpreadsheet() {
		...
		Common.createDefaultTimer(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				Common.clear(center);
				// Jadwal pelajaran belum dipilih -> hindari NPE selectedjadwalPelajaran.getId().
				if (selectedjadwalPelajaran == null || selectedjadwalPelajaran.getId() == null) {
					return;
				}
				...
				String sql = "select \n"
						+ "'Guru Utama' as jenis, a.nama as siswa,sum(case when absensi ilike '%'||a.id||',1%' then 1 else 0 end) as hadir, \n"
						...
						"from sekolah.guru a  \ninner join jadwalPelajaran d on (d.guru1=a.id) \n"
						+ "inner join pertemuan c on (c.jadwalPelajaran=d.id) \n" +

						"where d.id="
						+ selectedjadwalPelajaran.getId() + "  \ngroup by a.id \n" +

						"union all \n" +

						"select \n"
						+ "'Guru II' as jenis, a.nama as siswa,sum(case when absensi ilike '%'||a.id||',1%' then 1 else 0 end) as hadir, \n"
						+ "sum(case when absensi ilike '%'||a.id||',2%' then 1 else 0 end) as alpa, \n"
						+ "sum(case when absensi ilike '%'||a.id||',3%' then 1 else 0 end) as sakit, \n"
						+ "sum(case when absensi ilike '%'||a.id||',4%' then 1 else 0 end) as izin, \n"
						+ "(a.id) as kode_jadwalPelajaran   \n" +

						"from sekolah.guru a  \ninner join jadwalPelajaran d on (d.guru2=a.id) \n"
						+ "inner join pertemuan c on (c.jadwalPelajaran=d.id) \n" +
						...
						"select \n"
						+ "'Guru III' as jenis, a.nama as siswa,sum(case when absensi ilike '%'||a.id||',1%' then 1 else 0 end) as hadir, \n"
						+ "sum(case when absensi ilike '%'||a.id||',2%' then 1 else 0 end) as alpa, \n"
						+ "sum(case when absensi ilike '%'||a.id||',3%' then 1 else 0 end) as sakit, \n"
						+ "sum(case when absensi ilike '%'||a.id||',4%' then 1 else 0 end) as izin, \n"
						+ "(a.id) as kode_jadwalPelajaran   \n" +

						"from sekolah.guru a  \ninner join jadwalPelajaran d on (d.guru3=a.id) \n"
						+ "inner join pertemuan c on (c.jadwalPelajaran=d.id) \n" +
						...
						"select \n"
						+ "'Guru IV' as jenis, a.nama as siswa,sum(case when absensi ilike '%'||a.id||',1%' then 1 else 0 end) as hadir, \n"
						+ "sum(case when absensi ilike '%'||a.id||',2%' then 1 else 0 end) as alpa, \n"
						+ "sum(case when absensi ilike '%'||a.id||',3%' then 1 else 0 end) as sakit, \n"
						+ "sum(case when absensi ilike '%'||a.id||',4%' then 1 else 0 end) as izin, \n"
						+ "(a.id) as kode_jadwalPelajaran   \n" +

						"from sekolah.guru a  \ninner join jadwalPelajaran d on (d.guru4=a.id) \n"
						+ "inner join pertemuan c on (c.jadwalPelajaran=d.id) \n" +
						...
						"select \n"
						+ "'Guru V' as jenis, a.nama as siswa,sum(case when absensi ilike '%'||a.id||',1%' then 1 else 0 end) as hadir, \n"
						+ "sum(case when absensi ilike '%'||a.id||',2%' then 1 else 0 end) as alpa, \n"
						+ "sum(case when absensi ilike '%'||a.id||',3%' then 1 else 0 end) as sakit, \n"
						+ "sum(case when absensi ilike '%'||a.id||',4%' then 1 else 0 end) as izin, \n"
						+ "(a.id) as kode_jadwalPelajaran   \n" +

						"from sekolah.guru a  \ninner join jadwalPelajaran d on (d.guru5=a.id) \n"
						+ "inner join pertemuan c on (c.jadwalPelajaran=d.id) \n" +
				...
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 1, "Guru");
				...
				for (Object[] objects : jurusans) {
					...
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapAbsensiGuru.java:279");
					}
				...
				try {
					ais.ui.util.EcampusUtil.setBold(sheet,
							...
							true);
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapAbsensiGuru.java:302");
				}
			}
		});
