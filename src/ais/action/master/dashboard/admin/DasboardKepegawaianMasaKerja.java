...
/**
 * Komponen dashboard khusus untuk dasboard kepegawaian masa kerja. Kelas ini memilih variasi data
 ...
 * @see MyWindow
 */
public class DasboardKepegawaianMasaKerja extends MyWindow {
	...
	public DasboardKepegawaianMasaKerja() {
		super();
		try {
	...
	public DasboardKepegawaianMasaKerja(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Sebaran Pegawai per Masa Kerja",
				"Jumlah pegawai menurut masa kerja, beserta grafiknya.");
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DasboardKepegawaianMasaKerja.this.grid);
			}
		});
	}
	...
	private void reload() {
		...
		usias.add(new CommonVO("0", "<5"));
		usias.add(new CommonVO("1", ">=5"));
		usias.add(new CommonVO("2", ">=10"));
		usias.add(new CommonVO("3", ">=15"));
		usias.add(new CommonVO("4", ">=20"));
		usias.add(new CommonVO("5", ">=25"));
		usias.add(new CommonVO("6", ">=30"));
		usias.add(new CommonVO("7", ">=35"));
		usias.add(new CommonVO("8", ">=40"));
		usias.add(new CommonVO("9", ">=45"));
		usias.add(new CommonVO("10", "Tidak Ditentukan"));
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			for (CommonVO usia : usias) {
				...
				if (usia.getId().equals("0")) {
					mulai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 5);
				} else if (usia.getId().equals("1")) {
					sampai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 5);
					mulai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 10);
				} else if (usia.getId().equals("2")) {
					sampai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 10);
					mulai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 15);
				} else if (usia.getId().equals("3")) {
					sampai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 15);
					mulai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 20);
				} else if (usia.getId().equals("4")) {
					sampai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 20);
					mulai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 25);
				} else if (usia.getId().equals("5")) {
					sampai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 25);
					mulai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 30);
				} else if (usia.getId().equals("6")) {
					sampai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 30);
					mulai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 35);
				} else if (usia.getId().equals("7")) {
					sampai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 35);
					mulai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 40);
				} else if (usia.getId().equals("8")) {
					sampai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 40);
					mulai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 45);
				} else if (usia.getId().equals("9")) {
					sampai.set(Calendar.YEAR, mulai.get(Calendar.YEAR) - 45);
				...
				Integer count = ((Number) HibernateUtil.currentSession().createCriteria(Pegawai.class)
						...
						.add(usia.getId().equals("10") ? Restrictions.isNull("awalmasuk")
								: Restrictions.between("awalmasuk", mulai.getTime(), sampai.getTime()))
																...
																|| tipeMasaKerja.getSelectedItem().getValue() == null
																		...
																		: Restrictions.eq("tipeMasaKerja",
																				tipeMasaKerja.getSelectedItem()
																						.getValue()))
														.add(usia.getId().equals("10")
																? Restrictions.isNull("awalmasuk")
																: Restrictions.between("awalmasuk", mulai.getTime(),
																		sampai.getTime()));
		...
		row.appendChild(DashboardModernHtmlUtil.createAnyChart(categoryModel, "Dasbor Kepegawaian Masa Kerja", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
}
}
