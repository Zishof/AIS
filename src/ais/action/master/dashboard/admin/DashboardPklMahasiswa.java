...
/**
 * Komponen dashboard khusus untuk dashboard pkl mahasiswa. Kelas ini memilih variasi data atau
 ...
 * @see MyWindow
 */
public class DashboardPklMahasiswa extends MyWindow {
	...
	public DashboardPklMahasiswa(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"PKL Mahasiswa",
				"Rekap peserta PKL per program studi, beserta grafiknya.");
		...
		row.appendChild(new MyLabelConfig("PKL"));
		row.appendChild(formatNilai);
		Common.insertComboDanSemua(formatNilai, "nama", Pkl.class);
		...
		MyComboitemConfig comboitem = new MyComboitemConfig("Telah Disetujui");
	...
	private void reload() {
		...
		final Pkl pkl = (Pkl) (formatNilai.getSelectedItem() == null || formatNilai.getSelectedItem().getValue() == null
				? null
		...
		final Boolean status = (Boolean) (searchreqstatus.getSelectedItem() == null ? null
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
						...
						a.addEventListener("onClick", new EventListener() {
							...
							public void onEvent(Event arg0) throws Exception {
								...
								EventListener eventListener = (EventListener) Common.cetakDataCustomButton(
										MahasiswaDapatKelompokPkl.class, new DataCriteriaWithColumn() {
												...
												try {
													...
													Criteria criteria = HibernateUtil.currentSession()
															...
															.add(status == null ? Restrictions.sqlRestriction("1=1")
																	: Restrictions.eq("diterima", status))
															.createAlias("kelompokPkl", "kelompokPkl")
															.createAlias("kelompokPkl.pkl", "pkl")
															.createAlias("mahasiswa", "mahasiswa")
															.add(pkl == null ? Restrictions.sqlRestriction("true")
																	: Restrictions.eq("kelompokPkl.pkl", pkl))

															.add(Restrictions.eq("mahasiswa.jurusan", jurusan))
															.add(Restrictions.eq("pkl.tahunAkademik", tahunAjaran));

													return new Object[] { criteria,
															DashboardStatistikPengajuanPklPerJurusan.contents };
				...
				row.appendChild(DashboardModernHtmlUtil.createAnyChart(categoryModel, "Dasbor Pkl Mahasiswa", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
}
		});
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				for (final Jurusan jurusan : jurusans) {
					...
					i++;
					List<Object[]> data = session.createCriteria(MahasiswaDapatKelompokPkl.class)
							.add(status == null ? Restrictions.sqlRestriction("1=1")
									: Restrictions.eq("diterima", status))
							...
							.add(Restrictions.eq("mahasiswa.jurusan", jurusan))
							.add(Restrictions.in("pkl.tahunAkademik", tas))

							.setProjection(Projections.projectionList().add(Projections.rowCount())
									.add(Projections.groupProperty("pkl.tahunAkademik")))
							.list();
