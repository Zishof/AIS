...
/**
 * Komponen dashboard khusus untuk dasboard kepegawaian agama. Kelas ini memilih variasi data atau
 ...
 * @see MyWindow
 */
public class DasboardKepegawaianAgama extends MyWindow {
	...
	public DasboardKepegawaianAgama() {
		super();
		try {
	...
	public DasboardKepegawaianAgama(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Sebaran Pegawai per Agama",
				"Jumlah pegawai menurut agama, beserta grafiknya.");
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DasboardKepegawaianAgama.this.grid);
			}
		});
	}
	...
	private void reload() {
		...
		List<Agama> agamas = ConstantValues.simpleList(HibernateUtil.currentSession().createCriteria(Agama.class)
				.add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))).addOrder(Order.asc("nama")), Agama.class);

		agamas.add(null);
		Map<Long, MyColumnConfig> listCols = new HashMap<Long, MyColumnConfig>();
		for (Agama agama : agamas) {
			column = new MyColumnConfig(agama == null ? "Tidak ditentukan" : agama.getNama());
			column.setWidth("5%");
			column.setAlign("right");
			listCols.put(agama == null || agama.getId() == null ? -1L : agama.getId(), column);
		}
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			for (Agama agama : agamas) {
				Integer count = ((Number) HibernateUtil.currentSession().createCriteria(Pegawai.class)
						...
						.add(agama == null ? Restrictions.isNull("agama") : Restrictions.eq("agama", agama))
				...
				data.add(count);
				Integer colCount = listTotals.get(agama == null || agama.getId() == null ? -1L : agama.getId());
				if (colCount == null) {
					colCount = 0;
				}
				colCount += count;
				listTotals.put(agama == null || agama.getId() == null ? -1L : agama.getId(), colCount);
				...
				if (colCount > 0) {
					...
					if (count > 0) {
						categoryModel.setValue(satuanKerja == null ? "Tidak Ditentukan" : satuanKerja.getNama(),
								agama == null ? "Tidak ditentukan" : agama.getNama(), count);
					}
					A a = new A(count + "");
										...
										public Object[] initCriteria(boolean order) {
											...
											try {
												...
												Criteria criteria = HibernateUtil.currentSession()
														...
														.add(satuanKerja == null ? Restrictions.isNull("satuanKerja")
																: Restrictions.eq("satuanKerja", satuanKerja))
														.add(agama == null ? Restrictions.isNull("agama")
																: Restrictions.eq("agama", agama))
														.add(tipeMasaKerja.getSelectedItem() == null
																|| tipeMasaKerja.getSelectedItem().getValue() == null
																		? Restrictions.sqlRestriction("true")
		...
		for (final Agama agama : agamas) {
			...
			if (colCount > 0) {
				countCol++;
				listCols.get(agama == null || agama.getId() == null ? -1L : agama.getId()).setParent(columns);
		...
		row.appendChild(DashboardModernHtmlUtil.createAnyChart(categoryModel, "Dasbor Kepegawaian Agama", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
}
}
