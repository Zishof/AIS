...
/**
 * Komponen dashboard khusus untuk dasboard kepegawaian tipe pegawai. Kelas ini memilih variasi
 ...
 * @see MyWindow
 */
public class DasboardKepegawaianTipePegawai extends MyWindow {
	...
	public DasboardKepegawaianTipePegawai() {
		super();
		try {
	...
	public DasboardKepegawaianTipePegawai(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Sebaran Pegawai per Tipe Pegawai",
				"Jumlah pegawai menurut tipe pegawai, beserta grafiknya.");
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DasboardKepegawaianTipePegawai.this.grid);
			}
		});
	}
	...
	private void reload() {
		...
		List<TipePegawai> tipePegawais = ConstantValues.simpleList(HibernateUtil.currentSession()
				.createCriteria(TipePegawai.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))).addOrder(Order.asc("nama")),
				TipePegawai.class);

		tipePegawais.add(null);
		Map<Long, MyColumnConfig> listCols = new HashMap<Long, MyColumnConfig>();
		for (TipePegawai tipePegawai : tipePegawais) {
			column = new MyColumnConfig(tipePegawai == null ? "Tidak ditentukan" : tipePegawai.getNama());
			column.setWidth("5%");
			column.setAlign("right");
			listCols.put(tipePegawai == null || tipePegawai.getId() == null ? -1L : tipePegawai.getId(), column);
		}
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			for (TipePegawai tipePegawai : tipePegawais) {
				Integer count = ((Number) HibernateUtil.currentSession().createCriteria(Pegawai.class)
						...
						.add(tipePegawai == null ? Restrictions.isNull("tipePegawai")
								: Restrictions.eq("tipePegawai", tipePegawai))
				...
				data.add(count);
				Integer colCount = listTotals.get(tipePegawai == null || tipePegawai.getId() == null ? -1L : tipePegawai.getId());
				if (colCount == null) {
					colCount = 0;
				}
				colCount += count;
				listTotals.put(tipePegawai == null || tipePegawai.getId() == null ? -1L : tipePegawai.getId(), colCount);
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			int jml = 0;
			int i = 0;
			for (final TipePegawai tipePegawai : tipePegawais) {
				Integer colCount = listTotals.get(tipePegawai == null || tipePegawai.getId() == null ? -1L : tipePegawai.getId());
				if (colCount == null) {
					colCount = 0;
				}
				if (colCount > 0) {
					...
					if (count > 0) {
						categoryModel.setValue(satuanKerja == null ? "Tidak Ditentukan" : satuanKerja.getNama(),
								tipePegawai == null ? "Tidak ditentukan" : tipePegawai.getNama(), count);
					}
					A a = new A(count + "");
		...
		for (final TipePegawai tipePegawai : tipePegawais) {
			...
			if (colCount > 0) {
				countCol++;
				listCols.get(tipePegawai == null || tipePegawai.getId() == null ? -1L : tipePegawai.getId()).setParent(columns);
		...
		row.appendChild(DashboardModernHtmlUtil.createAnyChart(categoryModel, "Dasbor Kepegawaian Tipe Pegawai", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
}
}
