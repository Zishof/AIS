...
/**
 * Rekap absensi <b>PER SISWA dan MATA PELAJARAN</b> versi <b>SEKOLAH</b>
 * (berbasis JadwalPelajaran / Guru / MataPelajaran). Padanan sekolah dari versi
 * perkuliahan {@link DashboardRekapAbsensiMahasiswa} (Per Mahasiswa dan Matakuliah).
 *
 * <p>
 * Memakai ulang filter &amp; join {@link DashboardRekapAbsensiSiswa}
 * ({@code generateWhere(...)}). Kehadiran dihitung per (siswa &times; jadwal),
 * lalu DIAGREGASI PER (SISWA &times; MATA PELAJARAN) sehingga beberapa jadwal
 * paralel dari mata pelajaran yang sama digabung. Berbeda dari tab "Kehadiran
 * Siswa" yang menampilkan rincian per jadwal.
 * </p>
 */
public class DashboardRekapAbsensiPerSiswaDanMapel extends MyWindow {

	private static final long serialVersionUID = 790038368339375116L;
	...
	public DashboardRekapAbsensiPerSiswaDanMapel() {
		super();
		try {
			init();
			initYayasan();
	...
	public DashboardRekapAbsensiPerSiswaDanMapel(String title, String border, boolean closable) {
	...
	private void initSpreadsheet() {
		...
		Common.createDefaultTimer(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				Common.clear(center);
				String tahunAjaran = (String) (DashboardRekapAbsensiPerSiswaDanMapel.this.tahunAjaran
						.getSelectedItem() == null ? null
								: DashboardRekapAbsensiPerSiswaDanMapel.this.tahunAjaran.getSelectedItem().getValue());
				...
				sql = "select max(siswa) siswa, max(matapelajaran) matapelajaran,\n"
						+ "sum(hadir) hadir, sum(alpa) alpa, sum(sakit) sakit, sum(izin) izin, kode_siswa, kode_mapel\n"
						+ "from (" + sql
						+ ") aaa group by kode_siswa, kode_mapel \n having sum(hadir+alpa+sakit+izin)>0\norder by max(siswa), max(matapelajaran)";
				...
				new org.zkoss.zul.Html(
						ais.action.master.dashboard.helper.DashboardVisualHelper.kehadiran(sekolahs, 0, 2, 3, 4, 5))
				...
				ais.ui.util.EcampusUtil.setCellValue(sheet, 1, 0,
						"REKAPITULASI ABSENSI PER SISWA & MATA PELAJARAN \n " + "Yayasan" + " "
				...
				for (Object[] objects : sekolahs) {
					...
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapAbsensiPerSiswaDanMapel.java:449");
					}
				...
				try {
					ais.ui.util.EcampusUtil.setBold(sheet,
							...
							true);
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapAbsensiPerSiswaDanMapel.java:472");
				}
			}
		});
