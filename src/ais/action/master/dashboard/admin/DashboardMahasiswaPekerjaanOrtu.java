...
/**
 * Komponen dashboard khusus untuk dashboard mahasiswa pekerjaan ortu. Kelas ini memilih variasi
 ...
 * @see MyWindow
 */
public class DashboardMahasiswaPekerjaanOrtu extends MyWindow {
	...
	public DashboardMahasiswaPekerjaanOrtu() {
		super();
		try {
	...
	public DashboardMahasiswaPekerjaanOrtu(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Mahasiswa per Pekerjaan Orang Tua",
				"Sebaran mahasiswa menurut pekerjaan orang tua, beserta grafiknya.");
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DashboardMahasiswaPekerjaanOrtu.this.grid);
			}
		});
	...
	private void reload() {
		...
		final List<Pekerjaan> pekerjaans = HibernateUtil.currentSession().createCriteria(Pekerjaan.class).list();
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				column.setParent(columns);
				column = new MyColumnConfig("Pekerjaan");
				column.setParent(columns);

				Rows rows = new Rows();
				rows.setParent(grid);

				/* Donat DISTRIBUSI pekerjaan orang tua (ayah/ibu/wali) lintas prodi dari data
				 * yang SUDAH dimuat — tanpa query tambahan; via DashboardUiKit.donut (HTML/CSS). */
				java.util.LinkedHashMap<String, Double> distPekerjaan = new java.util.LinkedHashMap<String, Double>();
				for (Jurusan jurAgg : jurusans) {
					List<BiodataMahasiswa> dataAgg = datas.get(jurAgg.getId());
					if (dataAgg == null) {
						continue;
					}
					for (Pekerjaan pkAgg : pekerjaans) {
						int n = 0;
						for (BiodataMahasiswa o : dataAgg) {
							if (o != null && ((o.getJenisPekerjaanAyah() != null
									&& o.getJenisPekerjaanAyah().getId().equals(pkAgg.getId()))
									|| (o.getJenisPekerjaanIbu() != null
											&& o.getJenisPekerjaanIbu().getId().equals(pkAgg.getId()))
									|| (o.getJenisPekerjaanWali() != null
											&& o.getJenisPekerjaanWali().getId().equals(pkAgg.getId())))) {
								n++;
							}
						}
						if (n > 0) {
							Double pv = distPekerjaan.get(pkAgg.getNama());
							distPekerjaan.put(pkAgg.getNama(), (pv == null ? 0.0 : pv) + n);
						}
					}
				}
				if (!distPekerjaan.isEmpty()) {
					center.appendChild(new org.zkoss.zul.Html(ais.ui.util.DashboardUiKit.donut(
							"Distribusi Pekerjaan Orang Tua",
							"Sebaran pekerjaan orang tua (ayah/ibu/wali) dari seluruh program studi sesuai saringan.",
							distPekerjaan, false, "Belum ada data untuk ditampilkan.")));
				}
		...
		row.setValign("top");
					...
					for (final Pekerjaan pekerjaan : pekerjaans) {
						...
						for (BiodataMahasiswa o : data) {
							...
							if (o != null && ((o.getJenisPekerjaanAyah() != null
									&& o.getJenisPekerjaanAyah().getId().equals(pekerjaan.getId())) ||

									(o.getJenisPekerjaanIbu() != null
											&& o.getJenisPekerjaanIbu().getId().equals(pekerjaan.getId()))

									||

									(o.getJenisPekerjaanWali() != null
											&& o.getJenisPekerjaanWali().getId().equals(pekerjaan.getId()))
						...
						if (jumlah > 0) {
							...
							A a = new A(pekerjaan.getNama() + ":" + Common.numberFormat.get().format(jumlah) + ", ");
												...
												public Object[] initCriteria(boolean order) {
													...
													try {
														...
														Criteria criteria = HibernateUtil.currentSession()
																...
																.add(Restrictions.or(
																		Restrictions.eq("jenisPekerjaanWali",
																				pekerjaan),
																		Restrictions.or(
																				Restrictions.eq("jenisPekerjaanAyah",
																						pekerjaan),
																				Restrictions.eq("jenisPekerjaanIbu",
																						pekerjaan))))
														...
														return new Object[] { criteria, new String[] { "mahasiswa.nim",
																...
																"jenisPekerjaanAyah.nama", "jenisPekerjaanIbu.nama",
																"jenisPekerjaanWali.nama", "alamat", "rt", "rw",
