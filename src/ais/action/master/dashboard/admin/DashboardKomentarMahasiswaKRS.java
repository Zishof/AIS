...
/**
 * Komponen dashboard khusus untuk dashboard komentar mahasiswa krs. Kelas ini memilih variasi data
 ...
 * @see MyWindow
 */
public class DashboardKomentarMahasiswaKRS extends MyWindow {
	...
	public DashboardKomentarMahasiswaKRS() {
		super();
		try {
	...
	public DashboardKomentarMahasiswaKRS(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		print.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardKomentarMahasiswaKRS.java:349");
	...
	private void initSpreadsheet() throws Exception {
		...
		Common.createDefaultTimer(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				Common.clear(center);
				final String tahunAkademik = (String) (DashboardKomentarMahasiswaKRS.this.tahunAkademik
						.getSelectedItem() == null ? null
								: DashboardKomentarMahasiswaKRS.this.tahunAkademik.getSelectedItem().getValue());
				...
				final Label label = Common.displayLoadBar(DashboardKomentarMahasiswaKRS.this, file, center, sizedata);
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						rowhead.createCell(2).setCellValue("KOMENTAR");
						...
						List<Komentar> komentars = session.createCriteria(Komentar.class)
								.createAlias("mahasiswa", "mhs").add(Restrictions.sqlRestriction("mhs1_.id in " + sql))

								.add(dosen != null ? Restrictions.eq("mhs.dosen", dosen.getId())
										: Restrictions.sqlRestriction("1=1"))

								.createAlias("mhs.jurusan", "jurusan").addOrder(Order.desc("id"))
								...
								.add(jurusan == null ? Restrictions.sqlRestriction("1=1")
										: Restrictions.eq("mhs.jurusan", jurusan))

								.add(program == null ? Restrictions.sqlRestriction("1=1")
										: Restrictions.eq("mhs.program", program))

								.add(angkatan == null ? Restrictions.sqlRestriction("1=1")
										: Restrictions.eq("mhs.tahunangkatan", angkatan))

								.list();

						int size = komentars.size();
						...
						for (Komentar komentar : komentars) {
							...
							String kom = komentar.getKomentar() + " (" + komentar.getOleh() + " - "
									+ Common.dateFormat3.get().format(komentar.getTanggal()) + ")";

							kom = StringUtils.remove(kom, "<p>");
							kom = StringUtils.remove(kom, "</p>");

							cell = row.createCell(2);
							cell.setCellValue(kom);
