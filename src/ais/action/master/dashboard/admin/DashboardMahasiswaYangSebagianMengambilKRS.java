...
/**
 * Komponen dashboard khusus untuk dashboard mahasiswa yang sebagian mengambil krs. Kelas ini
 ...
 * @see MyWindow
 */
public class DashboardMahasiswaYangSebagianMengambilKRS extends MyWindow {
	...
	public DashboardMahasiswaYangSebagianMengambilKRS() {
		super();
		try {
	...
	public DashboardMahasiswaYangSebagianMengambilKRS(String title, String border, boolean closable) {
			...
			public void onEvent(Event event) throws Exception {
				try {
					Filedownload.save(new FileInputStream(file), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
							"KRS_SEBAGIAN_DISETUJUI_SEMUA.xlsx");
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardMahasiswaYangSebagianMengambilKRS.java:316");
	...
	private void initSpreadsheet() throws Exception {
		...
		Common.createDefaultTimer(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				Common.clear(center);
				final String tahunAkademik = (String) (DashboardMahasiswaYangSebagianMengambilKRS.this.tahunAkademik
						.getSelectedItem() == null ? null
								: DashboardMahasiswaYangSebagianMengambilKRS.this.tahunAkademik.getSelectedItem()
										.getValue());
				...
				final Label label = Common.displayLoadBar(DashboardMahasiswaYangSebagianMengambilKRS.this, file, center,
						sizedata);
				...
				new Thread(new Runnable() {
					...
					public void run() {
						...
						String sql = " (select aa.mahasiswa "
								...
								+ " group by aa.mahasiswa "
								+ " having sum(case when aa.persetujuan = 1 then 1 else 0 end) > 0 "
								+ " and sum(case when aa.persetujuan = 0 then 1 else 0 end) > 0  ) ";
