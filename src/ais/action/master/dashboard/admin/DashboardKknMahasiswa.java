...
/**
 * Komponen dashboard khusus untuk dashboard kkn mahasiswa. Kelas ini memilih variasi data atau
 ...
 * @see MyWindow
 */
public class DashboardKknMahasiswa extends MyWindow {
	...
	public DashboardKknMahasiswa(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"KKN Mahasiswa",
				"Rekap peserta KKN per program studi, beserta grafiknya.");
		...
		row.appendChild(new MyLabelConfig("KKN"));
		row.appendChild(formatNilai);
		Common.insertComboDanSemua(formatNilai, "nama", Kkn.class);
	...
	private void reload() {
		...
		final Kkn kkn = (Kkn) (formatNilai.getSelectedItem() == null || formatNilai.getSelectedItem().getValue() == null
				? null
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
						...
						a.addEventListener("onClick", new EventListener() {
							...
							public void onEvent(Event arg0) throws Exception {
								...
								EventListener eventListener = (EventListener) Common.cetakDataCustomButton(
										MahasiswaDapatKelompokKkn.class, new DataCriteriaWithColumn() {
											...
											public Object[] initCriteria(boolean order) {
												...
												try {
													...
													Criteria criteria = HibernateUtil.currentSession()
															...
															.add(kkn == null ? Restrictions.sqlRestriction("true")
																	: Restrictions.eq("kelompokKkn.kkn", kkn))

															.add(Restrictions.eq("mahasiswa.jurusan", jurusan))
															.add(Restrictions.eq("kkn.tahunAkademik", tahunAjaran));

													return new Object[] { criteria,
															DashboardStatistikPengajuanKknPerJurusan.contents };
				...
				row.appendChild(DashboardModernHtmlUtil.createAnyChart(categoryModel, "Dasbor Kkn Mahasiswa", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
}
		});
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				for (final Jurusan jurusan : jurusans) {
					...
					i++;
					List<Object[]> data = session.createCriteria(MahasiswaDapatKelompokKkn.class)
							.add(status == null ? Restrictions.sqlRestriction("1=1")
									: Restrictions.eq("diterima", status))
							...
							.add(Restrictions.eq("mahasiswa.jurusan", jurusan))
							.add(Restrictions.in("kkn.tahunAkademik", tas))

							.setProjection(Projections.projectionList().add(Projections.rowCount())
									.add(Projections.groupProperty("kkn.tahunAkademik")))
							.list();
