...
/**
 * Komponen dashboard khusus untuk dashboard bimbingan mahasiswa. Kelas ini memilih variasi data
 ...
 * {@code Combobox formatNilai}, {@code Center center}, {@code Combobox searchfakultas}, {@code Combobox
 ...
 * @see MyWindow
 */
public class DashboardBimbinganMahasiswa extends MyWindow {
	...
	public DashboardBimbinganMahasiswa() {
		super();
		try {
	...
	public DashboardBimbinganMahasiswa(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		MyTabConfig tab1 = new MyTabConfig("Bimbingan");
		tab1.setParent(tabs);

		MyTabConfig tab2 = new MyTabConfig("Sidang");
		tab2.setParent(tabs);

		MyTabConfig tab3 = new MyTabConfig("Wisuda");
		tab3.setParent(tabs);
		...
		tab2.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				if (tabpanel2.getChildren().size() == 0) {
					DashboardSidangMahasiswa dashboardSidangMahasiswa = new DashboardSidangMahasiswa();
					dashboardSidangMahasiswa.setHeight("100%");
					dashboardSidangMahasiswa.setWidth("100%");
					dashboardSidangMahasiswa.setParent(tabpanel2);
				}
			}
		});
		...
		tab3.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				if (tabpanel3.getChildren().size() == 0) {
					DashboardWisudaMahasiswa dashboardWisudaMahasiswa = new DashboardWisudaMahasiswa();
					dashboardWisudaMahasiswa.setHeight("100%");
					dashboardWisudaMahasiswa.setWidth("100%");
					dashboardWisudaMahasiswa.setParent(tabpanel3);
				}
			}
		});
		...
		Common.insertComboDanSemua(formatNilai, "nama", FormatNilaiProposalSkripsi.class,
	...
	private void reload() {
		...
		final FormatNilaiProposalSkripsi formatNilaiProposalSkripsi = (FormatNilaiProposalSkripsi) (formatNilai
				.getSelectedItem() == null || formatNilai.getSelectedItem().getValue() == null ? null
		...
		final String status = (String) (searchreqstatus.getSelectedItem() == null ? null
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
						...
						a.addEventListener("onClick", new EventListener() {
							...
							public void onEvent(Event arg0) throws Exception {
								...
								EventListener eventListener = (EventListener) Common.cetakDataCustomButton(
										MahasiswaRequestTugasAkhir.class, new DataCriteriaWithColumn() {
											...
											public Object[] initCriteria(boolean order) {
												...
												try {
													...
													Criteria criteria = HibernateUtil.currentSession()
															...
															.add(formatNilaiProposalSkripsi == null
																	? Restrictions.sqlRestriction("true")
																	: Restrictions.eq("formatNilaiProposalSkripsi",
													...
													return new Object[] { criteria,
															MahasiswaRequestTugasAkhirAction.contents };
				...
				row.appendChild(DashboardModernHtmlUtil.createAnyChart(categoryModel, "Dasbor Bimbingan Mahasiswa", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
}
		});
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				for (final Jurusan jurusan : jurusans) {
					...
					i++;
					List<Object[]> data = session.createCriteria(MahasiswaRequestTugasAkhir.class)
							.add(status == null ? Restrictions.sqlRestriction("1=1")
									: Restrictions.eq("status", status))
							.createAlias("mahasiswa", "mahasiswa")
							.add(formatNilaiProposalSkripsi == null ? Restrictions.sqlRestriction("true")
									: Restrictions.eq("formatNilaiProposalSkripsi", formatNilaiProposalSkripsi))
