...
/**
 * Komponen dashboard khusus untuk dashboard rekap absensi asisten dosen. Kelas ini memilih variasi
 ...
 * @see MyWindow
 */
public class DashboardRekapAbsensiAsistenDosen extends MyWindow {
	...
	public DashboardRekapAbsensiAsistenDosen(Perkuliahan perkuliahan) {
		super();
	...
	public DashboardRekapAbsensiAsistenDosen(String title, String border, boolean closable) {
	...
	private void initSpreadsheet() {
		...
		Common.createDefaultTimer(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				String sql = "select \n"
						+ "a.id as jenis, a.nama as mahasiswa,sum(case when absensi ilike '%'||a.id||',1%' then 1 else 0 end) as hadir, \n"
						...
						+ "inner join mahasiswa_jadi_asisten m on (d.id=m.perkuliahan) \n"
						+ "inner join mahasiswa a on (m.mahasiswa=a.id) \n" +

						"where d.id=" + selectedperkuliahan.getId() + "  \ngroup by a.id";
				...
				for (Object[] objects : jurusans) {
					if (objects[0] == null)
						continue;
					ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 0, "Asisten Dosen");
					...
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapAbsensiAsistenDosen.java:208");
					}
				...
				try {
					ais.ui.util.EcampusUtil.setBold(sheet,
							...
							true);
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapAbsensiAsistenDosen.java:231");
				}
			}
		});
