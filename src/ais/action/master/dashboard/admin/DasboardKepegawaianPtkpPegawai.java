...
/**
 * Komponen dashboard khusus untuk dasboard kepegawaian ptkp pegawai. Kelas ini memilih variasi
 ...
 * @see MyWindow
 */
public class DasboardKepegawaianPtkpPegawai extends MyWindow {
	...
	public DasboardKepegawaianPtkpPegawai() {
		super();
		try {
	...
	public DasboardKepegawaianPtkpPegawai(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Sebaran Pegawai per Status PTKP",
				"Jumlah pegawai menurut status PTKP, beserta grafiknya.");
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DasboardKepegawaianPtkpPegawai.this.grid);
			}
		});
	}
	...
	private void reload() {
		...
		List<PtkpPegawai> ptkpPegawais = ConstantValues.simpleList(HibernateUtil.currentSession()
				.createCriteria(PtkpPegawai.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))).addOrder(Order.asc("nama")),
				PtkpPegawai.class);

		ptkpPegawais.add(null);
		Map<Long, MyColumnConfig> listCols = new HashMap<Long, MyColumnConfig>();
		for (PtkpPegawai ptkpPegawai : ptkpPegawais) {
			column = new MyColumnConfig(ptkpPegawai == null ? "Tidak ditentukan" : ptkpPegawai.getNama());
			column.setWidth("5%");
			column.setAlign("right");
			listCols.put(ptkpPegawai == null || ptkpPegawai.getId() == null ? -1L : ptkpPegawai.getId(), column);
		}
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			for (PtkpPegawai ptkpPegawai : ptkpPegawais) {
				Integer count = ((Number) HibernateUtil.currentSession().createCriteria(Pegawai.class)
						...
						.add(ptkpPegawai == null ? Restrictions.isNull("ptkpPegawai")
								: Restrictions.eq("ptkpPegawai", ptkpPegawai))
				...
				data.add(count);
				Integer colCount = listTotals.get(ptkpPegawai == null || ptkpPegawai.getId() == null ? -1L : ptkpPegawai.getId());
				if (colCount == null) {
					colCount = 0;
				}
				colCount += count;
				listTotals.put(ptkpPegawai == null || ptkpPegawai.getId() == null ? -1L : ptkpPegawai.getId(), colCount);
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			int jml = 0;
			int i = 0;
			for (final PtkpPegawai ptkpPegawai : ptkpPegawais) {
				Integer colCount = listTotals.get(ptkpPegawai == null || ptkpPegawai.getId() == null ? -1L : ptkpPegawai.getId());
				if (colCount == null) {
					colCount = 0;
				}
				if (colCount > 0) {
					...
					if (count > 0) {
						categoryModel.setValue(satuanKerja == null ? "Tidak Ditentukan" : satuanKerja.getNama(),
								ptkpPegawai == null ? "Tidak ditentukan" : ptkpPegawai.getNama(), count);
					}
					A a = new A(count + "");
		...
		for (final PtkpPegawai ptkpPegawai : ptkpPegawais) {
			...
			if (colCount > 0) {
				countCol++;
				listCols.get(ptkpPegawai == null || ptkpPegawai.getId() == null ? -1L : ptkpPegawai.getId()).setParent(columns);
		...
		row.appendChild(DashboardModernHtmlUtil.createAnyChart(categoryModel, "Dasbor Kepegawaian Ptkp Pegawai", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
}
}
