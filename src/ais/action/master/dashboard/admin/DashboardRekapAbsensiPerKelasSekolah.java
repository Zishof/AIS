...
/**
 * Rekap absensi <b>PER KELAS</b> versi <b>SEKOLAH</b> (berbasis JadwalPelajaran /
 * Guru / MataPelajaran). Padanan sekolah dari versi perkuliahan
 * {@link DashboardRekapAbsensiPerKelas} (yang masih memakai Fakultas/Prodi/Dosen).
 *
 * <p>
 ...
 * PER KELAS (gabungan seluruh mata pelajaran kelas tersebut), sehingga setiap
 * baris ringkasan = satu kelas.
 * </p>
 */
public class DashboardRekapAbsensiPerKelasSekolah extends MyWindow {

	private static final long serialVersionUID = 790038368339375115L;
	...
	public DashboardRekapAbsensiPerKelasSekolah() {
		super();
		try {
			init();
			initYayasan();
	...
	public DashboardRekapAbsensiPerKelasSekolah(String title, String border, boolean closable) {
	...
	private void initSpreadsheet() {
		...
		Common.createDefaultTimer(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				Common.clear(center);
				String tahunAjaran = (String) (DashboardRekapAbsensiPerKelasSekolah.this.tahunAjaran
						.getSelectedItem() == null ? null
								: DashboardRekapAbsensiPerKelasSekolah.this.tahunAjaran.getSelectedItem().getValue());
				...
				sql = "select max(kelas) kelas,\n"
						+ "sum(hadir) hadir, sum(alpa) alpa, sum(sakit) sakit, sum(izin) izin, kode_kelas\n"
						+ "from (" + sql
						+ ") aaa group by kode_kelas \n having sum(hadir+alpa+sakit+izin)>0\norder by max(kelas)";
				...
				for (Object[] objects : sekolahs) {
					...
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapAbsensiPerKelasSekolah.java:444");
					}
				...
				try {
					ais.ui.util.EcampusUtil.setBold(sheet,
							...
							true);
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/admin/DashboardRekapAbsensiPerKelasSekolah.java:467");
				}
			}
		});
