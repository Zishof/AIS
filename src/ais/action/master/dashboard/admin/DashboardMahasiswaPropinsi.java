...
/**
 * Komponen dashboard khusus untuk dashboard mahasiswa propinsi. Kelas ini memilih variasi data
 ...
 * @see MyWindow
 */
public class DashboardMahasiswaPropinsi extends MyWindow {
	...
	public DashboardMahasiswaPropinsi() {
		super();
		try {
	...
	public DashboardMahasiswaPropinsi(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Mahasiswa per Provinsi",
				"Sebaran mahasiswa menurut provinsi asal, beserta grafiknya.");
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DashboardMahasiswaPropinsi.this.grid);
			}
		});
	...
	private void reload() {
		...
		final List<Long> propinsisId = HibernateUtil.currentSession().createCriteria(BiodataMahasiswa.class)
				.add(criteriaStatus).add(Restrictions.isNotNull("propinsi")).createAlias("mahasiswa", "mahasiswa")
				...
				.setProjection(Projections.groupProperty("propinsi.id"))
				...
				.add(jur == null ? Restrictions.sqlRestriction("true")
						: Restrictions.eq("jurusan.id", jur.getId()))
				...
				.addOrder(Order.asc("propinsi.id")).list();
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				column.setParent(columns);
				column = new MyColumnConfig("Propinsi");
				column.setParent(columns);
		...
		row.setValign("top");
					...
					for (final Long propinsiid : propinsisId) {
						final Propinsi propinsi = (Propinsi) ConstantValues.ambil(Propinsi.class.getName(), propinsiid);
						Number jumlah = 0;
						for (Object[] o : data) {
							Propinsi kec = (Propinsi) o[1];
							if (kec != null && kec.getId().equals(propinsi.getId())) {
								jumlah = (Number) o[0];
								break;
							}
						}
						...
						if (jumlah.intValue() > 0) {
							...
							A a = new A(propinsi.getNama() + ":" + Common.numberFormat.get().format(jumlah) + ", ");
												...
												public Object[] initCriteria(boolean order) {
													...
													try {
														...
														Criteria criteria = HibernateUtil.currentSession()
																...
																.add(criteriaStatus)
																.add(Restrictions.isNotNull("propinsi"))
																...
																.add(Restrictions.eq("propinsi", propinsi))
			...
			public void run() {
				...
				for (final Jurusan jurusan : jurusans) {
					...
					List<Object[]> data = session.createCriteria(BiodataMahasiswa.class).add(criteriaStatus)
							.add(Restrictions.isNotNull("propinsi")).createAlias("mahasiswa", "mahasiswa")
							...
							.setProjection(Projections.projectionList().add(Projections.rowCount())
									.add(Projections.groupProperty("propinsi")))
