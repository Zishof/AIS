...
/**
 * Komponen dashboard khusus untuk dashboard mahasiswa penghasilan ortu. Kelas ini memilih variasi
 ...
 * @see MyWindow
 */
public class DashboardMahasiswaPenghasilanOrtu extends MyWindow {
	...
	public DashboardMahasiswaPenghasilanOrtu() {
		super();
		try {
	...
	public DashboardMahasiswaPenghasilanOrtu(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		org.zkoss.zk.ui.Component[] hostPortal = ais.ui.util.DasborResponsifHelper.saringanDanIsi(this,
				...
				"Mahasiswa per Penghasilan Orang Tua",
				"Sebaran mahasiswa menurut penghasilan orang tua, beserta grafiknya.");
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DashboardMahasiswaPenghasilanOrtu.this.grid);
			}
		});
	...
	private void reload() {
		...
		final List<Penghasilan> penghasilans = HibernateUtil.currentSession().createCriteria(Penghasilan.class).list();
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				column.setParent(columns);
				column = new MyColumnConfig("Penghasilan");
				column.setParent(columns);

				// for (Penghasilan penghasilan :
				// penghasilans) {
				// column.setParent(columns);
				// column = new MyColumnConfig(
				// penghasilan.getNama().replaceAll("Kec.",
				// "").replaceAll("kec.",
				// "").trim());
		...
		row.setValign("top");
					...
					for (final Penghasilan penghasilan : penghasilans) {

						int jumlah = 0;
						for (BiodataMahasiswa o : data) {

							if (o != null && ((o.getJenisPenghasilanAyah() != null
									&& o.getJenisPenghasilanAyah().getId().equals(penghasilan.getId())) ||

									(o.getJenisPenghasilanIbu() != null
											&& o.getJenisPenghasilanIbu().getId().equals(penghasilan.getId()))

									||

									(o.getJenisPenghasilanWali() != null
											&& o.getJenisPenghasilanWali().getId().equals(penghasilan.getId()))
						...
						if (jumlah > 0) {
							...
							A a = new A(penghasilan.getNama() + ":" + Common.numberFormat.get().format(jumlah) + ", ");
												...
												public Object[] initCriteria(boolean order) {
													...
													try {
														...
														Criteria criteria = HibernateUtil.currentSession()
																...
																.add(Restrictions.or(
																		Restrictions.eq("jenisPenghasilanWali",
																				penghasilan),
																		Restrictions.or(
																				Restrictions.eq("jenisPenghasilanAyah",
																						penghasilan),
																				Restrictions.eq("jenisPenghasilanIbu",
																						penghasilan))))
														...
														return new Object[] { criteria, new String[] { "mahasiswa.nim",
																...
																"jenisPenghasilanAyah.nama", "jenisPenghasilanIbu.nama",
																"jenisPenghasilanWali.nama", "alamat", "rt", "rw",
