...
/**
 * Komponen dashboard khusus untuk dasboard surat keluar. Kelas ini memilih variasi data atau
 ...
 * satuanKerjaTreeModel}, {@code Grid grid}, {@code Combobox lokerSurat}, {@code MyDatebox start};
 ...
 * @see MyWindow
 */
public class DasboardSuratKeluar extends MyWindow {
	...
	private Grid grid;
	private Combobox lokerSurat;
	private MyDatebox start;
	private MyDatebox end;

	public DasboardSuratKeluar() {
		super();
		try {
	...
	public DasboardSuratKeluar(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		lokerSurat = new Combobox();
		row.appendChild(lokerSurat);
		Common.insertComboDanSemua(lokerSurat, "nama", "keterangan", LokerSurat.class,
				Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)));
		lokerSurat.setReadonly(true);
		lokerSurat.addEventListener("onChange", eventListener);
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DasboardSuratKeluar.this.grid);
			}
		});
	}
	...
	private void reload() {
		...
		List<KlasifikasiSuratKeluar> klasifikasiSuratKeluars = ConstantValues
				.simpleList(
						HibernateUtil.currentSession().createCriteria(KlasifikasiSuratKeluar.class)
		...
		klasifikasiSuratKeluars.add(null);
		...
		for (KlasifikasiSuratKeluar klasifikasiSuratKeluar : klasifikasiSuratKeluars) {
			MyColumnConfig column = new MyColumnConfig(
					klasifikasiSuratKeluar == null ? "Tidak ditentukan" : klasifikasiSuratKeluar.getNama());
			column.setTooltiptext(
					klasifikasiSuratKeluar == null ? "Tidak ditentukan" : klasifikasiSuratKeluar.getNama());
			column.setWidth("5%");
			column.setAlign("right");
			listCols.put(klasifikasiSuratKeluar == null || klasifikasiSuratKeluar.getId() == null ? -1L : klasifikasiSuratKeluar.getId(), column);
		}
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			for (KlasifikasiSuratKeluar klasifikasiSuratKeluar : klasifikasiSuratKeluars) {
				Integer count = ((Number) HibernateUtil.currentSession().createCriteria(SuratKeluar.class)
						...
						.add(klasifikasiSuratKeluar == null ? Restrictions.isNull("klasifikasiSuratKeluar")
								: Restrictions.eq("klasifikasiSuratKeluar", klasifikasiSuratKeluar))
						.add(lokerSurat.getSelectedItem() == null || lokerSurat.getSelectedItem().getValue() == null
								? Restrictions.sqlRestriction("true")
								: Restrictions.eq("lokerSurat", lokerSurat.getSelectedItem().getValue()))
						.uniqueResult()).intValue();
				data.add(count);
				Integer colCount = listTotals
						.get(klasifikasiSuratKeluar == null || klasifikasiSuratKeluar.getId() == null ? -1L : klasifikasiSuratKeluar.getId());
				if (colCount == null) {
					colCount = 0;
				}
				colCount += count;
				listTotals.put(klasifikasiSuratKeluar == null || klasifikasiSuratKeluar.getId() == null ? -1L : klasifikasiSuratKeluar.getId(), colCount);
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			int jml = 0;
			int i = 0;
			for (final KlasifikasiSuratKeluar klasifikasiSuratKeluar : klasifikasiSuratKeluars) {
				Integer colCount = listTotals
						.get(klasifikasiSuratKeluar == null || klasifikasiSuratKeluar.getId() == null ? -1L : klasifikasiSuratKeluar.getId());
				if (colCount == null) {
					colCount = 0;
				...
				if (colCount > 0) {
					...
					countData.add((klasifikasiSuratKeluar == null ? "" : "E" + klasifikasiSuratKeluar.getId()));
					...
					a.addEventListener("onClick", new EventListener() {
						...
						public void onEvent(Event arg0) throws Exception {
							EventListener eventListener = (EventListener) Common
									.cetakDataCustomButton(SuratKeluar.class, new DataCriteriaWithColumn() {
										...
										public Object[] initCriteria(boolean order) {
											...
											try {
												...
												Criteria criteria = HibernateUtil.currentSession()
														.createCriteria(SuratKeluar.class)

														.add(Restrictions
																.sqlRestriction("date(this_.tanggal) between date('"
														...
														.add(satuanKerja == null ? Restrictions.isNull("satuanKerja")
																: Restrictions.eq("satuanKerja", satuanKerja))
														.add(klasifikasiSuratKeluar == null
																? Restrictions.isNull("klasifikasiSuratKeluar")
																: Restrictions.eq("klasifikasiSuratKeluar",
																		klasifikasiSuratKeluar))
														.add(lokerSurat.getSelectedItem() == null
																|| lokerSurat.getSelectedItem().getValue() == null
																		? Restrictions.sqlRestriction("true")
																		: Restrictions.eq("lokerSurat", lokerSurat
																				.getSelectedItem().getValue()));

												return new Object[] { criteria, SuratKeluarAction.contents };
									...
									try {
										...
										Criteria criteria = HibernateUtil.currentSession()
												...
												.add(lokerSurat.getSelectedItem() == null
														|| lokerSurat.getSelectedItem().getValue() == null
																? Restrictions.sqlRestriction("true")
																: Restrictions.eq("lokerSurat",
																		lokerSurat.getSelectedItem().getValue()));
		...
		int totalCol = 0;
		for (final KlasifikasiSuratKeluar klasifikasiSuratKeluar : klasifikasiSuratKeluars) {
			Integer colCount = listTotals.get(klasifikasiSuratKeluar == null || klasifikasiSuratKeluar.getId() == null ? -1L : klasifikasiSuratKeluar.getId());
			if (colCount == null) {
				colCount = 0;
			}
			totalCol += colCount;
			if (colCount > 0) {
				countCol++;
				listCols.get(klasifikasiSuratKeluar == null || klasifikasiSuratKeluar.getId() == null ? -1L : klasifikasiSuratKeluar.getId()).setParent(contents);
						...
						.cetakDataCustomButton(SuratKeluar.class, new DataCriteriaWithColumn() {
							...
							public Object[] initCriteria(boolean order) {
								...
								try {
									...
									Criteria criteria = HibernateUtil.currentSession().createCriteria(SuratKeluar.class)
	...
	/**
	 * Pembawa data/helper lokal milik {@link DasboardSuratKeluar} untuk html category model. Tipe ini
	 * mengelompokkan nilai antara agar perhitungan atau rendering tidak memakai array/map tanpa kontrak yang
	 * jelas.
	 *
	 * <p><b>Scope:</b> tipe bersifat {@code static}; instance tidak menangkap object {@link DasboardSuratKeluar}.
