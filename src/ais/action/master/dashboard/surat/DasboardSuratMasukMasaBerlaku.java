...
/**
 * Komponen dashboard khusus untuk dasboard surat masuk masa berlaku. Kelas ini memilih variasi
 ...
 * @see MyWindow
 */
public class DasboardSuratMasukMasaBerlaku extends MyWindow {
	...
	public DasboardSuratMasukMasaBerlaku(String tipe) {
		super();
		this.tipe = tipe;
		try {
	...
	public DasboardSuratMasukMasaBerlaku(String tipe, String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DasboardSuratMasukMasaBerlaku.this.grid);
			}
		});
	}
	...
	private void reload() {
		...
		List<MasaBerlakuSurat> masaBerlakuSurats = ConstantValues
				.simpleList(
						HibernateUtil.currentSession().createCriteria(MasaBerlakuSurat.class)
								.add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))).addOrder(Order.asc("nama")),
						MasaBerlakuSurat.class);

		MyColumnConfig columnUtama = new MyColumnConfig("Satuan Kerja");
		columnUtama.setParent(contents);

		masaBerlakuSurats.add(null);
		Map<Long, MyColumnConfig> listCols = new HashMap<Long, MyColumnConfig>();
		for (MasaBerlakuSurat masaBerlakuSurat : masaBerlakuSurats) {
			MyColumnConfig column = new MyColumnConfig(
					masaBerlakuSurat == null ? "Tidak ditentukan" : masaBerlakuSurat.getNama());
			column.setTooltiptext(masaBerlakuSurat == null ? "Tidak ditentukan" : masaBerlakuSurat.getNama());
			column.setWidth("5%");
			column.setAlign("right");
			listCols.put(masaBerlakuSurat == null || masaBerlakuSurat.getId() == null ? -1L : masaBerlakuSurat.getId(), column);
		}
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			for (MasaBerlakuSurat masaBerlakuSurat : masaBerlakuSurats) {
				Integer count = ((Number) HibernateUtil.currentSession().createCriteria(SuratMasuk.class)
						...
						.add(masaBerlakuSurat == null ? Restrictions.isNull("klasifikasiSuratMasuk.masaBerlakuSurat")
								: Restrictions.eq("klasifikasiSuratMasuk.masaBerlakuSurat", masaBerlakuSurat))
				...
				data.add(count);
				Integer colCount = listTotals.get(masaBerlakuSurat == null || masaBerlakuSurat.getId() == null ? -1L : masaBerlakuSurat.getId());
				if (colCount == null) {
					colCount = 0;
				}
				colCount += count;
				listTotals.put(masaBerlakuSurat == null || masaBerlakuSurat.getId() == null ? -1L : masaBerlakuSurat.getId(), colCount);
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			int jml = 0;
			int i = 0;
			for (final MasaBerlakuSurat masaBerlakuSurat : masaBerlakuSurats) {
				Integer colCount = listTotals.get(masaBerlakuSurat == null || masaBerlakuSurat.getId() == null ? -1L : masaBerlakuSurat.getId());
				if (colCount == null) {
					colCount = 0;
				}
				if (colCount > 0) {
					...
					countData.add((masaBerlakuSurat == null ? "" : "E" + masaBerlakuSurat.getId()));
					...
					if (count > 0) {
						...
						categoryModel.setValue(satuanKerja == null ? "Tidak Ditentukan" : satuanKerja.getNama(),
								masaBerlakuSurat == null ? "Tidak ditentukan" : masaBerlakuSurat.getNama(), count);
					}
					A a = new A(count + "");
										...
										public Object[] initCriteria(boolean order) {
											...
											try {
												...
												Criteria criteria = HibernateUtil.currentSession()
														...
														.createAlias("klasifikasiSuratMasuk", "klasifikasiSuratMasuk")
														.add(masaBerlakuSurat == null
																? Restrictions.isNull(
																		"klasifikasiSuratMasuk.masaBerlakuSurat")
																: Restrictions.eq(
																		"klasifikasiSuratMasuk.masaBerlakuSurat",
																		masaBerlakuSurat))
		...
		for (final MasaBerlakuSurat masaBerlakuSurat : masaBerlakuSurats) {
			...
			if (colCount > 0) {
				countCol++;
				listCols.get(masaBerlakuSurat == null || masaBerlakuSurat.getId() == null ? -1L : masaBerlakuSurat.getId()).setParent(contents);
									...
									public Object[] initCriteria(boolean order) {
										...
										try {
											...
											Criteria criteria = HibernateUtil.currentSession()
													...
													.add(masaBerlakuSurat == null
															? Restrictions
																	.isNull("klasifikasiSuratMasuk.masaBerlakuSurat")
															: Restrictions.eq("klasifikasiSuratMasuk.masaBerlakuSurat",
																	masaBerlakuSurat))
													.add(loker.getSelectedItem() == null
															|| loker.getSelectedItem().getValue() == null
																	? Restrictions.sqlRestriction("true")
