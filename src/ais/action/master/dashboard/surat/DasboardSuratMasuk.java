...
/**
 * Komponen dashboard khusus untuk dasboard surat masuk. Kelas ini memilih variasi data atau
 ...
 * satuanKerjaTreeModel}, {@code Grid grid}, {@code Combobox loker}, {@code Integer desktopHeight};
 * inisialisasi/lifecycle ({@code buatHtmlDisposisiBergrup()}, {@code urutanGrupDariSetup()}, {@code init()});
 * pembacaan/pencarian ({@code reload()}); operasi domain lain ({@code urutkanGrupBerdasarkanNomor()}, {@code
 * nomorAwalGrup()}, {@code jenisJabatanDariStatus()}, {@code normalisasiGrup()}, {@code labelGrupDisposisi()},
 * {@code labelDisposisi()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di
 * atas.</p>
