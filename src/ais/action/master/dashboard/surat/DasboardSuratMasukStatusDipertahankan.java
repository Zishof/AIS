...
/**
 * Komponen dashboard khusus untuk dasboard surat masuk status dipertahankan. Kelas ini memilih
 ...
 * @see MyWindow
 */
public class DasboardSuratMasukStatusDipertahankan extends MyWindow {
	...
	public DasboardSuratMasukStatusDipertahankan(String tipe) {
		super();
		this.tipe = tipe;
		try {
	...
	public DasboardSuratMasukStatusDipertahankan(String tipe, String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DasboardSuratMasukStatusDipertahankan.this.grid);
			}
		});
	}
	...
	private void reload() {
		...
		List<StatusDipertahankan> statusDipertahankans = ConstantValues
				.simpleList(
						HibernateUtil.currentSession().createCriteria(StatusDipertahankan.class)
		...
		statusDipertahankans.add(null);
		Map<Long, MyColumnConfig> listCols = new HashMap<Long, MyColumnConfig>();
		for (StatusDipertahankan statusDipertahankan : statusDipertahankans) {
			MyColumnConfig column = new MyColumnConfig(
					statusDipertahankan == null ? "Tidak ditentukan" : statusDipertahankan.getNama());
			column.setTooltiptext(statusDipertahankan == null ? "Tidak ditentukan" : statusDipertahankan.getNama());
			column.setWidth("5%");
			column.setAlign("right");
			listCols.put(statusDipertahankan == null || statusDipertahankan.getId() == null ? -1L : statusDipertahankan.getId(), column);
		}
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			for (StatusDipertahankan statusDipertahankan : statusDipertahankans) {
				Integer count = ((Number) HibernateUtil.currentSession().createCriteria(SuratMasuk.class)
						...
						.add(statusDipertahankan == null
								? Restrictions.isNull("klasifikasiSuratMasuk.statusDipertahankan")
								: Restrictions.eq("klasifikasiSuratMasuk.statusDipertahankan", statusDipertahankan))
				...
				data.add(count);
				Integer colCount = listTotals.get(statusDipertahankan == null || statusDipertahankan.getId() == null ? -1L : statusDipertahankan.getId());
				if (colCount == null) {
					colCount = 0;
				}
				colCount += count;
				listTotals.put(statusDipertahankan == null || statusDipertahankan.getId() == null ? -1L : statusDipertahankan.getId(), colCount);
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			int jml = 0;
			int i = 0;
			for (final StatusDipertahankan statusDipertahankan : statusDipertahankans) {
				Integer colCount = listTotals.get(statusDipertahankan == null || statusDipertahankan.getId() == null ? -1L : statusDipertahankan.getId());
				if (colCount == null) {
					colCount = 0;
				}
				if (colCount > 0) {
					...
					countData.add((statusDipertahankan == null ? "" : "E" + statusDipertahankan.getId()));
										...
										public Object[] initCriteria(boolean order) {
											...
											try {
												...
												Criteria criteria = HibernateUtil.currentSession()
														...
														.add(statusDipertahankan == null
																? Restrictions.isNull(
																		"klasifikasiSuratMasuk.statusDipertahankan")
																: Restrictions.eq(
																		"klasifikasiSuratMasuk.statusDipertahankan",
																		statusDipertahankan))
		...
		for (final StatusDipertahankan statusDipertahankan : statusDipertahankans) {
			...
			if (colCount > 0) {
				countCol++;
				listCols.get(statusDipertahankan == null || statusDipertahankan.getId() == null ? -1L : statusDipertahankan.getId()).setParent(contents);
									...
									public Object[] initCriteria(boolean order) {
										...
										try {
											...
											Criteria criteria = HibernateUtil.currentSession()
													...
													.add(statusDipertahankan == null
															? Restrictions
																	.isNull("klasifikasiSuratMasuk.statusDipertahankan")
															: Restrictions.eq(
																	"klasifikasiSuratMasuk.statusDipertahankan",
																	statusDipertahankan))
													.add(loker.getSelectedItem() == null
	...
	/**
	 * Pembawa data/helper lokal milik {@link DasboardSuratMasukStatusDipertahankan} untuk html category model.
	 * Tipe ini mengelompokkan nilai antara agar perhitungan atau rendering tidak memakai array/map tanpa kontrak
	 * yang jelas.
	 *
	 * <p><b>Scope:</b> tipe bersifat {@code static}; instance tidak menangkap object {@link
	 * DasboardSuratMasukStatusDipertahankan}. Dependensi yang diperlukan harus diberikan secara eksplisit agar
	 ...
	 *
	 * @see DasboardSuratMasukStatusDipertahankan
	 */
