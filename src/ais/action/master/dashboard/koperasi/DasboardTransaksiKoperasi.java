package ais.action.master.dashboard.koperasi;
...
import ais.action.master.koperasi.TransaksiKoperasiAction;
...
/**
 * Komponen dashboard khusus untuk dasboard transaksi koperasi. Kelas ini memilih variasi data atau
 ...
 * @see MyWindow
 */
public class DasboardTransaksiKoperasi extends MyWindow {
	...
	public DasboardTransaksiKoperasi() {
		super();
		try {
	...
	public DasboardTransaksiKoperasi(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				UIUtil.downloadGrid(DasboardTransaksiKoperasi.this.grid);
			}
		});
	}
	...
	private void reload() {
		...
		treeMap.put(4, "Ditransfer");

		final TreeMap<Integer, Criterion> treeMapCriterion = new TreeMap<Integer, Criterion>();

		Criterion criterion = Restrictions.and(Restrictions.isNull("prosesTransfer.realisasikanOleh"),
				Restrictions.eq("status", TransaksiKoperasi.PENGAJUAN));
		treeMapCriterion.put(1, criterion);
		criterion = Restrictions.and(Restrictions.isNull("prosesTransfer.realisasikanOleh"),
				Restrictions.eq("status", TransaksiKoperasi.DISETUJU));
		treeMapCriterion.put(2, criterion);
		criterion = Restrictions.and(Restrictions.isNull("prosesTransfer.realisasikanOleh"),
				Restrictions.eq("status", TransaksiKoperasi.DITOLAK));
		treeMapCriterion.put(3, criterion);
		criterion = Restrictions.isNotNull("prosesTransfer.realisasikanOleh");
		...
		for (final SatuanKerja satuanKerja : satuanKerjas) {
			...
			for (Integer status : treeMap.keySet()) {
				...
				Criteria criteria = HibernateUtil.currentSession().createCriteria(TransaksiKoperasi.class)
					...
					a.addEventListener("onClick", new EventListener() {
						...
						public void onEvent(Event arg0) throws Exception {
							EventListener eventListener = (EventListener) Common
									.cetakDataCustomButton(TransaksiKoperasi.class, new DataCriteriaWithColumn() {
										...
										public Object[] initCriteria(boolean order) {
											...
											try {
												...
												Criteria criteria = HibernateUtil.currentSession()
														.createCriteria(TransaksiKoperasi.class)
														...
														.createAlias("daftarPengajuanTransfer",
												...
												return new Object[] { criteria, TransaksiKoperasiAction.contents };
		...
		row.appendChild(DashboardModernHtmlUtil.createAnyChart(categoryModel, "Dasbor Transaksi Koperasi", "Perbandingan data dibuat ringkas agar kelompok terbesar dan terkecil mudah terlihat.", String.valueOf("bar")));
}
}
