...
/**
 * Komponen dashboard khusus untuk dashboard rekap mahasiswa baru berdasarkan verifikasi. Kelas ini
 ...
 * @see MyWindow
 */
public class DashboardRekapMahasiswaBaruBerdasarkanVerifikasi extends MyWindow {
	...
	public DashboardRekapMahasiswaBaruBerdasarkanVerifikasi() {
		super();
		try {
			init();
			initFakultas();
	...
	private void initSpreadsheet() {
		...
		Common.createDefaultTimer(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				Common.clear(center);
				String tahunAkademik = (String) (DashboardRekapMahasiswaBaruBerdasarkanVerifikasi.this.tahunAkademik
						.getSelectedItem() == null ? null
								: DashboardRekapMahasiswaBaruBerdasarkanVerifikasi.this.tahunAkademik.getSelectedItem()
										.getValue());
				...
				DashboardRekapMahasiswaBaruBerdasarkanVerifikasi.this.angkatan.setValue(tahun);
				...
				for (MyCheckboxConfig checkboxConfig : kolom) {
					if (checkboxConfig.isChecked()) {
								...
								if (obj instanceof GeneralValueObject) {
									...
									if (generalValueObject.getNama() != null) {
										sql += "sum(case when aaa." + namaKolom + " = " + generalValueObject.getId()
												+ " and (ver.uploaded is null or ver.uploaded=false) then 1 else 0 end) as \""
												+ Common.getBahasaConfig(generalValueObject.getNama().trim())
												+ " Belum Upload\", ";
										sql += "sum(case when aaa." + namaKolom + " = " + generalValueObject.getId()
												+ " and ver.uploaded then 1 else 0 end) as \""
												+ Common.getBahasaConfig(generalValueObject.getNama().trim())
												+ " Telah Upload\", ";

										sql += "sum(case when aaa." + namaKolom + " = " + generalValueObject.getId()
												+ " and (ver.verified is null or ver.verified=false) then 1 else 0 end) as \""
												+ Common.getBahasaConfig(generalValueObject.getNama().trim())
												+ " Belum Verified\", ";
										sql += "sum(case when aaa." + namaKolom + " = " + generalValueObject.getId()
												+ " and ver.verified then 1 else 0 end) as \""
												+ Common.getBahasaConfig(generalValueObject.getNama().trim())
												+ " Telah Verified\", ";
									...
									sql += "sum(case when aaa." + namaKolom
											+ (obj instanceof String ? " ilike '"
													+ org.apache.commons.lang3.StringUtils.replace(
															org.apache.commons.lang3.StringUtils.replace(obj.toString(), "\'", "''"), "\"", "")
													+ "'" : " = " + obj.toString())
											+ " and ver.uploaded then 1 else 0 end) as \"" + obj + " Telah Upload\", ";
									...
									sql += "sum(case when aaa." + namaKolom
											+ (obj instanceof String ? " ilike '"
													+ org.apache.commons.lang3.StringUtils.replace(
															org.apache.commons.lang3.StringUtils.replace(obj.toString(), "\'", "''"), "\"", "")
													+ "'" : " = " + obj.toString())
											+ " and ver.verified then 1 else 0 end) as \"" + obj
											+ " Telah Verified\", ";
						...
						sql += "sum(case when aaa." + namaKolom
								+ " is not null and (ver.uploaded is null or ver.uploaded=false) then 1 else 0 end) as \"Belum Upload\", ";
						sql += "sum(case when aaa." + namaKolom
								+ " is not null and ver.uploaded then 1 else 0 end) as \"Telah Upload\", ";

						sql += "sum(case when aaa." + namaKolom
								+ " is not null and (ver.verified is null or ver.verified=false) then 1 else 0 end) as \"Belum Verified\", ";
						sql += "sum(case when aaa." + namaKolom
								+ " is not null and ver.verified then 1 else 0 end) as \"Telah Verified\", ";
						...
						sql += " sum(case when aaa." + namaKolom
								...
								+ "inner join fakultas y on (y.id = x.fakultas)      where ver_data.aktif "
...
Common.clear(center);spreadsheet.setParent(center);
				...
				spreadsheet.setMaxcolumns((maxColoumn * 5) + 7);
				...
				for (int indexData = 0; indexData < generalValueObjectsSemua.size(); indexData++) {
					...
					ais.ui.util.EcampusUtil.mergeCells(sheet, rowIndex - 1, 0, rowIndex - 1,
							(generalValueObjects.size() * 5) + 4, true);
					...
					for (Object obj : generalValueObjects) {
						...
						if (obj != null && !obj.toString().isEmpty()) {
							...
							ais.ui.util.EcampusUtil.mergeCells(sheet, rowIndex, colIndex, rowIndex, colIndex + 4, true);

							ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex + 1, colIndex, "Belum Upload");
							ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex + 1, colIndex + 1, "Telah Upload");
							ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex + 1, colIndex + 2, "Belum Verifikasi");
							ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex + 1, colIndex + 3, "Telah Verifikasi");
							ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex + 1, colIndex + 4, "Total");
					...
					try {
						ais.ui.util.EcampusUtil.setBorder(sheet,
								new Rect(0, rowIndex, (generalValueObjects.size() * 5) + 6, rowIndex + 1),
								BookHelper.BORDER_FULL, BorderStyle.THIN, color);
						ais.ui.util.EcampusUtil.setBold(sheet,
								new Rect(0, rowIndex, (generalValueObjects.size() * 5) + 6, rowIndex + 1), true);
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapMahasiswaBaruBerdasarkanVerifikasi.java:645");
					...
					for (Object[] objects : jurusans) {
						...
						Integer td = ((Number) (objects[objects.length - 5] == null ? 0 : objects[objects.length - 5]))
								.intValue();
						totalBelumUpload += td;
					...
					try {
						ais.ui.util.EcampusUtil.setBorder(sheet,
								new Rect(0, rowIndex - jurusans.size(), (generalValueObjects.size() * 5) + 6, rowIndex),
								BookHelper.BORDER_FULL, BorderStyle.THIN, color);
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapMahasiswaBaruBerdasarkanVerifikasi.java:774");
					}
					...
					try {
						...
						ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, colIndex + 2, totalBelumVerif);
						ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, colIndex + 3, totalTelahVerif);
						ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, colIndex + 4, total);
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapMahasiswaBaruBerdasarkanVerifikasi.java:807");
					...
					try {
						...
						ais.ui.util.EcampusUtil.setBold(sheet,
								new Rect(colIndex, rowIndex, (generalValueObjects.size() * 5) + 6, rowIndex), true);
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapMahasiswaBaruBerdasarkanVerifikasi.java:815");
