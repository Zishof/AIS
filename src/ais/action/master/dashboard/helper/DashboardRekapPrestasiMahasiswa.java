...
/**
 * Komponen dashboard khusus untuk dashboard rekap prestasi mahasiswa. Kelas ini memilih variasi
 ...
 * Combobox searchjurusan}, {@code Combobox searchprogram}, {@code Spreadsheet spreadsheet}, {@code Center
 ...
 * @see MyWindow
 */
public class DashboardRekapPrestasiMahasiswa extends MyWindow {
	...
	public DashboardRekapPrestasiMahasiswa(String namaKolom, String kolomGroup) {
		super();
	...
	private void initSpreadsheet() {
		...
		List<GeneralValueObject> generalValueObjects = session.createCriteria(PrestasiMahasiswa.class)
		...
		sql += " sum(case when aaa." + namaKolom
				+ " is not null then 1 else 0 end) as total from prestasi_mahasiswa aaa  "
				+ " inner join mahasiswa m on (aaa.mahasiswa = m.id  )    "
				+ " inner join jurusan x on (m.jurusan = x.id  )    "
				+ " inner join fakultas y on (y.id = x.fakultas)      where 1=1 and aaa.status='Disetujui' "
				+ (program == null ? "" : " and m.program = '" + program + "'")
				+ (jurusan == null ? "" : " and m.jurusan = " + jurusan.getId())
				+ (fakultas == null ? "" : " and x.fakultas = " + fakultas.getId()) + " and aaa.tanggal between date('"
		...
		for (Object[] objects : jurusans) {
			...
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapPrestasiMahasiswa.java:396");
			}
		...
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapPrestasiMahasiswa.java:428");
		}
		...
		// -- Panel visual: DONUT komposisi gender mahasiswa berprestasi (HTML/CSS, BUKAN JFreeChart) --
		try {
			chartPanel.setContent("<div style='padding:10px 12px 2px;'>"
					+ ais.ui.util.HtmlChartHelper.donut(
							"Komposisi Mahasiswa Berprestasi (Laki-laki vs Perempuan)",
							"Melihat sekilas perbandingan jumlah mahasiswa berprestasi laki-laki dan perempuan.",
							new String[] { "Laki-laki", "Perempuan" },
							new double[] { totalLaki, totalPerempuan },
							new String[] { "#2563eb", "#ec4899" },
							total + " prestasi")
					+ "</div>");
		} catch (Exception eChart) { ais.common.ErrorAuditUtil.record(eChart, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapPrestasiMahasiswa.java:444");
		}
		...
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapPrestasiMahasiswa.java:451");
		}
