...
/**
 * Komponen dashboard khusus untuk dashboard rekap parameter tambahan mahasiswa. Kelas ini memilih
 ...
 * spreadsheet}, {@code Center center}, {@code List kolom}, {@code Combobox searchstatus}; inisialisasi/lifecycle
 * ({@code initFakultas()}, {@code init()}, {@code initSpreadsheet()}). Bagian lain dari kontrak tetap mengikuti
 ...
 * @see MyWindow
 */
public class DashboardRekapParameterTambahanMahasiswa extends MyWindow {
	...
	public DashboardRekapParameterTambahanMahasiswa() {
		super();
	...
	private void init() throws Exception {
		...
		row.setValign("top");
				...
				for (KelompokParameterTambahanMahasiswa kelompokParameterTambahanMahasiswa : kelompokParameterTambahanMahasiswas) {
					...
					row = new MyFormRow();
					row.setParent(rowsParams);
					row.appendChild(new MyLabelBold(kelompokParameterTambahanMahasiswa.getNama()));
					...
					for (final ParameterTambahan parameterTambahan : parameterTambahans) {
						...
						checkboxConfig.setAttribute("kelompokParameterTambahanMahasiswa",
								kelompokParameterTambahanMahasiswa);
	...
	private void initSpreadsheet() {
		...
		StatusMahasiswa searchstatus = (StatusMahasiswa) (this.searchstatus.getSelectedItem() == null || this.searchstatus.getSelectedItem().getValue() == null ? null
			...
			if (checkboxConfig.isChecked()) {
				...
				for (Object obj : generalValueObjects) {
					if (obj != null && !obj.toString().isEmpty() && !obj.toString().equals("Tidak di-pilih")) {
						String like = kelompokParameterTambahanCalonMahasiswa.getId() + "->" + parameterTambahan.getId()
								+ "<=>"
						...
						sql += "sum(case when aaa.parametertambahaninds ilike '%" + like
								+ "%' and m.kelamin='Laki-laki' then 1 else 0 end) as \"" + obj
								+ " Laki-laki\", ";
						sql += "sum(case when aaa.parametertambahaninds ilike '%" + like
								+ "%' and m.kelamin='Perempuan' then 1 else 0 end) as \"" + obj
								+ " Perempuan\", ";
						sql += "sum(case when aaa.parametertambahaninds ilike '%" + like + "%' then 1 else 0 end) as \""
								+ obj + "\", ";
				...
				sql += "sum(case when aaa.parametertambahaninds is not null and m.kelamin='Laki-laki' then 1 else 0 end) as \"Laki-laki\", ";
				sql += "sum(case when aaa.parametertambahaninds is not null and m.kelamin='Perempuan' then 1 else 0 end) as \"Perempuan\", ";

				sql += " sum(case when aaa.parametertambahaninds is not null then 1 else 0 end) as total from biodata_mahasiswa aaa  "
		...
		for (int indexData = 0; indexData < generalValueObjectsSemua.size(); indexData++) {
			...
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapParameterTambahanMahasiswa.java:519");
			...
			try {
				ais.ui.util.EcampusUtil.setBorder(sheet,
						...
						BookHelper.BORDER_FULL, BorderStyle.THIN, color);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapParameterTambahanMahasiswa.java:616");
			}
			...
			try {
				...
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, colIndex + 2, total);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapParameterTambahanMahasiswa.java:639");
			...
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapParameterTambahanMahasiswa.java:647");
