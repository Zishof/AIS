...
/**
 * Komponen dashboard khusus untuk dashboard rekap kegiatan dosenan. Kelas ini memilih variasi data
 ...
 * @see MyWindow
 */
public class DashboardRekapKegiatanDosenan extends MyWindow {
	...
	public DashboardRekapKegiatanDosenan(String namaKolom, String kolomGroup) {
		super();
	...
	private void initSpreadsheet() {
		...
		List<GeneralValueObject> generalValueObjects = session.createCriteria(KegiatanKedosenanPunyaDosen.class)
		...
		sql += " sum(case when " + namaKolom + " is not null then 1 else 0 end) as total "
				+ " from kegiatan_kedosenan_punya_dosen aaa  "
				+ " inner join kegiatan_kedosenan k on (aaa.kegiatan_kedosenan = k.id  )    "
				+ " inner join dosen m on (aaa.dosen = m.id  )    "
				+ " inner join jurusan x on (m.jurusan = x.id  )    "
				+ " inner join fakultas y on (y.id = x.fakultas)      where 1=1 and aaa.persetujuan "
				+ (jurusan == null ? "" : " and m.jurusan = " + jurusan.getId())
				+ (fakultas == null ? "" : " and x.fakultas = " + fakultas.getId()) + " and aaa.mulai between date('"
		...
		java.util.Arrays.fill(nilaisLaki, 0);
		java.util.Arrays.fill(nilaisPerempuan, 0);
		java.util.Arrays.fill(nilais, 0);
		...
		for (Object[] objects : jurusans) {
			...
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapKegiatanDosenan.java:382");
			}
		...
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapKegiatanDosenan.java:414");
		}
		...
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapKegiatanDosenan.java:424");
		}
