...
/**
 * Komponen dashboard khusus untuk dashboard rekap kegiatan kesiswaan. Kelas ini memilih variasi
 ...
 * Combobox searchsekolah}, {@code Combobox searchprogram}, {@code Spreadsheet spreadsheet}, {@code Center
 * center}, {@code String namaKolom}, {@code String kolomGroup}, {@code MyDatebox mulai}; inisialisasi/lifecycle
 * ({@code initYayasan()}, {@code init()}, {@code initSpreadsheet()}). Bagian lain dari kontrak tetap mengikuti
 ...
 * @see MyWindow
 */
public class DashboardRekapKegiatanKesiswaan extends MyWindow {
	...
	public DashboardRekapKegiatanKesiswaan(String namaKolom, String kolomGroup) {
		super();
	...
	private void initSpreadsheet() {
		...
		List<GeneralValueObject> generalValueObjects = session.createCriteria(KegiatanKesiswaanPunyaSiswa.class)
				.createAlias("kegiatanKesiswaan", "kegiatanKesiswaan")
				.setProjection(Projections.groupProperty(kolomGroup)).add(Restrictions.isNotNull(kolomGroup))
				.add(Restrictions.between("mulai", mulai.getValue(), sampai.getValue())).list();
		...
		for (GeneralValueObject generalValueObject : generalValueObjects) {
			if (generalValueObject.getNama() != null && !generalValueObject.getNama().isEmpty()) {
				...
				sql += "sum(case when " + namaKolom + " = " + generalValueObject.getId() + " then 1 else 0 end) as \""
		...
		sql += " sum(case when " + namaKolom + " is not null then 1 else 0 end) as total "
				+ " from sekolah.kegiatan_kesiswaan_punya_siswa aaa  "
				+ " inner join sekolah.kegiatan_kesiswaan k on (aaa.kegiatan_kesiswaan = k.id  )    "
				+ " inner join sekolah.siswa m on (aaa.siswa = m.id  )    "
				+ " inner join sekolah.sekolah x on (k.sekolah = x.id  )    "
				+ " inner join sekolah.yayasan y on (y.id = x.yayasan_id)      where 1=1 and aaa.persetujuan "
				+ (program == null ? "" : " and m.program = '" + program + "'")
				+ (sekolah == null ? "" : " and k.sekolah = " + sekolah.getId())
				+ (yayasan == null ? "" : " and x.yayasan_id = " + yayasan.getId()) + " and aaa.mulai between date('"
				+ Common.databaseDateFormat.get().format(mulai.getValue()) + "') and date('"
				+ Common.databaseDateFormat.get().format(sampai.getValue())
				+ "') group by x.yayasan_id,k.sekolah order by max(y.nama), max(x.nama) ";
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, 1, 0, "REKAPITULASI PRESTASI SISWA");
		...
		for (Object[] objects : sekolahs) {
			...
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapKegiatanKesiswaan.java:391");
			}
		...
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapKegiatanKesiswaan.java:423");
		}
		...
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapKegiatanKesiswaan.java:433");
		}
