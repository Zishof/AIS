...
/**
 * Komponen dashboard khusus untuk dashboard rekap parameter tambahan alumni. Kelas ini memilih
 ...
 * Combobox searchjurusan}, {@code Combobox tahunAkademik}, {@code Combobox searchprogram}, {@code Spreadsheet
 * spreadsheet}, {@code Center center}, {@code List kolom}, {@code Combobox searchstatusAwal};
 ...
 * @see MyWindow
 */
public class DashboardRekapParameterTambahanAlumni extends MyWindow {
	...
	private Combobox tahunAkademikMasuk;

	private Combobox tahunAkademikSd;

	private Combobox tahunAkademikMasukSd;

	public DashboardRekapParameterTambahanAlumni() {
		super();
	...
	private void init() throws Exception {
		...
		tahunAkademik = Common.generateTahunDanSemua(tahunAkademik);
		tahunAkademikSd = Common.generateTahunDanSemua(tahunAkademikSd);
		Hbox hbox = new Hbox();
		hbox.appendChild(tahunAkademik);
		hbox.appendChild(new Label(ais.common.Common.getBahasaConfig("s.d")));
		hbox.appendChild(tahunAkademikSd);
		tahunAkademik.setCols(3);
		tahunAkademikSd.setCols(3);
		row.appendChild(hbox);
		Common.selectComboItem(tahunAkademik, null);
		Common.selectComboItem(tahunAkademikSd, null);
		...
		tahunAkademikSd.addEventListener("onChange", new EventListener() {
		...
		tahunAkademikMasuk = Common.generateTahunDanSemua(tahunAkademikMasuk);
		tahunAkademikMasukSd = Common.generateTahunDanSemua(tahunAkademikMasukSd);
		tahunAkademikMasuk.setCols(3);
		tahunAkademikMasukSd.setCols(3);
		hbox = new Hbox();
		hbox.appendChild(tahunAkademikMasuk);
		hbox.appendChild(new Label(ais.common.Common.getBahasaConfig("s.d")));
		hbox.appendChild(tahunAkademikMasukSd);

		row.appendChild(hbox);

		Common.selectComboItem(tahunAkademikMasuk, null);
		Common.selectComboItem(tahunAkademikMasukSd, null);

		tahunAkademikMasuk.addEventListener("onChange", new EventListener() {
		...
		tahunAkademikMasukSd.addEventListener("onChange", new EventListener() {
		...
		EventListener eventListener = new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				List<KelompokParameterTambahanAlumni> kelompokParameterTambahanAlumnis = ConstantValues
						.simpleList(
								session.createCriteria(ParameterTambahanAlumni.class)
										.createAlias("parameterTambahan", "parameterTambahan")
										.createAlias("kelompokParameterTambahanAlumni",
												"kelompokParameterTambahanAlumni")
				...
				for (KelompokParameterTambahanAlumni kelompokParameterTambahanAlumni : kelompokParameterTambahanAlumnis) {
					List<ParameterTambahanAlumni> parameterTambahanAlumnis = ConstantValues.simpleList(
							session.createCriteria(ParameterTambahanAlumni.class)
									.add(Restrictions.eq("kelompokParameterTambahanAlumni",
											kelompokParameterTambahanAlumni))

									.createAlias("parameterTambahan", "parameterTambahan")
									.createAlias("kelompokParameterTambahanAlumni", "kelompokParameterTambahanAlumni")
									.add(Restrictions.eq("parameterTambahan.aktif", true))
									.add(Restrictions.eq("parameterTambahan.tipeDataInputan",
					...
					row = new MyFormRow();
					row.setParent(rowsParams);
					row.appendChild(new MyLabelBold(kelompokParameterTambahanAlumni.getNama()));

					for (final ParameterTambahanAlumni parameterTambahanAlumni : parameterTambahanAlumnis) {
						row = new MyFormRow();
						row.setParent(rowsParams);
						MyCheckboxConfig checkboxConfig = new MyCheckboxConfig(
								parameterTambahanAlumni.getParameterTambahan().getLabelInputan());
						checkboxConfig.setAttribute("parameterTambahan",
								parameterTambahanAlumni.getParameterTambahan());
						checkboxConfig.setAttribute("kelompokParameterTambahanAlumni", kelompokParameterTambahanAlumni);
	...
	private void initSpreadsheet() {
		...
		if (!adaChecked) {
			new Html("<div style='padding:40px 24px;text-align:center;color:#999;font-size:13px;'>"
				...
				+ "<div>lalu klik <b>Tampilkan</b> untuk melihat rekap data alumni.</div>"
				+ "</div>").setParent(center);
			return;
		}

		Integer tahunAkademik = (Integer) (this.tahunAkademik.getSelectedItem() == null
				|| this.tahunAkademik.getSelectedItem().getValue() == null ? null
						: this.tahunAkademik.getSelectedItem().getValue());
		Integer tahunAkademikSd = (Integer) (this.tahunAkademikSd.getSelectedItem() == null
				|| this.tahunAkademikSd.getSelectedItem().getValue() == null ? null
						: this.tahunAkademikSd.getSelectedItem().getValue());
		...
		Integer tahunAkademikMasuk = (Integer) (this.tahunAkademikMasuk.getSelectedItem() == null
				|| this.tahunAkademikMasuk.getSelectedItem().getValue() == null ? null
						: this.tahunAkademikMasuk.getSelectedItem().getValue());
		Integer tahunAkademikMasukSd = (Integer) (this.tahunAkademikMasukSd.getSelectedItem() == null
				|| this.tahunAkademikMasukSd.getSelectedItem().getValue() == null ? null
						: this.tahunAkademikMasukSd.getSelectedItem().getValue());

		String tahun = tahunAkademik == null ? null : tahunAkademik.toString();
		String tahunSd = tahunAkademikSd == null ? null : tahunAkademikSd.toString();

		String tahunMasuk = tahunAkademikMasuk == null ? null : tahunAkademikMasuk.toString();
		String tahunMasukSd = tahunAkademikMasukSd == null ? null : tahunAkademikMasukSd.toString();
		...
		for (MyCheckboxConfig checkboxConfig : kolom) {
			if (checkboxConfig.isChecked()) {
				...
				KelompokParameterTambahanAlumni kelompokParameterTambahanAlumni = (KelompokParameterTambahanAlumni) checkboxConfig
				...
				List generalValueObjectsValues = new ArrayList();
				...
				for (String s : ss) {
					...
					if (kol.length > 1) {
						generalValueObjectsValues.add(kol[1]);
					} else {
						generalValueObjectsValues.add(kol[0]);
					}

					String a = kol[0];
					generalValueObjects.add(a);
				}
				Collections.sort(generalValueObjectsValues);
				Collections.sort(generalValueObjects);
				generalValueObjects.add("Tidak di-pilih");
				generalValueObjectsValues.add("Tidak di-pilih");
				...
				for (Object obj : generalValueObjectsValues) {
					if (obj != null && !obj.toString().isEmpty() && !obj.toString().equals("Tidak di-pilih")) {
						String like = kelompokParameterTambahanAlumni.getId() + "->" + parameterTambahan.getId() + "<=>"
								+ org.apache.commons.lang3.StringUtils.replace(
										org.apache.commons.lang3.StringUtils.replace(obj.toString(), "\'", "''"), "\"",
										"");
						likes.add(like);
						sql += "sum(case when aaa.parametertambahanindsalumni ilike '%" + like
								+ "%' and m.kelamin='Laki-laki' then 1 else 0 end) as \"" + obj + " Laki-laki\", ";
						sql += "sum(case when aaa.parametertambahanindsalumni ilike '%" + like
								+ "%' and m.kelamin='Perempuan' then 1 else 0 end) as \"" + obj + " Perempuan\", ";
						sql += "sum(case when aaa.parametertambahanindsalumni ilike '%" + like
				...
				for (String like : likes) {
					orIlike += orIlike.isEmpty() ? "(aaa.parametertambahanindsalumni not ilike '%" + like + "%')"
							: " and (aaa.parametertambahanindsalumni not ilike '%" + like + "%')";
				}

				sql += "sum(case when " + (orIlike.isEmpty() ? "true" : "(" + orIlike + ")")
						+ " and m.kelamin='Laki-laki' then 1 else 0 end) as \"Tidak di-pilih Laki-laki\", ";
				sql += "sum(case when " + (orIlike.isEmpty() ? "true" : "(" + orIlike + ")")
						+ " and m.kelamin='Perempuan' then 1 else 0 end) as \"Tidak di-pilih Perempuan\", ";
				sql += "sum(case when " + (orIlike.isEmpty() ? "true" : "(" + orIlike + ")")
						+ " then 1 else 0 end) as \"Tidak di-pilih\", ";

				sql += "sum(case when aaa.parametertambahanindsalumni is not null and m.kelamin='Laki-laki' then 1 else 0 end) as \"Laki-laki\", ";
				sql += "sum(case when aaa.parametertambahanindsalumni is not null and m.kelamin='Perempuan' then 1 else 0 end) as \"Perempuan\", ";

				sql += " sum(case when aaa.parametertambahanindsalumni is not null then 1 else 0 end) as total from biodata_mahasiswa aaa  "
						...
						+ (tahun == null ? "" : " and m.tahunlulus >= " + tahun)
						+ (tahunSd == null ? "" : " and m.tahunlulus <= " + tahunSd)

						+ (tahunMasuk == null ? "" : " and m.tahunangkatan >= " + tahunMasuk)
						+ (tahunMasukSd == null ? "" : " and m.tahunangkatan <= " + tahunMasukSd)

						+ (searchstatusAwalMahasiswa == null ? ""
								: " and m.status_awal_mahasiswa = " + searchstatusAwalMahasiswa.getId())
						+ " and m.status_keluar=1 group by x.fakultas,m.jurusan order by max(y.nama), max(x.nama) ";
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, 1, 0,
				"REKAPITULASI ALUMNI\n" + (fakultas == null ? "" : fakultas.getNama().toUpperCase() + "\n")
						...
						+ (tahunAkademik == null ? "SEMUA" : tahun) + " SAMPAI TAHUN LULUS "
						+ (tahunAkademikSd == null ? "SEMUA" : tahunSd) + "\nPROGRAM "
		...
		for (int indexData = 0; indexData < generalValueObjectsSemua.size(); indexData++) {
			...
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapParameterTambahanAlumni.java:598");
			...
			try {
				ais.ui.util.EcampusUtil.setBorder(sheet,
						...
						BookHelper.BORDER_FULL, BorderStyle.THIN, color);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapParameterTambahanAlumni.java:694");
			}
			...
			try {
				...
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, colIndex + 2, total);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapParameterTambahanAlumni.java:717");
			...
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapParameterTambahanAlumni.java:725");
