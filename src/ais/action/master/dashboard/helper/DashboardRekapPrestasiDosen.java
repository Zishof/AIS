...
/**
 * Komponen dashboard khusus untuk dashboard rekap prestasi dosen. Kelas ini memilih variasi data
 ...
 * Combobox searchjurusan}, {@code Spreadsheet spreadsheet}, {@code Center center}, {@code String namaKolom},
 * {@code String kolomGroup}, {@code MyDatebox mulai}, {@code MyDatebox sampai}; inisialisasi/lifecycle ({@code
 ...
 * @see MyWindow
 */
public class DashboardRekapPrestasiDosen extends MyWindow {
	...
	public DashboardRekapPrestasiDosen(String namaKolom, String kolomGroup) {
		super();
	...
	private void initSpreadsheet() {
		...
		List<GeneralValueObject> generalValueObjects = session.createCriteria(PrestasiDosen.class)
		...
		sql += " sum(case when aaa." + namaKolom + " is not null then 1 else 0 end) as total from prestasi_dosen aaa  "
				+ " inner join dosen m on (aaa.dosen = m.id  )    "
				+ " inner join jurusan x on (m.jurusan = x.id  )    "
				+ " inner join fakultas y on (y.id = m.fakultas)      where 1=1 and status='Disetujui' "
				+ (jurusan == null ? "" : " and m.jurusan = " + jurusan.getId())
				+ (fakultas == null ? "" : " and m.fakultas = " + fakultas.getId()) + " and aaa.tanggal between date('"
				+ Common.databaseDateFormat.get().format(mulai.getValue()) + "') and date('"
				+ Common.databaseDateFormat.get().format(sampai.getValue())
				+ "') group by m.fakultas,m.jurusan order by max(y.nama), max(x.nama) ";
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, 1, 0, "REKAPITULASI PRESTASI DOSEN");
		...
		for (Object[] objects : jurusans) {
			...
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapPrestasiDosen.java:383");
			}
		...
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapPrestasiDosen.java:415");
		}
		...
		// -- Panel visual: DONUT komposisi gender dosen berprestasi (HTML/CSS, BUKAN JFreeChart) --
		try {
			chartPanel.setContent("<div style='padding:10px 12px 2px;'>"
					+ ais.ui.util.HtmlChartHelper.donut(
							"Komposisi Dosen Berprestasi (Laki-laki vs Perempuan)",
							"Melihat sekilas perbandingan jumlah dosen berprestasi laki-laki dan perempuan.",
							new String[] { "Laki-laki", "Perempuan" },
							new double[] { totalLaki, totalPerempuan },
							new String[] { "#2563eb", "#ec4899" },
							total + " prestasi")
					+ "</div>");
		} catch (Exception eChart) { ais.common.ErrorAuditUtil.record(eChart, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapPrestasiDosen.java:431");
		}
		...
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/dashboard/helper/DashboardRekapPrestasiDosen.java:438");
		}
