...
/**
 * Tipe khusus untuk ppdb simple6. Kelas ini memberi nama dan batas tanggung jawab yang eksplisit
 ...
 * init()}); validasi/perhitungan ({@code checkNik()}); mutasi data ({@code onSave()}, {@code setdata()});
 * operasi domain lain ({@code chekNik()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface
 ...
 * @see PPDB
 */
public class PPDB_Simple6 extends PPDB {
	...
	private EventListener checkKesamaan = new EventListener() {
		...
		public void onEvent(Event arg0) throws Exception {
			CalonSiswaAction.CheckKesamaan checkKesamaan = new CalonSiswaAction.CheckKesamaan(calonSiswa,
					gelombangPendaftaranPsb, tanggalLahir, nama, namaIbu, PPDB_Simple6.this, new EventListener() {

						@Override
						public void onEvent(Event arg0) throws Exception {
							CalonSiswa calonSiswa1 = (CalonSiswa) arg0.getData();
							PPDB window = new PPDB_Simple6();
	...
	public PPDB_Simple6(CalonSiswa calonSiswa, GelombangPendaftaranPsb gelombangPendaftaranPsb,
	...
	public void init() {
		...
		try {
			...
			rowUtama1.setValign("top");
			rowUtama1.setParent(rows);
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sekolah/psb/form/PPDB_Simple6.java:272");
		}
		...
		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("3. NIK Siswa *"));
		row.appendChild(nik = new Textbox(calonSiswa.getNik()));
		nik.setWidth("90%");

		EventListener eventListenerNik = new EventListener() {

			@Override
			public void onEvent(Event arg0) throws Exception {
				nik.setValue(nik.getValue().trim());
				chekNik();
			}
		};

		nik.addEventListener("onChange", eventListenerNik);
		...
		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("4. Jenis Kelamin *"));
		...
		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("5. Tempat lahir *"));
		row.appendChild(tempatLahir = new Textbox(calonSiswa.getTempatLahir()));
		tempatLahir.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("6. Tanggal lahir *"));
		...
		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("7. Nama ayah kandung *"));
		...
		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("9. Telp/WA ayah *"));
		row.appendChild(hp1ayah = new Textbox(calonSiswa.getHp1ayah()));
		hp1ayah.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("10. Telp/WA ibu *"));
		...
		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("11. Telp/WA siswa *"));
		...
		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("12. Alamat *"));
		...
		try {
			...
			ais.common.ErrorAuditUtil.record(e, "PPDB_Simple6.tampilkanVerifikasiBerkas");
		}
		...
		if (calonSiswa.getId() != null) {
			try {
				...
				e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sekolah/psb/form/PPDB_Simple6.java:636");
			}
		...
		try {
			...
			// TODO Auto-generated catch block
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sekolah/psb/form/PPDB_Simple6.java:645");
		}
		...
		try {
			...
			// TODO Auto-generated catch block
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sekolah/psb/form/PPDB_Simple6.java:655");
		}
		...
		try {
			...
			// TODO Auto-generated catch block
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sekolah/psb/form/PPDB_Simple6.java:668");
		}

		rowjadwalPertemuanPSB = new MyFormRow();
		rowjadwalPertemuanPSB.setVisible(false);
		rowjadwalPertemuanPSB.setParent(rows);
		rowjadwalPertemuanPSB.appendChild(new Label("Jadwal Pertemuan Siswa / Orang Tua"));
		...
		final EventListener jadwalPertemuanEventListener = new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				GelombangPendaftaranPsb gel = gelombangPendaftaranPsb;
				...
				rowjadwalPertemuanPSB.setVisible(jadwalPertemuanPSB.getChildren().size() > 1);
			}
		};
			...
			public void onEvent(Event arg0) throws Exception {
				...
				if (jml >= d.getKuota()) {
					...
					MyMessageboxConfig.showFormatCb(
							"Mohon maaf, kuota untuk jadwal pertemuan \"{V1}\" sudah penuh. Langkah yang dapat dilakukan: (1) buka kembali pilihan Jadwal Pertemuan; (2) pilih jadwal lain yang masih tersedia kuotanya; (3) lanjutkan menyimpan pendaftaran.",
		...
		if (calonSiswa != null && calonSiswa.getId() != null) {
			fotoCalonSiswa = (FotoCalonSiswa) FotoCalonSiswa.ambil(calonSiswa.getId(), FotoCalonSiswa.DEFAULT_JENIS,
					FotoCalonSiswa.class, true);
		}

		vboxfotoCalonSiswa = new Vbox();
		vboxfotoCalonSiswa.setHeight("100%");
		vboxfotoCalonSiswa.setWidth("100%");
		vboxfotoCalonSiswa.setParent(row);

		try {
			Common.createDownloadUploadFoto(vboxfotoCalonSiswa, calonSiswa, FotoCalonSiswa.class, new EventListener() {
			...
			// TODO Auto-generated catch block
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sekolah/psb/form/PPDB_Simple6.java:790");
		}
		...
		save.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				if (onSave(event)) {
					...
					if (PPDB_Simple6.this.eventListener != null) {
						PPDB_Simple6.this.eventListener.onEvent(new Event("", save, PPDB_Simple6.this.calonSiswa));
					}
					...
					if (baru) {
						...
						informasi = org.apache.commons.lang3.StringUtils.replace(informasi, "[no_reg]",
								PPDB_Simple6.this.calonSiswa.getNoRegistrasi());
									...
									public void onEvent(Event arg0) throws Exception {
										...
										Common.createDefaultTimer(new EventListener() {
											...
											public void onEvent(Event arg0) throws Exception {
												...
												if (baru && PPDB_Simple6.this.calonSiswa.getGelombangPendaftaranPsb()
														.getOtomatisLoginSetelahDaftar()) {
													PPDB_Simple6.this.calonSiswa.setTelahLogin(true);
													PPDB_Simple6.this.calonSiswa
															.setWaktuLogin(ais.ui.util.WaktuUtil.getDate());
													Common.refreshUpdate(PPDB_Simple6.this.calonSiswa);

													Common.setLogin(PPDB_Simple6.this.calonSiswa);
													...
													CalonSiswaAction.onCetakKartu(PPDB_Simple6.this.calonSiswa, daftar);
												}
											}
										});
		...
		try {
			...
			// TODO Auto-generated catch block
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sekolah/psb/form/PPDB_Simple6.java:896");
		}
	...
	public boolean onSave(Event event) throws Exception {
		...
		if (rowjadwalPertemuanPSB.isVisible() && (jadwalPertemuanPSB.getSelectedItem() == null
				|| jadwalPertemuanPSB.getSelectedItem().getValue() == null)) {
			MyMessageboxConfig.show("Mohon Bapak/Ibu terlebih dahulu memilih Jadwal Pertemuan. Langkah yang dapat dilakukan: (1) buka pilihan Jadwal Pertemuan; (2) pilih jadwal yang masih tersedia kuotanya; (3) lanjutkan menyimpan pendaftaran.", "Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION, new EventListener() {
		...
		if (fotoCalonSiswa == null) {
			MyMessageboxConfig.show("Mohon Bapak/Ibu terlebih dahulu mengunggah Foto calon siswa. Langkah yang dapat dilakukan: (1) klik tombol unggah Foto; (2) pilih berkas foto calon siswa yang jelas; (3) lanjutkan menyimpan pendaftaran.", "Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION, new EventListener() {

						@Override
						public void onEvent(Event arg0) throws Exception {
							vboxfotoCalonSiswa.focus();
							Clients.scrollIntoView(vboxfotoCalonSiswa);
						}
					});
			return false;
		}
		...
		try {
			...
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sekolah/psb/form/PPDB_Simple6.java:1417");
		}

		return true;
	}

	private boolean chekNik() throws Exception {
		if (nik.getValue().trim().equals("")) {
			MyMessageboxConfig.show("Mohon Bapak/Ibu terlebih dahulu mengisi NIK calon siswa. Langkah yang dapat dilakukan: (1) klik kolom NIK; (2) isikan 16 digit NIK sesuai Kartu Keluarga/KTP; (3) lanjutkan menyimpan pendaftaran.", "Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION, new EventListener() {
		...
		if (nik.getValue().trim().length() != 16) {
			MyMessageboxConfig.show("Mohon maaf, NIK calon siswa harus terdiri atas 16 digit angka. Langkah yang dapat dilakukan: (1) periksa kembali NIK yang telah diisi; (2) pastikan jumlahnya tepat 16 digit sesuai Kartu Keluarga/KTP; (3) perbaiki bila terdapat kekeliruan.", "Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION, new EventListener() {
		...
		if (checkNik()) {
			...
			Session session = HibernateUtil.currentSession();
			CalonSiswa calonSiswa = (CalonSiswa) ConstantValues.simpleObject(session.createCriteria(CalonSiswa.class)
					.setMaxResults(1).addOrder(Order.desc("id")).add(Restrictions.eq("nik", nik.getValue().trim()))
					.add(Restrictions.eq("gelombangPendaftaranPsb", gelombangPendaftaranPsb))
					.add(this.calonSiswa.getId() == null ? Restrictions.sqlRestriction("1=1")
							: Restrictions.ne("id", this.calonSiswa.getId())),
					CalonSiswa.class);

			if (calonSiswa != null) {
				MyMessageboxConfig.showFormatCb(
						"Mohon maaf, NIK calon siswa sudah terdaftar pada jalur \"{V1}\" dengan Nomor Registrasi \"{V2}\" atas nama \"{V3}\". Langkah yang dapat dilakukan: (1) periksa kembali NIK yang telah diisi; (2) pastikan calon siswa belum pernah didaftarkan sebelumnya; (3) hubungi admin apabila memerlukan bantuan.",
						"Peringatan", MyMessageboxConfig.OK, MyMessageboxConfig.INFORMATION, new EventListener() {
						...
						}, calonSiswa.getGelombangPendaftaranPsb().getNama(), calonSiswa.getNoRegistrasi(),
						calonSiswa.getNama());
			} else {

				MyMessageboxConfig.showFormatCb(
						"Mohon maaf, NIK calon siswa sudah terdaftar pada jalur \"{V1}\". Langkah yang dapat dilakukan: (1) periksa kembali NIK yang telah diisi; (2) pastikan calon siswa belum pernah didaftarkan sebelumnya; (3) hubungi admin apabila memerlukan bantuan.",
						"Peringatan", MyMessageboxConfig.OK, MyMessageboxConfig.INFORMATION, new EventListener() {
						...
						}, gelombangPendaftaranPsb.getNama());
			}
			return false;
		}
	...
	private void setdata() {
		...
		try {
			...
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sekolah/psb/form/PPDB_Simple6.java:1569");
		}
	}

	public Boolean checkNik() {

		Integer kotaCount = null;
		Session session = HibernateUtil.currentSession();
		kotaCount = ((Number) session.createCriteria(CalonSiswa.class).setProjection(Projections.rowCount())
				.add(Restrictions.eq("nik", nik.getValue().trim()))
				.add(Restrictions.eq("gelombangPendaftaranPsb", gelombangPendaftaranPsb))
				.add(this.calonSiswa.getId() == null ? Restrictions.sqlRestriction("1=1")
						: Restrictions.ne("id", this.calonSiswa.getId()))
				.uniqueResult()).intValue();
