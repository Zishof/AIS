...
/**
 * Tipe khusus untuk ppdb simple2. Kelas ini memberi nama dan batas tanggung jawab yang eksplisit
 ...
 * tempatLahir}, {@code MyDatebox tanggalLahir}, {@code MyCheckboxConfig pernyataan}, {@code Textbox
 * teleponSiswa}, {@code TreeSet selectedKelasLesSiswa}, {@code Combobox penjurusanSekolah}, {@code Row
 * rowParameterTambahan}; inisialisasi/lifecycle ({@code init()}); mutasi data ({@code onSave()}, {@code
 * setdata()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di atas.</p>
 ...
 * @see PPDB
 */
public class PPDB_Simple2 extends PPDB {
	...
	private EventListener checkKesamaan = new EventListener() {
		...
		public void onEvent(Event arg0) throws Exception {
			CalonSiswaAction.CheckKesamaan checkKesamaan = new CalonSiswaAction.CheckKesamaan(calonSiswa,
					gelombangPendaftaranPsb, tanggalLahir, nama, null, PPDB_Simple2.this, new EventListener() {

						@Override
						public void onEvent(Event arg0) throws Exception {
							CalonSiswa calonSiswa1 = (CalonSiswa) arg0.getData();
							PPDB window = new PPDB_Simple2();
	...
	public PPDB_Simple2(CalonSiswa calonSiswa, GelombangPendaftaranPsb gelombangPendaftaranPsb,
	...
	public void init() {
		...
		try {
			...
			rowUtama1.setValign("top");
			rowUtama1.setParent(rows);
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sekolah/psb/form/PPDB_Simple2.java:247");
		}
		...
		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("5. Telp./Nomor WA *"));
		row.appendChild(teleponSiswa = new Textbox(calonSiswa.getTeleponSiswa()));
		teleponSiswa.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("6. Alamat *"));
		...
		try {
			...
			ais.common.ErrorAuditUtil.record(e, "PPDB_Simple2.tampilkanVerifikasiBerkas");
		}
		...
		if (calonSiswa.getId() != null) {
			try {
				...
				e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sekolah/psb/form/PPDB_Simple2.java:513");
			}
		...
		try {
			...
			// TODO Auto-generated catch block
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sekolah/psb/form/PPDB_Simple2.java:522");
		}
		...
		try {
			...
			// TODO Auto-generated catch block
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sekolah/psb/form/PPDB_Simple2.java:535");
		}
		...
		try {
			...
			// TODO Auto-generated catch block
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sekolah/psb/form/PPDB_Simple2.java:545");
		}
		...
		save.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				if (onSave(event)) {
					...
					if (PPDB_Simple2.this.eventListener != null) {
						PPDB_Simple2.this.eventListener.onEvent(new Event("", save, PPDB_Simple2.this.calonSiswa));
					}
					...
					if (baru) {
						...
						informasi = org.apache.commons.lang3.StringUtils.replace(informasi, "[no_reg]",
								PPDB_Simple2.this.calonSiswa.getNoRegistrasi());
									...
									public void onEvent(Event arg0) throws Exception {
										...
										Common.createDefaultTimer(new EventListener() {
											...
											public void onEvent(Event arg0) throws Exception {
												...
												if (baru && PPDB_Simple2.this.calonSiswa.getGelombangPendaftaranPsb()
														.getOtomatisLoginSetelahDaftar()) {
													PPDB_Simple2.this.calonSiswa.setTelahLogin(true);
													PPDB_Simple2.this.calonSiswa
															.setWaktuLogin(ais.ui.util.WaktuUtil.getDate());
													Common.refreshUpdate(PPDB_Simple2.this.calonSiswa);

													Common.setLogin(PPDB_Simple2.this.calonSiswa);
													...
													CalonSiswaAction.onCetakKartu(PPDB_Simple2.this.calonSiswa, daftar);
												}
											}
										});
	...
	public boolean onSave(Event event) throws Exception {
		...
		if (s != null && s.getPenjurusanBolehDipilihSaatPsb()) {
			if (penjurusanSekolah.getParent() != null && penjurusanSekolah.getParent().isVisible()
				...
				MyMessageboxConfig.show(
						"Mohon Bapak/Ibu terlebih dahulu memilih Penjurusan sebelum melanjutkan pendaftaran. Langkah yang dapat dilakukan: (1) buka pilihan Penjurusan; (2) pilih salah satu penjurusan yang tersedia; (3) lanjutkan menyimpan pendaftaran.",
		...
		if (gelombangPendaftaranPsb != null && gelombangPendaftaranPsb.getHarusSebagaiAlumni()) {
			if (alumni == null) {
				MyMessageboxConfig.show(
						"Mohon Bapak/Ibu terlebih dahulu mengisi data Siswa Alumni sebelum melanjutkan pendaftaran. Langkah yang dapat dilakukan: (1) klik kolom Siswa Alumni; (2) pilih data siswa alumni yang sesuai; (3) lanjutkan menyimpan pendaftaran.",
						"Peringatan", MyMessageboxConfig.OK,
						MyMessageboxConfig.INFORMATION, new EventListener() {
		...
		if (keluarga != null && keluarga.getParent() != null && keluarga.getParent().isVisible()) {
			...
			if (k == null) {
				MyMessageboxConfig.show(
						"Mohon Bapak/Ibu terlebih dahulu memilih data Anak Pegawai sebelum melanjutkan pendaftaran. Langkah yang dapat dilakukan: (1) buka pilihan Anak Pegawai; (2) pilih data yang sesuai; (3) lanjutkan menyimpan pendaftaran.",
						"Peringatan", MyMessageboxConfig.OK,
						MyMessageboxConfig.INFORMATION, new EventListener() {
		...
		if (paketPsb.getParent() != null && paketPsb.getParent().isVisible()
			...
			MyMessageboxConfig.show(
					"Mohon Bapak/Ibu terlebih dahulu memilih Paket sebelum melanjutkan pendaftaran. Langkah yang dapat dilakukan: (1) buka pilihan Paket; (2) pilih paket yang tersedia; (3) lanjutkan menyimpan pendaftaran.",
					"Peringatan", MyMessageboxConfig.OK,
		...
		if (nama.getValue().trim().equals("")) {
			MyMessageboxConfig.show(
					"Mohon Bapak/Ibu terlebih dahulu mengisi Nama Lengkap calon siswa. Langkah yang dapat dilakukan: (1) klik kolom Nama Lengkap; (2) isikan nama sesuai dokumen resmi (Akte Kelahiran); (3) lanjutkan menyimpan pendaftaran.",
					"Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION, new EventListener() {
		...
		if (tempatLahir.getValue().trim().equals("")) {
			MyMessageboxConfig.show(
					"Mohon Bapak/Ibu terlebih dahulu mengisi Tempat Lahir calon siswa. Langkah yang dapat dilakukan: (1) klik kolom Tempat Lahir; (2) isikan tempat lahir sesuai dokumen resmi (Akte Kelahiran); (3) lanjutkan menyimpan pendaftaran.",
					"Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION, new EventListener() {
		...
		if (tanggalLahir.getValue() == null) {
			MyMessageboxConfig.show(
					"Mohon Bapak/Ibu terlebih dahulu mengisi Tanggal Lahir calon siswa. Langkah yang dapat dilakukan: (1) klik kolom Tanggal Lahir; (2) pilih tanggal lahir sesuai dokumen resmi (Akte Kelahiran); (3) lanjutkan menyimpan pendaftaran.",
					"Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION, new EventListener() {
		...
		if (teleponSiswa.getValue().trim().equals("")) {
			MyMessageboxConfig.show(
					"Mohon Bapak/Ibu terlebih dahulu mengisi Telepon/Nomor WhatsApp calon siswa. Langkah yang dapat dilakukan: (1) klik kolom Telepon/Nomor WA; (2) isikan nomor yang aktif dan dapat dihubungi; (3) lanjutkan menyimpan pendaftaran.",
					"Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION, new EventListener() {
		...
		if (alamatSiswa.getValue().trim().equals("")) {
			MyMessageboxConfig.show(
					"Mohon Bapak/Ibu terlebih dahulu mengisi Alamat calon siswa. Langkah yang dapat dilakukan: (1) klik kolom Alamat; (2) isikan alamat tempat tinggal secara lengkap dan jelas; (3) lanjutkan menyimpan pendaftaran.",
					"Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION, new EventListener() {
						...
						public void onEvent(Event arg0) throws Exception {
							alamatSiswa.focus();
							alamatSiswa.select();
							Clients.scrollIntoView(alamatSiswa);
						}
					});
			return false;
		}
		...
		if (merupakanPindahan.isChecked()) {
			if (pindahanDariSekolah != null && pindahanDariSekolah.getParent() != null
				...
				MyMessageboxConfig.show(
						"Mohon Bapak/Ibu terlebih dahulu mengisi Asal Sekolah sebelumnya karena calon siswa ditandai sebagai pindahan. Langkah yang dapat dilakukan: (1) klik kolom Asal Sekolah; (2) isikan nama sekolah asal secara lengkap; (3) lanjutkan menyimpan pendaftaran.",
						"Peringatan", MyMessageboxConfig.OK,
						MyMessageboxConfig.INFORMATION, new EventListener() {
			...
			if (kelasSekolahPindahan != null && kelasSekolahPindahan.getParent() != null
				...
				MyMessageboxConfig.show(
						"Mohon Bapak/Ibu terlebih dahulu mengisi Kelas Terakhir pada sekolah sebelumnya. Langkah yang dapat dilakukan: (1) klik kolom Kelas Terakhir; (2) isikan kelas terakhir yang ditempuh; (3) lanjutkan menyimpan pendaftaran.",
						"Peringatan",
			...
			if (alamatSekolahPindahan != null && alamatSekolahPindahan.getParent() != null
				...
				MyMessageboxConfig.show(
						"Mohon Bapak/Ibu terlebih dahulu mengisi Alamat Sekolah sebelumnya. Langkah yang dapat dilakukan: (1) klik kolom Alamat Sekolah; (2) isikan alamat sekolah asal secara lengkap; (3) lanjutkan menyimpan pendaftaran.",
						"Peringatan", MyMessageboxConfig.OK,
						MyMessageboxConfig.INFORMATION, new EventListener() {
			...
			if (tanggalPindah != null && tanggalPindah.getParent() != null && tanggalPindah.getParent().isVisible()
				...
				MyMessageboxConfig.show(
						"Mohon Bapak/Ibu terlebih dahulu mengisi Tanggal Pindah. Langkah yang dapat dilakukan: (1) klik kolom Tanggal Pindah; (2) pilih tanggal kepindahan; (3) lanjutkan menyimpan pendaftaran.",
						"Peringatan", MyMessageboxConfig.OK,
						MyMessageboxConfig.INFORMATION, new EventListener() {
		...
		if (!pernyataan.isChecked()) {
			MyMessageboxConfig.show(
					"Mohon Bapak/Ibu terlebih dahulu menyetujui Pernyataan yang tersedia sebelum menyimpan pendaftaran. Langkah yang dapat dilakukan: (1) baca Pernyataan dengan saksama; (2) centang kotak Pernyataan sebagai tanda persetujuan; (3) lanjutkan menyimpan pendaftaran.",
					"Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION, new EventListener() {
		...
		try {
			...
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sekolah/psb/form/PPDB_Simple2.java:1064");
		}
	...
	private void setdata() {
		...
		try {
			...
			calonSiswa.setTeleponSiswa(teleponSiswa.getValue().trim());
			calonSiswa.setTempatLahir(tempatLahir.getValue().trim());
			...
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sekolah/psb/form/PPDB_Simple2.java:1130");
		}
	}
