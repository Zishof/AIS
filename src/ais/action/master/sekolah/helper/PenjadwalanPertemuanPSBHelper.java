...
public class PenjadwalanPertemuanPSBHelper {
	...
	class PertemuanRenderer extends ais.ui.util.MyRowRenderer {
		...
		public void render(Row arg0, Object arg1) throws Exception {
			...
			final Combobox pertemuana = new Combobox();
			Common.insertCombo(pertemuana, "nama", StatusPertemuan.class,
					Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)));
			Common.selectComboItem(pertemuana, pertemuan.getStatusPertemuan());
			pertemuana.setReadonly(true);
			arg0.appendChild(pertemuana);
			pertemuana.setWidth("90%");
			pertemuana.setReadonly(true);
			pertemuana.addEventListener("onChange", new EventListener() {

				@Override
				public void onEvent(Event arg0) throws Exception {
					pertemuan.setStatusPertemuan((StatusPertemuan) pertemuana.getSelectedItem().getValue());
					Session session = HibernateUtil.currentSession();
					Common.refreshUpdate(session, (pertemuan));
				}
			});
	...
	public boolean save() throws InterruptedException {
		...
		List<Pertemuan> pertemuans = session.createCriteria(Pertemuan.class)
				...
				.add(Restrictions.eq("jadwalPertemuanPSB", jadwalPertemuanPSB)).addOrder(Order.asc("tanggal"))
...
//			pertemuan.setStatusPertemuan(ConstantValues.UJIAN_ONLINE);
	...
	public void display(final JadwalPertemuanPSB jadwalPertemuanPSB, final DataLoader dataLoader) {
		...
		window.setTitle(jadwalPertemuanPSB.infoSimple());
		...
		String matkul1 = jadwalPertemuanPSB.getNama();

		String gelombangPendaftaranPsb = jadwalPertemuanPSB.getGelombangPendaftaranPsb() == null ? "Semua"
				: jadwalPertemuanPSB.getGelombangPendaftaranPsb().getNama();

		String harijam = (", Waktu : " + Common.dateFormat3.get().format(jadwalPertemuanPSB.getWaktuMulai()) + " s.d "
				+ Common.dateFormat3.get().format(jadwalPertemuanPSB.getWaktuSampai()));

		String groupTxt = "Materi Pertemuan : " + matkul1 + ", Gelombang Pendaftaran : " + gelombangPendaftaranPsb
				+ harijam;
		...
		button.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				pertemuan.setJadwalPertemuanPSB(jadwalPertemuanPSB);
				pertemuan.setTopik("Pertemuan ke " + (grid.getRows().getChildren().size()));
				pertemuan.setWaktuMulai(Common.dateFormat3.get().format(jadwalPertemuanPSB.getWaktuMulai()));
				pertemuan.setWaktuSelesai(Common.dateFormat3.get().format(jadwalPertemuanPSB.getWaktuSampai()));
	...
	public void onSearchDefault(Event event) {
		...
		List<Pertemuan> pertemuan = session.createCriteria(Pertemuan.class)
				.add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
				.add(Restrictions.eq("jadwalPertemuanPSB", jadwalPertemuanPSB)).addOrder(Order.asc("mulai"))
