...
public class AktifitasJadwalUjianPSBHelper {
	...
	public Toolbar initAgendaJadwalUjianPSB(final JadwalUjianPSB jadwalUjianPSB, final DataLoader dataLoader) {
		...
		MyToolbarbuttonConfig button = new MyToolbarbuttonConfig("Agenda Jadwal Ujian PSB", "/img/jadwal.png");
		button.addEventListener("onClick", new EventListener() {

			@Override
			public void onEvent(Event event) throws Exception {
				penjadwalanHelper.display(jadwalUjianPSB, dataLoader);
			}
		...
		button.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event event) throws Exception {
				CommonReportHelper.onLaporanAbsensi(jadwalUjianPSB, true);
			}

		});
		button.setParent(hbox);

		AktifitasPerkuliahanHelper.tampilCalender(hbox, dataLoader, jadwalUjianPSB);

		ClassRoomUtil.createButton(jadwalUjianPSB, dataLoader).setParent(hbox);
	...
	public void initDetail(final JadwalUjianPSB jadwalUjianPSB, final DataLoader mydataLoader, final Div groupbox)
		...
		final DataLoader dataLoader = mydataLoader == null ? new DataLoader() {
			...
			public void loadData(Object value) {
				try {
					initDetail(jadwalUjianPSB, groupbox);
				} catch (Exception e) {
		...
		Tab tab = new Tab("Agenda " + jadwalUjianPSB.getNama());
		tab.setParent(tabs);
		...
		myGroupbox.appendChild(initAgendaJadwalUjianPSB(jadwalUjianPSB, new DataLoader() {
		...
		TreeMap<String, Long> pertemuans = jadwalUjianPSB.ambilPertemuan();
		...
		if (tbmuser != null && tbmuser.getMahasiswa() == null &&  tbmuser.getSiswa() == null && tbmuser.getSiswa() == null
			...
			pertemuan.setTopik(jadwalUjianPSB.getNama());
			...
			try {
				...
				e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sekolah/helper/AktifitasJadwalUjianPSBHelper.java:187");
			}
			...
			Common.createDefaultTimer(new EventListener() {
				...
				public void onEvent(Event arg0) throws Exception {
					jadwalUjianPSB.belum();
					initDetail(jadwalUjianPSB, dataLoader, groupbox);
				}
			});
