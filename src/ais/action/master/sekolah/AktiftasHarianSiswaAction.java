...
/**
 * Controller/action ZK untuk aktiftas harian siswa. Tipe ini merupakan titik masuk UI yang
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code Textbox searchnama}, {@code Datebox
 * searchtanggal}, {@code MyGrid grid}, {@code Paging paging}, {@code MyToolbarbuttonConfig add}, {@code Tabpanel
 * panelDaftarSiswa}, {@code Tabpanel panelDashboard}, {@code Tabpanel panelJenisAktiftasHarianDefault};
 * inisialisasi/lifecycle ({@code doAfterCompose()}, {@code initDefaultMasterData()}, {@code initCriteria()},
 * {@code initCriteria()}, {@code init()}); pembacaan/pencarian ({@code onSearchDefault()}); mutasi data ({@code
 * onSave()}); penghapusan/pembatalan ({@code onCancel()}); operasi domain lain ({@code
 * onDashboardAktifitasHarianSiswa()}, {@code onBukuPenghubung()}, {@code showPopupForm()}, {@code
 * onDaftarAktifitasSiswa()}, {@code onJenisAktiftasHarianDefault()}, {@code onJenisMateriHarianDefault()}).
...
public class AktiftasHarianSiswaAction extends GenericAutowireComposer implements DataCriteria, DataSearchDefault, DataInitDefault {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link AktiftasHarianSiswaAction}. Kelas ini menerjemahkan satu item
	 * data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link AktiftasHarianSiswaAction} dan dapat mengakses
