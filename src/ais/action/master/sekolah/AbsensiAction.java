 ...
 * paging}, {@code MyGrid grid}, {@code AmbilDataGuruBanbox searchguru}, {@code Combobox searchkelas}, {@code
 * AmbilDataRuangBanbox searchruang}, {@code Combobox searchhari}, {@code Combobox searchTahunAjaran};
 * inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code doAfterCompose()}, {@code initCriteria()});
 * pembacaan/pencarian ({@code onSearchDefault()}); mutasi data ({@code onProsesKehadiran()}); operasi domain
 * lain ({@code onEkstrakurikuler()}, {@code onPraJadwalPelajaran()}, {@code onAbsensiSp()}, {@code
 * onAbsensiRemedial()}, {@code onLaporanKehadiranGuru()}, {@code onLaporanKehadiranMahasiswa()}). Bagian lain
...
public class AbsensiAction extends GenericAutowireComposer {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link AbsensiAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link AbsensiAction} dan dapat mengakses state kelas
	 * induk. Jangan menyimpan atau membagikannya lintas desktop/session.</p>
	 * <p>Kontrak yang tampak dari deklarasi ini meliputi state utama: {@code DetailpertemuanHelper
	 * detailpertemuanHelper}; operasi lokal: {@code render}(). Aturan bisnis bersama tetap berada pada kelas induk
