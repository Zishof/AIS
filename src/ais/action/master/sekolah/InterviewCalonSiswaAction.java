...
/**
 * Controller/action ZK untuk interview calon siswa. Tipe ini merupakan titik masuk UI yang
 * menghubungkan event layar dengan perilaku domain yang diwarisi atau dikonfigurasi khusus oleh
 * kelas ini.
 *
 * <p><b>Batas tanggung jawab:</b> perilaku umum, validasi, akses data, serta lifecycle tetap dimiliki {@link
 * org.zkoss.zk.ui.util.GenericAutowireComposer}. Kelas ini hanya boleh memuat perbedaan yang benar-benar
 * spesifik untuk variasi ini; perubahan yang berlaku bagi seluruh keluarga harus ditempatkan di kelas induk agar
 * fungsi tidak bercabang atau tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code Listbox lbDaftarSesi}, {@code Button
 * btnBuatSesiBar}; inisialisasi/lifecycle ({@code doAfterCompose()}, {@code buatFormGrid()});
 * pembacaan/pencarian ({@code tampilkanInterview()}); penghapusan/pembatalan ({@code hapusSesi()}); operasi
 * domain lain ({@code muatDaftarSesi()}, {@code addRow()}, {@code bukaFormSesi()}, {@code bukaFormPeserta()}).
