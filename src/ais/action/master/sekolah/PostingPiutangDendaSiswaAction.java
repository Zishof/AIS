...
/**
 * Controller/action ZK untuk posting piutang denda siswa. Tipe ini merupakan titik masuk UI yang
 ...
 */
public class PostingPiutangDendaSiswaAction extends GenericAutowireComposer {
					...
					public void onEvent(Event event) throws Exception {
						...
						if (i == MyMessageboxConfig.OK) {
							List<Tagihan> tagihans = initCriteria(true)
									.add(Restrictions.isNotNull("postingHistoryDenda")).list();

							for (Tagihan tagihan : tagihans) {
								tagihan.setPostingHistoryDenda(null);
								...
								HibernateUtil.currentSession()
										.createSQLQuery("delete from akunting.grup_transaksi where tagihan="
												...
												+ PostingHistory.JENIS_PIUTANG_DENDA_SISWA + "'" + " and closing is null")
										.executeUpdate();
							}
						}
								...
								if (i == MyMessageboxConfig.OK) {
									...
									new Thread(new Runnable() {
										...
										public void run() {
											...
											PostingHistory postingHistoryDenda = new PostingHistory(
													PostingHistory.JENIS_PIUTANG_DENDA_SISWA);
											postingHistoryDenda.setTanggal(tgl);
											postingHistoryDenda.setTbmuser(tbmuser);
											postingHistoryDenda.setKeterangan(keterangan.getValue().trim() + " \nTgl:"
													+ Common.dateFormat.get().format(tglMulai.getValue()) + " s.d "
													+ Common.dateFormat.get().format(tglSampai.getValue()));
											session.getTransaction().begin();
											session.save(postingHistoryDenda);
											session.getTransaction().commit();

											List<Tagihan> tagihans = initCriteria(true)
													.add(Restrictions.isNull("postingHistoryDenda")).list();

											int rowIndex = 1;
											for (Tagihan tagihan : tagihans) {
												if (tagihan != null && tagihan.getItemBiayaSekolah() != null) {

													Akun akunDebet = tagihan.getItemBiayaSekolah()
															.getAkunPiutangDenda();
													Akun akunKredit = tagihan.getItemBiayaSekolah().getAkunDenda();
													...
													if (akunDebet != null && akunKredit != null) {
														...
														try {
															...
															if (nilai > 0.1) {
																CommonAkunting.saveTransaksi(akunDebet, akunKredit,
																		...
																		postingHistoryDenda, apakahUangMasuk, ket,
																		tagihan.getTanggalDeadline(), nilai, denda,
																		tagihan,
																		tagihan.getPengaturanBiaya()
																				.getSekolah().getSatuanKerja(),
																		ais.action.master.helper.PostingJurnalHelper.REF_DENDA_SISWA, session);
															} else {
														...
														tagihan.setPostingHistoryDenda(postingHistoryDenda);
														session.getTransaction().begin();
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link PostingPiutangDendaSiswaAction}. Kelas ini menerjemahkan satu
	 * item data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link PostingPiutangDendaSiswaAction} dan dapat
	 ...
	 *
	 * @see PostingPiutangDendaSiswaAction
	 */
	class TagihanRenderer extends ais.ui.util.MyRowRenderer {
		...
		public void render(final Row arg0, Object arg1) throws Exception {
			...
			new Label(Common.dateFormat3.get().format(tagihan.getTanggalDeadline())).setParent(arg0);
			Akun akunDebet = tagihan.getItemBiayaSekolah().getAkunPiutangDenda();
			Akun akunKredit = tagihan.getItemBiayaSekolah().getAkunDenda();
			...
			new Label(tagihan.getPostingHistoryDenda() == null ? Common.getBahasaConfig("Belum diposting")
					: tagihan.getPostingHistoryDenda().toString()).setParent(arg0);
			...
			if (akunDebet != null && akunKredit != null) {
				...
				button.setVisible(edit && adminLain && tagihan.getPostingHistoryDenda() != null);
				...
				button.setVisible(edit && tagihan.getPostingHistoryDenda() == null && tbmuser != null
					...
					public void onEvent(Event event) throws Exception {
						...
						Common.createDefaultTimer(new EventListener() {
							...
							public void onEvent(Event arg0) throws Exception {
								...
								PostingHistory postingHistoryDenda = new PostingHistory(
										PostingHistory.JENIS_PIUTANG_DENDA_SISWA);
								postingHistoryDenda.setTbmuser(Common.getCurrentUser());
								postingHistoryDenda.setTanggal(ais.ui.util.WaktuUtil.getDate());
								postingHistoryDenda.setKeterangan(
								...
								if (akunDebet != null && akunKredit != null) {
									...
									if (nilai > 0.1) {
										CommonAkunting.saveTransaksi(akunDebet, akunKredit, akunDenda, akunPiutangDenda,
												postingHistoryDenda, apakahUangMasuk, ket, tagihan.getTanggalDeadline(),
	...
	public Criteria initCriteria(boolean order) {
		...
		Criteria criteria = session.createCriteria(Tagihan.class).add(ais.action.master.helper.PostingJurnalHelper.restriksiPosting("postingHistoryDenda", sudahPostingDasbor))
				...
				.add(searchtelahtampil.isChecked() ? Restrictions.isNotNull("postingHistoryDenda")
						: !searchtampil.isChecked() ? Restrictions.sqlRestriction("1=1")
						: Restrictions.isNull("postingHistoryDenda"))
	...
	/**
	 * Kriteria tagihan berdenda pada rentang tanggal -- sama dengan penghitung baris
	 * "Siswa - Piutang Denda" pada dasbor: punya tanggal jatuh tempo dan denda &gt; 0,1.
	 */
	...
	/**
	 * Batalkan posting SEMUA piutang denda dalam rentang.
	 *
	 * <p>Penandanya {@code postingHistoryDenda} dan penghapusannya menyaring
	 * {@code jenis = PIUTANG_DENDA_SISWA} -- satu baris {@code tagihan} memikul beberapa
	 * jurnal sekaligus.</p>
	 */
	@SuppressWarnings("unchecked")
		...
		try {
			...
			for (Tagihan tagihan : daftar) {
				try {
					...
					ais.common.ErrorAuditUtil.record(e, "PostingPiutangDendaSiswaAction jalur API");
				}
			}
		} finally {
			try {
	...
	/**
	 * Posting SEMUA piutang denda yang belum dijurnal dalam rentang.
	 *
	 * <p>Debet = {@code itemBiayaSekolah.akunPiutangDenda}, kredit =
	 * {@code itemBiayaSekolah.akunDenda}, nilai = {@code tagihan.denda}, tanggal jurnal =
	 * {@code tanggalDeadline} (tanggal jatuh tempo, bukan tanggal tagihan -- dendanya baru
	 * lahir saat lewat tempo). Bila nilainya &le; 0,1 posisi ditukar.</p>
	 *
	...
	public static int postingSemua(java.util.Date mulai, java.util.Date sampai, Tbmuser oleh,
		...
		try {
			List<Long> ids = kriteriaPostingStatic(session, mulai, sampai)
					.add(Restrictions.isNull("postingHistoryDenda"))
			...
			postingHistory.setKeterangan("Posting massal piutang denda siswa dari dasbor jurnal"
			...
			for (Long id : ids) {
				try {
					...
					try {
						...
						if (nilai > 0.1) {
							CommonAkunting.saveTransaksi(akunDebet, akunKredit, null, null, postingHistory,
									true, ket, tagihan.getTanggalDeadline(), nilai, Double.valueOf(0.0),
									tagihan, satuanKerja, ais.action.master.helper.PostingJurnalHelper.REF_DENDA_SISWA, session);
						} else {
					...
					if (tersimpan) {
						tagihan.setPostingHistoryDenda(postingHistory);
						session.getTransaction().begin();
						session.update(tagihan);
						session.getTransaction().commit();
						n++;
