...
/**
 * Controller/action ZK untuk yayasan. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * paging}, {@code MyGrid grid}, {@code Textbox searchnama}, {@code org.zkoss.zul.Checkbox abaikanDomain}, {@code
 * Textbox nama}, {@code Textbox alamat}, {@code Textbox email}; inisialisasi/lifecycle ({@code
 * doBeforeCompose()}, {@code doAfterCompose()}, {@code init()}, {@code init()}, {@code initCriteria()}, {@code
 * reInitByDomain()}); pembacaan/pencarian ({@code onSearchDefault()}); validasi/perhitungan ({@code
 * checkNamaYayasan()}); mutasi data ({@code onSave()}); operasi domain lain ({@code onAdd()}). Bagian lain dari
...
public class YayasanAction extends GenericAutowireComposer implements DataCriteria, DataSearchDefault, DataInitDefault {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link YayasanAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link YayasanAction} dan dapat mengakses state kelas
