...
/**
 * Tipe khusus untuk chat window. Kelas ini memberi nama dan batas tanggung jawab yang eksplisit
 ...
 * tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code ChatRoom chatroom}, {@code Chatter
 * chatter}, {@code boolean isLogin}, {@code Vbox msgBoard}, {@code Textbox msg}, {@code Tbmuser current}, {@code
 * Tbmuser friend}, {@code EventListener eventListener}; inisialisasi/lifecycle ({@code init()}); mutasi data
 * ({@code setLogin()}); operasi domain lain ({@code onCreate()}, {@code onOK()}, {@code onLogin()}, {@code
 * onExit()}, {@code onSendMsg()}, {@code isLogin()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau
