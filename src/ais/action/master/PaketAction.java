 ...
 * boolean edit}, {@code boolean delete}; inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code
 * doAfterCompose()}, {@code init()}, {@code initCriteria()}); pembacaan/pencarian ({@code onSearchDefault()});
 * validasi/perhitungan ({@code checkNamaPaket()}); mutasi data ({@code onSave()}); operasi domain lain ({@code
...
public class PaketAction extends GenericAutowireComposer {
	...
	private void init(Paket paket) {
		...
		row.appendChild(nama = new Textbox(paket.getKode()));
		nama.setWidth("90%");
	...
	public boolean onSave(Event event) throws Exception {
		if (nama.getValue().trim().equals("")) {
			PesanFormalHelper.tampilkanGagal("penyimpanan data Paket",
					"Kolom Nama Paket belum Bapak/Ibu isi, padahal kolom ini wajib diisi sebelum data dapat disimpan.",
					new String[] {
							"Isi/pilih terlebih dahulu Nama Paket.",
		...
		if (i) {
			PesanFormalHelper.tampilkanGagal("penyimpanan data Paket",
					"Nama Paket sudah terdaftar sebelumnya di database, sehingga tidak dapat disimpan kembali untuk menghindari duplikasi data.",
					new String[] {
							"Gunakan nama paket yang berbeda dari data yang sudah ada.",
	...
	public void onSearchDefault(Event event) {
		...
		List<Paket> paket = initCriteria(true).setMaxResults(Common.ROWS_COUNT_ON_PAGE)
