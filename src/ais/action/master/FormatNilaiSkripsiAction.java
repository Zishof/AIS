...
/**
 * Controller/action ZK untuk format nilai skripsi. Tipe ini merupakan titik masuk UI yang
 ...
 * searchjurusan}, {@code Combobox searchprogram}, {@code Combobox searchstatusAwalMahasiswa};
 * inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code doAfterCompose()}, {@code init()}, {@code
 * initCriteria()}); pembacaan/pencarian ({@code reloadKomponen()}, {@code onSearchDefault()});
 * validasi/perhitungan ({@code checkBobot()}); mutasi data ({@code onSave()}); operasi domain lain ({@code
 * onJenis()}, {@code onAdd()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut
 * di atas.</p>
...
public class FormatNilaiSkripsiAction extends GenericAutowireComposer {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link FormatNilaiSkripsiAction}. Kelas ini menerjemahkan satu item data
	 * menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link FormatNilaiSkripsiAction} dan dapat mengakses
	...
	private void init(FormatNilaiSkripsi formatNilaiSkripsi) {
		...
		Common.selectComboItem(program, formatNilaiSkripsi.getProgram());
		...
		Common.selectComboItem(true, statusAwalMahasiswa, formatNilaiSkripsi.getStatusAwalMahasiswa());
...
//		Common.selectComboItem(jenis, formatNilaiSkripsi.getJenis());
	...
	public boolean onSave(Event event) throws Exception {
		...
		formatNilaiSkripsi.setProgram(
				(String) (program.getSelectedItem() == null || program.getSelectedItem().getValue() == null ? null
						: program.getSelectedItem().getValue()));
		formatNilaiSkripsi.setStatusAwalMahasiswa((StatusAwalMahasiswa) (statusAwalMahasiswa.getSelectedItem() == null
