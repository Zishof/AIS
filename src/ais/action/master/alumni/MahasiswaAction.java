 ...
 * {@code Textbox searchnama}, {@code Textbox searchta}, {@code Combobox searchfakultas}, {@code Combobox
 * searchjurusan}, {@code Decimalbox searchtahun}, {@code Decimalbox searchtahunLulus}; inisialisasi/lifecycle
 * ({@code doBeforeCompose()}, {@code doAfterCompose()}, {@code initCriteria()}); pembacaan/pencarian ({@code
 * onTampilTranskripAkademik()}, {@code onSearchDefault()}, {@code loadData()}); operasi domain lain ({@code
 * onDashboardAlumni()}, {@code onKartuMahasiswa()}, {@code onGaleriFoto()}, {@code onParameterAlumni()}, {@code
 * onRekapJumlahAlumni()}, {@code onGrafikJumlahAlumni()}). Bagian lain dari kontrak tetap mengikuti kelas induk
...
public class MahasiswaAction extends GenericAutowireComposer implements DataLoader, DataCriteria, DataSearchDefault {
	 ...
	 * jenisSemester}, {@code String hr}, {@code String keyword}, {@code boolean merupakanPraPerkuliahan}, {@code
	 * Integer ekstrakurikuler}, {@code boolean merupakanRemedial}, {@code Tbmuser user}; operasi lokal: {@code
