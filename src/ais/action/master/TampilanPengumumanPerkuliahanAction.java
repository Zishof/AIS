...
/**
 * Controller/action ZK untuk tampilan pengumuman perkuliahan. Tipe ini merupakan titik masuk UI
 ...
 * PengumumanPerkuliahan pengumumanPerkuliahan}, {@code Textbox catatan}, {@code MyGrid gridKomentar}, {@code
 * MyGrid grids}, {@code Boolean readonly}, {@code Borderlayout utama}, {@code DiskusiPengumumanPerkuliahan
 * diskusiPengumumanPerkuliahan}; inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code doAfterCompose()},
 * {@code initCriteria()}, {@code init()}); pembacaan/pencarian ({@code loadMenu()}, {@code loadData()}, {@code
 * onSearchDefault()}, {@code loadDataAttachment()}, {@code loadData()}, {@code getReadonly()}); mutasi data
 * ({@code prosess()}, {@code onSave()}, {@code setReadonly()}); operasi domain lain ({@code
 * displayDetailPertemuanFileContent()}, {@code kirimEmail()}, {@code broadcastEmail()}, {@code
 * kirimEmailKeKorespondensi()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang
 * disebut di atas.</p>
...
public class TampilanPengumumanPerkuliahanAction extends GenericAutowireComposer {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link TampilanPengumumanPerkuliahanAction}. Kelas ini menerjemahkan
	 * satu item data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link TampilanPengumumanPerkuliahanAction} dan dapat
	 ...
	 *
	 * @see TampilanPengumumanPerkuliahanAction
	 */
	 ...
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link TampilanPengumumanPerkuliahanAction} dan dapat
	 * mengakses state kelas induk. Jangan menyimpan atau membagikannya lintas desktop/session.</p>
	 * <p>Kontrak yang tampak dari deklarasi ini meliputi state utama: {@code MyDetail detail}; operasi lokal:
