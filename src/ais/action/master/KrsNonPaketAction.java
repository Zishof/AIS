...
import ais.action.master.helper.KrsNonPaketHelper;
...
/**
 * Controller/action ZK untuk krs non paket. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * grid}, {@code Mahasiswa mahasiswa}, {@code MyColumnConfig colSemester}, {@code MyColumnConfig colTahapan},
 * {@code Integer semesterPendek}, {@code Combobox semesterMulai}, {@code Combobox semesterSampai};
 * inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code doAfterCompose()}); pembacaan/pencarian ({@code
 * onSearchDefaultKeDatabase()}, {@code onSearchDefault()}, {@code load()}). Bagian lain dari kontrak tetap
 ...
 */
public class KrsNonPaketAction extends GenericAutowireComposer {
		...
		if (mulai.equals(sampai)) {
			...
			for (String[] data : datas) {
				...
				if (semester > 0) {
					...
					KrsNonPaketHelper krsNonPaketHelper = new KrsNonPaketHelper(semesterPendek);
					krsNonPaketHelper.display(editable, mahasiswa, tahunAjaran, semester, tahapan, rowUtama, addWindow,
				...
				tabboxKrs.tambahTabLazy(index, tahunAjaran + "/" + semester,
						new ais.ui.util.MyButtonTabbox.PemuatTab() {
							...
							public void muat(org.zkoss.zul.Div panel) throws Exception {
								Boolean editable = true;
								KrsNonPaketHelper krsNonPaketHelper = new KrsNonPaketHelper(semesterPendek);
								krsNonPaketHelper.display(editable, mahasiswa, tahunAjaran, semester, tahapan, panel,
										addWindow, html, komentarshtml, keDatabase);
							}
						});
				if (ada) {
