...
/**
 * Tipe khusus untuk transaksi tabel bobot nilai. Kelas ini memberi nama dan batas tanggung jawab
 ...
 * @see MyWindow
 */
public class TransaksiTabelBobotNilai extends MyWindow {
	...
	public TransaksiTabelBobotNilai() {
		super();
		try {
			init();
			// initSpreadsheet();
	...
	public TransaksiTabelBobotNilai(String title, String border,
			boolean closable) {
	...
	private void init() throws Exception {
		...
		row.appendChild(searchjurusan);searchjurusan.setWidth("90%");
		...
		MyToolbarbuttonConfig print = new MyToolbarbuttonConfig("Export Epsbed (TBBNL.xls)",
				"/img/excel.png");
			...
			public void onEvent(Event event) throws Exception {
				try {
					...
					Filedownload.save(bout.toByteArray(),
							"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "TBBNL.xlsx");
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/epsbed/TransaksiTabelBobotNilai.java:221");
	...
	private void initSpreadsheet() throws Exception {
		...
		final List<NilaiHuruf> nilaiHurufs = HibernateUtil
				.currentSession()
				.createCriteria(NilaiHuruf.class)
				...
				// .add(Restrictions.gt("tahunAngkatan", 1800))
				.list();
		...
		spreadsheet.setMaxrows(nilaiHurufs.size() + 1);
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 0, "THSMSTBBN");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 1, "KDPTITBBN");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 2, "KDJENTBBN");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 3, "KDPSTTBBN");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 4, "NLAKHTBBN");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 5, "BOBOTTBBN");
		...
		// try {
		// Thread.sleep(500);
		// } catch (Exception e1) { ais.common.ErrorAuditUtil.record(e1, "auto-audit(empty-catch) src/ais/action/master/epsbed/TransaksiTabelBobotNilai.java:288");
		...
		for (NilaiHuruf nilaiHuruf : nilaiHurufs) {
			...
			// System.out.println("jurusan : " + jurusan.getId());
					...
					CommonEpsbed
							.getTahunSemesterPelaporan((String) tahunakademik
									...
									(String) jenisSemester.getSelectedItem()
											.getValue()));
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 1, perguruanTinggi == null || perguruanTinggi.getId() == null ? ""
					: perguruanTinggi.getKodePerguruanTinggi());
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 2,
					nilaiHuruf.getJurusan() == null ? "" : nilaiHuruf
							.getJurusan().getJenjang().getKode());
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 3,
					nilaiHuruf.getJurusan() == null ? "" : nilaiHuruf
							.getJurusan().getKodeEpsbed());

			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 4, nilaiHuruf.getNilaiHuruf());
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 5, nilaiHuruf.getNilaiDiIPK()
					.toString());
			rowIndex++;
		}
