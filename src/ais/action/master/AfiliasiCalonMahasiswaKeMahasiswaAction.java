...
import ais.action.master.pmb.AfiliasiCalonMahasiswaKeMahasiswaDetailAction;
...
/**
 * Controller/action ZK untuk afiliasi calon mahasiswa ke mahasiswa. Tipe ini merupakan titik masuk
 ...
 * {@code Textbox searchnama}, {@code Textbox searchcalon}, {@code Checkbox searchaktif}, {@code Combobox
 * searchTahunAjaran}, {@code MyToolbarbuttonConfig add}, {@code String contents}; inisialisasi/lifecycle ({@code
 ...
 */
public class AfiliasiCalonMahasiswaKeMahasiswaAction extends GenericAutowireComposer
	...
	public static String[] contents = new String[] { "afiliasiMahasiswa.nim", "afiliasiMahasiswa.nama", "noRegistrasi",
			"noUjian", "nama", "totalSkor", "alamat", "rt", "rw", "kelurahanCalon", "kecamatanCalon", "kotaCalon",
			"propinsiCalon", "namaSekolahAsal", "namaSekolahAsal.kode",
		...
		MyToolbarbuttonConfig cetakToolbarbutton = Common.cetakDataCustomButton(BiodataCalonMahasiswa.class,
				new DataCriteria() {
					...
					public Object initCriteria(boolean order) {
						List<Long> pegawais = AfiliasiCalonMahasiswaKeMahasiswaAction.this.initCriteria(true)
						...
						return session.createCriteria(BiodataCalonMahasiswa.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
								...
								.createAlias("afiliasiMahasiswa", "afiliasiMahasiswa")
								.addOrder(Order.asc("afiliasiMahasiswa.nama")).addOrder(Order.asc("noRegistrasi"))
								.add(pegawais.isEmpty() ? Restrictions.sqlRestriction("false")
										: Restrictions.in("afiliasiMahasiswa.id", pegawais));
					}
				}, "Download Afiliasi Calon Mahasiswa", "/img/print.png",
				AfiliasiCalonMahasiswaKeMahasiswaAction.contents);
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link AfiliasiCalonMahasiswaKeMahasiswaAction}. Kelas ini menerjemahkan
	 * satu item data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link AfiliasiCalonMahasiswaKeMahasiswaAction} dan
	 ...
	 *
	 * @see AfiliasiCalonMahasiswaKeMahasiswaAction
	 */
	class AfiliasiCalonMahasiswaRenderer extends ais.ui.util.MyRowRenderer {
		...
		public void render(final Row arg0, Object arg1) throws Exception {
			arg0.setValign("top");
			// TODO Auto-generated method stub
			final Mahasiswa afiliasiMahasiswa = (Mahasiswa) arg1;

			(new AfiliasiCalonMahasiswaKeMahasiswaDetailAction(afiliasiMahasiswa, searchTahunAjaran)).setParent(arg0);

			new MyLabelKecil(afiliasiMahasiswa.getNim() == null ? "" : afiliasiMahasiswa.getNim()).setParent(arg0);

			RevisiHelper.createNewRevisi(Mahasiswa.class, afiliasiMahasiswa, afiliasiMahasiswa.getNama())
					.setParent(arg0);

			CommonMedia.tampilkanGambarKecil(afiliasiMahasiswa).setParent(arg0);
			...
			Number d = (Number) session.createCriteria(BiodataCalonMahasiswa.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
					...
					.setProjection(Projections.rowCount()).add(Restrictions.eq("afiliasiMahasiswa", afiliasiMahasiswa))
					.uniqueResult();
								...
								public Object[] initCriteria(boolean order) {
									...
									try {
										...
										Criteria criteria = session.createCriteria(BiodataCalonMahasiswa.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
												...
												.add(Restrictions.eq("afiliasiMahasiswa", afiliasiMahasiswa));
			...
			new MyLabelKecil(afiliasiMahasiswa.getJurusan() == null ? "" : afiliasiMahasiswa.getJurusan().getNama())
					.setParent(arg0);

			new MyLabelKecil(
					afiliasiMahasiswa.getJurusan() == null || afiliasiMahasiswa.getJurusan().getFakultas() == null ? ""
							: afiliasiMahasiswa.getJurusan().getFakultas().getNama())
					.setParent(arg0);
		...
		List<Long> ids = session.createCriteria(BiodataCalonMahasiswa.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
				.add(searchTahunAjaran.getSelectedItem() == null
						|| searchTahunAjaran.getSelectedItem().getValue() == null ? Restrictions.sqlRestriction("true")
								: Restrictions.eq("tahunAkademik", searchTahunAjaran.getSelectedItem().getValue()))
				.add(Restrictions.isNotNull("afiliasiMahasiswa"))
				...
				.setProjection(Projections.groupProperty("afiliasiMahasiswa.id")).list();
	...
	public void onSearchDefault(Event event) {
		...
		List<Mahasiswa> afiliasiCalonMahasiswa = initCriteria(true).setMaxResults(Common.ROWS_COUNT_ON_PAGE)
