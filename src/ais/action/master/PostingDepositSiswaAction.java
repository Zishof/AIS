...
/**
 * Controller/action ZK untuk posting deposit siswa. Tipe ini merupakan titik masuk UI yang
 ...
 * {@code Textbox searchnama}, {@code Textbox searchnamamhs}, {@code Combobox searchyayasan}, {@code
 * MyCheckboxConfig searchtampil}, {@code MyCheckboxConfig searchtelahtampil}, {@code Combobox searchsekolah};
...
public class PostingDepositSiswaAction extends GenericAutowireComposer {
	...
	public void onPostingSemua(Event event) throws Exception {
		...
		addWindow.setTitle("Posting Tabungan Siswa");
								...
								if (i == MyMessageboxConfig.OK) {
									...
									new Thread(new Runnable() {
										...
										public void run() {
											...
											SatuanKerja s = (SatuanKerja) PostingDepositSiswaAction.this.satuanKerja
											...
											for (Deposit deposit : deposits) {
												...
												SatuanKerja satuanKerja = (SatuanKerja) (deposit != null
														&& deposit.getSiswa() != null
														&& deposit.getSiswa().getSekolah() != null
														&& deposit.getSiswa().getSekolah().getSatuanKerja() != null
																? deposit.getSiswa().getSekolah().getSatuanKerja()
																: s);
												...
												if (deposit != null && deposit.getJenisTabungan() != null) {
													...
													try {
														...
														if (akunDebet != null && akunKredit != null) {
															...
															try {
																...
																if (deposit.getSiswa() != null) {

																	Siswa siswa = (deposit.getSiswa() == null ? null
																			: deposit.getSiswa());
																	...
																	CalonSiswa calonSiswa = deposit.getCalonSiswa();
																	ket = "Tabungan "
																			+ (calonSiswa == null ? ""
													...
													} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/PostingDepositSiswaAction.java:430");
														// TODO: handle
														// exception
													}
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link PostingDepositSiswaAction}. Kelas ini menerjemahkan satu item
	 * data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link PostingDepositSiswaAction} dan dapat mengakses
	...
	class DepositRenderer extends ais.ui.util.MyRowRenderer {
		...
		public void render(final Row arg0, Object arg1) throws Exception {
			...
			if (deposit.getSiswa() != null) {
				new Label(deposit.getSiswa().getNim()).setParent(arg0);

				RevisiHelper.createNewRevisi(Deposit.class, deposit, deposit.getSiswa().getNama()).setParent(arg0);
				...
				new Label(deposit.getSiswa() == null || deposit.getSiswa().getSekolah() == null ? ""
						: deposit.getSiswa().getSekolah().getNama()).setParent(arg0);
				new Label(deposit.getSiswa() == null || deposit.getSiswa().getSekolah() == null
						|| deposit.getSiswa().getSekolah().getYayasan() == null ? ""
								: deposit.getSiswa().getSekolah().getYayasan().getNama())
						.setParent(arg0);
			} else if (deposit.getCalonSiswa() != null) {
				new Label(deposit.getCalonSiswa().getNoRegistrasi()).setParent(arg0);

				RevisiHelper.createNewRevisi(Deposit.class, deposit, deposit.getCalonSiswa().getNama()).setParent(arg0);
				...
				Sekolah sekolah = deposit.getCalonSiswa().getSekolah();

				new Label(sekolah == null ? "" : sekolah.getNama()).setParent(arg0);
				new Label(sekolah == null || sekolah.getYayasan() == null ? "" : sekolah.getYayasan().getNama())
						.setParent(arg0);
			}
								...
								if (akunDebet != null && akunKredit != null) {
									...
									try {
										...
										if (deposit.getSiswa() != null) {
											...
											Siswa siswa = (deposit.getSiswa() == null ? null : deposit.getSiswa());

											ket = "Tabungan "
													+ (siswa == null ? "" : siswa.getNim() + "-" + siswa.getNama())
													+ " - " + deposit.getJenisTabungan().getNama() + " - "
													+ deposit.getKeterangan();
										} else if (deposit.getCalonSiswa() != null) {
											CalonSiswa calonSiswa = deposit.getCalonSiswa();
											ket = "Tabungan "
													+ (calonSiswa == null ? ""
															: calonSiswa.getNoRegistrasi() + "-" + calonSiswa.getNama())
													+ " - " + deposit.getJenisTabungan().getNama() + " - "
													+ deposit.getKeterangan();
										}
									...
									SatuanKerja satuanKerja = (SatuanKerja) (deposit != null
											&& deposit.getSiswa() != null && deposit.getSiswa().getSekolah() != null
											&& deposit.getSiswa().getSekolah().getSatuanKerja() != null
													? deposit.getSiswa().getSekolah().getSatuanKerja()
	...
	public Criteria initCriteria(boolean order) {
		...
		Criteria criteria = session.createCriteria(Deposit.class)
				...
				.add(Restrictions.or(Restrictions.isNotNull("siswa"), Restrictions.isNotNull("calonSiswa")))
		...
		criteria.createAlias("siswa", "siswa", Criteria.LEFT_JOIN)
				...
				.add(searchnamamhs.getValue().trim().isEmpty() ? Restrictions.sqlRestriction("1=1")
						: Restrictions.or(
								Restrictions.ilike("siswa.nama", searchnamamhs.getValue().trim(), MatchMode.ANYWHERE),
								Restrictions.ilike("calonSiswa.nama", searchnamamhs.getValue().trim(),
										MatchMode.ANYWHERE)))
						...
						: Restrictions.or(
								Restrictions.or(
										Restrictions.ilike("siswa.nomorInduk", searchnama.getValue().trim(),
												MatchMode.ANYWHERE),
										Restrictions.ilike("calonSiswa.nomorInduk", searchnama.getValue().trim(),
												MatchMode.ANYWHERE)),
								Restrictions.or(
										Restrictions.ilike("siswa.nomorIndukNasional", searchnama.getValue().trim(),
												MatchMode.ANYWHERE),
										Restrictions.ilike("calonSiswa.nomorIndukNasional",
