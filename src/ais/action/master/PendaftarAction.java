...
/**
 * Controller/action ZK untuk pendaftar. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * searchaktif}, {@code Checkbox abaikanDomain}, {@code Textbox nama}; inisialisasi/lifecycle ({@code
 * doBeforeCompose()}, {@code doAfterCompose()}, {@code init()}, {@code init()}, {@code initCriteria()});
 * pembacaan/pencarian ({@code onSearchDefault()}); validasi/perhitungan ({@code checkNamaPendaftar()}); mutasi
 * data ({@code onSave()}); operasi domain lain ({@code onAdd()}, {@code gambarDaftarToko()}, {@code
 * gambarPenggunaPendaftar()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut
 * di atas.</p>
...
public class PendaftarAction extends GenericAutowireComposer
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link PendaftarAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link PendaftarAction} dan dapat mengakses state
