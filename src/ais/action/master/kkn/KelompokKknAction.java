...
/**
 * Controller/action ZK untuk kelompok kkn. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * searchkkn}, {@code AmbilDataDosenBanbox searchdosen1}, {@code AmbilDataMahasiswaBanbox searchmahasiswa};
 * inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code doAfterCompose()}, {@code init()}, {@code
 * initCriteria()}); pembacaan/pencarian ({@code tampilkanInfoDosen()}, {@code tampilkanInfoDosen()}, {@code
 * onSearchDefault()}, {@code getDspaceKkn()}, {@code getDspace()}); mutasi data ({@code onSave()});
 * pelaporan/ekspor ({@code cetakDataCustomButton()}); operasi domain lain ({@code displayRow()}, {@code
 * displayRow()}, {@code onAdd()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang
 * disebut di atas.</p>
...
public class KelompokKknAction extends GenericAutowireComposer implements DataSearchDefault, DataCriteria {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link KelompokKknAction}. Kelas ini menerjemahkan satu item data
	 * menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link KelompokKknAction} dan dapat mengakses state
