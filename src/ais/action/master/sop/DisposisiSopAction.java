...
/**
 * Controller/action ZK untuk disposisi sop. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * onSearchDefault()}, {@code reloadDataMenu()}); validasi/perhitungan ({@code check()}); mutasi data ({@code
 * onSave()}); operasi domain lain ({@code closeNativeSessionSafely()}, {@code onAddExternal()}, {@code onAdd()},
 * {@code createCriterionSop()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang
 * disebut di atas.</p>
