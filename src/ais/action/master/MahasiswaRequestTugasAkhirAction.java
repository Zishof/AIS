...
/**
 * Controller/action ZK untuk mahasiswa request tugas akhir. Tipe ini merupakan titik masuk UI yang
 ...
 * paging}, {@code MyGrid grid}, {@code Textbox searchnim}, {@code Textbox searchjudul}, {@code Textbox
 * searchnama}, {@code Combobox searchfakultas}, {@code Combobox searchjurusan}; inisialisasi/lifecycle ({@code
 * initReferensi()}, {@code doBeforeCompose()}, {@code doAfterCompose()}, {@code init()}, {@code
 * initCriteria()}); pembacaan/pencarian ({@code onTampilSeminar()}, {@code onTampilSeminar1()}, {@code
 * onDownloadLampiran()}, {@code tampilkanInfoMahasiswa()}, {@code tampilkanInfoJudul()}, {@code
 * tampilkanInfoDosenSimple()}); validasi/perhitungan ({@code checkSyaratPembimbingDanPenguji()}, {@code
 * checkNamaMahasiswaRequestTugasAkhir()}, {@code checkNamaMahasiswaRequestTugasAkhir()}); mutasi data ({@code
 * onSave()}, {@code setPersetujuan()}); pelaporan/ekspor ({@code tombolCetakSK()}, {@code
 * tombolCetakPengantar()}, {@code cetakData()}); operasi domain lain ({@code onJadwal()}, {@code
 * onFormatNilai()}, {@code onTahapan()}, {@code onKomponenPenilaian()}, {@code addReferensi()}, {@code
 * onAddExternal()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di
 * atas.</p>
...
//						System.out.println("parameters -> " + parameters);
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link MahasiswaRequestTugasAkhirAction}. Kelas ini menerjemahkan satu
	 * item data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link MahasiswaRequestTugasAkhirAction} dan dapat
	 ...
	 *
	 * @see MahasiswaRequestTugasAkhirAction
	 */
