...
public class RekapitulasiAudioHelper {
			...
			public void loadData(Object value) {
				try {
					reload(tbmuser, center, mulai.getValue(), sampai.getValue(), cari.getValue().trim(), true, true,
							perkuliahan);
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/helper/RekapitulasiAudioHelper.java:124");
					// TODO: handle exception
				}
			}
		}, 4, "Audio").setParent(hbox);
	...
	private static void reload(final List<Long> pertemuans, final Paging paging, final MyGrid grid,
		...
		if (awal) {
			...
			try {
				...
				} catch (Exception ae) { ais.common.ErrorAuditUtil.record(ae, "auto-audit(empty-catch) src/ais/action/master/helper/RekapitulasiAudioHelper.java:393");
					// TODO: handle exception
				}
			}
		}
	...
	public static class DetailPertemuanRenderer extends ais.ui.util.MyRowRenderer {
		...
		/**
		 * Merender satu baris untuk satu {@link AudioPertemuan} (lihat javadoc kelas
		 ...
		 * @param data instance {@link AudioPertemuan} untuk baris ini
		...
		public void render(final Row arg0, Object data) throws Exception {
			...
			String n = audioPertemuan == null ? ""
					: (audioPertemuan.getLink() == null || audioPertemuan.getLink().isEmpty() ? audioPertemuan.getNama()
			...
			A a;
			vbox.appendChild(a = new A("Audio pertemuan ke: " + pertemuan.getPertemuanKe() + ", " + pertemuan.info()
					+ ", "
					...
					+ ", " + pertemuan.getTopik() + (isi.isEmpty() ? "" : ", " + isi)));
