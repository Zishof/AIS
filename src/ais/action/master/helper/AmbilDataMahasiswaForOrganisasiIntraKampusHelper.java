...
public class AmbilDataMahasiswaForOrganisasiIntraKampusHelper {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link AmbilDataMahasiswaForOrganisasiIntraKampusHelper}. Kelas ini
	 ...
	 * AmbilDataMahasiswaForOrganisasiIntraKampusHelper} dan dapat mengakses state kelas induk. Jangan menyimpan
	 ...
	 *
	 * @see AmbilDataMahasiswaForOrganisasiIntraKampusHelper
	 */
	...
	public void save() throws InterruptedException {
		...
		for (Row row : list) {
				...
				if (checkbox.isChecked() && !checkbox.isDisabled()) {
					...
					if (Common.checkApakahMemenuhiSyaratOrganisasiKemahasiswaan(mahasiswa, organisasiIntraKampus)) {
						OrganisasiIntraKampusPunyaMahasiswa organisasiIntraKampusPunyaMahasiswa = (OrganisasiIntraKampusPunyaMahasiswa) session
								...
								.add(Restrictions.eq("mahasiswa", mahasiswa))
								.add(Restrictions.eq("organisasiIntraKampus", organisasiIntraKampus)).setMaxResults(1)
								.uniqueResult();
			...
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/helper/AmbilDataMahasiswaForOrganisasiIntraKampusHelper.java:166");
		...
		checkbox.addEventListener(Events.ON_CHECK, new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				for (Row row : rows) {
					...
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/helper/AmbilDataMahasiswaForOrganisasiIntraKampusHelper.java:341");
