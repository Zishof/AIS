...
/**
 * Tipe khusus untuk download tagihan mahasiswa. Kelas ini memberi nama dan batas tanggung jawab
 ...
 * @see MyWindow
 */
public class DownloadTagihanMahasiswa extends MyWindow {
	...
	public DownloadTagihanMahasiswa() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException(
					"Membuka jendela Unduh Tagihan Mahasiswa",
					e,
					new String[] {
							"Muat ulang (refresh) halaman ini, lalu buka kembali jendela Unduh Tagihan Mahasiswa.",
							"Periksa apakah data Jenis Kegiatan Pendaftaran Mahasiswa Lama sudah dikonfigurasi dengan benar.",
							"Jika jendela tetap gagal terbuka setelah beberapa kali percobaan, hubungi Administrator." });
		}
	}

	public DownloadTagihanMahasiswa(String title, String border,
			boolean closable) {
			...
			public void onEvent(Event event) throws Exception {
				...
				try {
					Filedownload.save(new FileInputStream(file),
							...
							"TAGIHAN_MAHASISWA_TAHUN_ANGKATAN_"
									+ searchangkatan.getValue() + ".xlsx");
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/helper/virtualaccount/DownloadTagihanMahasiswa.java:169");
	...
	private void initSpreadsheet() throws Exception {
		...
		if (searchangkatan.getValue() == null) {
			MyMessageboxConfig.show("Tahun Angkatan harus dipilih", "Peringatan",
					MyMessageboxConfig.OK, MyMessageboxConfig.INFORMATION);
			return;
		}
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				List<Mahasiswa> mahasiswas = session
						...
						.add(searchangkatan.getValue() == null ? Restrictions
								...
								"tahunangkatan", searchangkatan.getValue()))
				...
				for (Mahasiswa mahasiswa : mahasiswas) {
					label.setValue("Sedang memproses data "
							...
							+ Common.numberFormat.get().format(rowIndex * 100.0
									/ size) + " %)");
					createData(session, mahasiswa, sheet, rowIndex,
							lockedNumericStyle);
					rowIndex++;
				}
				...
				try {
					...
					Common.tampilErrorJikaAdmin(e);
					PesanFormalHelper.tampilkanGagalException(
							"Menyimpan berkas Excel hasil unduhan Tagihan Mahasiswa",
							e,
							new String[] {
			...
			MahasiswaVirtualAccountHelper.closeHibernateContextQuietly();
				...
				label.setValue("Error: " + PesanFormalHelper.pesanGagalException(
						"pengunduhan Tagihan Virtual Account Mahasiswa", null, e,
						new String[] {
					...
					if (session != null && session.isOpen()) {
						try { session.disconnect(); } catch (Exception eSf) { ais.common.ErrorAuditUtil.record(eSf, "auto-audit(empty-catch) src/ais/action/master/helper/virtualaccount/DownloadTagihanMahasiswa.java:291");}
						try { session.close(); } catch (Exception eSf) { ais.common.ErrorAuditUtil.record(eSf, "auto-audit(empty-catch) src/ais/action/master/helper/virtualaccount/DownloadTagihanMahasiswa.java:292");}
					}
				}
			}
		}).start();
	...
	private void createData(Session session, Mahasiswa mahasiswa,
			XSSFSheet sheet, int rowIndex, XSSFCellStyle lockedNumericStyle) {

		@SuppressWarnings("unchecked")
		Collection<DetailBiaya> detailBiayas = pembayaranUtil
				.getDetailBiayaMahasiswaBerdasarkanJenisKegiatan(mahasiswa,
		...
		cell.setCellValue(mahasiswa == null || mahasiswa.getId() == null ? -1L : mahasiswa.getId());
		...
		cell.setCellStyle(lockedNumericStyle);
		cell.setCellValue(mahasiswa.getJurusan() == null ? "" : mahasiswa
				.getJurusan().getNama());
