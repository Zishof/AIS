...
/**
 * Tipe khusus untuk download no registrasi calon mahasiswa. Kelas ini memberi nama dan batas
 ...
 * JenisKegiatan jenisKegiatan}, {@code Center center}, {@code Intbox searchangkatan}, {@code File file};
 * inisialisasi/lifecycle ({@code init()}, {@code initSpreadsheet()}); operasi domain lain ({@code
 * createData()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di atas.</p>
 ...
 * @see MyWindow
 */
public class DownloadNoRegistrasiCalonMahasiswa extends MyWindow {
	...
	public DownloadNoRegistrasiCalonMahasiswa() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException(
					"Membuka jendela Unduh Nomor Registrasi Calon Mahasiswa",
					e,
					new String[] {
							"Muat ulang (refresh) halaman ini, lalu buka kembali jendela Unduh Nomor Registrasi Calon Mahasiswa.",
							"Periksa apakah data Jenis Kegiatan Pendaftaran Calon Mahasiswa sudah dikonfigurasi dengan benar.",
							"Jika jendela tetap gagal terbuka setelah beberapa kali percobaan, hubungi Administrator." });
		}
	}

	public DownloadNoRegistrasiCalonMahasiswa(String title, String border,
			boolean closable) {
			...
			public void onEvent(Event event) throws Exception {
				...
				try {
					Filedownload.save(new FileInputStream(file),
							...
							"TAGIHAN_NO_REG_CALON_MAHASISWA_TAHUN_"
									+ searchangkatan.getValue() + ".xlsx");
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/helper/virtualaccount/DownloadNoRegistrasiCalonMahasiswa.java:169");
	...
	private void initSpreadsheet() throws Exception {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				rowhead.createCell(1).setCellValue("NO. REG. CALON MAHASISWA");
				...
				List<BiodataCalonMahasiswa> biodataCalonMahasiswas = session
						...
						.createAlias("gelombangPendaftaran",
								"gelombangPendaftaran")
						.add(Restrictions
								.eq("gelombangPendaftaran.bisaDipilihPendaftarOnline",
										true))
				...
				try {
					...
					Common.tampilErrorJikaAdmin(e);
					PesanFormalHelper.tampilkanGagalException(
							"Menyimpan berkas Excel hasil unduhan Nomor Registrasi Calon Mahasiswa",
							e,
							new String[] {
			...
			MahasiswaVirtualAccountHelper.closeHibernateContextQuietly();
				...
				// FIX "gagal diam-diam"/hang: sebelumnya blok Thread.run() ini adalah
				// try{...}finally{...} TANPA catch, sehingga exception (mis. gagal query/Excel)
				// menembus keluar Runnable.run() tak tertangani, thread mati, dan label.setValue("")
				// TIDAK PERNAH dijalankan -> popup progres menggantung selamanya tanpa umpan balik.
				Common.tampilErrorJikaAdmin(e);
				label.setValue("Error: " + PesanFormalHelper.pesanGagalException(
						"pengunduhan Nomor Registrasi Virtual Account Calon Mahasiswa", null, e,
						new String[] {
					...
					if (session != null && session.isOpen()) {
						try { session.disconnect(); } catch (Exception eSf) { ais.common.ErrorAuditUtil.record(eSf, "auto-audit(empty-catch) src/ais/action/master/helper/virtualaccount/DownloadNoRegistrasiCalonMahasiswa.java:295");}
						try { session.close(); } catch (Exception eSf) { ais.common.ErrorAuditUtil.record(eSf, "auto-audit(empty-catch) src/ais/action/master/helper/virtualaccount/DownloadNoRegistrasiCalonMahasiswa.java:296");}
					}
				}
			}
		}).start();
	...
	private void createData(Session session,
		...
		Jurusan myjurusan1 = biodataCalonMahasiswa.getProdi1() == null ? biodataCalonMahasiswa
				.getProdi2() : biodataCalonMahasiswa.getProdi1();
		if (myjurusan1 == null) {
		...
		cell.setCellStyle(lockedNumericStyle);
		cell.setCellValue(biodataCalonMahasiswa.getNoRegistrasi());
