...
/**
 * Tipe khusus untuk download no ujian calon mahasiswa bank btn. Kelas ini memberi nama dan batas
 ...
 * @see MyWindow
 */
public class DownloadNoUjianCalonMahasiswaBankBtn extends MyWindow {
	...
	public DownloadNoUjianCalonMahasiswaBankBtn() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException(
					"Membuka jendela Unduh Nomor Ujian Calon Mahasiswa (Bank BTN)",
					e,
					new String[] {
							"Muat ulang (refresh) halaman ini, lalu buka kembali jendela Unduh Nomor Ujian Calon Mahasiswa Bank BTN.",
							"Periksa apakah data Jenis Kegiatan Pendaftaran Ulang Mahasiswa Baru dan konfigurasi integrasi Bank BTN sudah benar.",
							"Jika jendela tetap gagal terbuka setelah beberapa kali percobaan, hubungi Administrator." });
		}
	}

	public DownloadNoUjianCalonMahasiswaBankBtn(String title, String border, boolean closable) {
				...
				try {
					Filedownload.save(new FileInputStream(file),
							...
							"PEMBAYARAN_MAHASISWA_BARU_" + Common.dateFormat32.get().format(ais.ui.util.WaktuUtil.getDate())
									+ ".xlsx");
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/helper/virtualaccount/DownloadNoUjianCalonMahasiswaBankBtn.java:202");
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					Common.tampilErrorJikaAdmin(e);
					PesanFormalHelper.tampilkanGagalException(
							"Menyimpan berkas Excel hasil unduhan Nomor Ujian Calon Mahasiswa (Bank BTN)",
							e,
							new String[] {
			...
			MahasiswaVirtualAccountHelper.closeHibernateContextQuietly();
				...
				label.setValue("Error: " + PesanFormalHelper.pesanGagalException(
						"pengunduhan Nomor Ujian Virtual Account Calon Mahasiswa (Bank BTN)", null, e,
						new String[] {
					...
					if (session != null && session.isOpen()) {
						try { session.disconnect(); } catch (Exception eSf) { ais.common.ErrorAuditUtil.record(eSf, "auto-audit(empty-catch) src/ais/action/master/helper/virtualaccount/DownloadNoUjianCalonMahasiswaBankBtn.java:330");}
						try { session.close(); } catch (Exception eSf) { ais.common.ErrorAuditUtil.record(eSf, "auto-audit(empty-catch) src/ais/action/master/helper/virtualaccount/DownloadNoUjianCalonMahasiswaBankBtn.java:331");}
					}
				}
			}
		}).start();
	...
	private boolean createData(Session session, BiodataCalonMahasiswa biodataCalonMahasiswa, XSSFSheet sheet,
		...
		if (myjadwalPembayaran != null) {
			...
			if (!pengaturanPembayaranBulanans.isEmpty()) {
				...
				virtualAccountBankBtn.setBulanan(bulanan);
				...
				virtualAccountBankBtn.setSemester(1);
				...
				if (countPengaturanBulanan == 0) {
					...
					VirtualAccountBank virtualAccountBankBtn = (VirtualAccountBank) session
							...
							.add(Restrictions.eq("nama", kodeUnik)).uniqueResult();
					if (virtualAccountBankBtn == null) {
						virtualAccountBankBtn = new VirtualAccountBank(
					...
					virtualAccountBankBtn.setNama(kodeUnik);
					virtualAccountBankBtn.setJenisKegiatan(jenisKegiatan);
					virtualAccountBankBtn.setKeterangan("");
					virtualAccountBankBtn.setTotal(total);
	...
	public static VirtualAccountBank downloadData(BiodataCalonMahasiswa biodataCalonMahasiswa,
			JadwalPembayaran myjadwalPembayaran, Collection detailBiayas, Grid gridCicilan, Integer smt)
			throws Exception {
		...
		for (Row row : mycicilanrows) {
			...
			JadwalPembayaran jdw = myjadwalPembayaran != null && myjadwalPembayaran.getKhususUntukNim() != null
					&& myjadwalPembayaran.getKhususUntukNim()
							.contains("," + biodataCalonMahasiswa.getNoRegistrasi() + ",") ? myjadwalPembayaran : null;
			if (jumlahCicilan.getValue() != null && jumlahCicilan.getValue().intValue() != 0) {
				...
				if (cicilanPembayaranSebelumnya.getId() == null) {
					try {
						...
						e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/helper/virtualaccount/DownloadNoUjianCalonMahasiswaBankBtn.java:616");
					}
				}
			}
		}
		...
		if (tagihan_expired_akhir_hari) {
			try {
				...
				e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/helper/virtualaccount/DownloadNoUjianCalonMahasiswaBankBtn.java:634");
			}
		} else {
			...
			if (!tagihan_expired_jam.isEmpty()) {
				if (!tagihan_expired_jam.isEmpty() && !tagihan_expired_jam.equalsIgnoreCase("0")) {
					try {
						...
						e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/helper/virtualaccount/DownloadNoUjianCalonMahasiswaBankBtn.java:646");
					}
				}
			} else {
				...
				if (!tagihan_expired_day.isEmpty() && !tagihan_expired_day.equalsIgnoreCase("0")) {
					try {
						...
						e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/helper/virtualaccount/DownloadNoUjianCalonMahasiswaBankBtn.java:659");
					}
				}
			}
		}
		...
		if (virtualAccountBankBtn == null) {
			...
			try {
				...
				e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/helper/virtualaccount/DownloadNoUjianCalonMahasiswaBankBtn.java:683");
			}
...
//			jsonObject.put("description", Common.maxPanjangAkhir(biodataCalonMahasiswa.getAlamat(), 60));
			...
			if (response.getString("rsp").equals("000")) {
				...
				virtualAccountBankBtn.setBiodataCalonMahasiswa(biodataCalonMahasiswa);
				virtualAccountBankBtn.setJadwalPembayaran(myjadwalPembayaran);
				virtualAccountBankBtn.setSemester(smt);
		...
		return virtualAccountBankBtn;
			...
			// finally menjamin penutupan walau terjadi exception (idempoten via isOpen()).
			if (session != null && session.isOpen()) {
				try { session.disconnect(); } catch (Exception eSf) { ais.common.ErrorAuditUtil.record(eSf, "auto-audit(empty-catch) src/ais/action/master/helper/virtualaccount/DownloadNoUjianCalonMahasiswaBankBtn.java:775");}
				try { session.close(); } catch (Exception eSf) { ais.common.ErrorAuditUtil.record(eSf, "auto-audit(empty-catch) src/ais/action/master/helper/virtualaccount/DownloadNoUjianCalonMahasiswaBankBtn.java:776");}
			}
		}
	}
}
