...
public class AmbilDataMahasiswaSeleksiPklHelper {
	...
	/** Kegiatan PKL tujuan pendaftaran, ditetapkan di {@link #display(Pkl, DataLoader, MyWindow)}. */
	private Pkl pkl;
	...
	/** Combobox filter fakultas pada form pencarian, diinisialisasi ulang tiap {@link #display(Pkl, DataLoader, MyWindow)}. */
	...
	/** Konstruktor tanpa argumen; state (pkl, grid, filter combobox) baru ditetapkan saat {@link #display(Pkl, DataLoader, MyWindow)} dipanggil. */
	...
	class MahasiswaRenderer extends ais.ui.util.MyRowRenderer {
		...
		/** DAO {@link MahasiswaDapatPklDao} dipakai untuk memperoleh {@link #session} Hibernate aktif. */
		 ...
		 * (tercentang bila mahasiswa sudah terdaftar dan diterima, {@code terima=1}, pada {@link #pkl}
		...
		public void render(Row arg0, Object arg1) throws Exception {
			...
			Integer jml = ((Number) session.createCriteria(MahasiswaDaftarPkl.class)
					.setProjection(Projections.rowCount()).add(Restrictions.eq("mahasiswa", mahasiswa))
					.add(Restrictions.eq("pkl", pkl)).add(Restrictions.eq("terima", 1)).uniqueResult()).intValue();
		...
		for (Row row : list) {
			...
			try {
				...
				if (checkbox.isChecked()) {
					...
					MahasiswaDaftarPkl mahasiswaDaftarPkl = (MahasiswaDaftarPkl) (session
					...
					if (mahasiswaDaftarPkl == null) {
						...
						mahasiswaDaftarPkl.setKeterangan("");
						mahasiswaDaftarPkl.setNama("");
						mahasiswaDaftarPkl.setMahasiswa(mahasiswa);
						session.save(mahasiswaDaftarPkl);
					}
				}
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/helper/AmbilDataMahasiswaSeleksiPklHelper.java:132");
		...
		checkbox.addEventListener(Events.ON_CHECK, new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				for (Row row : rows) {
					...
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/helper/AmbilDataMahasiswaSeleksiPklHelper.java:292");
