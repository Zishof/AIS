...
public class KrsKurikulumHelper implements DataLoader {
	...
	/** Panel informasi jam bentrok (langsung, paralel, antar-paralel), diisi ulang setiap {@link #loadData(Object)}. */
	private MyDiv jamBentrok = new MyDiv();
	/** Mahasiswa pemilik KRS yang sedang ditampilkan/dikelola. */
	private Mahasiswa mahasiswa;
	/** Nomor semester KRS yang ditampilkan; {@code 0}/negatif menyembunyikan sebagian panel (toolbar, jam bentrok, komentar). */
	...
	/** Tombol "Ambil Perkuliahan Konversi"; visibilitas/enable-nya bergantung status jendela KRS dan tahapan berjalan. */
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link KrsKurikulumHelper}. Kelas ini menerjemahkan satu item data
	 * menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link KrsKurikulumHelper} dan dapat mengakses state
	 * kelas induk. Jangan menyimpan atau membagikannya lintas desktop/session.</p>
	 * <p>Kontrak yang tampak dari deklarasi ini meliputi state utama: {@code boolean refresh}; operasi lokal:
	...
	class DetailMahasiswaRenderer extends ais.ui.util.MyRowRenderer {
		 ...
		 * tombol hapus (hanya tampil bila {@code totalNilai} masih di bawah 0.01).
		 *
		...
		public void render(final Row row, Object data) throws Exception {row.setValign("top");
			...
			ais.action.master.helper.PerkuliahanUIHelper.displayHariJamRuanganPerkuliahan(row, perkuliahan);
			...
			if ((detailperkuliahan.getTotalNilai() < 0.01)) {
	...
	public void display(final Boolean editable, final Mahasiswa mahasiswa, final String tahunAjaran,
		...
		buttonPerkuliahan = new MyButtonConfig("Ambil Perkuliahan Konversi", "/img/svg/edit-box-line.svg");
		...
		buttonPerkuliahan.addEventListener("onClick", new EventListener() {
			...
			private AmbilDataKurikulumPerkuliahanHelper ambilDataKurikulumPerkuliahanHelper = new AmbilDataKurikulumPerkuliahanHelper(
					semesterPendek);
			...
			public void onEvent(Event event) throws Exception {
					...
					if (!Common.checkStatusPembayaranMahasiswaSebelumnya(semester, tahapan, mahasiswa)) {
						...
						try {
							harusLunas = Double.parseDouble(Common.getKonfigurasi(
									...
									.getNilai().trim());
						} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/helper/KrsKurikulumHelper.java:465");
				...
				ambilDataKurikulumPerkuliahanHelper.display(mahasiswa, tahunAjaran, semester, KrsKurikulumHelper.this);
			}
