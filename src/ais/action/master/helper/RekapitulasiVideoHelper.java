			...
			public void loadData(Object value) {
				try {
					reload(tbmuser, center, mulai.getValue(), sampai.getValue(), cari.getValue().trim(), true, true,
							perkuliahan);
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/helper/RekapitulasiVideoHelper.java:124");
					// TODO: handle exception
				}
			}
		}, 5, "Video").setParent(hbox);
	...
	private static void reload(final List<Long> pertemuans, final Paging paging, final MyGrid grid,
		...
		if (awal) {
			...
			try {
				...
				} catch (Exception ae) { ais.common.ErrorAuditUtil.record(ae, "auto-audit(empty-catch) src/ais/action/master/helper/RekapitulasiVideoHelper.java:393");
					// TODO: handle exception
				}
			}
		}

		List<VideoPertemuan> tugasPertemuans = session.createCriteria(VideoPertemuan.class)
	...
	public static class DetailPertemuanRenderer extends ais.ui.util.MyRowRenderer {
		...
		public void render(final Row arg0, Object data) throws Exception {
			...
			String n = videoPertemuan == null ? ""
					: (videoPertemuan.getLink() == null || videoPertemuan.getLink().isEmpty() ? videoPertemuan.getNama()
			...
			A a;
			vbox.appendChild(a = new A("Video pertemuan ke: " + pertemuan.getPertemuanKe() + ", " + pertemuan.info()
					+ ", "
