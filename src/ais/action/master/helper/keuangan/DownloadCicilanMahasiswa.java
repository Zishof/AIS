...
/**
 * Tipe khusus untuk download cicilan mahasiswa. Kelas ini memberi nama dan batas tanggung jawab
 ...
 * searchfakultas}, {@code Combobox searchjurusan}, {@code Intbox searchangkatan}, {@code Intbox
 * searchangkatansd}, {@code Combobox searchprogram}, {@code Combobox searchsemester}, {@code Combobox
 * searchJenisSemester}; inisialisasi/lifecycle ({@code init()}, {@code initSpreadsheet()}); operasi domain lain
 * ({@code createData()}); konfigurasi constructor: {@code comboitem}. Bagian lain dari kontrak tetap mengikuti
 ...
 * @see MyWindow
 */
public class DownloadCicilanMahasiswa extends MyWindow {
	...
	private Combobox searchJenisSemester = new Combobox();
	...
	public DownloadCicilanMahasiswa() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException(
					"Membuka jendela Unduh Cicilan Mahasiswa",
					e,
					new String[] {
							"Muat ulang (refresh) halaman ini, lalu buka kembali jendela Unduh Cicilan Mahasiswa.",
							"Periksa apakah data Fakultas, Jurusan, Program Studi, dan Semester pada master data sudah terisi dengan benar.",
							"Jika jendela tetap gagal terbuka setelah beberapa kali percobaan, hubungi Administrator." });
		}
	}

	public DownloadCicilanMahasiswa(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		print.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/helper/keuangan/DownloadCicilanMahasiswa.java:359");
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				List<CicilanPembayaran> cicilanPembayarans = session.createCriteria(CicilanPembayaran.class)
						...
						.add(searchJenisSemester.getSelectedItem() == null
								|| searchJenisSemester.getSelectedItem().getValue() == null
										? Restrictions.sqlRestriction("1=1")
										: Restrictions.sqlRestriction("semster % 2 = " + (searchJenisSemester
						...
						.add(nimMahasiswa.getValue().trim().isEmpty() ? Restrictions.sqlRestriction("true")
								: Restrictions.or(
										Restrictions.ilike("mahasiswa.nim", nimMahasiswa.getValue().trim(),
												MatchMode.ANYWHERE),
										Restrictions.ilike("mahasiswa.nama", nimMahasiswa.getValue().trim(),
												MatchMode.ANYWHERE)))
						...
						.add(Restrictions.between("mahasiswa.tahunangkatan", angkatanMulai, angkatanSampai))

						.addOrder(Order.asc("mahasiswa.nim")).addOrder(Order.asc("kegiatan.semster"))
				...
				try {
					...
					Common.tampilErrorJikaAdmin(e);
					PesanFormalHelper.tampilkanGagalException(
							"Menyimpan berkas Excel hasil unduhan Cicilan Mahasiswa",
							e,
							new String[] {
				...
				label.setValue("");
					...
					label.setValue("Error: " + PesanFormalHelper.pesanGagalException(
							"pengunduhan (Excel) data Cicilan Mahasiswa", null, e,
							new String[] {
