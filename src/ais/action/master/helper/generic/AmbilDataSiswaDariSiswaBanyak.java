...
import ais.database.model.sekolah.*;
...
/**
 * Tipe khusus untuk ambil data siswa dari siswa banyak. Kelas ini memberi nama dan batas tanggung
 ...
 * eventListener}, {@code List siswas}, {@code List siswasHanyaDitampilkan}, {@code Set ids}, {@code MyTextbox
 * kodeSiswaan}, {@code MyTextbox nama}; pembacaan/pencarian ({@code onSearchDefault()}, {@code
 ...
 * @see MyWindow
 */
public class AmbilDataSiswaDariSiswaBanyak extends MyWindow {
	...
	public AmbilDataSiswaDariSiswaBanyak(List<Siswa> siswas) {
		super();
		this.siswas = siswas;
		display();
	...
	public AmbilDataSiswaDariSiswaBanyak(List<Siswa> siswas, List<Siswa> siswasHanyaDitampilkan) {
		super();
		this.siswas = siswas;
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link AmbilDataSiswaDariSiswaBanyak}. Kelas ini menerjemahkan satu item
	 * data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link AmbilDataSiswaDariSiswaBanyak} dan dapat
	 ...
	 *
	 * @see AmbilDataSiswaDariSiswaBanyak
	 */
	...
	public void display() {
		...
		cancel.addEventListener("onClick", new EventListener() {
			@Override
			public void onEvent(Event event) throws Exception {
				AmbilDataSiswaDariSiswaBanyak.this.detach();
			}
		});
			...
			public void onEvent(Event event) throws Exception {
				if (eventListener != null && grid.getRows() != null && grid.getRows().getChildren() != null) {
					...
					for (Row row : rows) {
						...
						} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/helper/generic/AmbilDataSiswaDariSiswaBanyak.java:268");
	...
	public void onSearchDefault(Event event) {
		...
		if (siswas != null) {
			for (Siswa siswa : siswas) {
				valuesTidakPerluTampil.add(siswa.getId());
			}
		}
		...
		List<Siswa> siswa = ConstantValues.simpleList(
				session.createCriteria(Siswa.class).add(Restrictions.isNotNull("namaSiswa")).add(Restrictions.ne("namaSiswa","")).add(Restrictions.isNotNull("sekolah"))
						.setProjection(Projections.property("id")).addOrder(Order.desc("tahunMasuk"))
						.addOrder(Order.asc("nomorInduk"))
						.add(ids.size() == 0 ? Restrictions.sqlRestriction("1!=1") : Restrictions.in("id", ids)),
				Siswa.class, false);

		List<Siswa> mySiswa = ConstantValues.simpleList(session.createCriteria(Siswa.class).add(Restrictions.isNotNull("namaSiswa")).add(Restrictions.ne("namaSiswa",""))
				.add(Restrictions.isNotNull("sekolah")).setProjection(Projections.property("id"))
				...
				.add(siswasHanyaDitampilkan == null ? Restrictions.sqlRestriction("1=1")
				...
				.add(kodeSiswaan.getValue().trim().isEmpty() ? Restrictions.sqlRestriction("1=1")
						: Restrictions.or(
								Restrictions.ilike("nomorIndukNasional", kodeSiswaan.getValue().trim(),
										MatchMode.ANYWHERE),
								Restrictions.ilike("nomorInduk", kodeSiswaan.getValue().trim(), MatchMode.ANYWHERE)))

				.setMaxResults(Common.MAX_RESULT_500), Siswa.class, false);
