...
public class PenjadwalanKknHelper {
	...
	/** Kelompok KKN yang jadwal pertemuannya dikelola, ditetapkan di {@link #display(KelompokKkn, DataLoader)}. */
	private KelompokKkn kelompokKkn;
	/** Grid daftar pertemuan yang sedang ditampilkan, diisi ulang oleh {@link #onSearchDefault(Event)}. */
	private MyGrid grid;
	/** Callback penyegar tampilan pemanggil, dipanggil setelah simpan/hapus/tambah pertemuan. */
	...
	class PertemuanRenderer extends ais.ui.util.MyRowRenderer {
		...
		/**
		 * Merender satu baris pertemuan ({@code arg1}, harus {@link Pertemuan}): lihat dokumentasi
		 * kelas {@link PertemuanRenderer} untuk rincian kontrol yang dibangun.
		 */
		@Override
		public void render(Row arg0, Object arg1) throws Exception {
			...
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/helper/PenjadwalanKknHelper.java:107");
			...
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/helper/PenjadwalanKknHelper.java:119");
									...
									if (i == MyMessageboxConfig.OK) {
										try {
											...
											PesanFormalHelper.tampilkanGagalException(
													"menghapus data jadwal pertemuan KKN",
													e, new String[] {
	...
	public boolean save() throws InterruptedException {
		...
		kelompokKkn.reInitPertemuan(session);
	...
	public void display(final KelompokKkn kelompokKkn, final DataLoader dataLoader) {
		...
		window.setTitle(kelompokKkn.infoSimple());
		...
		MyToolbarbuttonConfig button = new MyToolbarbuttonConfig("Tambah Kegiatan KKN", "/img/new.gif");
		button.addEventListener("onClick", new EventListener() {
			@Override
			public void onEvent(Event event) throws Exception {
				...
				pertemuan.setTopik("Topik KKN ke " + (grid.getRows().getChildren().size()));
		...
		cari.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				Common.createDefaultTimer(new EventListener() {
					...
					@Override
					public void onEvent(Event arg0) throws Exception {
						kelompokKkn.reInitPertemuan(HibernateUtil.currentSession());
						// Nama event TIDAK boleh null (Event.<init> melempar NPE). Pakai nama netral.
		...
		// PenjadwalanHelper.tampilTombol(toolbar, null, kelompokKkn, null,
		// null, null);
		PenjadwalanHelper.tampilTombolAmbil(toolbar, null, kelompokKkn, null, null, null, null, null, null);

		PenjadwalanHelper.tampilTombolAturUlangWaktu(toolbar, null, kelompokKkn, null, null, null, null, null, null,
				new EventListener() {
		...
		PenjadwalanHelper.tampilTombolDownload(toolbar, contents, null, kelompokKkn, null, null, null, null, null,
				null);

		PenjadwalanHelper.tampilTombolHapus(toolbar, null, kelompokKkn, null, null, null, null, null, null,
				new EventListener() {
