...
/**
 * Tipe khusus untuk rencana item gaji pegawai tree model. Kelas ini memberi nama dan batas
 ...
 * RencanaGajiPunyaPegawai rencanaGajiPunyaPegawai}, {@code ItemGajiPegawaiTreeModel itemGajiPegawaiTreeModel},
 * {@code MyJSONObject jsonObject}; pembacaan/pencarian ({@code getChildren()}, {@code getChildren()}, {@code
 * getChild()}, {@code getChildCount()}, {@code getParentCount()}, {@code getParentSet()}); validasi/perhitungan
 * ({@code checkExistingItemGaji()}); mutasi data ({@code reset()}); penghapusan/pembatalan ({@code
 * deleteChilds()}); operasi domain lain ({@code populateData()}, {@code populateData()}, {@code
 * generateAllChildren()}, {@code isLeaf()}, {@code copyByFormat()}, {@code copyByItemGajiPegawai()});
 * konfigurasi constructor: {@code itemGajiPegawaiTreeModel}. Bagian lain dari kontrak tetap mengikuti kelas
