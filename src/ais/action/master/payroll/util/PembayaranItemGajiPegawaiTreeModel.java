...
/**
 * Tipe khusus untuk pembayaran item gaji pegawai tree model. Kelas ini memberi nama dan batas
 ...
 * PembayaranGajiPunyaPegawai pembayaranGajiPunyaPegawai}, {@code ItemGajiPegawaiTreeModel
 * itemGajiPegawaiTreeModel}, {@code MyJSONObject jsonObject}; pembacaan/pencarian ({@code getChildren()}, {@code
 * getChildren()}, {@code getChildren()}, {@code getChild()}, {@code getChildCount()}, {@code getChildCount()});
 * validasi/perhitungan ({@code checkExistingItemGaji()}); mutasi data ({@code reset()}, {@code setLunas()});
 * penghapusan/pembatalan ({@code deleteChilds()}, {@code deleteChildsRecursive()}); operasi domain lain ({@code
 * populateData()}, {@code populateData()}, {@code populateDataRecursive()}, {@code generateAllChildren()},
 * {@code generateAllChildrenRecursive()}, {@code isLeaf()}); konfigurasi constructor: {@code
 * itemGajiPegawaiTreeModel}. Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di
 * atas.</p>
