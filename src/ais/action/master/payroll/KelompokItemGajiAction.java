...
/**
 * Controller/action ZK untuk kelompok item gaji. Tipe ini merupakan titik masuk UI yang
 ...
 * paging}, {@code MyGrid grid}, {@code Textbox searchnama}, {@code AmbilDataAkunBanbox searchakun}, {@code
 * Textbox nama}, {@code Combobox induk}, {@code Textbox keterangan}; inisialisasi/lifecycle ({@code
 * doBeforeCompose()}, {@code doAfterCompose()}, {@code init()}, {@code initCriteria()}, {@code init()});
 * pembacaan/pencarian ({@code onSearchDefault()}); validasi/perhitungan ({@code checkNamaKelompokItemGaji()},
 * {@code checkKodeKelompokItemGaji()}); mutasi data ({@code onSave()}); operasi domain lain ({@code onAdd()}).
...
public class KelompokItemGajiAction extends GenericAutowireComposer
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link KelompokItemGajiAction}. Kelas ini menerjemahkan satu item data
	 * menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link KelompokItemGajiAction} dan dapat mengakses
