...
/**
 * Controller/action ZK untuk cara pembayaran gaji. Tipe ini merupakan titik masuk UI yang
 ...
 * searchaktif}, {@code AmbilDataSatuanKerjaBanbox searchparent}, {@code Textbox nama}; inisialisasi/lifecycle
 * ({@code doBeforeCompose()}, {@code doAfterCompose()}, {@code init()}, {@code initCriteria()}, {@code init()});
 * pembacaan/pencarian ({@code onSearchDefault()}); validasi/perhitungan ({@code checkKode()}, {@code
 * checkNama()}); mutasi data ({@code onSave()}); operasi domain lain ({@code onAdd()}). Bagian lain dari kontrak
...
public class CaraPembayaranGajiAction extends GenericAutowireComposer implements DataInitDefault {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link CaraPembayaranGajiAction}. Kelas ini menerjemahkan satu item data
	 * menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link CaraPembayaranGajiAction} dan dapat mengakses
