...
/**
 * Controller/action ZK untuk departemen punya pegawai. Tipe ini merupakan titik masuk UI yang
 ...
 * tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code Departemen departemen}, {@code Paging
 ...
 * @see MyDetail
 */
public class DepartemenPunyaPegawaiAction extends MyDetail {
	...
	public DepartemenPunyaPegawaiAction(Departemen departemen) {
		super();
		...
		this.addEventListener(Events.ON_OPEN, new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				Common.clear(DepartemenPunyaPegawaiAction.this);
				if (isOpen()) {
					display();
				}
			}
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link DepartemenPunyaPegawaiAction}. Kelas ini menerjemahkan satu item
	 * data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link DepartemenPunyaPegawaiAction} dan dapat
	 ...
	 *
	 * @see DepartemenPunyaPegawaiAction
	 */
	...
	private Criteria initCriteria(boolean order) {
		...
		.add(critKode).add(critNama).add(Restrictions.eq("departemen", departemen));
	...
	public void loadData(Event event) {
		...
		List<Pegawai> pegawais = departemen == null || departemen.getId() == null ? new ArrayList<Pegawai>()
	...
	private void display() {
		...
		button.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				List<Pegawai> pegawais = session.createCriteria(Pegawai.class).add(Restrictions.or(Restrictions.eq("aktif", true), Restrictions.isNull("aktif")))
						.add(Restrictions.eq("departemen", departemen)).list();
						...
						if (i == MyMessageboxConfig.OK) {
							try {
								...
								session.createSQLQuery(
										"update pegawai set departemen=null where departemen = " + departemen.getId())
										.executeUpdate();
