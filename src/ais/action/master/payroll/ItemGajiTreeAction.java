...
/**
 * Controller/action ZK untuk item gaji tree. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * AmbilDataFormatItemGajiBanbox searchFormatItemGaji}, {@code Tree tree}, {@code MyCheckboxConfig
 * tampilkanRumus}, {@code Textbox nama}, {@code Textbox defaultFormula}, {@code Textbox kode}, {@code MyIntbox
 * nomorUrut}; inisialisasi/lifecycle ({@code doAfterCompose()}, {@code initTree()}, {@code init()});
 * pembacaan/pencarian ({@code onReloadTree()}, {@code reloadTreeitem()}, {@code reloadTreeitem()}, {@code
 * reloadTotal()}); mutasi data ({@code onSave()}); pelaporan/ekspor ({@code onCetak()}); operasi domain lain
