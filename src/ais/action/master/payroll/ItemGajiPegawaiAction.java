...
/**
 * Controller/action ZK untuk item gaji pegawai. Tipe ini merupakan titik masuk UI yang
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code Window addWindow}, {@code
 * AmbilDataFormatItemGajiBanbox searchFormatItemGaji}, {@code AmbilDataPegawaiBanbox searchPegawai}, {@code
 * MyCheckboxConfig tampilkanRumus}, {@code Tree tree}, {@code Textbox nama}, {@code Textbox defaultFormula},
 * {@code Textbox kode}; inisialisasi/lifecycle ({@code doAfterCompose()}, {@code initTree()}, {@code init()});
 * pembacaan/pencarian ({@code onReloadTree()}, {@code reloadTreeitem()}, {@code reloadTreeitem()}, {@code
 * reloadTotal()}); mutasi data ({@code onSave()}, {@code onResetTree()}); pelaporan/ekspor ({@code onCetak()});
