...
/**
 * Controller/action ZK untuk item gaji. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code Window addWindow}, {@code Paging
 * paging}, {@code MyGrid grid}, {@code Textbox searchkode}, {@code Textbox searchnama}, {@code
 * AmbilDataFormatItemGajiBanbox searchFormatItemGaji}, {@code Textbox nama}, {@code
 * AmbilDataFormatItemGajiBanbox formatItemGaji}; inisialisasi/lifecycle ({@code doAfterCompose()}, {@code
 * init()}, {@code init()}, {@code initCriteria()}); pembacaan/pencarian ({@code onSearchDefault()}, {@code
 * getKode()}); mutasi data ({@code onSave()}, {@code setKode()}); operasi domain lain ({@code onAdd()}). Bagian
...
public class ItemGajiAction extends GenericAutowireComposer
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link ItemGajiAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link ItemGajiAction} dan dapat mengakses state
