...
/**
 * Controller/action ZK untuk pembayaran gaji. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * {@code MyTextbox searchnama}, {@code MyTextbox searchnamaPegawai}, {@code Combobox bulan}, {@code Intbox
 * tahun}, {@code Combobox caraBayar}, {@code Combobox caraBayarByr}; inisialisasi/lifecycle ({@code
 * doAfterCompose()}, {@code initCriteria()}, {@code init()}); pembacaan/pencarian ({@code onSearchDefault()},
 * {@code ambil()}, {@code ambilClass()}, {@code getindex()}); mutasi data ({@code onSave()}, {@code
 * setPersetujuan()}); pelaporan/ekspor ({@code cetak()}, {@code cetakData()}); operasi domain lain ({@code
 * onAdd()}, {@code form()}, {@code istilah()}, {@code generateCode()}). Bagian lain dari kontrak tetap mengikuti
...
public class PembayaranGajiAction extends GenericAutowireComposer implements DataInitDefault, FormSop {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link PembayaranGajiAction}. Kelas ini menerjemahkan satu item data
	 * menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link PembayaranGajiAction} dan dapat mengakses
