...
/**
 * Controller/action ZK untuk mahasiswa registrasi wisuda. Tipe ini merupakan titik masuk UI yang
 ...
 * paging}, {@code MyGrid grid}, {@code MyLabelConfig labelPersetujuanAdministrasi}, {@code MyLabelConfig
 * labelPersetujuanAdministrasiFakultas}, {@code MyLabelConfig labelPersetujuanKeuangan}, {@code MyLabelConfig
 * labelPersetujuanPerpustakaan}, {@code MyLabelConfig labelPersetujuanPerpustakaanFakultas};
 * inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code doAfterCompose()}, {@code initMain()}, {@code
 * init()}, {@code initCriteria()}); pembacaan/pencarian ({@code onSearchDefault()}); mutasi data ({@code
 * onSave()}, {@code onSaveMahasiswa()}); operasi domain lain ({@code onAddExternal()}). Bagian lain dari kontrak
...
public class MahasiswaRegistrasiWisudaAction extends GenericAutowireComposer {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link MahasiswaRegistrasiWisudaAction}. Kelas ini menerjemahkan satu
	 * item data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link MahasiswaRegistrasiWisudaAction} dan dapat
	 ...
	 *
	 * @see MahasiswaRegistrasiWisudaAction
	 */
