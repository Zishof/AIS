...
/**
 * DKPS Tabel 2.1-3 — Kerjasama Tridharma Bagian 3: Kerjasama Pengabdian kepada Masyarakat (PkM).
 *
 * <p>Menampilkan daftar kerjasama program studi dengan lembaga mitra dalam bidang
 * Pengabdian kepada Masyarakat (PkM) selama periode evaluasi akreditasi LAMDIK 2.0.
 * Data mencakup identitas lembaga mitra, tingkat kerjasama (lokal/nasional/internasional),
 * judul kegiatan, manfaat yang diperoleh program studi, serta periode berlakunya
 * kerjasama.</p>
 *
 ...
 * Perguruan Tinggi, khususnya pada ranah pengabdian dan pemberdayaan masyarakat.
 * Kerjasama PkM yang memberikan dampak sosial nyata menjadi indikator penting dalam
 * penilaian akreditasi.</p>
 *
 ...
 *
 * <h3>Sumber data</h3>
 * <p>Data diambil dari tabel {@code kerjasama} dengan filter {@code jenis='PENGABDIAN'}.
 ...
 */
public class LaporanDkps_2_1_3_KerjasamaPkm extends AkreditasiBaseWindow {

    /** Kode sheet untuk konfigurasi grid, grafik, dan Excel template. */
    public static final String sheetCode = "DKPS-2.1-3";
     ...
     */
    public LaporanDkps_2_1_3_KerjasamaPkm() {
        super();
        try {
            initFakultasJurusan();
            buildBase(false);
     ...
     */
    public LaporanDkps_2_1_3_KerjasamaPkm(String title, String border, boolean closable) throws Exception {
    ...
    /**
     * Memuat data kerjasama PkM dari database di background thread.
     *
     * <p>Query utama mengambil data dari tabel {@code kerjasama} dengan filter
     * {@code jenis='PENGABDIAN'}. Kolom tingkat kerjasama dipetakan ke tiga
                ...
                public void run() {
                    ...
                    try {
                        ...
                        try {
                            ...
                            queryBerhasil = true;
                        } catch (Exception ignored) { ais.common.ErrorAuditUtil.record(ignored, "auto-audit(empty-catch) src/ais/action/master/akreditasi/LaporanDkps_2_1_3_KerjasamaPkm.java:165");
                        }
                        ...
                        ex.printStackTrace(); ais.common.ErrorAuditUtil.record(ex, "auto-audit src/ais/action/master/akreditasi/LaporanDkps_2_1_3_KerjasamaPkm.java:181");
                        label.setValue("");
                    } finally {
                        if (session != null) {
                            try { session.disconnect(); } catch (Exception ignored) { ais.common.ErrorAuditUtil.record(ignored, "auto-audit(empty-catch) src/ais/action/master/akreditasi/LaporanDkps_2_1_3_KerjasamaPkm.java:185");}
                            try { session.close(); } catch (Exception ignored) { ais.common.ErrorAuditUtil.record(ignored, "auto-audit(empty-catch) src/ais/action/master/akreditasi/LaporanDkps_2_1_3_KerjasamaPkm.java:186");}
                        }
