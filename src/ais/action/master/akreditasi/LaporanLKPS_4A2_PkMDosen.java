...
/**
 * LKPS Tabel 4.A.2 — Pengabdian kepada Masyarakat (PkM) Dosen Tetap Program Studi.
 *
 * <p>Menampilkan jumlah kegiatan Pengabdian kepada Masyarakat yang dilakukan
 * dosen tetap program studi, dikelompokkan per sumber pendanaan selama tiga
 * tahun terakhir (TS-2, TS-1, TS).</p>
 *
 * <p>PkM adalah salah satu dari tiga dharma perguruan tinggi (Tridharma).
 * Borang BAN-PT menghitung PkM per sumber pendanaan untuk menilai diversifikasi
 * kemitraan dosen. Idealnya ada keseimbangan antara pendanaan mandiri, institusi,
 * dan pemerintah/lembaga eksternal.</p>
 *
 * <p>Sumber pendanaan PkM mencakup:</p>
 * <ol>
 *   <li>Pembiayaan sendiri oleh pelaksana</li>
 ...
 * </ol>
 *
 * <p>Hanya PkM berstatus {@code DISETUJUI} yang dihitung. Data diambil dari
 * entitas {@link PengajuanPenelitianDanPengabdian} dengan tipe PENGABDIAN.</p>
 *
 * <h3>Format data baris (LKPS-4.A.2, dataStartRow=7)</h3>
 ...
 *   <li>Kolom data dipindah ke posisi 0-3 (sebelumnya 2-4 tanpa label)</li>
 *   <li>Filter Fakultas ditambahkan dan dihubungkan dengan Program Studi</li>
 *   <li>Try-finally ditambahkan agar data selalu dikirim ke UI meski exception</li>
 *   <li>{@code HibernateUtil.closeSession()} dihapus — menggunakan {@code currentSession()}
 *       yang dikelola otomatis oleh framework</li>
 * </ul>
 *
 * @author AIS Development Team
 * @version 2.0
 */
public class LaporanLKPS_4A2_PkMDosen extends AkreditasiBaseWindow {

    /** Kode sheet untuk konfigurasi grid, grafik, dan Excel template. */
    public static final String sheetCode = "LKPS-4.A.2";
    private static final long serialVersionUID = 1L;

    private Combobox tahunAjaran;

    /**
     * Daftar nama sumber dana PkM sesuai borang BAN-PT LKPS 4.A.2.
     * String kosong di akhir mewakili "Sumber Lain".
     */
    private static final String[] SUMBER_LIST = {
        "Pembiayaan sendiri oleh pelaksana",
     ...
     */
    public LaporanLKPS_4A2_PkMDosen() {
        super();
        try {
            initFakultasJurusan();
     ...
     */
    public LaporanLKPS_4A2_PkMDosen(String title, String border, boolean closable) throws Exception {
    ...
    /**
     * Memuat data PkM dosen dari database di background thread.
     *
     * <p>Untuk setiap sumber dana dalam {@link #SUMBER_LIST}, metode menghitung
     * jumlah PkM yang disetujui per tahun (TS-2, TS-1, TS) menggunakan Hibernate
     * Criteria. Setiap baris data dibangun dengan format
     * {@code [Sumber Dana, TS-2, TS-1, TS]}. String kosong ditampilkan sebagai
     * "Sumber Lain".</p>
     *
     * <p>Catatan: menggunakan {@code currentSession()} yang dikelola otomatis
     * oleh framework dan tidak perlu ditutup manual.</p>
     *
    ...
    public void onCetak(Event event) {
        ...
        try {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        // 7 baris kosong sesuai dataStartRow konfigurasi LKPS-4.A.2
                        ...
                        ex.printStackTrace(); ais.common.ErrorAuditUtil.record(ex, "auto-audit src/ais/action/master/akreditasi/LaporanLKPS_4A2_PkMDosen.java:217");
                        label.setValue("");
                    }
