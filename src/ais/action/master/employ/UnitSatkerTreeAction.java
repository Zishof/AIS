...
import ais.action.master.employ.util.UnitSatkerTreeModel;
...
/**
 * Controller/action ZK untuk unit satker tree. Tipe ini merupakan titik masuk UI yang
 ...
 * {@code Textbox nama}, {@code Textbox keterangan}, {@code MyCheckboxConfig defaultItem}, {@code Combobox
 * pejabatStruktural}, {@code boolean edit}, {@code boolean delete}; inisialisasi/lifecycle ({@code
 * doBeforeCompose()}, {@code doAfterCompose()}, {@code initTree()}, {@code init()}); pembacaan/pencarian ({@code
 * onReloadTree()}, {@code reloadTreeitem()}, {@code reloadTreeitem()}, {@code reloadTotal()}); mutasi data
 * ({@code onSave()}); operasi domain lain ({@code onAdd()}, {@code openChilds()}, {@code closeChilds()}, {@code
 * hasSomeChilds()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di
 * atas.</p>
 ...
 */
public class UnitSatkerTreeAction extends GenericAutowireComposer {
	...
	private Combobox pejabatStruktural;
	...
	private UnitSatkerTreeModel unitSakerTreeModel;

	private TreeMap<UnitKerja, Treecell[]> treecellMap = new TreeMap<UnitKerja, Treecell[]>();
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		Common.insertCombo(pejabatStruktural = new Combobox(), "nama", JabatanStruktural.class,
	...
	public void onAdd(Event event) throws Exception {
		...
		UnitKerja myUnitKerja = new UnitKerja();
		myUnitKerja.setParent(null);

		init(myUnitKerja, new EventListener() {
	...
	private void init(UnitKerja unitkerja, final EventListener eventListener) {
		...
		defaultItem.setChecked(unitkerja.getDefaultItem() != null && unitkerja.getDefaultItem());

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Pejabat Struktural"));
		row.appendChild(pejabatStruktural);
		Common.selectComboItem(pejabatStruktural,
				unitkerja.getJabatanStruktural() == null ? null : unitkerja.getJabatanStruktural());
		...
		save.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				if (onSave(event)) {
					// onReloadTree(null);
					eventListener.onEvent(new Event("", null, UnitSatkerTreeAction.this.unitKerja));
					addWindow.setVisible(false);
				}
			}
		});
	...
	public boolean onSave(Event event) throws Exception {
		...
		unitKerja.setDefaultItem(defaultItem.isChecked());
		unitKerja.setNama(nama.getValue());
		unitKerja.setKeterangan(keterangan.getValue());
		unitKerja.setJabatanStruktural((JabatanStruktural) (pejabatStruktural.getSelectedItem() == null ? null
				: pejabatStruktural.getSelectedItem().getValue()));
	...
	public void onReloadTree(Event event) throws Exception {
		...
		unitSakerTreeModel = new UnitSatkerTreeModel(true);
		tree.setModel(unitSakerTreeModel);
		tree.setItemRenderer(new ais.ui.util.MyTreeitemRenderer() {
			...
			public void render(final Treeitem treeitem, Object arg1) throws Exception {
				...
				try {
					...
					hasSomeChilds(treerow, unitKerja);
					...
					button.addEventListener("onClick", new EventListener() {
						...
						public void onEvent(Event event) throws Exception {
							...
							UnitKerja myUnitKerja = (UnitKerja) unitKerja.clone();
							myUnitKerja.setParent(unitKerja);
							myUnitKerja.setId(null);
							init(myUnitKerja, new EventListener() {

								@Override
								public void onEvent(Event arg0) throws Exception {
									render(treeitem, unitKerja);
												...
												public void onEvent(Event arg0) throws Exception {
													...
													try {
														Treeitem myTreeitem = (Treeitem) treecellMap
																.get(unitKerja)[0].getParent().getParent();

														render(myTreeitem, unitKerja);
					...
					button.addEventListener("onClick", new EventListener() {
						...
						public void onEvent(Event event) throws Exception {
							...
							UnitKerja myUnitkerja = (UnitKerja) unitKerja.clone();
							myUnitkerja.setParent(unitKerja.getParent());
							myUnitkerja.setId(null);
							init(myUnitkerja, new EventListener() {
					...
					button.addEventListener("onClick", new EventListener() {
						@Override
						public void onEvent(Event event) throws Exception {
							init(unitKerja, new EventListener() {
										...
										public void onEvent(Event event) throws Exception {
											...
											if (i == MyMessageboxConfig.OK) {
												try {
													...
													unitSakerTreeModel.deleteChilds(unitKerja);

													Common.refreshDelete((unitKerja));
	...
	private void hasSomeChilds(Treerow treerow, final UnitKerja unitKerja) {

		Treecell treecell = new Treecell(unitKerja.toString());
		treecell.setTooltiptext(unitKerja.toString());
		treecell.setStyle("font-size:x-small;text-align: left;");
		treecell.setParent(treerow);

		Treecell treecellAktif = new Treecell(
				unitKerja.getDefaultItem() == null || !unitKerja.getDefaultItem() ? "Tidak" : "Aktif");
		...
		treecellMap.put(unitKerja, new Treecell[] { treecellAktif });
