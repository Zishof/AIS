...
/**
 * Controller/action ZK untuk makan. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 */
public class MakanAction extends GenericAutowireComposer implements DataCriteria, DataSearchDefault {
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		String[] contents = new String[] { "id", "keterangan", "golongan", "peraturan", "masaKerja", "makan",
				"tanggalEfektif" };
		MyToolbarbuttonConfig cetakToolbarbutton = Common.cetakData(Makan.class, this, contents);
		Common.appendKeToolbar(cetakToolbarbutton, add, comp);

		MyToolbarbuttonConfig upload = Common.uploadData(this, Makan.class, contents);
		if (upload != null) { upload.setVisible((add != null && add.isVisible()) && edit && delete); }
		Common.appendKeToolbar(upload, add, comp);
	}

	/**
	 * Renderer lokal untuk layar/komponen {@link MakanAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link MakanAction} dan dapat mengakses state kelas
	 ...
	 * @see MakanAction
	 */
	class MakanRenderer extends ais.ui.util.MyRowRenderer {

		@Override
		public void render(final Row arg0, Object arg1) throws Exception {
			arg0.setValign("top");
			// TODO Auto-generated method stub
			final Makan makan = (Makan) arg1;

			new Label(makan.getGolongan() == null ? "" : makan.getGolongan().getNama()).setParent(arg0);

			Vbox a;
			(a = RevisiHelper.createNewRevisi(Makan.class, makan,
					makan.getPeraturan() == null ? "-" : makan.getPeraturan().getNama())).setParent(arg0);
			if (makan.getPeraturan() != null) {
				Vbox myvbox = new Vbox();
				myvbox.setParent(a);

				Hbox hbox = new Hbox();
				hbox.setParent(myvbox);
				LampiranLain.createDownloadUploadFileLain(hbox, makan.getPeraturan().getId(),
						Peraturan.class.getName(), "Peraturan Dokumen", false, null, null, false, false, false, false);
			}

			new Label(
					makan.getMasaKerja() == null ? "0" : Common.numberFormat.get().format(makan.getMasaKerja()) + " th")
					.setParent(arg0);

			new Label(makan.getMakan() == null ? "0" : Common.numberFormat.get().format(makan.getMakan()))
					.setParent(arg0);

			new Label(makan.getTanggalEfektif() == null ? "0"
					: Common.dateFormat1.get().format(makan.getTanggalEfektif())).setParent(arg0);
			new Label(makan.getKeterangan()).setParent(arg0);
	...
	private void init(Makan makan) throws Exception {
		this.makan = makan;
		addWindow.setTitle(makan.getId() == null ? "Tambah Makan" : "Ubah Makan");
		...
		Common.selectComboItem(golongan, makan.getGolongan());
		...
		Common.selectComboItem(masaKerja, makan.getMasaKerja() == null ? null : makan.getMasaKerja());
		masaKerja.setWidth("90%");
		masaKerja.setReadonly(true);

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Makan (MAKAN) *"));
		row.appendChild(gaji = new MyDoublebox(makan.getMakan()));
		gaji.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Tanggal Efektif *"));
		row.appendChild(tanggalEfektif = new MyDatebox(makan.getTanggalEfektif()));

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Keterangan"));
		row.appendChild(keterangan = new Textbox(makan.getKeterangan()));
	...
	public boolean onSave(Event event) throws Exception {
		...
		if (gaji.getValue() == null) {
			MyMessageboxConfig.show("Mohon maaf, Tunjangan Makan belum diisi. Langkah yang dapat dilakukan: (1) isi kolom Makan pada form; (2) pastikan nilai berupa angka yang valid; (3) ulangi proses simpan. Jika masih mengalami kendala, hubungi Administrator atau tim teknis.", "Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION);
			return false;
		}
		...
		if (makan.getId() != null) {
			...
			Common.refreshUpdate(session, makan);
		}
	...
	public Criteria initCriteria(boolean order) {
		Session session = HibernateUtil.currentSession();
		Criteria criteria = session.createCriteria(Makan.class).createAlias("golongan", "golongan")
	...
	public void onSearchDefault(Event event) {
		...
		List<Makan> makan = initCriteria(true).setMaxResults(Common.ROWS_COUNT_ON_PAGE)
				.setFirstResult(Common.ROWS_COUNT_ON_PAGE * (paging == null ? 0 : paging.getActivePage())).list();
		ListModel strset = new SimpleListModel(makan);
		grid.setRowRenderer(new MakanRenderer());
