...
/**
 * Controller/action ZK untuk gaji pokok. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * pembacaan/pencarian ({@code onDownload()}, {@code onUploadGapok()}, {@code onSearchDefault()}); mutasi data
 * ({@code onSave()}); operasi domain lain ({@code onKodeTunjangan()}, {@code onInsentif()}, {@code onMakan()},
 * {@code onTransport()}, {@code onAdd()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface
...
public class GajiPokokAction extends GenericAutowireComposer
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link GajiPokokAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link GajiPokokAction} dan dapat mengakses state
