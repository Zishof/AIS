...
public class RiwayatKeluarNegeriPegawaiHelper {
	...
	private RiwayatKeluarNegeriPegawai riwayatKeluarNegeriPegawai;

	private Textbox nama;
	private Textbox tujuan;
	private Textbox lama;
	...
	class RiwayatKeluarNegeriPegawaiRenderer extends ais.ui.util.MyRowRenderer {
		...
		public void render(final Row row, Object arg1) throws Exception {
			...
			final RiwayatKeluarNegeriPegawai riwayatKeluarNegeriPegawai = (RiwayatKeluarNegeriPegawai) arg1;

			new ais.ui.util.MyHtml(
					"<font style=\"font-size: x-small;\">" + (riwayatKeluarNegeriPegawai.getPegawai() == null ? ""
							: riwayatKeluarNegeriPegawai.getPegawai().toString()) + "</font>")
					.setParent(row);

			new Label(riwayatKeluarNegeriPegawai.getNama() == null ? "" : riwayatKeluarNegeriPegawai.getNama())
					.setParent(row);
			new Label(riwayatKeluarNegeriPegawai.getTujuan() == null ? "" : riwayatKeluarNegeriPegawai.getTujuan())
					.setParent(row);
			new Label(riwayatKeluarNegeriPegawai.getLama() + "").setParent(row);
			new Label(riwayatKeluarNegeriPegawai.getAlamat() == null ? "" : riwayatKeluarNegeriPegawai.getAlamat())
					.setParent(row);

			new Image(riwayatKeluarNegeriPegawai.getStatus() ? "/img/svg/check2.svg" : "/img/svg/warning-outline.svg")
					.setParent(row);
			...
			button.addEventListener("onClick", new EventListener() {
				...
				public void onEvent(Event event) throws Exception {
					init(riwayatKeluarNegeriPegawai);
			...
			button.setVisible(!riwayatKeluarNegeriPegawai.getStatus());
								...
								public void onEvent(Event event) throws Exception {
									...
									if (i == MyMessageboxConfig.OK) {
										try {
											...
											session.delete((riwayatKeluarNegeriPegawai));
	...
	public Borderlayout display() throws Exception {
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				init(new RiwayatKeluarNegeriPegawai());
			}
		});
		...
		column = new MyColumnConfig();
		column.setParent(columns);
		column.setLabel("Kota/Propinsi/Negara");
	...
	public void onSearchDefault(Event event) {
		...
		Session session = HibernateUtil.currentSession();
		List<RiwayatKeluarNegeriPegawai> riwayatKeluarNegeriPegawai = session
				.createCriteria(RiwayatKeluarNegeriPegawai.class)
		...
		ListModel strset = new SimpleListModel(riwayatKeluarNegeriPegawai);

		grid.setRowRenderer(new RiwayatKeluarNegeriPegawaiRenderer());
	...
	public void init(final RiwayatKeluarNegeriPegawai riwayatKeluarNegeriPegawai) throws Exception {
		...
		final MyWindow window = new MyWindow("Pendataan Riwayat Keluar Negeri", "none", true);
		window.setWidth("90%");
		window.setHeight("97%");
		...
		east.appendChild(new FotoLampiranPegawaiHelper(gridFotoGambar = new MyGrid())
				.initDetail(riwayatKeluarNegeriPegawai, RiwayatKeluarNegeriPegawai.class));
		...
		ambilDataPegawaiBanbox.setValue(riwayatKeluarNegeriPegawai.getPegawai() == null ? ""
				: riwayatKeluarNegeriPegawai.getPegawai().getNama());
		ambilDataPegawaiBanbox.setAttribute("pegawai", riwayatKeluarNegeriPegawai.getPegawai());
		...
		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Kota/Propinsi/Negara"));
		row.appendChild(nama = new Textbox(riwayatKeluarNegeriPegawai.getNama()));
		nama.setWidth("90%");
		nama.setRows(3);

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Tujuan Kunjungan"));
		row.appendChild(tujuan = new Textbox(riwayatKeluarNegeriPegawai.getTujuan()));
		tujuan.setWidth("90%");
		tujuan.setRows(3);

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Lama"));
		row.appendChild(lama = new Textbox(riwayatKeluarNegeriPegawai.getLama()));
		lama.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Dibiayai oleh"));
		row.appendChild(alamat = new Textbox(riwayatKeluarNegeriPegawai.getAlamat()));
		alamat.setWidth("90%");
		alamat.setRows(3);

		if (riwayatKeluarNegeriPegawai.getStatus()) {
		...
		status.setChecked(riwayatKeluarNegeriPegawai.getStatus());
	...
	public boolean save(Event event) throws Exception {
		...
		if (nama.getValue().trim().equals("")) {
			MyMessageboxConfig.show("Mohon maaf, Negara belum diisi. Langkah yang dapat dilakukan: (1) isi kolom Negara pada form; (2) pastikan nama negara tidak kosong; (3) ulangi proses simpan. Jika masih mengalami kendala, hubungi Administrator atau tim teknis.", MyMessageboxConfig.INFORMATION, MyMessageboxConfig.OK, "");
			return false;
		}

		if (tujuan.getValue().trim().equals("")) {
			MyMessageboxConfig.show("Mohon maaf, Tujuan belum diisi. Langkah yang dapat dilakukan: (1) isi kolom Tujuan kepergian pada form; (2) pastikan tujuan tidak kosong; (3) ulangi proses simpan. Jika masih mengalami kendala, hubungi Administrator atau tim teknis.", MyMessageboxConfig.INFORMATION, MyMessageboxConfig.OK, "");
			return false;
		}

		if (lama.getValue().trim().equals("")) {
			MyMessageboxConfig.show("Mohon maaf, Tahun/Lama belum diisi. Langkah yang dapat dilakukan: (1) isi kolom Tahun/Lama kepergian pada form; (2) pastikan nilai tidak kosong; (3) ulangi proses simpan. Jika masih mengalami kendala, hubungi Administrator atau tim teknis.", MyMessageboxConfig.INFORMATION, MyMessageboxConfig.OK, "");
			return false;
		}

		if (alamat.getValue().trim().equals("")) {
			MyMessageboxConfig.show("Mohon maaf, kolom Dibiayai oleh belum diisi. Langkah yang dapat dilakukan: (1) isi kolom Dibiayai oleh pada form; (2) pastikan tidak kosong; (3) ulangi proses simpan. Jika masih mengalami kendala, hubungi Administrator atau tim teknis.", MyMessageboxConfig.INFORMATION, MyMessageboxConfig.OK,
					"");
			return false;
		}
		...
		Session session = HibernateUtil.currentSession();
		if (riwayatKeluarNegeriPegawai.getId() != null) {
			riwayatKeluarNegeriPegawai = (RiwayatKeluarNegeriPegawai) session.load(RiwayatKeluarNegeriPegawai.class,
					riwayatKeluarNegeriPegawai.getId());
		}

		riwayatKeluarNegeriPegawai.setStatus(status.isChecked());
		riwayatKeluarNegeriPegawai.setTujuan(tujuan.getValue());
		riwayatKeluarNegeriPegawai.setLama(lama.getValue());
		riwayatKeluarNegeriPegawai.setPegawai((Pegawai) ambilDataPegawaiBanbox.getAttribute("pegawai"));
		riwayatKeluarNegeriPegawai.setAlamat(alamat.getValue());
		riwayatKeluarNegeriPegawai.setNama(nama.getValue());

		if (riwayatKeluarNegeriPegawai.getId() != null) {
			session.update(riwayatKeluarNegeriPegawai);
		} else {
			session.save(riwayatKeluarNegeriPegawai);
		}
		...
		try {
			...
			for (Row row : rowsDocument) {
				FotoLampiranPegawai fotoLampiranPegawai = (FotoLampiranPegawai) row.getAttribute("fotoLampiranPegawai");
				fotoLampiranPegawai.setItem(riwayatKeluarNegeriPegawai.getId());
				fotoLampiranPegawai.setClazz(RiwayatKeluarNegeriPegawai.class.getName());
