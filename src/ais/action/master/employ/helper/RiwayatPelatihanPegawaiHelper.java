...
public class RiwayatPelatihanPegawaiHelper {
	...
	private RiwayatPelatihanPegawai riwayatPelatihanPegawai;
	...
	class RiwayatPelatihanPegawaiRenderer extends ais.ui.util.MyRowRenderer {
		...
		public void render(final Row row, Object arg1) throws Exception {
			...
			final RiwayatPelatihanPegawai riwayatPelatihanPegawai = (RiwayatPelatihanPegawai) arg1;
			new ais.ui.util.MyHtml(
					"<font style=\"font-size: x-small;\">" + (riwayatPelatihanPegawai.getPegawai() == null ? ""
							: riwayatPelatihanPegawai.getPegawai().getNama()) + "</font>")
					.setParent(row);

			new Label(riwayatPelatihanPegawai.getMulai() == null ? ""
					: Common.dateFormat6.get().format(riwayatPelatihanPegawai.getMulai())).setParent(row);
			new Label(riwayatPelatihanPegawai.getSelesai() == null ? ""
					: Common.dateFormat6.get().format(riwayatPelatihanPegawai.getSelesai())).setParent(row);

			new Label(riwayatPelatihanPegawai.getNama() == null ? "" : riwayatPelatihanPegawai.getNama())
					.setParent(row);
			new Label(riwayatPelatihanPegawai.getJenisPelatihan() == null ? ""
					: riwayatPelatihanPegawai.getJenisPelatihan().getNama()).setParent(row);
			new Label(riwayatPelatihanPegawai.getJurusan()).setParent(row);
			new Label(riwayatPelatihanPegawai.getNoIjazah()).setParent(row);
			new Label(riwayatPelatihanPegawai.getAlamat() == null ? "" : riwayatPelatihanPegawai.getAlamat())
					.setParent(row);
			new Label(riwayatPelatihanPegawai.getKeterangan()).setParent(row);
			new Image(riwayatPelatihanPegawai.getStatus() ? "/img/svg/check2.svg" : "/img/svg/warning-outline.svg")
					.setParent(row);
			...
			button.setVisible(!riwayatPelatihanPegawai.getStatus());
			button.addEventListener("onClick", new EventListener() {
				@Override
				public void onEvent(Event event) throws Exception {
								...
								public void onEvent(Event event) throws Exception {
									...
									if (i == MyMessageboxConfig.OK) {
										try {
											...
											session.delete((riwayatPelatihanPegawai));
	...
	public Borderlayout display() throws Exception {
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				init(new RiwayatPelatihanPegawai());
			}
		});
	...
	public void onSearchDefault(Event event) {
		...
		Session session = HibernateUtil.currentSession();
		List<RiwayatPelatihanPegawai> riwayatPelatihanPegawai = session.createCriteria(RiwayatPelatihanPegawai.class)
		...
		ListModel strset = new SimpleListModel(riwayatPelatihanPegawai);

		grid.setRowRenderer(new RiwayatPelatihanPegawaiRenderer());
	...
	public void init(final RiwayatPelatihanPegawai riwayatPelatihanPegawai) throws Exception {
		...
		final MyWindow window = new MyWindow("Pendataan Riwayat Pelatihan", "none", true);
		window.setWidth("90%");
		window.setHeight("97%");
		...
		east.appendChild(new FotoLampiranPegawaiHelper(gridFotoGambar = new MyGrid())
				.initDetail(riwayatPelatihanPegawai, RiwayatPelatihanPegawai.class));
		...
		ambilDataPegawaiBanbox.setValue(
				riwayatPelatihanPegawai.getPegawai() == null ? "" : riwayatPelatihanPegawai.getPegawai().getNama());
		ambilDataPegawaiBanbox.setAttribute("pegawai", riwayatPelatihanPegawai.getPegawai());
		...
		row.appendChild(mulai = new MyDatebox(riwayatPelatihanPegawai.getMulai()));
		mulai.setFormat(Common.dateFormat1.get().toPattern());

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Sampai *"));
		row.appendChild(selesai = new MyDatebox(riwayatPelatihanPegawai.getSelesai()));
		selesai.setFormat(Common.dateFormat1.get().toPattern());

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Nama Pelatihan / Diklat / Kursus"));
		row.appendChild(nama = new Textbox(riwayatPelatihanPegawai.getNama()));
		nama.setWidth("90%");
		nama.setRows(3);

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Jenis Pelatihan / Diklat / Kursus"));
		row.appendChild(jenisPelatihan = new AmbilDataJenisPelatihanBanbox());
		jenisPelatihan.setWidth("90%");
		jenisPelatihan.setAttribute("jenisPelatihan", riwayatPelatihanPegawai.getJenisPelatihan());
		jenisPelatihan.setValue(riwayatPelatihanPegawai.getJenisPelatihan() == null ? ""
				: riwayatPelatihanPegawai.getJenisPelatihan().getNama());

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Tema Pelatihan / Diklat / Kursus"));
		row.appendChild(jurusan = new Textbox(riwayatPelatihanPegawai.getJurusan()));
		jurusan.setWidth("90%");
		jurusan.setRows(3);

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Ada Sertifikat ?"));
		sertifikasi = new MyCheckboxConfig();
		row.appendChild(sertifikasi);
		sertifikasi.setChecked(riwayatPelatihanPegawai.getSertifikasi());

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Sertifikat / Tanda Lulus / Ijazah"));
		row.appendChild(noIjazah = new Textbox(riwayatPelatihanPegawai.getNoIjazah()));
		noIjazah.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Lokasi Pelatihan / Diklat / Kursus"));
		row.appendChild(alamat = new Textbox(riwayatPelatihanPegawai.getAlamat()));
		alamat.setWidth("90%");
		alamat.setRows(3);
		...
		row.appendChild(keterangan = new Textbox(riwayatPelatihanPegawai.getKeterangan()));
		keterangan.setWidth("90%");
		keterangan.setRows(3);

		if (riwayatPelatihanPegawai.getStatus()) {
		...
		status.setChecked(riwayatPelatihanPegawai.getStatus());
	...
	public boolean save(Event event) throws Exception {
		...
		if (nama.getValue().trim().equals("")) {
			MyMessageboxConfig.show("Mohon maaf, Nama Pelatihan/Diklat/Kursus belum diisi. Langkah yang dapat dilakukan: (1) isi kolom Nama Pelatihan; (2) pastikan nama tidak kosong; (3) ulangi proses simpan. Jika masih mengalami kendala, hubungi Administrator atau tim teknis.", MyMessageboxConfig.INFORMATION,
					MyMessageboxConfig.OK, "");
			return false;
		}

		if (alamat.getValue().trim().equals("")) {
			MyMessageboxConfig.show("Mohon maaf, Lokasi Pelatihan/Diklat/Kursus belum diisi. Langkah yang dapat dilakukan: (1) isi kolom Lokasi Pelatihan; (2) pastikan nama lokasi tidak kosong; (3) ulangi proses simpan. Jika masih mengalami kendala, hubungi Administrator atau tim teknis.", MyMessageboxConfig.INFORMATION,
					MyMessageboxConfig.OK, "");
			return false;
		}

		Date masuk = mulai.getValue();
		Date lulus = selesai.getValue();
		if (masuk.after(lulus)) {
			MyMessageboxConfig.show("Tanggal masuk tidak boleh lebih besar dari tahun lulus",
		...
		Session session = HibernateUtil.currentSession();
		if (riwayatPelatihanPegawai.getId() != null) {
			riwayatPelatihanPegawai = (RiwayatPelatihanPegawai) session.load(RiwayatPelatihanPegawai.class,
					riwayatPelatihanPegawai.getId());
		}

		riwayatPelatihanPegawai.setJenisPelatihan((JenisPelatihan) jenisPelatihan.getAttribute("jenisPelatihan"));
		riwayatPelatihanPegawai.setStatus(status.isChecked());
		riwayatPelatihanPegawai.setJurusan(jurusan.getValue());
		riwayatPelatihanPegawai.setNoIjazah(noIjazah.getValue());
		riwayatPelatihanPegawai.setSertifikasi(sertifikasi.isChecked());
		riwayatPelatihanPegawai.setPegawai((Pegawai) ambilDataPegawaiBanbox.getAttribute("pegawai"));
		riwayatPelatihanPegawai.setMulai(mulai.getValue());
		riwayatPelatihanPegawai.setSelesai(selesai.getValue());
		riwayatPelatihanPegawai.setAlamat(alamat.getValue());
		riwayatPelatihanPegawai.setNama(nama.getValue());
		riwayatPelatihanPegawai.setKeterangan(keterangan.getValue());

		if (riwayatPelatihanPegawai.getId() != null) {
			session.update(riwayatPelatihanPegawai);
		} else {
			session.save(riwayatPelatihanPegawai);
		}
		...
		try {
			...
			for (Row row : rowsDocument) {
				FotoLampiranPegawai fotoLampiranPegawai = (FotoLampiranPegawai) row.getAttribute("fotoLampiranPegawai");
				fotoLampiranPegawai.setItem(riwayatPelatihanPegawai.getId());
				fotoLampiranPegawai.setClazz(RiwayatPelatihanPegawai.class.getName());
