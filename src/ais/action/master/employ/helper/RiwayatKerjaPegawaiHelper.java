...
public class RiwayatKerjaPegawaiHelper {
	...
	private RiwayatKerjaPegawai riwayatKerjaPegawai;
	...
	class RiwayatKerjaPegawaiRenderer extends ais.ui.util.MyRowRenderer {
		...
		public void render(final Row row, Object arg1) throws Exception {
			...
			final RiwayatKerjaPegawai riwayatKerjaPegawai = (RiwayatKerjaPegawai) arg1;

			new ais.ui.util.MyHtml("<font style=\"font-size: x-small;\">"
					+ (riwayatKerjaPegawai.getPegawai() == null ? "" : riwayatKerjaPegawai.getPegawai().toString())
					+ "</font>").setParent(row);

			new Label(riwayatKerjaPegawai.getNama() == null ? "" : riwayatKerjaPegawai.getNama()).setParent(row);
			new Label(riwayatKerjaPegawai.getKedudukan()).setParent(row);
			new Label(riwayatKerjaPegawai.getTahunMulai() + "").setParent(row);
			new Label(riwayatKerjaPegawai.getTahunSelesai() + "").setParent(row);
			new Label(riwayatKerjaPegawai.getAlamat() == null ? "" : riwayatKerjaPegawai.getAlamat()).setParent(row);
			new Label(riwayatKerjaPegawai.getPimpinan()).setParent(row);

			new Image(riwayatKerjaPegawai.getStatus() ? "/img/svg/check2.svg" : "/img/svg/warning-outline.svg")
					.setParent(row);
			...
			button.setVisible(!riwayatKerjaPegawai.getStatus());
			button.addEventListener("onClick", new EventListener() {
				@Override
				public void onEvent(Event event) throws Exception {
								...
								public void onEvent(Event event) throws Exception {
									...
									if (i == MyMessageboxConfig.OK) {
										try {
											...
											session.delete((riwayatKerjaPegawai));
	...
	public Borderlayout display() throws Exception {
		...
		column = new MyColumnConfig();
		column.setParent(columns);
		column.setLabel("Nama Perusahaan");
	...
	public void onSearchDefault(Event event) {
		...
		Session session = HibernateUtil.currentSession();
		List<RiwayatKerjaPegawai> riwayatKerjaPegawai = session.createCriteria(RiwayatKerjaPegawai.class)
		...
		ListModel strset = new SimpleListModel(riwayatKerjaPegawai);

		grid.setRowRenderer(new RiwayatKerjaPegawaiRenderer());
	...
	public void init(final RiwayatKerjaPegawai riwayatKerjaPegawai) throws Exception {
		...
		final MyWindow window = new MyWindow("Pendataan Riwayat Perusahaan Lain", "none", true);
		window.setWidth("90%");
		window.setHeight("97%");
		...
		east.appendChild(new FotoLampiranPegawaiHelper(gridFotoGambar = new MyGrid()).initDetail(riwayatKerjaPegawai,
				RiwayatKerjaPegawai.class));
		...
		ambilDataPegawaiBanbox
				.setValue(riwayatKerjaPegawai.getPegawai() == null ? "" : riwayatKerjaPegawai.getPegawai().getNama());
		ambilDataPegawaiBanbox.setAttribute("pegawai", riwayatKerjaPegawai.getPegawai());
		...
		row.appendChild(nama = new Textbox(riwayatKerjaPegawai.getNama()));
		nama.setWidth("90%");
		nama.setRows(3);
		...
		row.appendChild(kedudukan = new Textbox(riwayatKerjaPegawai.getKedudukan()));
		kedudukan.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Tahun Mulai *"));
		row.appendChild(tahunMulai = new Intbox(riwayatKerjaPegawai.getTahunMulai()));
		tahunMulai.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Tahun Sampai *"));
		row.appendChild(tahunSelesai = new Intbox(riwayatKerjaPegawai.getTahunSelesai()));
		tahunSelesai.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Alamat *"));
		row.appendChild(alamat = new Textbox(riwayatKerjaPegawai.getAlamat()));
		alamat.setWidth("90%");
		alamat.setRows(3);
		...
		row.appendChild(pimpinan = new Textbox(riwayatKerjaPegawai.getPimpinan()));
		pimpinan.setWidth("90%");

		if (riwayatKerjaPegawai.getStatus()) {
		...
		status.setChecked(riwayatKerjaPegawai.getStatus());
	...
	public boolean save(Event event) throws Exception {
		...
		if (nama.getValue().trim().equals("")) {
			MyMessageboxConfig.show("Mohon maaf, Nama Perusahaan belum diisi. Langkah yang dapat dilakukan: (1) isi kolom Nama Perusahaan; (2) pastikan nama tidak kosong; (3) ulangi proses simpan. Jika masih mengalami kendala, hubungi Administrator atau tim teknis.", MyMessageboxConfig.INFORMATION,
		...
		Integer masuk = tahunMulai.getValue();
		Integer lulus = (Integer) tahunSelesai.getValue();
		if (masuk > lulus) {
		...
		Session session = HibernateUtil.currentSession();
		if (riwayatKerjaPegawai.getId() != null) {
			riwayatKerjaPegawai = (RiwayatKerjaPegawai) session.load(RiwayatKerjaPegawai.class,
					riwayatKerjaPegawai.getId());
		}

		riwayatKerjaPegawai.setStatus(status.isChecked());
		riwayatKerjaPegawai.setKedudukan(kedudukan.getValue());
		riwayatKerjaPegawai.setPimpinan(pimpinan.getValue());
		riwayatKerjaPegawai.setTahunSelesai(tahunSelesai.getValue());
		riwayatKerjaPegawai.setTahunMulai(tahunMulai.getValue());
		riwayatKerjaPegawai.setPegawai((Pegawai) ambilDataPegawaiBanbox.getAttribute("pegawai"));
		riwayatKerjaPegawai.setAlamat(alamat.getValue());
		riwayatKerjaPegawai.setNama(nama.getValue());

		if (riwayatKerjaPegawai.getId() != null) {
			session.update(riwayatKerjaPegawai);
		} else {
			session.save(riwayatKerjaPegawai);
		}
		...
		try {
			...
			for (Row row : rowsDocument) {
				FotoLampiranPegawai fotoLampiranPegawai = (FotoLampiranPegawai) row.getAttribute("fotoLampiranPegawai");
				fotoLampiranPegawai.setItem(riwayatKerjaPegawai.getId());
				fotoLampiranPegawai.setClazz(RiwayatKerjaPegawai.class.getName());
