...
public class RiwayatOrganisasiLainPegawaiHelper {
	...
	private RiwayatOrganisasiLainPegawai riwayatOrganisasiLainPegawai;
	...
	class RiwayatOrganisasiLainPegawaiRenderer extends ais.ui.util.MyRowRenderer {
		...
		public void render(final Row row, Object arg1) throws Exception {
			...
			final RiwayatOrganisasiLainPegawai riwayatOrganisasiLainPegawai = (RiwayatOrganisasiLainPegawai) arg1;

			new ais.ui.util.MyHtml(
					"<font style=\"font-size: x-small;\">" + (riwayatOrganisasiLainPegawai.getPegawai() == null ? ""
							: riwayatOrganisasiLainPegawai.getPegawai().toString()) + "</font>")
					.setParent(row);

			new Label(riwayatOrganisasiLainPegawai.getNama() == null ? "" : riwayatOrganisasiLainPegawai.getNama())
					.setParent(row);
			new Label(riwayatOrganisasiLainPegawai.getKedudukan()).setParent(row);
			new Label(riwayatOrganisasiLainPegawai.getTahunMulai() + "").setParent(row);
			new Label(riwayatOrganisasiLainPegawai.getTahunSelesai() + "").setParent(row);
			new Label(riwayatOrganisasiLainPegawai.getAlamat() == null ? "" : riwayatOrganisasiLainPegawai.getAlamat())
					.setParent(row);
			new Label(riwayatOrganisasiLainPegawai.getPimpinan()).setParent(row);
			new Label(riwayatOrganisasiLainPegawai.getPeriode()).setParent(row);

			new Image(riwayatOrganisasiLainPegawai.getStatus() ? "/img/svg/check2.svg" : "/img/svg/warning-outline.svg")
					.setParent(row);
			...
			button.addEventListener("onClick", new EventListener() {
				...
				public void onEvent(Event event) throws Exception {
					init(riwayatOrganisasiLainPegawai);
			...
			button.setVisible(!riwayatOrganisasiLainPegawai.getStatus());
			button.addEventListener("onClick", new EventListener() {
				@Override
				public void onEvent(Event event) throws Exception {
								...
								public void onEvent(Event event) throws Exception {
									...
									if (i == MyMessageboxConfig.OK) {
										try {
											...
											session.delete((riwayatOrganisasiLainPegawai));
	...
	public Borderlayout display() throws Exception {
		...
		toolbarbutton.addEventListener("onClick", new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				init(new RiwayatOrganisasiLainPegawai());
			}
		});
	...
	public void onSearchDefault(Event event) {
		...
		Session session = HibernateUtil.currentSession();
		List<RiwayatOrganisasiLainPegawai> riwayatOrganisasiLainPegawai = session
				.createCriteria(RiwayatOrganisasiLainPegawai.class)
		...
		ListModel strset = new SimpleListModel(riwayatOrganisasiLainPegawai);

		grid.setRowRenderer(new RiwayatOrganisasiLainPegawaiRenderer());
	...
	public void init(final RiwayatOrganisasiLainPegawai riwayatOrganisasiLainPegawai) throws Exception {
		...
		final MyWindow window = new MyWindow("Pendataan Riwayat Organisasi Lain", "none", true);
		window.setWidth("90%");
		window.setHeight("97%");
		...
		east.appendChild(new FotoLampiranPegawaiHelper(gridFotoGambar = new MyGrid())
				.initDetail(riwayatOrganisasiLainPegawai, RiwayatOrganisasiLainPegawai.class));
		...
		ambilDataPegawaiBanbox.setValue(riwayatOrganisasiLainPegawai.getPegawai() == null ? ""
				: riwayatOrganisasiLainPegawai.getPegawai().getNama());
		ambilDataPegawaiBanbox.setAttribute("pegawai", riwayatOrganisasiLainPegawai.getPegawai());
		...
		row.appendChild(nama = new Textbox(riwayatOrganisasiLainPegawai.getNama()));
		nama.setWidth("90%");
		nama.setRows(3);
		...
		row.appendChild(kedudukan = new Textbox(riwayatOrganisasiLainPegawai.getKedudukan()));
		kedudukan.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Tahun Mulai"));
		row.appendChild(tahunMulai = new Intbox(riwayatOrganisasiLainPegawai.getTahunMulai()));
		tahunMulai.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Tahun Sampai"));
		row.appendChild(tahunSelesai = new Intbox(riwayatOrganisasiLainPegawai.getTahunSelesai()));
		tahunSelesai.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Tempat"));
		row.appendChild(alamat = new Textbox(riwayatOrganisasiLainPegawai.getAlamat()));
		alamat.setWidth("90%");
		alamat.setRows(3);
		...
		row.appendChild(pimpinan = new Textbox(riwayatOrganisasiLainPegawai.getPimpinan()));
		pimpinan.setWidth("90%");

		if (riwayatOrganisasiLainPegawai.getStatus()) {
		...
		row.appendChild(periode = new Textbox(riwayatOrganisasiLainPegawai.getPeriode()));
		periode.setWidth("90%");
		...
		status.setChecked(riwayatOrganisasiLainPegawai.getStatus());
	...
	public boolean save(Event event) throws Exception {
		...
		if (tahunMulai.getValue() == null) {
			MyMessageboxConfig.show("Mohon maaf, Tahun Mulai belum diisi. Langkah yang dapat dilakukan: (1) isi kolom Tahun Mulai pada form; (2) pastikan nilai berupa tahun yang valid; (3) ulangi proses simpan. Jika masih mengalami kendala, hubungi Administrator atau tim teknis.", MyMessageboxConfig.INFORMATION, MyMessageboxConfig.OK,
					"");
			return false;
		}

		if (tahunSelesai.getValue() == null) {
			MyMessageboxConfig.show("Mohon maaf, Tahun Selesai belum diisi. Langkah yang dapat dilakukan: (1) isi kolom Tahun Selesai pada form; (2) pastikan nilai berupa tahun yang valid; (3) ulangi proses simpan. Jika masih mengalami kendala, hubungi Administrator atau tim teknis.", MyMessageboxConfig.INFORMATION, MyMessageboxConfig.OK,
					"");
			return false;
		}
		...
		Session session = HibernateUtil.currentSession();
		if (riwayatOrganisasiLainPegawai.getId() != null) {
			riwayatOrganisasiLainPegawai = (RiwayatOrganisasiLainPegawai) session
					.load(RiwayatOrganisasiLainPegawai.class, riwayatOrganisasiLainPegawai.getId());
		}

		riwayatOrganisasiLainPegawai.setPeriode(periode.getValue());
		riwayatOrganisasiLainPegawai.setStatus(status.isChecked());
		riwayatOrganisasiLainPegawai.setKedudukan(kedudukan.getValue());
		riwayatOrganisasiLainPegawai.setPimpinan(pimpinan.getValue());
		riwayatOrganisasiLainPegawai.setTahunSelesai(tahunSelesai.getValue());
		riwayatOrganisasiLainPegawai.setTahunMulai(tahunMulai.getValue());
		riwayatOrganisasiLainPegawai.setPegawai((Pegawai) ambilDataPegawaiBanbox.getAttribute("pegawai"));
		riwayatOrganisasiLainPegawai.setAlamat(alamat.getValue());
		riwayatOrganisasiLainPegawai.setNama(nama.getValue());

		if (riwayatOrganisasiLainPegawai.getId() != null) {
			session.update(riwayatOrganisasiLainPegawai);
		} else {
			session.save(riwayatOrganisasiLainPegawai);
		}
		...
		try {
			...
			for (Row row : rowsDocument) {
				FotoLampiranPegawai fotoLampiranPegawai = (FotoLampiranPegawai) row.getAttribute("fotoLampiranPegawai");
				fotoLampiranPegawai.setItem(riwayatOrganisasiLainPegawai.getId());
				fotoLampiranPegawai.setClazz(RiwayatOrganisasiLainPegawai.class.getName());
