...
/**
 * Controller/action ZK untuk insentif. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * paging}, {@code MyGrid grid}, {@code Combobox searchgolongan}, {@code Combobox searchperaturan}, {@code Intbox
 * searchmasakerja}, {@code Combobox golongan}, {@code Combobox masaKerja}; inisialisasi/lifecycle ({@code
 ...
 */
public class InsentifAction extends GenericAutowireComposer implements DataCriteria, DataSearchDefault {
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		String[] contents = new String[] { "id", "keterangan", "golongan", "peraturan", "masaKerja", "insentif",
				"tanggalEfektif" };
		MyToolbarbuttonConfig cetakToolbarbutton = Common.cetakData(Insentif.class, this, contents);
		Common.appendKeToolbar(cetakToolbarbutton, add, comp);

		MyToolbarbuttonConfig upload = Common.uploadData(this, Insentif.class, contents);
		if (upload != null) { upload.setVisible((add != null && add.isVisible()) && edit && delete); }
		Common.appendKeToolbar(upload, add, comp);
	}

	/**
	 * Renderer lokal untuk layar/komponen {@link InsentifAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link InsentifAction} dan dapat mengakses state
	...
	class InsentifRenderer extends ais.ui.util.MyRowRenderer {
		...
		public void render(final Row arg0, Object arg1) throws Exception {
			...
			new Label(insentif.getGolongan() == null ? "" : insentif.getGolongan().getNama()).setParent(arg0);

			Vbox a;
			(a = RevisiHelper.createNewRevisi(Insentif.class, insentif,
					insentif.getPeraturan() == null ? "-" : insentif.getPeraturan().getNama())).setParent(arg0);
			if (insentif.getPeraturan() != null) {
				Vbox myvbox = new Vbox();
				myvbox.setParent(a);

				Hbox hbox = new Hbox();
				hbox.setParent(myvbox);
				LampiranLain.createDownloadUploadFileLain(hbox, insentif.getPeraturan().getId(),
						Peraturan.class.getName(), "Peraturan Dokumen", false, null, null, false, false, false, false);
			}

			new Label(
					insentif.getMasaKerja() == null ? "0" : Common.numberFormat.get().format(insentif.getMasaKerja()) + " th")
					.setParent(arg0);

			new Label(insentif.getInsentif() == null ? "0" : Common.numberFormat.get().format(insentif.getInsentif()))
					.setParent(arg0);

			new Label(insentif.getTanggalEfektif() == null ? "0"
					: Common.dateFormat1.get().format(insentif.getTanggalEfektif())).setParent(arg0);
	...
	private void init(Insentif insentif) throws Exception {
		this.insentif = insentif;
		addWindow.setTitle(insentif.getId() == null ? "Tambah Intensif" : "Ubah Intensif");
		...
		Common.selectComboItem(golongan, insentif.getGolongan());
		...
		Common.selectComboItem(masaKerja, insentif.getMasaKerja() == null ? null : insentif.getMasaKerja());
		masaKerja.setWidth("90%");
		masaKerja.setReadonly(true);

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Intensif (INSENTIF) *"));
		row.appendChild(gaji = new MyDoublebox(insentif.getInsentif()));
		gaji.setWidth("90%");

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Tanggal Efektif *"));
		row.appendChild(tanggalEfektif = new MyDatebox(insentif.getTanggalEfektif()));

		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Keterangan"));
		row.appendChild(keterangan = new Textbox(insentif.getKeterangan()));
	...
	public boolean onSave(Event event) throws Exception {
		...
		if (gaji.getValue() == null) {
			MyMessageboxConfig.show("Mohon maaf, Intensif belum diisi. Langkah yang dapat dilakukan: (1) isi kolom Intensif pada form; (2) pastikan nilai berupa angka yang valid; (3) ulangi proses simpan. Jika masih mengalami kendala, hubungi Administrator atau tim teknis.", "Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION);
			return false;
		}

		Session session = HibernateUtil.currentSession();
		if (insentif.getId() != null) {
			insentif = (Insentif) session.load(Insentif.class, insentif.getId());

		}

		insentif.setGolongan(
				(Golongan) (golongan.getSelectedItem() == null ? null : golongan.getSelectedItem().getValue()));
		// insentif.setMasaKerja(masaKerja.getValue());

		insentif.setInsentif(gaji.getValue());
		insentif.setMasaKerja(
				(Integer) (masaKerja.getSelectedItem() == null ? null : masaKerja.getSelectedItem().getValue()));
		insentif.setKeterangan(keterangan.getValue());

		insentif.setTanggalEfektif(tanggalEfektif.getValue());

		if (insentif.getId() != null) {
			session.save(insentif);
		} else {
			Common.refreshUpdate(session, insentif);
		}
	...
	public Criteria initCriteria(boolean order) {
		Session session = HibernateUtil.currentSession();
		Criteria criteria = session.createCriteria(Insentif.class).createAlias("golongan", "golongan")
	...
	public void onSearchDefault(Event event) {
		...
		List<Insentif> insentif = initCriteria(true).setMaxResults(Common.ROWS_COUNT_ON_PAGE)
