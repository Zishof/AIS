...
/**
 * Controller/action ZK untuk nomor surat. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * {@code Longbox mulaiUrutanKe}, {@code Combobox kolom1}; inisialisasi/lifecycle ({@code doBeforeCompose()},
 * {@code doAfterCompose()}, {@code initKolom()}, {@code init()}, {@code initCriteria()}, {@code init()});
 * pembacaan/pencarian ({@code onSearchDefault()}); validasi/perhitungan ({@code checkNamaNomorSurat()}); mutasi
 * data ({@code onSave()}); operasi domain lain ({@code onVariableSurat()}, {@code onKelompokNomorSurat()},
...
public class NomorSuratAction extends GenericAutowireComposer
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link NomorSuratAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link NomorSuratAction} dan dapat mengakses state
