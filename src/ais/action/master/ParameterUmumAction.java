...
/**
 * Controller/action ZK untuk parameter umum. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * tabpanelsKonfigurasi}; inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code doAfterCompose()}, {@code
 * doAfterComposeOri()}); pembacaan/pencarian ({@code onSearchDefault()}); operasi domain lain ({@code
 * createSpan()}, {@code createSpan()}, {@code createRowActiveDefault()}, {@code createRowActiveDefault()},
 * {@code createRowActive()}, {@code createRowActive()}). Bagian lain dari kontrak tetap mengikuti kelas induk
