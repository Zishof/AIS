...
/**
 * Controller/action ZK untuk tindakan. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * searchketerangan}, {@code Combobox searchjenisTindakan}, {@code MyTextbox nama}; inisialisasi/lifecycle
 * ({@code doAfterCompose()}, {@code init()}, {@code initCriteria()}); pembacaan/pencarian ({@code
 * onUploadBiaya()}, {@code onDownloadBiaya()}, {@code onSearchDefault()}); mutasi data ({@code onSave()});
 * pelaporan/ekspor ({@code onCetakBiayaTindakan()}); operasi domain lain ({@code onAdd()}, {@code
