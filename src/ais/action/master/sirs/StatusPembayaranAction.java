...
import ais.database.model.sirs.StatusPembayaran;
...
public class StatusPembayaranAction extends GenericCrudAction<StatusPembayaran> {
    ...
    @Override
    protected Class<StatusPembayaran> getEntityClass() { return StatusPembayaran.class; }

    @Override
    protected StatusPembayaran createNewEntity() { return new StatusPembayaran(); }
    ...
    public Criteria initCriteria(boolean order) {
        ...
        Criteria criteria = session.createCriteria(StatusPembayaran.class);
    ...
    protected MyRowRenderer createRenderer() {
        return new StatusPembayaranRenderer();
    }
    ...
    protected void buildFormContent(MyWindow window, final StatusPembayaran statusPembayaran) throws Exception {
        ...
        nama = new Textbox(statusPembayaran.getNama() == null ? "" : statusPembayaran.getNama());
        nama.setWidth("100%");
        fb.addRow("Nama Status Pembayaran", nama);

        keterangan = new Textbox(statusPembayaran.getKeterangan() == null ? "" : statusPembayaran.getKeterangan());
    ...
    public boolean onSave(Event event) throws Exception {
        ...
        if (checkNamaStatusPembayaran()) {
            MyMessageboxConfig.show("Mohon maaf, Nama Status Pembayaran yang Bapak/Ibu masukkan sudah terdaftar sebelumnya. Langkah yang dapat dilakukan: (1) gunakan nama status pembayaran yang berbeda; (2) periksa kembali data yang telah ada melalui pencarian; (3) lakukan perubahan pada data yang sudah ada apabila diperlukan.", "Peringatan",
                    MyMessageboxConfig.OK, MyMessageboxConfig.INFORMATION);
            return false;
        }
        Session session = HibernateUtil.currentSession();
        StatusPembayaran entity = currentEntity;
        if (entity.getId() != null) {
            entity = (StatusPembayaran) session.load(StatusPembayaran.class, entity.getId());
    ...
    class StatusPembayaranRenderer extends MyRowRenderer {
        ...
        public void render(final Row arg0, Object arg1) throws Exception {
            arg0.setValign("top");
            final StatusPembayaran statusPembayaran = (StatusPembayaran) arg1;

            RevisiHelper.createNewRevisi(StatusPembayaran.class, statusPembayaran, statusPembayaran.getNama()).setParent(arg0);
            new Label(statusPembayaran.getKeterangan()).setParent(arg0);

            Common.copyEditDeleteButtons(edit, delete, statusPembayaran, StatusPembayaranAction.this).setParent(arg0);
        }
    }
}
