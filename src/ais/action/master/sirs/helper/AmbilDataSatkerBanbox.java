...
/**
 * Tipe khusus untuk ambil data satker banbox. Kelas ini memberi nama dan batas tanggung jawab yang
 ...
 * @see Bandbox
 */
public class AmbilDataSatkerBanbox extends Bandbox {
		...
		this.addEventListener(Events.ON_OK, new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				if (getValue().trim().equals("")) {
					setAttribute("satker", null);
					setValue("");
					return;
				}

				Satker satker = (Satker) HibernateUtil
						.currentSession()
						.createCriteria(Satker.class)
						.add(Restrictions.ilike("nama",
								AmbilDataSatkerBanbox.this.getValue().trim(),
								MatchMode.EXACT)).setMaxResults(1)
						.uniqueResult();
				if (satker == null) {
					MyMessageboxConfig.showFormat(
							"Mohon maaf, data Satker dengan nama \"{V1}\" tidak ditemukan di dalam sistem. Langkah yang dapat Bapak/Ibu lakukan: (1) periksa kembali ketepatan penulisan nama Satker yang dimasukkan; (2) pastikan data Satker tersebut telah terdaftar pada sistem; (3) gunakan fitur pencarian pada tabel untuk menelusuri data yang tersedia.",
							"Peringatan", MyMessageboxConfig.OK,
							MyMessageboxConfig.EXCLAMATION,
							AmbilDataSatkerBanbox.this.getValue().trim());
					return;
				}
				AmbilDataSatkerBanbox.this.setOpen(false);
				AmbilDataSatkerBanbox.this.setAttribute("satker", satker);
				AmbilDataSatkerBanbox.this.setValue(satker.getNama());
	...
	class SatkerRenderer extends ais.ui.util.MyRowRenderer {
		...
		public void render(Row arg0, Object arg1) throws Exception {
			...
			new Label(satker.getNama()).setParent(arg0);
	...
	public void display() {
		...
		row.setParent(rows);
		row.appendChild(new Label(ais.common.Common.getBahasaConfig("Nama Satker")));
	...
	public void onSearchDefault(Event event) {
		...
		System.out.println(satker);
		ListModel strset = new SimpleListModel(satker);
		grid.setRowRenderer(new SatkerRenderer());
