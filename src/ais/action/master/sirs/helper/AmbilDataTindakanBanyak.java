...
/**
 * Tipe khusus untuk ambil data tindakan banyak. Kelas ini memberi nama dan batas tanggung jawab
 ...
 * ais.ui.util.AmbilDataPagingHelper pagingHelper}, {@code EventListener eventListener}, {@code List tindakans},
 * {@code List tindakansHanyaDitampilkan}, {@code Set ids}, {@code Boolean tindakanLab}, {@code Boolean
 * tindakanOperasi}; pembacaan/pencarian ({@code getTindakanLab()}, {@code onSearchDefault()}, {@code
 * setEventListener()}, {@code getEventListener()}); mutasi data ({@code setTindakanLab()}); operasi domain lain
 ...
 * @see Window
 */
public class AmbilDataTindakanBanyak extends Window {
	...
	public AmbilDataTindakanBanyak(List<Tindakan> tindakans,
			Boolean tindakanLab, Boolean tindakanOperasi,
			Boolean tindakanRadiologi, Boolean tindakanVk,
			Boolean tindakanRenalUnit, Boolean tindakanGizi) {
		super();
	...
	public AmbilDataTindakanBanyak(List<Tindakan> tindakans,
			List<Tindakan> tindakansHanyaDitampilkan) {
		super();
		this.tindakans = tindakans;
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link AmbilDataTindakanBanyak}. Kelas ini menerjemahkan satu item data
	 * menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link AmbilDataTindakanBanyak} dan dapat mengakses
	...
	class TindakanRenderer extends ais.ui.util.MyRowRenderer {
		...
		public void render(Row arg0, Object arg1) throws Exception {
			...
			for (Tindakan myTindakan : tindakans) {
				if (myTindakan != null && myTindakan.getId() != null
						&& myTindakan.getId().equals(tindakan.getId())) {
			...
			new Label(tindakan.getJenisTindakan() == null ? "" : tindakan
					.getJenisTindakan().getNama()).setParent(arg0);
	...
	public void display() {
		...
		row.setStyle("border:0px;background: transparent;");
		row.setParent(rows);
		row.setVisible(tindakanLab == null && tindakanOperasi == null
				&& tindakanRadiologi == null && tindakanVk == null
				&& tindakanRenalUnit == null && tindakanGizi == null);
		row.appendChild(new Hbox(new Component[] {
				tindakanLabCheck = new Checkbox("Lab."),
				tindakanOperasiCheck = new Checkbox("Operasi"),
				tindakanRadiologiCheck = new Checkbox("Radiologi"),
				tindakanVkCheck = new Checkbox("VK"),
				tindakanRenalUnitCheck = new Checkbox("Renal Unit"),
		...
		cancel.addEventListener("onClick", new EventListener() {
			@Override
			public void onEvent(Event event) throws Exception {
				AmbilDataTindakanBanyak.this.detach();
			}
		});
				...
				if (eventListener != null && grid.getRows() != null
					...
					for (Row row : rows) {
						...
						if (checkbox.isChecked() && !checkbox.isDisabled()) {
							Tindakan myTindakan = (Tindakan) row
									.getAttribute("tindakan");
			...
			public void onEvent(Event event) throws Exception {
				...
				if (tindakanLab != null) {
					criterion = Restrictions.or(criterion,
							Restrictions.eq("tindakanLab", tindakanLab));
					adaOr = true;
				}
				if (tindakanGizi != null) {
					criterion = Restrictions.or(criterion,
							Restrictions.eq("tindakanGizi", tindakanGizi));
					adaOr = true;
				}
				if (tindakanOperasi != null) {
					criterion = Restrictions.or(criterion,
							Restrictions.eq("tindakanOperasi", tindakanOperasi));
					adaOr = true;
				}
				if (tindakanRadiologi != null) {
					criterion = Restrictions.or(criterion, Restrictions.eq(
							"tindakanRadiologi", tindakanRadiologi));
					adaOr = true;
				}
				if (tindakanRenalUnit != null) {
					criterion = Restrictions.or(criterion, Restrictions.eq(
							"tindakanRenalUnit", tindakanRenalUnit));
					adaOr = true;
				}
				if (tindakanVk != null) {
					criterion = Restrictions.or(criterion,
							Restrictions.eq("tindakanVk", tindakanVk));
					adaOr = true;
				}

				if (tindakanLabCheck.isChecked()) {
					criterion = Restrictions.or(criterion,
							Restrictions.eq("tindakanLab", true));
					adaOr = true;
				}
				if (tindakanGiziCheck.isChecked()) {
					criterion = Restrictions.or(criterion,
							Restrictions.eq("tindakanGizi", true));
					adaOr = true;
				}
				if (tindakanOperasiCheck.isChecked()) {
					criterion = Restrictions.or(criterion,
							Restrictions.eq("tindakanOperasi", true));
					adaOr = true;
				}
				if (tindakanRadiologiCheck.isChecked()) {
					criterion = Restrictions.or(criterion,
							Restrictions.eq("tindakanRadiologi", true));
					adaOr = true;
				}
				if (tindakanRenalUnitCheck.isChecked()) {
					criterion = Restrictions.or(criterion,
							Restrictions.eq("tindakanRenalUnit", true));
					adaOr = true;
				}
				if (tindakanVkCheck.isChecked()) {
					criterion = Restrictions.or(criterion,
							Restrictions.eq("tindakanVk", true));
					adaOr = true;
				}

				JenisTindakan jenisTindakan = (JenisTindakan) (AmbilDataTindakanBanyak.this.jenisTindakan
						.getSelectedItem() == null ? null
						: AmbilDataTindakanBanyak.this.jenisTindakan
								.getSelectedItem().getValue());
				List<Tindakan> myTindakan = session
						.createCriteria(Tindakan.class)
	...
	public void onSearchDefault(Event event) {
		...
		if (tindakanRadiologi != null) {
			criterion = Restrictions.or(criterion,
					Restrictions.eq("tindakanRadiologi", tindakanRadiologi));
			adaOr = true;
		}
		if (tindakanRenalUnit != null) {
			criterion = Restrictions.or(criterion,
					Restrictions.eq("tindakanRenalUnit", tindakanRenalUnit));
			adaOr = true;
		}
		if (tindakanVk != null) {
			criterion = Restrictions.or(criterion,
		...
		JenisTindakan jenisTindakan = (JenisTindakan) (this.jenisTindakan
				.getSelectedItem() == null ? null : this.jenisTindakan
				.getSelectedItem().getValue());
		...
		List<Tindakan> tindakan = session
				.createCriteria(Tindakan.class)
				.add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
				.add(adaOr ? criterion : Restrictions.sqlRestriction("1=1"))
				.add(Restrictions.eq("jenisPaket",
						Tindakan.JENIS_PERAWATAN_BUKAN_PAKET))
		...
		List<Tindakan> myTindakan = session
				...
				.add(tindakansHanyaDitampilkan == null || values.size() == 0 ? Restrictions
				...
				.add(Restrictions.ilike("nama", nama.getValue().trim(),
						MatchMode.ANYWHERE))
				.add(Restrictions.ilike("kode", kodeTindakanan.getValue()
