...
/**
 * Controller/action ZK untuk dusun. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * {@code Paging paging}, {@code MyTextbox searchnamadusun}, {@code Combobox kelurahan}, {@code MyTextbox nama},
 * {@code Toolbarbutton add}, {@code boolean edit}; inisialisasi/lifecycle ({@code doAfterCompose()}, {@code
 * init()}, {@code initCriteria()}); pembacaan/pencarian ({@code onSearchDefault()}); validasi/perhitungan
 * ({@code checkNamaDusun()}); mutasi data ({@code onSave()}); operasi domain lain ({@code onAdd()}). Bagian lain
...
public class DusunAction extends GenericAutowireComposer {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link DusunAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link DusunAction} dan dapat mengakses state kelas
