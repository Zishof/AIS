...
/**
 * Controller/action ZK untuk diagnosa penyakit detail. Tipe ini merupakan titik masuk UI yang
 ...
 * grid}, {@code Paging paging}, {@code Combobox apakahMenular}, {@code AmbilDataIcdBanbox diagnosaAwal1}, {@code
 * AmbilDataIcdBanbox diagnosaAwal2}, {@code AmbilDataIcdBanbox diagnosaAwal3}, {@code AmbilDataIcdBanbox
 * diagnosaAkhir1}; inisialisasi/lifecycle ({@code doAfterCompose()}, {@code init()}, {@code initCriteria()});
 * pembacaan/pencarian ({@code onSearchDefault()}); mutasi data ({@code onSave()}); penghapusan/pembatalan
 * ({@code onDelete()}); operasi domain lain ({@code onAdd()}, {@code createDiagnosa()}, {@code
 * createPoliDanDokter()}, {@code createCatatan()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau
...
public class DiagnosaPenyakitDetailAction extends GenericAutowireComposer implements OnSave {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link DiagnosaPenyakitDetailAction}. Kelas ini menerjemahkan satu item
	 * data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link DiagnosaPenyakitDetailAction} dan dapat
	 ...
	 *
	 * @see DiagnosaPenyakitDetailAction
	 */
