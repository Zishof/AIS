...
public class CalendarJadwalPolyComposer extends GenericForwardComposer {
	...
	private void init(final JadwalDokter jadwalDokter) throws Exception {
		...
		row.appendChild(new Label(jadwalDokter.getPoly() == null ? "" : jadwalDokter.getPoly().getNama()));
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		if (penjadwalanjamMulai.getNilai().equals(Konfigurasi.AKTIF)) {
			...
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/sirs/jadwal_dokter/CalendarJadwalPolyComposer.java:374");
			}
		...
		if (penjadwalanjamSelesai.getNilai().equals(Konfigurasi.AKTIF)) {
			...
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/sirs/jadwal_dokter/CalendarJadwalPolyComposer.java:382");
			}
	...
	public void onEventCreate$calendars(ForwardEvent event) throws Exception {
		...
		Poly myPoly = (Poly) (poly.getSelectedItem() == null ? null : poly.getSelectedItem().getValue());
		if (myPoly == null) {
			MyMessageboxConfig.show("Mohon maaf, Poli wajib dipilih terlebih dahulu. Langkah yang dapat dilakukan: (1) pilih Poli pada daftar yang tersedia; (2) pastikan pilihan tidak dikosongkan; (3) ulangi kembali setelah Poli ditentukan.", "Peringatan", MyMessageboxConfig.OK, MyMessageboxConfig.EXCLAMATION);
			return;
		}
	...
	public void onClick$cancelBtn$addWindow(ForwardEvent event) {
		...
		try {
			((CalendarsEvent) addWindow.getAttribute("calevent")).clearGhost();
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/sirs/jadwal_dokter/CalendarJadwalPolyComposer.java:488");
		}
		editWindow.setVisible(false);
		try {
			((CalendarsEvent) editWindow.getAttribute("calevent")).clearGhost();
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/sirs/jadwal_dokter/CalendarJadwalPolyComposer.java:493");
		}
	}
	...
	public void onClick$deleteBtn$editWindow(ForwardEvent event) {
		try {
			...
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/master/sirs/jadwal_dokter/CalendarJadwalPolyComposer.java:621");
		}
	}
