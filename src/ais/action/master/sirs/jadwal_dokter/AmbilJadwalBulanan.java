...
public class AmbilJadwalBulanan extends Window {
	...
	private void init() {
		...
		calendars.addEventListener("onEventEdit", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				if (ce != null) {
					...
					eventListener.onEvent(new Event("", calendars, new Object[] { jadwalDokter, dilayaniTanggal }));
					AmbilJadwalBulanan.this.detach();
				}
			}
		});
		...
		if (penjadwalanjamMulai.getNilai().equals(Konfigurasi.AKTIF)) {
			...
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/sirs/jadwal_dokter/AmbilJadwalBulanan.java:199");
			}
		...
		if (penjadwalanjamSelesai.getNilai().equals(Konfigurasi.AKTIF)) {
			...
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/sirs/jadwal_dokter/AmbilJadwalBulanan.java:207");
			}
