...
/**
 * Controller/action ZK untuk alat medis. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * MyTextbox searchketerangan}, {@code Combobox searchjenisAlatMedis}, {@code Combobox searchsatuan};
 ...
 * checkNamaAlatMedis()}); mutasi data ({@code onSave()}); pelaporan/ekspor ({@code onCetakBiayaAlatMedis()});
 * operasi domain lain ({@code onAdd()}, {@code generateCode()}). Bagian lain dari kontrak tetap mengikuti kelas
 ...
 */
public class AlatMedisAction extends GenericAutowireComposer implements OnSave {
	...
	public void onUploadBiaya(Event event) throws Exception {
		CommonAlatMedis.onUploadBiaya(event, null, AlatMedis.JENIS_UMUM);
	}
	...
	public void onCetakBiayaAlatMedis(Event event) throws Exception {
		...
		laporanBiayaAlatMedis.setTitle("Laporan Biaya Alat Medis / Perawatan / Jasa");
	...
	private void init(final AlatMedis alatMedis) throws Exception {
		...
		row.appendChild(kode = new Label(alatMedis.getKode() == null ? "" : alatMedis.getKode()));

		row.setStyle("border:0px;background: transparent;");
		row.setParent(rows);
		row.appendChild(new Label(ais.common.Common.getBahasaConfig("Nama ")));
		row.appendChild(nama = new MyTextbox(alatMedis.getNama() == null ? "" : alatMedis.getNama()));
		nama.setWidth("90%");
		...
		nama.addEventListener("onChange", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				if (mynama.trim().equals("")) {
					MyMessageboxConfig.show("Mohon maaf, nama wajib diisi terlebih dahulu. Langkah yang dapat dilakukan: (1) isikan nama pada kolom yang tersedia; (2) kemudian lanjutkan kembali proses Bapak/Ibu.", "Peringatan", MyMessageboxConfig.OK, MyMessageboxConfig.EXCLAMATION);
					nama.focus();
					return;
				}
		...
		Common.selectComboItem(jenisAlatMedis, alatMedis.getJenisAlatMedis());
	...
	public boolean onSave(Event event) throws Exception {
		if (nama.getValue().trim().equals("")) {
			MyMessageboxConfig.show("Mohon maaf, nama alat medis wajib diisi terlebih dahulu. Langkah yang dapat dilakukan: (1) isikan nama alat medis pada kolom yang tersedia; (2) kemudian simpan kembali data Bapak/Ibu.", "Peringatan", MyMessageboxConfig.OK, MyMessageboxConfig.EXCLAMATION);
			return false;
		}
		...
		if (checkNamaAlatMedis(nama.getValue().trim())) {
			MyMessageboxConfig.show("Mohon maaf, nama alat medis yang Bapak/Ibu masukkan sudah terdaftar sebelumnya. Langkah yang dapat dilakukan: (1) gunakan nama alat medis yang berbeda; (2) atau periksa kembali data alat medis yang telah tersedia.", "Peringatan", MyMessageboxConfig.OK, MyMessageboxConfig.EXCLAMATION);
			return false;
		}
		...
		alatMedis.setJenis(AlatMedis.JENIS_UMUM);
		...
		alatMedis.setJenisAlatMedis((JenisAlatMedis) (jenisAlatMedis.getSelectedItem() == null ? null
				: jenisAlatMedis.getSelectedItem().getValue()));
		alatMedis.setNama(nama.getValue());
		...
		if (alatMedis.getId() != null) {
			...
			generateCode(nama.getValue());
			alatMedis.setKode(kode.getValue());
	...
	private Criteria initCriteria(boolean order) {
		...
		Criteria criteria = session.createCriteria(AlatMedis.class)
				...
				.add(Restrictions.or(Restrictions.isNull("jenis"), Restrictions.eq("jenis", AlatMedis.JENIS_UMUM)))
	...
	public void onSearchDefault(Event event) {
		...
		List<AlatMedis> alatMedis = ConstantValues
				.simpleList(
