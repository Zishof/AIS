...
/**
 * Controller/action ZK untuk transaksi retur. Tipe ini merupakan titik masuk UI yang menghubungkan
 ...
 * searchnama}, {@code Combobox searchbagian}, {@code Combobox searchkelas}; inisialisasi/lifecycle ({@code
 * doAfterCompose()}, {@code init()}, {@code initCriteria()}); pembacaan/pencarian ({@code onSearchDefault()});
 * validasi/perhitungan ({@code checkKodeTransaksi()}); mutasi data ({@code onSave()}); penghapusan/pembatalan
 * ({@code onDelete()}); pelaporan/ekspor ({@code onCetak()}); operasi domain lain ({@code onAdd()}, {@code
 * createMain()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di atas.</p>
...
public class TransaksiReturAction extends GenericAutowireComposer {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link TransaksiReturAction}. Kelas ini menerjemahkan satu item data
	 * menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link TransaksiReturAction} dan dapat mengakses
	...
	/**
	 * Tipe implementasi bersarang {@link TransaksiDetailAction} milik {@link TransaksiReturAction}. Kelas ini
	 * memberi nama pada state atau perilaku lokal agar tanggung jawabnya tidak tersebar sebagai blok anonim.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link TransaksiReturAction} dan dapat mengakses
	 * state kelas induk. Jangan menyimpan atau membagikannya lintas desktop/session.</p>
	 * <p>Kontrak yang tampak dari deklarasi ini meliputi state utama: {@code Grid grid}, {@code Footer total},
	 * {@code Footer totalRtr}; operasi lokal: {@code loadData()}, {@code loadTotal()}, {@code display}(). Aturan
