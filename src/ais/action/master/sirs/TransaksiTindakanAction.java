...
/**
 * Controller/action ZK untuk transaksi tindakan. Tipe ini merupakan titik masuk UI yang
 ...
 */
public class TransaksiTindakanAction extends GenericAutowireComposer implements GetTransaksi, TransaksiListener {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link TransaksiTindakanAction}. Kelas ini menerjemahkan satu item data
	 * menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link TransaksiTindakanAction} dan dapat mengakses
	...
	private TransaksiLayananDetailHelper transaksiDetailAction;
		...
		if (transaksi != null && transaksi.getId() != null) {
			...
			if (!pakets.isEmpty()) {
				...
				CommonPendaftaranUtil.transaksiDetailPaket(eastInfoPasien, pakets, transaksi.getPendaftaran(),
						PaketPerawatanDetail.PAKET_TINDAKAN);
				add.setDisabled(true);
				simpan.setDisabled(false);
			} else {
				Common.clear(eastInfoPasien);
				transaksiDetailAction = new TransaksiLayananDetailHelper(TransaksiTindakanAction.this);
								...
								if (i == MyMessageboxConfig.OK) {
									...
									if (onSave(event)) {
										...
										if (pakets != null && !pakets.isEmpty()) {
											...
											CommonPendaftaranUtil.validasiTransaksiLayanan(transaksi, SUMBER, true);
										}
	...
	public boolean onSave(Event event) throws Exception {
		...
		transaksi.setJenisTransaksi(TransaksiMedis.TRX_TINDAKAN);
	...
	private Criteria initCriteria(boolean order) {
		...
		criteria.add(Restrictions.eq("sumber", SUMBER)).add(Restrictions.eq("jenisTransaksi", TransaksiMedis.TRX_TINDAKAN))
	...
	public void onBebas(Boolean checked) throws Exception {
		...
		Common.clear(eastInfoPasien);
		if (checked) {
			transaksiDetailAction = new TransaksiLayananDetailHelper(this);
			eastInfoPasien.appendChild(transaksiDetailAction);
		}
	}
	...
	public void onBerubah(Boolean bebas, Pendaftaran pendaftaran, Pasien pasien, String nama, Date tanggalTransaksi,
		...
		final TransaksiMedis mytransaksi = (TransaksiMedis) HibernateUtil.currentSession().createCriteria(TransaksiMedis.class)
				.add(Restrictions.eq("sumber", SUMBER)).add(Restrictions.eq("jenisTransaksi", TransaksiMedis.TRX_TINDAKAN))
