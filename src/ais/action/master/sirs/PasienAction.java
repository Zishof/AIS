...
/**
 * Controller/action ZK untuk pasien. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * window}, {@code Tabpanel tambahData}, {@code Grid grid}, {@code Paging paging}, {@code MyTextbox searchkode},
 * {@code MyTextbox searchnama}, {@code MyTextbox searchalamat}; inisialisasi/lifecycle ({@code
 * doAfterCompose()}, {@code initComponent()}, {@code init()}, {@code initCriteria()}); pembacaan/pencarian
 * ({@code onSearchDefault()}); validasi/perhitungan ({@code checkKodePasien()}); mutasi data ({@code onSave()});
 * penghapusan/pembatalan ({@code onDelete()}); operasi domain lain ({@code onAdd()}, {@code onExternalAdd()},
 * {@code createKode()}, {@code createDinas()}, {@code createBiodata()}, {@code generateCode()}). Bagian lain
...
public class PasienAction extends GenericAutowireComposer {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link PasienAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link PasienAction} dan dapat mengakses state kelas
