...
/**
 * Controller/action ZK untuk transaksi alat medis. Tipe ini merupakan titik masuk UI yang
 ...
 */
public class TransaksiAlatMedisAction extends GenericAutowireComposer implements GetTransaksi, TransaksiListener {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link TransaksiAlatMedisAction}. Kelas ini menerjemahkan satu item data
	 * menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link TransaksiAlatMedisAction} dan dapat mengakses
	...
	class TransaksiRenderer extends ais.ui.util.MyRowRenderer {
		...
		public void render(final Row arg0, Object arg1) throws Exception {
			...
			new ais.action.master.sirs.detail.TransaksiAlatMedisDetailAction(transaksi).setParent(arg0);
	...
	private TransaksiAlatMedisDetailHelper transaksiDetailAction;
		...
		if (transaksi != null && transaksi.getId() != null) {
			...
			if (!pakets.isEmpty()) {
				...
				CommonPendaftaranUtil.transaksiDetailPaket(eastInfoPasien, pakets, transaksi.getPendaftaran(),
						PaketPerawatanDetail.PAKET_ALAT_MEDIS);
				add.setDisabled(true);
				simpan.setDisabled(false);
			} else {
				Common.clear(eastInfoPasien);
				transaksiDetailAction = new TransaksiAlatMedisDetailHelper(TransaksiAlatMedisAction.this);
								...
								if (i == MyMessageboxConfig.OK) {
									...
									if (onSave(event)) {
										...
										if (pakets != null && !pakets.isEmpty()) {
											...
											CommonPendaftaranUtil.validasiTransaksiAlatMedis(transaksi, SUMBER, true);
										}
	...
	public void onCetak(TransaksiMedis transaksi) throws Exception {
		if (pakets == null || pakets.isEmpty()) {
			...
			Report.generateWindowReport(Report.PDF, parameters, "sirs/transaksi_alat_medis",
	...
	public boolean onSave(Event event) throws Exception {
		...
		transaksi.setJenisTransaksi(TransaksiMedis.TRX_ALAT_MEDIS);
	...
	private Criteria initCriteria(boolean order) {
		...
		criteria.add(Restrictions.eq("sumber", SUMBER)).add(Restrictions.eq("jenisTransaksi", TransaksiMedis.TRX_ALAT_MEDIS))
	...
	public void onBebas(Boolean checked) throws Exception {
		...
		Common.clear(eastInfoPasien);
		if (checked) {
			transaksiDetailAction = new TransaksiAlatMedisDetailHelper(this);
			eastInfoPasien.appendChild(transaksiDetailAction);
		}
	}
	...
	public void onBerubah(Boolean bebas, Pendaftaran pendaftaran, Pasien pasien, String nama, Date tanggalTransaksi,
		...
		final Pendaftaran myPendaftaran = pendaftaran;
		...
		final TransaksiMedis mytransaksi = (TransaksiMedis) HibernateUtil.currentSession().createCriteria(TransaksiMedis.class)
				.add(Restrictions.eq("sumber", SUMBER)).add(Restrictions.eq("jenisTransaksi", TransaksiMedis.TRX_ALAT_MEDIS))
		...
		if (mytransaksi != null && transaksi.getId() == null) {
			...
			if (transaksiDetailAction != null) {
				...
				timer.addEventListener("onTimer", new EventListener() {
					...
					@Override
					public void onEvent(Event arg0) throws Exception {
						transaksiDetailAction.loadData(myPendaftaran);
						timer.detach();
					}
				});
				timer.start();
