...
/**
 * Controller/action ZK untuk kelurahan. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * {@code Paging paging}, {@code MyTextbox searchnamakelurahan}, {@code Combobox kecamatan}, {@code MyTextbox
 * nama}, {@code Toolbarbutton add}, {@code boolean edit}; inisialisasi/lifecycle ({@code doAfterCompose()},
 * {@code init()}, {@code initCriteria()}); pembacaan/pencarian ({@code onSearchDefault()}); validasi/perhitungan
 * ({@code checkNamaKelurahan()}); mutasi data ({@code onSave()}); operasi domain lain ({@code onAdd()}). Bagian
...
public class KelurahanAction extends GenericAutowireComposer {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link KelurahanAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link KelurahanAction} dan dapat mengakses state
