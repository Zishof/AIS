...
public class JenisPembayaranAction extends GenericCrudAction<JenisPembayaranMedis> {
    ...
    public Criteria initCriteria(boolean order) {
        ...
        Criteria criteria = session.createCriteria(JenisPembayaranMedis.class);
    ...
    protected void buildFormContent(MyWindow window, final JenisPembayaranMedis jenisPembayaran) throws Exception {
        ...
        nama = new Textbox(jenisPembayaran.getNama() == null ? "" : jenisPembayaran.getNama());
        nama.setWidth("100%");
        fb.addRow("Nama Jenis Pembayaran", nama);

        keterangan = new Textbox(jenisPembayaran.getKeterangan() == null ? "" : jenisPembayaran.getKeterangan());
    ...
    public boolean onSave(Event event) throws Exception {
        ...
        if (checkNamaJenisPembayaran()) {
            MyMessageboxConfig.show("Mohon maaf, Nama Jenis Pembayaran yang Bapak/Ibu masukkan sudah terdaftar sebelumnya. Langkah yang dapat dilakukan: (1) gunakan nama jenis pembayaran yang berbeda; (2) periksa kembali data yang telah ada melalui pencarian; (3) lakukan perubahan pada data yang sudah ada apabila diperlukan.", "Peringatan",
                    MyMessageboxConfig.OK, MyMessageboxConfig.INFORMATION);
            return false;
        }
        Session session = HibernateUtil.currentSession();
        JenisPembayaranMedis entity = currentEntity;
        if (entity.getId() != null) {
            entity = (JenisPembayaranMedis) session.load(JenisPembayaranMedis.class, entity.getId());
    ...
    class JenisPembayaranRenderer extends MyRowRenderer {
        ...
        public void render(final Row arg0, Object arg1) throws Exception {
            arg0.setValign("top");
            final JenisPembayaranMedis jenisPembayaran = (JenisPembayaranMedis) arg1;

            RevisiHelper.createNewRevisi(JenisPembayaranMedis.class, jenisPembayaran,
                    jenisPembayaran.getNama()).setParent(arg0);
            new Label(jenisPembayaran.getKeterangan()).setParent(arg0);

            Common.copyEditDeleteButtons(edit, delete, jenisPembayaran, JenisPembayaranAction.this).setParent(arg0);
        }
    }
}
