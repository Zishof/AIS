...
/**
 * Controller/action ZK untuk pindah tempat tidur rawat inap. Tipe ini merupakan titik masuk UI
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code Window addWindow}, {@code Tabpanel
 * tambahData}, {@code Grid grid}, {@code Paging paging}, {@code MyTextbox searchkode}, {@code MyTextbox
 * searchnama}, {@code MyTextbox searchalamat}, {@code boolean edit}; inisialisasi/lifecycle ({@code
 * doAfterCompose()}, {@code initComponent()}, {@code init()}, {@code initCriteria()}); pembacaan/pencarian
 * ({@code onSearchDefault()}); mutasi data ({@code onSave()}); penghapusan/pembatalan ({@code onDelete()});
 * operasi domain lain ({@code onAdd()}, {@code onExternalAdd()}, {@code createRuangPerawatan()}, {@code
 * createPindahDari()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di
 * atas.</p>
...
public class PindahTempatTidurRawatInapAction extends GenericAutowireComposer {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link PindahTempatTidurRawatInapAction}. Kelas ini menerjemahkan satu
	 * item data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link PindahTempatTidurRawatInapAction} dan dapat
	 ...
	 *
	 * @see PindahTempatTidurRawatInapAction
	 */
