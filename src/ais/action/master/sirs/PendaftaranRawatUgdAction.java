...
/**
 * Controller/action ZK untuk pendaftaran rawat ugd. Tipe ini merupakan titik masuk UI yang
 ...
 * searchdokter}, {@code MyTextbox searchmr}, {@code MyTextbox searchtelp}; inisialisasi/lifecycle ({@code
 * doAfterCompose()}, {@code initUgd()}, {@code init()}, {@code initCriteria()}); pembacaan/pencarian ({@code
 * onSearchDefault()}, {@code getTransaksi()}, {@code getLokasi()}, {@code getAdd()}, {@code getSimpan()}, {@code
 * getKelasPerawatan()}); mutasi data ({@code onSave()}, {@code onSaveTransaksi()}); penghapusan/pembatalan
 * ({@code onDelete()}); operasi domain lain ({@code onAdd()}). Bagian lain dari kontrak tetap mengikuti kelas
...
public class PendaftaranRawatUgdAction extends GenericAutowireComposer implements GetTransaksi {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link PendaftaranRawatUgdAction}. Kelas ini menerjemahkan satu item
	 * data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link PendaftaranRawatUgdAction} dan dapat mengakses
