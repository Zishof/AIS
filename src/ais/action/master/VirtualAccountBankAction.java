...
/**
 * Controller/action ZK untuk virtual account bank. Tipe ini merupakan titik masuk UI yang
 ...
 * {@code Textbox mahasiswa}, {@code Textbox kode}, {@code Textbox keterangan}, {@code Textbox bank}, {@code
 * Textbox jenis}, {@code MyDatebox start}; inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code
 * buatTombolCekUlang()}, {@code buatStatusPembayaran()}, {@code doAfterCompose()}, {@code initCriteria()});
 * pembacaan/pencarian ({@code refreshSetelahCekPembayaran()}, {@code tampilkanHasilCekPembayaran()}, {@code
 * tampilkanErrorCekPembayaran()}, {@code resolveWaktuBayarUntukTampilan()}, {@code
 * resolveAkunPembayaranSiswaUntukTampilan()}, {@code onSearchDefault()}); validasi/perhitungan ({@code
 * cekUlangSatuVaMasal()}, {@code bolehMengoreksiPembayaranVa()}); mutasi data ({@code updateProgress()});
 * penghapusan/pembatalan ({@code batalkanPembayaranVaSalah()}); pelaporan/ekspor ({@code
 * renderDashboardVirtualAccount()}); operasi domain lain ({@code escapeHtml()}, {@code userAccessRestriction()},
 * {@code buildDashboardSummary()}, {@code buildFilterDescription()}, {@code appendFilterText()}, {@code
 * countData()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di atas.</p>
