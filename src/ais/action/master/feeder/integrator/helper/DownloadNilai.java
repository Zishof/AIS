...
/**
 * Tipe khusus untuk download nilai. Kelas ini memberi nama dan batas tanggung jawab yang eksplisit
 ...
 * searchprogram}, {@code AmbilDataMasaPerkuliahanBanbox searchmasaperkulaiahan}, {@code Combobox
 * searchTahunAjaran}, {@code Combobox searchJenisSemester}; inisialisasi/lifecycle ({@code init()}, {@code
 * initSpreadsheet()}); operasi domain lain ({@code createData()}); konfigurasi constructor: {@code comboitem}.
 ...
 * @see MyWindow
 */
public class DownloadNilai extends MyWindow {
	...
	public DownloadNilai(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		print.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/feeder/integrator/helper/DownloadNilai.java:327");
		...
		telahDinilai = new MyCheckboxConfig("Telah dinilai");
		telahDinilai.setParent(toolbar);
		belumDinilai = new MyCheckboxConfig("Belum dinilai");
		belumDinilai.setParent(toolbar);

		belumDinilai.addEventListener("onClick", new EventListener() {
		...
		telahDinilai.addEventListener("onClick", new EventListener() {
	...
	private void initSpreadsheet() throws Exception {
		...
		saringan.semesterKe = semester;
		saringan.tahunAjaran = (String) (searchTahunAjaran.getSelectedItem() == null ? null : searchTahunAjaran.getSelectedItem().getValue());
		saringan.jenisSemester = (String) (searchJenisSemester.getSelectedItem() == null ? null : searchJenisSemester.getSelectedItem().getValue());
		saringan.kodeMatakuliah = kodeMatakuliah.getValue();
		saringan.telahDinilai = telahDinilai.isChecked();
		saringan.belumDinilai = belumDinilai.isChecked();
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					// Susunan berkas milik EksporNilaiFeeder; layar ini hanya
					...
					int jumlah = ais.action.master.feeder.integrator.ekspor.EksporNilaiFeeder.tulis(
							file, saringan,
					...
					label.setValue("Error: " + ais.common.PesanFormalHelper.pesanGagalException(
							"penyiapan berkas ekspor Nilai", null, e,
							new String[] {
