...
/**
 * Tipe khusus untuk download aktifitas mahasiwa pkl peserta mahasiswa. Kelas ini memberi nama dan
 ...
 * @see MyWindow
 */
public class DownloadAktifitasMahasiwaPklPesertaMahasiswa extends MyWindow {
	...
	public DownloadAktifitasMahasiwaPklPesertaMahasiswa() {
		super();
		try {
	...
	public DownloadAktifitasMahasiwaPklPesertaMahasiswa(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		print.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/feeder/integrator/helper/DownloadAktifitasMahasiwaPklPesertaMahasiswa.java:188");
			...
			public void run() {
				...
				List<MahasiswaDapatKelompokPkl> kelompokPkls = ConstantValues.simpleList(
						session.createCriteria(MahasiswaDapatKelompokPkl.class)
								...
								.add(kel != null && !kel.trim().isEmpty()
										? Restrictions.ilike("kelompokPkl.nama_kelompok", kel.trim(), MatchMode.EXACT)
				...
				label.setValue("");
					...
					label.setValue("Error: " + ais.common.PesanFormalHelper.pesanGagalException(
							"pengambilan data Aktivitas Mahasiswa PKL (Peserta Mahasiswa) untuk Neo Feeder",
							null, e,
							new String[] {
									"Periksa kembali koneksi ke server Neo Feeder (Pengaturan Koneksi) dan coba ulangi.",
									"Pastikan data Mahasiswa peserta Kelompok PKL terkait sudah lengkap dan benar.",
