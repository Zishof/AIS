...
/**
 * Tipe khusus untuk download aktifitas mahasiwa pkl peserta dosen. Kelas ini memberi nama dan
 ...
 * @see MyWindow
 */
public class DownloadAktifitasMahasiwaPklPesertaDosen extends MyWindow {
	...
	public DownloadAktifitasMahasiwaPklPesertaDosen() {
		super();
		try {
	...
	public DownloadAktifitasMahasiwaPklPesertaDosen(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		print.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/master/feeder/integrator/helper/DownloadAktifitasMahasiwaPklPesertaDosen.java:187");
			...
			public void run() {
				...
				for (KelompokPkl kelompokPkl : kelompokPkls) {
					...
					for (Dosen dosen : kelompokPkl.populateDosenBuNama()) {
						...
						cell.setCellValue(kelompokPkl.getPkl().getJurusan() == null ? ""
				...
				label.setValue("");
					...
					label.setValue("Error: " + ais.common.PesanFormalHelper.pesanGagalException(
							"pengambilan data Aktivitas Mahasiswa PKL (Peserta Dosen) untuk Neo Feeder",
							null, e,
							new String[] {
									"Periksa kembali koneksi ke server Neo Feeder (Pengaturan Koneksi) dan coba ulangi.",
									"Pastikan data Dosen Pembimbing PKL pada Kelompok PKL terkait sudah lengkap dan benar.",
