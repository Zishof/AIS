...
/**
 * Controller/action ZK untuk permintaan pengadaan master asset. Tipe ini merupakan titik masuk UI
 ...
 * grid}, {@code Paging paging}, {@code Textbox searchkode}, {@code Textbox searchketerangan}, {@code Textbox
 * searchanggaran}, {@code Combobox searchlokasi}, {@code Checkbox searchaktif}; inisialisasi/lifecycle ({@code
 * doBeforeCompose()}, {@code doAfterCompose()}, {@code init()}, {@code initCriteria()}); pembacaan/pencarian
 * ({@code onSearchDefault()}, {@code ambil()}, {@code ambilClass()}, {@code getindex()}); mutasi data ({@code
 * onSave()}, {@code buildFilterPersetujuan()}, {@code setPersetujuan()}); pelaporan/ekspor ({@code cetakData()},
 * {@code cetak()}); operasi domain lain ({@code onStatistik()}, {@code onRiwayatHargaBarangJasa()}, {@code
 * onTraceStatusBarangJasa()}, {@code onAddExternal()}, {@code onAdd()}, {@code parameter()}); konfigurasi
