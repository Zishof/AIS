...
/**
 * Controller/action ZK untuk pembayaran calon mahasiswa. Tipe ini merupakan titik masuk UI yang
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code Combobox akun}, {@code MyDatebox
 * tanggalValidasi}, {@code Row rowNim}, {@code Row rowNama}, {@code Row rowKewarganegaraan}, {@code Row
 * rowJenisKuliah}, {@code Row rowProdi}, {@code Row rowTahunMasuk}; inisialisasi/lifecycle ({@code
 * doBeforeCompose()}, {@code doAfterCompose()}); pembacaan/pencarian ({@code onCariBiodataCalonMahasiswa()},
 * {@code listBiaya()}, {@code loadData()}); validasi/perhitungan ({@code hitungJumlahBiayaSeharusnya()}); mutasi
 * data ({@code reset()}, {@code setVisibleSafe()}, {@code onSave()}); operasi domain lain ({@code
 * autoGenerateNoUjianJikaBelumAda()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang
 * disebut di atas.</p>
...
public class PembayaranCalonMahasiswaAction extends GenericAutowireComposer {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link PembayaranCalonMahasiswaAction}. Kelas ini menerjemahkan satu
	 * item data menjadi baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link PembayaranCalonMahasiswaAction} dan dapat
	 ...
	 *
	 * @see PembayaranCalonMahasiswaAction
	 */
