...
/**
 * Penyusun/penyaji laporan untuk laporan inventaris. Kelas ini mengubah data domain menjadi bentuk
 ...
 * lokasi}, {@code AmbilDataRuangBanbox ruang}, {@code MyDatebox mulai}, {@code MyDatebox sampai}, {@code
 * Combobox status}, {@code Combobox jenisAsset}, {@code Combobox kelompokAsset}; inisialisasi/lifecycle ({@code
 ...
 * @see MyWindow
 */
public class LaporanInventaris extends MyWindow {
	...
	public LaporanInventaris() {
		super();
		try {
			init();
		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Inventaris", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	private void init() throws Exception {
		...
		Common.insertComboDanSemua(status, new String[] { "nama" }, StatusAsset.class);
		status.setWidth("90%");
		...
		}, "asset/inventaris", null, new EventListener() {
	...
	private Map generateParameter() throws Exception {
		...
		StatusAsset myStatusAsset = (StatusAsset) (status.getSelectedItem() == null
		...
		parameters.put("status", myStatusAsset == null || myStatusAsset.getId() == null ? -1L : myStatusAsset.getId());
	...
	public void onReport(Event event) {
		...
		try {
			...
			File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(), "asset/inventaris",
					ais.ui.util.WaktuUtil.getDate(), toolbar);
			CommonReport.tampilkanReportPDF(center, file);

		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Inventaris", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
					new String[] {
