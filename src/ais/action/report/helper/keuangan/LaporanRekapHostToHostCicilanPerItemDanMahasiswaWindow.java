...
public class LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow extends MyWindow {
	...
	public LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow() {
		super();
		try {
			init();
		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Rekap Host To Host Cicilan Per Item Dan Mahasiswa Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow(String title, String border, boolean closable) {
	...
	private void init() {
		...
		row.appendChild(searchmhs = new MyTextbox());
		searchmhs.setWidth("90%");
		...
		try {
			...
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/helper/keuangan/LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.java:387");
			PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Host To Host Cicilan Per Item Dan Mahasiswa Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
				new String[] {
	...
	private void initSpreadsheet() throws Exception {
		...
		if (angkatan.getValue() == null && searchmhs.getValue().trim().isEmpty()) {
			MyMessageboxConfig.show("Pilihan tahun angkatan atau mahasiswa harus diisi", "Peringatan",
					MyMessageboxConfig.OK, MyMessageboxConfig.INFORMATION);
			return;
		}

		final String mhs = searchmhs.getValue().trim();
		...
		final JenisKegiatan jenisPembayaran = (JenisKegiatan) (LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.this.jenisPembayaran
				.getSelectedItem() == null ? null
						: LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.this.jenisPembayaran.getSelectedItem()
								.getValue());
		final JenisSeleksi jenisSeleksi = (JenisSeleksi) (LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.this.jenisSeleksi
				.getSelectedItem() == null ? null
						: LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.this.jenisSeleksi.getSelectedItem()
								.getValue());

		final String tahunAkademik = (String) (LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.this.tahunAkademik
				.getSelectedItem() == null ? null
						: LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.this.tahunAkademik.getSelectedItem()
								.getValue());
		final String semester = (String) (LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.this.semesterAbsensi
				.getSelectedItem() == null ? null
						: LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.this.semesterAbsensi.getSelectedItem()
								.getValue());
		...
		final Jenjang jenjang = (Jenjang) (LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.this.jenjang
				.getSelectedItem() == null ? null
						: LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.this.jenjang.getSelectedItem()
								.getValue());

		final Integer smt = (Integer) (LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.this.semester
				.getValue() == null
				|| LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.this.semester.getSelectedItem() == null ? null
						: LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.this.semester.getSelectedItem()
								.getValue());

		final TreeMap<String, TreeMap<String, Object[]>> jurusans = new TreeMap<String, TreeMap<String, Object[]>>();

		final TreeMap<String, Set<Long>> items = new TreeMap<String, Set<Long>>();
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				try {
					...
					for (Map<String, Object[]> objectsMap : jurusans.values()) {
						try {
							...
							Object[] a = objectsMap.values().iterator().next();

							GeneralValueObject generalValueObject = (GeneralValueObject) a[0];
							...
							String key = (mahasiswa != null ? mahasiswa.getId() + "_mhs"
									: biodataCalonMahasiswa != null ? biodataCalonMahasiswa.getId() + "_calon_mhs"
											: "");
							Set<Long> itemBiayas = items.get(key);

							if (itemBiayas.size() > lebarCol) {
								lebarCol = itemBiayas.size();
							}

							int indexCol = 4;
							for (Long itemBiayaId : itemBiayas) {
								ItemBiaya itemBiaya = (ItemBiaya) ConstantValues.ambil(ItemBiaya.class.getName(),
										itemBiayaId);
							...
							for (String keySmt : objectsMap.keySet()) {

								row = sheet.createRow(rowIndex);

								Object[] objects = objectsMap.get(keySmt);
								Map<Long, Double> dataTotal = (Map<Long, Double>) objects[1];

								cell = row.createCell(4);
								cell.setCellStyle(hlink_style);
								cell.setCellValue(keySmt);

								indexCol = 4;
								Double total = 0.0;
								for (Long itemBiayaId : itemBiayas) {

									Double t = dataTotal.get(itemBiayaId);
									if (t == null) {
										t = 0.0;
									}
									cell = row.createCell(++indexCol);
							...
							e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/helper/keuangan/LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.java:736");
							PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Host To Host Cicilan Per Item Dan Mahasiswa Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
								new String[] {
									"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
									"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
					...
					spreadsheet.setMaxcolumns(6 + lebarCol);
					spreadsheet.setMaxrows(jurusans.size() + 25);
					for (int i = 0; i < spreadsheet.getMaxcolumns(); i++) {
						try {
							sheet.autoSizeColumn(i);
						} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/helper/keuangan/LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.java:775");
					...
					e4.printStackTrace(); ais.common.ErrorAuditUtil.record(e4, "auto-audit src/ais/action/report/helper/keuangan/LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.java:782");
				}
			}
		});
			...
			public void run() {
				...
				try {
					...
					try {
						...
						Criteria criteria = session1.createCriteria(CicilanPembayaran.class)
						...
						if ((tahunAngkatan != null && tahunAngkatan >= 1900) || !mhs.trim().isEmpty()) {
							criteria.createAlias("kegiatan", "kegiatan")
									...
									.add(tahunAngkatan == null || tahunAngkatan < 1900
											? Restrictions.sqlRestriction("true")
											: Restrictions.or(Restrictions.eq("mahasiswa.tahunangkatan", tahunAngkatan),
													Restrictions.eq("calonMahasiswa.tahun", tahunAngkatan)))

									.add(mhs.trim().isEmpty() ? Restrictions.sqlRestriction("true") :

											Restrictions.or(
													Restrictions.ilike("calonMahasiswa.noUjian", mhs.trim(),
															MatchMode.ANYWHERE),
													Restrictions.or(
															Restrictions.ilike("calonMahasiswa.noRegistrasi",
																	mhs.trim(), MatchMode.ANYWHERE),
															Restrictions.or(
																	Restrictions.ilike("mahasiswa.nim", mhs.trim(),
																			MatchMode.ANYWHERE),
																	Restrictions.or(
						...
						e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/helper/keuangan/LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.java:857");
						PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Host To Host Cicilan Per Item Dan Mahasiswa Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
							new String[] {
								"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
								"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
					...
					for (Long cicilanPembayaranId : cicilanPembayarans) {
						try {
											...
											if (jenisPembayaran == null || jenisPembayaran.getId()
												...
												if (program == null
														|| (biodataCalonMahasiswa != null
																...
																&& program.equals(biodataCalonMahasiswa.getProgram()))
													...
													if (statusAwal == null
															...
															|| (mahasiswa != null
																	...
																	&& statusAwal.getId().equals(
																			mahasiswa.getStatusAwalMahasiswa().getId()))
														...
														if (jenisSeleksi == null
															...
															if (fakultas == null || fakultas.getId().equals(-1L)
																...
																if (jurusan == null || jurusan.getId().equals(-1L)
																	...
																	if (jenjang == null || jenjang.getId().equals(-1L)

																			|| (mahasiswa != null
																					&& mahasiswa.getJurusan() != null
																					&& mahasiswa.getJurusan()
																							.getJenjang().getId()
																							.equals(jenjang.getId()))
																		...
																		String key = (mahasiswa != null
																				? kegiatan.getMahasiswa().getId()
																						+ "_mhs"
																				: biodataCalonMahasiswa != null
																						? kegiatan.getCalonMahasiswa()
																								.getId() + "_calon_mhs"
																						: "");

																		String smt = kegiatan.getTahunAkademik() + "/"
																				+ kegiatan.getSemster();

																		TreeMap<String, Object[]> mapdata = jurusans
																				.get(key);
																		if (mapdata == null) {
																			mapdata = new TreeMap<String, Object[]>();
																			jurusans.put(key, mapdata);
																		}

																		Object[] objSbm = mapdata.get(smt);
																		Map<Long, Double> dataTotal;

																		if (objSbm == null) {
																			dataTotal = new HashMap<Long, Double>();
																		} else {
																			dataTotal = (Map<Long, Double>) objSbm[1];
																		}

																		Set<Long> itemBiayas = items.get(key);
																		if (itemBiayas == null) {
																			itemBiayas = new TreeSet<Long>();
																			items.put(key, itemBiayas);
																		}
																		itemBiayas.add(cicilanPembayaran.getItemBiaya()
																				.getId());

																		Double total = dataTotal.get(cicilanPembayaran
																				.getItemBiaya().getId());
																		if (total == null) {
																			total = 0.0;
																		}
																		total += cicilanPembayaran.getNilai();
																		dataTotal.put(cicilanPembayaran.getItemBiaya()
																				.getId(), total);
							...
							e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/helper/keuangan/LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.java:1069");
							PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Host To Host Cicilan Per Item Dan Mahasiswa Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
								new String[] {
									"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
									"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
					...
					label.setValue("Ambil data (100%)");
					cicilanPembayarans = null;

				} catch (Exception e) {
					e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/helper/keuangan/LaporanRekapHostToHostCicilanPerItemDanMahasiswaWindow.java:1076");
					PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Host To Host Cicilan Per Item Dan Mahasiswa Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
						new String[] {
							"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
							"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
