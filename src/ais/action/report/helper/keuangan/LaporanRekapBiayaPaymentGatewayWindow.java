...
/**
 * Penyusun/penyaji laporan untuk laporan rekap biaya payment gateway window. Kelas ini mengubah
 ...
 * @see MyWindow
 */
public class LaporanRekapBiayaPaymentGatewayWindow extends MyWindow {
	...
	public LaporanRekapBiayaPaymentGatewayWindow() {
		super();
		try {
			init();
		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Rekap Biaya Payment Gateway Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanRekapBiayaPaymentGatewayWindow(String title, String border, boolean closable) {
		...
		Common.createDefaultTimer(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				String sql = "select to_char(a1.tanggal, 'DD-MM-YYYY') as tanggal,  "
						...
						+ "COUNT(a1.id) as jumlah,  sum(a1.biayapaymentgateway) as total " + "from log_pembayaran a1 "
				...
				} catch (Exception e4) { ais.common.ErrorAuditUtil.record(e4, "auto-audit(empty-catch) src/ais/action/report/helper/keuangan/LaporanRekapBiayaPaymentGatewayWindow.java:186");

				}

				ais.ui.util.EcampusUtil.setCellValue(sheet, 1, 0, "REKAPITULASI BIAYA PAYMENT GATEWAY ");
				...
				for (Object[] objects : jurusans) {
					...
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/helper/keuangan/LaporanRekapBiayaPaymentGatewayWindow.java:262");
						PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Biaya Payment Gateway Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
							new String[] {
				...
				try {
					...
					ais.ui.util.EcampusUtil.setBold(sheet,
							...
							true);
				} catch (Exception e3) { ais.common.ErrorAuditUtil.record(e3, "auto-audit(empty-catch) src/ais/action/report/helper/keuangan/LaporanRekapBiayaPaymentGatewayWindow.java:292");

				}

				sql = "select "
						+ "(case a1.validator when a1.validator then a1.validator else 'Tidak ada validator' end) as nama_bank, "
						+ "COUNT(a1.id) as jumlah,  sum(a1.biayapaymentgateway) as total "
				...
				} catch (Exception e2) { ais.common.ErrorAuditUtil.record(e2, "auto-audit(empty-catch) src/ais/action/report/helper/keuangan/LaporanRekapBiayaPaymentGatewayWindow.java:322");
				...
				try {
					...
					ais.ui.util.EcampusUtil.setBold(sheet,
							new Rect(0, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex), true);
				} catch (Exception e1) { ais.common.ErrorAuditUtil.record(e1, "auto-audit(empty-catch) src/ais/action/report/helper/keuangan/LaporanRekapBiayaPaymentGatewayWindow.java:334");
				}
				rowIndex++;
				jumlahTotal = 0.0;
				jumlah = 0;
				for (Object[] objects : jurusans) {
					...
					try {
						ais.ui.util.EcampusUtil.setBorder(sheet, new Rect(0, rowIndex, 2, rowIndex),
								BookHelper.BORDER_FULL, BorderStyle.THIN, color);
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/helper/keuangan/LaporanRekapBiayaPaymentGatewayWindow.java:352");
						PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Biaya Payment Gateway Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
							new String[] {
								"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
								"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
				...
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/helper/keuangan/LaporanRekapBiayaPaymentGatewayWindow.java:364");
					// Common.tampilErrorJikaAdmin(e);
				}
