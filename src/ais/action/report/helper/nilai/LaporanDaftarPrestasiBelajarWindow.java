...
import ais.action.report.format1.akademik.LaporanRekapNilaiHurufMahasiswaProdi;
...
/**
 * Penyusun/penyaji laporan untuk laporan daftar prestasi belajar window. Kelas ini mengubah data
 ...
 * @see MyWindow
 */
public class LaporanDaftarPrestasiBelajarWindow extends MyWindow {
	...
	public LaporanDaftarPrestasiBelajarWindow() {
		super();
		...
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Daftar Prestasi Belajar Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanDaftarPrestasiBelajarWindow(Perkuliahan perkuliahan) {
		super();
		Tbmuser tbmuser = Common.getCurrentUser();
		selectedMahasiswa = tbmuser == null ? null : tbmuser.getMahasiswa();
	...
	public LaporanDaftarPrestasiBelajarWindow(String title, String border, boolean closable) {
	...
	private void init() {
		...
		MyTabConfig tab1 = new MyTabConfig("Prestasi Belajar Mahasiswa");
		tab1.setParent(tabs);

		MyTabConfig tab11 = new MyTabConfig("Prestasi Teori/Praktek");
		tab11.setVisible(Common.bolehKonfigurasi("tampilkan_teori_praktek", Konfigurasi.TIDAK_AKTIF));
		tab11.setParent(tabs);

		MyTabConfig tab2 = new MyTabConfig("Prosentasi Nilai Huruf");
		tab2.setParent(tabs);
		...
		tabpanel11.setVisible(tab11.isVisible());
		tab11.addEventListener("onClick", new EventListener() {

			@Override
			public void onEvent(Event arg0) throws Exception {
				if (tabpanel11.getChildren().size() == 0) {
					LaporanDaftarPrestasiBelajarTeoriPraktekWindow laporan = new LaporanDaftarPrestasiBelajarTeoriPraktekWindow();
		...
		tab2.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				if (tabpanel22.getChildren().size() == 0) {
					LaporanRekapNilaiHurufMahasiswaProdi laporan = new LaporanRekapNilaiHurufMahasiswaProdi();
					laporan.setHeight("100%");
					laporan.setWidth("100%");
		...
		borderlayout.setParent(selectedPerkuliahan == null && selectedMahasiswa == null ? tabpanel1 : this);
		...
		} catch (Exception e1) { ais.common.ErrorAuditUtil.record(e1, "auto-audit(empty-catch) src/ais/action/report/helper/nilai/LaporanDaftarPrestasiBelajarWindow.java:324");
		}
		...
		if (selectedPerkuliahan != null || selectedMahasiswa != null) {
			try {
				...
				e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/helper/nilai/LaporanDaftarPrestasiBelajarWindow.java:451");
				PesanFormalHelper.tampilkanGagalException("pembuatan berkas Excel Laporan Daftar Prestasi Belajar Window", "Sistem mengalami kendala teknis saat menyusun berkas Excel laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap atau format datanya tidak sesuai dengan yang diharapkan oleh template ekspor.", e,
					new String[] {
	...
	private void initMahasiswa() {
		...
		if (selectedPerkuliahan != null) {
			...
			matakuliahs.put(matakuliah.getId(), matakuliah);
				...
				for (Long detailperkuliahan2id : detailperkuliahans) {
					...
					if (detailperkuliahan2 != null) {
						if (krsMahasiswa.getSemester().equals(detailperkuliahan2.getSemester())) {
							...
							matakuliahs.put(matakuliah2.getId(), matakuliah2);
						}
					}
				}
			}
...
Common.clear(center);spreadsheet.setParent(center);
				...
				spreadsheet.setMaxcolumns((matakuliahs.size() * 3) + 10);
				...
				for (Matakuliah matakuliah : matakuliahs.values()) {
					...
					ais.ui.util.EcampusUtil.setCellValue(sheet, 5, colIndex, "M");
					ais.ui.util.EcampusUtil.setCellValue(sheet, 5, colIndex + 1, "K");
					ais.ui.util.EcampusUtil.setCellValue(sheet, 5, colIndex + 2, "MxK");
				...
				try {
					...
					String tahunAkademik = (String) (LaporanDaftarPrestasiBelajarWindow.this.tahunAkademik
							.getSelectedItem() == null ? null
									: LaporanDaftarPrestasiBelajarWindow.this.tahunAkademik.getSelectedItem()
											.getValue());
					String semester = (String) (LaporanDaftarPrestasiBelajarWindow.this.semesterAbsensi
							.getSelectedItem() == null ? null
									: LaporanDaftarPrestasiBelajarWindow.this.semesterAbsensi.getSelectedItem()
											.getValue());
					...
					Common.tampilErrorJikaAdmin(e);
					PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Daftar Prestasi Belajar Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
							new String[] {
				...
				} catch (Exception e1) { ais.common.ErrorAuditUtil.record(e1, "auto-audit(empty-catch) src/ais/action/report/helper/nilai/LaporanDaftarPrestasiBelajarWindow.java:693");
				...
				try {
					ais.ui.util.EcampusUtil.mergeCells(sheet, 4, 1, 5, 1, false);
					ais.ui.util.EcampusUtil.mergeCells(sheet, 4, 2, 5, 2, false);
				} catch (Exception e1) { ais.common.ErrorAuditUtil.record(e1, "auto-audit(empty-catch) src/ais/action/report/helper/nilai/LaporanDaftarPrestasiBelajarWindow.java:711");
				...
				for (CommonSorter commonSorter : mahasiswas) {
					...
					for (Matakuliah matakuliah : matakuliahs.values()) {
						...
						if (detailperkuliahan != null) {
							...
							try {
								ais.ui.util.EcampusUtil.setCellValue(sheet, i, y, detailperkuliahan.getNilaiHuruf());
								ais.ui.util.EcampusUtil.setCellValue(sheet, i, y + 1, matakuliah.getSks());
								Double ip = detailperkuliahan.getTotalIP();
								Double mxk = ip * matakuliah.getSks().doubleValue();
								ais.ui.util.EcampusUtil.setCellValue(sheet, i, y + 2, mxk);
