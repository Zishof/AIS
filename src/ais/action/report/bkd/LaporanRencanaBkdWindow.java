...
/**
 * Penyusun/penyaji laporan untuk laporan rencana bkd window. Kelas ini mengubah data domain
 ...
 * Combobox semesterAbsensi}, {@code AmbilDataPegawaiBanbox pegawai}, {@code Toolbar toolbar}, {@code Center
 * center}; inisialisasi/lifecycle ({@code init()}); operasi domain lain ({@code generateParameter()}, {@code
 * onLaporanAngketDosenPerDosen()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang
 * disebut di atas.</p>
 ...
 * @see MyWindow
 */
public class LaporanRencanaBkdWindow extends MyWindow {
	...
	public LaporanRencanaBkdWindow() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Rencana Bkd Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanRencanaBkdWindow(String title, String border, boolean closable) throws Exception {
	...
	private void init() {
		...
		}, "form_rencana_kinerja_dosen", null, new EventListener() {
	...
	private Map generateParameter() throws Exception {
		...
		if (myDosen.getDosen() != null) {
			String ttd = null;

			LampiranLain lam = LampiranLain.ambil(myDosen.getDosen().getId(), LampiranLain.TTD_DOSEN);
			...
			if (nama != null) {
				if (nama.toLowerCase().endsWith(".jpg") || nama.toLowerCase().endsWith(".png")
					...
					parameters.put("ttd_dosen", ttd);
				}
			}

			System.out.println("ttd_dosen => " + ttd);

			if (myDosen.getDosen().getAtasanlangsung() != null) {

				lam = LampiranLain.ambil(myDosen.getDosen().getAtasanlangsung(), LampiranLain.TTD_DOSEN);
				...
				if (nama != null) {
					if (nama.toLowerCase().endsWith(".jpg") || nama.toLowerCase().endsWith(".png")
						...
						parameters.put("ttd_atasan", ttd);
					}
				}

				System.out.println("ttd_atasan => " + ttd);
			}
		}
	...
	public void onLaporanAngketDosenPerDosen(Event event) throws Exception {
		...
		Common.createDefaultTimer(new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(), "form_rencana_kinerja_dosen",
