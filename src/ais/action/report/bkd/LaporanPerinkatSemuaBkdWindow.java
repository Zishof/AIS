...
/**
 * Penyusun/penyaji laporan untuk laporan perinkat semua bkd window. Kelas ini mengubah data domain
 ...
 * Combobox semesterAbsensi}, {@code Toolbar toolbar}, {@code Center center}, {@code Combobox urutBerdasar};
 ...
 * @see MyWindow
 */
public class LaporanPerinkatSemuaBkdWindow extends MyWindow {
	...
	private Combobox urutBerdasar;

	public LaporanPerinkatSemuaBkdWindow() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Perinkat Semua Bkd Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanPerinkatSemuaBkdWindow(String title, String border, boolean closable) throws Exception {
	...
	private void init() {
		...
		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Urut Berdasar"));
		row.appendChild(urutBerdasar = new Combobox());
		comboitem = new org.zkoss.zul.Comboitem(); 
		comboitem.setLabel("Kinerja"); 
		comboitem.setValue(1);
		urutBerdasar.appendChild(comboitem);
		comboitem = new MyComboitemConfig();
		comboitem.setLabel("Beban");
		comboitem.setValue(2);
		urutBerdasar.appendChild(comboitem);
		urutBerdasar.setWidth("90%");
		Common.selectComboItem(urutBerdasar, 1);
		urutBerdasar.setReadonly(true);
		...
		}, "rangking_kinerja_dosen_semua", null, new EventListener() {
	...
	private Map generateParameter() throws Exception {
		...
		parameters.put("urut", (Serializable) urutBerdasar.getSelectedItem().getValue());
	...
	public void onLaporanAngketDosenPerDosen(Event event) throws Exception {
		...
		Common.createDefaultTimer(new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(), "rangking_kinerja_dosen_semua",
