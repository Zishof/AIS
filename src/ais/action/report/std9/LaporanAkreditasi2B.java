...
/**
 * Penyusun/penyaji laporan untuk laporan akreditasi2 b. Kelas ini mengubah data domain menjadi
 ...
 * Combobox searchJurusan}, {@code Center center}, {@code Toolbar toolbar}, {@code Map parameters}, {@code
 * Combobox tahunAjaran}; inisialisasi/lifecycle ({@code init()}, {@code initData()}); pelaporan/ekspor ({@code
 ...
 * @see MyWindow
 */
public class LaporanAkreditasi2B extends MyWindow {
	...
	public LaporanAkreditasi2B() {
		super();
		try {
			init();
		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Akreditasi2 B", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanAkreditasi2B(String title, String border, boolean closable) throws Exception {
	...
	private void init() {
		...
		}, "std9/2b", null, new EventListener() {
		...
		try {
			...
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/std9/LaporanAkreditasi2B.java:176");
			PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Akreditasi2 B", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
				new String[] {
			...
			public void run() {
				...
				for (Jurusan jurusan : jurusans) {
					...
					try {
						...
						String sql = "sum(case when this_.tahunangkatan=" + (tahun - 2) + " then 1 else 0 end) as t2,"
								+ "sum(case when this_.tahunangkatan=" + (tahun - 1) + " then 1 else 0 end) as t1,"
								+ "sum(case when this_.tahunangkatan=" + (tahun) + " then 1 else 0 end) as t0";

						Object[] n6 = (Object[]) session.createCriteria(Mahasiswa.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
								.add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
								.add(Restrictions.eq("warganegara", Mahasiswa.WNA))
								.add(Restrictions.eq("jurusan", jurusan))

								.setProjection(Projections.projectionList()
										.add(Projections.sqlProjection(sql, new String[] { "t2", "t1", "t0" },
						...
						map.put("fakultas", jurusan.getFakultas().getNama() + " / " + jurusan.getNama());

						map.put("TS2", n6 == null || n6[0] == null ? 0.0 : Double.parseDouble(n6[0].toString().trim()));
						map.put("TS1", n6 == null || n6[1] == null ? 0.0 : Double.parseDouble(n6[1].toString().trim()));
						map.put("TS", n6 == null || n6[2] == null ? 0.0 : Double.parseDouble(n6[2].toString().trim()));

						maps.add(map);
					} catch (Exception e) {
						e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/std9/LaporanAkreditasi2B.java:252");
						PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Akreditasi2 B", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
							new String[] {
								"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
								"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
		...
		timer.addEventListener("onTimer", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				if (ais.action.report.helper.LoadingReportUtil.isError(label)) {
					...
					File file = Report.generateFileReportWithProgress(Report.PDF, parameters, "std9/2b", ais.ui.util.WaktuUtil.getDate(),
							toolbar);
