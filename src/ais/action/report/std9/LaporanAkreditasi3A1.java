...
/**
 * Penyusun/penyaji laporan untuk laporan akreditasi3 a1. Kelas ini mengubah data domain menjadi
 ...
 * @see MyWindow
 */
public class LaporanAkreditasi3A1 extends MyWindow {
	...
	public LaporanAkreditasi3A1(String title, String border, boolean closable) throws Exception {
	...
	private void init() {
		...
		}, "std9/3a1", null, new EventListener() {
		...
		try {
			...
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/std9/LaporanAkreditasi3A1.java:166");
			PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Akreditasi3 A1", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
				new String[] {
	...
	private void initData(final Label label, final Map parameters, final Fakultas fakultas, final Jurusan jurusan) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				List<Long> idsS3 = session
						.createCriteria(Pendidikan.class).setProjection(Projections.property("id")).add(Restrictions
								.in("nama", new String[] { "S3", "s3", "S3 Terapan", "S-3T", "Sp-2", "Strata 3" }))
						.list();
				String idStrS3 = "";
				for (Long id : idsS3) {
				...
				List<Long> idsS2 = session
						.createCriteria(Pendidikan.class).setProjection(Projections.property("id")).add(Restrictions
								.in("nama", new String[] { "S2", "s2", "S2 Terapan", "S-2T", "Sp-1", "Strata 2" }))
						.list();
				String idStrS2 = "";
				for (Long id : idsS2) {
				...
				List<Long> idsProfesi = session.createCriteria(Pendidikan.class)
						.setProjection(Projections.property("id"))
						.add(Restrictions.in("nama", new String[] { "Profesi", "profesi" })).list();
				String idStrProfesi = "";
				...
				for (Jurusan jurusan : jurusans) {
					...
					try {
						...
						String sql = "sum(case when this_.pendidikan in (" + idStrS3 + ") then 1 else 0 end) as t2,"
								+ "sum(case when this_.pendidikan in (" + idStrS2 + ") then 1 else 0 end) as t1,"
								+ "sum(case when this_.pendidikan in (" + idStrProfesi + ") then 1 else 0 end) as t0";
						...
						map.put("jmlPendidikanDoktor",
								n6 == null || n6[0] == null ? 0.0 : Double.parseDouble(n6[0].toString().trim()));
						map.put("jmlPendidikanMagister",
								n6 == null || n6[1] == null ? 0.0 : Double.parseDouble(n6[1].toString().trim()));
						map.put("jmlPendidikanProfesi",
								n6 == null || n6[2] == null ? 0.0 : Double.parseDouble(n6[2].toString().trim()));

						maps.add(map);
					} catch (Exception e) {
						e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/std9/LaporanAkreditasi3A1.java:268");
						PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Akreditasi3 A1", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
							new String[] {
								"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
								"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
		...
		timer.addEventListener("onTimer", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				if (ais.action.report.helper.LoadingReportUtil.isError(label)) {
					...
					File file = Report.generateFileReportWithProgress(Report.PDF, parameters, "std9/3a1", ais.ui.util.WaktuUtil.getDate(),
							toolbar);
