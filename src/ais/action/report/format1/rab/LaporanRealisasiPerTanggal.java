...
/**
 * Penyusun/penyaji laporan untuk laporan realisasi per tanggal. Kelas ini mengubah data domain
 ...
 * AmbilDataSatuanKerjaBanbox satuanKerja}, {@code AmbilDataSumberDanaBanbox sumberDana}, {@code MyDatebox
 * tanggal}, {@code Combobox level}, {@code Center center}, {@code Toolbar toolbar}, {@code
 ...
 * @see MyWindow
 */
public class LaporanRealisasiPerTanggal extends MyWindow {
	...
	public LaporanRealisasiPerTanggal() {
		super();
		try {
			init();
		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Realisasi Per Tanggal", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanRealisasiPerTanggal(String title, String border, boolean closable) throws Exception {
	...
	private void init() throws Exception {
		...
		}, "rab/Realisasi_Anggaran_Bulanan", null, new EventListener() {
	...
	private Map generateParameter() throws Exception {
		...
		if (tanggal.getValue() == null) {
			MyMessageboxConfig.show("Per tanggal harus diisi", "Peringatan", MyMessageboxConfig.OK,
					MyMessageboxConfig.INFORMATION);
			return null;
		}
		...
		Date mulai = calendar.getTime();
		Date sampai = tanggal.getValue();
		...
		for (final Workspace workspace : workspaces) {
			if ((selectedWorkspace != null && selectedWorkspace.getId().equals(workspace.getId()))
				...
				maps.add(map);
				rabReportHelper.generateRencanaAnggaran(workspace.getParentId(), workspaceTreeModel, workspace.getId(),
						workspaces, maps, mulai, sampai);
			}
		}
	...
	public void onReport(Event event) {
		...
		try {
			...
			File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(), "rab/Realisasi_Anggaran_Bulanan",
					ais.ui.util.WaktuUtil.getDate(), null, toolbar);
			CommonReport.tampilkanReportPDF(center, file);

		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Realisasi Per Tanggal", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
					new String[] {
