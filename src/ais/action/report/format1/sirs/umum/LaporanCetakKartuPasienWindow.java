 ...
 * {@code Center center}; inisialisasi/lifecycle ({@code init()}); pelaporan/ekspor ({@code onCetakTracer()}).
...
public class LaporanCetakKartuPasienWindow extends Window {
	...
	public LaporanCetakKartuPasienWindow() {
		...
		try {
			...
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/umum/LaporanCetakKartuPasienWindow.java:49");
	...
	public void onCetakTracer(Event event) {
		...
		try {
			if (pasienBanbox.getAttribute("pasien") == null) {
				MyMessageboxConfig.show("Mohon maaf Bapak/Ibu, data pasien belum dipilih. Langkah yang dapat dilakukan: (1) klik kolom Pilih Pasien; (2) cari lalu pilih salah satu data pasien; (3) lanjutkan kembali proses cetak kartu pasien.", "Peringatan", MyMessageboxConfig.OK, MyMessageboxConfig.INFORMATION);
				return;
			}
			...
			String alamat = (pasien.getAlamat() + " "
					+ (pasien.getKelurahan() == null ? "" : pasien.getKelurahan().getNama())
			...
			// Report.generatePDFReport(Report.PDF, parameters, "sirs/kartu_pasien",
			// new Date());

			File file = Report.generateFileReportWithProgress("sirs/kartu_pasien", Report.PDF, parameters, "sirs/kartu_pasien", new Date(),
					Sessions.getCurrent().getWebApp());
			CommonReport.tampilkanReportPDF(center, file);

		} catch (Exception e) {
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/umum/LaporanCetakKartuPasienWindow.java:177");
