...
/**
 * Penyusun/penyaji laporan untuk laporan biaya alat medis. Kelas ini mengubah data domain menjadi
 ...
 * @see Window
 */
public class LaporanBiayaAlatMedis extends Window {
	...
	public LaporanBiayaAlatMedis() {
		super();
		try {

			init();
		} catch (Exception e) {
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/umum/LaporanBiayaAlatMedis.java:45");
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Biaya Alat Medis", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, unit/ruangan, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
				new String[] {
	...
	public LaporanBiayaAlatMedis(String title, String border, boolean closable) throws Exception {
	...
	private void init() {
		...
		Common.insertCombo(jenisBarang, "nama", JenisAlatMedis.class);
	...
	private Map generateParameter() throws Exception {
		...
		JenisAlatMedis myJenisAlatMedis = (JenisAlatMedis) (jenisBarang.getSelectedItem() == null ? null
				: jenisBarang.getSelectedItem().getValue());

		Map parameters = new HashMap();
		parameters.put("jenisAlatMedis", myJenisAlatMedis == null || myJenisAlatMedis.getId() == null ? -1L : myJenisAlatMedis.getId());
		return parameters;
	}
	...
	public void onCetak(Event event) {
		...
		try {
			...
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/umum/LaporanBiayaAlatMedis.java:135");
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas Excel Laporan Biaya Alat Medis", "Sistem mengalami kendala teknis saat menyusun berkas Excel laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap atau format datanya tidak sesuai dengan yang diharapkan oleh template ekspor.", e,
				new String[] {
