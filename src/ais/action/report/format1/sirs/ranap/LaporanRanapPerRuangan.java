...
/**
 * Penyusun/penyaji laporan untuk laporan ranap per ruangan. Kelas ini mengubah data domain menjadi
 ...
 * {@code Combobox bulan}, {@code Combobox statusPendaftaran}; inisialisasi/lifecycle ({@code init()});
 ...
 * @see Window
 */
public class LaporanRanapPerRuangan extends Window {
	...
	private Combobox statusPendaftaran;

	public LaporanRanapPerRuangan() {
		super();
		try {

			init();
		} catch (Exception e) {
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/ranap/LaporanRanapPerRuangan.java:50");
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Ranap Per Ruangan", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, unit/ruangan, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
				new String[] {
	...
	public LaporanRanapPerRuangan(String title, String border, boolean closable) throws Exception {
	...
	private void init() {
		...
		row.appendChild(statusPendaftaran = new Combobox());
		Comboitem comboitem = new Comboitem(Pendaftaran.TERDAFTAR);
		comboitem.setValue(Pendaftaran.TERDAFTAR);
		statusPendaftaran.appendChild(comboitem);
		statusPendaftaran.setSelectedItem(comboitem);

		comboitem = new Comboitem(Pendaftaran.KELUAR);
		comboitem.setValue(Pendaftaran.KELUAR);
		statusPendaftaran.appendChild(comboitem);

		comboitem = new Comboitem(Pendaftaran.MENINGGAL);
		comboitem.setValue(Pendaftaran.MENINGGAL);
		statusPendaftaran.appendChild(comboitem);

		statusPendaftaran.setWidth("90%");
		statusPendaftaran.addEventListener("onChange", eventListener);
	...
	private Map generateParameter() throws Exception {
		...
		String status = (String) (this.statusPendaftaran.getSelectedItem() == null ? "Semua"
				: this.statusPendaftaran.getSelectedItem().getValue());
	...
	public void onCetak(Event event) {
		...
		try {
			...
			File file = Report.generateFileReportWithProgress("sirs/ranap_laporan_perruangan", Report.PDF, parameters,
					"sirs/ranap_laporan_perruangan", new Date());
			CommonReport.tampilkanReportPDF(center, file);

		} catch (Exception e) {
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/ranap/LaporanRanapPerRuangan.java:182");
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Ranap Per Ruangan", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
				new String[] {
