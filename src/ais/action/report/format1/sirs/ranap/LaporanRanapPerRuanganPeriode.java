...
/**
 * Penyusun/penyaji laporan untuk laporan ranap per ruangan periode. Kelas ini mengubah data domain
 ...
 * MyDatebox tanggalSampai}, {@code Center center}; inisialisasi/lifecycle ({@code init()}); pelaporan/ekspor
 * ({@code onCetakStatusPasien()}); operasi domain lain ({@code generateParameter()}). Bagian lain dari kontrak
 ...
 * @see Window
 */
public class LaporanRanapPerRuanganPeriode extends Window {
	...
	public LaporanRanapPerRuanganPeriode() {
		super();
		try {
			init();
		} catch (Exception e) {
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/ranap/LaporanRanapPerRuanganPeriode.java:48");
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Ranap Per Ruangan Periode", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, unit/ruangan, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
				new String[] {
	...
	public LaporanRanapPerRuanganPeriode(String title, String border, boolean closable) throws Exception {
	...
	private void init() {
		...
		}, "sirs/ranap_laporan_perruangan_periode"));
	...
	public void onCetakStatusPasien(Event event) {
		...
		try {
			...
			File file = Report.generateFileReportWithProgress("sirs/ranap_laporan_perruangan_periode", Report.PDF, parameters,
					"sirs/ranap_laporan_perruangan_periode", new Date(), Sessions.getCurrent().getWebApp());
			CommonReport.tampilkanReportPDF(center, file);

		} catch (Exception e) {
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/ranap/LaporanRanapPerRuanganPeriode.java:175");
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Ranap Per Ruangan Periode", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
				new String[] {
