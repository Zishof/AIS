...
/**
 * Penyusun/penyaji laporan untuk laporan penerimaan order window. Kelas ini mengubah data domain
 ...
 * @see Window
 */
public class LaporanPenerimaanOrderWindow extends Window {
	...
	public LaporanPenerimaanOrderWindow() {
		super();
		try {
			init();
		} catch (Exception e) {
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/inventory/LaporanPenerimaanOrderWindow.java:51");
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Penerimaan Order Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, unit/ruangan, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
				new String[] {
	...
	public LaporanPenerimaanOrderWindow(String title, String border, boolean closable) throws Exception {
	...
	private void init() {
		...
		}, "sirs/delivery_order_per_periode"));
	...
	public void onCetakStatusPasien(Event event) {
		...
		try {
			...
			File file = Report.generateFileReportWithProgress("sirs/delivery_order_per_periode", Report.XLS, parameters,
					"sirs/delivery_order_per_periode", new Date());
			CommonReport.tampilkanReportXLS(center, file);

		} catch (Exception e) {
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/inventory/LaporanPenerimaanOrderWindow.java:200");
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas Excel Laporan Penerimaan Order Window", "Sistem mengalami kendala teknis saat menyusun berkas Excel laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap atau format datanya tidak sesuai dengan yang diharapkan oleh template ekspor.", e,
				new String[] {
