...
/**
 * Penyusun/penyaji laporan untuk laporan pemakaian retur item window. Kelas ini mengubah data
 ...
 * @see Window
 */
public class LaporanPemakaianReturItemWindow extends Window {
	...
	public LaporanPemakaianReturItemWindow() {
		super();
		try {
			init();
		} catch (Exception e) {
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/inventory/LaporanPemakaianReturItemWindow.java:52");
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Pemakaian Retur Item Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, unit/ruangan, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
				new String[] {
	...
	public LaporanPemakaianReturItemWindow(String title, String border, boolean closable) throws Exception {
	...
	public void onCetakStatusPasien(Event event) {
		...
		try {
			...
			File file = Report.generateFileReportWithProgress("sirs/pemakaian_retur_item_periode", Report.XLS, parameters,
					"sirs/pemakaian_retur_item_periode", new Date(), Sessions.getCurrent().getWebApp());
			CommonReport.tampilkanReportXLS(center, file);

		} catch (Exception e) {
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/inventory/LaporanPemakaianReturItemWindow.java:201");
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas Excel Laporan Pemakaian Retur Item Window", "Sistem mengalami kendala teknis saat menyusun berkas Excel laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap atau format datanya tidak sesuai dengan yang diharapkan oleh template ekspor.", e,
				new String[] {
