...
/**
 * Penyusun/penyaji laporan untuk laporan pemakaian item window. Kelas ini mengubah data domain
 ...
 * @see Window
 */
public class LaporanPemakaianItemWindow extends Window {
	...
	public LaporanPemakaianItemWindow() {
		super();
		try {
			init();
		} catch (Exception e) {
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/inventory/LaporanPemakaianItemWindow.java:51");
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Pemakaian Item Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, unit/ruangan, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
				new String[] {
	...
	public LaporanPemakaianItemWindow(String title, String border, boolean closable) throws Exception {
	...
	public void onCetakStatusPasien(Event event) {
		...
		try {
			...
			File file = Report.generateFileReportWithProgress("sirs/pemakaian_item_periode", Report.XLS, parameters,
					"sirs/pemakaian_item_periode", new Date());
			CommonReport.tampilkanReportXLS(center, file);

		} catch (Exception e) {
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/inventory/LaporanPemakaianItemWindow.java:200");
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas Excel Laporan Pemakaian Item Window", "Sistem mengalami kendala teknis saat menyusun berkas Excel laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap atau format datanya tidak sesuai dengan yang diharapkan oleh template ekspor.", e,
				new String[] {
