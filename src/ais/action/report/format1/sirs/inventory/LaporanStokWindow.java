...
/**
 * Penyusun/penyaji laporan untuk laporan stok window. Kelas ini mengubah data domain menjadi
 ...
 * onCetakStatusPasien()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di
 * atas.</p>
 ...
 * @see Window
 */
public class LaporanStokWindow extends Window {
	...
	public LaporanStokWindow() {
		super();
		try {
			init();
		} catch (Exception e) {
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/inventory/LaporanStokWindow.java:47");
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Stok Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, unit/ruangan, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
				new String[] {
	...
	public LaporanStokWindow(String title, String border, boolean closable) throws Exception {
	...
	public void onCetakStatusPasien(Event event) {
		...
		try {
			...
			File file = Report.generateFileReportWithProgress("sirs/laporan_stok", Report.XLS, parameters, "sirs/laporan_stok", new Date(),
					Sessions.getCurrent().getWebApp());
			CommonReport.tampilkanReportXLS(center, file);

		} catch (Exception e) {
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/inventory/LaporanStokWindow.java:140");
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas Excel Laporan Stok Window", "Sistem mengalami kendala teknis saat menyusun berkas Excel laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap atau format datanya tidak sesuai dengan yang diharapkan oleh template ekspor.", e,
				new String[] {
