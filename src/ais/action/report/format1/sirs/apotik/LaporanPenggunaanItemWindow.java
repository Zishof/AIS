package ais.action.report.format1.sirs.apotik;
...
/**
 * Penyusun/penyaji laporan untuk laporan penggunaan item window. Kelas ini mengubah data domain
 ...
 * tanggalMulai}, {@code MyDatebox tanggalSampai}, {@code Combobox ranapRajal}, {@code Combobox jenisPasien},
 * {@code AmbilDataSatkerBanbox satker}, {@code Combobox lunas}, {@code Center center}; inisialisasi/lifecycle
 * ({@code init()}); pelaporan/ekspor ({@code onCetakStatusPasien()}); operasi domain lain ({@code
 ...
 * @see Window
 */
public class LaporanPenggunaanItemWindow extends Window {
	...
	private Combobox ranapRajal;
	private Combobox jenisPasien;
	private AmbilDataSatkerBanbox satker;
	private Combobox lunas;
	private Center center;

	public LaporanPenggunaanItemWindow() {
		super();
		try {
			init();
		} catch (Exception e) {
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/apotik/LaporanPenggunaanItemWindow.java:61");
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Penggunaan Item Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, unit/ruangan, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
				new String[] {
	...
	public LaporanPenggunaanItemWindow(String title, String border,
	...
	private void init() {
		...
		row.setStyle("border:0px;background: transparent;");
		row.setParent(rows);
		row.appendChild(new Label(ais.common.Common.getBahasaConfig("Status Lunas")));
		row.appendChild(lunas = new Combobox());
		Comboitem comboitem = new Comboitem("Lunas");
		comboitem.setValue("Lunas");
		lunas.appendChild(comboitem);
		comboitem = new Comboitem("Belum Lunas");
		comboitem.setValue("Belum Lunas");
		lunas.appendChild(comboitem);
		lunas.setWidth("90%");
		lunas.addEventListener("onChange", new EventListener() {
		...
		row.setStyle("border:0px;background: transparent;");
		row.setParent(rows);
		row.appendChild(new Label("Ranap/Rajal"));
		row.appendChild(ranapRajal = new Combobox());
		comboitem = new Comboitem(Pendaftaran.RAWAT_JALAN);
		comboitem.setValue(Pendaftaran.RAWAT_JALAN);
		ranapRajal.appendChild(comboitem);
		comboitem = new Comboitem(Pendaftaran.RAWAT_INAP);
		comboitem.setValue(Pendaftaran.RAWAT_INAP);
		ranapRajal.appendChild(comboitem);
		ranapRajal.setWidth("90%");
		ranapRajal.addEventListener("onChange", new EventListener() {
	...
	private Map generateParameter() throws Exception {
		...
		Satker satker = (Satker) (this.satker.getAttribute("satker") == null ? null
				: this.satker.getAttribute("satker"));
		JenisPasien jenisPasien = (JenisPasien) (this.jenisPasien
				.getSelectedItem() == null ? null : this.jenisPasien
				.getSelectedItem().getValue());

		String rajalRanap = (String) (this.ranapRajal.getSelectedItem() == null ? "-1"
				: this.ranapRajal.getSelectedItem().getValue());

		String lunas = (String) (this.lunas.getSelectedItem() == null ? "-1"
				: this.lunas.getSelectedItem().getValue());

		Map parameters = new HashMap();
		parameters.put("lunas", lunas);
		parameters.put("rajalRanap", rajalRanap);
	...
	public void onCetakStatusPasien(Event event) {
		...
		try {
			...
			File file = Report.generateFileReportWithProgress("sirs/penggunaan_obat",
					Report.XLS, parameters, "sirs/penggunaan_obat", new Date(),
					Sessions.getCurrent().getWebApp());
			CommonReport.tampilkanReportXLS(center, file);

		} catch (Exception e) {
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/sirs/apotik/LaporanPenggunaanItemWindow.java:305");
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas Excel Laporan Penggunaan Item Window", "Sistem mengalami kendala teknis saat menyusun berkas Excel laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap atau format datanya tidak sesuai dengan yang diharapkan oleh template ekspor.", e,
				new String[] {
