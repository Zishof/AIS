...
/**
 * Penyusun/penyaji laporan untuk rekap jumlah pegawai base unit kerja. Kelas ini mengubah data
 ...
 * tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code Combobox searchunitkerja}, {@code
 * Combobox searchstatus}, {@code Spreadsheet spreadsheet}, {@code Center center}; inisialisasi/lifecycle ({@code
 * initFakultas()}, {@code init()}, {@code initSpreadsheet()}). Bagian lain dari kontrak tetap mengikuti kelas
 ...
 * @see MyWindow
 */
public class RekapJumlahPegawaiBaseUnitKerja extends MyWindow {
	...
	private Combobox searchunitkerja = new Combobox();
	...
	public RekapJumlahPegawaiBaseUnitKerja() {
		super();
		try {
			init();
			initFakultas();
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Rekap Jumlah Pegawai Base Unit Kerja", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public RekapJumlahPegawaiBaseUnitKerja(String title, String border, boolean closable) {
	...
	private void initFakultas() {
		Common.insertCombo(searchunitkerja, new String[] { "nama", "id" }, UnitKerja.class,Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)));
	...
	private void init() throws Exception {
		...
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Pilih Unit Kerja"));
		row.appendChild(searchunitkerja);
		searchunitkerja.setWidth("90%");
		...
		print.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				Filedownload.save(bout.toByteArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
						"Rekap_jumlah_pegawai_base_unit_kerja.xlsx");
			}
		});
		print.setParent(toolbar);
	...
	private void initSpreadsheet() {
		...
		UnitKerja unitKerja = (UnitKerja) (searchunitkerja.getSelectedItem() == null ? null
				: searchunitkerja.getSelectedItem().getValue());

		Session session = HibernateUtil.currentSession();
		List<Object[]> unitkerja = new ArrayList<Object[]>();

		String sql = "select " + "b.nama as unit_kerja, " + "count(a.id) as jumlah " + "from pegawai a "
				+ "left join employ.unit_kerja b on (a.unit_kerja=b.id) "
				+ "left join status_pegawai c on (a.status_pegawai=b.id) " + " where 1=1 "
				+ (statusPegawai == null ? "" : " and a.status_pegawai= " + statusPegawai.getId())
				+ (unitKerja == null ? "" : " and a.unit_kerja = " + unitKerja.getId()) + " group by b.nama "
				+ " order by b.nama ";
		...
		// Center 1 anak -> Vlayout: panel CHART (bar per unit kerja) di atas, Spreadsheet di bawah.
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, 1, 0,
				"REKAPITULASI JUMLAH PEGAWAI \n " + "STATUS PEGAWAI "
						...
						+ (unitKerja == null ? "SEMUA" : unitKerja.getNama().toUpperCase()));
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 0, "Unit Kerja");
		...
		for (Object[] objects : unitkerja) {
			...
			try {
				ais.ui.util.EcampusUtil.setBorder(sheet,
						new Rect(colIndex, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex), BookHelper.BORDER_FULL,
						BorderStyle.THIN, color);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/employ/RekapJumlahPegawaiBaseUnitKerja.java:259");
				PesanFormalHelper.tampilkanGagalException("pemrosesan Rekap Jumlah Pegawai Base Unit Kerja", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
					new String[] {
		...
		// -- Panel visual: BAR jumlah pegawai per unit kerja (HTML/CSS, BUKAN JFreeChart) --
		try {
			double[] cv = new double[chartVal.size()];
			for (int i = 0; i < cv.length; i++) { cv[i] = chartVal.get(i); }
			chartPanel.setContent("<div style='padding:10px 12px 2px;'>"
					+ ais.ui.util.HtmlChartHelper.barHorizontal(
							...
							"Melihat berapa banyak pegawai pada tiap unit kerja, dan unit mana yang paling banyak.",
							chartLabel.toArray(new String[0]), cv, "#2563eb")
					+ "</div>");
		} catch (Exception eChart) { ais.common.ErrorAuditUtil.record(eChart, "auto-audit(empty-catch) src/ais/action/report/format1/employ/RekapJumlahPegawaiBaseUnitKerja.java:285");
		}
		...
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/employ/RekapJumlahPegawaiBaseUnitKerja.java:292");
			PesanFormalHelper.tampilkanGagalException("pemrosesan Rekap Jumlah Pegawai Base Unit Kerja", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
				new String[] {
					"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
					"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
