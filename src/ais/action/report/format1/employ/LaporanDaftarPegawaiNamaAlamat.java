...
/**
 * Penyusun/penyaji laporan untuk laporan daftar pegawai nama alamat. Kelas ini mengubah data
 ...
 * tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code Combobox golongan}, {@code Combobox
 * statusPegawai}, {@code Combobox kelamin}, {@code Combobox pendidikan}, {@code AmbilDataJenisPelatihanBanbox
 * jenisPelatihan}, {@code Combobox jenisTandaJasa}, {@code Center center}, {@code Toolbar toolbar};
 ...
 * @see MyWindow
 */
public class LaporanDaftarPegawaiNamaAlamat extends MyWindow {
	...
	public LaporanDaftarPegawaiNamaAlamat() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Daftar Pegawai Nama Alamat", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanDaftarPegawaiNamaAlamat(String title, String border, boolean closable) throws Exception {
	...
	private void init() throws Exception {
		...
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Pangkat / Golongan"));
		...
		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Pendidikan Terakhir"));
		...
		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Jenis Pelatihan"));
		...
		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Jenis Tanda Jasa"));
		...
		}, "employ/daftar_pegawai", null, new EventListener() {
	...
	private Map generateParameter() throws Exception {
		...
		Golongan golongan = (Golongan) (this.golongan.getSelectedItem() == null ? null
				: this.golongan.getSelectedItem().getValue());

		Pendidikan pendidikan = (Pendidikan) (this.pendidikan.getSelectedItem() == null ? null
				: this.pendidikan.getSelectedItem().getValue());

		StatusPegawai statusPegawai = (StatusPegawai) (this.statusPegawai.getSelectedItem() == null ? null
				: this.statusPegawai.getSelectedItem().getValue());
		...
		JenisTandaJasa jenisTandaJasa = (JenisTandaJasa) (this.jenisTandaJasa.getSelectedItem() == null ? null
				: this.jenisTandaJasa.getSelectedItem().getValue());

		JenisPelatihan jenisPelatihan = (JenisPelatihan) (this.jenisPelatihan.getAttribute("jenisPelatihan"));

		final Map parameters = ais.common.HashMapGenerator.getRand();
		parameters.put("kelamin", kelamin);
		parameters.put("statusPerkawinan", statusPerkawinan);
		parameters.put("golongan", golongan == null || golongan.getId() == null ? -1L : golongan.getId());
		parameters.put("pendidikan", pendidikan == null || pendidikan.getId() == null ? -1L : pendidikan.getId());

		parameters.put("jenisPelatihan", jenisPelatihan == null || jenisPelatihan.getId() == null ? -1L : jenisPelatihan.getId());

		parameters.put("jenisTandaJasa", jenisTandaJasa == null || jenisTandaJasa.getId() == null ? -1L : jenisTandaJasa.getId());
	...
	public void onTranskrip(Event event) throws Exception {
		...
		try {
			...
			File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(), "employ/daftar_pegawai_nama_alamat",
					ais.ui.util.WaktuUtil.getDate(), toolbar);
			CommonReport.tampilkanReportPDF(center, file);

		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Daftar Pegawai Nama Alamat", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
					new String[] {
