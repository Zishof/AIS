...
/**
 * Penyusun/penyaji laporan untuk laporan rekap angket dosen per mahasiswa. Kelas ini mengubah data
 ...
 * @see MyWindow
 */
public class LaporanRekapAngketDosenPerMahasiswa extends MyWindow {
	...
	public LaporanRekapAngketDosenPerMahasiswa() throws Exception {
		super();
		init();
	}

	public LaporanRekapAngketDosenPerMahasiswa(String title, String border, boolean closable) throws Exception {
	...
	private void init() throws Exception {
		...
		final MyTabConfig tabPerkuliahan = new MyTabConfig("Per Perkuliahan");
		...
		final Tabpanel tabpanelDataPerkuliahan = new ais.ui.util.MyTabpanel();
		tabpanelDataPerkuliahan.setParent(tabpanels);
		tabPerkuliahan.addEventListener("onClick", new EventListener() {

			@Override
			public void onEvent(Event arg0) throws Exception {
				if (tabpanelDataPerkuliahan.getChildren().size() == 0) {
					LaporanRekapAngketDosenPerPerkuliahan laporanAngketUmumWindow = new LaporanRekapAngketDosenPerPerkuliahan();
					tabpanelDataPerkuliahan.appendChild(laporanAngketUmumWindow);
				}
			}
		});
		...
		tabDosen.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				if (tabpanelDataDosen.getChildren().size() == 0) {
					LaporanRekapAngketDosenPerDosen laporanAngketUmumWindow = new LaporanRekapAngketDosenPerDosen();
		...
		printAmbil.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanRekapAngketDosenPerMahasiswa.java:262");
					PesanFormalHelper.tampilkanGagalException("pembuatan berkas Excel Laporan Rekap Angket Dosen Per Mahasiswa", "Sistem mengalami kendala teknis saat menyusun berkas Excel laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap atau format datanya tidak sesuai dengan yang diharapkan oleh template ekspor.", e,
						new String[] {
		...
		try {
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					TreeMap<String, List<ChecklistBaruPenilaianDosenOlehMahasiswa>> myItems = new TreeMap<String, List<ChecklistBaruPenilaianDosenOlehMahasiswa>>();
					...
					for (ChecklistBaruPenilaianDosenOlehMahasiswa checklistBaruPenilaianDosenOlehMahasiswa : checklistBaruPenilaianDosenOlehMahasiswas) {
						try {
							List<ChecklistBaruPenilaianDosenOlehMahasiswa> baruPenilaianDosenOlehMahasiswas = myItems
									.get(checklistBaruPenilaianDosenOlehMahasiswa.getMahasiswa().getNim());
							if (baruPenilaianDosenOlehMahasiswas == null) {
								...
								myItems.put(checklistBaruPenilaianDosenOlehMahasiswa.getMahasiswa().getNim(),
							...
							for (String d : StringUtils.split(checklistBaruPenilaianDosenOlehMahasiswa.getKeterangan(),
								...
								try {
									...
									e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanRekapAngketDosenPerMahasiswa.java:369");
									PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Angket Dosen Per Mahasiswa", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
										new String[] {
							...
							e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanRekapAngketDosenPerMahasiswa.java:373");
							PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Angket Dosen Per Mahasiswa", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
								new String[] {
									"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
									"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
					...
					for (List<ChecklistBaruPenilaianDosenOlehMahasiswa> aa : myItems.values()) {
						...
						try {
							Mahasiswa mahasiswa = aa.get(0).getMahasiswa();
							label.setValue("Sedang memproses mahasiswa " + mahasiswa.toString());
							...
							} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanRekapAngketDosenPerMahasiswa.java:448");
							...
							for (Long idGrup : checklistPenilaianDosens.keySet()) {
								...
								System.out.println("keyGrup -> " + idGrup + " " + treeMap.size() + " mahasiswa -> "
										+ mahasiswa.toString());
									...
									for (Perkuliahan perkuliahan : perkuliahans.values()) {
										...
										for (Long dosenId : dosens) {
											...
											if (baruPenilaianDosenOlehMahasiswa != null) {
												...
												String key = perkuliahan.getId() + "_" + dosenId;
								...
								for (Perkuliahan perkuliahan : perkuliahans.values()) {
									List<Long> dosens = perkuliahan.populateDosenBuId();
									for (Long idDosen : dosens) {
										String key = perkuliahan.getId() + "_" + idDosen;
										Integer sebelumnya = jumlahMasingMasing.get(key);
										if (sebelumnya == null) {
											sebelumnya = 0;
										}
							...
							e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanRekapAngketDosenPerMahasiswa.java:577");
							PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Angket Dosen Per Mahasiswa", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
								new String[] {
									"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
									"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
