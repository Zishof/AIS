...
/**
 * Penyusun/penyaji laporan untuk laporan checklist hasil penilaian umum per mahasiswa. Kelas ini
 ...
 * @see MyWindow
 */
public class LaporanChecklistHasilPenilaianUmumPerMahasiswa extends MyWindow {
	...
	public LaporanChecklistHasilPenilaianUmumPerMahasiswa() throws Exception {
		super();
		init();
	}

	public LaporanChecklistHasilPenilaianUmumPerMahasiswa(String title, String border, boolean closable)
			throws Exception {
	...
	private void init() throws Exception {
		...
		printAmbil.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanChecklistHasilPenilaianUmumPerMahasiswa.java:200");
					PesanFormalHelper.tampilkanGagalException("pembuatan berkas Excel Laporan Checklist Hasil Penilaian Umum Per Mahasiswa", "Sistem mengalami kendala teknis saat menyusun berkas Excel laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap atau format datanya tidak sesuai dengan yang diharapkan oleh template ekspor.", e,
						new String[] {
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					List<ChecklistHasilPenilaianUmum> checklistHasilPenilaianUmums = session
							.createCriteria(ChecklistHasilPenilaianUmum.class).createAlias("mahasiswa", "mahasiswa")
					...
					for (ChecklistHasilPenilaianUmum checklistHasilPenilaianUmum : checklistHasilPenilaianUmums) {
						try {
							List<ChecklistHasilPenilaianUmum> baruPenilaianDosenOlehMahasiswas = myItems
									.get(checklistHasilPenilaianUmum.getMahasiswa().getNim());
							if (baruPenilaianDosenOlehMahasiswas == null) {
								baruPenilaianDosenOlehMahasiswas = new ArrayList<ChecklistHasilPenilaianUmum>();
								myItems.put(checklistHasilPenilaianUmum.getMahasiswa().getNim(),
										baruPenilaianDosenOlehMahasiswas);
							}
							baruPenilaianDosenOlehMahasiswas.add(checklistHasilPenilaianUmum);
							...
							e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanChecklistHasilPenilaianUmumPerMahasiswa.java:296");
							PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Checklist Hasil Penilaian Umum Per Mahasiswa", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
								new String[] {
					...
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanChecklistHasilPenilaianUmumPerMahasiswa.java:352");
					...
					for (List<ChecklistHasilPenilaianUmum> aa : myItems.values()) {
						...
						try {
							...
							e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanChecklistHasilPenilaianUmumPerMahasiswa.java:412");
							PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Checklist Hasil Penilaian Umum Per Mahasiswa", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
								new String[] {
									"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
									"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
