...
/**
 * Penyusun/penyaji laporan untuk laporan rekap jumlah mahasiswa per jenis sekolah. Kelas ini
 ...
 * @see MyWindow
 */
public class LaporanRekapJumlahMahasiswaPerJenisSekolah extends MyWindow {
	...
	public LaporanRekapJumlahMahasiswaPerJenisSekolah() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Rekap Jumlah Mahasiswa Per Jenis Sekolah", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanRekapJumlahMahasiswaPerJenisSekolah(String title, String border, boolean closable) {
			...
			public void onEvent(Event event) throws Exception {
				...
				try {
					Filedownload.save(new FileInputStream(file), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
							"REKAPITULASI JUMLAH MAHASISWA JENIS SEKOLAH.xlsx");
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanRekapJumlahMahasiswaPerJenisSekolah.java:182");
					PesanFormalHelper.tampilkanGagalException("pembuatan berkas Excel Laporan Rekap Jumlah Mahasiswa Per Jenis Sekolah", "Sistem mengalami kendala teknis saat menyusun berkas Excel laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap atau format datanya tidak sesuai dengan yang diharapkan oleh template ekspor.", e,
						new String[] {
	...
	private void initSpreadsheet() throws Exception {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				List<JenisSekolahMahasiswaBaru> jenisSekolahMahasiswaBarues = session
						.createCriteria(JenisSekolahMahasiswaBaru.class)
						.add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
						.addOrder(Order.asc("nama")).list();
				jenisSekolahMahasiswaBarues.add(null);
				int qtyLk = 2;

				for (JenisSekolahMahasiswaBaru jenisSekolahMahasiswaBaru : jenisSekolahMahasiswaBarues) {

					rowhead.createCell(qtyLk).setCellValue(
							jenisSekolahMahasiswaBaru == null ? "Tidak dipilih" : jenisSekolahMahasiswaBaru.getNama());
							...
							try {
								...
								Integer jmlLaki = ((Number) session
										.createSQLQuery("select count(*) as jumlah from history_status_mahasiswa a "
												...
												+ (jenisSekolahMahasiswaBaru == null ? " and c.jenis_sekolah is null "
														: " and c.jenis_sekolah=" + jenisSekolahMahasiswaBaru.getId()))
