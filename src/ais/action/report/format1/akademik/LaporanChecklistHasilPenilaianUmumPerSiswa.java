...
/**
 * Penyusun/penyaji laporan untuk laporan checklist hasil penilaian umum per siswa. Kelas ini
 ...
 * @see MyWindow
 */
public class LaporanChecklistHasilPenilaianUmumPerSiswa extends MyWindow {
	...
	public LaporanChecklistHasilPenilaianUmumPerSiswa() throws Exception {
		super();
		init();
	}

	public LaporanChecklistHasilPenilaianUmumPerSiswa(String title, String border, boolean closable) throws Exception {
	...
	private void init() throws Exception {
		...
		printAmbil.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanChecklistHasilPenilaianUmumPerSiswa.java:200");
					PesanFormalHelper.tampilkanGagalException("pembuatan berkas Excel Laporan Checklist Hasil Penilaian Umum Per Siswa", "Sistem mengalami kendala teknis saat menyusun berkas Excel laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap atau format datanya tidak sesuai dengan yang diharapkan oleh template ekspor.", e,
						new String[] {
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					List<ChecklistHasilPenilaianUmum> checklistHasilPenilaianUmums = session
							.createCriteria(ChecklistHasilPenilaianUmum.class).createAlias("siswa", "siswa")
							...
							.add(sekolah.getSelectedItem() == null || sekolah.getSelectedItem().getValue() == null
									|| sekolah.getSelectedItem().getValue() == null ? Restrictions.sqlRestriction("1=1")
											: CommonSearchFilterHelper.eqSelectedWithId("siswa.sekolah", sekolah, false))
							...
							.add(yayasan.getSelectedItem() == null || yayasan.getSelectedItem().getValue() == null
									|| yayasan.getSelectedItem().getValue() == null ? Restrictions.sqlRestriction("1=1")
											: CommonSearchFilterHelper.eqSelectedWithId("siswa.yayasan", yayasan, false))
					...
					for (ChecklistHasilPenilaianUmum checklistHasilPenilaianUmum : checklistHasilPenilaianUmums) {
						try {
							List<ChecklistHasilPenilaianUmum> baruPenilaianSiswaOlehSiswas = myItems
									.get(checklistHasilPenilaianUmum.getSiswa().getNama() + "-"
											+ checklistHasilPenilaianUmum.getSiswa().getId());
							if (baruPenilaianSiswaOlehSiswas == null) {
								baruPenilaianSiswaOlehSiswas = new ArrayList<ChecklistHasilPenilaianUmum>();
								myItems.put(
										checklistHasilPenilaianUmum.getSiswa().getNama() + "-"
												+ checklistHasilPenilaianUmum.getSiswa().getId(),
										baruPenilaianSiswaOlehSiswas);
							}
							baruPenilaianSiswaOlehSiswas.add(checklistHasilPenilaianUmum);
							...
							e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanChecklistHasilPenilaianUmumPerSiswa.java:295");
							PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Checklist Hasil Penilaian Umum Per Siswa", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
								new String[] {
					...
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanChecklistHasilPenilaianUmumPerSiswa.java:351");
					...
					for (List<ChecklistHasilPenilaianUmum> aa : myItems.values()) {
						...
						try {
							Siswa siswa = aa.get(0).getSiswa();
							label.setValue("Sedang memproses siswa " + siswa.toString());
							...
							sub.add(siswa.getSekolah() == null ? "" : siswa.getSekolah().getNama());
							sub.add(kelasSiswa == null ? "" : kelasSiswa.getNama());
							sub.add(siswa.getTahunMasuk() + "");
							Integer jumlah = 0;
							Integer total = 0;
							...
							e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanChecklistHasilPenilaianUmumPerSiswa.java:412");
							PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Checklist Hasil Penilaian Umum Per Siswa", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
								new String[] {
									"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
									"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
