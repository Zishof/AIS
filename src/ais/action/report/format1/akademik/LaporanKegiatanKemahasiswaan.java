...
/**
 * Penyusun/penyaji laporan untuk laporan kegiatan kemahasiswaan. Kelas ini mengubah data domain
 ...
 * bandboxMahasiswa}, {@code Center center}, {@code Toolbar toolbar}, {@code Combobox semesterMulai}, {@code
 * Combobox semesterSampai}, {@code MyDatebox tanggal}; inisialisasi/lifecycle ({@code init()}, {@code
 * buatPilihanSemester()}); pembacaan/pencarian ({@code ambilSemester()}); validasi/perhitungan ({@code
 * validasiSemester()}); operasi domain lain ({@code aturPilihanSemester()}, {@code isiPilihanSemester()}, {@code
 * generateParameter()}, {@code generateParameter()}, {@code onKHS()}). Bagian lain dari kontrak tetap mengikuti
