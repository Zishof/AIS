...
/**
 * Penyusun/penyaji laporan untuk laporan checklist hasil penilaian umum per dosen. Kelas ini
 ...
 * @see MyWindow
 */
public class LaporanChecklistHasilPenilaianUmumPerDosen extends MyWindow {
	...
	public LaporanChecklistHasilPenilaianUmumPerDosen() throws Exception {
		super();
		init();
	}

	public LaporanChecklistHasilPenilaianUmumPerDosen(String title, String border, boolean closable) throws Exception {
	...
	private void init() throws Exception {
		...
		printAmbil.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanChecklistHasilPenilaianUmumPerDosen.java:199");
					PesanFormalHelper.tampilkanGagalException("pembuatan berkas Excel Laporan Checklist Hasil Penilaian Umum Per Dosen", "Sistem mengalami kendala teknis saat menyusun berkas Excel laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap atau format datanya tidak sesuai dengan yang diharapkan oleh template ekspor.", e,
						new String[] {
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					List<ChecklistHasilPenilaianUmum> checklistHasilPenilaianUmums = session
							.createCriteria(ChecklistHasilPenilaianUmum.class).createAlias("dosen", "dosen")
							...
							.add(fakultas.getSelectedItem() == null || fakultas.getSelectedItem().getValue() == null
									|| fakultas.getSelectedItem().getValue() == null
											? Restrictions.sqlRestriction("1=1")
											: CommonSearchFilterHelper.eqSelectedWithId("dosen.fakultas", fakultas, false))
					...
					for (ChecklistHasilPenilaianUmum checklistHasilPenilaianUmum : checklistHasilPenilaianUmums) {
						try {
							List<ChecklistHasilPenilaianUmum> baruPenilaianDosenOlehDosens = myItems
									.get(checklistHasilPenilaianUmum.getDosen().getNama() + "-"
											+ checklistHasilPenilaianUmum.getDosen().getId());
							if (baruPenilaianDosenOlehDosens == null) {
								baruPenilaianDosenOlehDosens = new ArrayList<ChecklistHasilPenilaianUmum>();
								myItems.put(
										checklistHasilPenilaianUmum.getDosen().getNama() + "-"
												+ checklistHasilPenilaianUmum.getDosen().getId(),
							...
							e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanChecklistHasilPenilaianUmumPerDosen.java:295");
							PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Checklist Hasil Penilaian Umum Per Dosen", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
								new String[] {
					...
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanChecklistHasilPenilaianUmumPerDosen.java:351");
					...
					for (List<ChecklistHasilPenilaianUmum> aa : myItems.values()) {
						...
						try {
							...
							e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanChecklistHasilPenilaianUmumPerDosen.java:410");
							PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Checklist Hasil Penilaian Umum Per Dosen", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
								new String[] {
									"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
									"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
