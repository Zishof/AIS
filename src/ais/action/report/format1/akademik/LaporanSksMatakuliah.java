...
/**
 * Penyusun/penyaji laporan untuk laporan sks matakuliah. Kelas ini mengubah data domain menjadi
 ...
 * Combobox fakultas}, {@code Combobox jurusan}, {@code Combobox searchTahunAjaran}, {@code Combobox
 * jenis_semester}, {@code Center center}; inisialisasi/lifecycle ({@code init()}, {@code initSpreadsheet()}).
 ...
 * @see MyWindow
 */
public class LaporanSksMatakuliah extends MyWindow {
	...
	public LaporanSksMatakuliah() {
		super();
		try {
			init();
		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Sks Matakuliah", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanSksMatakuliah(String title, String border, boolean closable) {
	...
	private void initSpreadsheet() throws Exception {
		...
		} catch (Exception e4) { ais.common.ErrorAuditUtil.record(e4, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanSksMatakuliah.java:216");

		}

		ais.ui.util.EcampusUtil.setCellValue(sheet, 1, 0, "DATA SKS MATAKULIAH");
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 2, "Total SKS");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 3, "Perkuliahan");
		...
		TreeMap<Long, Matakuliah> matakuliahsData = new TreeMap<Long, Matakuliah>();
		TreeMap<Long, List<Perkuliahan>> matakuliahsMap = new TreeMap<Long, List<Perkuliahan>>();
		for (Perkuliahan perkuliahan : perkuliahans) {
			Matakuliah d = perkuliahan.getMatakuliah();
			if (matakuliahsMap.containsKey(d.getId())) {
				matakuliahsMap.get(d.getId()).add(perkuliahan);
			} else {
				matakuliahsData.put(d.getId(), d);
				List<Perkuliahan> itemDetails = new ArrayList<Perkuliahan>();
				itemDetails.add(perkuliahan);
				matakuliahsMap.put(d.getId(), itemDetails);
			}

		}

		Integer sksTotal = 0;
		for (Long key : matakuliahsMap.keySet()) {
			List<Perkuliahan> matakuliahs = matakuliahsMap.get(key);
			Matakuliah matakuliah = matakuliahsData.get(key);

			String itemYangDipinjam = "";
			Integer sks = 0;
			for (Perkuliahan kembali : matakuliahs) {
				sks += kembali.getMatakuliah().getSks();
				String s = kembali.info();
				itemYangDipinjam += itemYangDipinjam.isEmpty() ? s : " ," + s;
			}

			sksTotal += sks;

			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 0, matakuliah.getNama());
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 1, matakuliah.getSks());
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 2, sks);
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 3, itemYangDipinjam);
		...
		ais.ui.util.EcampusUtil.setBold(sheet, new Rect(colIndex, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex),
				true);
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 1, "SKS Total");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 2, sksTotal);

		Utils.setColumnWidth(sheet, 0, 250);
		Utils.setColumnWidth(sheet, 1, 50);
		Utils.setColumnWidth(sheet, 2, 50);
		Utils.setColumnWidth(sheet, 3, 1000);
