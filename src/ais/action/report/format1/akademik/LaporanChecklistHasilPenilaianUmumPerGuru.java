...
/**
 * Penyusun/penyaji laporan untuk laporan checklist hasil penilaian umum per guru. Kelas ini
 ...
 * center}, {@code MyToolbarbuttonConfig printAmbil}, {@code Combobox yayasan}, {@code Combobox sekolah}, {@code
 ...
 * @see MyWindow
 */
public class LaporanChecklistHasilPenilaianUmumPerGuru extends MyWindow {
	...
	public LaporanChecklistHasilPenilaianUmumPerGuru() throws Exception {
		super();
		init();
	}

	public LaporanChecklistHasilPenilaianUmumPerGuru(String title, String border, boolean closable) throws Exception {
	...
	private void init() throws Exception {
		...
		printAmbil.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanChecklistHasilPenilaianUmumPerGuru.java:199");
					PesanFormalHelper.tampilkanGagalException("pembuatan berkas Excel Laporan Checklist Hasil Penilaian Umum Per Guru", "Sistem mengalami kendala teknis saat menyusun berkas Excel laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap atau format datanya tidak sesuai dengan yang diharapkan oleh template ekspor.", e,
						new String[] {
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					List<ChecklistHasilPenilaianUmum> checklistHasilPenilaianUmums = session
							.createCriteria(ChecklistHasilPenilaianUmum.class).createAlias("guru", "guru")
							...
							.add(sekolah.getSelectedItem() == null || sekolah.getSelectedItem().getValue() == null
									|| sekolah.getSelectedItem().getValue() == null ? Restrictions.sqlRestriction("1=1")
											: CommonSearchFilterHelper.eqSelectedWithId("guru.sekolah", sekolah, false))
							...
							.add(yayasan.getSelectedItem() == null || yayasan.getSelectedItem().getValue() == null
									|| yayasan.getSelectedItem().getValue() == null ? Restrictions.sqlRestriction("1=1")
											: CommonSearchFilterHelper.eqSelectedWithId("guru.yayasan", yayasan, false))
					...
					for (ChecklistHasilPenilaianUmum checklistHasilPenilaianUmum : checklistHasilPenilaianUmums) {
						try {
							List<ChecklistHasilPenilaianUmum> baruPenilaianGuruOlehGurus = myItems
									.get(checklistHasilPenilaianUmum.getGuru().getNama() + "-"
											+ checklistHasilPenilaianUmum.getGuru().getId());
							if (baruPenilaianGuruOlehGurus == null) {
								baruPenilaianGuruOlehGurus = new ArrayList<ChecklistHasilPenilaianUmum>();
								myItems.put(
										checklistHasilPenilaianUmum.getGuru().getNama() + "-"
												+ checklistHasilPenilaianUmum.getGuru().getId(),
										baruPenilaianGuruOlehGurus);
							}
							baruPenilaianGuruOlehGurus.add(checklistHasilPenilaianUmum);
							...
							e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanChecklistHasilPenilaianUmumPerGuru.java:294");
							PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Checklist Hasil Penilaian Umum Per Guru", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
								new String[] {
					...
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanChecklistHasilPenilaianUmumPerGuru.java:350");
					...
					for (List<ChecklistHasilPenilaianUmum> aa : myItems.values()) {
						...
						try {
							Guru guru = aa.get(0).getGuru();
							label.setValue("Sedang memproses guru " + guru.toString());

							ArrayList sub = new ArrayList();
							sub.add(nomor);
							sub.add(guru.getNuptk());
							sub.add(guru.getNama());
							sub.add(guru.getSekolah() == null ? "" : guru.getSekolah().getNama());
							sub.add(guru.getYayasan() == null ? "" : guru.getYayasan().getNama());
							sub.add(guru.getStatusKepegawaian() == null ? "" : guru.getStatusKepegawaian().getNama());
							Integer jumlah = 0;
							Integer total = 0;
							...
							e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanChecklistHasilPenilaianUmumPerGuru.java:409");
							PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Checklist Hasil Penilaian Umum Per Guru", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
								new String[] {
									"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
									"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
