...
/**
 * Penyusun/penyaji laporan untuk laporan rekap jumlah mahasiswa angkatan. Kelas ini mengubah data
 ...
 * fakultas}, {@code Combobox jurusan}, {@code Combobox program}, {@code Decimalbox tahunAngkatan}, {@code
 * Toolbar toolbar}, {@code Combobox tahunAjaran}, {@code Combobox ganjilGenap}; inisialisasi/lifecycle ({@code
 ...
 * @see MyWindow
 */
public class LaporanRekapJumlahMahasiswaAngkatan extends MyWindow {
	...
	public LaporanRekapJumlahMahasiswaAngkatan() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Rekap Jumlah Mahasiswa Angkatan", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanRekapJumlahMahasiswaAngkatan(String title, String border, boolean closable) throws Exception {
	...
	private void init() {
		...
		// Tinggi PASTI 2000px agar Borderlayout tidak kolaps ke 0px saat berada di dalam tabpanel
		// ber-min-height (rantai height:100% tidak menghasilkan tinggi pasti) → konten tab tampil.
		...
		}, "rekap_jumlah_mahasiswa_angkatan_semua", null, new EventListener() {
	...
	protected void generateDataDanImageAlbum(Label label) {
		...
		List<Long> dataMhs = HibernateUtil.currentSession().createCriteria(Mahasiswa.class).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
				.setProjection(Projections.property("id")).add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))).list();
		...
		for (Long generalValueObjectid : dataMhs) {
			...
			try {
				...
				e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanRekapJumlahMahasiswaAngkatan.java:513");
				PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Jumlah Mahasiswa Angkatan", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
					new String[] {
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(),
						"rekap_jumlah_mahasiswa_angkatan_semua", ais.ui.util.WaktuUtil.getDate(), toolbar);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanRekapJumlahMahasiswaAngkatan.java:565");
					PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Rekap Jumlah Mahasiswa Angkatan", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
						new String[] {
