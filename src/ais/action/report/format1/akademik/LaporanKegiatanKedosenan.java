...
/**
 * Penyusun/penyaji laporan untuk laporan kegiatan kedosenan. Kelas ini mengubah data domain
 ...
 * @see MyWindow
 */
public class LaporanKegiatanKedosenan extends MyWindow {
	...
	public LaporanKegiatanKedosenan() {
		super();
		try {
			init();
		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Kegiatan Kedosenan", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanKegiatanKedosenan(String title, String border, boolean closable) throws Exception {
	...
	private void init() throws Exception {
		...
		}, "Angka_Kredit_Kegiatan_Dosen", null, new EventListener() {
	...
	public static Map generateParameter(Dosen dosen, Date tanggal) throws Exception {
		...
		String code = dosen.getNama() + "\n" + dosen.getNidn() + "\n" + Common.dateFormat5.get().format(WaktuUtil.getDate());

		File myfilebarcode1 = new File(Common.ambilREAL_PATH_REPORT() + "/crcode_" + dosen.getId() + ".png");

		BarcodeCommon.generateCRCode(code, myfilebarcode1);
		parameters.put("cr_code", myfilebarcode1.getAbsolutePath());
		parameters.put("qr_code", Common.desEncrypter.get().encrypt(Dosen.class.getName() + ":" + dosen.getId()));
	...
	public void onKHS(Event event) throws Exception {
		...
		try {
			...
			File file = Report.generateFileReportWithProgress(Report.PDF,
					generateParameter((Dosen) bandboxDosen.getAttribute("dosen"), tanggal.getValue()),
					"Angka_Kredit_Kegiatan_Dosen", ais.ui.util.WaktuUtil.getDate(), toolbar);
			CommonReport.tampilkanReportPDF(center, file);

		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Kegiatan Kedosenan", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
					new String[] {
