...
/**
 * Penyusun/penyaji laporan untuk laporan jadwal ujian. Kelas ini mengubah data domain menjadi
 ...
 * @see MyWindow
 */
public class LaporanJadwalUjian extends MyWindow {
	...
	public LaporanJadwalUjian() {
		...
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Jadwal Ujian", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanJadwalUjian(String title, String border, boolean closable) throws Exception {
	...
	private void init() {
		...
		}, "format1/laporan_jadwal_ujian", null, new EventListener() {
		...
		try {
			...
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanJadwalUjian.java:268");
			PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Jadwal Ujian", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
				new String[] {
	...
	private Map generateParameter() throws Exception {
		...
		// Alasan sederhana (untuk pengguna awam) mengapa jadwal ujian kosong; ditampilkan di laporan
		// saat tidak ada data. Kosong ("") bila jadwal ADA (printWhen di JRXML menyembunyikannya).
	...
	/** Alasan (bahasa sederhana) mengapa jadwal ujian kosong; ditampilkan di laporan saat tak ada data. */
	...
	protected void generateDataDanImageAlbum(final Label label) throws Exception {
		...
		// 1. EKSTRAKSI VARIABEL UI (Wajib dilakukan di Thread Utama agar tidak bentrok)
		Jurusan myJurusan = (Jurusan) (jurusan.getSelectedItem() == null || jurusan.getSelectedItem().getValue() == null
				? null
				: jurusan.getSelectedItem().getValue());
		Fakultas myFakultas = (Fakultas) (fakultas.getSelectedItem() == null
		...
		Object valTahunAkademik = tahunAkademik.getSelectedItem() == null ? null
				: tahunAkademik.getSelectedItem().getValue();
		Object valGenapGanjil = genapGanjil.getSelectedItem() == null ? null : genapGanjil.getSelectedItem().getValue();
		Object valProgram = searchprogram.getSelectedItem() == null ? null : searchprogram.getSelectedItem().getValue();

		Kelas kel = (Kelas) kelas.getAttribute("kelas");
		String valKelas = kel != null ? kel.getNama() : null;
		boolean isEkstra = ekstrakurikuler.isChecked();

		Session session = null;
		List<Long> pertemuansIds = new ArrayList<Long>();

		// 2. MANAJEMEN SESSION UTAMA & PENGAMBILAN ID SAJA (Efisiensi Memori)
		try {
			...
			Criteria criteria = session.createCriteria(Pertemuan.class)
					...
					.add(valTahunAkademik == null ? Restrictions.sqlRestriction("true")
							: Restrictions.eq("perkuliahan.tahunAjaran", valTahunAkademik))
					.add(valGenapGanjil == null ? Restrictions.sqlRestriction("true")
							: valGenapGanjil.equals(Perkuliahan.SP)
									? Restrictions.eq("perkuliahan.statusSemesterPendek", Perkuliahan.SEMESTER_PENDEK)
									: Restrictions.in("perkuliahan.semester",
											valGenapGanjil.equals(Perkuliahan.GANJIL) ? Common.ganjil : Common.genap))

					.add(valProgram == null ? Restrictions.sqlRestriction("true")
							: Restrictions.eq("perkuliahan.program", valProgram))
					.add(valKelas == null ? Restrictions.sqlRestriction("true")
							: Restrictions.eq("perkuliahan.kelas", valKelas))
					...
					.setProjection(Projections.property("id")); // Cukup tarik ID agar ringan

			pertemuansIds = criteria.list();

			// Alasan sederhana bila jadwal ujian kosong/belum dijadwal (helper reuse), selagi session aktif.
			alasanKosong = ais.action.report.helper.DiagnosaJadwalUjianHelper.alasan(session,
					...
					valTahunAkademik, valGenapGanjil, valProgram, valKelas, isEkstra, statusPertemuan);
		} catch (Exception e) {
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanJadwalUjian.java:395");
			PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Jadwal Ujian", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
				new String[] {
					"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
					"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
			...
			if (session != null && session.isOpen()) {
				try {
					session.close();
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanJadwalUjian.java:400");
					PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Jadwal Ujian", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
						new String[] {
							"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
							"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
		...
		// 3. SETUP ARRAY ORDERING & MULTI-THREADING (ZK PUSH)
		final List<Long> finalPertemuanIds = pertemuansIds;
		// Gunakan Array of Lists karena 1 Pertemuan bisa memiliki hingga 4
		// Pengawas/Pegawai
		...
		ParallelTaskExecutor.process(listIndex, ParallelTaskExecutor.getDefaultReportMaxThreads(), new ParallelTaskExecutor.Task<Integer>() {
			...
			public void execute(final Integer idx) throws Exception {
				...
				try {
					// Buka Sesi Database Lokal Khusus untuk Thread Ini
					...
					// Update Progress Bar UI (Aman Lintas Thread)
					...
					if (label != null && desktop != null) {
						if (currentCount % 5 == 0 || currentCount == size) {
							...
							} catch (Exception ex) { ais.common.ErrorAuditUtil.record(ex, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanJadwalUjian.java:457");
							}
						}
					}
					...
					for (Pegawai pegawai : pegawais) {
						...
						// EKSEKUSI LAZY LOAD SANGAT AMAN KARENA BERADA DI LOCAL THREAD SESSION
					...
					// AMANKAN URUTAN LAPORAN (Flattening Structure)
					orderedMaps[idx] = resultsForPertemuan;

				} catch (Exception e) {
					e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanJadwalUjian.java:625");
					PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Jadwal Ujian", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
						new String[] {
							"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
							"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
					...
					// PENTING: Sesi lokal thread harus ditutup untuk mencegah Memory Leak
					if (threadSession != null && threadSession.isOpen()) {
						try {
							threadSession.close();
						} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanJadwalUjian.java:631");
							PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Jadwal Ujian", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
								new String[] {
									"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
									"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
		...
		// 5. GABUNGKAN HASIL (Sesuai Urutan Asli dari Database)
		this.maps = new ArrayList<Map>();
	...
	public void onCetak(Event event) throws Exception {
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(), "format1/laporan_jadwal_ujian",
		...
		ais.common.AsyncTaskManager.jalankanDenganPush(desktop, new Runnable() {
			...
			public void run() {
				try {
					...
					e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanJadwalUjian.java:678");
					PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Jadwal Ujian", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
						new String[] {
