...
public class LaporanRekapitulasiKelas extends MyWindow {
	...
	private void init() throws Exception {
		...
		}, "Rekap_kelas", null, new EventListener() {
	...
	public void onLaporanPerkuliahan(Event event) throws Exception {
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(), "Rekap_kelas",
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanRekapitulasiKelas.java:368");
	...
	protected void generateDataDanImageAlbum(Label label) {
		...
		TreeMap<String, KrsMahasiswa> urutkanKelas = new TreeMap<String, KrsMahasiswa>();
		for (Long generalValueObjectid : dataMhs) {
			...
			if (mahasiswa != null) {
				label.setValue("Populate data kelas -> " + mahasiswa + " ("
				...
				if (krsMahasiswa != null && krsMahasiswa.getKelas() != null
						&& !krsMahasiswa.getKelas().trim().isEmpty()) {

					if (kelas == null || (kelas.getNama() != null && krsMahasiswa.getKelas().equals(kelas.getNama()))) {
						urutkanKelas.put(krsMahasiswa.getKelas().trim() + "_____" + generalValueObjectid, krsMahasiswa);
					}
				}
			}
		}
		...
		index = 0;
		for (String kel : urutkanKelas.keySet()) {

			try {
				KrsMahasiswa krsMahasiswa = urutkanKelas.get(kel);
				...
				if (fak == null || fak.getId().equals(mahasiswa.getJurusan().getFakultas().getId())) {

					if (jur == null || jur.getId().equals(mahasiswa.getJurusan().getId())) {

						if (kelaminA == null
								|| (mahasiswa.getKelamin() != null && kelaminA.equals(mahasiswa.getKelamin()))) {
										...
										if (tahunangkatan == null || tahunangkatan <= mahasiswa.getTahunangkatan()) {
											...
											if (tahunangkatanSd == null
												...
												Integer semester = Common.getSemester(mahasiswa.getTahunangkatan(), ta,
														jenisSemester, mahasiswa.getPindahKeKampusIniMasukSemester(),
												...
												if (selectedStatusMahasiswa == null || (historyStatusMahasiswa != null
													...
													Common.insertProperty(BiodataMahasiswa.class, biodataMahasiswa, map,
															"biodata");
													...
													map.put("fakultas_id",
															mahasiswa.getJurusan().getFakultas().getId());
													map.put("fak", mahasiswa.getJurusan().getFakultas().getNama());
													...
													map.put("dosen_pa",
															krsMahasiswa == null || krsMahasiswa.getDosenPa() == null
																	? ""
																	: krsMahasiswa.getDosenPa().getNama());
													map.put("nip_dosen_pa",
															krsMahasiswa == null || krsMahasiswa.getDosenPa() == null
																	? ""
																	: krsMahasiswa.getDosenPa().getCode());
													map.put("nidn_dosen_pa",
															krsMahasiswa == null || krsMahasiswa.getDosenPa() == null
																	? ""
													...
													map.put("nama_dekan",
															mahasiswa.getJurusan().getFakultas().getDekan() == null ? ""
																	: mahasiswa.getJurusan().getFakultas().getDekan()
																			.getNama());
													map.put("nip_dekan",
															mahasiswa.getJurusan().getFakultas().getDekan() == null ? ""
																	: mahasiswa.getJurusan().getFakultas().getDekan()
													...
													map.put("nama_pudek1",
															mahasiswa.getJurusan().getFakultas().getPudek1() == null
																	? ""
																	: mahasiswa.getJurusan().getFakultas().getPudek1()
																			.getNama());
													map.put("nip_pudek1",
															mahasiswa.getJurusan().getFakultas().getPudek1() == null
																	? ""
													...
													map.put("nama_pudek2",
															mahasiswa.getJurusan().getFakultas().getPudek2() == null
																	? ""
																	: mahasiswa.getJurusan().getFakultas().getPudek2()
																			.getNama());
													map.put("nip_pudek2",
															mahasiswa.getJurusan().getFakultas().getPudek2() == null
																	? ""
													...
													map.put("nama_pudek3",
															mahasiswa.getJurusan().getFakultas().getPudek3() == null
																	? ""
																	: mahasiswa.getJurusan().getFakultas().getPudek3()
																			.getNama());
													map.put("nip_pudek3",
															mahasiswa.getJurusan().getFakultas().getPudek3() == null
																	? ""
													...
													map.put("nama_kajur",
															mahasiswa.getJurusan().getGrupJurusan() == null || mahasiswa
																	.getJurusan().getGrupJurusan().getKajur() == null
																			? ""
													...
													Judisium judisium = Common.hitungJudisium(mahasiswa, semester,
															null);
													...
													java.time.LocalDate dt = java.time.LocalDate.parse(ActualDate,
															formatter);
													...
													LocalDate now = new LocalDate(mahasiswa.getTanggalLulus() == null
															? ais.ui.util.WaktuUtil.getDate()
															: mahasiswa.getTanggalLulus());
													...
													try {
														...
														map.put("masa_studi",
																period.getYears() + " tahun, " + period.getMonths()
																		+ " bulan, " + period.getDays() + " hari. ");

													} catch (Exception e) {
														e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanRekapitulasiKelas.java:690");
														PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekapitulasi Kelas", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
															new String[] {
													...
													if (mahasiswa.getTanggalLulus() == null) {
														...
														map.put("bulan_satuan_lulus_en",
																mahasiswa.getTanggalLulus() == null ? ""
																		: Common.monthFormat2En.get()
																				.format(mahasiswa.getTanggalLulus()));
														map.put("tahun_satuan_lulus_en", tahun);

														map.put("tanggal_satuan_lulus", tanggal_tgl);
														map.put("bulan_satuan_lulus",
																mahasiswa.getTanggalLulus() == null ? ""
																		: Common.monthFormat2.get()
																				.format(mahasiswa.getTanggalLulus()));
													...
													if (biodataMahasiswa != null) {
														...
														if (!Common
																.checkIsStringNull(biodataMahasiswa.getKelurahan())) {
													...
													System.out.print("Kelas : " + krsMahasiswa.getKelas());
				...
				e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanRekapitulasiKelas.java:854");
				PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekapitulasi Kelas", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
					new String[] {
						"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
						"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
