...
/**
 * Penyusun/penyaji laporan untuk laporan penilaian oleh dosen per dosen window. Kelas ini mengubah
 ...
 * @see MyWindow
 */
public class LaporanPenilaianOlehDosenPerDosenWindow extends MyWindow {
	...
	public LaporanPenilaianOlehDosenPerDosenWindow() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Penilaian Oleh Dosen Per Dosen Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanPenilaianOlehDosenPerDosenWindow(String title, String border, boolean closable) throws Exception {
	...
	private void init() {
		...
		MyTabConfig tab1 = new MyTabConfig("Laporan Per Perkuliahan");
		tab1.setParent(tabs);

		MyTabConfig tab2 = new MyTabConfig("Laporan Per Dosen");
		tab2.setParent(tabs);
		...
		tab2.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				if (tabpanel51.getChildren().size() == 0) {
					LaporanPenilaianPerDosenWindow laporanIjazahAkademik = new LaporanPenilaianPerDosenWindow();
		...
		}, "rekap_koreksi_nilai_dosen", null, new EventListener() {
	...
	protected void generateDataDanImageAlbum(Label label) {
		...
		try {
			...
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanPenilaianOlehDosenPerDosenWindow.java:541");
			PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Penilaian Oleh Dosen Per Dosen Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
				new String[] {
		...
		for (Perkuliahan perkuliahan : perkuliahans) {
			...
			try {
							...
							if (d != null) {
								...
								uts += formatNilai.getNama().trim().equalsIgnoreCase("UTS")
										|| (formatNilai.getJenisEvaluasi() != null
												&& formatNilai.getJenisEvaluasi().getNama() != null && formatNilai
														.getJenisEvaluasi().getNama().trim().equalsIgnoreCase("UTS"))
												&& verify ? 1 : 0;
				...
				map.put("obe", perkuliahan.getKurikulum() == null ? false : perkuliahan.getKurikulum().apakahObe(
				...
				for (Pertemuan pertemuan : pertemuans) {
					if (pertemuan.getStatusPertemuan() != null && ((pertemuan.getFormatNilai() != null
						...
						tgs = ((Number) session.createCriteria(TugasKelompok.class).add(Restrictions.ne("judul", ""))
								.add(Restrictions.isNotNull("judul"))
								.add(Restrictions.eq("pertemuan", pertemuan.getId()))
								.setProjection(Projections.rowCount()).uniqueResult()).intValue();
				...
				map.put("utsUjianPg", utsUjianPg);
				map.put("utsUjianEssay", utsUjianEssay);
				map.put("uasUjianPg", uasUjianPg);
				map.put("uasUjianEssay", uasUjianEssay);

				map.put("utsTugas", utsTugas);
				map.put("uasTugas", uasTugas);

				map.put("utsUjian", utsUjianPg + utsUjianEssay);
				map.put("uasUjian", uasUjianPg + uasUjianEssay);
				...
				for (Dosen dsn : perkuliahan.populateDosenBuNama()) {
					try {
						...
						e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanPenilaianOlehDosenPerDosenWindow.java:776");
						PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Penilaian Oleh Dosen Per Dosen Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
							new String[] {
								"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
								"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
				...
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanPenilaianOlehDosenPerDosenWindow.java:785");
//					Common.tampilErrorJikaAdmin(e);
				}
				...
				try {
					nilai_honor = Double.parseDouble(defaultHonor);
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanPenilaianOlehDosenPerDosenWindow.java:795");
//					Common.tampilErrorJikaAdmin(e);
				}
				...
				for (int p = 1; p <= 16; p++) {
					defaultHonor = Common.getKonfigurasi("default_nilai_honor_pertemuan", "0", perkuliahan.getProgram(),
							perkuliahan.getJurusan(), p + "").getNilai().trim();
					nilai_honor = 0.0;
					try {
						nilai_honor = Double.parseDouble(defaultHonor);
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanPenilaianOlehDosenPerDosenWindow.java:806");
//						Common.tampilErrorJikaAdmin(e);
					}
					map.put("nilai_pertemuan_" + p, nilai_honor);
				}
				...
				try {
					nilai_honor = Double.parseDouble(defaultHonor);
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanPenilaianOlehDosenPerDosenWindow.java:817");
//					Common.tampilErrorJikaAdmin(e);
				}
				map.put("nilai_pertemuan", nilai_honor);

				List<Dosen> dosensNama = perkuliahan.populateDosenBuNama();
				String namaDosen = "";
				String dosenIds = "";
				for (Dosen d : dosensNama) {
					namaDosen += namaDosen.isEmpty() ? d.getNama() : ", " + d.getNama();
					dosenIds += namaDosen.isEmpty() ? d.getId() : "," + d.getId();
				}
				map.put("jumlah_dosen", dosensNama.size());
				...
				e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanPenilaianOlehDosenPerDosenWindow.java:843");
				PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Penilaian Oleh Dosen Per Dosen Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
					new String[] {
						"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
						"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
	...
	public void onLaporanAngketDosenPerDosen(Event event) throws Exception {
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(), "rekap_koreksi_nilai_dosen",
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/akademik/LaporanPenilaianOlehDosenPerDosenWindow.java:874");
					PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Penilaian Oleh Dosen Per Dosen Window", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
						new String[] {
