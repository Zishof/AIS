...
/**
 * Penyusun/penyaji laporan untuk laporan angket dosen per dosen saja window. Kelas ini mengubah
 ...
 * @see MyWindow
 */
public class LaporanAngketDosenPerDosenSajaWindow extends MyWindow {
	...
	public LaporanAngketDosenPerDosenSajaWindow() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Angket Dosen Per Dosen Saja Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanAngketDosenPerDosenSajaWindow(Dosen dsn) {
		super();
		this.dsn = dsn;
		try {
	...
	public LaporanAngketDosenPerDosenSajaWindow(String title, String border, boolean closable) throws Exception {
...
//		semesterAbsensi.appendChild(comboitem);
		...
		// tsb): skor laporan ini GABUNGAN (pooled) lintas semua Program Studi yang diampu dosen, berbeda
		// dari laporan "Rangking per Program Studi" yang memisahkan skor per Program Studi. Mencegah
		// user salah menyimpulkan perbedaan angka antar kedua laporan sebagai kesalahan hitung.
		// Penjelasan LENGKAP (rumus + langkah validasi) tercetak otomatis di halaman pertama PDF yang
		// dihasilkan (lihat blok "KETERANGAN METODOLOGI PERHITUNGAN SKOR" pada judul laporan, diisi via
		// parameter keterangan_metodologi pada generateParameter()).
		Common.initKeteranganHtml(rows, AngketDosenMetodologiUtil.keteranganPanelFull());
		...
		}, "rekap_angket_dosen_per_dosen", null, new EventListener() {
	...
	private Map generateParameter() throws Exception {
		...
		// parameter Jasper agar berkas rekap_angket_dosen_per_dosen.jrxml TIDAK menyimpan redaksi secara
		// literal -- perubahan wording cukup di satu tempat (class util), tak perlu edit jrxml.
		parameters.put("keterangan_metodologi", AngketDosenMetodologiUtil.keteranganJrxmlFull());
	...
	public void onLaporanAngketDosenPerDosen(Event event) throws Exception {
		...
		Common.createDefaultTimer(new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(), "rekap_angket_dosen_per_dosen",
