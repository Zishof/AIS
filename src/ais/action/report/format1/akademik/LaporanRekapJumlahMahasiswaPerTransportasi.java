...
/**
 * Penyusun/penyaji laporan untuk laporan rekap jumlah mahasiswa per transportasi. Kelas ini
 ...
 * searchfakultas}, {@code Combobox searchjurusan}, {@code Combobox tahunAjaran}, {@code Combobox ganjilGenap},
 * {@code Combobox status}, {@code File file}, {@code Combobox program}; inisialisasi/lifecycle ({@code init()},
 * {@code initSpreadsheet()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut
 * di atas.</p>
 ...
 * @see MyWindow
 */
public class LaporanRekapJumlahMahasiswaPerTransportasi extends MyWindow {
	...
	public LaporanRekapJumlahMahasiswaPerTransportasi() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Rekap Jumlah Mahasiswa Per Transportasi", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanRekapJumlahMahasiswaPerTransportasi(String title, String border, boolean closable) {
	...
	private void init() throws Exception {
		...
		print.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akademik/LaporanRekapJumlahMahasiswaPerTransportasi.java:184");
					PesanFormalHelper.tampilkanGagalException("pembuatan berkas Excel Laporan Rekap Jumlah Mahasiswa Per Transportasi", "Sistem mengalami kendala teknis saat menyusun berkas Excel laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap atau format datanya tidak sesuai dengan yang diharapkan oleh template ekspor.", e,
						new String[] {
	...
	private void initSpreadsheet() throws Exception {
		...
		final StatusMahasiswa statusMahasiswa = (StatusMahasiswa) (status.getSelectedItem() == null
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				List<AlatTransportasiMahasiswa> alatTransportasiMahasiswaes = session
						.createCriteria(AlatTransportasiMahasiswa.class)
						.add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true)))
						.addOrder(Order.asc("nama")).list();
				alatTransportasiMahasiswaes.add(null);
				int qtyLk = 2;

				for (AlatTransportasiMahasiswa alatTransportasiMahasiswa : alatTransportasiMahasiswaes) {

					rowhead.createCell(qtyLk).setCellValue(
							alatTransportasiMahasiswa == null ? "Tidak dipilih" : alatTransportasiMahasiswa.getNama());
							...
							try {
								...
								Integer jmlLaki = ((Number) session
										.createSQLQuery("select count(*) as jumlah from history_status_mahasiswa a "
												...
												+ (alatTransportasiMahasiswa == null
														? " and c.alat_transportasi_mahasiswa is null "
														: " and c.alat_transportasi_mahasiswa="
