...
public class LaporanRekapitulasiMahasiswaYangTidakMengambilMatakuliah extends MyWindow {
	...
	public LaporanRekapitulasiMahasiswaYangTidakMengambilMatakuliah() {
		...
		try {
			...
			/**
			 * Event listener lokal milik {@link LaporanRekapitulasiMahasiswaYangTidakMengambilMatakuliah}. Kelas ini
			 * menangani event untuk komponen induk dan meneruskan pekerjaan domain ke method/service yang sudah tersedia.
			 *
			 * <p><b>Scope:</b> setiap instance terikat pada instance {@link
			 * LaporanRekapitulasiMahasiswaYangTidakMengambilMatakuliah} dan dapat mengakses state kelas induk. Jangan
			 ...
			 *
			 * @see LaporanRekapitulasiMahasiswaYangTidakMengambilMatakuliah
			 */
	...
	private void init() throws Exception {
		...
		row = new MyFormRow();
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig("Matakuliah yg tidak diambil *"));
		...
		}, "Rekap_Data_Mahasiswa_Tidak_Matakuliah", null, new EventListener() {
	...
	protected Map generateParameter() throws Exception {
		...
		parameters.put("kode_Matakuliah", myMatakuliah.getKode());
			...
			public void onEvent(Event arg0) throws Exception {
				try {
					...
					File file = Report.generateFileReportWithProgress(Report.PDF, parameters,
							"Rekap_Data_Mahasiswa_Tidak_Matakuliah", ais.ui.util.WaktuUtil.getDate(),
							toolbar);
