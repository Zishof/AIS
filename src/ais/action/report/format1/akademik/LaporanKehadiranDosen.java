...
/**
 * Penyusun/penyaji laporan untuk laporan kehadiran dosen. Kelas ini mengubah data domain menjadi
 ...
 * Toolbar toolbar}, {@code Textbox matakuliah}, {@code AmbilDataKelasBanbox kelas}; inisialisasi/lifecycle
 * ({@code init()}); pelaporan/ekspor ({@code onCetak()}); operasi domain lain ({@code generateParameter()}).
 ...
 * @see MyWindow
 */
public class LaporanKehadiranDosen extends MyWindow {
	...
	private MyCheckboxConfig termasukTglRealisasi;

	private MyCheckboxConfig mk;

	private Combobox tetap;

	private ArrayList<MyCheckboxConfig> listJenisPertemuan;

	public LaporanKehadiranDosen() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Kehadiran Dosen", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanKehadiranDosen(String title, String border, boolean closable) throws Exception {
	...
	private void init() {
		...
		row.appendChild(this.mk = new MyCheckboxConfig("Tampilkan per matakuliah"));
		mk.setChecked(true);
		...
		row.setParent(rows);
		row.appendChild(new ais.ui.util.MyLabelConfig(""));
		row.appendChild(termasukTglRealisasi = new MyCheckboxConfig("Termasuk tanggal realisasi"));
		...
		}, "LaporanAbsensiDosen", null, new EventListener() {
	...
	private Map generateParameter() throws Exception {
		...
		List<Map<String, Serializable>> maps = CommonReportHelper.generateParameterMapAbsensiRinciDosen(myFakultas,
				...
				sampai.getValue(), hanyaYgStatusMasuk.isChecked(), termasukTglRealisasi.isChecked(), mk.isChecked(),
	...
	public void onCetak(Event event) {
		...
		try {
			...
			File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(), "LaporanAbsensiDosen",
					ais.ui.util.WaktuUtil.getDate(), toolbar);
			CommonReport.tampilkanReportPDF(center, file);

		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Kehadiran Dosen", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
					new String[] {
