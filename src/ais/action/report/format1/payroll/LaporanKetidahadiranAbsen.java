...
/**
 * Penyusun/penyaji laporan untuk laporan ketidahadiran absen. Kelas ini mengubah data domain
 ...
 * @see MyWindow
 */
public class LaporanKetidahadiranAbsen extends MyWindow {
	...
	private int currTahun;

	private Desktop desktop;

	public LaporanKetidahadiranAbsen() {
		super();
		try {
			initKHS();
			init();
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Ketidahadiran Absen", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanKetidahadiranAbsen(Pegawai pegawai) {
		super();
	...
	public LaporanKetidahadiranAbsen(String title, String border, boolean closable) throws Exception {
	...
	private void init() throws Exception {
		...
		tahun = new Combobox();
		currTahun = ais.ui.util.WaktuUtil.getCalendar().get(Calendar.YEAR);
		for (int i = currTahun - 10; i <= currTahun; i++) {
		...
		}, "Rekapitulasi_Ketidakhadiran_Tahunan", null, new EventListener() {
	...
	private Map generateParameter() throws Exception {
		...
		if (!selectedtahun.equals(currTahun)) {
	...
	public void generateDataDanImageAlbum(final Label label) throws Exception {
		...
		try {
			...
			e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/payroll/LaporanKetidahadiranAbsen.java:410");
			throw e;
		} finally {
		...
		ParallelTaskExecutor.process(listIndex, ParallelTaskExecutor.getDefaultReportMaxThreads(), new ParallelTaskExecutor.Task<Integer>() {
			...
			public void execute(final Integer idx) throws Exception {
				...
				// UI Update (Aman dari Freeze Lintas Thread)
				...
				if (label != null && desktop != null) {
					if (currentCount % 5 == 0 || currentCount == size) {
						...
						} catch (Exception ex) { ais.common.ErrorAuditUtil.record(ex, "auto-audit(empty-catch) src/ais/action/report/format1/payroll/LaporanKetidahadiranAbsen.java:473");
						}
					}
				}
				...
				try {
					...
					for (Statusabsensi statusabsensi : statusabsensis.values()) {
					...
					// Mengambil Cuti milik Pegawai secara Instan O(1)
					...
					while (calendar.getTime().before(s.getTime())) {
						...
						// Pointer Calendar wajib di-add sebelum `continue`
						...
						if (cutiDanIzin != null) {
							...
							if (cutiDanIzin.getStatusabsensi() != null) {
								Calendar cCutiS = ais.ui.util.WaktuUtil.getCalendar();
								cCutiS.setTime(cutiDanIzin.getMulai());

								String strSampaiCutiS;
								strSampaiCutiS = Common.dateFormat8.get().format(cutiDanIzin.getSampai());

								while (true) {
									boolean matchTglS = false;
									matchTglS = Common.dateFormat8.get().format(cCutiS.getTime())
											.equals(strSampaiCutiS);

									if (!(matchTglS || cCutiS.getTime().before(cutiDanIzin.getSampai())))
										break;

									if (Common.isHolidayMerahDanAtauHariLibur(cCutiS.getTime(), pegawai)) {
										cCutiS.add(Calendar.DATE, 1);
										continue;
									}

									int thnCutiS = cCutiS.get(Calendar.YEAR);
									if (selectedtahun == null || thnCutiS == selectedtahun.intValue()) {
										Date t = cCutiS.getTime();
										String formatT;
										formatT = Common.simpleDateFormat2.get().format(t);

										String c = cutiDanIzin.getStatusabsensi().getKode() + "=" + formatT;
										keterangandata += keterangandata.isEmpty() ? c : "," + c;

										List<String> ssList = keterangandatamap
												.get(cutiDanIzin.getStatusabsensi().getKode());
										if (ssList == null) {
						...
						if (skh.ambilMasukjam() == null && (cutiDanIzin == null || !cutiDanIzin.getSetujui())) {
							if (tanggal.before(sekarang)) {
								String formatTglTh;
								formatTglTh = Common.simpleDateFormat2.get().format(tanggal);

								List<String> ssList = keterangandatamap.get("TH");
								if (ssList == null) {
									ssList = new ArrayList<String>();
									keterangandatamap.put("TH", ssList);
								}
								ssList.add(formatTglTh);
								keterangandata += keterangandata.isEmpty() ? formatTglTh : "," + formatTglTh;
								alpa++;
							}
						}
						...
						for (String ss : d) {
							try {
								if (!ss.isEmpty() && !ss.startsWith("700")) {
									...
									tm = Common.timeFormat.get().format(dss);
									tm2 = Common.timeFormat2.get().format(dss);

									mapS.put("log_waktu_" + indexLog, tm);
									mapS.put("log_waktu1_" + indexLog, tm2);
									indexLog++;
								}
							} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/payroll/LaporanKetidahadiranAbsen.java:842");
								PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Ketidahadiran Absen", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
									new String[] {
					...
					for (Object o : ConstantValues.ambilBerdasarClass(Statusabsensi.class).values()) {
						...
						} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/payroll/LaporanKetidahadiranAbsen.java:906");
						}
					}
					...
					e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/payroll/LaporanKetidahadiranAbsen.java:956");
					PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Ketidahadiran Absen", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
						new String[] {
							"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
							"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(),
						"Rekapitulasi_Ketidakhadiran_Tahunan", ais.ui.util.WaktuUtil.getDate(), null, toolbar);
		...
		ais.common.AsyncTaskManager.jalankanDenganPush(desktop, new Runnable() {
			...
			public void run() {
				try {
					...
					e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/payroll/LaporanKetidahadiranAbsen.java:1007");
					PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Ketidahadiran Absen", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
						new String[] {
