...
/**
 * Penyusun/penyaji laporan untuk laporan absensi pegawai detail. Kelas ini mengubah data domain
 ...
 * tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code Checkbox haris}, {@code Combobox
 * bulan}, {@code Center center}, {@code Toolbar toolbar}, {@code Combobox tahun}, {@code AmbilDataPegawaiBanbox
 * searchparent}, {@code Map parameters}, {@code AmbilDataSatuanKerjaBanbox searchSatker}; inisialisasi/lifecycle
 * ({@code initKHS()}, {@code init()}); operasi domain lain ({@code generateParameter()}, {@code
 * generateDataDanImageAlbum()}, {@code inisialisasiRelasiLaporan()}, {@code onKHS()}). Bagian lain dari kontrak
...
public class LaporanAbsensiPegawaiDetail extends MyWindow {
	...
	public void onKHS(Event event) throws Exception {
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				// KE-FIX: sebelumnya tidak ada try/catch di sini -- kegagalan pembuatan laporan
				// (mis. Report$ReportGenerationException) tembus mentah ke ZK (UiException teknis
				// yang membingungkan pengguna payroll/HR), padahal method ini punya pola pesan
				// ramah yang sama persis di generateDataDanImageAlbum() tepat di bawah (lihat
				// PesanFormalHelper.tampilkanGagalException). Samakan penanganannya di sini.
				try {
