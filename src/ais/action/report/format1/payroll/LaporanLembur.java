...
/**
 * Penyusun/penyaji laporan untuk laporan lembur. Kelas ini mengubah data domain menjadi bentuk
 ...
 * @see MyWindow
 */
public class LaporanLembur extends MyWindow {
	...
	public LaporanLembur() {
		...
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Lembur", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanLembur(Pegawai pegawai) {
		super();
	...
	public LaporanLembur(String title, String border, boolean closable) throws Exception {
	...
	private void init() throws Exception {
		...
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/payroll/LaporanLembur.java:215");
			PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Lembur", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
				new String[] {
		...
		}, "Laporan_Lembur", null, new EventListener() {
	...
	private Map generateParameter() throws Exception {
		...
		for (MyCheckboxConfig checkbox : haris) {
			...
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/payroll/LaporanLembur.java:304");
	...
	public void generateDataDanImageAlbum(final Label label) throws Exception {
		...
		try {
			...
			if (!pegawaisAsli.isEmpty()) {
				...
				pengajuanPegawaisSemua = session.createCriteria(PengajuanPegawai.class)
						...
						.add(Restrictions.or(Restrictions.isNull("jenisPengajuanPegawai.masukLembur"),
								Restrictions.eq("jenisPengajuanPegawai.masukLembur", true)))
						.add(Restrictions.or(
		...
		// 2. O(1) PRE-MAPPING PENGELOMPOKAN DATA
		if (cbData == null)
		...
		// PENGAMANAN URUTAN LAPORAN (ORDER PRESERVING)
			...
			public void execute(final Integer idx) throws Exception {
				...
				if (label != null && desktop != null) {
					if (currentCount % 5 == 0 || currentCount == totalPegawai) {
						...
						} catch (Exception ex) { ais.common.ErrorAuditUtil.record(ex, "auto-audit(empty-catch) src/ais/action/report/format1/payroll/LaporanLembur.java:486");
						}
					}
				}

				if (pegawai != null && pegawai.getTipePegawai() != null && !pegawai.getTipePegawai().getMasukLembur()) {
					return;
				}
				...
				try {
					...
					// O(1) Pre-mapping Pengajuan Harian untuk Pegawai Ini
					...
					if (myPengajuans != null) {
						for (PengajuanPegawai pp : myPengajuans) {
							Calendar cSub = ais.ui.util.WaktuUtil.getCalendar();
							cSub.setTime(pp.getWaktu());

							String strWaktuSampai;
							strWaktuSampai = Common.dateFormat8.get().format(pp.getWaktuSampai());

							int ind = 0;
							while (true) {
								boolean matchTgl = false;
								String key83;
								matchTgl = Common.dateFormat8.get().format(cSub.getTime()).equals(strWaktuSampai);
								key83 = Common.dateFormat83.get().format(cSub.getTime());

								if (!(matchTgl || cSub.getTime().before(pp.getWaktuSampai())))
									break;
								...
								pengajuanPegawaisData.put(key83, pp);
								cSub.add(Calendar.DATE, 1);
							}
						}
					}
					...
					for (CutiDanIzin cutiDanIzin : cutiDanIzins) {
						...
						if (cutiDanIzin.getStatusabsensi() != null) {
							String n = cutiDanIzin.getStatusabsensi().getNama();
							m.put(n, (m.get(n) == null ? 0 : m.get(n)) + 1);
						}
					...
					// AMANKAN PARAMETERS (GLOBAL JASPER MAP)
					if (parameters != null) {
					...
					while (cDaily.getTime().before(sDaily.getTime())) {
						...
						// Memajukan pointer tanggal SEBELUM continue
						...
						PengajuanPegawai pp = pengajuanPegawaisData.get(keyTgl);
						if (pp != null && pp.getSatuanKerjaPengaju() != null) {
							Double lembur = jmlJamLemburPerSatker.get(pp.getSatuanKerjaPengaju().getId());
							if (lembur == null)
								lembur = 0.0;
							lembur += lemburDa;
							jmlJamLemburPerSatker.put(pp.getSatuanKerjaPengaju().getId(), lembur);
						}
						...
						if (pp != null) {
							Common.insertProperty(PengajuanPegawai.class, pp, mapS, "pengajuan", 1, "pegawai");
							List<CommonVO> commonVOs = pp.ambilDataParameterTambahan();
						...
						for (String ss : d) {
							...
							} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/payroll/LaporanLembur.java:859");
								PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Lembur", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
									new String[] {
										"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
										"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
						...
						if (adaHadir || cutiDanIzin == null || !cutiDanIzin.getSetujui()) {
							...
							if (skh.getMasukjam() != null && skh.getDetailJenisShiftPegawai() != null
								...
								double jamS = 0.0, jamM = 0.0;

								jamS = Double.parseDouble(
										Common.timeFormat2.get().format(skh.getDetailJenisShiftPegawai().getMulai()));
								jamM = Double.parseDouble(Common.timeFormat2.get().format(skh.getMasukjam()));
					...
					for (Object o : ConstantValues.ambilBerdasarClass(Statusabsensi.class).values()) {
						...
						} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/payroll/LaporanLembur.java:923");
						}
					}
	...
	public void onKHS(Event event) throws Exception {
		...
		final Label label = Common.displayLoadBar(new EventListener() {
			...
			@Override
			public void onEvent(Event arg0) throws Exception {
				File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(), "Laporan_Lembur",
		...
		ais.common.AsyncTaskManager.jalankanDenganPush(desktop, new Runnable() {
			...
			public void run() {
				try {
					...
					e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit src/ais/action/report/format1/payroll/LaporanLembur.java:1037");
					PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Lembur", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
						new String[] {
