...
/**
 * Penyusun/penyaji laporan untuk laporan slip gaji pegawai per orang. Kelas ini mengubah data
 ...
 * {@code Combobox bulan}, {@code Center center}, {@code Toolbar toolbar}, {@code Combobox tahun}, {@code Pegawai
 * pegawai}, {@code Combobox pilihFormat}; inisialisasi/lifecycle ({@code initKHS()}, {@code init()}); operasi
 * domain lain ({@code generateParameter()}, {@code generateDataDanImageAlbum()}, {@code onKHS()}). Bagian lain
 ...
 * @see MyWindow
 */
public class LaporanSlipGajiPegawaiPerOrang extends MyWindow {
	...
	public LaporanSlipGajiPegawaiPerOrang() {
		super();
		try {
			initKHS();
			init();
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Slip Gaji Pegawai Per Orang", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanSlipGajiPegawaiPerOrang(Pegawai pegawai) {
		super();
	...
	public LaporanSlipGajiPegawaiPerOrang(String title, String border, boolean closable) throws Exception {
	...
	private void init() throws Exception {
		...
		}, "payroll/SlipGaji", null, new EventListener() {
	...
	private Map generateParameter() throws Exception {
		...
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/payroll/LaporanSlipGajiPegawaiPerOrang.java:328");
						...
						for (ParameterTambahan parameterTambahan : parameterTambahans) {
							...
							for (String d : spl) {
								...
								if (value[0].trim().equalsIgnoreCase(jenis)) {
									...
									} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/payroll/LaporanSlipGajiPegawaiPerOrang.java:380");
										PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Slip Gaji Pegawai Per Orang", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
											new String[] {
		...
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/payroll/LaporanSlipGajiPegawaiPerOrang.java:392");
			// TODO: handle exception
		}
		parameters.put("maps", generateDataDanImageAlbum(parameters, formatItemGaji));
		return parameters;
	}

	@SuppressWarnings({ "rawtypes" })
	public List generateDataDanImageAlbum(Map maps, FormatItemGaji formatItemGaji) throws Exception {
		...
		ItemGajiPegawaiTreeModel itemGajiPegawaiTreeModel = new ItemGajiPegawaiTreeModel(false, formatItemGaji, pegawai,
				PembayaranGajiPunyaPegawai.ambilMulai(tahun, bulan - 1));
		itemGajiPegawaiTreeModel.populateData(list, maps, calendar.getTime(), bulan, tahun, null);
		return list;
	}
	...
	public void onKHS(Event event) throws Exception {
		...
		try {
			...
			File fileG = fileGaji == null ? null : fileGaji.ambilFile();

			if (fileG != null) {
				toolbar.getParent().setVisible(false);
				CommonReport.tampilkanReportPDF(center, fileG);
			} else {
				File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(), "payroll/SlipGaji",
			...
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Slip Gaji Pegawai Per Orang", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
					new String[] {
