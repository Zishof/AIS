...
/**
 * Penyusun/penyaji laporan untuk laporan apresiasi siswa. Kelas ini mengubah data domain menjadi
 ...
 * {@code Center center}, {@code Toolbar toolbar}, {@code MyDatebox tanggal}, {@code MyDatebox sampai}, {@code
 * Combobox yayasan}, {@code Combobox sekolah}; inisialisasi/lifecycle ({@code init()}); operasi domain lain
 * ({@code generateParameter()}, {@code generateParameter()}, {@code onKHS()}). Bagian lain dari kontrak tetap
 ...
 * @see MyWindow
 */
public class LaporanApresiasiSiswa extends MyWindow {
	...
	public LaporanApresiasiSiswa() {
		super();
		try {
			init();
		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Apresiasi Siswa", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanApresiasiSiswa(String title, String border, boolean closable) throws Exception {
	...
	private void init() throws Exception {
		...
		}, "sekolah/kartu_apresiasi", null, new EventListener() {
	...
	public Map generateParameter() throws Exception {
		...
		List<ApresiasiSiswa> apresiasiSiswas = session.createCriteria(ApresiasiSiswa.class)
		...
		for (ApresiasiSiswa apresiasiSiswa : apresiasiSiswas) {
			Map map = new HashMap();
			Common.insertProperty(ApresiasiSiswa.class, apresiasiSiswa, map, "");

			Double point = 0.0;
			Set<Penghargaan> penghargaans = apresiasiSiswa.getPenghargaans();
			for (Penghargaan penghargaan : penghargaans) {
				point += penghargaan.getPoin();
			}

			Double kredit = 0.0;
			Set<Apresiasi> apresiasis = apresiasiSiswa.getApresiasis();
			for (Apresiasi apresiasi : apresiasis) {
				kredit += apresiasi.getKredit();
			}
			KelasSiswa kelas = Siswa.ambilKelas(apresiasiSiswa.getSiswa(), apresiasiSiswa.getTa());
	...
	public static Map generateParameter(ApresiasiSiswa apresiasiSiswa) throws Exception {
		Calendar calendar = ais.ui.util.WaktuUtil.getCalendar();
		calendar.setTime(apresiasiSiswa.getWaktu());
		calendar.set(Calendar.YEAR, calendar.get(Calendar.YEAR) - 2);
		...
		if (apresiasiSiswa.getSiswa() != null) {
			apresiasiSiswa.getSiswa().putPhoto(parameters);
		}
		Yayasan yayasan = apresiasiSiswa.getYayasan();
		Sekolah sekolah = apresiasiSiswa.getSekolah();

		parameters.put("yayasan", yayasan == null ? "" : yayasan.getNama());
		parameters.put("sekolah", sekolah == null ? "" : sekolah.getNama());
		parameters.put("nama", apresiasiSiswa.getSiswa() == null ? "" : (apresiasiSiswa.getSiswa().getNama()));
		parameters.put("siswa_id", apresiasiSiswa.getSiswa() == null ? 1L : apresiasiSiswa.getSiswa().getId());
		parameters.put("tanggal", tanggal);
		parameters.put("mulai", tanggal);
		parameters.put("sampai", sampai);
		...
		Session session = HibernateUtil.currentSession();
		session.refresh(apresiasiSiswa);
	...
	public void onKHS(Event event) throws Exception {
		...
		try {
			...
			File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(), "sekolah/kartu_apresiasi",
					ais.ui.util.WaktuUtil.getDate(), toolbar);
			CommonReport.tampilkanReportPDF(center, file);

		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Apresiasi Siswa", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
					new String[] {
