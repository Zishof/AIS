...
/**
 * Penyusun/penyaji laporan untuk laporan rekap absen guru piket harian. Kelas ini mengubah data
 ...
 * sampai}, {@code Center center}, {@code Combobox yayasan}, {@code Combobox sekolah}, {@code Combobox searchta},
 * {@code Combobox searchsmt}, {@code Toolbar toolbar}; inisialisasi/lifecycle ({@code init()}); pelaporan/ekspor
 ...
 * @see MyWindow
 */
public class LaporanRekapAbsenGuruPiketHarian extends MyWindow {
	...
	public LaporanRekapAbsenGuruPiketHarian() throws Exception {
		super();
		init();
	}

	public LaporanRekapAbsenGuruPiketHarian(String title, String border, boolean closable) throws Exception {
	...
	public void onCetak(Event event) {
		...
		try {
				...
				public void run() {
					...
					for (Sekolah sekolah : kelases.values()) {
						...
						try {
							...
							List<Guru> gurus = ConstantValues.simpleList(
									session.createCriteria(Guru.class).add(Restrictions.isNotNull("sekolah"))
								...
								for (Guru guru : gurus) {
									...
									if (jamKes.isEmpty()) {
										...
										AbsenGuruPiket absenGuruPiket = myItems
												.get(Common.dateFormat1.get().format(date) + "_" + jamke);
										...
										String mulai = absenGuruPiket.retreiveAbsensiMulai(guru.getId() + "");
										String sampai = absenGuruPiket.retreiveAbsensiSampai(guru.getId() + "");

										Map map = new java.util.HashMap();

										map.put("apakah_dosen", sekolah.getNama());
										map.put("nama_satuan_kerja", "Guru");
										map.put("pegawai", guru.getId());
										map.put("nama", guru.getNama());
										map.put("nip", guru.getKode());
							...
							PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Absen Guru Piket Harian", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
									new String[] {
				...
				public void onEvent(Event arg0) throws Exception {
					...
					if (ais.action.report.helper.LoadingReportUtil.isSelesai(label)) {
						...
						File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(),
								"Laporan_Absensi_Per_Hari_Guru", ais.ui.util.WaktuUtil.getDate(), null, toolbar);
			...
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Rekap Absen Guru Piket Harian", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
					new String[] {
