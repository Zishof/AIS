...
/**
 * Penyusun/penyaji laporan untuk laporan pembelian siswa. Kelas ini mengubah data domain menjadi
 ...
 * @see MyWindow
 */
public class LaporanPembelianSiswa extends MyWindow {
	...
	public LaporanPembelianSiswa() throws Exception {
		super();
		init();
	}

	public LaporanPembelianSiswa(String title, String border, boolean closable) throws Exception {
	...
	private void init() throws Exception {
		...
		vbox.setParent(row);
		vbox.setWidth("100%");
		vbox.appendChild(new ais.ui.util.MyLabelConfig("Toko"));
		vbox.appendChild(toko = new Combobox());
		Common.insertComboDanSemua(toko, "nama", Toko.class, Restrictions.eq("aktif", true));
		toko.setCols(5);
		toko.setReadonly(true);

		Toko currentToko = Common.getCurrentToko();
		if (currentToko != null) {
		...
		MyTabConfig tab1 = new MyTabConfig("Rincian Pembelian");
		tab1.setParent(tabs);

		MyTabConfig tab2 = new MyTabConfig("Pembelian Per Toko");
		tab2.setParent(tabs);

		MyTabConfig tab3 = new MyTabConfig("Rincian Item Pembelian");
		tab3.setParent(tabs);

		MyTabConfig tab4 = new MyTabConfig("Pembelian Per Kasir");
		tab4.setParent(tabs);

		MyTabConfig tab5 = new MyTabConfig("Pembelian Per Kelas");
		tab5.setParent(tabs);
		...
		tabpanel1.setAttribute("report", "sekolah/pembayaran/pembelian_siswa_detail");
		tabpanel1.setParent(tabpanels);
		tabpanel1.setHeight("100%");

		tabpanel1 = new ais.ui.util.MyTabpanel();
		tabpanel1.setVisible(selectedSiswa == null);
		tabpanel1.setAttribute("report", "sekolah/pembayaran/pembelian_siswa");
		tabpanel1.setParent(tabpanels);
		tabpanel1.setHeight("100%");

		tabpanel1 = new ais.ui.util.MyTabpanel();
		tabpanel1.setVisible(selectedSiswa == null);
		tabpanel1.setAttribute("report", "sekolah/pembayaran/pembelian_siswa_per_item_biaya");
		tabpanel1.setParent(tabpanels);
		tabpanel1.setHeight("100%");

		tabpanel1 = new ais.ui.util.MyTabpanel();
		tabpanel1.setVisible(selectedSiswa == null);
		tabpanel1.setAttribute("report", "sekolah/pembayaran/pembelian_siswa_per_kasir");
		tabpanel1.setParent(tabpanels);
		tabpanel1.setHeight("100%");

		tabpanel1 = new ais.ui.util.MyTabpanel();
		tabpanel1.setVisible(selectedSiswa == null);
		tabpanel1.setAttribute("report", "sekolah/pembayaran/pembelian_siswa_per_kelas");
		tabpanel1.setParent(tabpanels);
		tabpanel1.setHeight("100%");
		...
		}, "sekolah/pembayaran/pembelian_siswa_detail", null, new EventListener() {
	...
	private Map generateParameter() throws Exception {
		...
		Toko toko = (Toko) (this.toko.getSelectedItem() == null ? null : this.toko.getSelectedItem().getValue());
		...
		parameters.put("toko", toko == null || toko.getId() == null ? -1L : toko.getId());
		parameters.put("kantin", toko == null || toko.getId() == null ? -1L : toko.getId());
		...
		try {
			...
			if (Common.isMobile()) {
				File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(),
						"sekolah/pembayaran/pembelian_siswa_detail", ais.ui.util.WaktuUtil.getDate(), toolbar);
			...
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Pembelian Siswa", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
					new String[] {
