...
/**
 * Penyusun/penyaji laporan untuk laporan trial balance. Kelas ini mengubah data domain menjadi
 ...
 * AmbilDataSatuanKerjaBanbox searchsatuanKerja}, {@code MyTextbox bukti}, {@code Center centera};
 ...
 * @see MyWindow
 */
public class LaporanTrialBalance extends MyWindow {
	...
	public LaporanTrialBalance() {
		super();
		try {
			init();
		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Trial Balance", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanTrialBalance(String title, String border, boolean closable) throws Exception {
	...
	private void init() throws Exception {
		...
		if (!saldoAwalDefault.isEmpty()) {
			try {
				...
				row.setVisible(true);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/akunting/LaporanTrialBalance.java:182");
		...
		final EventListener cariAkun = new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				Criteria criteria = session.createCriteria(Transaksi.class)
						...
						.add(Restrictions.isNotNull("akun")).setProjection(Projections.groupProperty("akun.id"))
	...
	public void onReport(Event event) {
		...
		try {
			...
			File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(), "akunting/jurnal_tial_balance",
					ais.ui.util.WaktuUtil.getDate(), toolbar);
			CommonReport.tampilkanReportPDF(centera, file);

		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e);
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Trial Balance", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
					new String[] {
