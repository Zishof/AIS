...
/**
 * Penyusun/penyaji laporan untuk laporan denda belum lunas window. Kelas ini mengubah data domain
 ...
 * @see MyWindow
 */
public class LaporanDendaBelumLunasWindow extends MyWindow {
	...
	public LaporanDendaBelumLunasWindow() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Denda Belum Lunas Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanDendaBelumLunasWindow(String title, String border, boolean closable) {
	...
	private void init() {
		...
		print.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				bout.close();
				Filedownload.save(bout.toByteArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "DATA_DENDA_BELUM_LUNAS.xlsx");
			}
		});
		print.setParent(toolbar);
	...
	private void initSpreadsheet() throws Exception {
		...
		List<KembaliPengadaanItemDetail> kembaliPengadaanItemDetails = session
				...
				.add(Restrictions.sqlRestriction("this_.denda>this_.dibayarsejumlah"))
		...
		} catch (Exception e4) { ais.common.ErrorAuditUtil.record(e4, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanDendaBelumLunasWindow.java:201");
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 4, "Item / Buku");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 5, "Denda");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 6, "Dibayar");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 7, "Kode Pengembalian");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 8, "Di-input oleh");
		...
		for (KembaliPengadaanItemDetail kembaliPengadaanItemDetail : kembaliPengadaanItemDetails) {
			...
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 4, s);
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 5, kembaliPengadaanItemDetail.getDenda());
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 6, kembaliPengadaanItemDetail.getDibayarSejumlah());
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 7, kembaliPengadaanItem.getKode());
		...
		ais.ui.util.EcampusUtil.setBold(sheet, new Rect(colIndex, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex),
				true);
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 4, "Denda Total");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 5, dendaTotal);
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 6, dibayarTotal);
		...
		Utils.setColumnWidth(sheet, 6, 70);
		Utils.setColumnWidth(sheet, 7, 130);
		Utils.setColumnWidth(sheet, 8, 130);
