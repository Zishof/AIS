...
/**
 * Penyusun/penyaji laporan untuk laporan rekap peminjaman per perpustakaan window. Kelas ini
 ...
 * @see MyWindow
 */
public class LaporanRekapPeminjamanPerPerpustakaanWindow extends MyWindow {
	...
	public LaporanRekapPeminjamanPerPerpustakaanWindow() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Rekap Peminjaman Per Perpustakaan Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanRekapPeminjamanPerPerpustakaanWindow(String title, String border, boolean closable) {
	...
	private void initSpreadsheet() throws Exception {
		...
		String sql = "select  to_char(a.tanggal_pembuatan, 'DD-MM-YYYY') as tanggal,  "
				...
				+ "from library.peminjaman_pengadaan_item a    "
		...
		} catch (Exception e4) { ais.common.ErrorAuditUtil.record(e4, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapPeminjamanPerPerpustakaanWindow.java:198");

		}

		ais.ui.util.EcampusUtil.setCellValue(sheet, 1, 0,
				"REKAPITULASI PEMINJAMAN ANGGOTA\n " + (perpustakaan == null ? "SEMUA PERPUSTAKAAN"
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 2, "Jumlah Peminjaman");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 3, "Jumlah Peminjaman\nPer Tanggal");
		...
		for (Object[] objects : jurusans) {
			...
			try {
				ais.ui.util.EcampusUtil.setBorder(sheet,
						new Rect(colIndex, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex), BookHelper.BORDER_FULL,
						BorderStyle.THIN, color);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapPeminjamanPerPerpustakaanWindow.java:260");
				PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Peminjaman Per Perpustakaan Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
					new String[] {
		...
		} catch (Exception e3) { ais.common.ErrorAuditUtil.record(e3, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapPeminjamanPerPerpustakaanWindow.java:284");
