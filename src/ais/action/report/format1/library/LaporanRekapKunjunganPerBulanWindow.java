...
/**
 * Penyusun/penyaji laporan untuk laporan rekap kunjungan per bulan window. Kelas ini mengubah data
 ...
 * @see MyWindow
 */
public class LaporanRekapKunjunganPerBulanWindow extends MyWindow {
	...
	public LaporanRekapKunjunganPerBulanWindow() {
		super();
		try {
	...
	public LaporanRekapKunjunganPerBulanWindow(String title, String border, boolean closable) {
		...
		print.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				Filedownload.save(bout.toByteArray(),
						...
						"REKAP_KUNJUNGAN_ANGGOTA_PER_BULAN.xlsx");
			}
		});
		print.setParent(toolbar);
	...
	private void initSpreadsheet() throws Exception {
		...
		for (int i = 1; i <= maxDate; i++) {
			...
			subStr += " sum(case when a.tgl = '" + thn + "-" + blnStr + "-" + tglStr
		...
		sql += " sum(case when a.tgl is not null and a.id is not null then 1 else 0 end) as total " +

				" from library.tipe_anggota e  " + " left join library.anggota b on (e.id = b.tipe_anggota) "
				+ " left join library.kunjungan_anggota a on (b.id = a.anggota)   "
				...
				+ " and a.tgl between '" + thn + "-" + blnStr + "-01' and  '" + thn + "-" + blnStr + "-" + maxDate + "'"
		...
		} catch (Exception e4) { ais.common.ErrorAuditUtil.record(e4, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapKunjunganPerBulanWindow.java:230");

		}

		ais.ui.util.EcampusUtil.setCellValue(sheet, 1, 0,
				"REKAPITULASI KUNJUNGAN ANGGOTA PER BULAN\n " + (perpustakaan == null ? "SEMUA PERPUSTAKAAN"
		...
		for (Object[] objects : jurusans) {
			...
			try {
				ais.ui.util.EcampusUtil.setBorder(sheet,
						new Rect(colIndex, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex), BookHelper.BORDER_FULL,
						BorderStyle.THIN, color);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapKunjunganPerBulanWindow.java:308");
				PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Kunjungan Per Bulan Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
					new String[] {
		...
		} catch (Exception e3) { ais.common.ErrorAuditUtil.record(e3, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapKunjunganPerBulanWindow.java:337");
