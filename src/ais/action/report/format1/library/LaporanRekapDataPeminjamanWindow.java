...
/**
 * Penyusun/penyaji laporan untuk laporan rekap data peminjaman window. Kelas ini mengubah data
 ...
 * @see MyWindow
 */
public class LaporanRekapDataPeminjamanWindow extends MyWindow {
	...
	public LaporanRekapDataPeminjamanWindow() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Rekap Data Peminjaman Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanRekapDataPeminjamanWindow(String title, String border, boolean closable) {
		...
		Common.createDefaultTimer(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				try {
					...
					// KE-16: auto-load saat window dibuka bisa kena koneksi DB transient
	...
	private void initSpreadsheet() throws Exception {
		...
		spreadsheet.setMaxrows(peminjamanPengadaanItems.size() + 5);
		...
		} catch (Exception e4) { ais.common.ErrorAuditUtil.record(e4, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapDataPeminjamanWindow.java:210");
		...
		int rowIndex = 2;
		int colIndex = 0;
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 0, "Anggota");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 1, "Jenis");
		...
		Map<Anggota, Integer[]> totalPeminjaman = new HashMap<Anggota, Integer[]>();

		for (PeminjamanPengadaanItem peminjamanPengadaanItem : peminjamanPengadaanItems) {

			int jumlahPinjam = ((Number) session.createCriteria(PeminjamanPengadaanItemDetail.class)
					.add(Restrictions.eq("peminjamanPengadaanItem", peminjamanPengadaanItem))
					.setProjection(Projections.rowCount()).uniqueResult()).intValue();

			int jumlahKembali = ((Number) session.createCriteria(KembaliPengadaanItemDetail.class)
					.createCriteria("peminjamanPengadaanItemDetail")
					.add(Restrictions.eq("peminjamanPengadaanItem", peminjamanPengadaanItem))
					.setProjection(Projections.rowCount()).uniqueResult()).intValue();

			Integer[] integers = totalPeminjaman.get(peminjamanPengadaanItem.getAnggota());
			if (integers == null) {
				integers = new Integer[] { jumlahPinjam, jumlahKembali };
			} else {
				integers = new Integer[] { integers[0] + jumlahPinjam, integers[1] + jumlahKembali };
			}

			totalPeminjaman.put(peminjamanPengadaanItem.getAnggota(), integers);
		}

		for (Anggota anggota : totalPeminjaman.keySet()) {
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 0, anggota.toString());
			...
			if (anggota.getMahasiswa() != null) {
				...
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 3, anggota.getMahasiswa().getJurusan().getNama());
			} else if (anggota.getDosen() != null) {
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 2, anggota.getDosen().getJurusan() == null ? ""
			...
			Integer[] integers = totalPeminjaman.get(anggota);

			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 4, integers == null ? 0 : integers[0]);
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 5, integers == null ? 0 : integers[1]);

			try {
				ais.ui.util.EcampusUtil.setBorder(sheet,
						new Rect(colIndex, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex), BookHelper.BORDER_FULL,
						BorderStyle.THIN, color);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapDataPeminjamanWindow.java:296");
				PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Data Peminjaman Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
					new String[] {
		...
		Utils.setColumnWidth(sheet, 2, 330);
		Utils.setColumnWidth(sheet, 3, 230);
