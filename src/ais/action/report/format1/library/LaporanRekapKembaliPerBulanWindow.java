...
/**
 * Penyusun/penyaji laporan untuk laporan rekap kembali per bulan window. Kelas ini mengubah data
 ...
 * @see MyWindow
 */
public class LaporanRekapKembaliPerBulanWindow extends MyWindow {
	...
	public LaporanRekapKembaliPerBulanWindow() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Rekap Kembali Per Bulan Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanRekapKembaliPerBulanWindow(String title, String border, boolean closable) {
		...
		print.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				Filedownload.save(bout.toByteArray(),
						...
						"REKAP_KEMBALI_ANGGOTA_PER_BULAN.xlsx");
			}
		});
		print.setParent(toolbar);
	...
	private void initSpreadsheet() throws Exception {
		...
		sql += " sum(case when a.tanggal_pembuatan is not null and a.id is not null then 1 else 0 end) as total " +
				...
				+ " left join library.peminjaman_pengadaan_item aa on (b.id = aa.anggota)   "
				+ " left join library.kembali_pengadaan_item a on (a.peminjaman_pengadaan_item = aa.id)   "
		...
		} catch (Exception e4) { ais.common.ErrorAuditUtil.record(e4, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapKembaliPerBulanWindow.java:232");

		}

		ais.ui.util.EcampusUtil.setCellValue(sheet, 1, 0,
				"REKAPITULASI KEMBALI ANGGOTA PER BULAN\n " + (perpustakaan == null ? "SEMUA PERPUSTAKAAN"
		...
		for (Object[] objects : jurusans) {
			...
			try {
				ais.ui.util.EcampusUtil.setBorder(sheet,
						new Rect(colIndex, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex), BookHelper.BORDER_FULL,
						BorderStyle.THIN, color);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapKembaliPerBulanWindow.java:310");
				PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Kembali Per Bulan Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
					new String[] {
		...
		} catch (Exception e3) { ais.common.ErrorAuditUtil.record(e3, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapKembaliPerBulanWindow.java:339");
