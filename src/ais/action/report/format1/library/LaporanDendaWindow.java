...
/**
 * Penyusun/penyaji laporan untuk laporan denda window. Kelas ini mengubah data domain menjadi
 ...
 * @see MyWindow
 */
public class LaporanDendaWindow extends MyWindow {
	...
	public LaporanDendaWindow() {
		...
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Denda Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanDendaWindow(String title, String border, boolean closable) {
	...
	private void initSpreadsheet() throws Exception {
		...
		} catch (Exception e4) { ais.common.ErrorAuditUtil.record(e4, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanDendaWindow.java:203");
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 3, "Waktu/Hari/Tanggal");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 4, "Denda");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 5, "Dibayar");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 6, "Kode Pengembalian");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 7, "Di-input oleh");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 8, "Item yang dipinjam");
		...
		TreeMap<Long, List<KembaliPengadaanItemDetail>> kembaliPengadaanItemsMap = new TreeMap<Long, List<KembaliPengadaanItemDetail>>();
		for (KembaliPengadaanItemDetail kembaliPengadaanItemDetail : kembaliPengadaanItemDetails) {
			if (kembaliPengadaanItemsMap.containsKey(kembaliPengadaanItemDetail.getKembaliPengadaanItem().getId())) {
				kembaliPengadaanItemsMap.get(kembaliPengadaanItemDetail.getKembaliPengadaanItem().getId())
						.add(kembaliPengadaanItemDetail);
			} else {
				List<KembaliPengadaanItemDetail> itemDetails = new ArrayList<KembaliPengadaanItemDetail>();
				itemDetails.add(kembaliPengadaanItemDetail);
				kembaliPengadaanItemsMap.put(kembaliPengadaanItemDetail.getKembaliPengadaanItem().getId(), itemDetails);
			}
		}

		Double dendaTotal = 0.0;
		Double dibayarTotal = 0.0;
		for (Long key : kembaliPengadaanItemsMap.keySet()) {
			List<KembaliPengadaanItemDetail> kembaliPengadaanItems = kembaliPengadaanItemsMap.get(key);
			KembaliPengadaanItemDetail kembaliPengadaanItemDetail = kembaliPengadaanItems.get(0);
			KembaliPengadaanItem kembaliPengadaanItem = kembaliPengadaanItemDetail.getKembaliPengadaanItem();

			String itemYangDipinjam = "";
			Double denda = 0.0;
			Double dibayar = 0.0;
			for (KembaliPengadaanItemDetail kembali : kembaliPengadaanItems) {
				denda += kembali.getDenda();
				dibayar += kembali.getDibayarSejumlah();
				String s = kembali.getItemPunyaBarcode() == null ? kembali.getItem().getNama()
						: kembali.getItemPunyaBarcode().getBarcode() + "-" + kembali.getItem().getNama();
				s += "=>Denda: " + Common.numberFormat.get().format(kembali.getDenda()) + ", Dibayar: "
						+ Common.numberFormat.get().format(kembali.getDibayarSejumlah());
			...
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 4, denda);
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 5, dibayar);
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 6, kembaliPengadaanItem.getKode());
			...
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 8, itemYangDipinjam);
		...
		ais.ui.util.EcampusUtil.setBold(sheet, new Rect(colIndex, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex),
				true);
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 3, "Denda Total");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 4, dendaTotal);
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 5, dibayarTotal);
		...
		Utils.setColumnWidth(sheet, 4, 70);
		Utils.setColumnWidth(sheet, 5, 70);
		Utils.setColumnWidth(sheet, 6, 130);
		Utils.setColumnWidth(sheet, 7, 130);
		Utils.setColumnWidth(sheet, 8, 500);
