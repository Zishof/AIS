...
/**
 * Penyusun/penyaji laporan untuk laporan rekap denda per item window. Kelas ini mengubah data
 ...
 * @see MyWindow
 */
public class LaporanRekapDendaPerItemWindow extends MyWindow {
	...
	public LaporanRekapDendaPerItemWindow() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Rekap Denda Per Item Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanRekapDendaPerItemWindow(String title, String border, boolean closable) {
	...
	private void init() {
		...
		print.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event event) throws Exception {
				...
				bout.close();
				Filedownload.save(bout.toByteArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "REKAP_DENDA_ITEM.xlsx");
			}
		});
		print.setParent(toolbar);
	...
	private void initSpreadsheet() throws Exception {
		...
		String sql = "select  to_char(c1.tanggal, 'DD-MM-YYYY') as tanggal,    "
				...
				+ "count(distinct(c1.item)) as jumlah,    sum(c1.denda) as denda,    sum(c1.dibayarsejumlah) as dibayarsejumlah    "
		...
		} catch (Exception e4) { ais.common.ErrorAuditUtil.record(e4, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapDendaPerItemWindow.java:194");

		}

		ais.ui.util.EcampusUtil.setCellValue(sheet, 1, 0,
				"REKAPITULASI DENDA ITEM\n " + (perpustakaan == null ? "SEMUA PERPUSTAKAAN"
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 2, "Jumlah Item");
		...
		for (Object[] objects : jurusans) {
			...
			try {
				ais.ui.util.EcampusUtil.setBorder(sheet,
						new Rect(colIndex, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex), BookHelper.BORDER_FULL,
						BorderStyle.THIN, color);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapDendaPerItemWindow.java:280");
				PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Denda Per Item Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
					new String[] {
		...
		} catch (Exception e3) { ais.common.ErrorAuditUtil.record(e3, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapDendaPerItemWindow.java:308");
		...
		try {
			...
			ais.ui.util.EcampusUtil.setBold(sheet, new Rect(0, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex),
					true);
		} catch (Exception e2) { ais.common.ErrorAuditUtil.record(e2, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapDendaPerItemWindow.java:334");

		}
		++rowIndex;
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 0, "Di-input oleh");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 1, "Jumlah Item");
		...
		try {
			...
			ais.ui.util.EcampusUtil.setBold(sheet, new Rect(0, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex),
					true);
		} catch (Exception e1) { ais.common.ErrorAuditUtil.record(e1, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapDendaPerItemWindow.java:347");
		}
		rowIndex++;
		jumlahTotal = 0.0;
		...
		for (Object[] objects : jurusans) {
			...
			try {
				ais.ui.util.EcampusUtil.setBorder(sheet, new Rect(0, rowIndex, 3, rowIndex), BookHelper.BORDER_FULL,
						BorderStyle.THIN, color);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapDendaPerItemWindow.java:368");
				PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Denda Per Item Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
					new String[] {
						"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
						"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
		...
		} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapDendaPerItemWindow.java:380");
			// Common.tampilErrorJikaAdmin(e);
		}
