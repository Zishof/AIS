...
/**
 * Penyusun/penyaji laporan untuk laporan rekap peminjaman window. Kelas ini mengubah data domain
 ...
 * @see MyWindow
 */
public class LaporanRekapPeminjamanWindow extends MyWindow {
	...
	public LaporanRekapPeminjamanWindow() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Rekap Peminjaman Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	private void initFakultas() {
		...
		/**
		 * Event listener lokal milik {@link LaporanRekapPeminjamanWindow}. Kelas ini menangani event untuk komponen
		 * induk dan meneruskan pekerjaan domain ke method/service yang sudah tersedia.
		 *
		 * <p><b>Scope:</b> setiap instance terikat pada instance {@link LaporanRekapPeminjamanWindow} dan dapat
		 ...
		 *
		 * @see LaporanRekapPeminjamanWindow
		 */
	...
	public LaporanRekapPeminjamanWindow(String title, String border, boolean closable) {
	...
	private void init() {
		...
		MyTabConfig tab1 = new MyTabConfig("Rekap Peminjaman Mahasiswa");
		tab1.setParent(tabs);

		MyTabConfig tab12 = new MyTabConfig("Rekap Peminjaman Anggota");
		tab12.setParent(tabs);

		MyTabConfig tab13 = new MyTabConfig("Rekap Peminjaman Anggota Per Bulan");
		tab13.setParent(tabs);

		MyTabConfig tab2 = new MyTabConfig("Data Peminjaman Anggota");
		tab2.setParent(tabs);

		MyTabConfig tab22 = new MyTabConfig("Rekap Peminjaman");
		tab22.setParent(tabs);

		MyTabConfig tab21 = new MyTabConfig("Data Peminjaman Per Item");
		...
		tab12.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				if (tabpanel12.getChildren().size() == 0) {
					LaporanRekapPeminjamanPerPerpustakaanWindow laporanKHS = new LaporanRekapPeminjamanPerPerpustakaanWindow();
		...
		tab13.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				if (tabpanel13.getChildren().size() == 0) {
					LaporanRekapPeminjamanPerBulanWindow laporanKHS = new LaporanRekapPeminjamanPerBulanWindow();
					laporanKHS.setHeight("100%");
					laporanKHS.setWidth("100%");
		...
		tab2.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				if (tabpanel2.getChildren().size() == 0) {
					LaporanPeminjamanWindow laporanKHS = new LaporanPeminjamanWindow();
					laporanKHS.setHeight("100%");
					laporanKHS.setWidth("100%");
		...
		tab22.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				if (tabpanel22.getChildren().size() == 0) {
					LaporanRekapDataPeminjamanWindow laporanKHS = new LaporanRekapDataPeminjamanWindow();
					laporanKHS.setHeight("100%");
					laporanKHS.setWidth("100%");
		...
		tab21.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				if (tabpanel21.getChildren().size() == 0) {
					LaporanPeminjamanItemWindow laporanKHS = new LaporanPeminjamanItemWindow();
					laporanKHS.setHeight("100%");
					laporanKHS.setWidth("100%");
	...
	private void initSpreadsheet() throws Exception {
		...
		String sql = "select  to_char(a.tanggal_pembuatan, 'DD-MM-YYYY') as tanggal,  "
				+ "max(d.nama) as perpustakaan,    " + "max(f.nama) as fakultas,    " + "max(e.nama) as jurusan,    "
				+ "count(c.id) as jumlah    " + "from library.peminjaman_pengadaan_item a    "
				...
				+ (start.getValue() == null ? " "
						: " and (a.tanggal_pembuatan) >= ('" + Common.databaseDateFormat.get().format(start.getValue())
								+ " 00:00:00') ")

				+ (end.getValue() == null ? " "
						: " and (a.tanggal_pembuatan) <= ('" + Common.databaseDateFormat.get().format(end.getValue())
								+ " 23:59:59') ")

				+ " group by d.id,f.id,e.id,to_char(a.tanggal_pembuatan, 'DD-MM-YYYY')  "
				+ " order by max(a.tanggal_pembuatan),max(d.nama)";
		...
		} catch (Exception e4) { ais.common.ErrorAuditUtil.record(e4, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapPeminjamanWindow.java:387");

		}

		ais.ui.util.EcampusUtil.setCellValue(sheet, 1, 0,
				"REKAPITULASI PEMINJAMAN MAHASISWA\n " + (perpustakaan == null ? "SEMUA PERPUSTAKAAN"
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 4, "Jumlah Peminjaman");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 5, "Jumlah Peminjaman\nPer Tanggal");
		...
		for (Object[] objects : jurusans) {
			...
			try {
				ais.ui.util.EcampusUtil.setBorder(sheet,
						new Rect(colIndex, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex), BookHelper.BORDER_FULL,
						BorderStyle.THIN, color);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapPeminjamanWindow.java:453");
				PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Peminjaman Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
					new String[] {
		...
		} catch (Exception e3) { ais.common.ErrorAuditUtil.record(e3, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapPeminjamanWindow.java:477");
