...
/**
 * Penyusun/penyaji laporan untuk laporan kembali window. Kelas ini mengubah data domain menjadi
 ...
 * @see MyWindow
 */
public class LaporanKembaliWindow extends MyWindow {
	...
	public LaporanKembaliWindow() {
		...
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Kembali Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanKembaliWindow(String title, String border, boolean closable) {
		...
		Common.createDefaultTimer(new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				try {
					...
					// KE-15: auto-load saat window dibuka bisa kena koneksi DB transient
	...
	private void initSpreadsheet() throws Exception {
		...
		} catch (Exception e4) { ais.common.ErrorAuditUtil.record(e4, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanKembaliWindow.java:208");
		...
		for (KembaliPengadaanItem kembaliPengadaanItem : kembaliPengadaanItems) {
			...
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 6, jumlahKembali);

			try {
				ais.ui.util.EcampusUtil.setBorder(sheet,
						new Rect(colIndex, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex), BookHelper.BORDER_FULL,
						BorderStyle.THIN, color);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanKembaliWindow.java:278");
				PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Kembali Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
					new String[] {
