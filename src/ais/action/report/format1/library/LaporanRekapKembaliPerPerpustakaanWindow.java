...
/**
 * Penyusun/penyaji laporan untuk laporan rekap kembali per perpustakaan window. Kelas ini mengubah
 ...
 * @see MyWindow
 */
public class LaporanRekapKembaliPerPerpustakaanWindow extends MyWindow {
	...
	public LaporanRekapKembaliPerPerpustakaanWindow() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Rekap Kembali Per Perpustakaan Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanRekapKembaliPerPerpustakaanWindow(String title, String border, boolean closable) {
	...
	private void initSpreadsheet() throws Exception {
		...
		String sql = "select  to_char(a.tanggal_pembuatan, 'DD-MM-YYYY') as tanggal,  "
				+ "max(d.nama) as perpustakaan,    " + "count(a.id) as jumlah    "
				+ "from library.kembali_pengadaan_item a    "
				...
				+ " group by d.id,to_char(a.tanggal_pembuatan, 'DD-MM-YYYY')  "
		...
		} catch (Exception e4) { ais.common.ErrorAuditUtil.record(e4, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapKembaliPerPerpustakaanWindow.java:201");

		}

		ais.ui.util.EcampusUtil.setCellValue(sheet, 1, 0,
				"REKAPITULASI KEMBALI ANGGOTA\n " + (perpustakaan == null ? "SEMUA PERPUSTAKAAN"
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 2, "Jumlah Kembali");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 3, "Jumlah Kembali\nPer Tanggal");
		...
		for (Object[] objects : jurusans) {
			...
			try {
				ais.ui.util.EcampusUtil.setBorder(sheet,
						new Rect(colIndex, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex), BookHelper.BORDER_FULL,
						BorderStyle.THIN, color);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapKembaliPerPerpustakaanWindow.java:263");
				PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Kembali Per Perpustakaan Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
					new String[] {
		...
		} catch (Exception e3) { ais.common.ErrorAuditUtil.record(e3, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapKembaliPerPerpustakaanWindow.java:287");
