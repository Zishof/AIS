...
/**
 * Penyusun/penyaji laporan untuk laporan kunjungan item window. Kelas ini mengubah data domain
 ...
 * @see MyWindow
 */
public class LaporanKunjunganItemWindow extends MyWindow {
	...
	public LaporanKunjunganItemWindow() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Kunjungan Item Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanKunjunganItemWindow(String title, String border, boolean closable) {
	...
	private void initSpreadsheet() throws Exception {
		...
		for (KunjunganAnggota kunjunganAnggota : kembaliPengadaanItems) {
			...
			List pinjam = session.createCriteria(PeminjamanPengadaanItemDetail.class)
					.setProjection(Projections.property("item")).createCriteria("peminjamanPengadaanItem")
					.add(Restrictions.eq("kunjunganAnggota", kunjunganAnggota)).list();

			List kembali = session.createCriteria(KembaliPengadaanItemDetail.class)
					.setProjection(Projections.property("item")).createCriteria("kembaliPengadaanItem")
					.add(Restrictions.eq("kunjunganAnggota", kunjunganAnggota)).list();
			...
			hashMap.put(kunjunganAnggota.getId(), new Object[] { pinjam, kembali });
		}
		...
		} catch (Exception e4) { ais.common.ErrorAuditUtil.record(e4, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanKunjunganItemWindow.java:223");
		...
		for (KunjunganAnggota kunjunganAnggota : kembaliPengadaanItems) {
			...
			Object[] objects = hashMap.get(kunjunganAnggota.getId());
			...
			if (banyak > 0) {
				for (int i = 0; i < banyak; i++) {
					...
					} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanKunjunganItemWindow.java:319");
						PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Kunjungan Item Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
							new String[] {
				...
				try {
					ais.ui.util.EcampusUtil.setBorder(sheet,
							new Rect(colIndex, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex),
							BookHelper.BORDER_FULL, BorderStyle.THIN, color);
				} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanKunjunganItemWindow.java:333");
					PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Kunjungan Item Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
						new String[] {
							"Periksa kembali data/parameter/filter yang Bapak/Ibu masukkan pada layar ini.",
							"Ulangi kembali proses yang tadi dijalankan beberapa saat lagi.",
