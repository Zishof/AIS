...
/**
 * Penyusun/penyaji laporan untuk laporan rekap kunjungan anggota aktif window. Kelas ini mengubah
 ...
 * @see MyWindow
 */
public class LaporanRekapKunjunganAnggotaAktifWindow extends MyWindow {
	...
	public LaporanRekapKunjunganAnggotaAktifWindow() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Rekap Kunjungan Anggota Aktif Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanRekapKunjunganAnggotaAktifWindow(String title, String border, boolean closable) {
	...
	private void initSpreadsheet() throws Exception {
		...
		List<KunjunganAnggota> kembaliPengadaanItems = session.createCriteria(KunjunganAnggota.class)
				...
				.createAlias("anggota", "anggota").add(Restrictions.isNotNull("anggota.mahasiswa"))
		...
		} catch (Exception e4) { ais.common.ErrorAuditUtil.record(e4, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapKunjunganAnggotaAktifWindow.java:211");
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 3, "Jumlah Kunjungan");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 4, "Jumlah Mhs Aktif");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 5, "Jumlah Mhs Kadaluarsa");
		...
		for (KunjunganAnggota kunjunganAnggota : kembaliPengadaanItems) {
			Jurusan jurusan = kunjunganAnggota.getAnggota().getMahasiswa().getJurusan();
			Perpustakaan pus = kunjunganAnggota.getPerpustakaan();

			String key = (jurusan == null || jurusan.getId() == null ? -1L : jurusan.getId()) + "_" + pus.getId();

			Map d = data.get(key);
			if (d == null) {
				d = new HashMap();
				d.put("jurusan", jurusan);
				d.put("perpustakaan", pus);
			}
			Integer jml = (Integer) (d.get("jumlah") == null ? 0 : d.get("jumlah"));
			d.put("jumlah", jml + 1);

			if (kunjunganAnggota.getAnggota().getMahasiswa() != null
					&& kunjunganAnggota.getAnggota().getMahasiswa().getStatusKeluar() != null) {
				Integer lulus = (Integer) (d.get("lulus") == null ? 0 : d.get("lulus"));
				d.put("lulus", lulus + 1);
			} else {
				Integer blmlulus = (Integer) (d.get("blmlulus") == null ? 0 : d.get("blmlulus"));
				d.put("blmlulus", blmlulus + 1);
			}
		...
		for (Map dataKunjungan : data.values()) {
			...
			Jurusan jurusan = (Jurusan) dataKunjungan.get("jurusan");
			...
			Integer lulus = (Integer) (dataKunjungan.get("lulus") == null ? 0 : dataKunjungan.get("lulus"));
			Integer blmlulus = (Integer) (dataKunjungan.get("blmlulus") == null ? 0 : dataKunjungan.get("blmlulus"));

			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 0, pus.getNama());
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 1, jurusan.getNama());
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 2,
					jurusan == null ? "" : jurusan.getFakultas().getNama());

			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 3, jml);
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 4, blmlulus);
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 5, lulus);

			try {
				ais.ui.util.EcampusUtil.setBorder(sheet,
						new Rect(colIndex, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex), BookHelper.BORDER_FULL,
						BorderStyle.THIN, color);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapKunjunganAnggotaAktifWindow.java:296");
				PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Kunjungan Anggota Aktif Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
					new String[] {
