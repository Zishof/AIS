...
/**
 * Penyusun/penyaji laporan untuk laporan kunjungan window. Kelas ini mengubah data domain menjadi
 ...
 * @see MyWindow
 */
public class LaporanKunjunganWindow extends MyWindow {
	...
	public LaporanKunjunganWindow() {
		...
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Kunjungan Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	public LaporanKunjunganWindow(String title, String border, boolean closable) {
	...
	private void initSpreadsheet() throws Exception {
		...
		// Timer callback: pakai session dedicated agar koneksi OSIV yang mungkin basi tidak
		// menyebabkan JDBCConnectionException "This connection has been closed."
		...
		} catch (Exception e4) { ais.common.ErrorAuditUtil.record(e4, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanKunjunganWindow.java:208");
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 6, "Berkunjung dari");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 7, "Jumlah \nPeminjaman");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 8, "Jumlah \nPengembalian");
		...
		for (KunjunganAnggota kunjunganAnggota : kembaliPengadaanItems) {
			...
			if (anggota == null) {
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 0, kunjunganAnggota.getPerpustakaan().getNama());
				ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 1, kunjunganAnggota.getNama());
			...
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 6, kunjunganAnggota.getKeterangan());

			int jumlahPinjam = ((Number) session.createCriteria(PeminjamanPengadaanItemDetail.class)
					.createCriteria("peminjamanPengadaanItem")
					.add(Restrictions.eq("kunjunganAnggota", kunjunganAnggota)).setProjection(Projections.rowCount())
					.uniqueResult()).intValue();

			int jumlahKembali = ((Number) session.createCriteria(KembaliPengadaanItemDetail.class)
					.createCriteria("kembaliPengadaanItem").add(Restrictions.eq("kunjunganAnggota", kunjunganAnggota))
					.setProjection(Projections.rowCount()).uniqueResult()).intValue();

			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 7, jumlahPinjam);
			ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 8, jumlahKembali);

			try {
				ais.ui.util.EcampusUtil.setBorder(sheet,
						new Rect(colIndex, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex), BookHelper.BORDER_FULL,
						BorderStyle.THIN, color);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanKunjunganWindow.java:300");
				PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Kunjungan Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
					new String[] {
		...
		ais.ui.util.PratinjauXlsxHelper.gantiSpreadsheetDenganGrid(spreadsheet);
			...
			try { session.clear(); session.close(); } catch (Exception eignore) { ais.common.ErrorAuditUtil.record(eignore, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanKunjunganWindow.java:317");}
		}
	}
}
