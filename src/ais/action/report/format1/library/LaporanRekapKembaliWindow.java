...
/**
 * Penyusun/penyaji laporan untuk laporan rekap kembali window. Kelas ini mengubah data domain
 ...
 * @see MyWindow
 */
public class LaporanRekapKembaliWindow extends MyWindow {
	...
	public LaporanRekapKembaliWindow() {
		super();
		try {
			...
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Rekap Kembali Window", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	private void initFakultas() {
		...
		/**
		 * Event listener lokal milik {@link LaporanRekapKembaliWindow}. Kelas ini menangani event untuk komponen induk
		 * dan meneruskan pekerjaan domain ke method/service yang sudah tersedia.
		 *
		 * <p><b>Scope:</b> setiap instance terikat pada instance {@link LaporanRekapKembaliWindow} dan dapat mengakses
	...
	public LaporanRekapKembaliWindow(String title, String border, boolean closable) {
	...
	private void init() {
		...
		MyTabConfig tab1 = new MyTabConfig("Rekap Kembali Mahasiswa");
		tab1.setParent(tabs);

		MyTabConfig tab12 = new MyTabConfig("Rekap Kembali Anggota");
		tab12.setParent(tabs);

		MyTabConfig tab13 = new MyTabConfig("Rekap Kembali Anggota Per Bulan");
		tab13.setParent(tabs);

		MyTabConfig tab2 = new MyTabConfig("Data Kembali Anggota");
		tab2.setParent(tabs);

		MyTabConfig tab21 = new MyTabConfig("Data Kembali Per Item");
		...
		tab12.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				if (tabpanel12.getChildren().size() == 0) {
					LaporanRekapKembaliPerPerpustakaanWindow laporanKHS = new LaporanRekapKembaliPerPerpustakaanWindow();
		...
		tab13.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				if (tabpanel13.getChildren().size() == 0) {
					LaporanRekapKembaliPerBulanWindow laporanKHS = new LaporanRekapKembaliPerBulanWindow();
					laporanKHS.setHeight("100%");
					laporanKHS.setWidth("100%");
		...
		tab2.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				if (tabpanel2.getChildren().size() == 0) {
					LaporanKembaliWindow laporanKHS = new LaporanKembaliWindow();
					laporanKHS.setHeight("100%");
					laporanKHS.setWidth("100%");
		...
		tab21.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				if (tabpanel21.getChildren().size() == 0) {
					LaporanKembaliItemWindow laporanKHS = new LaporanKembaliItemWindow();
					laporanKHS.setHeight("100%");
					laporanKHS.setWidth("100%");
	...
	private void initSpreadsheet() throws Exception {
		...
		String sql = "select  to_char(a.tanggal_pembuatan, 'DD-MM-YYYY') as tanggal,  "
				...
				+ "count(c.id) as jumlah    " + "from library.kembali_pengadaan_item a    "
				+ "inner join library.peminjaman_pengadaan_item aa on (a.peminjaman_pengadaan_item = aa.id)   "
				+ "inner join library.anggota b on (b.id = aa.anggota)    "
		...
		} catch (Exception e4) { ais.common.ErrorAuditUtil.record(e4, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapKembaliWindow.java:372");

		}

		ais.ui.util.EcampusUtil.setCellValue(sheet, 1, 0,
				"REKAPITULASI KEMBALI MAHASISWA\n " + (perpustakaan == null ? "SEMUA PERPUSTAKAAN"
		...
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 4, "Jumlah Kembali");
		ais.ui.util.EcampusUtil.setCellValue(sheet, rowIndex, 5, "Jumlah Kembali\nPer Tanggal");
		...
		for (Object[] objects : jurusans) {
			...
			try {
				ais.ui.util.EcampusUtil.setBorder(sheet,
						new Rect(colIndex, rowIndex, spreadsheet.getMaxcolumns() - 1, rowIndex), BookHelper.BORDER_FULL,
						BorderStyle.THIN, color);
			} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapKembaliWindow.java:438");
				PesanFormalHelper.tampilkanGagalException("pemrosesan Laporan Rekap Kembali Window", "Sistem mengalami kendala teknis saat memproses permintaan pada layar laporan ini, kemungkinan disebabkan oleh data yang tidak lengkap, parameter/filter yang tidak sesuai, atau gangguan sementara pada sistem.", e,
					new String[] {
		...
		} catch (Exception e3) { ais.common.ErrorAuditUtil.record(e3, "auto-audit(empty-catch) src/ais/action/report/format1/library/LaporanRekapKembaliWindow.java:462");
