...
/**
 * Penyusun/penyaji laporan untuk laporan surat per tanggal. Kelas ini mengubah data domain menjadi
 ...
 * @see MyWindow
 */
public class LaporanSuratPerTanggal extends MyWindow {
	...
	public LaporanSuratPerTanggal() {
		super();
		try {
			init();
		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e); 
			PesanFormalHelper.tampilkanGagalException("pemuatan data awal layar Laporan Surat Per Tanggal", "Sistem mengalami kendala teknis saat memuat data awal untuk layar laporan ini, kemungkinan karena data referensi (mis. periode, program studi/unit, atau parameter filter terkait) belum lengkap, atau terjadi gangguan sementara pada koneksi ke basis data.", e,
					new String[] {
	...
	private void init() throws Exception {
		...
		}, "surat/laporan_daftar_surat_per_tanggal", null, new EventListener() {
	...
	public void onReport(Event event) {
		...
		try {
			File file = Report.generateFileReportWithProgress(Report.PDF, generateParameter(),
					"surat/laporan_daftar_surat_per_tanggal", ais.ui.util.WaktuUtil.getDate(), toolbar);
			CommonReport.tampilkanReportPDF(center, file);

		} catch (Exception e) {
			Common.tampilErrorJikaAdmin(e); 
			PesanFormalHelper.tampilkanGagalException("pembuatan berkas PDF Laporan Surat Per Tanggal", "Sistem mengalami kendala teknis saat menyusun berkas PDF laporan ini, kemungkinan karena salah satu data sumber laporan tidak lengkap, format datanya tidak sesuai dengan yang diharapkan oleh template laporan, atau terjadi gangguan sementara pada proses pembuatan berkas.", e,
					new String[] {
