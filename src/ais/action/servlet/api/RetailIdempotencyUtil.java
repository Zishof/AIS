...
public final class RetailIdempotencyUtil {
	...
	public static JSONObject mulai(Session session, String action, String key, String requestHash) throws Exception {
		...
		String cleanRequestHash = fingerprint(requestHash);
		...
		insert.setString(3, cleanRequestHash);
		...
		rs.close();
		select.close();
		// Kompatibilitas data lama: sebelum 2026-08-15 sebagian pemanggil
		// menyimpan payload mentah (bukan SHA-256) bila panjangnya kebetulan <= 64.
		// Terima nilai lama itu, tetapi seluruh request baru selalu disimpan sebagai
		// fingerprint 64 karakter agar payload checkout besar tidak melampaui kolom.
		String legacyRequestHash = requestHash == null ? null : requestHash.trim();
		if (existingHash != null && cleanRequestHash != null
				&& !existingHash.equals(cleanRequestHash)
				&& !existingHash.equals(legacyRequestHash)) {
	...
	/** Fingerprint tetap 64 karakter; parameter method boleh berupa payload besar. */
	private static String fingerprint(String value) throws Exception {
		if (value == null || value.trim().isEmpty()) return null;
		byte[] digest = MessageDigest.getInstance("SHA-256")
				.digest(value.trim().getBytes("UTF-8"));
