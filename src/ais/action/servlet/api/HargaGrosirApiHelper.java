...
public final class HargaGrosirApiHelper {
	...
	public static Double hargaSatuan(Connection conn, long tokoId, long produkId, double qtyDasar,
		...
		PreparedStatement ps = conn.prepareStatement(
				"SELECT harga FROM koperasi.aturan_harga_produk"
		...
		try {
			...
			try { return rs.next() ? Double.valueOf(rs.getDouble(1)) : null; }
			finally { rs.close(); }
		} finally { ps.close(); }
	}
	...
	public static void simpan(Tbmuser tbmuser, JSONObject request, JSONObject hasil) throws Exception {
		...
		if (produkId == null || minQty <= 0 || harga <= 0) {
			hasil.put("status", "91");
			hasil.put("description", "produk_id, min_qty_dasar (>0), dan harga (>0) wajib diisi.");
			return;
		}
		Session session = HibernateUtil.getSessionFactory().openSession();
		try {
