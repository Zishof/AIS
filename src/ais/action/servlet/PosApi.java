...
public class PosApi extends HttpServlet {
	...
	private void proses(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			if ("katalog".equals(action)) {
			...
			} else if ("so_batalkan".equals(action)) {
				KantinHelper.soBatalkan(tbmuser, payload, hasil);
				normalisasiStatusKantinHelper(hasil, "so_batalkan");
			} else if ("so_perubahan_stok".equals(action)) {
				KantinHelper.soPerubahanStok(tbmuser, payload, hasil);
				normalisasiStatusKantinHelper(hasil, "so_perubahan_stok");
			...
			} else if ("so_harian".equals(action)) {
				KantinHelper.soHarian(tbmuser, payload, hasil);
				normalisasiStatusKantinHelper(hasil, "so_harian");
			} else if ("so_harian_download_excel".equals(action)) {
				KantinHelper.soHarianDownloadExcel(tbmuser, payload, hasil);
				normalisasiStatusKantinHelper(hasil, "so_harian_download_excel");
			} else if ("so_harian_upload_excel_preview".equals(action)) {
				KantinHelper.soHarianUploadExcelPreview(tbmuser, payload, hasil);
				normalisasiStatusKantinHelper(hasil, "so_harian_upload_excel_preview");
			} else if ("so_harian_upload_excel".equals(action)) {
				KantinHelper.soHarianUploadExcel(tbmuser, payload, hasil);
				normalisasiStatusKantinHelper(hasil, "so_harian_upload_excel");
			} else if ("so_harian_ekspor_excel".equals(action)) {
				// Alias kompatibilitas untuk build POS sebelum action kanonis
				// so_harian_download_excel diperkenalkan.
				KantinHelper.soHarianEksporExcel(tbmuser, payload, hasil);
				normalisasiStatusKantinHelper(hasil, "so_harian_ekspor_excel");
			...
			} else if ("uom_simpan".equals(action)) {
				ais.action.servlet.api.SatuanProdukApiHelper.uomSimpan(tbmuser, payload, hasil);
				normalisasiStatusKantinHelper(hasil, "uom_simpan");
			} else if ("uom_hapus".equals(action)) {
				ais.action.servlet.api.SatuanProdukApiHelper.uomHapus(tbmuser, payload, hasil);
				normalisasiStatusKantinHelper(hasil, "uom_hapus");
				...
				ais.action.servlet.api.DanaTalanganApiHelper.proses(action, tbmuser, payload, hasil);
				normalisasiStatusKantinHelper(hasil, action);
				...
				ais.action.servlet.api.PertangungjawabanKasBesarApiHelper.proses(action, tbmuser, payload, hasil);
				normalisasiStatusKantinHelper(hasil, action);
			} else if (action.startsWith("kas_besar_")) {
				// Grup "Keuangan": pengeluaran kas besar -- dipindahkan dari layar ZK
				// KasBesarAction; nilainya selalu dihitung ulang dari rincian di server.
				ais.action.servlet.api.KasBesarApiHelper.proses(action, tbmuser, payload, hasil);
				normalisasiStatusKantinHelper(hasil, action);
			} else if (action.startsWith("pj_uang_muka_")) {
				// Grup "Keuangan": pertanggungjawaban uang muka (LPJ) -- dipindahkan dari layar ZK
				// PertangungjawabanAction; nilai LPJ selalu dihitung ulang di server.
				ais.action.servlet.api.PertangungjawabanApiHelper.proses(action, tbmuser, payload, hasil);
				normalisasiStatusKantinHelper(hasil, action);
			...
			} else if ("harga_grosir_list".equals(action)) {
				ais.action.servlet.api.HargaGrosirApiHelper.daftar(tbmuser, payload, hasil);
				normalisasiStatusKantinHelper(hasil, "harga_grosir_list");
			} else if ("harga_grosir_simpan".equals(action)) {
				ais.action.servlet.api.HargaGrosirApiHelper.simpan(tbmuser, payload, hasil);
				normalisasiStatusKantinHelper(hasil, "harga_grosir_simpan");
			} else if ("harga_grosir_hapus".equals(action)) {
				ais.action.servlet.api.HargaGrosirApiHelper.hapus(tbmuser, payload, hasil);
				normalisasiStatusKantinHelper(hasil, "harga_grosir_hapus");
			...
			} else if ("draft_jurnal_ringkasan".equals(action) || "draft_jurnal_rincian".equals(action)
					|| "draft_jurnal_posting".equals(action) || "draft_jurnal_batal_posting".equals(action)) {
				...
				ais.action.servlet.api.DraftJurnalApiHelper.proses(action, tbmuser, payload, hasil);
			...
			} else if ("laporan_metode_bayar_opsi".equals(action)) {
				prosesLaporanMetodeBayarOpsi(tbmuser, payload, hasil);
	...
	private static boolean bolehAksesActionKantin(Tbmuser tbmuser, String action) {
		...
		if (action.startsWith("apotik_pasien_")) {
			return menu.optBoolean("apotik_resep", false)
					|| menu.optBoolean("apotik_kasir", false);
		}
		...
		if (action.startsWith("apotik_racikan_") || "apotik_bayar_racikan".equals(action)
				|| action.startsWith("apotik_produksi_")) {
			return menu.optBoolean("apotik_racikan", false)
					|| menu.optBoolean("apotik_kasir", false);
		}
	...
	private static void ikatParam(java.sql.PreparedStatement ps, int idx, Object v) throws Exception {
		// null WAJIB jadi SQL NULL. Tanpa cabang ini nilainya jatuh ke cabang
		// String di bawah dan terkirim sbg teks "null" -- PostgreSQL menolaknya
		// saat di-cast (mis. pada filter toko opsional COALESCE(?::bigint,...)).
		if (v == null) ps.setNull(idx, java.sql.Types.OTHER);
	...
	private void prosesDashboardUmum(Tbmuser tbmuser, JSONObject payload, JSONObject hasil) throws Exception {
		...
		if (semuaToko && tbmuser.getPedagang() != null && !bolehLihatSemuaToko(tbmuser)) {
		...
		String kondisiToko = semuaToko ? batasPendaftar("", pendaftarLingkup) : " AND toko = ?";
		...
		try {
			...
			String subKpi = "SELECT COALESCE(a.pembelian_anggota_koperasi,a.id) id_trx,MAX(a.waktu) waktu,"
					...
					+ "WHERE COALESCE(a.aktif,true)=true" + (semuaToko ? batasPendaftar("a", pendaftarLingkup) : " AND a.toko = ?") + " GROUP BY 1";
			...
			String subTren = "SELECT COALESCE(a.pembelian_anggota_koperasi,a.id) id_trx,MAX(a.waktu) waktu,"
					...
					+ (semuaToko ? batasPendaftar("a", pendaftarLingkup) : " AND a.toko = ?") + " GROUP BY 1";
	...
	private void prosesLaporanOrderList(Tbmuser tbmuser, JSONObject payload, JSONObject hasil) throws Exception {
		...
		Session session = HibernateUtil.getSessionFactory().openSession();
		try {
			// tokoId boleh null (= seluruh toko); session.get() melempar bila id null.
	...
	private void prosesTransaksiBackupStatus(Tbmuser tbmuser, JSONObject payload, JSONObject hasil) throws Exception {
		...
		try {
			...
			int no = 1; isiParamToko(ps, no++, tokoId);
	...
	private void prosesLaporanRiwayatPenjualanAnalitik(Tbmuser tbmuser, JSONObject payload, JSONObject hasil) throws Exception {
		...
		try {
			...
			java.sql.PreparedStatement pk = conn.prepareStatement("SELECT COUNT(*),COALESCE(SUM(total_nilai),0),COALESCE(AVG(total_nilai),0),"
					...
					+ " FROM (" + trx + ") x");
			isiParamToko(pk,1, tokoId); pk.setString(2, mulai); pk.setString(3, sampai);
			...
			isiParamToko(pp,1, tokoId); pp.setString(2, mulaiLalu); pp.setString(3, sampaiLalu);
			...
			isiParamToko(pt,1, tokoId); pt.setString(2, mulai); pt.setString(3, sampai);
			...
			isiParamToko(pm,1, tokoId); pm.setString(2, mulai); pm.setString(3, sampai);
			...
			isiParamToko(pj,1, tokoId); pj.setString(2, mulai); pj.setString(3, sampai);
			...
			isiParamToko(pKeranjang,1, tokoId); pKeranjang.setString(2, mulai); pKeranjang.setString(3, sampai);
			...
			isiParamToko(pKasir,1, tokoId); pKasir.setString(2, mulai); pKasir.setString(3, sampai);
	...
	private void prosesLaporanPenjualanKasirList(Tbmuser tbmuser, JSONObject payload, JSONObject hasil) throws Exception {
		...
		try {
			...
			StringBuilder fbK = new StringBuilder();
			if (kasirDipilih.length() > 0) { fbK.append(fbK.length() == 0 ? " WHERE " : " AND ").append("LOWER(TRIM(kasir))=LOWER(?)"); }
			if (metodeDipilih.length() > 0) { fbK.append(fbK.length() == 0 ? " WHERE " : " AND ").append("LOWER(TRIM(metode))=LOWER(?)"); }
			String batasKasir = fbK.toString();
			java.sql.PreparedStatement ps = conn.prepareStatement(
					"SELECT kasir,COUNT(*),COALESCE(SUM(nilai),0) FROM (" + sub + ") trx" + batasKasir + " GROUP BY kasir ORDER BY kasir");
			int ix = 1;
			isiParamToko(ps, ix++, tokoId); ps.setString(ix++, tglMulai); ps.setString(ix++, tglSampai);
			...
			if (bolehSemua) {
				...
				isiParamToko(psPilihan,1, tokoId); psPilihan.setString(2, tglMulai); psPilihan.setString(3, tglSampai);
	 ...
	 * dipenuhi metode lama yang sudah tak terpakai. Dipakai bersama filter {@code metode}
	 * di laporan penerimaan/penjualan per kasir (permintaan rekonsiliasi tim keuangan).
	 */
	private void prosesLaporanMetodeBayarOpsi(Tbmuser tbmuser, JSONObject payload, JSONObject hasil) throws Exception {
		Long tokoId = resolveTokoId(tbmuser, payload);
		final Long pendaftarLingkup = pendaftarIdPengguna(tbmuser);
		if (tokoId == null && tbmuser.getPedagang() != null && !bolehLihatSemuaToko(tbmuser)) { hasil.put("status", "error"); hasil.put("message", "Toko tidak diketahui utk akun ini."); return; }
		String hariIni = new java.text.SimpleDateFormat("yyyy-MM-dd").format(new java.util.Date());
		...
		try {
			...
			java.sql.ResultSet rs = ps.executeQuery();
			JSONArray arr = new JSONArray();
			while (rs.next()) { arr.put(rs.getString(1)); }
			rs.close();
			ps.close();
			hasil.put("status", "success");
			hasil.put("data", arr);
	...
	private void prosesLaporanPenerimaanKasirList(Tbmuser tbmuser, JSONObject payload, JSONObject hasil) throws Exception {
		...
		try {
			...
			int ix = 1; isiParamToko(pc, ix++, tokoId); pc.setString(ix++, mulai); pc.setString(ix++, sampai);
			if (kasir.length() > 0) pc.setString(ix++, kasir);
			if (metode.length() > 0) pc.setString(ix++, metode);
			...
			ix = 1; isiParamToko(pd, ix++, tokoId); pd.setString(ix++, mulai); pd.setString(ix++, sampai);
			if (kasir.length() > 0) pd.setString(ix++, kasir);
			if (metode.length() > 0) pd.setString(ix++, metode);
			pd.setInt(ix++, pageSize); pd.setInt(ix++, offset);
			...
			if (bolehSemua) {
				...
				isiParamToko(pp,1, tokoId); pp.setString(2, mulai); pp.setString(3, sampai);
	...
	private static void prosesErrorLogHealth(JSONObject hasil) throws Exception {
		...
		try {
			...
			// Kontrak API modern memakai "success". Kode numerik lama tetap
			// disediakan terpisah agar klien lama tidak kehilangan informasi.
			hasil.put("status", "success");
			hasil.put("kodeStatus", "00");
	...
	private void prosesLaporanPenerimaanKasirDetail(Tbmuser tbmuser, JSONObject payload, JSONObject hasil) throws Exception {
		...
		try {
			// tokoId boleh null (= seluruh toko); session.get() melempar bila id null.
			Toko toko = tokoId == null ? null : (Toko) session.get(Toko.class, tokoId);
			String kode = toko == null || toko.getKode() == null || toko.getKode().trim().isEmpty() ? (tokoId == null ? "semua" : "toko" + tokoId) : toko.getKode().trim();
			...
			if (rincianSesi) {
				...
				while (rs.next()) {
					...
					java.sql.PreparedStatement pd = conn.prepareStatement(
							...
							+ " FROM koperasi.pembelian_anggota_koperasi WHERE " + kondisiToko("", pendaftarLingkup) + " AND (sesi_kas_kasir=? OR (sesi_kas_kasir IS NULL"
							+ " AND LOWER(NULLIF(TRIM(kasir_login_nama),''))=LOWER(?) AND tanggal_pembayaran>=? AND tanggal_pembayaran<=?))");
					isiParamToko(pd,1, tokoId); pd.setLong(2, sesiId); pd.setString(3, namaKasir); pd.setTimestamp(4, buka);
	...
	private JSONObject daftarOrderDenganSesi(Session session, Long tokoId, String tokoKode, JSONObject payload) throws Exception {
		...
		idx = 1;
		isiParamToko(psData, idx++, tokoId); // sesi_bertingkat.toko
	...
	private void prosesLaporanSesiList(Tbmuser tbmuser, JSONObject payload, JSONObject hasil) throws Exception {
		...
		Session session = HibernateUtil.getSessionFactory().openSession();
		try {
			// tokoId null = seluruh toko; session.get() akan melempar bila
			// id-nya null, jadi pemuatannya dilewati dan label diberi nama umum.
			Toko toko = tokoId == null ? null : (Toko) session.get(Toko.class, tokoId);
			String tokoKode = toko == null || toko.getKode() == null || toko.getKode().trim().isEmpty()
					? (tokoId == null ? "semua" : ("toko" + tokoId)) : toko.getKode().trim();
			String tokoNama = toko == null ? (tokoId == null ? "Semua Toko" : "") : str(toko.getNama());
			java.sql.Connection conn = session.connection();

			StringBuilder where = new StringBuilder("" + kondisiToko("", pendaftarLingkup) + "");
		...
		try {
			...
			java.sql.PreparedStatement psLaba = conn.prepareStatement(
					"SELECT DATE(a.waktu), a.produk, SUM(a.total), SUM(a.qty) FROM koperasi.pembelian a "
							...
							+ (semuaToko ? batasPendaftar("a", pendaftarLingkup) : " AND a.toko = ?")
			...
			java.sql.PreparedStatement psResep = conn.prepareStatement(
					"SELECT p.id, COALESCE(t.nama,'-'), p.nama, COALESCE(p.hargajual,0) FROM koperasi.produk p "
							...
							+ (semuaToko ? batasPendaftar("p", pendaftarLingkup) : " AND p.toko = ?") + " ORDER BY p.nama");
			...
			java.sql.PreparedStatement psBahan = conn.prepareStatement(
					"SELECT COALESCE(pr.nama,'-'), COALESCE(SUM(pb.qty),0), COALESCE(SUM(pb.qty * COALESCE(pr.hargabeli,0)),0) "
							...
							+ (semuaToko ? batasPendaftar("pb", pendaftarLingkup) : " AND pb.toko = ?")
	...
	private void prosesDashboardProduk(Tbmuser tbmuser, JSONObject payload, JSONObject hasil) throws Exception {
		...
		String kondisiToko = semuaToko ? batasPendaftar("a", pendaftarLingkup) : " AND a.toko = ?";
		String tanggalAcuan = tanggalAcuanDariPayload(payload);
		String acuanSql = "DATE '" + tanggalAcuan + "'";
		...
		try {
			...
			java.sql.PreparedStatement psStok = conn.prepareStatement(
					"SELECT c.nama, COALESCE(c.stok,0) FROM koperasi.produk c WHERE (c.aktif = true OR c.aktif IS NULL)"
							+ (semuaToko ? batasPendaftar("c", pendaftarLingkup) : " AND c.toko = ?") + " ORDER BY c.stok ASC LIMIT 100");
			...
			java.sql.PreparedStatement psNilaiStok = conn.prepareStatement(
					"SELECT COALESCE(SUM(COALESCE(stok,0) * COALESCE(hargabeli,0)),0) FROM koperasi.produk "
							...
							+ (semuaToko ? batasPendaftar("", pendaftarLingkup) : " AND toko = ?"));
			if (!semuaToko) {
			...
			java.sql.PreparedStatement psTerlaris = conn.prepareStatement(
					"SELECT c.nama, SUM(a.qty) FROM koperasi.pembelian a INNER JOIN koperasi.produk c ON a.produk = c.id "
							...
							+ (semuaToko ? batasPendaftar("a", pendaftarLingkup) : " AND a.toko = ?")
							+ " GROUP BY c.id, c.nama ORDER BY 2 DESC LIMIT 10");
			if (!semuaToko) psTerlaris.setLong(1, tokoId.longValue());
			...
			java.sql.PreparedStatement psMetode = conn.prepareStatement(
					"SELECT COALESCE(a.carabayar,'-'), SUM(a.total) FROM koperasi.pembelian a "
							+ "WHERE DATE(a.waktu) >= " + acuanSql + " - INTERVAL '1 month' AND DATE(a.waktu) <= " + acuanSql
							+ (semuaToko ? batasPendaftar("a", pendaftarLingkup) : " AND a.toko = ?")
							+ " GROUP BY a.carabayar ORDER BY 2 DESC");
			if (!semuaToko) psMetode.setLong(1, tokoId.longValue());
			...
			java.sql.PreparedStatement psRekapTerlaris = conn.prepareStatement(
					"SELECT c.nama, SUM(a.qty), SUM(a.total) FROM koperasi.pembelian a INNER JOIN koperasi.produk c ON a.produk = c.id "
							+ "WHERE " + intervalRekap
							+ (semuaToko ? batasPendaftar("a", pendaftarLingkup) : " AND a.toko = ?")
							+ " GROUP BY c.id, c.nama ORDER BY 2 DESC");
			if (!semuaToko) psRekapTerlaris.setLong(1, tokoId.longValue());
			...
			java.sql.PreparedStatement psJamSibukProduk = conn.prepareStatement(
					"SELECT c.id, c.nama, EXTRACT(HOUR FROM a.waktu) AS jam, SUM(a.qty) "
							...
							+ "WHERE DATE(a.waktu) >= " + acuanSql + " - INTERVAL '30 days' AND DATE(a.waktu) <= " + acuanSql
							+ (semuaToko ? batasPendaftar("a", pendaftarLingkup) : " AND a.toko = ?") + " "
							+ "AND c.id IN (SELECT b.produk FROM koperasi.pembelian b WHERE DATE(b.waktu) >= " + acuanSql
							+ " - INTERVAL '30 days' AND DATE(b.waktu) <= " + acuanSql + " "
							+ (semuaToko ? batasPendaftar("b", pendaftarLingkup) : " AND b.toko = ?")
							+ " GROUP BY b.produk ORDER BY SUM(b.qty) DESC LIMIT 5) "
			...
			java.sql.PreparedStatement psTurnover = conn.prepareStatement(
					"SELECT pr.kode, pr.nama, SUM(COALESCE(a.qty,0)), COALESCE(pr.stok,0), "
							...
							+ "WHERE " + intervalRekap
							+ (semuaToko ? batasPendaftar("a", pendaftarLingkup) : " AND a.toko = ?")
							+ " GROUP BY pr.id, pr.kode, pr.nama, pr.stok ORDER BY 5 DESC");
			if (!semuaToko) psTurnover.setLong(1, tokoId.longValue());
			...
			java.sql.PreparedStatement psKepuasan = conn.prepareStatement(
					"SELECT COALESCE(AVG(s.rating),0), COUNT(*) FROM koperasi.survey_kepuasan_pos s "
							+ "WHERE s.waktu >= " + acuanSql + " - INTERVAL '30 days' AND DATE(s.waktu) <= " + acuanSql
							+ (semuaToko ? batasPendaftar("s", pendaftarLingkup) : " AND s.toko = ?"));
			if (!semuaToko) psKepuasan.setLong(1, tokoId.longValue());
			...
			java.sql.PreparedStatement psKurang = conn.prepareStatement(
					"SELECT c.nama, COALESCE(SUM(a.qty),0) FROM koperasi.produk c "
							...
							+ "WHERE (c.aktif = true OR c.aktif IS NULL)"
							+ (semuaToko ? batasPendaftar("c", pendaftarLingkup) : " AND c.toko = ?")
							+ " GROUP BY c.id, c.nama HAVING COALESCE(SUM(a.qty),0) <= 5 ORDER BY 2 ASC, c.nama ASC LIMIT 20");
			if (!semuaToko) psKurang.setLong(1, tokoId.longValue());
	 ...
	 * SEMUA toko aktif; peran multi-toko mendapat daftar tokonya sendiri;
	 * pedagang/kasir biasa hanya tokonya. Balasan menyertakan
	 * {@code bolehSemuaToko} supaya klien tahu perlu menampilkan pilihan
	 * "Semua Toko" atau tidak -- keputusan itu TIDAK dibuat di klien.</p>
	 */
	private void prosesTokoFilterList(Tbmuser tbmuser, JSONObject hasil) throws Exception {
		boolean semua = bolehLihatSemuaToko(tbmuser);
		JSONArray arr = new JSONArray();
		Session session = HibernateUtil.getSessionFactory().openSession();
		Long pendaftarId = null;
		try {
			pendaftarId = pendaftarIdPengguna(tbmuser);
			...
			for (Toko t : daftar) {
				...
				JSONObject o = new JSONObject();
				o.put("id", t.getId());
				o.put("nama", t.getNama() == null ? ("Toko " + t.getId()) : t.getNama());
				arr.put(o);
			}
		} finally {
			HibernateUtil.closeSessionQuietly(session);
		...
		hasil.put("status", "success");
		// "Semua Toko" kini juga tersedia bagi pengguna yang terikat pendaftar:
		// artinya seluruh toko MILIKNYA, bukan seluruh basis data. Pembatasnya
		// ada di setiap kueri lewat kondisiToko(...), bukan di sini.
	...
	/**
	 * Isi parameter filter toko yang boleh KOSONG (= seluruh toko).
	 *
	 * <p>Dipakai bersama pola SQL {@code " + kondisiToko("x", pendaftarLingkup) + "}:
	 * bila parameternya NULL, syaratnya selalu benar sehingga seluruh toko
	 * ikut -- TERMASUK baris yang kolom tokonya kosong, sama spt tab Dashboard
	 * yang melepas filternya. COALESCE pada KOLOM diperlukan justru untuk baris
	 * kosong itu: tanpa pembungkus, {@code toko = toko} bernilai UNKNOWN saat
	 * toko NULL dan barisnya diam-diam hilang (terbukti di UAT: 85% baris
	 * koperasi.produk ber-toko NULL). Bila terisi, parameter menyaring ke toko
	 * itu. Bentuk ini sengaja dipilih
	 * daripada menghapus kondisi toko dari SQL, karena menghapus kondisi
	 * berarti MENGGESER nomor parameter berikutnya -- sumber bug diam-diam
	 * (tanggal atau kata kunci masuk ke posisi yang salah) pada laporan yang
	 * dipakai untuk penagihan.</p>
	 */
	private static void isiParamToko(java.sql.PreparedStatement ps, int posisi, Long tokoId)
			throws java.sql.SQLException {
		if (tokoId == null) {
			ps.setNull(posisi, java.sql.Types.BIGINT);
		} else {
			ps.setLong(posisi, tokoId.longValue());
		}
	}
	 ...
	 * <ul>
	 *   <li>filter toko OPSIONAL lewat satu parameter (lihat
	 *       {@link #isiParamToko}) -- kosong berarti semua toko;</li>
	 *   <li>pembatas pendaftar, bila penggunanya terikat tenant. Nilainya
	 *       disisipkan sbg LITERAL, bukan parameter, supaya nomor parameter
	 *       berikutnya tidak bergeser. Aman: angkanya berasal dari relasi di
	 *       basis data ({@code Tbmuser.pendaftar}), bukan dari masukan klien.</li>
	 * </ul>
	 *
	 * <p>Bagi pengguna terikat tenant, baris yang kolom tokonya KOSONG sengaja
	 * tidak ikut: baris tanpa toko bukan milik tenant mana pun.</p>
	 *
	 * @param alias awalan kolom pada kueri (mis. {@code "a"}), boleh kosong
	 */
	private String kondisiToko(String alias, Long pendaftarId) {
		...
		return dasar + " AND " + a + "toko IN (SELECT id FROM koperasi.toko WHERE pendaftar = "
				+ pendaftarId.longValue() + ")";
	}

	/**
	 * Pembatas tenant murni, tanpa filter parameter toko.
	 *
	 * <p>Dipakai tab Dashboard yang menyaring toko dgn cara MELEPAS kondisinya
	 * saat "semua toko" -- melepas kondisi tanpa pembatas ini berarti melihat
	 * seluruh basis data, termasuk tenant lain. Nilainya literal (bukan
	 * parameter) supaya nomor parameter yang sudah ada tidak bergeser; angkanya
	 * berasal dari basis data, bukan masukan klien.</p>
	 */
	private String batasPendaftar(String alias, Long pendaftarId) {
		if (pendaftarId == null) {
			return "";
		}
		String a = (alias == null || alias.length() == 0) ? "" : (alias + ".");
		return " AND " + a + "toko IN (SELECT id FROM koperasi.toko WHERE pendaftar = "
				+ pendaftarId.longValue() + ")";
	}
	 ...
	 * Grup Pengguna, tab Hak Akses Pedagang). Default false -- akses lintas
	 * toko harus dinyalakan sadar.</p>
	 */
	private boolean bolehLihatSemuaToko(Tbmuser tbmuser) {
		try {
			Tbmrole role = tbmuser == null ? null : tbmuser.hakAkses();
			return role != null && Boolean.TRUE.equals(role.getBolehLihatSemuaToko());
		} catch (Exception e) {
			return false;
		}
	}
	...
	private Long resolveTokoId(Tbmuser tbmuser, JSONObject payload) {
		...
		if (bolehLihatSemuaToko(tbmuser)) {
			...
			try {
				Long pendaftarId = pendaftarIdPengguna(tbmuser);
				if (pendaftarId == null) {
				...
				if (tokoDariPayload != null) {
					...
					// Toko yang disebut BUKAN milik pendaftarnya -> jangan layani
					// toko itu; jatuhkan ke agregat tenant sendiri.
					return null;
				}
				// Tidak memilih toko = agregat SELURUH toko miliknya. Aman krn
				// setiap kueri laporan sudah dibatasi kondisiToko(...) ke
