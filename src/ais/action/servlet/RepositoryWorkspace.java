...
public class RepositoryWorkspace extends HttpServlet {
    ...
    private String publicItemUrl(HttpServletRequest request,Long id)throws Exception{String configured=clean(System.getProperty("ais.repository.publicBaseUrl"));if(configured.length()>0){java.net.URL url=new java.net.URL(configured);String protocol=url.getProtocol();if(!("http".equalsIgnoreCase(protocol)||"https".equalsIgnoreCase(protocol))||url.getUserInfo()!=null||url.getQuery()!=null||url.getRef()!=null)throw new IllegalStateException("ais.repository.publicBaseUrl tidak valid.");return configured.replaceAll("/+$","")+request.getContextPath()+"/repository/item/"+id;}String scheme=clean(request.getScheme()).toLowerCase(),host=clean(request.getServerName());if(!("http".equals(scheme)||"https".equals(scheme))||!host.matches("[A-Za-z0-9.-]+|[0-9a-fA-F:]+"))throw new IllegalStateException("Origin publik Repository tidak valid.");String authority=host.indexOf(':')>=0?"["+host+"]":host;String origin=scheme+"://"+authority+((request.getServerPort()==80||request.getServerPort()==443)?"":":"+request.getServerPort());return origin+request.getContextPath()+"/repository/item/"+id;}
