...
/**
 * Servlet penyaji berkas lampiran pengumuman penelitian
 * ({@link LampiranPengumumanPenelitian}) berdasarkan <i>primary key</i>-nya sendiri.
 *
 * <p>Terdaftar di {@code web.xml} dengan nama servlet {@code PengumumanPenelitian} pada
 * alamat {@code /AmbilPengumumanPenelitian}. Susunan kelas ini identik
 * baris-demi-baris dengan {@code AmbilPengumumanAkademis}, hanya kelas entitas
 * sasarannya yang berbeda. Kelas ini <b>tidak memakai</b> {@code LampiranLain} maupun
 * {@code FileFotoLain.ambil(...)}: {@link #process} langsung mengueri
 * {@link LampiranPengumumanPenelitian} lewat {@code Restrictions.idEq(id)} pada parameter
 ...
 * {@code /PengumumanPenelitian = IS_AUTHENTICATED_REMEMBERED}, DAN {@link #process} kini
 * mensyaratkan sesi login serta menolak baris yang induknya
 * ({@link ais.database.model.penelitiandanpengabdian.PengumumanPenelitian#getAktif()}) bernilai
 * {@code false}. Catatan: kelas ini TIDAK punya kolom {@code ditampilkan} per-lampiran
 * (berbeda dari {@code LampiranPengumumanAkademis}/{@code LampiranPengumumanPerkuliahan}),
 * sehingga hanya status {@code aktif} induk yang bisa disaring. Pola akses-tebak-ID ini
 * SEBELUMNYA sama persis dipakai enam kelas {@code Ambil*}/{@code AmbilFile*} bertetangga di
 * paket ini (lih. Javadoc {@code AmbilFilePengajuanPengajuanPenelitianDanPengabdian} untuk
 * daftar lengkapnya) &mdash; kelima kelas itu SUDAH DITAMBAL juga.</p>
 *
 * @see LampiranPengumumanPenelitian
 */
public class AmbilPengumumanPenelitian extends HttpServlet {
	/**
	 ...
	 */
	public AmbilPengumumanPenelitian() {
		super();
	...
	/**
	 * Mencari baris {@link LampiranPengumumanPenelitian} ber-{@code id} yang diminta dan
	 ...
	 * bila semuanya gagal dikenali. Berkas fisik diambil lewat {@code ambilFile()}. Sesi
	 * Hibernate dibersihkan (clear/disconnect/close) di blok {@code finally} tanpa
	 * bergantung pada hasil query.</p>
	 *
	 ...
	 *                {@link LampiranPengumumanPenelitian}
	...
	private void process(HttpServletRequest request, HttpServletResponse resp) throws Exception {
		...
		try {
			...
			LampiranPengumumanPenelitian fileFoto = (LampiranPengumumanPenelitian) session
					.createCriteria(LampiranPengumumanPenelitian.class).add(Restrictions.idEq(Long.parseLong(id)))
					.setMaxResults(1).uniqueResult();

			if (fileFoto != null) {
				if (fileFoto.getPengumumanPenelitian() == null
						|| !Boolean.TRUE.equals(fileFoto.getPengumumanPenelitian().getAktif())) {
			...
			if (session != null) {
				try { session.clear(); } catch (Exception e2) { ais.common.ErrorAuditUtil.record(e2, "auto-audit(empty-catch) src/ais/action/servlet/AmbilPengumumanPenelitian.java:122");}
				try { session.disconnect(); } catch (Exception e2) { ais.common.ErrorAuditUtil.record(e2, "auto-audit(empty-catch) src/ais/action/servlet/AmbilPengumumanPenelitian.java:123");}
				try { session.close(); } catch (Exception e2) { ais.common.ErrorAuditUtil.record(e2, "auto-audit(empty-catch) src/ais/action/servlet/AmbilPengumumanPenelitian.java:124");}
			}
		}
