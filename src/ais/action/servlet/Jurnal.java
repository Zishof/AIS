...
public final class Jurnal extends HttpServlet {
    ...
    private static String date(java.util.Date d){if(d==null)d=new java.util.Date(0);SimpleDateFormat f=new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'");f.setTimeZone(TimeZone.getTimeZone("UTC"));return f.format(d);}private static String rfc822(java.util.Date d){if(d==null)d=new java.util.Date(0);SimpleDateFormat f=new SimpleDateFormat("EEE, dd MMM yyyy HH:mm:ss Z",java.util.Locale.ENGLISH);f.setTimeZone(TimeZone.getTimeZone("GMT"));return f.format(d);}private static String xml(String v){if(v==null)return"";return v.replace("&","&amp;").replace("<","&lt;").replace(">","&gt;").replace("\"","&quot;").replace("'","&apos;");}
}
