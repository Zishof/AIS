...
public class AntarJemput extends HttpServlet {
	...
	private void renderGatePage(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		html.append("<div class='tools'><a class='btn secondary' href='").append(root).append("/antarJemput?action=card'>Cetak Kartu QR</a></div></div>");
		...
		html.append("<script>");
		html.append("var root='").append(js(root)).append("',stream=null,detector=null,scanning=false,last='';");
	...
	private void renderCardPage(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		html.append("<form class='toolbar' method='get' action='").append(root).append("/antarJemput'><input type='hidden' name='action' value='card'><input name='nomor' value='").append(esc(nomor)).append("' placeholder='Nomor kartu / QR'><button>Cari</button><a class='btn' href='").append(root).append("/antarJemput'>Verifikasi Gerbang</a><button type='button' onclick='window.print()'>Cetak</button></form>");
