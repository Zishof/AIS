...
/**
 * Controller/action ZK untuk pmb. Tipe ini merupakan titik masuk UI yang menghubungkan event layar
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code String KONFIG_PMB_LOGIN_LOGOUT_COOKIE},
 * {@code String COOKIE_PMB_BIODATA}, {@code String COOKIE_PMB_USERID}, {@code Div kopBox}, {@code Div
 * container}, {@code Div footer}, {@code Div wacontainer}, {@code Toolbar menubar}; inisialisasi/lifecycle
 * ({@code doBeforeCompose()}, {@code initHeader()}, {@code initBg()}, {@code initFooter()}, {@code
 * initPencarianMenu()}, {@code doAfterCompose()}); pembacaan/pencarian ({@code getKonfigurasiNilai()}, {@code
 * getUrlRedirectLogoutPMB()}, {@code tampilWa()}, {@code onClickCariDataUjian()}, {@code
 * onClickCariPembayaran()}, {@code onClickCariDataDaftar()}); mutasi data ({@code redirectSetelahLogoutPMB()},
 * {@code setLogoutCalonMahasiswaPMB()}, {@code setLogoutCalonMahasiswaPMB()}); penghapusan/pembatalan ({@code
 * hapusCookiePMB()}, {@code hapusCookie()}); operasi domain lain ({@code isBlank()}, {@code
 * isKonfigurasiAktif()}, {@code escapeJavaScript()}, {@code menggunakanCookieLoginLogoutPMB()}, {@code
 * isLoginCalonMahasiswaPMB()}, {@code isLoginCalonMahasiswaPMB()}). Bagian lain dari kontrak tetap mengikuti
