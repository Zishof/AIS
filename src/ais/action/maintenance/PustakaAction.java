...
/**
 * Controller/action ZK untuk pustaka. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code MyWindow main}, {@code Center
 * papanPengumuman}, {@code Borderlayout pustakaLayout}, {@code Image imgLogo}, {@code Label namaSeleksi}, {@code
 * Label namaSekolah}, {@code Label alamatSekolah}, {@code Integer desktopHeight}; inisialisasi/lifecycle ({@code
 * doBeforeCompose()}, {@code initHeader()}, {@code doAfterCompose()}, {@code initCriteria()}, {@code
 * initCriteria()}); pembacaan/pencarian ({@code onSearchDefault()}, {@code onSearchDefault()}); operasi domain
 * lain ({@code onInfo()}, {@code berkunjung()}, {@code displayPustaka()}). Bagian lain dari kontrak tetap
...
public class PustakaAction extends GenericAutowireComposer implements DataCriteria {
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link PustakaAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link PustakaAction} dan dapat mengakses state kelas
