...
/**
 * Controller/action ZK untuk tbmrole. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * addMenu}, {@code MyGrid grid}, {@code Paging paging}, {@code MyTextbox searchnama}, {@code MyTextbox
 * searchid}, {@code Checkbox searchaktif}, {@code MyTextbox nama}; inisialisasi/lifecycle ({@code
 * doBeforeCompose()}, {@code doAfterCompose()}, {@code init()}, {@code initCriteria()}); pembacaan/pencarian
 * ({@code onSearchDefault()}); validasi/perhitungan ({@code tambahCheckboxJurnal()}, {@code
 * validasiTidakMenaikkanHakSendiri()}, {@code jurnalChecked()}, {@code checkKodeTbmrole()}, {@code
 * checkNamaTbmrole()}); mutasi data ({@code parseTokoAksesJson()}, {@code onSave()}); operasi domain lain
 * ({@code onAdd()}, {@code bangunPanelHakAksesJurnal()}, {@code labelAksiJurnal()}, {@code
 * buildJurnalAksesJson()}, {@code appendBarisKatalogSisanya()}, {@code tambahHeaderKolomCrud()}). Bagian lain
