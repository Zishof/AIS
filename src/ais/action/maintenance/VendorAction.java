...
/**
 * Controller/action ZK untuk vendor. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * Borderlayout vendorLayout}, {@code MyWindow main}, {@code Image imgLogo}, {@code Row rowPencarian}, {@code Row
 ...
 */
public class VendorAction extends GenericAutowireComposer {
	...
	private Borderlayout vendorLayout;
...
//		logout.setVisible(penyediaAsset != null);
		...
		if (desktopWidth < ConstantValues.UKURAN_BATAS_MOBILE) {
			...
			Label namaSekolah = new Label(Common
					.getKonfigurasi("label_penyedia_header", "Informasi Proyek Pekerjaan dan Pengumuman").getNilai());
...
//			hbox.appendChild(logout);
			...
			MyToolbarbutton daftar = new MyToolbarbutton("fa-check-square-o fa-big", "Daftarkan Perusahaan", true);
...
//			hbox1.appendChild(logout);
		...
		login.addEventListener("onClick", new EventListener() {
			...
			public void onEvent(Event arg0) throws Exception {
				...
				Common.displayWindow("/pages/vendor/login_vendor.zul", true, "180px",
...
//		});
	...
	public void onInfo(ClientInfoEvent evt) throws Exception {
		...
		if (!vendorLayout.getChildren().isEmpty()) {
			return;
		}
		...
		String tinggi = (Common.isMobile(request) ? "1800"
				: Common.getKonfigurasi("tinggi_halaman_utama_vendor", "800").getNilai());
		main.setHeight(tinggi + "px");

		Common.clear(vendorLayout);
		papanPengumuman = new Center();
		papanPengumuman.setBorder("none");
		vendorLayout.appendChild(papanPengumuman);
		...
		String notelp = Common.getKonfigurasi("no_whatsapp_vendor", "0811111111111111").getNilai();
		...
		vendorLayout.appendChild(footer);
		...
		if (notelp != null && !notelp.trim().isEmpty()) {
			...
			String tanya = Common.getKonfigurasi("tanya_whatsapp_vendor", "Salamat Datang, apa yang bisa kami bantu?")
					.getNilai();
			String jawab = Common
					.getKonfigurasi("jawab_whatsapp_vendor",
							"Saya ingin menanyakan tentang informasi seleksi vendor, apakah Anda bisa membantu?")
					.getNilai();
...
//		System.out.println("execution => " + execution.getParameterMap());
	...
	public void onClickPMB() throws Exception {
		PenyediaAsset penyediaAsset = new PenyediaAsset();

		PenyediaAssetAction.onAddExternalDaftar(null, new EventListener() {
	...
	public void onPengumuman(PengumumanAkademis selectedPengumumanAkademis) {
		if (papanPengumuman != null) {
			...
			PenyediaAsset penyediaAsset = Common.isLoginPenyediaAsset();
			TampilanPengumumanAkademisAction.tampilGelombang(selectedPerguruanTinggi, north, penyediaAsset, mobil);
			...
			if (selectedPengumumanAkademis != null) {
				...
				List<PengumumanAkademis> listPengumumanAkademis = ConstantValues.simpleList(
						TampilanPengumumanVendorAction.initCriteriaStatic(true, selectedSekolah, selectedYayasan)
	...
	private void initMenuPengumuman(final Textbox cari) {
		...
		Criteria criteria = TampilanPengumumanVendorAction.initCriteriaStatic(true, selectedSekolah, selectedYayasan);
