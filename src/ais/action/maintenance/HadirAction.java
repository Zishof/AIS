...
/**
 * Controller/action ZK untuk hadir. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * tumpang tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code MyWindow main}, {@code Borderlayout
 * hadirLayout}, {@code Image imgLogo}, {@code Html htmlKehadiran}, {@code Integer desktopHeight}, {@code Integer
 * desktopWidth}, {@code boolean merupakanMobile}, {@code Tbmuser tbmuser}; inisialisasi/lifecycle ({@code
 * doBeforeCompose()}, {@code initHeader()}, {@code doAfterCompose()}); pembacaan/pencarian ({@code
 * refreshKehadiran()}); pelaporan/ekspor ({@code renderKehadiranHtml()}); operasi domain lain ({@code
 * onInfo()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di atas.</p>
