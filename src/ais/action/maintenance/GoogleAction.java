...
/**
 * Controller/action ZK untuk google. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * String SESSION_ORIGIN_CALLBACK_URL}, {@code OAuth20Service service}, {@code Tbmuser currentUser};
 * inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code doAfterCompose()}); mutasi data ({@code proses()});
 * operasi domain lain ({@code gangguanJaringanGoogle()}, {@code executeGoogleUserInfo()}, {@code
 * resolveOriginCallbackUrl()}, {@code safeUrlDecode()}, {@code resolveExternalContextPath()}, {@code
 * resolveExternalHostWithContextPath()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface
