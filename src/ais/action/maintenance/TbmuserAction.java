...
/**
 * Controller/action ZK untuk tbmuser. Tipe ini merupakan titik masuk UI yang menghubungkan event
 ...
 * grid}, {@code Textbox searchnama}, {@code AmbilDataSatuanKerjaBanbox searchparent}, {@code MyCheckboxConfig
 * searchFacebook}, {@code MyCheckboxConfig searchGoogle}, {@code MyCheckboxConfig searchTwitter}, {@code
 * MyCheckboxConfig searchLinkedin}; inisialisasi/lifecycle ({@code doBeforeCompose()}, {@code doAfterCompose()},
 * {@code init()}, {@code initCriteria()}); pembacaan/pencarian ({@code tampilkanSocialMediaProfile()}, {@code
 * onDownloadFotoMassal()}, {@code onUploadFotoMassal()}, {@code tampilkanIdPenggunaSudahDipakai()}, {@code
 * onSearchDefault()}); mutasi data ({@code updateUser()}, {@code onSave()}); operasi domain lain ({@code
 * onAddExternal()}, {@code onAddExternal()}, {@code onAdd()}, {@code adalahDuplikatIdPengguna()}). Bagian lain
...
//	private Textbox searchemail;
	...
	/**
	 * Renderer lokal untuk layar/komponen {@link TbmuserAction}. Kelas ini menerjemahkan satu item data menjadi
	 * baris atau komponen ZK dengan memakai state dan aturan tampilan milik kelas induk.
	 *
	 * <p><b>Scope:</b> setiap instance terikat pada instance {@link TbmuserAction} dan dapat mengakses state kelas
