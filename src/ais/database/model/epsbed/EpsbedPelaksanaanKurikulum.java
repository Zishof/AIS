...
/**
 * Tabel referensi berkode untuk <b>status pelaksanaan kurikulum</b> pada pelaporan EPSBED
 * ("Evaluasi Program Studi Berbasis Evaluasi Diri", format pelaporan Dikti/Kemdikbud yang lebih
 * tua dari feeder PDDIKTI) — mis. apakah kurikulum program studi sudah dilaksanakan sepenuhnya,
 * sedang transisi, atau kondisi lain sesuai kamus data EPSBED. Setiap baris hanya berupa
 * pasangan {@code kode} (maks. 2 karakter) dan {@code nama} (label tampilan); nilai kode aktual
 * berasal dari data yang di-seed ke database, bukan dari kode Java — tidak ditemukan seed
 * literal untuk tabel ini di {@code InitDataHelper}/{@code InitData}, jadi daftar kode aktif
 * harus dicek langsung di tabel {@code epsbed.epsbed_pelaksanaan_kurikulum}.
 *
 * <p><b>Pemakaian aktif</b> — kelas ini BUKAN entity yatim: dirujuk sebagai {@code ManyToOne}
 * oleh {@link ais.database.model.JenjangProgramStudi#getEpsbedPelaksanaanKurikulum()} (kolom
 * {@code epsbed_pelaksanaan_kurikulum} pada tabel {@code jenjang_program_studi}) dan diisi lewat
 ...
 * yang sama pada {@code JenjangProgramStudi}: {@code pelaksanaanKurikulum} adalah teks bebas
 * (kolom {@code pelaksanaan_kurikulum}), sedangkan referensi berkode ke kelas ini
 * ({@code epsbedPelaksanaanKurikulum}) diisi terpisah pada layar yang sama dan <b>tidak
 * disinkronkan otomatis</b> satu sama lain — keduanya independen dari sumbu
 * {@link EpsbedFrekuensiKurikulum} (frekuensi peninjauan kurikulum), yang juga merupakan pasangan
 * teks-bebas/berkode terpisah pada entity yang sama; frekuensi (seberapa sering ditinjau) dan
 * pelaksanaan (status penerapan) adalah dua sumbu yang independen, bukan dua nama untuk hal yang
 * sama.</p>
 *
...
@Audited
@Table(schema = "epsbed", name = "epsbed_pelaksanaan_kurikulum")



public class EpsbedPelaksanaanKurikulum extends GeneralValueObject {
	...
	/** Label tampilan kode ini, mis. untuk combobox pemilihan status pelaksanaan kurikulum. */
	private String nama;
	/** Kode resmi EPSBED (maks. 2 karakter) untuk status pelaksanaan kurikulum ini. */
	private String kode;

	/** Konstruktor tanpa argumen yang disyaratkan Hibernate untuk instansiasi lewat refleksi. */
	public EpsbedPelaksanaanKurikulum() {
	}
	...
	/**
	 * Mengembalikan kode resmi EPSBED (maks. 2 karakter) untuk status pelaksanaan kurikulum ini.
	 *
	 * @return {@link #kode}
	 */
