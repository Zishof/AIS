...
/**
 * Tabel referensi berkode untuk <b>peran penulis</b> pada pelaporan EPSBED ("Evaluasi Program
 * Studi Berbasis Evaluasi Diri", format pelaporan Dikti/Kemdikbud yang lebih tua dari feeder
 * PDDIKTI) — mis. penulis utama/ketua, anggota, penulis mandiri, dsb. sesuai kamus data EPSBED.
 * Setiap baris hanya berupa pasangan {@code kode} (maks. 2 karakter) dan {@code nama} (label
 * tampilan); nilai kode aktual berasal dari data yang di-seed ke database, bukan dari kode
 * Java — tidak ditemukan seed literal untuk tabel ini di {@code InitDataHelper}/{@code
 * InitData}, jadi daftar kode aktif harus dicek langsung di tabel {@code
 * epsbed.epsbed_peran_penulisan}.
 *
 * <p><b>Pemakaian aktif</b> — kelas ini BUKAN entity yatim: dirujuk sebagai {@code ManyToOne}
 * ({@code kodeAuthor}, kolom {@code kode_author}) oleh kedua entity transaksi publikasi dosen
 * di paket ini, {@code EpsbedPublikasiDosen} dan {@code EpsbedPublikasiIlmiahDosen}, dan diisi
 * lewat combobox di {@code ais.action.master.epsbed.PublikasiDosenHelper} serta {@code
 * ais.action.master.epsbed.PublikasiIlmiahDosenHelper} — pada kedua helper tersebut ada baris
 * kode yang sengaja dikomentari ({@code // EpsbedPeranPenulisan.class}) tepat sebelum
 * pemanggilan combobox yang sebenarnya, menunjukkan konfigurasi combobox ini pernah diubah/
 * dicoba ulang secara manual.</p>
 *
 * <p>Konsep peran penulis di sini tidak berelasi secara teknis (tidak ada FK/import) dengan
 * modul penelitian yang lebih baru di {@code ais.database.model.penelitiandanpengabdian} (mis.
 * {@code AnggotaArtikel}, {@code JenisJabatanPenelitianDanPengabdian}) — keduanya memodelkan hal
 * yang mirip secara domain namun berjalan sebagai dua sistem kode yang terpisah dan tidak
 * sinkron.</p>
 *
...
@Audited
@Table(schema = "epsbed", name = "epsbed_peran_penulisan")



public class EpsbedPeranPenulisan extends GeneralValueObject {
	...
	/** Label tampilan kode ini, mis. untuk combobox pemilihan peran dalam penelitian. */
	private String nama;
	/** Kode resmi EPSBED (maks. 2 karakter) untuk peran penulis ini. */
	private String kode;

	/** Konstruktor tanpa argumen yang disyaratkan Hibernate untuk instansiasi lewat refleksi. */
	public EpsbedPeranPenulisan() {
	}
	...
	/**
	 * Mengembalikan kode resmi EPSBED (maks. 2 karakter) untuk peran penulis ini.
	 *
	 * @return {@link #kode}
	 */
