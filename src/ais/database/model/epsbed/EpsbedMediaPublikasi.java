...
/**
 * Tabel referensi berkode untuk <b>media publikasi</b> pada pelaporan EPSBED ("Evaluasi Program
 * Studi Berbasis Evaluasi Diri", format pelaporan Dikti/Kemdikbud yang lebih tua dari feeder
 * PDDIKTI) — tempat karya ilmiah dosen dipublikasikan (mis. jurnal terakreditasi, jurnal
 * ber-ISSN tanpa akreditasi, prosiding seminar nasional/internasional, dsb. sesuai kamus data
 * EPSBED). Setiap baris hanya berupa pasangan {@code kode} (maks. 2 karakter) dan {@code nama}
 * (label tampilan); nilai kode aktual berasal dari data yang di-seed ke database, bukan dari
 * kode Java — tidak ditemukan seed literal untuk tabel ini di {@code
 * InitDataHelper}/{@code InitData}, jadi daftar kode aktif harus dicek langsung di tabel
 * {@code epsbed.epsbed_media_publikasi}.
 *
 * <p><b>Pemakaian aktif</b> — kelas ini BUKAN entity yatim: dirujuk sebagai {@code ManyToOne}
 * ({@code kodeMediaPublikasi}, kolom {@code media_publikasi}) oleh kedua entity transaksi
 * publikasi dosen di paket ini, {@code EpsbedPublikasiDosen} dan
 * {@code EpsbedPublikasiIlmiahDosen}, dan diisi lewat combobox di {@code
 * ais.action.master.epsbed.PublikasiDosenHelper} serta {@code
 * ais.action.master.epsbed.PublikasiIlmiahDosenHelper}. Data yang terkumpul lewat combobox ini
 * lalu dipakai kembali oleh laporan akreditasi BAN-PT yang masih aktif: {@code
 * ais.action.master.akreditasi.LaporanLKPS_3C2_PublikasiDosen} melakukan {@code LEFT JOIN}
 * langsung ke tabel {@code epsbed.epsbed_media_publikasi} untuk mengambil nama jenis media pada
 * laporan LKPS-3.C.2 — modul EPSBED yang tampak legasi ini ternyata masih dipakai sebagai
 * sumber data untuk pelaporan akreditasi terkini, bukan sekadar peninggalan format lama.</p>
 *
 * <p>Konsep media publikasi di sini tidak berelasi secara teknis (tidak ada FK/import) dengan
 * modul penelitian yang lebih baru di {@code ais.database.model.penelitiandanpengabdian} (mis.
 * {@code JurnalPenelitian}) — keduanya memodelkan hal yang mirip secara domain namun berjalan
 * sebagai dua sistem kode yang terpisah dan tidak sinkron.</p>
 *
...
@Audited
@Table(schema = "epsbed", name = "epsbed_media_publikasi")



public class EpsbedMediaPublikasi extends GeneralValueObject {
	...
	/** Label tampilan kode ini, mis. untuk combobox pemilihan media publikasi. */
	private String nama;
	/** Kode resmi EPSBED (maks. 2 karakter) untuk jenis media publikasi ini. */
	private String kode;

	/** Konstruktor tanpa argumen yang disyaratkan Hibernate untuk instansiasi lewat refleksi. */
	public EpsbedMediaPublikasi() {
	}
	...
	/**
	 * Mengembalikan kode resmi EPSBED (maks. 2 karakter) untuk jenis media publikasi ini.
	 *
	 * @return {@link #kode}
	 */
