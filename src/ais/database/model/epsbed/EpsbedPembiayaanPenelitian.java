...
/**
 * Tabel referensi berkode untuk <b>sumber pembiayaan penelitian</b> pada pelaporan EPSBED
 * ("Evaluasi Program Studi Berbasis Evaluasi Diri", format pelaporan Dikti/Kemdikbud yang lebih
 * tua dari feeder PDDIKTI) — mis. dibiayai perguruan tinggi sendiri, Depdiknas/Ditjen Dikti,
 * sumber luar negeri, mandiri, dsb. sesuai kamus data EPSBED. Setiap baris hanya berupa pasangan
 * {@code kode} (maks. 2 karakter) dan {@code nama} (label tampilan); nilai kode aktual berasal
 * dari data yang di-seed ke database, bukan dari kode Java — tidak ditemukan seed literal untuk
 * tabel ini di {@code InitDataHelper}/{@code InitData}, jadi daftar kode aktif harus dicek
 * langsung di tabel {@code epsbed.epsbed_pembiayaan_penelitian}.
 *
 * <p><b>Pemakaian aktif</b> — kelas ini BUKAN entity yatim: dirujuk sebagai {@code ManyToOne}
 * ({@code kodePembiayaan}, kolom {@code kode_pembiayaan}) oleh kedua entity transaksi publikasi
 * dosen di paket ini, {@code EpsbedPublikasiDosen} dan {@code EpsbedPublikasiIlmiahDosen}
 * (berdampingan dengan {@code jumlahBiaya} pada entity yang sama), dan diisi lewat combobox di
 * {@code ais.action.master.epsbed.PublikasiDosenHelper} serta {@code
 * ais.action.master.epsbed.PublikasiIlmiahDosenHelper}.</p>
 *
 * <p>Konsep sumber pembiayaan di sini tidak berelasi secara teknis (tidak ada FK/import) dengan
 * modul penelitian yang lebih baru di {@code ais.database.model.penelitiandanpengabdian} (mis.
 * {@code SumberDanaPenelitianDanPengabdian}) — keduanya memodelkan hal yang mirip secara domain
 * namun berjalan sebagai dua sistem kode yang terpisah dan tidak sinkron.</p>
 *
...
@Audited
@Table(schema = "epsbed", name = "epsbed_pembiayaan_penelitian")



public class EpsbedPembiayaanPenelitian extends GeneralValueObject {
	...
	/** Label tampilan kode ini, mis. untuk combobox pemilihan sumber pembiayaan penelitian. */
	private String nama;
	/** Kode resmi EPSBED (maks. 2 karakter) untuk sumber pembiayaan penelitian ini. */
	private String kode;

	/** Konstruktor tanpa argumen yang disyaratkan Hibernate untuk instansiasi lewat refleksi. */
	public EpsbedPembiayaanPenelitian() {
	}
	...
	/**
	 * Mengembalikan kode resmi EPSBED (maks. 2 karakter) untuk sumber pembiayaan penelitian ini.
	 *
	 * @return {@link #kode}
	 */
