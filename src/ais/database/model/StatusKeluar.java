...
/**
 * Entity master data <b>status keluar</b> pada tabel {@code public.status_keluar}. Mendaftar
 * alasan/status keluarnya seorang mahasiswa dari perguruan tinggi (mis. lulus, mengundurkan diri,
 * drop-out, meninggal dunia) yang dirujuk oleh {@code Mahasiswa}, lengkap dengan
 * {@link #getFeeder() kode padanan Feeder/PDDIKTI} untuk keperluan pelaporan.
 *
...
@Audited
@Table(schema = "public", name = "status_keluar")
public class StatusKeluar extends GeneralValueObject {
	...
	/** Kunci utama tabel {@code public.status_keluar} ({@code IDENTITY}). */
	private Long id;
	...
	/** Nama status keluar (mis. "Lulus", "Mengundurkan Diri", "Drop Out"); wajib diisi. */
	private String nama;
	/** Catatan/keterangan bebas tentang status ini. */
	private String keterangan;
	/** Kode padanan status ini pada skema Feeder/PDDIKTI. */
	...
	/** @param id kunci utama baris ini, langsung disetel lewat konstruktor ini (mis. sebagai referensi ringan). */
	public StatusKeluar(Long id) {
		this.id = id;
	}
	...
	/** @return nama status keluar, di-trim; {@code null} bila belum pernah diisi. */
	@Column(name = "nama", nullable = false, length = 255)
	public String getNama() {
		return this.nama == null ? null : this.nama.trim();
	}

	/** @param nama nama status keluar; disimpan apa adanya, trimming terjadi di {@link #getNama()}. */
	public void setNama(String nama) {
		this.nama = nama;
	}

	/** @return keterangan bebas status ini, atau string kosong (bukan {@code null}) bila belum pernah diisi. */
	@Column(name = "keterangan", nullable = true)
	public String getKeterangan() {
		return this.keterangan == null ? "" : this.keterangan;
	}

	/** @param keterangan catatan/keterangan bebas untuk status ini. */
	public void setKeterangan(String keterangan) {
		this.keterangan = keterangan;
	}

	/** @return kode padanan Feeder/PDDIKTI, di-trim; {@code null} bila kosong/belum pernah diisi. */
	public String getFeeder() {
		return feeder == null || feeder.trim().isEmpty() ? null : feeder.trim();
	}

	/** @param feeder kode padanan status ini pada skema Feeder/PDDIKTI; disimpan apa adanya. */
