...
/**
 * Entitas hasil sinkronisasi SISTER untuk endpoint referensi <b>{@code referensi/gelar_akademik}</b> (daftar
 * gelar akademik, mis. S.Kom., M.T., Ph.D.). Struktur kolom inti, mekanisme upsert refleksi generik
 * ({@link ais.database.model.sister.RefAgamaSister}), pola field audit shadow, dan default aktif=true —
 ...
 * {@link #singkatan} (field JSON {@code "singkatan"}, mis. "S.Kom"). {@link #kode}=id item SISTER (kunci
 * upsert); {@link #keterangan}=JSON mentah baris ini. Dipetakan di {@link SisterEntitasRegistry} pada key
 * {@code "referensi/gelar_akademik"}. {@code @Audited} (tabel bayangan {@code sister_ref_gelar_akademik_AUD}
 * otomatis via hbm2ddl).
 */
@Entity
@org.hibernate.annotations.Entity(dynamicInsert = true, dynamicUpdate = true)
@Audited
@Table(schema = "public", name = "sister_ref_gelar_akademik")
public class RefGelarAkademikSister extends GeneralValueObject {
	...
	/** Nama lengkap gelar akademik (field JSON {@code "nama"}). */
	private String nama;
	/** Singkatan gelar akademik (field JSON {@code "singkatan"}, mis. "S.Kom") — kolom tambahan kelas ini, tak ada di 32 kelas {@code Ref*Sister} murni. */
	private String singkatan;

	/** Constructor default (kontrak JPA/Hibernate — instansiasi via refleksi). */
	public RefGelarAkademikSister() {}
	...
	/** @return nama lengkap gelar akademik. */
	@Column(name = "nama", columnDefinition = "text") public String getNama() { return nama; }
	/** @param v nama lengkap gelar akademik baru. */
	public void setNama(String v) { this.nama = v; }
	/** @return singkatan gelar akademik. */
	@Column(name = "singkatan", columnDefinition = "text") public String getSingkatan() { return singkatan; }
	/** @param v singkatan gelar akademik baru. */
	public void setSingkatan(String v) { this.singkatan = v; }
