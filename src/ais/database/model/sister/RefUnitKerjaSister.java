...
/**
 * Entitas hasil sinkronisasi SISTER untuk endpoint referensi <b>{@code referensi/unit_kerja}</b> (daftar unit
 * kerja/organisasi pada perguruan tinggi — fakultas, program studi, biro, dan sejenisnya — milik
 * {@code id_perguruan_tinggi} tertentu, lihat parameter query di {@code DataSisterApi.daftarEndpoint}).
 * Struktur kolom inti, mekanisme upsert refleksi generik, pola field audit shadow, dan default aktif=true —
 * identik dengan kelas pola {@link RefAgamaSister}; lihat javadoc di sana untuk penjelasan lengkap mesin
 * sinkronisasi. Berbeda dari 32 kelas {@code Ref*Sister} murni, kelas ini punya SATU kolom tambahan:
 * {@link #idJenisUnit} (field JSON {@code "id_jenis_unit"}). {@link #kode}=id item SISTER (kunci upsert);
 * {@link #keterangan}=JSON mentah baris ini. Dipetakan di {@link SisterEntitasRegistry} pada key
 * {@code "referensi/unit_kerja"}; kode hasil sinkronisasi kelas ini juga dipakai ulang oleh
 * {@code DataSisterApi.synDataSister()} untuk menarik {@code referensi/detail_unit_kerja} per unit dan
 * meng-upsertnya ke tabel {@code rab.SatuanKerja} (pemetaan lintas-tabel, bukan kelas {@code Ref*Sister}).
 * {@code @Audited} (tabel bayangan {@code sister_ref_unit_kerja_AUD} otomatis via hbm2ddl).
 */
@Entity
@org.hibernate.annotations.Entity(dynamicInsert = true, dynamicUpdate = true)
@Audited
@Table(schema = "public", name = "sister_ref_unit_kerja")
public class RefUnitKerjaSister extends GeneralValueObject {
	...
	/** Kode/id item SISTER (field JSON {@code "id"}) — kunci upsert sinkronisasi; dipakai ulang oleh {@code DataSisterApi} untuk menyusun URL {@code referensi/detail_unit_kerja?id_unit_kerja=...}. */
	private String kode;
	...
	/** Nama unit kerja (field JSON {@code "nama"}). */
	private String nama;
	/** ID jenis unit kerja (field JSON {@code "id_jenis_unit"}, mis. fakultas/prodi/biro) — kolom tambahan kelas ini, tak ada di 32 kelas {@code Ref*Sister} murni. */
	private Integer idJenisUnit;

	/** Constructor default (kontrak JPA/Hibernate — instansiasi via refleksi). */
	public RefUnitKerjaSister() {}
	...
	/** @param v nama unit kerja baru. */
	public void setNama(String v) { this.nama = v; }
	/** @return id jenis unit kerja. */
	@Column(name = "id_jenis_unit") public Integer getIdJenisUnit() { return idJenisUnit; }
	/** @param v id jenis unit kerja baru. */
	public void setIdJenisUnit(Integer v) { this.idJenisUnit = v; }
