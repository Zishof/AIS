...
/**
 * <h2>Entitas Tridharma tersinkron SISTER &mdash; Kepangkatan</h2>
 *
 * <p>Baris tabel {@code sister_trid_kepangkatan}, hasil sinkronisasi endpoint SISTER (Kemdikbudristek)
 * <b>{@code kepangkatan}</b> &mdash; riwayat pangkat/golongan kepegawaian seorang dosen (mis. Penata Muda,
 * Pembina). Mengikuti KERANGKA STRUKTUR &amp; MEKANISME SINKRONISASI yang identik dengan
 * {@link TridPenelitianSister} (lihat javadoc kelas tersebut untuk penjelasan lengkap): terdaftar di
 * {@link SisterEntitasRegistry} pada kunci {@code "kepangkatan"}, dirutekan &amp; di-upsert secara reflektif
 * oleh {@link ais.common.DataSisterApi} berdasarkan {@link #getKode()}, dengan {@link #getKeterangan()}
 * menyimpan salinan JSON mentah dan {@link #getAktif()} tetap flag satu-arah. Seperti kebanyakan endpoint
 * Tridharma "karier" lain (BUKAN {@code penelitian}/{@code pengabdian}), endpoint {@code kepangkatan}
 * TIDAK BER-PAGINASI &mdash; seluruh riwayat kepangkatan seorang dosen ditarik dalam satu respons.</p>
 *
 * <p>Field bisnis endpoint ini: {@link #getPangkatGolongan() pangkatGolongan} (nama pangkat/golongan),
 * {@link #getSk() sk} (nomor SK kenaikan pangkat), {@link #getTanggalMulai() tanggalMulai} (tanggal mulai
 * berlaku pangkat).</p>
 *
...
@Audited
@Table(schema = "public", name = "sister_trid_kepangkatan")
public class TridKepangkatanSister extends GeneralValueObject {
	...
	/** Nama pangkat/golongan kepegawaian (mis. Penata Muda, Pembina). */
	private String pangkatGolongan;
	/** Nomor Surat Keputusan (SK) kenaikan pangkat/golongan. */
	private String sk;
	/** Tanggal mulai berlaku pangkat/golongan (string sesuai format SISTER, bukan {@code Date}). */
	private String tanggalMulai;

	/** Konstruktor bawaan (dibutuhkan Hibernate &amp; refleksi upsert {@code DataSisterApi}); field diisi lewat setter. */
	public TridKepangkatanSister() {}
