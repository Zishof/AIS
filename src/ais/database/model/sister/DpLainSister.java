...
/**
 * Entitas hasil sinkronisasi SISTER untuk endpoint <b>data_pribadi/lain</b>. Kolom bernama = field SISTER;
 ...
 * {@link ais.database.model.sister.SisterEntitasRegistry} berdasarkan kunci {@code "data_pribadi/lain"},
 * {@code kode} dipaksa sama dengan {@link #getIdSdm()}). Kelas ini adalah SAUDARA {@code DpProfilSister},
 * bukan detailnya. Namanya generik ("lain"/"other") tetapi isinya sejauh ini murni data perpajakan
 * (NPWP); tidak ada indikasi field lain yang tersembunyi di {@link #getKeterangan()} yang belum dipetakan
 * — respons endpoint ini sesuai peninjauan hanya berisi dua field terstruktur di bawah.</p>
 *
 * <h2>Peringatan privasi</h2>
 * <p>{@link #getNpwp()} menyimpan Nomor Pokok Wajib Pajak, data pribadi terkait keuangan yang sensitif.
 * Sama seperti {@code DpProfilSister}, tidak ada kolom scoping tenant/satuan-kerja dan (per 5 Sep 2026)
 * tidak ditemukan Action/JSP yang membaca balik tabel ini untuk ditampilkan — risiko murni pada sisi tulis
 * lewat layar "Data SISTER". Memperkuat pola tercatat (filter tenant lemah/hilang pada entitas SISTER),
 * bukan temuan baru yang berdiri sendiri.</p>
 *
 * @see DpProfilSister
 */
@Entity
...
@Audited
@Table(schema = "public", name = "sister_dp_lain")
public class DpLainSister extends GeneralValueObject {
	...
	private String kode;
	/** JSON mentah respons endpoint SISTER untuk satu dosen ({@code data_pribadi/lain/{id_sdm}} apa adanya), disimpan berdampingan dengan kolom terstruktur sebagai cadangan. */
	...
	/** Nomor Pokok Wajib Pajak (NPWP) dosen/SDM sebagaimana tercatat di SISTER. */
	private String npwp;
	/** Nama wajib pajak sebagaimana tercatat pada NPWP di SISTER (dapat berbeda dari {@code DpProfilSister.getNama()} bila NPWP belum diperbarui). */
	...
	/**
	 * Mengembalikan Nomor Pokok Wajib Pajak (NPWP) dosen/SDM.
	 *
	 * @return NPWP, atau {@code null} bila belum terisi
	 */
	@Column(name = "npwp", columnDefinition = "text") public String getNpwp() { return npwp; }
	/**
	 * Menyetel Nomor Pokok Wajib Pajak (NPWP) dosen/SDM. Tanpa validasi format.
	 *
	 * @param v NPWP baru
	 */
	public void setNpwp(String v) { this.npwp = v; }
	/**
	 * Mengembalikan nama wajib pajak sebagaimana tercatat pada NPWP di SISTER.
	 *
	 * @return nama wajib pajak, atau {@code null} bila belum terisi
	 */
	@Column(name = "nama_wp", columnDefinition = "text") public String getNamaWp() { return namaWp; }
	/**
	 * Menyetel nama wajib pajak. Tanpa validasi.
	 *
	 * @param v nama wajib pajak baru
	 */
	public void setNamaWp(String v) { this.namaWp = v; }
	/**
