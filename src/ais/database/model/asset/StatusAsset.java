...
/**
 * Entity master <b>Status Aset</b> (tabel {@code asset.status_asset}) — katalog datar
 * {@code nama/keterangan} berisi status siklus hidup sebuah unit aset (mis. "Baik", "Rusak
 * Ringan", "Rusak Berat", "Hilang"), dipilih pada baris {@link MasterAsset} lewat layar CRUD
 * {@code ais.action.master.asset.StatusAssetAction} dan diakses lewat
 * {@code ais.database.dao.asset.StatusAssetDao}.
 *
 * <h2>Sumbu klasifikasi yang berbeda dari {@link JenisAsset}/{@link KategoriAsset}</h2>
 * <p>Nama kelas ini mudah tertukar dengan dua katalog lain di paket yang sama, tapi ketiganya
 * mewakili sumbu klasifikasi terpisah pada {@code MasterAsset} dan tidak saling menunjuk:
 * {@link JenisAsset} adalah jenis barang/jasa, {@link KategoriAsset} adalah pengelompokan
 * administratif (dengan {@code kode} dan flag {@code aktif} sendiri), sedangkan
 * {@code StatusAsset} murni menyatakan <i>kondisi</i> unit aset saat ini — sumbu yang bisa
 * berubah sepanjang umur aset, berbeda dari jenis/kategori yang biasanya tetap.</p>
 *
 ...
 * @see JenisAsset
 * @see KategoriAsset
 * @see ais.action.master.asset.StatusAssetAction
 */
@Entity
...
@Audited
@Table(schema = "asset", name = "status_asset")



public class StatusAsset extends GeneralValueObject {
	...
	/** Primary key auto-generated (IDENTITY) tabel {@code asset.status_asset}. */
	private Long id;
	...
	/**
	 * Representasi ringkas untuk log/debug dan tampilan combobox generik. Berbeda dari
	 * kebanyakan entity saudara di paket ini yang menggabungkan {@code id} dan {@code nama},
	 * kelas ini hanya mengembalikan {@link #nama} apa adanya.
	 *
	...
	/** Nama status aset (mis. "Baik", "Rusak Ringan", "Rusak Berat", "Hilang"). */
	private String nama;
	...
	/**
	 * Mengembalikan nama status, di-trim untuk menghindari whitespace tak sengaja dari input.
	 *
	...
	/**
	 * Mengisi nama status. Tidak melakukan trim di sisi setter — trimming terjadi hanya saat
