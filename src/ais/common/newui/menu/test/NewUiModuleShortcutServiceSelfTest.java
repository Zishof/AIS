...
public final class NewUiModuleShortcutServiceSelfTest {
    ...
    public static void main(String[] args) {
        ...
        role.setInfoKegiatan(Boolean.TRUE);
        List<NewUiModuleShortcut> cards = NewUiModuleShortcutService.build(role, snapshot);
        check(cards.size() == 4, "seluruh flag aktif yang punya target READ harus menjadi kartu");
        ...
        check("Kalender Akademik".equals(cards.get(2).getLabel()), "Kalender Akademik belum terpetakan");
        check("Info Kegiatan".equals(cards.get(3).getLabel()), "Info Kegiatan belum terpetakan");
