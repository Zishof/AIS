...
public final class NewUiTransferWorkflowService {
    public Snapshot load(Filter f){Session s=HibernateUtil.openSession();try{Criteria c=criteria(s,f,true);Number n=(Number)criteria(s,f,false).setProjection(Projections.rowCount()).uniqueResult();Snapshot out=new Snapshot();out.total=n==null?0:n.intValue();List<DaftarPengajuanTransfer> values=c.setFirstResult(Math.max(0,f.page)*f.size).setMaxResults(f.size).list();for(DaftarPengajuanTransfer v:values){Row r=row(v);out.rows.add(r);count(out,r.status);out.amount+=r.amount;}Dashboard d=dashboard(s,f);out.waiting=d.waiting;out.submitted=d.submitted;out.transitory=d.transitory;out.transferred=d.transferred;out.allAmount=d.amount;return out;}finally{s.close();}}
    ...
    private Criteria criteria(Session s,Filter f,boolean order){Criteria c=s.createCriteria(DaftarPengajuanTransfer.class,"d").createAlias("prosesTransfer","p",Criteria.LEFT_JOIN);if(f.id!=null)c.add(Restrictions.idEq(f.id));if(f.start!=null)c.add(Restrictions.ge("waktu",start(f.start)));if(f.end!=null)c.add(Restrictions.lt("waktu",next(f.end)));if(f.activeOnly)c.add(Restrictions.or(Restrictions.isNull("aktif"),Restrictions.eq("aktif",Boolean.TRUE)));if(f.workUnitId!=null)c.add(Restrictions.eq("satuanKerja.id",f.workUnitId));if(clean(f.query)!=null)c.add(Restrictions.or(Restrictions.ilike("kode",f.query.trim(),MatchMode.ANYWHERE),Restrictions.or(Restrictions.ilike("nama",f.query.trim(),MatchMode.ANYWHERE),Restrictions.ilike("keterangan",f.query.trim(),MatchMode.ANYWHERE))));applyStatus(c,f);applyKinds(c,f);if(order)c.addOrder(Order.desc("waktu")).addOrder(Order.desc("id"));return c;}
    ...
    private Dashboard dashboard(Session s,Filter f){Filter x=f.copy();x.page=0;x.size=1;Dashboard d=new Dashboard();for(Object o:criteria(s,x,false).list()){Row r=row((DaftarPengajuanTransfer)o);if("WAITING".equals(r.status))d.waiting++;else if("SUBMITTED".equals(r.status))d.submitted++;else if("TRANSITORY".equals(r.status))d.transitory++;else d.transferred++;d.amount+=r.amount;}return d;}
