...
public final class NewUiJournalService {
    ...
    public void unpost(Long id){Session s=HibernateUtil.openSession();Transaction tx=null;try{tx=s.beginTransaction();GrupTransaksi g=require(s,id);if(g.getClosing()!=null)throw new IllegalStateException("Posting jurnal closing tidak dapat dibatalkan.");PostingHistory p=g.getPostingHistory();if(p==null)return;List<Transaksi> lines=lines(s,id);g.setPostingHistory(null);s.update(g);for(Transaksi t:lines){t.setPostingHistory(null);t.setStatusPosting(Transaksi.STATUS_POSTING_BELUM);t.setTanggalPosting(null);s.update(t);}s.flush();if(!stillReferenced(s,p))s.delete(p);s.flush();tx.commit();}catch(RuntimeException e){rollback(tx);throw e;}finally{s.close();}}
