...
import java.util.ArrayList;import java.util.Date;import java.util.HashMap;import java.util.Iterator;import java.util.List;import java.util.Map;import java.util.Set;import org.hibernate.Criteria;import org.hibernate.Session;import org.hibernate.Transaction;import org.hibernate.criterion.MatchMode;import org.hibernate.criterion.Order;import org.hibernate.criterion.Projections;import org.hibernate.criterion.Restrictions;import org.json.JSONObject;import ais.database.hibernate.HibernateUtil;import ais.database.model.Bank;import ais.database.model.Tbmuser;import ais.database.model.akunting.ProsesTransferStandingInstruction;import ais.database.model.akunting.StandingInstruction;import ais.database.model.rab.SatuanKerja;
/** Headless parity daftar/filter/statistik StandingInstructionAction. */
@SuppressWarnings({"rawtypes","unchecked"}) public final class NewUiStandingInstructionService{
 public Snapshot load(String q,Date start,Date end,Long unitId,boolean pending,boolean approved,boolean transferred,int page,int size,Set<SatuanKerja>allowed){Session s=HibernateUtil.openSession();try{Snapshot out=new Snapshot();Criteria count=criteria(s,q,start,end,unitId,allowed);List all=count.addOrder(Order.desc("id")).list();List<Row> filtered=new ArrayList<Row>();for(int i=0;i<all.size();i++){Row r=row(s,(StandingInstruction)all.get(i));if(matches(r.status,pending,approved,transferred)){out.status.put(r.status,Integer.valueOf(value(out.status,r.status)+1));String type=r.payroll?"Penggajian":"Pengajuan Lain";out.types.put(type,Integer.valueOf(value(out.types,type)+1));filtered.add(r);}}out.total=filtered.size();int from=Math.min(out.total,Math.max(0,page)*size),to=Math.min(out.total,from+size);out.rows.addAll(filtered.subList(from,to));return out;}finally{s.close();}}
 public void updateNote(Long id,String note,Set<SatuanKerja>allowed,Tbmuser user){Session s=HibernateUtil.openSession();Transaction tx=null;try{tx=s.beginTransaction();StandingInstruction v=(StandingInstruction)s.get(StandingInstruction.class,id);if(v==null)throw new IllegalArgumentException("Standing instruction tidak ditemukan.");ensureScope(v,allowed);v.setKeterangan(clean(note));if(user!=null){v.setOleh(user.getUserNama());v.setOlehId(user.getUserId());}s.update(v);s.flush();tx.commit();}catch(RuntimeException e){if(tx!=null)try{tx.rollback();}catch(Exception ignored){}throw e;}finally{s.close();}}
