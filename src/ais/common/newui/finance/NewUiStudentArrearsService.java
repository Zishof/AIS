...
import java.util.ArrayList;import java.util.List;import org.hibernate.Criteria;import org.hibernate.Session;import org.hibernate.Transaction;import org.hibernate.Hibernate;import org.hibernate.criterion.MatchMode;import org.hibernate.criterion.Order;import org.hibernate.criterion.Projections;import org.hibernate.criterion.Restrictions;import ais.database.hibernate.HibernateUtil;import ais.database.model.DetailKegiatan;import ais.database.model.Fakultas;import ais.database.model.Jenjang;import ais.database.model.Jurusan;import ais.database.model.Kegiatan;import ais.database.model.Konsentrasi;import ais.database.model.Mahasiswa;import ais.database.model.Perkuliahan;import ais.database.model.StatusAwalMahasiswa;import ais.database.model.StatusMahasiswa;import ais.database.model.Tbmuser;import ais.database.model.TunggakanMahasiswa;import ais.database.model.TunggakanMahasiswaDetail;
/** Daftar, rincian biaya/terbayar, status lunas, rekap, dan hapus dari TunggakanMahasiswaAction. */
@SuppressWarnings({"rawtypes","unchecked"})public final class NewUiStudentArrearsService{
 public Snapshot load(Filter f){Session s=HibernateUtil.openSession();try{Snapshot o=new Snapshot();Number n=(Number)criteria(s,f,false).setProjection(Projections.rowCount()).uniqueResult();o.total=n==null?0:n.intValue();for(Object v:criteria(s,f,true).setFirstResult(Math.max(0,f.page)*f.size).setMaxResults(f.size).list())o.rows.add(row((TunggakanMahasiswa)v));Object[]sum=(Object[])criteria(s,f,false).setProjection(Projections.projectionList().add(Projections.sum("jumlahTunggakan")).add(Projections.count("id"))).uniqueResult();o.debt=sum==null||sum[0]==null?0:((Number)sum[0]).doubleValue();o.records=sum==null||sum[1]==null?0:((Number)sum[1]).intValue();return o;}finally{s.close();}}
 public Details details(Long id){Session s=HibernateUtil.openSession();try{TunggakanMahasiswa x=require(s,id);Details o=new Details();for(Object v:s.createCriteria(TunggakanMahasiswaDetail.class).add(Restrictions.eq("tunggakanMahasiswa",x)).addOrder(Order.asc("nama")).list()){TunggakanMahasiswaDetail d=(TunggakanMahasiswaDetail)v;o.charges.add(new Detail(d.getItemBiaya()==null?d.getNama():d.getItemBiaya().getNama(),number(d.getNilaiBiaya())));}if(x.getKegiatan()!=null)for(Object v:s.createCriteria(DetailKegiatan.class).add(Restrictions.eq("kegiatan",x.getKegiatan())).list()){DetailKegiatan d=(DetailKegiatan)v;o.payments.add(new Detail(d.getDetailBiaya()==null||d.getDetailBiaya().getItemBiaya()==null?"Pembayaran":d.getDetailBiaya().getItemBiaya().getNama(),number(d.getBiaya())));}return o;}finally{s.close();}}
 public Options options(){Session s=HibernateUtil.openSession();try{Options o=new Options();for(Object v:s.createCriteria(StatusMahasiswa.class).addOrder(Order.asc("nama")).list()){StatusMahasiswa x=(StatusMahasiswa)v;o.statuses.add(new Option(x.getId(),x.getNama(),null));}for(Object v:s.createCriteria(StatusAwalMahasiswa.class).addOrder(Order.asc("nama")).list()){StatusAwalMahasiswa x=(StatusAwalMahasiswa)v;o.initialStatuses.add(new Option(x.getId(),x.getNama(),null));}for(Object v:s.createCriteria(Fakultas.class).addOrder(Order.asc("nama")).list()){Fakultas x=(Fakultas)v;o.faculties.add(new Option(x.getId(),x.getNama(),null));}for(Object v:s.createCriteria(Jurusan.class).addOrder(Order.asc("nama")).list()){Jurusan x=(Jurusan)v;o.studyPrograms.add(new Option(x.getId(),x.getNama(),x.getFakultas()==null?null:x.getFakultas().getId()));}for(Object v:s.createCriteria(Konsentrasi.class).addOrder(Order.asc("nama")).list()){Konsentrasi x=(Konsentrasi)v;o.concentrations.add(new Option(x.getId(),x.getNama(),null));}for(Object v:s.createCriteria(Jenjang.class).addOrder(Order.asc("nama")).list()){Jenjang x=(Jenjang)v;o.levels.add(new Option(x.getId(),x.getNama(),null));}return o;}finally{s.close();}}
 public void settle(Long id,boolean value,Tbmuser user){Session s=HibernateUtil.openSession();Transaction tx=null;try{tx=s.beginTransaction();TunggakanMahasiswa x=require(s,id);if(x.getKegiatan()==null)throw new IllegalArgumentException("Tunggakan belum mempunyai transaksi pembayaran.");x.setDianggapLunas(Boolean.valueOf(value));stamp(x,user);s.update(x);s.flush();tx.commit();}catch(RuntimeException e){rollback(tx);throw e;}finally{s.close();}}
 public void delete(Long id){Session s=HibernateUtil.openSession();Transaction tx=null;try{tx=s.beginTransaction();TunggakanMahasiswa x=require(s,id);s.delete(x);s.flush();tx.commit();}catch(RuntimeException e){rollback(tx);throw e;}finally{s.close();}}
