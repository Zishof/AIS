...
/**
 * Adapter layanan eksternal/per-pengguna untuk calendar util. Tipe ini membungkus autentikasi,
 ...
 * String username}, {@code FileDataStoreFactory dataStoreFactory}, {@code Calendar service}, {@code
 * HttpTransport httpTransport}, {@code Tbmuser tbmuser}; inisialisasi/lifecycle ({@code initService()});
 * pembacaan/pencarian ({@code getCredentials()}); mutasi data ({@code proses()}, {@code proses()}, {@code
 * proses()}, {@code proses()}); operasi domain lain ({@code gangguanJaringanAtauBelumOtorisasi()}, {@code
 * chekSudahAda()}, {@code displayLink()}, {@code cretaeTimerWaiting()}, {@code kirimEvent()}, {@code
 * kirimEvent()}); konfigurasi constructor: {@code dataStoreFactory}, {@code httpTransport}, {@code lokasi},
 * {@code username}. Bagian lain dari kontrak tetap mengikuti kelas induk atau interface yang disebut di
 * atas.</p>
...
//			System.out.println("emails -> " + emails);
	...
	public Credential getCredentials(FileDataStoreFactory dataStoreFactory, final String username) throws Exception {
		...
		// OAuth mentah tidak tersimpan lama di DB tanpa enkripsi (risiko data-at-rest).
	...
	private static void hapusGCalendarCode(String username) {
		...
		try {
			...
			if (tx != null) {
				try {
					tx.rollback();
				} catch (Exception abaikan) { ais.common.ErrorAuditUtil.record(abaikan, "auto-audit(empty-catch) src/ais/common/calendar/CalendarUtil.java:hapusGCalendarCode");
				}
			}
		} finally {
