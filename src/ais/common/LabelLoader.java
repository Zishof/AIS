 ...
 * 
 * <p>
 * Notice that the encoding of the Locale dependent file (*.properties) is
 * assumed to be UTF-8. If it is not the case, please refer to <a href=
 * "http://books.zkoss.org/wiki/ZK_Configuration_Reference/zk.xml/The_library-property_Element/Library_Properties#org.zkoss.util.label.web.charset"
 * >ZK Configuration Reference</a> for more information.
 * 
 * @author tomyeh
 */
public class LabelLoader {
	...
	/**
	 * A map of (Locale l, Map(String key, ExValue label)). We use two maps to
	 * speed up the access of labels. _labels allows concurrent access without
	 * synchronization. _syncLabels requires synchronization and used for
	 * update.
	 */
	private Map _labels = Collections.EMPTY_MAP;
	/**
	 * A map of (Locale 1, Map<String key1, Map<String key2...> or ExValue
	 * label>) It is used by variable resolver and allows ${labels.key1.key2}.
	 * _segLabels allows concurrent access without synchronization. See also
	 * {@link #getSegmentedLabels}.
	 */
	...
	public LabelLoader() {
		_fmfilter = new FilterMap.Filter() {
			public Object filter(Object key, Object value) {
				return value instanceof ExValue ? ((ExValue) value).getValue()
						: value;
			}
		};
	...
	/**
	 * Returns the label of the specified key for the current locale, or null if
	 * not found.
	 * 
	...
	/**
	 * Returns the label of the specified key for the specified locale, or null
	 * if not found.
	 * 
	 * @since 5.0.7
	 */
	 ...
	 * Unlike {@link #getLabel}, if a key of the label contains dot, it will be
	 * splitted into multiple keys and then grouped into map. It is so-called
	 * segmented.
	 * <p>
	 * For example, the following property file will parsed into a couple of
	 * maps, and <code>getSegmentedLabels()</code> returns a map containing a
	 * single entry. The entry's key is <code>"a"</code> and the value is
	 * another map with two entries <code>"b"</code> and <code>"c"</code>. And,
	 * the value for <code>"b"</code> is another two-entries map (containing
	 ...
	 * </code>
	 * <p>
	 * This method is designed to make labels easier to be accessed in EL
	 * expressions.
	 * <p>
	 * On the other hand, {@link #getLabel} does not split them, and you could
	 * access them by, say, <code>getLabel("a.b.d")</code>.
	 * 
	 * @since 5.0.7
	 */
	...
	/**
	 * Sets the variable resolver, which is used if an EL expression is
	 * specified.
	 * 
	 * @since 3.0.0
	 */
	...
	/**
	 * Resets all cached labels and next call to {@link #getLabel} will cause
	 * re-loading the Locale-dependent labels.
	 */
	public void reset() {
	...
	private final Map loadLabels(Locale locale) {
		...
		if (_jarcharset == null)
			_jarcharset = Library.getProperty(
					"org.zkoss.util.label.classpath.charset", "UTF-8");
		if (_warcharset == null) {
			_warcharset = Library.getProperty(
					"org.zkoss.util.label.web.charset", null);
			if (_warcharset == null)
				_warcharset = Library.getProperty(
						"org.zkoss.util.label.WEB-INF.charset", "UTF-8"); // backward
																			// compatible
		}
		...
		try {
			...
			for (Enumeration en = locator
					.getResources(locale == null ? "metainfo/i3-label.properties"
							: "metainfo/i3-label_" + locale + ".properties"); en
					.hasMoreElements();) {
			...
			if (locale != null) {
				...
				final Map superlabels = loadLabels(var != null
						&& var.length() > 0 ? new Locale(lang, cnty)
						: cnty != null && cnty.length() > 0 ? new Locale(lang,
								"") : null);
				if (labels.isEmpty()) {
					labels = superlabels.isEmpty() ? Collections.EMPTY_MAP
							: superlabels;
	...
	// Copy _syncLabels to _labels. It must be called in
	// synchronized(_syncLabels)
	...
	private Map segment(Map map) {
		for (Iterator it = map.keySet().iterator(); it.hasNext();) {
			...
			if (key.indexOf(".") >= 0)
				return segmentInner(new HashMap(map)); // clone since we'll
														// modify it
		}
	...
	private FilterMap segmentInner(Map map) {
		...
		for (Iterator it = segFound.entrySet().iterator(); it.hasNext();) {
			...
			Object o = map.put(me.getKey(),
					seged = segmentInner((Map) me.getValue()));
	...
	private static final void load(Map labels, URL url, String charset)
			throws IOException {
		// log.info("Opening "+url); //don't use MCommon since Messages might
		// call getLabel
	...
	private static final void load(Map labels, InputStream is, String charset)
		...
		try {
			...
			try {
				is.close();
			} catch (Throwable ex) { ais.common.ErrorAuditUtil.record(ex, "auto-audit(empty-catch) src/ais/common/LabelLoader.java:421");
			}
		}
		...
		public ExValue(String val) {
			...
			if ((j = val.indexOf("${")) >= 0 && val.indexOf('}', j + 2) >= 0) {
				try {
					...
					return;
				} catch (Throwable ex) { ais.common.ErrorAuditUtil.record(ex, "auto-audit(empty-catch) src/ais/common/LabelLoader.java:440");
