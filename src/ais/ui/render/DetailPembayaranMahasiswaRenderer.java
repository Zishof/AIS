 ...
 * komponen/baris antarmuka tanpa mengambil alih aturan bisnis milik action atau service pemanggil.
 *
 * <p><b>Batas tanggung jawab:</b> perilaku umum, validasi, akses data, serta lifecycle tetap dimiliki {@link
 * ais.ui.util.MyRowRenderer}. Kelas ini hanya boleh memuat perbedaan yang benar-benar spesifik untuk variasi
 ...
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code Kegiatan kegiatan}, {@code Label
 * labelFooterTagihan}, {@code List pengurangan}, {@code Map dataTagihan}, {@code EventListener eventListener},
 * {@code Tbmuser tbmuser}, {@code Grid gridCicilan}, {@code Mahasiswa mahasiswa}; inisialisasi/lifecycle ({@code
 * buatBaruJikaBelumAda()}); pembacaan/pencarian ({@code ambilJumlahDetailBiayaTanpaDendaCustom()}, {@code
 * ambilJumlahBulananTanpaDendaCustom()}, {@code tampilkanNominalAsliSebelumDiskon()}, {@code tampilkanKunci()},
 * {@code tampilDasboard()}, {@code tampilPanelJurnalPembayaranMahasiswa()}); validasi/perhitungan ({@code
 * tidakBolehMengangsur()}, {@code hitungTagihanDenganDendaCustom()}, {@code hitungUlang()}, {@code
 * hitungDashboardPembayaran()}, {@code hitungNominalBulananMurni()}, {@code hitungTagihanBulananAman()}); mutasi
 * data ({@code executeNativeUpdateTransaction()}, {@code executeNativeSaveTransaction()}, {@code
 * ubahWarnaStatus()}); penghapusan/pembatalan ({@code executeNativeDeleteTransaction()}); pelaporan/ekspor
 * ({@code render()}); operasi domain lain ({@code abaikanNilaiMinus()}, {@code menggunakanDendaCustom()}, {@code
 * nilaiDendaCustom()}, {@code masukkanNilai()}, {@code pasangPanelAnalisisPembayaran()}, {@code
 * kumpulkanSumberAkunJurnalDariGrid()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau interface
...
public class DetailPembayaranMahasiswaRenderer extends ais.ui.util.MyRowRenderer {
	...
	private void bukaPopupUbahNominal(final Row rowPembayaran, final DetailKegiatan detailKegiatan,
		...
		alasan.setWidth("100%");
		// ZK 5.5 belum menyediakan setPlaceholder pada Textbox.
