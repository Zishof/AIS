...
/**
 * Komponen/konfigurasi ZK khusus AIS untuk my grid. Tipe ini membakukan default dan perilaku
 * tampilan di atas komponen induk supaya layar tidak mengulang konfigurasi widget yang sama.
 *
 * <p><b>Batas tanggung jawab:</b> perilaku umum, validasi, akses data, serta lifecycle tetap dimiliki {@link
 * Grid}. Kelas ini hanya boleh memuat perbedaan yang benar-benar spesifik untuk variasi ini; perubahan yang
 * berlaku bagi seluruh keluarga harus ditempatkan di kelas induk agar fungsi tidak bercabang atau tumpang
 * tindih.</p>
 * <p>Perbedaan lokal yang dapat diamati adalah state lokal utama: {@code boolean hasInit}, {@code String
 * cachedBgStyle}, {@code long bgCacheTime}, {@code long BG_CACHE_TTL_MS}; inisialisasi/lifecycle ({@code
 * initBg()}, {@code init()}); pembacaan/pencarian ({@code ambilComponentTersembunyi()}); validasi/perhitungan
 * ({@code setModelCheckMobile()}, {@code setModelCheckMobile()}); mutasi data ({@code setRowRenderer()});
 * operasi domain lain ({@code isNorthRingkas()}). Bagian lain dari kontrak tetap mengikuti kelas induk atau
