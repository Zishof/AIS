...
String dev = deviceNumber.equalsIgnoreCase("0") ? "0" : "devices.length-1";
%>
...
<body>
	<div id="camBox" style="width: 100%; height: 100%;">
		<div class="revdivshowimg"
			...
			<button id="gantiKamera" class="btn" onClick="window.location.replace('read_qr_code_kartu.jsp?deviceNumber=<%=deviceNumberUpdate%>&q=<%=host%>');"><i class="fa fa-refresh fa-2x"></i> Ganti Kamera</button>
			
			<br>
			<table id="customers">
			  <tr>
				<th><%= Common.getBahasaConfig("NIM") %></th>
...
<script>
	...
	function showPosition(position) {
		...
		document.getElementById("location").innerHTML = "<div style=\"color: blue;font-size: 10px\">Lokasi Anda :</div><br><iframe style=\"width:100%;height:250px\" frameborder=\"0\" scrolling=\"no\" marginheight=\"0\"  marginwidth=\"0\" src=\"https://maps.google.com/maps?q="+lat+","+lng+"&hl=id&z=14&amp;output=embed\"></iframe>";
	}
	...
	function loadDoc(qrCodeMessage) {
	  ...
	  xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
		  ...
		  cell1.innerHTML = obj.nim;
		  cell2.innerHTML = obj.nama;
		  cell3.innerHTML = obj.status;
		}
	  };
	  getLocation();
	  xhttp.open("GET", "<%=request.getParameter("q")%>&data="+encodeURIComponent(qrCodeMessage)+"&lat="+lat+"&lng="+lng, true);
	  xhttp.send();
	}
