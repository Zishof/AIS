...
<div id="accordion_lessons_jarak_jauh" role="tablist"
	class="add_bottom_45">
	...
	for (JSONObject jsonObject : jsonObjects) {
		...
		if (komponenDataProdukKursus != null) {
			...
			if (komponenProdukKursus.equals(KomponenProdukKursus.PEMBELAJARAN_JARAK_JAUH)) {
	...
	<div class="card">
		<div class="card-header" role="tab" id="headingTree<%=key%>">
			<h5 class="mb-0">
				<a <%=(indexdata == 1 ? "class=\"collapsed\"" : "")%>
					data-toggle="collapse" href="#collapseTree<%=key%>"
					aria-expanded="<%=(indexdata == 1 ? "true" : "false")%>"
					aria-controls="collapseTree<%=key%>"><i
					class="indicator <%=(indexdata == 1 ? " ti-plus" : " ti-minus")%>"></i>
					<%=komponenDataProdukKursus.getNama()%></a>
			</h5>
		</div>

		<div id="collapseTree<%=key%>"
			class="collapse<%=(indexdata == 1 ? " show" : "")%>" role="tabpanel"
			aria-labelledby="headingTree<%=key%>"
			data-parent="#accordion_lessons_jarak_jauh">
			<div class="card-body">
	...
	e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit webapp/WEB-INF/o/kursus/content/jarak_jauh.jsp:149");
	}
	%>
