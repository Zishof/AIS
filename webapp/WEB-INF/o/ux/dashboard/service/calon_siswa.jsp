...
List<Sekolah> sekolahs = ConstantValues.simpleList(mySession.createCriteria(Sekolah.class)
		.add(sekolahData == null ? Restrictions.sqlRestriction("true") : Restrictions.eq("id", sekolahData.getId()))
		.addOrder(Order.asc("nama"))
		.add(Restrictions.or(Restrictions.isNull("aktif"), Restrictions.eq("aktif", true))), Sekolah.class);
...
for (Sekolah sekolah : sekolahs) {
	...
	int total = 0;
	for (int tahun = angkatan; tahun <= angkatansd; tahun++) {
		Integer count = ((Number) mySession.createCriteria(CalonSiswa.class)
