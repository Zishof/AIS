...
checks = {
    ...
    "Mahasiswa parity keeps every function in native New UI": all(token in mahasiswa_parity_provider for token in ("NEW_UI_NATIVE", "NEW_UI_NATIVE_PANEL", "ais.action.master.MahasiswaAction", "nativePanelKey")) and "legacyRoute" not in mahasiswa_parity_provider,
    "Mahasiswa parity native panel": all(token in crud_page + shared_js for token in ("data-gc-parity", "data-gc-native-panel", "buildParityActions", "openNativePanel")) and "openLegacy" not in shared_js,
    "runtime parity inventory produces native panels": all(token in zkoss_runtime_parity for token in ("getResourcePaths", "getResourceAsStream", "NEW_UI_NATIVE_PANEL", "nativePanelKey", "getEntityClass().getSimpleName() + \"Action\"")) and "legacyRoute" not in zkoss_runtime_parity,
