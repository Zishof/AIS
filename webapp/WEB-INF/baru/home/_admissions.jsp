...
<section class="home-section home-admission-section" id="penerimaan" aria-labelledby="admission-title">
    <div class="home-shell home-admission-grid">
        <article class="home-admission-card">
            ...
            <p><%=h(vm.admission.description)%></p><% if (vm.admission.period.length() > 0) { %><strong class="home-admission-period"><%=h(vm.admission.period)%><% if (vm.admission.endDate.length() > 0) { %> · sampai <%=h(vm.admission.endDate)%><% } %></strong><% } %>
