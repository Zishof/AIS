...
} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/pagesmasterobeprofesilulusanzul/index.jsp:49");}
            ...
            <div class="card border-0 shadow-sm rounded-4 border-top border-primary border-4">
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-2">
                        <div>
                            <h5 class="fw-bold text-dark mb-1">
                                <i class="fas fa-briefcase text-primary me-2"></i><%=Common.getBahasaConfig("Manajemen Profesi Lulusan")%>
                            </h5>
                            <small class="text-muted"><%=Common.getBahasaConfig("Kelola data target profesi atau karir lulusan berdasarkan Program Studi.")%></small>
                        </div>
                        
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            <div class="input-group input-group-sm shadow-sm" style="min-width:180px;flex:1 1 200px;">
                                <span class="input-group-text bg-white border-end-0"><i class="fas fa-search text-muted"></i></span>
                                <input type="text" class="form-control border-start-0 ps-0 bg-white fw-medium" id="searchProfesi<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Cari Kode / Nama Profesi...")%>">
                            </div>
                            ...
                            <% if (add) { %>
                            <button class="btn btn-sm btn-primary rounded-pill px-4 shadow-sm fw-bold text-nowrap" onclick="bukaFormProfesi<%=rnd%>()">
                ...
                <div class="card-body p-4 pt-3" id="printArea<%=rnd%>">
                    <div class="table-responsive border rounded-3 shadow-sm bg-white">
                        <table class="table table-hover table-striped align-middle mb-0" id="tabelHTMLDataProfesi<%=rnd%>">
                            <thead class="table-dark text-center align-middle">
                                <tr>
                                    ...
                                    <th class="text-start fw-semibold text-uppercase small px-3"><i class="fas fa-user-tie me-1"></i><%=Common.getBahasaConfig("Nama Profesi")%></th>
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mt-4 no-print">
                        <div class="small fw-medium text-muted bg-light px-3 py-1 rounded-pill border" id="pagingInfoProfesi<%=rnd%>"></div>
                        <nav>
                            <ul class="pagination pagination-sm mb-0 shadow-sm">
                                <li class="page-item"><button class="page-link text-primary rounded-start-pill px-3 fw-bold" id="btnPrevPage<%=rnd%>" onclick="changePageProfesi<%=rnd%>(-1)"><i class="fas fa-chevron-left"></i></button></li>
                                <li class="page-item disabled"><span class="page-link bg-light text-dark px-3 fw-bold" id="pageNumberDisplay<%=rnd%>">1</span></li>
                                <li class="page-item"><button class="page-link text-primary rounded-end-pill px-3 fw-bold" id="btnNextPage<%=rnd%>" onclick="changePageProfesi<%=rnd%>(1)"><i class="fas fa-chevron-right"></i></button></li>
                            </ul>
                        </nav>
                    </div>
                </div>
...
<script>
    ...
    // ==========================================
    const classModel<%=rnd%> = "<%=ProfesiLulusan.class.getName()%>";
    ...
    const injectDOMElements<%=rnd%> = () => {
        ...
        if(!document.getElementById('modalFormProfesi<%=rnd%>')) {
            var modalHtml = 
            '<div class="modal fade" id="modalFormProfesi<%=rnd%>" tabindex="-1" aria-labelledby="modalLabel<%=rnd%>" aria-hidden="true" data-bs-backdrop="static">' +
                '<div class="modal-dialog modal-dialog-centered modal-lg">' +
                    '<div class="modal-content border-0 shadow-lg rounded-4">' +
                        '<div class="modal-header border-bottom-0 pb-0 pt-4 px-4">' +
                            '<h5 class="modal-title fw-bold text-dark" id="modalLabel<%=rnd%>">' +
                                '<i class="fas fa-edit text-primary me-2"></i><span id="formTitleText<%=rnd%>"><%=Common.getBahasaConfig("Formulir Profesi Lulusan")%></span>' +
                            '</h5>' +
                            '<button type="button" class="btn-close shadow-sm" data-bs-dismiss="modal" aria-label="Close"></button>' +
                        '</div>' +
                        '<div class="modal-body p-4">' +
                            '<form id="formProfesi<%=rnd%>" onsubmit="event.preventDefault(); simpanProfesi<%=rnd%>();">' +
                                '<input type="hidden" id="inputIdProfesi<%=rnd%>">' +
                                
                                '<div class="row g-3">' +
                                    '<div class="col-md-4">' +
                                        '<label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Kode Profesi")%></label>' +
                                        '<input type="text" class="form-control shadow-sm fw-bold" id="inputKode<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Cth: PRF-001")%>">' +
                                    '</div>' +
                                    '<div class="col-md-8">' +
                                        '<label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Nama Profesi")%> <span class="text-danger">*</span></label>' +
                                        '<input type="text" class="form-control shadow-sm fw-bold" id="inputNama<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Cth: Software Engineer")%>" required>' +
                                    '</div>' +
                                    ...
                                    '<div class="col-md-12">' +
                                        ...
                                        '<textarea class="form-control shadow-sm" id="inputKeterangan<%=rnd%>" rows="3" placeholder="<%=Common.getBahasaConfig("Deskripsi mengenai profesi ini...")%>"></textarea>' +
                                    '</div>' +
    ...
    // ==========================================
    // BAGIAN 3: LOAD DROP DOWN (FAKULTAS & JURUSAN)
    // ==========================================
    const loadFakultasUI<%=rnd%> = async (targetId, selectedId) => {
        ...
        if (idFakultasLogin<%=rnd%> !== null) {
            reqObj.where1 += " AND id = " + idFakultasLogin<%=rnd%>;
        }
    ...
    const loadJurusanUI<%=rnd%> = async (idFak, targetId, selectedId) => {
        ...
        if (idJurusanLogin<%=rnd%> !== null) {
            filterWhere.push("id = " + idJurusanLogin<%=rnd%>);
        } else {
    ...
    const buildFilterSQL<%=rnd%> = () => {
        const keyword = document.getElementById('searchProfesi<%=rnd%>').value.trim().replace(/'/g, "''");
        let filters = [];
    ...
    const resetAndLoadData<%=rnd%> = () => {
        currentPage<%=rnd%> = 1; 
        loadDataProfesiLulusan<%=rnd%>();
    };

    const loadDataProfesiLulusan<%=rnd%> = async () => {
        const tbody = document.getElementById('tabelDataProfesi<%=rnd%>');
        ...
        try {
            ...
            if (recordList.length === 0) {
                ...
                recordList.forEach((row) => {
                    ...
                    let actionBtns = '';
                    if (privEdit<%=rnd%>) {
                        actionBtns += '<button class="btn btn-sm btn-outline-warning text-dark shadow-sm px-2 me-1 fw-bold no-print" onclick="editProfesiLulusan<%=rnd%>(' + row.id + ')" title="<%=Common.getBahasaConfig("Ubah Data")%>"><i class="fas fa-edit"></i></button>';
                    }
                    if (privDelete<%=rnd%>) {
                        actionBtns += '<button class="btn btn-sm btn-outline-danger shadow-sm px-2 fw-bold no-print" onclick="hapusProfesiLulusan<%=rnd%>(' + row.id + ')" title="<%=Common.getBahasaConfig("Hapus Data")%>"><i class="fas fa-trash-alt"></i></button>';
                    }
    ...
    const updatePaginationUI<%=rnd%> = () => {
        ...
        document.getElementById('pagingInfoProfesi<%=rnd%>').innerHTML = '<i class="fas fa-list-ul text-primary me-2"></i><%=Common.getBahasaConfig("Menampilkan")%> <b class="text-dark">' + startInfo + ' - ' + endIdx + '</b> <%=Common.getBahasaConfig("dari")%> <b class="text-dark">' + totalRecords<%=rnd%> + '</b> <%=Common.getBahasaConfig("data")%>';
    ...
    const changePageProfesi<%=rnd%> = (direction) => {
    ...
    // ==========================================
    const bukaFormProfesi<%=rnd%> = () => {
        document.getElementById('formProfesi<%=rnd%>').reset();
        document.getElementById('inputIdProfesi<%=rnd%>').value = '';
        document.getElementById('inputAktif<%=rnd%>').checked = true;
        
        document.getElementById('formTitleText<%=rnd%>').innerText = '<%=Common.getBahasaConfigJS("Tambah Data Profesi")%>';
        
        // Reset Dropdown sesuai Sesi
        ...
        const modalForm = new bootstrap.Modal(document.getElementById('modalFormProfesi<%=rnd%>'));
        modalForm.show();
    };

    const editProfesiLulusan<%=rnd%> = async (id) => {
        showLoading<%=rnd%>();
        try {
            ...
            if (result.status === '00' || result.status === 'success') {
                const data = result.data;
                document.getElementById('inputIdProfesi<%=rnd%>').value = data.id;
                ...
                document.getElementById('formTitleText<%=rnd%>').innerText = '<%=Common.getBahasaConfigJS("Ubah Data Profesi")%>';
    ...
    const simpanProfesi<%=rnd%> = async () => {
        const idData = document.getElementById('inputIdProfesi<%=rnd%>').value;
        ...
        const payload = { 
            action: "simpanDataRinci", log: "true", class: classModel<%=rnd%>, data: dataObj
        };
        ...
        try {
            ...
            if (result.status === '00' || result.status === 'success' || result.id) {
                showToast<%=rnd%>('<%=Common.getBahasaConfigJS("Data profesi berhasil disimpan!")%>', 'bg-success text-white');
                bootstrap.Modal.getInstance(document.getElementById('modalFormProfesi<%=rnd%>')).hide();
                loadDataProfesiLulusan<%=rnd%>();
            } else {
    ...
    const hapusProfesiLulusan<%=rnd%> = async (id) => {
        if (typeof proseshapusDataRinci === 'function') {
            proseshapusDataRinci(classModel<%=rnd%>, id, function(){ 
                 if (document.querySelectorAll('#tabelDataProfesi<%=rnd%> tr').length === 1 && currentPage<%=rnd%> > 1) {
                     currentPage<%=rnd%>--;
                 }
                 loadDataProfesiLulusan<%=rnd%>();
            });
    ...
    const downloadExcel<%=rnd%> = () => {
        try {
            const table = document.getElementById('tabelHTMLDataProfesi<%=rnd%>');
            // Clone table agar bisa hapus kolom 'Aksi' tanpa memodifikasi tampilan asli
            const cloneTable = table.cloneNode(true);
            const elementsToRemove = cloneTable.querySelectorAll('.no-print');
            elementsToRemove.forEach(el => el.parentNode.removeChild(el));

            if(typeof XLSX !== 'undefined') {
                const wb = XLSX.utils.table_to_book(cloneTable, {sheet: "Profesi_Lulusan"});
                XLSX.writeFile(wb, "Data_Profesi_Lulusan.xlsx");
            } else {
                showToast<%=rnd%>('<%=Common.getBahasaConfigJS("Library Excel (SheetJS) tidak dimuat. Gagal mengunduh.")%>', 'bg-warning text-dark');
            }
    ...
    const uploadExcel<%=rnd%> = async (input) => {
        ...
        showLoading<%=rnd%>();
        try {
            // Memanggil endpoint upload Excel (disesuaikan dengan controller Anda, misalnya /UploadExcel)
    ...
    const cetakPDF<%=rnd%> = () => {
        // Melakukan trigger print dari browser
        // CSS @media print akan secara otomatis menyembunyikan class "no-print"
        window.print();
    };
    ...
    document.addEventListener("DOMContentLoaded", async () => {
        ...
        showLoading<%=rnd%>(); // Loading saat pertama diakses

        document.getElementById('searchProfesi<%=rnd%>').addEventListener("keydown", (e) => {
            if (e.key === "Enter") { e.preventDefault(); resetAndLoadData<%=rnd%>(); }
        });

        // Tunggu dropdown masters selesai di-load
        ...
        // Tunggu tabel data selesai di-load
        await loadDataProfesiLulusan<%=rnd%>();
        
        hideLoading<%=rnd%>(); // Tutup Loading
    });
</script>