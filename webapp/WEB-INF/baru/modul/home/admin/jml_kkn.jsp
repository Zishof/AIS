...
<%
    ...
    // 2. Query Builder (Lebih Rapi & Efisien)
    StringBuilder sb = new StringBuilder();
    sb.append("SELECT b.id as jurusan_id, b.nama as jurusan, cc.tahunakademik, cc.semester, count(*) as jumlah_siswa ");
    sb.append("FROM mahasiswa_dapat_kelompok_kelompok_kkn aa ");
    sb.append("INNER JOIN kelompok_kkn bb ON (aa.kelompok_kkn = bb.id) ");
    sb.append("INNER JOIN kkn cc ON (bb.kkn = cc.id) ");
    ...
    sb.append("WHERE b.aktif and a.aktif and aa.diterima  "); // Pastikan status aktif/diterima
    ...
    sb.append("GROUP BY b.id, cc.tahunakademik, cc.semester ");
    sb.append("ORDER BY b.id, cc.tahunakademik DESC, cc.semester");

    // 3. Eksekusi Query
    List<Object[]> dataKKN = Common.ambilSql(sb.toString());
    if (dataKKN == null) {
        dataKKN = new ArrayList<Object[]>(); // Prevent NullPointer
    }
%>
    ...
    <div class="card shadow-sm border-0 h-100">
        <div class="card-header bg-white py-3 border-bottom-0 d-flex align-items-center justify-content-between">
            <h5 class="mb-0 text-primary fw-bold">
                <i class="fas fa-users-cog me-2"></i><%=Common.getBahasaConfig("Jumlah Peserta KKN") %>
            </h5>
        </div>
        
        <div class="card-body p-0">
            <div id="Jml_kkn" data-list='{"valueNames":["prodi","tahun","smt","jumlahMhs"],"page":5,"pagination":true}'>
                            ...
                            %>
                                <tr>
                                    <td colspan="4" class="text-center py-4 text-muted fst-italic">
                                        <%=Common.getBahasaConfig("Belum ada data KKN") %>
                                    </td>
                ...
                <jsp:include page="/WEB-INF/baru/componen/paging_default.jsp">
		            <jsp:param name="size" value="<%=dataKKN.size() %>" />
		        </jsp:include>
