...
<div class="repo-modern">
  ...
  %>
    ...
    <section class="repo-wrap repo-card repo-search-panel">
      ...
      <%if(publicUser!=null){%><details class="repo-saved-searches"><summary>Pencarian tersimpan dan alert (<%=savedSearches.size()+searchAlerts.size()%>)</summary><div><%for(PreferenceView p:savedSearches){%><a href="<%=rh(p.queryValue)%>"><strong><%=rh(p.label)%></strong><small>Pencarian tersimpan</small></a><%}for(PreferenceView p:searchAlerts){%><a href="<%=rh(p.queryValue)%>"><strong><%=rh(p.label)%></strong><small>Alert pencarian aktif</small></a><%}if(savedSearches.isEmpty()&&searchAlerts.isEmpty()){%><p>Belum ada pencarian tersimpan.</p><%}%></div></details><%}%>
    </section>
