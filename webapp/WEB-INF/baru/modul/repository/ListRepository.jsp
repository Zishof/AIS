  ...
  <% } else if ("home".equals(view)) {
    ...
    <section class="repo-wrap repo-hero-v2" aria-labelledby="repo-hero-title">
      ...
      <aside class="repo-hero-insight" aria-label="Ringkasan repositori publik">
        ...
        </div><%if(summary.downloads30Days>0){%><a class="repo-recent-downloads" href="<%=root%>/repository/search?access=OPEN_ACCESS"><strong><%=nf.format(summary.downloads30Days)%></strong> unduhan dalam 30 hari</a><%}%><%if(summary.metadataQuality>0){%><div class="repo-quality"><span><b>Kelengkapan metadata</b><strong><%=summary.metadataQuality%>%</strong></span><progress max="100" value="<%=summary.metadataQuality%>"><%=summary.metadataQuality%>%</progress></div><%}%>
    ...
    <div class="repo-wrap repo-search-layout">
      ...
      <section aria-live="polite">
        <div class="repo-results-head"><div><strong><%=nf.format(search.total)%> hasil</strong><div class="repo-chips"><% if (query.documentType.length() > 0) { %><span class="repo-chip"><%=rh(query.documentType)%></span><% } if (query.accessPolicy.length() > 0) { %><span class="repo-chip repo-chip-open"><%=rh(rlabel(query.accessPolicy))%></span><% } if(query.author.length()>0){%><span class="repo-chip">Penulis: <%=rh(query.author)%></span><%}if(query.subject.length()>0){%><span class="repo-chip">Subjek: <%=rh(query.subject)%></span><%}if(query.programStudy.length()>0){%><span class="repo-chip">Program: <%=rh(query.programStudy)%></span><%}%></div></div>
          <div class="repo-result-tools"><button class="repo-btn repo-mobile-filter" type="button" data-repo-filter-toggle aria-expanded="false">Filter</button><a class="repo-btn <%="list".equals(resultMode)?"is-active":""%>" href="<%=baseSearch%>&layout=list">Daftar</a><a class="repo-btn <%="card".equals(resultMode)?"is-active":""%>" href="<%=baseSearch%>&layout=card">Kartu</a>
