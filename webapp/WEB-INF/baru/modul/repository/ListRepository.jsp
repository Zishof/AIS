...
<div class="repo-modern">
  ...
  <% } else if ("home".equals(view)) {
    ...
    <section class="repo-wrap repo-hero-v2" aria-labelledby="repo-hero-title">
      <div class="repo-hero-content">
        ...
        <form class="repo-search repo-search-v2" action="<%=root%>/repository" method="get" role="search" data-repo-suggest-form data-suggest-url="<%=root%>/repository?action=suggest">
          ...
          <select id="repo-home-field" name="field" aria-label="Cari berdasarkan"><option value="all">Semua metadata dan isi</option><option value="title">Judul</option><option value="author">Penulis</option><option value="abstract">Abstrak</option><option value="subject">Kata kunci / subjek</option><option value="program">Program studi</option><option value="advisor">Pembimbing</option><option value="identifier">DOI / Handle / Identifier</option><option value="fulltext">Isi dokumen</option></select>
      ...
      <aside class="repo-hero-insight" aria-label="Ringkasan repositori publik">
        ...
        <%if(summary!=null&&summary.totalItems>0){%><div class="repo-stat-grid">
          ...
          <a class="repo-stat" href="<%=root%>/repository/search?fullText=WITH_FILE"><b><%=nf.format(summary.openFileItems)%></b><span>Full text terbuka</span><small><%=openFilePercent%>% dari karya terbit</small></a>
        ...
        </div><div class="repo-quality repo-access-breakdown"><span><b>Metadata publik</b><strong><%=nf.format(summary.totalItems)%></strong></span><small><%=nf.format(summary.openFileItems)%> dengan full text terbuka · <%=nf.format(summary.restrictedItems)%> terbatas · <%=nf.format(summary.embargoedItems)%> embargo</small></div><%if(summary.downloads30Days>0){%><a class="repo-recent-downloads" href="<%=root%>/repository/search?fullText=WITH_FILE"><strong><%=nf.format(summary.downloads30Days)%></strong> unduhan dalam 30 hari</a><%}%>
    ...
    <%if(!recommendations.isEmpty()){%><section class="repo-wrap repo-section"><div class="repo-section-head"><div><h2>Rekomendasi untuk Anda</h2><p>Berdasarkan subjek karya yang Anda simpan.</p></div></div><div class="repo-recommend-grid"><%for(ItemCard item:recommendations){%><article class="repo-card repo-result-card"><h2><a href="<%=root%>/repository/item/<%=item.id%>"><%=rh(item.title)%></a></h2><div class="repo-meta"><%=rh(item.authors)%> · <%=rh(item.year)%></div><p class="repo-abstract"><%=rh(item.abstractText)%></p><a class="repo-link" href="<%=root%>/repository/item/<%=item.id%>">Lihat detail →</a></article><%}%></div></section><%}%>
  ...
  %>
    ...
    <section class="repo-wrap repo-card repo-search-panel">
      <form class="repo-search repo-search-v2" action="<%=root%>/repository" method="get" role="search" data-repo-suggest-form data-suggest-url="<%=root%>/repository?action=suggest">
        ...
        <fieldset class="repo-search-scope"><legend>Cakupan pencarian</legend><label><input type="radio" name="scope" value="metadata" <%="metadata".equals(query.searchScope)?"checked":""%>> Metadata saja</label><label><input type="radio" name="scope" value="all" <%="all".equals(query.searchScope)?"checked":""%>> Metadata dan isi dokumen</label></fieldset>
      </form>
    ...
    <div class="repo-wrap repo-search-layout">
      <aside class="repo-card repo-facets" data-repo-facets aria-label="Filter hasil">
        ...
        <div class="repo-facet"><h3>Ketersediaan naskah</h3><%for(Map.Entry<String,Long> f:search.fullTextFacets.entrySet()){%><a href="<%=root%>/repository?view=search&q=<%=ru(query.keyword)%>&field=<%=ru(query.searchField)%>&type=<%=ru(query.documentType)%>&access=<%=ru(query.accessPolicy)%>&scope=<%=ru(query.searchScope)%>&fullText=<%=ru(f.getKey())%>"><span><%="WITH_FILE".equals(f.getKey())?"Dengan full text terbuka":"Metadata saja"%></span><b><%=f.getValue()%></b></a><%}%></div>
      </aside>
