...
<div class="repo-modern">
  ...
  <% } else if ("search".equals(view) || "browse".equals(view)) {
      ...
      String baseSearch = root + "/repository?view=search&q=" + ru(query.keyword)
              ...
              + "&language=" + ru(query.language) + "&identifier=" + ru(query.identifier)
  ...
  %>
    ...
    <section class="repo-wrap repo-card repo-search-panel">
      ...
      <details class="repo-advanced-search" <%=(query.author.length()+query.subject.length()+query.language.length()+query.identifier.length()>0)?"open":""%>><summary>Pencarian lanjut</summary><form action="<%=root%>/repository" method="get" class="repo-form-grid"><input type="hidden" name="view" value="search"><label>Kata kunci umum<input name="q" maxlength="200" value="<%=rh(query.keyword)%>"></label><label>Penulis<input name="author" maxlength="200" value="<%=rh(query.author)%>"></label><label>Subjek/kata kunci<input name="subject" maxlength="200" value="<%=rh(query.subject)%>"></label><label>Identifier/handle<input name="identifier" maxlength="255" value="<%=rh(query.identifier)%>"></label><label>Bahasa<select name="language"><option value="">Semua bahasa</option><option value="id" <%="id".equals(query.language)?"selected":""%>>Indonesia</option><option value="en" <%="en".equals(query.language)?"selected":""%>>English</option></select></label><label>Koleksi<select name="collection"><option value="">Semua koleksi</option><%for(CollectionView c:search.collections){%><option value="<%=c.id%>" <%=c.id.equals(query.collectionId)?"selected":""%>><%=rh(c.nama)%></option><%}%></select></label><div class="repo-field-full"><button class="repo-btn repo-btn-primary" type="submit">Terapkan pencarian lanjut</button> <a class="repo-btn" href="<%=root%>/repository?view=search">Reset</a></div></form></details>
    </section>
    <div class="repo-wrap repo-search-layout">
      ...
      <section aria-live="polite">
        <div class="repo-results-head"><div><strong><%=nf.format(search.total)%> hasil</strong><div class="repo-chips"><% if (query.documentType.length() > 0) { %><span class="repo-chip"><%=rh(query.documentType)%></span><% } if (query.accessPolicy.length() > 0) { %><span class="repo-chip repo-chip-open"><%=rh(rlabel(query.accessPolicy))%></span><% } %></div></div>
          ...
          <form method="get" action="<%=root%>/repository" style="display:inline"><input type="hidden" name="view" value="search"><input type="hidden" name="q" value="<%=rh(query.keyword)%>"><input type="hidden" name="type" value="<%=rh(query.documentType)%>"><input type="hidden" name="access" value="<%=rh(query.accessPolicy)%>"><input type="hidden" name="author" value="<%=rh(query.author)%>"><input type="hidden" name="subject" value="<%=rh(query.subject)%>"><input type="hidden" name="language" value="<%=rh(query.language)%>"><input type="hidden" name="identifier" value="<%=rh(query.identifier)%>"><select class="repo-btn" name="sort" onchange="this.form.submit()"><option value="newest" <%="newest".equals(query.sort) ? "selected" : ""%>>Terbaru</option><option value="oldest" <%="oldest".equals(query.sort) ? "selected" : ""%>>Terlama</option><option value="title" <%="title".equals(query.sort) ? "selected" : ""%>>Judul A–Z</option><option value="author" <%="author".equals(query.sort) ? "selected" : ""%>>Penulis A–Z</option></select></form></div>
        </div>
  ...
  <% } else if ("item".equals(view)) {
    ...
    <div class="repo-wrap repo-detail">
      <section>
        <header class="repo-card repo-paper-head"><div class="repo-chips"><span class="repo-chip"><%=rh(item.documentType)%></span><span class="repo-chip <%="OPEN_ACCESS".equals(item.accessPolicy) ? "repo-chip-open" : "repo-chip-warning"%>"><%=rh(rlabel(item.accessPolicy))%></span><% if (item.year.length() > 0) { %><span class="repo-chip">Terbit <%=rh(item.year)%></span><% } %></div><h1><%=rh(item.title)%></h1><div class="repo-authors"><%=rh(item.authors)%></div><div class="repo-meta"><span><%=rh(item.collectionName)%></span><% if (item.publisher.length() > 0) { %><span>•</span><span><%=rh(item.publisher)%></span><% } %><span>•</span><span><%=item.viewCount%> dilihat · <%=item.downloadCount%> unduhan</span></div><div class="repo-paper-actions"><a class="repo-btn repo-btn-primary" href="<%=root%>/repository?action=citation&format=ris&id=<%=item.id%>">RIS</a><a class="repo-btn" href="<%=root%>/repository?action=citation&format=bibtex&id=<%=item.id%>">BibTeX</a><a class="repo-btn" href="<%=root%>/repository?action=citation&format=endnote&id=<%=item.id%>">EndNote</a><a class="repo-btn" href="<%=root%>/repository?action=citation&format=csl&id=<%=item.id%>">CSL-JSON</a><button class="repo-btn" type="button" data-repo-copy="<%=rh(request.getRequestURL().toString() + "?view=item&id=" + item.id)%>">Salin tautan</button></div></header>
