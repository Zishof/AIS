...
<a class="repo-browse-card" href="<%=root%>/repository?view=search&fullText=WITH_FILE"><span class="repo-browse-icon">○</span><span><strong>Naskah lengkap</strong><%if(summary!=null&&summary.openFileItems>0){%><b><%=nf.format(summary.openFileItems)%></b><%}%><small><%=anonymousFullText?"Dapat diunduh tanpa login":"Tersedia setelah login eCampus"%></small></span><i>→</i></a>
  ...
  <% } else if("ask".equals(view)) { RepositoryAnswer answer=(RepositoryAnswer)request.getAttribute("repoAnswer"); %>
    <section class="repo-wrap repo-page-head"><div class="repo-breadcrumb"><a href="<%=root%>/repository">Beranda</a> / Tanya Repository</div><p class="repo-eyebrow">Jawaban berbasis sumber</p><h1>Tanya Repository</h1><p>Jawaban hanya disusun dari metadata karya yang tersedia dan selalu menyertakan tautan sumber.</p><form class="repo-search repo-search-v2" action="<%=root%>/repository/ask" method="get"><label class="visually-hidden" for="repo-ask-q">Pertanyaan</label><input id="repo-ask-q" name="q" maxlength="500" value="<%=answer==null?"":rh(answer.question)%>" placeholder="Apa penelitian terbaru mengenai pendidikan Al-Qur’an?"><button class="repo-btn repo-btn-primary">Cari jawaban</button></form></section><%if(answer!=null&&answer.question.length()>0){%><section class="repo-wrap repo-section repo-home-grid"><article class="repo-card repo-article"><h2>Ringkasan</h2><p><%=rh(answer.answer)%></p><p class="repo-muted">Ringkasan otomatis wajib diverifikasi dengan membaca sumber.</p></article><aside class="repo-card repo-card-pad"><h2>Sumber</h2><ol class="repo-source-list"><%for(ItemCard source:answer.sources){%><li><a href="<%=root%>/repository/item/<%=source.id%>"><strong><%=rh(source.title)%></strong></a><small><%=rh(source.authors)%> · <%=rh(source.year)%></small></li><%}%></ol></aside></section><%}%>
    <section class="repo-wrap repo-section"><div class="repo-section-head"><div><h2>Pertanyaan yang sering diajukan</h2><p>Jawaban cepat mengenai layanan Repository.</p></div><a class="repo-link" href="<%=root%>/repository/help">Buka pusat bantuan →</a></div><div class="repo-faq"><details class="repo-card"><summary>Bagaimana cara mengunggah karya ilmiah?</summary><p>Masuk dengan akun eCampus, pilih Karya Saya atau Unggah Karya, lengkapi sembilan tahap deposit, unggah berkas berwatermark, lalu kirim untuk review.</p></details><details class="repo-card"><summary>Bagaimana cara mengunduh naskah lengkap?</summary><p>Masuk menggunakan akun eCampus. Pengguna umum tetap dapat membaca metadata dan abstrak, sedangkan akses naskah lengkap mengikuti kebijakan item.</p></details><details class="repo-card"><summary>Mengapa tombol unduh tidak muncul?</summary><p>Berkas mungkin belum tersedia, sedang embargo, dibatasi, atau akun Anda belum mempunyai hak akses. Periksa badge akses pada halaman detail.</p></details><details class="repo-card"><summary>Bagaimana memperbaiki metadata yang keliru?</summary><p>Catat URL atau identifier OAI karya, lalu hubungi pengelola Repository melalui kontak pada halaman Bantuan.</p></details></div></section>
...
<div class="repo-facet"><h3>Ketersediaan naskah</h3><%for(Map.Entry<String,Long> f:search.fullTextFacets.entrySet()){%><a href="<%=root%>/repository?view=search&q=<%=ru(query.keyword)%>&field=<%=ru(query.searchField)%>&type=<%=ru(query.documentType)%>&access=<%=ru(query.accessPolicy)%>&scope=<%=ru(query.searchScope)%>&fullText=<%=ru(f.getKey())%>"><span><%="WITH_FILE".equals(f.getKey())?"Dengan naskah lengkap":"Metadata saja"%></span><b><%=f.getValue()%></b></a><%}%></div>
  ...
  <% } else { %>
    ...
    <section class="repo-wrap repo-section"><article class="repo-card repo-article"><h2 id="upload-steps">Langkah mengunggah karya ilmiah</h2><ol class="repo-upload-steps"><li><strong>Masuk</strong><span>Login menggunakan akun eCampus civitas akademika.</span></li><li><strong>Buka Karya Saya</strong><span>Pilih Unggah Karya atau buat deposit baru.</span></li><li><strong>Pilih jenis karya</strong><span>Tentukan skripsi, tesis, disertasi, atau jenis dokumen lain dan koleksi tujuan.</span></li><li><strong>Lengkapi informasi utama</strong><span>Isi judul, abstrak, kata kunci, bahasa, dan daftar pustaka.</span></li><li><strong>Lengkapi data akademik</strong><span>Isi penulis, program studi, fakultas, pembimbing, dan penguji.</span></li><li><strong>Unggah berkas</strong><span>Unggah PDF final yang telah diberi watermark sesuai ketentuan institusi.</span></li><li><strong>Tentukan akses</strong><span>Pilih lisensi, status akses, dan tanggal embargo bila diperlukan.</span></li><li><strong>Periksa preview</strong><span>Pastikan metadata dan berkas yang ditampilkan sudah benar.</span></li><li><strong>Kirim untuk review</strong><span>Tambahkan catatan bila perlu dan pantau notifikasi perbaikan/persetujuan.</span></li></ol><div class="repo-paper-actions"><%if(publicUser==null){%><a class="repo-btn repo-btn-primary" href="<%=root%>/login2">Login eCampus</a><%}else if(canDeposit){%><a class="repo-btn repo-btn-primary" href="<%=root%>/repository-workspace?view=deposit">Unggah karya sekarang</a><%}%><a class="repo-btn" href="<%=root%>/repository/policies#submission">Baca kebijakan pengajuan</a></div></article><aside class="repo-card repo-article"><h2>Kontak pengelola Repository</h2><p>Hubungi pengelola dengan menyertakan nama, URL/identifier karya, dan penjelasan kebutuhan.</p><dl class="repo-contact-list"><%if(institutionEmail.length()>0){%><dt>Email</dt><dd><a href="mailto:<%=rh(institutionEmail)%>"><%=rh(institutionEmail)%></a></dd><%}if(institutionPhone.length()>0){%><dt>Telepon</dt><dd><%=rh(institutionPhone)%></dd><%}if(institutionAddress.length()>0){%><dt>Alamat</dt><dd><%=rh(institutionAddress)%></dd><%}%></dl></aside></section>
  <% } %>
  </main>
