...
(function(){
  ...
  var loadList=async function(){ var j=await post({aksi:"list"}); var d=j.data||[]; var h="";
    if(!d.length){ h='<tr><td colspan="'+(BOLEH?8:7)+'" class="text-center text-muted py-3"><%=Common.getBahasaConfig("Belum ada catatan produksi.")%></td></tr>'; }
      ...
      +(BOLEH?('<td class="text-end"><button class="btn btn-sm btn-link p-0 me-2" onclick=\'ed<%=R%>('+JSON.stringify(x)+')\'><i class="fas fa-pen"></i></button><button class="btn btn-sm btn-link text-danger p-0" onclick="del<%=R%>('+x.id+')"><i class="fas fa-trash"></i></button></td>'):'')+'</tr>'; });
    g("tbl").innerHTML=h; };
