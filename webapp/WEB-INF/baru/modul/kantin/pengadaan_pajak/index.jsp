...
(function(){
  ...
  // Tombol cetak bukti setor pajak. Templatnya BARU -- versi ZKoss tidak punya
  // dokumen per baris untuk pajak, hanya ekspor daftar.
  function tombolCetak(r){
    return '<button class="btn btn-sm btn-outline-secondary me-1" title="Cetak bukti setor"'
         + ' onclick="cetakDokumen' + RND + '(\'pajak\',' + r.id
         + ',\'' + esc(r.kode || "") + '\')">'
  ...
  window["pjMuat" + RND] = function(){
    ...
    api({action:"pengadaan_pajak_daftar"}).then(function(d){
      ...
      for (var i=0;i<setoran.length;i++){
        var r = setoran[i];
        var aksi = tombolCetak(r) + (r.aktif
