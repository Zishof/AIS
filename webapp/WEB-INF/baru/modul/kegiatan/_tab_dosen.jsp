...
Dosen loginDosen = tbmuser.ambilDosen();
Long idDosenLogin = (loginDosen != null) ? loginDosen.getId() : null;
...
if (idDosenLogin != null) {
    baseWhere += " AND k.diajukan_oleh = " + idDosenLogin;
            ...
            <div class="card border-0 shadow-sm rounded-4 border-top border-primary border-4">
                <div class="card-header bg-white border-0 pt-4 pb-0 px-4">
                    <h5 class="fw-bold text-dark mb-1">
                        <i class="fas fa-chalkboard-teacher text-primary me-2"></i><%=Common.getBahasaConfig("Pusat Kegiatan Dosen")%>
                    </h5>
                    <small class="text-muted mb-3 d-block"><%=Common.getBahasaConfig("Pantau dan kelola data pencapaian kegiatan dosen secara komprehensif.")%></small>
                    <ul class="nav nav-tabs border-bottom-0 flex-wrap" id="kegiatanTabDosen<%=rnd%>" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active fw-bold text-secondary" data-bs-toggle="tab" data-bs-target="#dash-pane-<%=rnd%>" type="button" role="tab">
                                <i class="fas fa-chart-pie me-2"></i><%=Common.getBahasaConfig("Ringkasan")%>
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link fw-bold text-secondary" data-bs-toggle="tab" data-bs-target="#data-pane-<%=rnd%>" type="button" role="tab" onclick="loadDataDosen<%=rnd%>(1)">
                                ...
                                <div class="col-md-6">
                                    <div class="card border-0 shadow-sm rounded-4 h-100">
                                        <div class="card-header bg-white border-0 pt-3 pb-0 px-4">
                                            <h6 class="fw-bold mb-0"><i class="fas fa-chart-pie me-2 text-primary"></i><%=Common.getBahasaConfig("Distribusi Status")%></h6>
                                        </div>
                            ...
                            <div class="card border-0 shadow-sm rounded-4 mb-3">
                                <div class="card-body p-3">
                                    <div class="d-flex flex-wrap gap-2 align-items-center">
                                        <input type="text" id="cariKegiatan<%=rnd%>" class="form-control form-control-sm" style="min-width:180px;flex:1 1 200px;" placeholder="<%=Common.getBahasaConfig("Cari nama kegiatan...")%>" oninput="loadDataDosen<%=rnd%>(1)">
                                        <select id="filterStatus<%=rnd%>" class="form-select form-select-sm" style="max-width:180px;" onchange="loadDataDosen<%=rnd%>(1)">
                                        ...
                                        <button class="btn btn-sm btn-outline-secondary" onclick="document.getElementById('cariKegiatan<%=rnd%>').value='';document.getElementById('filterStatus<%=rnd%>').value='';loadDataDosen<%=rnd%>(1)">
                                            <i class="fas fa-sync-alt"></i>
                                        </button>
                                    </div>
                                </div>
                            </div>
                            <div id="listDataDosen<%=rnd%>">
                                <div class="text-center py-4"><div class="spinner-border text-primary"></div></div>
                            </div>
                            <nav><ul class="pagination pagination-sm justify-content-end mt-3" id="pagDosen<%=rnd%>"></ul></nav>
                        </div>
...
(function(){
    ...
    async function fetchSql(q){
        try{var r=await fetch(ROOT+'/Data',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'sql',sql:q})});return(await r.json()).data||[];}catch(e){return[];}
    }
    ...
    async function loadDashboard(){
        ...
        if(t.length){var x=t[0],tot=parseInt(x.total||0),ok=parseInt(x.ok||0),pr=parseInt(x.proses||0),pct=tot>0?Math.round(ok*100/tot):0;
            ...
            '<div class="col-6 col-md-3"><div class="card border-0 shadow-sm rounded-4 text-center p-3 border-top border-primary border-3"><div style="font-size:2rem;font-weight:900;color:#0d6efd;">'+tot+'</div><small class="text-muted fw-semibold"><%=Common.getBahasaConfig("Total")%></small></div></div>'+
    ...
    // Data tab
    window['loadDataDosen<%=rnd%>'] = function(pg){
        page<%=rnd%>=pg||1;
        var cari=document.getElementById('cariKegiatan<%=rnd%>').value.trim();
        var st=document.getElementById('filterStatus<%=rnd%>').value;
        var w=baseWhere;
        if(cari) w+=" AND (k.nama ILIKE '%"+cari.replace(/'/g,"''")+"'%' OR k.tempat ILIKE '%"+cari.replace(/'/g,"''")+"'%')";
        if(st)   w+=" AND k.status='"+st.replace(/'/g,"''")+"'";
        var offset=(page<%=rnd%>-1)*limit<%=rnd%>;
        var el=document.getElementById('listDataDosen<%=rnd%>');
        ...
        Promise.all([
            ...
            fetchSql('SELECT k.id, k.nama, k.status, k.tempat, k.mulai, k.sampai, d.nama as nama_dosen FROM public.kegiatan_kedosenan k LEFT JOIN public.dosen d ON k.diajukan_oleh=d.id '+w+' ORDER BY k.id DESC LIMIT '+limit<%=rnd%>+' OFFSET '+offset)
        ]).then(function(res){
            total<%=rnd%>=parseInt((res[0][0]||{}).total||0);
            var rows=res[1];
            if(!rows.length){el.innerHTML='<p class="text-center text-muted py-4"><%=Common.getBahasaConfig("Tidak ada data.")%></p>';renderPagDosen<%=rnd%>();return;}
            var h='<div class="row g-3">';
            rows.forEach(function(r,i){
                h+='<div class="col-md-6"><div class="card border-0 shadow-sm rounded-3 h-100 p-3">'+
                   '<div class="d-flex justify-content-between align-items-start mb-2">'+
                   '<span class="fw-bold text-dark" style="font-size:.9rem;">'+(r.nama||'-')+'</span>'+badge(r.status)+'</div>'+
                   '<div class="text-muted small"><i class="fas fa-map-marker-alt me-1"></i>'+(r.tempat||'-')+'</div>'+
    ...
    function renderPagDosen<%=rnd%>(){
        var pages=Math.ceil(total<%=rnd%>/limit<%=rnd%>);
        var el=document.getElementById('pagDosen<%=rnd%>');
        if(pages<=1){el.innerHTML='';return;}
        var h='';
        h+='<li class="page-item'+(page<%=rnd%><=1?' disabled':'')+'"><a class="page-link" href="#" onclick="event.preventDefault();loadDataDosen<%=rnd%>(page<%=rnd%>-1)">‹</a></li>';
        for(var i=Math.max(1,page<%=rnd%>-2);i<=Math.min(pages,page<%=rnd%>+2);i++){
            h+='<li class="page-item'+(i===page<%=rnd%>?' active':'')+'"><a class="page-link" href="#" onclick="event.preventDefault();loadDataDosen<%=rnd%>('+i+')">'+i+'</a></li>';
        }
        h+='<li class="page-item'+(page<%=rnd%>>=pages?' disabled':'')+'"><a class="page-link" href="#" onclick="event.preventDefault();loadDataDosen<%=rnd%>(page<%=rnd%>+1)">›</a></li>';
        el.innerHTML=h;
    }
