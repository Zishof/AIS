...
Siswa loginSiswa = tbmuser.getSiswa();
Long idSiswaLogin = (loginSiswa != null) ? loginSiswa.getId() : null;

String baseWhere = " WHERE k.id IS NOT NULL ";
if (idSiswaLogin != null) {
    baseWhere += " AND k.diajukan_oleh = " + idSiswaLogin;
}
%>
            ...
            <div class="card border-0 shadow-sm rounded-4 border-top border-success border-4">
                <div class="card-header bg-white border-0 pt-4 pb-0 px-4">
                    <h5 class="fw-bold text-dark mb-1">
                        <i class="fas fa-child text-success me-2"></i><%=Common.getBahasaConfig("Pusat Kegiatan Siswa")%>
                    </h5>
                    <small class="text-muted mb-3 d-block"><%=Common.getBahasaConfig("Pantau dan kelola data kegiatan kesiswaan.")%></small>
                    <ul class="nav nav-tabs border-bottom-0 flex-wrap" role="tablist">
                        ...
                        <li class="nav-item" role="presentation">
                            <button class="nav-link fw-bold text-secondary" data-bs-toggle="tab" data-bs-target="#data-pane-<%=rnd%>" type="button" onclick="loadDataSiswa<%=rnd%>(1)">
                                ...
                                <div class="col-md-6">
                                    <div class="card border-0 shadow-sm rounded-4 h-100">
                                        <div class="card-header bg-white border-0 pt-3 pb-0 px-4">
                                            <h6 class="fw-bold mb-0"><i class="fas fa-chart-pie me-2 text-success"></i><%=Common.getBahasaConfig("Distribusi Status")%></h6>
                                        </div>
                            ...
                            <div class="card border-0 shadow-sm rounded-4 mb-3">
                                <div class="card-body p-3">
                                    <div class="d-flex flex-wrap gap-2 align-items-center">
                                        <input type="text" id="cariKegiatan<%=rnd%>" class="form-control form-control-sm" style="min-width:180px;flex:1 1 200px;" placeholder="<%=Common.getBahasaConfig("Cari nama kegiatan...")%>" oninput="loadDataSiswa<%=rnd%>(1)">
                                        <select id="filterStatus<%=rnd%>" class="form-select form-select-sm" style="max-width:180px;" onchange="loadDataSiswa<%=rnd%>(1)">
                                        ...
                                        <button class="btn btn-sm btn-outline-secondary" onclick="document.getElementById('cariKegiatan<%=rnd%>').value='';document.getElementById('filterStatus<%=rnd%>').value='';loadDataSiswa<%=rnd%>(1)">
                                            <i class="fas fa-sync-alt"></i>
                                        </button>
                                    </div>
                                </div>
                            </div>
                            <div id="listDataSiswa<%=rnd%>">
                                <div class="text-center py-4"><div class="spinner-border text-success"></div></div>
                            </div>
                            <nav><ul class="pagination pagination-sm justify-content-end mt-3" id="pagSiswa<%=rnd%>"></ul></nav>
                        </div>
...
(function(){
    ...
    async function sql(q){try{var r=await fetch(ROOT+'/Data',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'sql',sql:q})});return(await r.json()).data||[];}catch(e){return[];}}
    ...
    async function loadDashboard(){
        ...
        var t=await sql('SELECT COUNT(*) as total, SUM(CASE WHEN k.status=\'Disetujui\' THEN 1 ELSE 0 END) as ok, SUM(CASE WHEN k.status=\'Sedang diproses\' THEN 1 ELSE 0 END) as proses '+q);
        if(t.length){var x=t[0],tot=parseInt(x.total||0),ok=parseInt(x.ok||0),pr=parseInt(x.proses||0),pct=tot>0?Math.round(ok*100/tot):0;
            ...
            '<div class="col-6 col-md-3"><div class="card border-0 shadow-sm rounded-4 text-center p-3 border-top border-success border-3"><div style="font-size:2rem;font-weight:900;color:#198754;">'+tot+'</div><small class="text-muted fw-semibold"><%=Common.getBahasaConfig("Total")%></small></div></div>'+
        ...
        var sr=await sql('SELECT k.status, COUNT(*) as jml '+q+' GROUP BY k.status ORDER BY jml DESC');
        ...
        var tr=await sql('SELECT k.nama, k.status, k.tempat, s.nama as nama_siswa FROM sekolah.kegiatan_kesiswaan k LEFT JOIN sekolah.siswa s ON k.diajukan_oleh=s.id '+baseWhere+' ORDER BY k.id DESC LIMIT 8');
        ...
        if(!tr.length){le.innerHTML='<p class="text-muted text-center small py-3"><%=Common.getBahasaConfig("Belum ada data.")%></p>';}else{
            ...
            tr.forEach(function(r){h2+='<li class="list-group-item px-0 py-2 border-0 border-bottom"><div class="d-flex justify-content-between align-items-start gap-2"><div class="fw-semibold text-dark" style="font-size:.85rem;">'+(r.nama||'-')+'</div>'+badge(r.status)+'</div><small class="text-muted">'+(r.tempat||'')+'</small></li>';});
            le.innerHTML=h2+'</ul>';
        }
    }

    window['loadDataSiswa<%=rnd%>']=function(pg){
        page<%=rnd%>=pg||1;
        ...
        var el=document.getElementById('listDataSiswa<%=rnd%>');
        el.innerHTML='<div class="text-center py-4"><div class="spinner-border spinner-border-sm text-success"></div></div>';
        Promise.all([
            sql('SELECT COUNT(*) as total FROM sekolah.kegiatan_kesiswaan k '+w),
            sql('SELECT k.id,k.nama,k.status,k.tempat,k.mulai,k.sampai FROM sekolah.kegiatan_kesiswaan k '+w+' ORDER BY k.id DESC LIMIT '+limit<%=rnd%>+' OFFSET '+off)
        ]).then(function(res){
            total<%=rnd%>=parseInt((res[0][0]||{}).total||0);
            var rows=res[1];
            if(!rows.length){el.innerHTML='<p class="text-center text-muted py-4"><%=Common.getBahasaConfig("Tidak ada data.")%></p>';renderPagSiswa<%=rnd%>();return;}
    ...
    function renderPagSiswa<%=rnd%>(){
        var pages=Math.ceil(total<%=rnd%>/limit<%=rnd%>);
        var el=document.getElementById('pagSiswa<%=rnd%>');
        if(pages<=1){el.innerHTML='';return;}
        var h='<li class="page-item'+(page<%=rnd%><=1?' disabled':'')+'"><a class="page-link" href="#" onclick="event.preventDefault();loadDataSiswa<%=rnd%>(page<%=rnd%>-1)">‹</a></li>';
        for(var i=Math.max(1,page<%=rnd%>-2);i<=Math.min(pages,page<%=rnd%>+2);i++)
            h+='<li class="page-item'+(i===page<%=rnd%>?' active':'')+'"><a class="page-link" href="#" onclick="event.preventDefault();loadDataSiswa<%=rnd%>('+i+')">'+i+'</a></li>';
        h+='<li class="page-item'+(page<%=rnd%>>=pages?' disabled':'')+'"><a class="page-link" href="#" onclick="event.preventDefault();loadDataSiswa<%=rnd%>(page<%=rnd%>+1)">›</a></li>';
        el.innerHTML=h;
    }
