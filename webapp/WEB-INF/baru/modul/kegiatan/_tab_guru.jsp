<%@page import="ais.database.model.sekolah.KegiatanKesiswaan"%>
...
Guru loginGuru = tbmuser.ambilGuru();
Long idGuruLogin = (loginGuru != null) ? loginGuru.getId() : null;

// Guru sees kesiswaan where they are pembina; admin sees all
String baseWhere = " WHERE k.id IS NOT NULL ";
if (idGuruLogin != null) {
    baseWhere += " AND (k.guru_pmbina1 = " + idGuruLogin + " OR k.guru_pmbina2 = " + idGuruLogin + ")";
}
%>
            ...
            <div class="card border-0 shadow-sm rounded-4 border-top border-warning border-4">
                <div class="card-header bg-white border-0 pt-4 pb-0 px-4">
                    <h5 class="fw-bold text-dark mb-1">
                        <i class="fas fa-user-tie text-warning me-2"></i><%=Common.getBahasaConfig("Kegiatan Kesiswaan - Pembina")%>
                    </h5>
                    <small class="text-muted mb-3 d-block"><%=Common.getBahasaConfig("Pantau kegiatan kesiswaan yang Anda bina.")%></small>
                    <ul class="nav nav-tabs border-bottom-0 flex-wrap" role="tablist">
                        ...
                        <li class="nav-item">
                            <button class="nav-link fw-bold text-secondary" data-bs-toggle="tab" data-bs-target="#data-pane-<%=rnd%>" type="button" onclick="loadDataGuru<%=rnd%>(1)">
                                ...
                                <div class="col-md-6">
                                    <div class="card border-0 shadow-sm rounded-4 h-100">
                                        <div class="card-header bg-white border-0 pt-3 pb-0 px-4">
                                            <h6 class="fw-bold mb-0"><i class="fas fa-chart-pie me-2 text-warning"></i><%=Common.getBahasaConfig("Distribusi Status")%></h6>
                                        </div>
                            ...
                            <div class="card border-0 shadow-sm rounded-4 mb-3">
                                <div class="card-body p-3">
                                    <div class="d-flex flex-wrap gap-2 align-items-center">
                                        <input type="text" id="cariKegiatan<%=rnd%>" class="form-control form-control-sm" style="min-width:180px;flex:1 1 200px;" placeholder="<%=Common.getBahasaConfig("Cari nama kegiatan...")%>" oninput="loadDataGuru<%=rnd%>(1)">
                                        <select id="filterStatus<%=rnd%>" class="form-select form-select-sm" style="max-width:180px;" onchange="loadDataGuru<%=rnd%>(1)">
                                        ...
                                        <button class="btn btn-sm btn-outline-secondary" onclick="document.getElementById('cariKegiatan<%=rnd%>').value='';document.getElementById('filterStatus<%=rnd%>').value='';loadDataGuru<%=rnd%>(1)">
                                            <i class="fas fa-sync-alt"></i>
                                        </button>
                                    </div>
                                </div>
                            </div>
                            <div id="listDataGuru<%=rnd%>">
                                <div class="text-center py-4"><div class="spinner-border text-warning"></div></div>
                            </div>
                            <nav><ul class="pagination pagination-sm justify-content-end mt-3" id="pagGuru<%=rnd%>"></ul></nav>
                        </div>
...
(function(){
    ...
    async function loadDashboard(){
        var q='FROM sekolah.kegiatan_kesiswaan k LEFT JOIN sekolah.siswa s ON k.diajukan_oleh=s.id '+baseWhere;
        ...
        if(t.length){var x=t[0],tot=parseInt(x.total||0),ok=parseInt(x.ok||0),pr=parseInt(x.proses||0),pct=tot>0?Math.round(ok*100/tot):0;
            ...
            '<div class="col-6 col-md-3"><div class="card border-0 shadow-sm rounded-4 text-center p-3 border-top border-warning border-3"><div style="font-size:2rem;font-weight:900;color:#ffc107;">'+tot+'</div><small class="text-muted fw-semibold"><%=Common.getBahasaConfig("Total")%></small></div></div>'+
            ...
            '<div class="col-6 col-md-3"><div class="card border-0 shadow-sm rounded-4 text-center p-3 border-top border-secondary border-3"><div style="font-size:2rem;font-weight:900;color:#6c757d;">'+(tot-ok-pr)+'</div><small class="text-muted fw-semibold"><%=Common.getBahasaConfig("Belum/Proses")%></small></div></div>'+
        ...
        var tr=await fetchSql('SELECT k.nama, k.status, k.tempat, s.nama as nama_siswa FROM sekolah.kegiatan_kesiswaan k LEFT JOIN sekolah.siswa s ON k.diajukan_oleh=s.id '+baseWhere+' ORDER BY k.id DESC LIMIT 8');
        ...
        if(!tr.length){le.innerHTML='<p class="text-muted text-center small py-3"><%=Common.getBahasaConfig("Belum ada data.")%></p>';}else{
            ...
            tr.forEach(function(r){h2+='<li class="list-group-item px-0 py-2 border-0 border-bottom"><div class="d-flex justify-content-between align-items-start gap-2"><div class="fw-semibold text-dark" style="font-size:.85rem;">'+(r.nama||'-')+'</div>'+badge(r.status)+'</div><small class="text-muted">'+(r.nama_siswa||r.tempat||'')+'</small></li>';});
            le.innerHTML=h2+'</ul>';
        }
    }

    window['loadDataGuru<%=rnd%>']=function(pg){
        page<%=rnd%>=pg||1;
        ...
        var el=document.getElementById('listDataGuru<%=rnd%>');
        el.innerHTML='<div class="text-center py-4"><div class="spinner-border spinner-border-sm text-warning"></div></div>';
        Promise.all([
            fetchSql('SELECT COUNT(*) as total FROM sekolah.kegiatan_kesiswaan k '+w),
            fetchSql('SELECT k.id,k.nama,k.status,k.tempat,k.mulai,k.sampai,s.nama as nama_siswa FROM sekolah.kegiatan_kesiswaan k LEFT JOIN sekolah.siswa s ON k.diajukan_oleh=s.id '+w+' ORDER BY k.id DESC LIMIT '+limit<%=rnd%>+' OFFSET '+off)
        ]).then(function(res){
            total<%=rnd%>=parseInt((res[0][0]||{}).total||0);
            var rows=res[1];
            if(!rows.length){el.innerHTML='<p class="text-center text-muted py-4"><%=Common.getBahasaConfig("Tidak ada data.")%></p>';renderPagGuru<%=rnd%>();return;}
            ...
            rows.forEach(function(r){
                h+='<div class="col-md-6"><div class="card border-0 shadow-sm rounded-3 h-100 p-3">'+
                   ...
                   (r.nama_siswa?'<div class="text-muted small"><i class="fas fa-user-graduate me-1"></i>'+r.nama_siswa+'</div>':'')+
    ...
    function renderPagGuru<%=rnd%>(){
        var pages=Math.ceil(total<%=rnd%>/limit<%=rnd%>);
        var el=document.getElementById('pagGuru<%=rnd%>');
        if(pages<=1){el.innerHTML='';return;}
        var h='<li class="page-item'+(page<%=rnd%><=1?' disabled':'')+'"><a class="page-link" href="#" onclick="event.preventDefault();loadDataGuru<%=rnd%>(page<%=rnd%>-1)">‹</a></li>';
        for(var i=Math.max(1,page<%=rnd%>-2);i<=Math.min(pages,page<%=rnd%>+2);i++)
            h+='<li class="page-item'+(i===page<%=rnd%>?' active':'')+'"><a class="page-link" href="#" onclick="event.preventDefault();loadDataGuru<%=rnd%>('+i+')">'+i+'</a></li>';
        h+='<li class="page-item'+(page<%=rnd%>>=pages?' disabled':'')+'"><a class="page-link" href="#" onclick="event.preventDefault();loadDataGuru<%=rnd%>(page<%=rnd%>+1)">›</a></li>';
        el.innerHTML=h;
    }
