...
<%
    ...
    ais.database.model.library.Anggota anggotaHeader = tbmuser == null ? null : ais.database.model.library.Anggota.buatAtauAmbilAnggota(tbmuser, false);
    boolean anggotaHeaderAktif = anggotaHeader != null && anggotaHeader.getId() != null && !Boolean.FALSE.equals(anggotaHeader.getAktif());
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarPerpus<%=rnd%>">
            <ul class="navbar-nav ms-auto mb-2 mb-lg-0 fw-medium">
                ...
                <% if (anggotaPustaka) { %>
                    <li class="nav-item dropdown px-1"><a class="nav-link dropdown-toggle text-dark hover-primary library-member-identity" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false"><i class="fas fa-user-circle me-1 text-primary"></i><span><strong><%=org.apache.commons.lang.StringEscapeUtils.escapeHtml(tbmuser.getUserNama())%></strong><small><%=Common.getBahasaConfig("Anggota")%> · <%=Common.getBahasaConfig(anggotaHeaderAktif?"Aktif":"Belum aktif")%></small></span></a><ul class="dropdown-menu border-0 shadow-sm rounded-3"><li><a class="dropdown-item" href="<%=Common.ROOT%>/pustaka?s=beranda_anggota"><%=Common.getBahasaConfig("Ringkasan")%></a></li><li><a class="dropdown-item" href="<%=Common.ROOT%>/pustaka?s=beranda_anggota&tab=loans"><%=Common.getBahasaConfig("Pinjaman Saya")%></a></li><li><a class="dropdown-item" href="<%=Common.ROOT%>/pustaka?s=beranda_anggota&tab=holds"><%=Common.getBahasaConfig("Reservasi")%></a></li><li><a class="dropdown-item" href="<%=Common.ROOT%>/pustaka?s=beranda_anggota&tab=favorites"><%=Common.getBahasaConfig("Daftar Bacaan & Favorit")%></a></li><li><a class="dropdown-item" href="<%=Common.ROOT%>/pustaka?s=beranda_anggota&tab=saved"><%=Common.getBahasaConfig("Pencarian tersimpan")%></a></li><li><a class="dropdown-item" href="<%=Common.ROOT%>/pustaka?s=layanan_anggota"><%=Common.getBahasaConfig("Layanan anggota")%></a></li></ul></li>
                <% } %>
            </ul>
