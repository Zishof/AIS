...
<%
    /**
     * _wawancara.jsp — Antarmuka Pengguna Fitur Wawancara PMB
     * =========================================================
     *
     * DESKRIPSI
     * ---------
     * Halaman antarmuka calon mahasiswa untuk mengikuti proses wawancara
     * (interview) dalam alur Penerimaan Mahasiswa Baru (PMB). Halaman ini
     * ditampilkan di dalam modal Bootstrap yang dipanggil dari _sukses_login.jsp
     * melalui fungsi bukaModalWawancara() ketika calon mengklik tombol "Wawancara".
     *
     * FITUR UTAMA
     * -----------
     *  1. Skeleton loading: UI bertahap muncul selama data dimuat dari server,
     *     sehingga pengguna tidak menghadapi layar kosong atau teks buram.
     *
     *  2. Jadwal wawancara: Menampilkan tanggal dan rentang waktu (mulai–selesai)
     *     sesi wawancara aktif yang dijadwalkan untuk calon bersangkutan.
     *
     *  3. Countdown / status waktu: Jika wawancara belum dimulai, tampilkan
     *     hitung mundur. Jika sedang berlangsung, tampilkan badge "Sedang
     *     Berlangsung" berwarna hijau. Jika sudah lewat, tampilkan keterangan.
     *
     *  4. Profil pewawancara: Foto, nama, dan nama sesi pewawancara ditampilkan
     *     secara prominan untuk membangun kepercayaan dan orientasi calon.
     *
     *  5. Video konferensi: Jika sesi diadakan online (Jitsi/Zoom/GMeet/BBB/
     *     Skype/WhatsApp/Lain), tampilkan tombol bergaya bergradasi dengan ikon
     *     platform dan tautan "Bergabung" yang langsung dapat diklik peserta.
     *
     *  6. Informasi wawancara: Menampilkan konten HTML bebas dari konfigurasi
     *     GelombangPendaftaran.infoSaatInterview — biasanya berisi tata tertib,
     *     hal yang perlu disiapkan, atau panduan teknis dari panitia.
     *
     *  7. Formulir kesiapan: Toggle switch besar yang ramah mobile + textarea
     *     catatan opsional (link konferensi peserta, pesan ke pewawancara, dll).
     *
     *  8. State pasca-kirim: Setelah submit berhasil, formulir dikunci (read-only),
     *     tombol submit diganti badge hijau "Sudah Siap", dan jika pewawancara
     *     memiliki nomor WA, tautan WA otomatis dibuka di tab baru.
     *
     *  9. Gate states: Penanganan khusus untuk status error:
     *       berkas_kurang — instruksi rinci untuk mengunggah berkas wajib.
     *       no_schedule   — informasi bahwa jadwal belum tersedia.
     *       forbidden     — instruksi untuk menyelesaikan pembayaran.
     *
     * POLA DESAIN JAVASCRIPT
     * ----------------------
     * Semua fungsi JavaScript dikemas dalam IIFE (Immediately Invoked Function
     * Expression) untuk mengisolasi scope dan menghindari polusi namespace global
     * halaman induk. Fungsi yang perlu dipanggil dari luar di-expose ke window
     * dengan sufiks unik (rnd) yang dihasilkan server untuk setiap instance.
     *
     * RESPONSIVITAS
     * -------------
     * Desain menggunakan Bootstrap 5 Grid (col-md-*) untuk layout responsive.
     * Pada layar ≤ 767px (mobile), semua card ditumpuk secara vertikal, foto
     * pewawancara dipusatkan, dan tombol aksi memenuhi lebar layar penuh.
     * Pada ≥ 768px (tablet/desktop), foto dan info pewawancara sejajar horizontal.
     *
     * DEPENDENSI
     * ----------
     *   - Bootstrap 5.x (CSS + JS): sudah dimuat di halaman induk.
     *   - Font Awesome 6 (free): ikon di seluruh elemen.
     *   - fetch API (native browser): tidak ada dependensi jQuery/library eksternal.
     *
     * @author  Tim Pengembang AIS
     * @version 2026-07-16
     * @see     _wawancara_service.jsp
     * @see     _sukses_login.jsp#bukaModalWawancara
     */
    String rnd    = "";
    ...
    try {
        ...
        rnd = "wwn" + System.currentTimeMillis();
    }
...
<%-- ═══════════════════════════════════════════════════════════════
     ROOT CONTAINER — diganti kontennya jika terjadi error fatal
     ════════════════════════════════════════════════════════════════ --%>
<div id="wawRoot<%= rnd %>">
  ...
  <div class="ww-card-<%= rnd %> bg-white">
    ...
    <%-- ── Konten utama (tersembunyi sampai data tiba) ────────────── --%>
    <div id="wawContent<%= rnd %>" class="px-3 px-md-5 pb-5 pt-3" style="display:none">
      ...
      <div class="ww-section-<%= rnd %> mb-4" id="wawJadwal<%= rnd %>">
        <div class="row g-3 align-items-center">
          <div class="col-auto">
            <div class="rounded-3 p-3 text-center" style="background:#dcfce7;min-width:56px">
              <i class="fas fa-calendar-days fa-lg" style="color:#059669"></i>
            </div>
          </div>
          <div class="col">
          ...
          <div class="text-center text-md-start">
            ...
            <div class="mt-2">
              <span class="badge rounded-pill border border-success text-success px-3 py-2"
                    style="background:#f0fdf4;font-size:.75rem">
      ...
      <%-- ── C. Tautan Video Konferensi (tampil hanya jika online) ─── --%>
      ...
      <%-- ── F. Status sudah siap (tampil menggantikan form) ─────────── --%>
    ...
    <div id="wawFooter<%= rnd %>" class="card-footer bg-white border-top px-3 px-md-5 py-4" style="display:none">
      <div class="d-grid d-md-flex justify-content-md-end align-items-center gap-3">
        ...
        <button id="wawBtnKirim<%= rnd %>"
                type="button"
                class="btn btn-success px-5 py-3 rounded-pill fw-bold shadow-sm d-none"
...
<script>
/**
 * Modul Wawancara PMB — instance <%= rnd %>
 *
 * Tanggung jawab modul ini:
 *   1. Memuat data jadwal wawancara dari server via fetch API (JSON).
 *   2. Merender semua seksi UI sesuai data: jadwal, pewawancara, video
 *      konferensi, info, dan formulir kesiapan.
 *   3. Mengelola countdown timer yang menghitung mundur ke waktu mulai
 *      sesi, atau menampilkan badge "Sedang Berlangsung" / "Sudah Selesai".
 *   4. Menangani submit pernyataan siap dengan state loading, locking form
 *      pasca-submit, dan membuka WhatsApp otomatis jika link tersedia.
 *   5. Menangani seluruh kondisi error (berkas_kurang, no_schedule,
 *      forbidden, error teknis) dengan tampilan yang informatif.
 *
 * Pola: IIFE — semua variabel terisolasi dari namespace halaman induk.
 * Fungsi yang perlu dipanggil dari luar di-expose via window.waw*<%= rnd %>.
 */
(function () {
    'use strict';

    var camaId     = '<%= camaId != null ? camaId : "" %>';
    var nowMs      = <%= tglServer.getTime() %>;  // server time — hindari Date.now()
    var countdownTimer = null;  // referensi setInterval untuk cleanup
    ...
    // ─────────────────────────────────────────────────────────────────────
    // HELPER: Konfigurasi visual tiap platform video
    // Menghasilkan: { icon, bg, textColor, buttonColor }
    // ─────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────
    // HELPER: Render badge status waktu + mulai countdown
    // ─────────────────────────────────────────────────────────────────────
    function renderStatusWaktu(mulaiMs, sampaiMs) {
        ...
        function update() {
            ...
            // Aman: nowMs adalah server time; kita tambahkan selisih client-ms
            // (Date.now() dipakai hanya untuk menghitung delta sejak load, bukan
            //  sebagai waktu absolut — sehingga timezone server tetap jadi acuan)
            ...
            if (now < mulaiMs) {
                ...
                badgeEl.innerHTML =
                    ...
                    + '<i class="fas fa-circle me-1" style="animation:pulse 1s infinite alternate"></i>'
        ...
        if (nowMs < mulaiMs) {
            countdownTimer = setInterval(function () {
                nowMs += 1000;  // increment simulasi (1 tick = 1 detik)
                update();
            }, 1000);
        }
    }
    ...
    // ─────────────────────────────────────────────────────────────────────
    // HELPER: Tampilkan konten error/info sebagai pengganti root
    // ─────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────
    // HELPER: Lock form (pasca-submit berhasil atau sudah siap sebelumnya)
    // ─────────────────────────────────────────────────────────────────────
    function kunciForms(catatan) {
        ...
        if (catatanEl) {
            catatanEl.textContent = catatan && catatan.trim().length > 0
                ? '"' + catatan + '"' : '';
        }
    ...
    // ─────────────────────────────────────────────────────────────────────
    // FUNGSI 1: Muat data wawancara dari server
    // ─────────────────────────────────────────────────────────────────────
    window.wawLoadData<%= rnd %> = async function () {
        var url = '<%= Common.ROOT %>/pmb?hanya_tampil_jsp=true&p=pmb&s=_wawancara_service&action=get_data&id='
            + camaId;
        try {
            ...
            if (data.status === 'success') {
                ...
                var namaSesiEl= document.getElementById('wawNamaSesi<%= rnd %>');
                if (fotoEl && data.pegawaiPhoto) {
                    fotoEl.src = data.pegawaiPhoto;
                } else if (fotoEl) {
                    fotoEl.style.display = 'none';
                    var fallback = document.getElementById('wawAvatarFallback<%= rnd %>');
                    if (fallback) { fallback.style.removeProperty('display'); fallback.classList.remove('d-none'); fallback.style.display = 'flex'; }
                }
                if (pegawaiEl)  { pegawaiEl.textContent  = data.pegawaiName  || '-'; }
                if (namaSesiEl) { namaSesiEl.textContent = data.interviewerName || '-'; }
                ...
                if (data.onlineMenggunakan && data.onlineMenggunakan > 0 && data.videoLink) {
                    ...
                    if (vidLink) {
                        vidLink.href  = data.videoLink;
                        vidLink.style.background = vc.btn;
                        vidLink.style.border     = 'none';
                    }
                }
                ...
                // ── Formulir kesiapan + footer ────────────────────────────
                if (footer) { footer.style.display = ''; }

                if (data.isSiap) {
                    // Sudah pernah submit — tampilkan state "sudah siap"
                    kunciForms(data.catatan);
                } else {
                    // Belum — tampilkan form
                    var formSec = document.getElementById('wawFormSection<%= rnd %>');
                ...
                tampilkanPesanRoot(
                    'fas fa-triangle-exclamation', '#f59e0b',
                ...
                root.innerHTML =
                    ...
                    +   '<%= Common.getBahasaConfigJS("Jadwal Belum Tersedia") %>'
                    + '</h5>'
                    + '<p class="text-muted mb-0 small">' + (data.message || '') + '</p>'
                    + '<p class="text-muted small mt-2">'
                    +   '<%= Common.getBahasaConfigJS("Mohon periksa kembali secara berkala atau hubungi panitia PMB.") %>'
                    + '</p>'
                    + '</div>';
                ...
                tampilkanPesanRoot(
                    'fas fa-lock', '#dc2626',
                    '<%= Common.getBahasaConfigJS("Pembayaran Diperlukan") %>',
                    data.message || '',
                    'border-danger'
                );
                ...
                tampilkanPesanRoot(
                    'fas fa-circle-exclamation', '#dc2626',
    ...
    // ─────────────────────────────────────────────────────────────────────
    // FUNGSI 2: Konfirmasi sebelum submit kesiapan
    // ─────────────────────────────────────────────────────────────────────
    window.wawSubmitSiap<%= rnd %> = function () {
        ...
        if (typeof window.showConfirmModal === 'function') {
            window.showConfirmModal(pesan, function () {
    ...
    // ─────────────────────────────────────────────────────────────────────
    // FUNGSI 3: Eksekusi pengiriman kesiapan ke server
    // ─────────────────────────────────────────────────────────────────────
    window.wawEksekusiSubmit<%= rnd %> = async function () {
        ...
        try {
            var url = '<%= Common.ROOT %>/pmb?hanya_tampil_jsp=true&p=pmb&s=_wawancara_service'
                + '&action=submit_siap&id=' + camaId
            ...
            if (data.status === 'success') {
                ...
                // Toast global (jika tersedia dari halaman induk)
                if (typeof window.tampilkanToast === 'function') {
                    window.tampilkanToast(data.message, 'bg-success text-white');
                }

                // Buka WhatsApp ke pewawancara (delay kecil agar toast tampil dulu)
            ...
            if (typeof window.tampilkanToast === 'function') {
                window.tampilkanToast(
                    '<%= Common.getBahasaConfigJS("Gagal menyimpan kesiapan. Periksa koneksi Anda.") %>',
