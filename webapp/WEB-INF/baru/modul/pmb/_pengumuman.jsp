...
<%
    ...
    // Limit per halaman diatur menjadi 5
    int limitPerPage = 5;
    ...
    try {
        ...
        criteria.add(
            Restrictions.ilike("diperuntukkan", PengumumanAkademis.UNTUK_CALON_MAHASISWA, MatchMode.ANYWHERE)
        );
        ...
        e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit webapp/WEB-INF/baru/modul/pmb/_pengumuman.jsp:66");
    } finally {
        if (sess != null) {
            try { sess.clear(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/pmb/_pengumuman.jsp:69");}
            try { sess.disconnect(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/pmb/_pengumuman.jsp:70");}
            try { sess.close(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/pmb/_pengumuman.jsp:71");}
        }
    }
%>
...
<div class="list-group list-group-flush rounded-bottom-4">
    ...
    %>
        <div class="p-4 text-center text-muted">
            ...
            <span class="small"><%= Common.getBahasaConfig("Saat ini tidak terdapat informasi terbaru.") %></span>
        </div>
    <% 
    } else { 
        ...
        for(PengumumanAkademis p : pagedList) {
            ...
            if (judulDisplay != null) {
                judulDisplay = Jsoup.parse(judulDisplay).text();
            } else {
                judulDisplay = "";
            }
           ...
           class="list-group-item list-group-item-action p-3 border-start border-4 border-primary border-0 mb-1 hover-bg-light transition-base text-decoration-none animate__animated animate__fadeIn">
            <div class="d-flex w-100 justify-content-between align-items-start mb-1">
                ...
                <small class="text-muted text-nowrap ms-2" style="font-size: 0.75rem;">
                    <i class="far fa-calendar-days me-1"></i><%= tglDisplay %>
                </small>
            </div>
