...
} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/pagesmasterobecapaianpembelajaranlulusanzul/index.jsp:50");}
...
<style>
    ...
    .sub-cpmk-row:nth-child(even) { background-color: #f8f9fa; }
    .sub-cpmk-row { border-bottom: 1px dashed #dee2e6; padding-bottom: 0.5rem; margin-bottom: 0.5rem; }
</style>
            ...
            <div class="card border-0 shadow-sm rounded-4 border-top border-primary border-4">
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-2">
                        <div>
                            <h5 class="fw-bold text-dark mb-1">
                                <i class="fas fa-list-ol text-primary me-2"></i><%=Common.getBahasaConfig("Manajemen Capaian Pembelajaran Lulusan (CPMK)")%>
                            </h5>
                            <small class="text-muted"><%=Common.getBahasaConfig("Kelola data CPMK dan Sub-CPMK yang dibebankan pada matakuliah.")%></small>
                        </div>
                        
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            <div class="input-group input-group-sm shadow-sm" style="min-width:180px;flex:1 1 200px;">
                                <span class="input-group-text bg-white border-end-0"><i class="fas fa-search text-muted"></i></span>
                                <input type="text" class="form-control border-start-0 ps-0 bg-white fw-medium" id="searchCpmk<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Cari Kode / Isi CPMK...")%>">
                            </div>
                            ...
                            <% if (add) { %>
                            <button class="btn btn-sm btn-primary rounded-pill px-4 shadow-sm fw-bold text-nowrap" onclick="bukaFormCpmk<%=rnd%>()">
                ...
                <div class="card-body p-4 pt-3" id="printArea<%=rnd%>">
                    <div class="table-responsive border rounded-3 shadow-sm bg-white">
                        <table class="table table-hover table-striped align-middle mb-0" id="tabelHTMLDataCpmk<%=rnd%>">
                            <thead class="table-dark text-center align-middle">
                                <tr>
                                    ...
                                    <th class="text-start fw-semibold text-uppercase small px-3" style="width: 250px;"><i class="fas fa-list-ol me-1"></i><%=Common.getBahasaConfig("Isi CPMK")%></th>
                                    <th class="text-start fw-semibold text-uppercase small px-3"><i class="fas fa-stream me-1"></i><%=Common.getBahasaConfig("Sub-CPMK")%></th>
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mt-4 no-print">
                        <div class="small fw-medium text-muted bg-light px-3 py-1 rounded-pill border" id="pagingInfoCpmk<%=rnd%>"></div>
                        <nav>
                            <ul class="pagination pagination-sm mb-0 shadow-sm">
                                <li class="page-item"><button class="page-link text-primary rounded-start-pill px-3 fw-bold" id="btnPrevPage<%=rnd%>" onclick="changePageCpmk<%=rnd%>(-1)"><i class="fas fa-chevron-left"></i></button></li>
                                <li class="page-item disabled"><span class="page-link bg-light text-dark px-3 fw-bold" id="pageNumberDisplay<%=rnd%>">1</span></li>
                                <li class="page-item"><button class="page-link text-primary rounded-end-pill px-3 fw-bold" id="btnNextPage<%=rnd%>" onclick="changePageCpmk<%=rnd%>(1)"><i class="fas fa-chevron-right"></i></button></li>
                            </ul>
                        </nav>
                    </div>
                </div>
...
<script>
    ...
    // ==========================================
    const classModel<%=rnd%> = "<%=CapaianPembelajaranLulusan.class.getName()%>";
    ...
    // Array untuk menyimpan data Sub-CPMK dinamis pada form
    let subCpmkArray<%=rnd%> = [];
    ...
    const injectDOMElements<%=rnd%> = () => {
        ...
        if(!document.getElementById('modalFormCpmk<%=rnd%>')) {
            var modalHtml = 
            '<div class="modal fade" id="modalFormCpmk<%=rnd%>" tabindex="-1" aria-labelledby="modalLabel<%=rnd%>" aria-hidden="true" data-bs-backdrop="static">' +
                '<div class="modal-dialog modal-dialog-centered modal-xl modal-dialog-scrollable">' +
                    '<div class="modal-content border-0 shadow-lg rounded-4">' +
                        '<div class="modal-header border-bottom-0 pb-0 pt-4 px-4">' +
                            '<h5 class="modal-title fw-bold text-dark" id="modalLabel<%=rnd%>">' +
                                '<i class="fas fa-edit text-primary me-2"></i><span id="formTitleText<%=rnd%>"><%=Common.getBahasaConfig("Formulir CPMK")%></span>' +
                            '</h5>' +
                            '<button type="button" class="btn-close shadow-sm" data-bs-dismiss="modal" aria-label="Close"></button>' +
                        '</div>' +
                        '<div class="modal-body p-4">' +
                            '<form id="formCpmk<%=rnd%>" onsubmit="event.preventDefault(); simpanCpmk<%=rnd%>();">' +
                                '<input type="hidden" id="inputIdCpmk<%=rnd%>">' +
                                
                                '<div class="row g-3">' +
                                    '<div class="col-md-3">' +
                                        '<label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Kode CPMK")%></label>' +
                                        '<input type="text" class="form-control shadow-sm fw-bold" id="inputKode<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Cth: CPMK-01")%>">' +
                                    '</div>' +
                                    '<div class="col-md-9">' +
                                        '<label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Isi CPMK")%> <span class="text-danger">*</span></label>' +
                                        '<textarea class="form-control shadow-sm fw-semibold" id="inputNama<%=rnd%>" rows="2" placeholder="<%=Common.getBahasaConfig("Contoh: Mampu menjelaskan konsep...")%>" required></textarea>' +
                                    '</div>' +
                                    ...
                                    '<div class="col-md-12 mt-4">' +
                                        '<div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-3">' +
                                            '<label class="form-label small fw-bold text-secondary m-0"><i class="fas fa-stream text-info me-2"></i><%=Common.getBahasaConfig("SUB-CPMK (Formula)")%></label>' +
                                            '<button type="button" class="btn btn-sm btn-outline-primary shadow-sm" onclick="addSubCpmk<%=rnd%>()"><i class="fas fa-plus me-1"></i><%=Common.getBahasaConfig("Tambah Sub-CPMK")%></button>' +
                                        '</div>' +
                                        '<div id="subCpmkContainer<%=rnd%>" class="px-2 py-3 bg-light border rounded shadow-sm" style="min-height: 80px;">' +
                                        '</div>' +
                                    '</div>' +
    ...
    // ==========================================
    // BAGIAN 4: LOGIKA FORMULA (SUB-CPMK JSON ARRAY)
    // ==========================================
    const renderSubCpmk<%=rnd%> = () => {
        const container = document.getElementById('subCpmkContainer<%=rnd%>');
        let html = '';
        let count = 0;
        
        subCpmkArray<%=rnd%>.forEach((item, index) => {
            if(!item.key) return; // Lewati yang sudah dihapus/kosong
            count++;
            let valKode = item.kode ? item.kode.replace(/"/g, '&quot;') : '';
            let valNama = item.nama ? item.nama.replace(/"/g, '&quot;') : '';
            
            html += '<div class="row g-2 sub-cpmk-row align-items-start">' +
                        '<div class="col-md-3">' +
                            '<input type="text" class="form-control form-control-sm shadow-sm border-info fw-bold" placeholder="<%=Common.getBahasaConfig("Kode Sub-CPMK")%>" value="' + valKode + '" onchange="updateSubCpmk<%=rnd%>(' + index + ', \'kode\', this.value)">' +
                        '</div>' +
                        '<div class="col-md-8">' +
                            '<textarea class="form-control form-control-sm shadow-sm" rows="2" placeholder="<%=Common.getBahasaConfig("Isi / Nama Sub-CPMK")%>" onchange="updateSubCpmk<%=rnd%>(' + index + ', \'nama\', this.value)">' + valNama + '</textarea>' +
                        '</div>' +
                        '<div class="col-md-1 text-end pt-1">' +
                            '<button type="button" class="btn btn-sm btn-outline-danger shadow-sm" onclick="removeSubCpmk<%=rnd%>(' + index + ')" title="<%=Common.getBahasaConfig("Hapus Baris Ini")%>"><i class="fas fa-trash-alt"></i></button>' +
                        '</div>' +
                    '</div>';
        });

        if(count === 0) {
            html = '<div class="text-muted small fst-italic text-center py-3"><i class="fas fa-info-circle me-1"></i><%=Common.getBahasaConfig("Belum ada data Sub-CPMK. Silakan klik tombol Tambah Sub-CPMK.")%></div>';
        }
        
        container.innerHTML = html;
    };

    const addSubCpmk<%=rnd%> = () => {
        // Generate random key mirip Math.abs(Common.randLong()) di Java
        let newKey = new Date().getTime() + Math.floor(Math.random() * 1000);
        subCpmkArray<%=rnd%>.push({ "key": newKey, "kode": "", "nama": "" });
        renderSubCpmk<%=rnd%>();
    };

    const updateSubCpmk<%=rnd%> = (index, field, value) => {
        if(subCpmkArray<%=rnd%>[index]) {
            subCpmkArray<%=rnd%>[index][field] = value;
        }
    };

    const removeSubCpmk<%=rnd%> = (index) => {
        if(confirm('<%=Common.getBahasaConfigJS("Apakah Anda yakin ingin menghapus Sub-CPMK ini?")%>')) {
            // Sesuai Java logic, kita bisa hapus objectnya (atau diset kosong)
            subCpmkArray<%=rnd%>.splice(index, 1);
            renderSubCpmk<%=rnd%>();
        }
    };
    ...
    const buildFilterSQL<%=rnd%> = () => {
        const keyword = document.getElementById('searchCpmk<%=rnd%>').value.trim().replace(/'/g, "''");
        let filters = [];
    ...
    const loadDataCpmk<%=rnd%> = async () => {
        const tbody = document.getElementById('tabelDataCpmk<%=rnd%>');
        ...
        try {
            ...
            if (recordList.length === 0) {
                ...
                recordList.forEach((row) => {
                    ...
                    // Parse Formula (JSONArray Sub-CPMK) untuk ditampilkan di tabel
                    let subCpmkHtml = '<span class="text-muted small fst-italic">-</span>';
                    if(row.formula) {
                        try {
                            let parsedArr = JSON.parse(row.formula);
                            if(parsedArr && parsedArr.length > 0) {
                                subCpmkHtml = '<ul class="text-start mb-0 ps-3 small text-secondary">';
                                parsedArr.forEach(sc => {
                                    if(sc.key && (sc.kode || sc.nama)) {
                                        subCpmkHtml += '<li><b>' + (sc.kode || '-') + '</b>: ' + (sc.nama || '-') + '</li>';
                                    }
                                });
                                subCpmkHtml += '</ul>';
                            }
                    ...
                    let actionBtns = '';
                    if (privEdit<%=rnd%>) {
                        actionBtns += '<button class="btn btn-sm btn-outline-warning text-dark shadow-sm px-2 me-1 fw-bold no-print" onclick="editCpmk<%=rnd%>(' + row.id + ')" title="<%=Common.getBahasaConfig("Ubah Data")%>"><i class="fas fa-edit"></i></button>';
                    }
                    if (privDelete<%=rnd%>) {
                        actionBtns += '<button class="btn btn-sm btn-outline-danger shadow-sm px-2 fw-bold no-print" onclick="hapusCpmk<%=rnd%>(' + row.id + ')" title="<%=Common.getBahasaConfig("Hapus Data")%>"><i class="fas fa-trash-alt"></i></button>';
                    }
                    ...
                    htmlList += '<tr>' +
                                    ...
                                    '<td class="text-start">' + subCpmkHtml + '</td>' +
    ...
    const updatePaginationUI<%=rnd%> = () => {
        ...
        document.getElementById('pagingInfoCpmk<%=rnd%>').innerHTML = '<i class="fas fa-list-ul text-primary me-2"></i><%=Common.getBahasaConfig("Menampilkan")%> <b class="text-dark">' + startInfo + ' - ' + endIdx + '</b> <%=Common.getBahasaConfig("dari")%> <b class="text-dark">' + totalRecords<%=rnd%> + '</b> <%=Common.getBahasaConfig("data")%>';
    ...
    // ==========================================
    const bukaFormCpmk<%=rnd%> = () => {
        document.getElementById('formCpmk<%=rnd%>').reset();
        document.getElementById('inputIdCpmk<%=rnd%>').value = '';
        ...
        document.getElementById('formTitleText<%=rnd%>').innerText = '<%=Common.getBahasaConfigJS("Tambah Data CPMK & SUB-CPMK")%>';
        
        // Reset JSON Array Sub CPMK
        subCpmkArray<%=rnd%> = [];
        renderSubCpmk<%=rnd%>();
        ...
        const modalForm = new bootstrap.Modal(document.getElementById('modalFormCpmk<%=rnd%>'));
        modalForm.show();
    };

    const editCpmk<%=rnd%> = async (id) => {
        showLoading<%=rnd%>();
        try {
            ...
            if (result.status === '00' || result.status === 'success') {
                const data = result.data;
                document.getElementById('inputIdCpmk<%=rnd%>').value = data.id;
                ...
                // Load data Formula (Sub-CPMK Array JSON)
                subCpmkArray<%=rnd%> = [];
                if(data.formula) {
                    try {
                        let parsed = JSON.parse(data.formula);
                        if(Array.isArray(parsed)) {
                            // Filter out empty ones that might come from backend
                            subCpmkArray<%=rnd%> = parsed.filter(item => item.key);
                        }
                    } catch(e) { console.error("Gagal parsing formula JSON"); }
                }
                renderSubCpmk<%=rnd%>();

                document.getElementById('formTitleText<%=rnd%>').innerText = '<%=Common.getBahasaConfigJS("Ubah Data CPMK & SUB-CPMK")%>';
    ...
    const simpanCpmk<%=rnd%> = async () => {
        const idData = document.getElementById('inputIdCpmk<%=rnd%>').value;
        ...
        // Filter valid Sub-CPMK sebelum di convert ke JSON String
        let validSubCpmk = subCpmkArray<%=rnd%>.filter(item => item.key);
        ...
        const dataObj = {
            ...
            formula: JSON.stringify(validSubCpmk)
        };
        ...
        try {
            ...
            if (result.status === '00' || result.status === 'success' || result.id) {
                showToast<%=rnd%>('<%=Common.getBahasaConfigJS("Data CPMK berhasil disimpan!")%>', 'bg-success text-white');
                bootstrap.Modal.getInstance(document.getElementById('modalFormCpmk<%=rnd%>')).hide();
    ...
    const hapusCpmk<%=rnd%> = async (id) => {
        if (typeof proseshapusDataRinci === 'function') {
            proseshapusDataRinci(classModel<%=rnd%>, id, function(){ 
                 if (document.querySelectorAll('#tabelDataCpmk<%=rnd%> tr').length === 1 && currentPage<%=rnd%> > 1) {
                     currentPage<%=rnd%>--;
                 }
                 loadDataCpmk<%=rnd%>();
            });
    ...
    const downloadExcel<%=rnd%> = () => {
        try {
            const table = document.getElementById('tabelHTMLDataCpmk<%=rnd%>');
            const cloneTable = table.cloneNode(true);
            const elementsToRemove = cloneTable.querySelectorAll('.no-print');
            elementsToRemove.forEach(el => el.parentNode.removeChild(el));

            if(typeof XLSX !== 'undefined') {
                const wb = XLSX.utils.table_to_book(cloneTable, {sheet: "CPMK"});
                XLSX.writeFile(wb, "Data_CPMK_SubCPMK.xlsx");
            } else {
    ...
    document.addEventListener("DOMContentLoaded", async () => {
        ...
        document.getElementById('searchCpmk<%=rnd%>').addEventListener("keydown", (e) => {
