...
<%
    try { out.clear(); out.clearBuffer(); } catch (Exception ignore) { ais.common.ErrorAuditUtil.record(ignore, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/pembayaran/_service_piutang_mahasiswa_per_validator.jsp:20");}
    ...
    try {
        ...
        Criteria crit = sess.createCriteria(Kegiatan.class).add(Restrictions.eq("aktif", true))
            .setProjection(Projections.projectionList()
                .add(Projections.groupProperty("validator")).add(Projections.groupProperty("jenisKegiatan"))
            ...
            .addOrder(Order.asc("validator")).addOrder(Order.asc("jenisKegiatan")).setMaxResults(500);
        ...
        for (Object[] r : rows) {
            String val = (String) r[0]; JenisKegiatan jk = (JenisKegiatan) r[1];
        ...
        e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit webapp/WEB-INF/baru/modul/pembayaran/_service_piutang_mahasiswa_per_validator.jsp:108"); 
        out.print("{\"status\":\"error\", \"message\":\"Terjadi galat saat memproses data pangkalan.\"}"); 
        out.flush();
    } finally { 
        try{HibernateUtil.closeSessionQuietly(sess);}catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/pembayaran/_service_piutang_mahasiswa_per_validator.jsp:112");} 
    }
%>