...
<%
    try { out.clear(); out.clearBuffer(); } catch (Exception ignore) { ais.common.ErrorAuditUtil.record(ignore, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/pembayaran/_service_piutang_mahasiswa.jsp:20");}
    ...
    try {
        ...
        String sql = "sum(case when this_.tagihan>0.1 then 1 else 0 end) as jumlahTagihan,"
                   + "sum(case when this_.dibayar>0.1 then 1 else 0 end) as jumlahDibayar,"
                   + "sum(case when (this_.tagihan-this_.dibayar)>0.1 then 1 else 0 end) as jumlahPiutang";
        ...
        Criteria crit = sess.createCriteria(Kegiatan.class).add(Restrictions.eq("aktif", true))
            ...
            .addOrder(Order.asc("jurusan")).addOrder(Order.asc("jenisKegiatan")).setMaxResults(500);
        ...
        for (Object[] r : rows) {
            Jurusan j = (Jurusan) r[0]; JenisKegiatan jk = (JenisKegiatan) r[1];
            ...
            if (tagihan > 0.01) {
                ...
                String nmKegiatan = jk != null && jk.getNamaKegiatan() != null ? jk.getNamaKegiatan() : "-";
                
                o.put("kelompok", j != null ? j.getNama() : "-"); 
                o.put("jenisKegiatan", nmKegiatan);
                o.put("tagihan", tagihan); 
                o.put("dibayar", dibayar); 
                o.put("piutang", piutang);
        ...
        e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit webapp/WEB-INF/baru/modul/pembayaran/_service_piutang_mahasiswa.jsp:113");
        out.print("{\"status\":\"error\", \"message\":\"Terjadi galat saat memproses data pangkalan.\"}"); 
        out.flush();
    } finally { 
        try{HibernateUtil.closeSessionQuietly(sess);}catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/pembayaran/_service_piutang_mahasiswa.jsp:117");} 
    }
%>