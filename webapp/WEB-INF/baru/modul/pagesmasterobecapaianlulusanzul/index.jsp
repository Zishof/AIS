...
} catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/pagesmasterobecapaianlulusanzul/index.jsp:52");}
...
<style>
    ...
    /* Styling checkbox container dengan tinggi otomatis sesuai isi */
            ...
            <div class="card border-0 shadow-sm rounded-4 border-top border-primary border-4">
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-2">
                        <div>
                            <h5 class="fw-bold text-dark mb-1">
                                <i class="fas fa-bullseye text-primary me-2"></i><%=Common.getBahasaConfig("Manajemen Capaian Lulusan")%>
                            </h5>
                            <small class="text-muted"><%=Common.getBahasaConfig("Kelola data Capaian Pembelajaran Lulusan (CPL) per Program Studi.")%></small>
                        </div>
                        
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            <div class="input-group input-group-sm shadow-sm" style="min-width:180px;flex:1 1 200px;">
                                <span class="input-group-text bg-white border-end-0"><i class="fas fa-search text-muted"></i></span>
                                <input type="text" class="form-control border-start-0 ps-0 bg-white fw-medium" id="searchCapaian<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Cari Kode / Nama Capaian...")%>">
                            </div>
                            ...
                            <% if (add) { %>
                            <button class="btn btn-sm btn-primary rounded-pill px-4 shadow-sm fw-bold text-nowrap" onclick="bukaFormCapaian<%=rnd%>()">
                ...
                <div class="card-body p-4 pt-3" id="printArea<%=rnd%>">
                    <div class="table-responsive border rounded-3 shadow-sm bg-white">
                        <table class="table table-hover table-striped align-middle mb-0" id="tabelHTMLDataCapaian<%=rnd%>">
                            <thead class="table-dark text-center align-middle">
                                <tr>
                                    ...
                                    <th class="text-start fw-semibold text-uppercase small px-3"><i class="fas fa-bullseye me-1"></i><%=Common.getBahasaConfig("Nama Capaian Lulusan")%></th>
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mt-4 no-print">
                        <div class="small fw-medium text-muted bg-light px-3 py-1 rounded-pill border" id="pagingInfoCapaian<%=rnd%>"></div>
                        <nav>
                            <ul class="pagination pagination-sm mb-0 shadow-sm">
                                <li class="page-item"><button class="page-link text-primary rounded-start-pill px-3 fw-bold" id="btnPrevPage<%=rnd%>" onclick="changePageCapaian<%=rnd%>(-1)"><i class="fas fa-chevron-left"></i></button></li>
                                <li class="page-item disabled"><span class="page-link bg-light text-dark px-3 fw-bold" id="pageNumberDisplay<%=rnd%>">1</span></li>
                                <li class="page-item"><button class="page-link text-primary rounded-end-pill px-3 fw-bold" id="btnNextPage<%=rnd%>" onclick="changePageCapaian<%=rnd%>(1)"><i class="fas fa-chevron-right"></i></button></li>
                            </ul>
                        </nav>
                    </div>
                </div>
...
<script>
    ...
    // ==========================================
    const classModel<%=rnd%> = "<%=CapaianLulusan.class.getName()%>";
    const classProfil<%=rnd%> = "<%=ProfilLulusan.class.getName()%>";
    const classBahanKajian<%=rnd%> = "<%=BahanKajian.class.getName()%>";
    ...
    const injectDOMElements<%=rnd%> = () => {
        ...
        if(!document.getElementById('modalFormCapaian<%=rnd%>')) {
            var modalHtml = 
            '<div class="modal fade" id="modalFormCapaian<%=rnd%>" tabindex="-1" aria-labelledby="modalLabel<%=rnd%>" aria-hidden="true" data-bs-backdrop="static">' +
                '<div class="modal-dialog modal-dialog-centered modal-lg modal-dialog-scrollable">' +
                    '<div class="modal-content border-0 shadow-lg rounded-4">' +
                        '<div class="modal-header border-bottom-0 pb-0 pt-4 px-4">' +
                            '<h5 class="modal-title fw-bold text-dark" id="modalLabel<%=rnd%>">' +
                                '<i class="fas fa-edit text-primary me-2"></i><span id="formTitleText<%=rnd%>"><%=Common.getBahasaConfig("Formulir Capaian Lulusan")%></span>' +
                            '</h5>' +
                            '<button type="button" class="btn-close shadow-sm" data-bs-dismiss="modal" aria-label="Close"></button>' +
                        '</div>' +
                        '<div class="modal-body p-4">' +
                            '<form id="formCapaian<%=rnd%>" onsubmit="event.preventDefault(); simpanCapaian<%=rnd%>();">' +
                                '<input type="hidden" id="inputIdCapaian<%=rnd%>">' +
                                
                                '<div class="row g-3">' +
                                    '<div class="col-md-3">' +
                                        '<label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Kode Capaian")%></label>' +
                                        '<input type="text" class="form-control shadow-sm fw-bold" id="inputKode<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Cth: CPL-01")%>">' +
                                    '</div>' +
                                    '<div class="col-md-9">' +
                                        '<label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Nama Capaian Lulusan")%> <span class="text-danger">*</span></label>' +
                                        '<input type="text" class="form-control shadow-sm fw-bold" id="inputNama<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Contoh: Mampu merancang sistem...")%>" required>' +
                                    '</div>' +
                                    ...
                                    '<div class="col-md-6">' +
                                        ...
                                        '<select class="form-select shadow-sm" id="selectJurusan<%=rnd%>" onchange="triggerLoadCheckboxes<%=rnd%>(this.value)" required>' +
                                    ...
                                    '<div class="col-md-12 mt-4">' +
                                        '<label class="form-label small fw-bold text-secondary"><i class="fas fa-users-cog text-info me-1"></i><%=Common.getBahasaConfig("Korelasi Profil Lulusan")%></label>' +
                                        '<div id="containerProfil<%=rnd%>" class="checkbox-container-scroll row g-2 m-0 shadow-sm">' +
                                            '<div class="text-muted small fst-italic text-center py-2"><%=Common.getBahasaConfig("Silakan pilih Program Studi (Jurusan) terlebih dahulu.")%></div>' +
                                        '</div>' +
                                    '</div>' +

                                    '' +
                                    '<div class="col-md-12 mt-3">' +
                                        '<label class="form-label small fw-bold text-secondary"><i class="fas fa-book-open text-info me-1"></i><%=Common.getBahasaConfig("Korelasi Bahan Kajian")%></label>' +
                                        '<div id="containerBahanKajian<%=rnd%>" class="checkbox-container-scroll row g-2 m-0 shadow-sm">' +
                                            '<div class="text-muted small fst-italic text-center py-2"><%=Common.getBahasaConfig("Silakan pilih Program Studi (Jurusan) terlebih dahulu.")%></div>' +
                                        '</div>' +
                                    '</div>' +
    ...
    const loadJurusanUI<%=rnd%> = async (idFak, targetId, selectedId, skipCheckboxesTrigger = false) => {
        let reqObj = { 
        ...
        try {
            ...
            if(targetId === 'selectJurusan<%=rnd%>' && data.length === 1 && !skipCheckboxesTrigger){
                 triggerLoadCheckboxes<%=rnd%>(data[0].id);
            }
    ...
    // ==========================================
    // BAGIAN 4: LOAD DYNAMIC CHECKBOXES
    // ==========================================
    const triggerLoadCheckboxes<%=rnd%> = (idJur) => {
        loadCheckboxesProfil<%=rnd%>(idJur, null);
        loadCheckboxesBahanKajian<%=rnd%>(idJur, null);
    };

    const loadCheckboxesProfil<%=rnd%> = async (idJur, selectedStr) => {
        let targetEl = document.getElementById('containerProfil<%=rnd%>');
        if (!idJur) { 
            targetEl.innerHTML = '<div class="text-muted small fst-italic text-center py-2"><%=Common.getBahasaConfig("Silakan pilih Program Studi (Jurusan) terlebih dahulu.")%></div>'; 
            return; 
        }
        targetEl.innerHTML = '<div class="text-center py-2"><div class="spinner-border spinner-border-sm text-primary"></div></div>';
        
        let reqObj = { 
            "action": "daftar", "class": classProfil<%=rnd%>, 
            "where1": "jurusan = " + idJur + " AND (aktif = true or aktif is null)", "order1": "asc", "sort1": "nama" 
        };
        try {
            const res = await fetch('<%=Common.ROOT%>/Data', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(reqObj) });
            const result = await res.json();
            ...
            if (data.length === 0) {
                targetEl.innerHTML = '<div class="text-danger small fst-italic text-center py-2"><i class="fas fa-exclamation-triangle me-1"></i><%=Common.getBahasaConfig("Belum ada data Profil Lulusan untuk jurusan ini.")%></div>';
                return;
            }
            let html = '';
            data.forEach((row) => {
                ...
                html += '<div class="col-md-6">' +
                        ...
                        '    <input class="form-check-input chk-profil<%=rnd%> shadow-sm" type="checkbox" value="' + row.id + '" id="chkProfil_' + row.id + '<%=rnd%>" ' + (isChecked ? 'checked' : '') + '>' +
                        '    <label class="form-check-label text-dark small cursor-pointer" for="chkProfil_' + row.id + '<%=rnd%>">' + labelText + '</label>' +
                        '  </div>' +
                        '</div>';
            });
    ...
    const loadCheckboxesBahanKajian<%=rnd%> = async (idJur, selectedStr) => {
        let targetEl = document.getElementById('containerBahanKajian<%=rnd%>');
        if (!idJur) { 
            targetEl.innerHTML = '<div class="text-muted small fst-italic text-center py-2"><%=Common.getBahasaConfig("Silakan pilih Program Studi (Jurusan) terlebih dahulu.")%></div>'; 
            return; 
        }
        ...
        let reqObj = { 
            "action": "daftar", "class": classBahanKajian<%=rnd%>, 
            "where1": "jurusan = " + idJur + " AND (aktif = true or aktif is null)", "order1": "asc", "sort1": "nama" 
        };
        try {
            const res = await fetch('<%=Common.ROOT%>/Data', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(reqObj) });
            ...
            if (data.length === 0) {
                targetEl.innerHTML = '<div class="text-danger small fst-italic text-center py-2"><i class="fas fa-exclamation-triangle me-1"></i><%=Common.getBahasaConfig("Belum ada data Bahan Kajian untuk jurusan ini.")%></div>';
                return;
            }
            let html = '';
            data.forEach((row) => {
                ...
                html += '<div class="col-md-6">' +
                        '  <div class="form-check">' +
                        '    <input class="form-check-input chk-bk<%=rnd%> shadow-sm border-info" type="checkbox" value="' + row.id + '" id="chkBK_' + row.id + '<%=rnd%>" ' + (isChecked ? 'checked' : '') + '>' +
                        '    <label class="form-check-label text-dark small cursor-pointer" for="chkBK_' + row.id + '<%=rnd%>">' + labelText + '</label>' +
                        '  </div>' +
                        '</div>';
            });
            targetEl.innerHTML = html;
    ...
    const buildFilterSQL<%=rnd%> = () => {
        const keyword = document.getElementById('searchCapaian<%=rnd%>').value.trim().replace(/'/g, "''");
        let filters = [];
    ...
    const resetAndLoadData<%=rnd%> = () => {
        currentPage<%=rnd%> = 1; 
        loadDataCapaianLulusan<%=rnd%>();
    };

    const loadDataCapaianLulusan<%=rnd%> = async () => {
        const tbody = document.getElementById('tabelDataCapaian<%=rnd%>');
        ...
        try {
            ...
            if (recordList.length === 0) {
                ...
                recordList.forEach((row) => {
                    ...
                    let actionBtns = '';
                    if (privEdit<%=rnd%>) {
                        actionBtns += '<button class="btn btn-sm btn-outline-warning text-dark shadow-sm px-2 me-1 fw-bold no-print" onclick="editCapaianLulusan<%=rnd%>(' + row.id + ')" title="<%=Common.getBahasaConfig("Ubah Data")%>"><i class="fas fa-edit"></i></button>';
                    }
                    if (privDelete<%=rnd%>) {
                        actionBtns += '<button class="btn btn-sm btn-outline-danger shadow-sm px-2 fw-bold no-print" onclick="hapusCapaianLulusan<%=rnd%>(' + row.id + ')" title="<%=Common.getBahasaConfig("Hapus Data")%>"><i class="fas fa-trash-alt"></i></button>';
                    }
    ...
    const updatePaginationUI<%=rnd%> = () => {
        ...
        document.getElementById('pagingInfoCapaian<%=rnd%>').innerHTML = '<i class="fas fa-list-ul text-primary me-2"></i><%=Common.getBahasaConfig("Menampilkan")%> <b class="text-dark">' + startInfo + ' - ' + endIdx + '</b> <%=Common.getBahasaConfig("dari")%> <b class="text-dark">' + totalRecords<%=rnd%> + '</b> <%=Common.getBahasaConfig("data")%>';
    ...
    // ==========================================
    const bukaFormCapaian<%=rnd%> = () => {
        document.getElementById('formCapaian<%=rnd%>').reset();
        document.getElementById('inputIdCapaian<%=rnd%>').value = '';
        document.getElementById('inputAktif<%=rnd%>').checked = true;
        
        // Reset Kolom Search MK
        document.getElementById('inputMk<%=rnd%>').value = '';
        document.getElementById('hiddenIdMk<%=rnd%>').value = '';

        document.getElementById('formTitleText<%=rnd%>').innerText = '<%=Common.getBahasaConfigJS("Tambah Data Capaian Lulusan")%>';
        
        document.getElementById('containerProfil<%=rnd%>').innerHTML = '<div class="text-muted small fst-italic text-center py-2"><%=Common.getBahasaConfig("Silakan pilih Program Studi (Jurusan) terlebih dahulu.")%></div>';
        document.getElementById('containerBahanKajian<%=rnd%>').innerHTML = '<div class="text-muted small fst-italic text-center py-2"><%=Common.getBahasaConfig("Silakan pilih Program Studi (Jurusan) terlebih dahulu.")%></div>';
        ...
        const modalForm = new bootstrap.Modal(document.getElementById('modalFormCapaian<%=rnd%>'));
        modalForm.show();
    };

    const editCapaianLulusan<%=rnd%> = async (id) => {
        showLoading<%=rnd%>();
        try {
            ...
            if (result.status === '00' || result.status === 'success') {
                const data = result.data;
                document.getElementById('inputIdCapaian<%=rnd%>').value = data.id;
                ...
                document.getElementById('formTitleText<%=rnd%>').innerText = '<%=Common.getBahasaConfigJS("Ubah Data Capaian Lulusan")%>';

                let idFak = data["jurusan.fakultas.id"];
                let idJur = data["jurusan.id"];
                let valProfil = data.profil;
                let valBahanKajian = data.bahanKajian;

                await loadFakultasUI<%=rnd%>('selectFakultas<%=rnd%>', idFak);
                await loadJurusanUI<%=rnd%>(idFak, 'selectJurusan<%=rnd%>', idJur, true);
                
                await loadCheckboxesProfil<%=rnd%>(idJur, valProfil);
                await loadCheckboxesBahanKajian<%=rnd%>(idJur, valBahanKajian);
    ...
    const simpanCapaian<%=rnd%> = async () => {
        const idData = document.getElementById('inputIdCapaian<%=rnd%>').value;
        ...
        let arrProfil = [];
        document.querySelectorAll('.chk-profil<%=rnd%>:checked').forEach(chk => { arrProfil.push(chk.value); });
        let valProfil = arrProfil.length > 0 ? "," + arrProfil.join(",") + "," : null;
        
        let arrBk = [];
        document.querySelectorAll('.chk-bk<%=rnd%>:checked').forEach(chk => { arrBk.push(chk.value); });
        let valBk = arrBk.length > 0 ? "," + arrBk.join(",") + "," : null;
        ...
        try {
            ...
            if (result.status === '00' || result.status === 'success' || result.id) {
                showToast<%=rnd%>('<%=Common.getBahasaConfigJS("Data Capaian Lulusan berhasil disimpan!")%>', 'bg-success text-white');
                bootstrap.Modal.getInstance(document.getElementById('modalFormCapaian<%=rnd%>')).hide();
                
                // Clear filter MK setelah save jika diisi di Modal
    ...
    const hapusCapaianLulusan<%=rnd%> = async (id) => {
        if (typeof proseshapusDataRinci === 'function') {
            proseshapusDataRinci(classModel<%=rnd%>, id, function(){ 
                 if (document.querySelectorAll('#tabelDataCapaian<%=rnd%> tr').length === 1 && currentPage<%=rnd%> > 1) {
                     currentPage<%=rnd%>--;
                 }
                 loadDataCapaianLulusan<%=rnd%>();
            });
    ...
    const downloadExcel<%=rnd%> = () => {
        try {
            const table = document.getElementById('tabelHTMLDataCapaian<%=rnd%>');
            const cloneTable = table.cloneNode(true);
            const elementsToRemove = cloneTable.querySelectorAll('.no-print');
            elementsToRemove.forEach(el => el.parentNode.removeChild(el));

            if(typeof XLSX !== 'undefined') {
                const wb = XLSX.utils.table_to_book(cloneTable, {sheet: "Capaian_Lulusan"});
                XLSX.writeFile(wb, "Data_Capaian_Lulusan.xlsx");
            } else {
    ...
    document.addEventListener("DOMContentLoaded", async () => {
        ...
        document.getElementById('searchCapaian<%=rnd%>').addEventListener("keydown", (e) => {
        ...
        await loadDataCapaianLulusan<%=rnd%>();
