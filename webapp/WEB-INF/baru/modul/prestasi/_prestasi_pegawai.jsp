...
<%@page import="ais.database.model.PrestasiPegawai"%>
<%@page import="ais.database.model.CabangPrestasiPegawai"%>
<%@page import="ais.database.model.KategoriPrestasiPegawai"%>
...
// Penyesuaian hak akses berdasarkan level login
Pegawai loginSebagaiPegawai = tbmuser.ambilPegawai();
Long idPegawaiLogin = (loginSebagaiPegawai != null) ? loginSebagaiPegawai.getId() : null;
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-2">
                        <div>
                            <h5 class="fw-bold text-dark mb-1">
                                <i class="fas fa-award text-primary me-2"></i><%=Common.getBahasaConfig("Manajemen Prestasi Pegawai")%>
                            </h5>
                            <small class="text-muted"><%=Common.getBahasaConfig("Kelola data pencapaian dan prestasi pegawai.")%></small>
                        </div>
        
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            <div class="input-group input-group-sm shadow-sm" style="min-width:180px;flex:1 1 200px;">
                                <span class="input-group-text bg-white border-end-0"><i class="fas fa-search text-muted"></i></span>
                                <input type="text" class="form-control border-start-0 ps-0 bg-white fw-medium" id="searchPrestasi<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Cari Prestasi, NIK / Nama...")%>">
                            </div>
                        ...
                        <div class="card card-body bg-light border-0 shadow-sm rounded-3 p-3">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select id="filterFakultas<%=rnd%>" class="form-select form-select-sm shadow-sm" onchange="loadJurusan<%=rnd%>(this.value, 'filterJurusan<%=rnd%>', null)">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select id="filterStatus<%=rnd%>" class="form-select form-select-sm shadow-sm" multiple size="3">
                                        <option value="<%=PrestasiPegawai.BELUM_DIPROSES%>"><%=PrestasiPegawai.BELUM_DIPROSES%></option>
                                        <option value="<%=PrestasiPegawai.SEDANG_DIPROSES%>"><%=PrestasiPegawai.SEDANG_DIPROSES%></option>
                                        <option value="<%=PrestasiPegawai.DISETUJUI%>"><%=PrestasiPegawai.DISETUJUI%></option>
                                        <option value="<%=PrestasiPegawai.DITOLAK%>"><%=PrestasiPegawai.DITOLAK%></option>
                                    </select>
                                </div>
                                <div class="col-md-4">
                        ...
                        <table class="table table-hover table-striped align-middle mb-0" id="tabelHTMLDataPrestasi<%=rnd%>">
                            <thead class="table-dark text-center align-middle">
                                <tr>
                                    ...
                                    <th class="text-start fw-semibold text-uppercase small px-3"><i class="fas fa-user-tie me-1"></i><%=Common.getBahasaConfig("Pegawai")%></th>
                            ...
                            <tbody id="tabelDataPrestasi<%=rnd%>">
                                <tr><td colspan="8" class="text-center py-5"><div class="spinner-border text-primary mb-3"></div><br><span class="text-muted fw-medium"><%=Common.getBahasaConfig("Memuat data prestasi pegawai...")%></span></td></tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="card-header bg-white border-0 pt-4 pb-0 px-4">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="fw-bold text-dark mb-0" id="formTitle<%=rnd%>">
                            <i class="fas fa-edit text-success me-2"></i><%=Common.getBahasaConfig("Formulir Prestasi Pegawai")%>
                        </h5>
                    </div>
                </div>
                <div class="card-body p-4">
                    <form id="formPrestasi<%=rnd%>" onsubmit="event.preventDefault(); simpanPrestasi<%=rnd%>();">
                        ...
                        <div class="row g-4 mt-1" id="formContainer<%=rnd%>">
                            ...
                            <div class="col-md-12">
                                <h6 class="fw-bold text-primary border-bottom pb-2 mb-0"><i class="fas fa-user-tie me-2"></i>1. <%=Common.getBahasaConfig("Data Pegawai & Penempatan")%></h6>
                            </div>
                            
                            <% if (idPegawaiLogin == null) { %>
                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Fakultas (Opsional Filter)")%></label>
                                <select class="form-select shadow-sm bg-light field-to-disable<%=rnd%>" id="selectFakultas<%=rnd%>" onchange="loadJurusan<%=rnd%>(this.value, 'selectJurusan<%=rnd%>', null)">
                                    <option value=""><%=Common.getBahasaConfig("-- Pilih Fakultas --")%></option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Jurusan (Opsional Filter)")%></label>
                            ...
                            <div class="col-md-12">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Cari Pegawai (Code / MyCode / Nama)")%> <span class="text-danger">*</span></label>
                                <input type="text" class="form-control fw-bold shadow-sm field-to-disable<%=rnd%>" id="inputCariPegawai<%=rnd%>" list="dlPegawai<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Ketik Code, MyCode, atau Nama untuk mencari...")%>" oninput="onInputPegawai<%=rnd%>(this.value)" onchange="checkSelectedPegawai<%=rnd%>(this.value)" autocomplete="off" required>
                                <datalist id="dlPegawai<%=rnd%>"></datalist>
                                <input type="hidden" id="hiddenIdPegawai<%=rnd%>" required>
                            </div>
                            <% } else { %>
                                <input type="hidden" id="hiddenIdPegawai<%=rnd%>" value="<%=idPegawaiLogin%>">
                            <% } %>
                            ...
                            <div class="col-md-12 mt-4">
                                <h6 class="fw-bold text-primary border-bottom pb-2 mb-0"><i class="fas fa-info-circle me-2"></i>2. <%=Common.getBahasaConfig("Detail Prestasi")%></h6>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control fw-bold shadow-sm field-to-disable<%=rnd%>" id="inputNama<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Contoh: Pegawai Teladan Tingkat Nasional")%>" required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Nama Prestasi (Bahasa Inggris)")%></label>
                                <input type="text" class="form-control shadow-sm field-to-disable<%=rnd%>" id="inputNamaEn<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Contoh: Exemplary Employee at National Level")%>">
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <textarea class="form-control shadow-sm field-to-disable<%=rnd%>" id="inputTempat<%=rnd%>" rows="1" placeholder="<%=Common.getBahasaConfig("Tempat kegiatan berlangsung")%>"></textarea>
                            </div>
                            ...
                            <% } else { %>
                                <input type="hidden" id="inputStatus<%=rnd%>" value="<%=PrestasiPegawai.BELUM_DIPROSES%>">
                            <% } %>
...
<script>
    ...
    // ==========================================
    const masterModelClass<%=rnd%> = "<%=PrestasiPegawai.class.getName()%>";
    ...
    const disableFormIfApproved<%=rnd%> = (status) => {
        ...
        const isApproved = (status === '<%=PrestasiPegawai.DISETUJUI%>');
        
        fieldsToDisable.forEach(field => {
            field.disabled = isApproved;
        });
        
        <% if (idPegawaiLogin != null && (add || edit)) { %>
    ...
    const downloadExcelPrestasi<%=rnd%> = () => {
        ...
        const wb = XLSX.utils.table_to_book(cloneTable, {sheet: "<%=Common.getBahasaConfig("Data Prestasi Pegawai")%>"});
        XLSX.writeFile(wb, "Data_Prestasi_Pegawai_<%=System.currentTimeMillis()%>.xlsx");
    };
    ...
    async function checkExistingFile<%=rnd%>(idPrestasi, isApproved) {
        var reqObj = { 
            ...
            "jenis": "<%=PrestasiPegawai.class.getName()%>",
            "kondisiTambahan": "",
            "refresh": "false",
            "usingId": false
        };
    ...
    const loadFakultas<%=rnd%> = async (targetId, selectedId) => {
        ...
        try {
            ...
            if(targetId.includes('filter')) {
                options = '<option value="">-- <%=Common.getBahasaConfig("Semua Fakultas")%> --</option>';
            }
    ...
    const loadJurusan<%=rnd%> = async (idFakultas, targetId, selectedId) => {
        ...
        var reqObj = { 
            ...
            // Gunakan properti 'fakultas' secara langsung (tanpa .id) untuk menghindari error join
            "where1": "fakultas = " + idFakultas + " AND (aktif = true or aktif is null)",
            "order1": "asc", 
            "sort1": "nama" 
        };
        try {
    ...
    const loadCabang<%=rnd%> = async (targetId, selectedId) => {
        var reqObj = { 
            "action": "daftar", 
            "class": "<%=CabangPrestasiPegawai.class.getName()%>", 
            "order1": "asc", 
            "sort1": "nama" 
        };
        try {
    ...
    const loadKategori<%=rnd%> = async (targetId, selectedId) => {
        var reqObj = { 
            "action": "daftar", 
            "class": "<%=KategoriPrestasiPegawai.class.getName()%>", 
            "order1": "asc", 
            "sort1": "nama" 
        };
        try {
    ...
    // Pencarian Pegawai Datalist
    const onInputPegawai<%=rnd%> = async (val) => {
        if (val.length < 2) return;
        
        let whereClause = "(aktif = true or aktif is null) AND (code ILIKE '%" + val + "%' OR mycode ILIKE '%" + val + "%' OR nama ILIKE '%" + val + "%')";
        
        // Filter tambahan berdasarkan form
        const idFakultas = document.getElementById('selectFakultas<%=rnd%>').value;
        const idJurusan = document.getElementById('selectJurusan<%=rnd%>').value;
        if(idJurusan) {
            // Gunakan penamaan properti di class Pegawai.java
            whereClause += " AND tendikJurusan = " + idJurusan;
        } else if (idFakultas) {
            whereClause += " AND tendikFakultas = " + idFakultas;
        }

        var reqObj = { 
            "action": "daftar", 
            "class": "<%=Pegawai.class.getName()%>", 
            "where1": whereClause,
            "max": 20
        };
        try {
            ...
            let options = '';
            data.forEach((row) => {
            	options += '<option data-id="' + row.id + '" value="' + (row.code || row.mycode || '-') + ' - ' + (row.nama || '-') + '"></option>';
            });
            document.getElementById('dlPegawai<%=rnd%>').innerHTML = options;
        } catch (e) { console.error("Error search Pegawai", e); }
    };

    const checkSelectedPegawai<%=rnd%> = (val) => {
        const dl = document.getElementById('dlPegawai<%=rnd%>');
        const options = dl.options;
        for(let i=0; i<options.length; i++) {
            if(options[i].value === val) {
                document.getElementById('hiddenIdPegawai<%=rnd%>').value = options[i].getAttribute('data-id');
                return;
            }
        }
        document.getElementById('hiddenIdPegawai<%=rnd%>').value = '';
    };
    ...
    const buildFilterSQL<%=rnd%> = () => {
        ...
        <% if (idPegawaiLogin != null) { %>
            filters.push("pegawai = <%=idPegawaiLogin%>");
        <% } %>

        if (keyword !== '') {
            filters.push("(this_.nama ILIKE '%" + keyword + "%' OR pegawai1_.code ILIKE '%" + keyword + "%' OR pegawai1_.mycode ILIKE '%" + keyword + "%' OR pegawai1_.nama ILIKE '%" + keyword + "%')");
        }
        ...
        <% if (idPegawaiLogin == null) { %>
        const fFakultas = document.getElementById('filterFakultas<%=rnd%>').value;
        // Gunakan properti tendikFakultas langsung dari alias pegawai1_
        if(fFakultas) filters.push("pegawai1_.tendikFakultas = " + fFakultas);
        
        const fJurusan = document.getElementById('filterJurusan<%=rnd%>').value;
        if(fJurusan) filters.push("pegawai1_.tendikJurusan = " + fJurusan);
	    <% } %>
	    
	    const cbg = getMultiSelectValues<%=rnd%>('filterCabang<%=rnd%>');
	    // Sesuaikan dengan nama properti di class model (camelCase)
	    if(cbg) filters.push("this_.cabangPrestasiPegawai IN (" + cbg + ")");
	    
	    const kat = getMultiSelectValues<%=rnd%>('filterKategori<%=rnd%>');
	    if(kat) filters.push("this_.kategoriPrestasiPegawai IN (" + kat + ")");
    ...
    const loadDataPrestasi<%=rnd%> = async () => {
        ...
        var reqObj = {
                "action": "daftar", 
                "class": "<%=PrestasiPegawai.class.getName()%>",
                "deep": "1", 
                ...
                "sort1": "id", 
                "count": "true",
                "alias1": "pegawai" // Penting untuk filter fakultas/jurusan pegawai
            };
        
        // Memastikan join ke table pegawai terjadi
        reqObj["alias1"] = "pegawai"; 
        ...
        try {
            ...
            if (recordList.length === 0) {
                htmlList = '<tr><td colspan="8" class="text-center text-muted py-5"><i class="fas fa-box-open fa-3x mb-3 opacity-50 d-block"></i><%=Common.getBahasaConfig("Tidak ada data prestasi pegawai ditemukan.")%></td></tr>';
            } else {
                ...
                recordList.forEach((row) => {
                    ...
                    const pegCode = row["pegawai.code"] || row["pegawai.mycode"] || '-';
                    const pegNama = row["pegawai.nama"] || '-';
                    const kategoriNama = row["kategoriPrestasiPegawai.nama"] || '-';
                    const cabangNama = row["cabangPrestasiPegawai.nama"] || '-';
                    const tgl = row["tanggal.formated2"] || row["tanggal.datetime_format"] || row["tanggal"] || '-';
                    const statusVal = row.status || '<%=PrestasiPegawai.BELUM_DIPROSES%>';
                    
                    const infoPegawai = '<div class="fw-bold text-dark fs-6">' + pegNama + '</div>' + 
                                     '<small class="text-muted"><i class="fas fa-id-card me-1"></i>NIK: ' + pegCode + '</small>';

                    let statusBadge = '<span class="badge bg-secondary">' + statusVal + '</span>';
                    if (statusVal === '<%=PrestasiPegawai.DISETUJUI%>') {
                        statusBadge = '<span class="badge bg-success">' + statusVal + '</span>';
                    } else if (statusVal === '<%=PrestasiPegawai.DITOLAK%>') {
                        statusBadge = '<span class="badge bg-danger">' + statusVal + '</span>';
                    } else if (statusVal === '<%=PrestasiPegawai.SEDANG_DIPROSES%>') {
                    ...
                    htmlList += '<tr>' +
                            ...
                            '<td class="text-start">' + infoPegawai + '</td>' +
                            '<td class="text-start fw-semibold text-primary">' + safeNama + '</td>' +
                            '<td class="text-start text-muted small">' + kategoriNama + ' / ' + cabangNama + '</td>' +
    ...
    const bukaFormTambah<%=rnd%> = () => {
        ...
        isDocumentUploaded<%=rnd%> = false; 
        disableFormIfApproved<%=rnd%>('<%=PrestasiPegawai.BELUM_DIPROSES%>');
        
        <% if (idPegawaiLogin == null) { %>
            document.getElementById('inputCariPegawai<%=rnd%>').value = '';
            document.getElementById('hiddenIdPegawai<%=rnd%>').value = '';
            if (document.getElementById('inputStatus<%=rnd%>')) {
                document.getElementById('inputStatus<%=rnd%>').value = '<%=PrestasiPegawai.BELUM_DIPROSES%>';
            }
            loadFakultas<%=rnd%>('selectFakultas<%=rnd%>', null);
            loadJurusan<%=rnd%>(null, 'selectJurusan<%=rnd%>', null);
        <% } else { %>
            if (document.getElementById('inputStatus<%=rnd%>')) {
                document.getElementById('inputStatus<%=rnd%>').value = '<%=PrestasiPegawai.BELUM_DIPROSES%>';
            }
        ...
        document.getElementById('formTitle<%=rnd%>').innerHTML = '<i class="fas fa-plus-circle text-primary me-2"></i><%=Common.getBahasaConfig("Tambah Data Prestasi Pegawai")%>';
    ...
    const simpanPrestasi<%=rnd%> = async () => {
        ...
        const pegVal = document.getElementById('hiddenIdPegawai<%=rnd%>').value;
        if(!pegVal) {
            showToast<%=rnd%>('<%=Common.getBahasaConfigJS("Harap isi dan pilih data Pegawai dengan benar.")%>', 'bg-warning text-dark');
            return;
        }
        ...
        dataObj.pegawai = "null";
        dataObj.cabangPrestasiPegawai = "null";
        dataObj.kategoriPrestasiPegawai = "null";
        
        dataObj.pegawai = parseInt(pegVal);
        
        const cbgVal = document.getElementById('selectCabang<%=rnd%>').value;
        dataObj.cabangPrestasiPegawai = cbgVal ? parseInt(cbgVal) : null;
        
        const katVal = document.getElementById('selectKategori<%=rnd%>').value;
        dataObj.kategoriPrestasiPegawai = katVal ? parseInt(katVal) : null;
    ...
    const editPrestasi<%=rnd%> = async (idPrestasi) => {
        ...
        try {
            ...
            if (result.status === '00' || result.status === 'success' || result.id) {
                ...
                const isApproved = (data.status === '<%=PrestasiPegawai.DISETUJUI%>');
                ...
                if (document.getElementById('inputStatus<%=rnd%>')) {
                    document.getElementById('inputStatus<%=rnd%>').value = data.status || '<%=PrestasiPegawai.BELUM_DIPROSES%>';
                }

                if(data["pegawai.id"]) {
                    document.getElementById('hiddenIdPegawai<%=rnd%>').value = data["pegawai.id"];
                    <% if (idPegawaiLogin == null) { %>
                    document.getElementById('inputCariPegawai<%=rnd%>').value = (data["pegawai.code"] || data["pegawai.mycode"] || '-') + ' - ' + (data["pegawai.nama"] || '');
                    <% } %>
                }
                
                loadCabang<%=rnd%>('selectCabang<%=rnd%>', data["cabangPrestasiPegawai.id"] ? data["cabangPrestasiPegawai.id"] : null);
                loadKategori<%=rnd%>('selectKategori<%=rnd%>', data["kategoriPrestasiPegawai.id"] ? data["kategoriPrestasiPegawai.id"] : null);

                disableFormIfApproved<%=rnd%>(data.status);
                checkExistingFile<%=rnd%>(data.id, isApproved);
                
                document.getElementById('uploadPhotoArea<%=rnd%>').style.display = 'block';
                document.getElementById('formTitle<%=rnd%>').innerHTML = '<i class="fas fa-edit text-warning me-2"></i><%=Common.getBahasaConfig("Ubah Data Prestasi Pegawai")%>';
                document.getElementById('viewList<%=rnd%>').style.display = 'none';
                document.getElementById('viewForm<%=rnd%>').style.display = 'block';
            } else {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        loadJurusan<%=rnd%>(null, 'filterJurusan<%=rnd%>', null);
        <% } %>
