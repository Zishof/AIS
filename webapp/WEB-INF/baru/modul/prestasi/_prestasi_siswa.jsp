<%@page import="ais.database.model.sekolah.PrestasiSiswa"%>
                ...
                <div class="card-header bg-white border-0 pt-4 pb-3 px-4 border-bottom border-light">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-2">
                        <div>
                            <h5 class="fw-bold text-dark mb-1">
                                <i class="fas fa-trophy text-primary me-2"></i><%=Common.getBahasaConfig("Manajemen Prestasi Siswa")%>
                            </h5>
                            <small class="text-muted"><%=Common.getBahasaConfig("Kelola data pencapaian dan prestasi siswa.")%></small>
                        </div>
                        
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            <div class="input-group input-group-sm shadow-sm" style="min-width:180px;flex:1 1 200px;">
                                <span class="input-group-text bg-white border-end-0"><i class="fas fa-search text-muted"></i></span>
                                <input type="text" class="form-control border-start-0 ps-0 bg-white fw-medium" id="searchPrestasi<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Cari Prestasi, NIS / Nama...")%>">
                            </div>
                    ...
                    <div class="collapse mt-3" id="filterPanel<%=rnd%>">
                        <div class="card card-body bg-light border-0 shadow-sm rounded-3 p-3">
                            <h6 class="fw-bold text-secondary mb-3"><i class="fas fa-sliders-h me-2"></i>Filter Lanjutan</h6>
                            <div class="row g-3">
                                <div class="col-md-3">
                                    <label class="form-label small fw-bold text-muted mb-1"><%=Common.getBahasaConfig("Tahun Pelajaran")%></label>
                                    <select id="filterTahunPelajaran<%=rnd%>" class="form-select form-select-sm shadow-sm">
                        ...
                        <table class="table table-hover table-striped align-middle mb-0" id="tabelHTMLDataPrestasi<%=rnd%>">
                            <thead class="table-dark text-center align-middle">
                                <tr>
                                    ...
                                    <th class="text-start fw-semibold text-uppercase small px-3"><i class="fas fa-user-graduate me-1"></i><%=Common.getBahasaConfig("Siswa")%></th>
                            ...
                            <tbody id="tabelDataPrestasi<%=rnd%>">
                                <tr><td colspan="8" class="text-center py-5"><div class="spinner-border text-primary mb-3"></div><br><span class="text-muted fw-medium"><%=Common.getBahasaConfig("Memuat data prestasi...")%></span></td></tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="card-header bg-white border-0 pt-4 pb-0 px-4">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="fw-bold text-dark mb-0" id="formTitle<%=rnd%>">
                            <i class="fas fa-edit text-success me-2"></i><%=Common.getBahasaConfig("Formulir Prestasi Siswa")%>
                        </h5>
                    </div>
                </div>
                <div class="card-body p-4">
                    <form id="formPrestasi<%=rnd%>" onsubmit="event.preventDefault(); simpanPrestasi<%=rnd%>();">
                        ...
                        <div class="row g-4 mt-1" id="formContainer<%=rnd%>">
                            <div class="col-md-12">
                                <h6 class="fw-bold text-primary border-bottom pb-2 mb-0"><i class="fas fa-user-graduate me-2"></i>1. Data Siswa & Akademik</h6>
                            </div>
                            
                            <% if (idSiswaLogin == null) { %>
                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Cari Siswa (NIS / NISN / Nama)")%> <span class="text-danger">*</span></label>
                                <input type="text" class="form-control fw-bold shadow-sm field-to-disable<%=rnd%>" id="inputCariSiswa<%=rnd%>" list="dlSiswa<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Ketik NIS atau Nama untuk mencari...")%>" oninput="onInputSiswa<%=rnd%>(this.value)" onchange="checkSelectedSiswa<%=rnd%>(this.value)" autocomplete="off" required>
                                <datalist id="dlSiswa<%=rnd%>"></datalist>
                                <input type="hidden" id="hiddenIdSiswa<%=rnd%>" required>
                                <small class="text-muted fst-italic"><%=Common.getBahasaConfig("Pilih dari daftar. Filter Yayasan/Sekolah di bawah akan mempengaruhi hasil pencarian.")%></small>
                            </div>
                            ...
                            <% } else { %>
                                <input type="hidden" id="hiddenIdSiswa<%=rnd%>" value="<%=idSiswaLogin%>">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select shadow-sm field-to-disable<%=rnd%>" id="inputTahunPelajaran<%=rnd%>" required>
                            ...
                            <div class="col-md-12 mt-4">
                                <h6 class="fw-bold text-primary border-bottom pb-2 mb-0"><i class="fas fa-info-circle me-2"></i>2. Detail Prestasi Utama</h6>
                            </div>
                            ...
                            <div class="col-md-8">
                                ...
                                <input type="text" class="form-control fw-bold shadow-sm field-to-disable<%=rnd%>" id="inputNama<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Contoh: Juara 1 Lomba Sains Nasional")%>" required>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Tahun Penyelenggaraan")%></label>
                            ...
                            <div class="col-md-12">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Tempat / Alamat Penyelenggaraan")%></label>
                                <textarea class="form-control shadow-sm field-to-disable<%=rnd%>" id="inputTempat<%=rnd%>" rows="2" placeholder="<%=Common.getBahasaConfig("Alamat atau lokasi kegiatan")%>"></textarea>
                            </div>

                            <div class="col-md-12 mt-4">
                                <h6 class="fw-bold text-primary border-bottom pb-2 mb-0"><i class="fas fa-layer-group me-2"></i>3. Klasifikasi & Informasi Pendukung</h6>
                            </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="text" class="form-control shadow-sm field-to-disable<%=rnd%>" id="inputNoSk<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Nomor SK Prestasi")%>">
                            </div>
                            <div class="col-md-3">
                            ...
                            <div class="col-md-4">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Guru Pembina 1 (Opsional)")%></label>
                                <input type="text" class="form-control shadow-sm field-to-disable<%=rnd%>" id="inputCariGuru1<%=rnd%>" list="dlGuru1<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Ketik Nama / NIP Guru...")%>" oninput="onInputGuru<%=rnd%>(this.value, 1)" onchange="checkSelectedGuru<%=rnd%>(this.value, 1)" autocomplete="off">
                                <datalist id="dlGuru1<%=rnd%>"></datalist>
                                <input type="hidden" id="hiddenIdGuru1<%=rnd%>">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Guru Pembina 2 (Opsional)")%></label>
                                <input type="text" class="form-control shadow-sm field-to-disable<%=rnd%>" id="inputCariGuru2<%=rnd%>" list="dlGuru2<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Ketik Nama / NIP Guru...")%>" oninput="onInputGuru<%=rnd%>(this.value, 2)" onchange="checkSelectedGuru<%=rnd%>(this.value, 2)" autocomplete="off">
                                <datalist id="dlGuru2<%=rnd%>"></datalist>
                                <input type="hidden" id="hiddenIdGuru2<%=rnd%>">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Guru Pembina 3 (Opsional)")%></label>
                                <input type="text" class="form-control shadow-sm field-to-disable<%=rnd%>" id="inputCariGuru3<%=rnd%>" list="dlGuru3<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Ketik Nama / NIP Guru...")%>" oninput="onInputGuru<%=rnd%>(this.value, 3)" onchange="checkSelectedGuru<%=rnd%>(this.value, 3)" autocomplete="off">
                            ...
                            <% if (idSiswaLogin == null) { %>
                            <div class="col-md-12 mt-4">
                                <h6 class="fw-bold text-primary border-bottom pb-2 mb-0"><i class="fas fa-clipboard-check me-2"></i>4. Status Persetujuan & Dokumen</h6>
                            </div>
                            <div class="col-md-6">
...
<script>
    ...
    // ==========================================
    const masterModelClass<%=rnd%> = "<%=PrestasiSiswa.class.getName()%>";
    ...
    const downloadExcelPrestasi<%=rnd%> = () => {
        ...
        const wb = XLSX.utils.table_to_book(cloneTable, {sheet: "Data Prestasi"});
        XLSX.writeFile(wb, "Data_Prestasi_Siswa_<%=System.currentTimeMillis()%>.xlsx");
    };
    ...
    async function checkExistingFile<%=rnd%>(idPrestasi, isApproved) {
        var reqObj = { 
            ...
            "jenis": "<%=PrestasiSiswa.class.getName()%>",
            "kondisiTambahan": "",
            "refresh": "false",
            "usingId": false
        };
    ...
    const resetUploadButtons<%=rnd%> = (isApproved) => {
        if (!isApproved) {
            var btnUploadHtml = '<input type="file" id="input_dofile_foto_<%=rnd%>" class="d-none" accept=".pdf, image/*" onchange="uploadFile<%=rnd%>()">' +
                                '<button type="button" class="btn btn-primary fw-bold shadow-sm px-4 field-to-disable<%=rnd%>" id="lbl-upload-<%=rnd%>" onclick="document.getElementById(\'input_dofile_foto_<%=rnd%>\').click();" style="border-radius: 0;">' +
                                '   <i class="fas fa-upload me-2"></i><%=Common.getBahasaConfig("Unggah Bukti")%>' +
                                '</button>';
            $('#action-buttons-<%=rnd%>').html(btnUploadHtml);
        } else {
            $('#action-buttons-<%=rnd%>').html('<span class="text-danger fw-bold"><i class="fas fa-ban me-1"></i>Dokumen Tidak Ada (Read-Only)</span>');
        }
    };
    ...
    const loadYayasan<%=rnd%> = async (targetId, selectedId) => {
        ...
        try {
            ...
            if (data.length === 1) {
                ...
                options = '<option value="">-- ' + (targetId.includes('filter') ? 'Semua Yayasan' : 'Pilih Yayasan') + ' --</option>';
                data.forEach((row) => {
    ...
    const loadFilterSekolah<%=rnd%> = async (idYayasan) => {
        ...
        <% if (idSekolahLogin != null) { %>
            filterWhere.push("this_.id = <%=idSekolahLogin%>");
        <% } else { %>
            if(idYayasan) {
                filterWhere.push("this_.yayasan_id = " + idYayasan);
            } <% if (idYayasanLogin != null) { %> else {
                filterWhere.push("this_.yayasan_id = <%=idYayasanLogin%>");
            } <% } %>
        <% } %>
        ...
        try {
            ...
            if (data.length === 1) {
                ...
                options = '<option value="">-- Semua Sekolah --</option>';
                data.forEach((row) => {
    ...
    const loadSekolah<%=rnd%> = async (idYayasan, selectedId) => {
        ...
        //console.log("loadSekolah reqObj -> ", reqObj);
        ...
        try {
            ...
            if (data.length === 1) {
                ...
                options = '<option value="">-- Pilih Sekolah --</option>';
                data.forEach((row) => {
                    const selected = (selectedId && selectedId == row.id) ? "selected" : "";
                    options += '<option value="' + row.id + '" ' + selected + '>' + row.nama + '</option>';
                });
    ...
    const loadCabang<%=rnd%> = async (targetId, selectedId) => {
        var reqObj = { 
            "action": "daftar", 
            "class": "ais.database.model.sekolah.CabangPrestasiSiswa", 
            "order1": "asc", 
            "sort1": "nama" 
        };
        try {
            ...
            let options = targetId.includes('filter') ? '' : '<option value="">-- Pilih Cabang --</option>';
            data.forEach((row) => {
                const selected = (selectedId && selectedId == row.id) ? "selected" : "";
                options += '<option value="' + row.id + '" ' + selected + '>' + row.nama + '</option>';
            });
    ...
    const loadKategori<%=rnd%> = async (targetId, selectedId) => {
        var reqObj = { 
            "action": "daftar", 
            "class": "ais.database.model.sekolah.KategoriPrestasiSiswa", 
            "order1": "asc", 
            "sort1": "nama" 
        };
        try {
            ...
            let options = targetId.includes('filter') ? '' : '<option value="">-- Pilih Kategori --</option>';
            data.forEach((row) => {
                const selected = (selectedId && selectedId == row.id) ? "selected" : "";
                options += '<option value="' + row.id + '" ' + selected + '>' + row.nama + '</option>';
            });
    ...
    // Pencarian Siswa Datalist
    <% if (idSiswaLogin == null) { %>
    const onInputSiswa<%=rnd%> = async (val) => {
        if (val.length < 1) return;
        let valYayasan = document.getElementById('selectYayasan<%=rnd%>').value;
        let valSekolah = document.getElementById('selectSekolah<%=rnd%>').value;
        
        let extSql = "aktif = true AND (this_.nomor_induk ILIKE '%" + val + "%' OR this_.nomor_induk_nasional ILIKE '%" + val + "%' OR nama_siswa ILIKE '%" + val + "%')";
        
        if (valSekolah) {
            extSql += " AND this_.sekolah_id = " + valSekolah;
        } else if (valYayasan) {
            extSql += " AND this_.sekolah_id IN (SELECT id FROM sekolah.sekolah WHERE yayasan_id = " + valYayasan + ")";
        }
        ...
        try {
            ...
            let options = '';
            data.forEach((row) => {
                let identifier = row.nomorIndukNasional ? row.nomorIndukNasional : (row.nomorInduk ? row.nomorInduk : '-');
                options += '<option data-id="' + row.id + '" value="' + identifier + ' - ' + row.namaSiswa + '"></option>';
            });
            document.getElementById('dlSiswa<%=rnd%>').innerHTML = options;
        } catch (e) { console.error("Error search Siswa", e); }
    };

    const checkSelectedSiswa<%=rnd%> = (val) => {
        const dl = document.getElementById('dlSiswa<%=rnd%>');
    ...
    // Pencarian Guru Datalist
    const onInputGuru<%=rnd%> = async (val, no) => {
        if (val.length < 2) return;
        var reqObj = { 
            "action": "daftar", 
            "class": "ais.database.model.sekolah.Guru", 
            "where1": "aktif = true AND (nama_guru ILIKE '%" + val + "%' OR nip ILIKE '%" + val + "%' OR kode ILIKE '%" + val + "%')",
            "max": 20
        };
        try {
            const res = await fetch('<%=Common.ROOT%>/Data', {
            ...
            let options = '';
            data.forEach((row) => {
                options += '<option data-id="' + row.id + '" value="' + (row.nip || row.kode || '-') + ' - ' + row.namaGuru + '"></option>';
            });
            document.getElementById('dlGuru' + no + '<%=rnd%>').innerHTML = options;
        } catch (e) { console.error("Error search Guru", e); }
    };

    const checkSelectedGuru<%=rnd%> = (val, no) => {
        const dl = document.getElementById('dlGuru' + no + '<%=rnd%>');
        const options = dl.options;
        for(let i=0; i<options.length; i++) {
            if(options[i].value === val) {
                document.getElementById('hiddenIdGuru' + no + '<%=rnd%>').value = options[i].getAttribute('data-id');
                return;
            }
        }
        document.getElementById('hiddenIdGuru' + no + '<%=rnd%>').value = '';
    };
    ...
    const buildFilterSQL<%=rnd%> = () => {
        ...
        <% if (idSiswaLogin != null) { %>
            filters.push("this_.siswa_id = <%=idSiswaLogin%>");
        <% } else { %>
	        <% if (idSekolahLogin != null) { %>
	            filters.push("this_.sekolah_id = <%=idSekolahLogin%>");
	        <% } else if (idYayasanLogin != null) { %>
	            filters.push("this_.yayasan_id = <%=idYayasanLogin%>");
	        <% } %>
        <% } %>

        if (keyword !== '') {
            filters.push("(this_.nama ILIKE '%" + keyword + "%' OR siswa1_.nomor_induk ILIKE '%" + keyword + "%' OR siswa1_.nomor_induk_nasional ILIKE '%" + keyword + "%' OR siswa1_.nama_siswa ILIKE '%" + keyword + "%')");
        }
        
        const fTa = document.getElementById('filterTahunPelajaran<%=rnd%>').value;
        ...
        <% if (idSiswaLogin == null) { %>
            const fYay = document.getElementById('filterYayasan<%=rnd%>').value;
            if(fYay) filters.push("this_.yayasan_id = " + fYay);
            
            const fSek = document.getElementById('filterSekolah<%=rnd%>').value;
            if(fSek) filters.push("this_.sekolah_id = " + fSek);
        <% } %>
        
        const cbg = getMultiSelectValues<%=rnd%>('filterCabang<%=rnd%>');
        if(cbg) filters.push("this_.cabangPrestasiSiswa IN (" + cbg + ")");
        
        const kat = getMultiSelectValues<%=rnd%>('filterKategori<%=rnd%>');
        if(kat) filters.push("this_.kategoriPrestasiSiswa IN (" + kat + ")");
    ...
    const loadDataPrestasi<%=rnd%> = async () => {
        ...
        var reqObj = {
            "action": "daftar", 
            "class": "<%=PrestasiSiswa.class.getName()%>",
            "deep": "1", 
        ...
        try {
            ...
            if (recordList.length === 0) {
                htmlList = '<tr><td colspan="8" class="text-center text-muted py-5"><i class="fas fa-box-open fa-3x mb-3 opacity-50 d-block"></i><%=Common.getBahasaConfig("Tidak ada data prestasi ditemukan.")%></td></tr>';
            } else {
                ...
                recordList.forEach((row) => {
                    ...
                    const siswaIdentifier = row["siswa.nomorIndukNasional"] ? row["siswa.nomorIndukNasional"] : (row["siswa.nomorInduk"] || '-');
                    const siswaNama = row["siswa.namaSiswa"] || '-';
                    const kategoriNama = row["kategoriPrestasiSiswa.nama"] || '-';
                    const tgl = row["tanggal.formated2"] || row["tanggal.datetime_format"] || row["tanggal"] || '-';
                    const statusVal = row.status || 'Belum diproses';
                    
                    const infoSiswa = '<div class="fw-bold text-dark fs-6">' + siswaNama + '</div>' + 
                                      '<small class="text-muted"><i class="fas fa-id-card me-1"></i>NIS/NISN: ' + siswaIdentifier + '</small>';
                    ...
                    let buktiHtml = '<span class="text-danger small fw-bold fst-italic"><i class="fas fa-times-circle me-1"></i>Belum Diupload</span>';
                    if (row.lampiranData) {
                        ...
                        if (isImage) {
                            buktiHtml = '<img src="' + fileUrl + '" class="rounded-3 shadow-sm object-fit-cover border border-light" style="width: 50px; height: 50px; cursor:pointer;" onclick="window.open(\'' + fileUrl + '\', \'_blank\')" alt="Bukti" title="' + lampiranData.nama + '">';
                        } else {
                            buktiHtml = '<a href="' + fileUrl + '" target="_blank" class="btn btn-sm btn-outline-info shadow-sm py-1 fw-bold" title="' + lampiranData.nama + '"><i class="fas fa-download me-1"></i>Cek Bukti</a>';
                        }
                    }
                    ...
                    htmlList += '<tr>' +
                            ...
                            '<td class="text-start">' + infoSiswa + '</td>' +
                            ...
                            '<td class="text-center text-nowrap no-export">' +
                                '<button class="btn btn-sm btn-outline-warning text-dark shadow-sm px-2 me-1 fw-bold" onclick="editPrestasi<%=rnd%>(' + row.id + ')" title="<%=Common.getBahasaConfig("Edit / Lihat Detail")%>"><i class="fas fa-edit"></i></button>' +
                                actionBtns +
                            '</td>' +
                        '</tr>';
                });
    ...
    const bukaFormTambah<%=rnd%> = () => {
        ...
        document.getElementById('inputTahunPelajaran<%=rnd%>').value = '<%=selectedTa%>';
        document.getElementById('inputJenisSemester<%=rnd%>').value = '<%=selectedSmt%>';
        
        document.getElementById('inputCariGuru1<%=rnd%>').value = '';
        document.getElementById('hiddenIdGuru1<%=rnd%>').value = '';
        ...
        document.getElementById('formTitle<%=rnd%>').innerHTML = '<i class="fas fa-plus-circle text-primary me-2"></i><%=Common.getBahasaConfig("Tambah Data Prestasi")%>';
    ...
    const uploadFile<%=rnd%> = async () => {
        ...
        progressBar.innerText = 'Mengunggah...';
    ...
    const simpanPrestasi<%=rnd%> = async () => {
        ...
        const siswaVal = document.getElementById('hiddenIdSiswa<%=rnd%>').value;
        if(!siswaVal) {
            showToast<%=rnd%>('<%=Common.getBahasaConfigJS("Harap isi dan pilih data Siswa dengan benar.")%>', 'bg-warning text-dark');
            return;
        }
        ...
        const dataObj = {
            ...
            tahunAkademik: document.getElementById('inputTahunPelajaran<%=rnd%>').value || null,
        ...
        dataObj.cabangPrestasiSiswa = "null";
        dataObj.kategoriPrestasiSiswa = "null";
        dataObj.guru = "null";
        dataObj.guru2 = "null";
        dataObj.guru3 = "null";
        
        dataObj.siswa = parseInt(siswaVal);
        
        <% if (idSiswaLogin == null) { %>
            const yayVal = document.getElementById('selectYayasan<%=rnd%>').value;
            dataObj.yayasan = yayVal ? parseInt(yayVal) : null;
            
            const sekVal = document.getElementById('selectSekolah<%=rnd%>').value;
            dataObj.sekolah = sekVal ? parseInt(sekVal) : null;
        <% } %>
        
        const cbgVal = document.getElementById('selectCabang<%=rnd%>').value;
        dataObj.cabangPrestasiSiswa = cbgVal ? parseInt(cbgVal) : null;
        
        const katVal = document.getElementById('selectKategori<%=rnd%>').value;
        dataObj.kategoriPrestasiSiswa = katVal ? parseInt(katVal) : null;

        const guru1Val = document.getElementById('hiddenIdGuru1<%=rnd%>').value;
        dataObj.guru = guru1Val ? parseInt(guru1Val) : null;

        const guru2Val = document.getElementById('hiddenIdGuru2<%=rnd%>').value;
        dataObj.guru2 = guru2Val ? parseInt(guru2Val) : null;
        
        const guru3Val = document.getElementById('hiddenIdGuru3<%=rnd%>').value;
        dataObj.guru3 = guru3Val ? parseInt(guru3Val) : null;
    ...
    const editPrestasi<%=rnd%> = async (idPrestasi) => {
        ...
        try {
            ...
            if (result.status === '00' || result.status === 'success' || result.id) {
                ...
                document.getElementById('inputTahunPelajaran<%=rnd%>').value = data.tahunAkademik || '';
                ...
                document.getElementById('inputTglSk<%=rnd%>').value = data["tglSk.datetime_format"] || '';
                ...
                if(data["siswa.id"]) {
                    ...
                    <% if (idSiswaLogin == null) { %>
                    let identifier = data["siswa.nomorIndukNasional"] ? data["siswa.nomorIndukNasional"] : (data["siswa.nomorInduk"] ? data["siswa.nomorInduk"] : '-');
                    document.getElementById('inputCariSiswa<%=rnd%>').value = identifier + ' - ' + (data["siswa.namaSiswa"] || '');
                    <% } %>
                }

                if(data["guru.id"]) {
                    document.getElementById('hiddenIdGuru1<%=rnd%>').value = data["guru.id"];
                    document.getElementById('inputCariGuru1<%=rnd%>').value = (data["guru.nip"] || data["guru.kode"] || '-') + ' - ' + (data["guru.namaGuru"] || '');
                } else {
                    document.getElementById('hiddenIdGuru1<%=rnd%>').value = '';
                    document.getElementById('inputCariGuru1<%=rnd%>').value = '';
                }
                ...
                if(data["guru2.id"]) {
                    document.getElementById('hiddenIdGuru2<%=rnd%>').value = data["guru2.id"];
                    document.getElementById('inputCariGuru2<%=rnd%>').value = (data["guru2.nip"] || data["guru2.kode"] || '-') + ' - ' + (data["guru2.namaGuru"] || '');
                } else {
                    document.getElementById('hiddenIdGuru2<%=rnd%>').value = '';
                    document.getElementById('inputCariGuru2<%=rnd%>').value = '';
                }
                ...
                if(data["guru3.id"]) {
                    document.getElementById('hiddenIdGuru3<%=rnd%>').value = data["guru3.id"];
                    document.getElementById('inputCariGuru3<%=rnd%>').value = (data["guru3.nip"] || data["guru3.kode"] || '-') + ' - ' + (data["guru3.namaGuru"] || '');
                } else {
                    document.getElementById('hiddenIdGuru3<%=rnd%>').value = '';
                    document.getElementById('inputCariGuru3<%=rnd%>').value = '';
                }

                let idYayasan = data["yayasan.id"] ? data["yayasan.id"] : null;
                let idSekolah = data["sekolah.id"] ? data["sekolah.id"] : null;
                
                <% if (idSiswaLogin == null) { %>
                loadYayasan<%=rnd%>('selectYayasan<%=rnd%>', idYayasan);
                if(idYayasan) {
                    loadSekolah<%=rnd%>(idYayasan, idSekolah);
                } else {
                    loadSekolah<%=rnd%>(null, idSekolah);
                }
                <% } %>
                
                loadCabang<%=rnd%>('selectCabang<%=rnd%>', data["cabangPrestasiSiswa.id"] ? data["cabangPrestasiSiswa.id"] : null);
                loadKategori<%=rnd%>('selectKategori<%=rnd%>', data["kategoriPrestasiSiswa.id"] ? data["kategoriPrestasiSiswa.id"] : null);

                disableFormIfApproved<%=rnd%>(data.status);
                checkExistingFile<%=rnd%>(data.id, isApproved);
                
                document.getElementById('uploadPhotoArea<%=rnd%>').style.display = 'block';
                document.getElementById('formTitle<%=rnd%>').innerHTML = '<i class="fas fa-edit text-warning me-2"></i><%=Common.getBahasaConfig("Ubah Data Prestasi")%>';
                document.getElementById('viewList<%=rnd%>').style.display = 'none';
                document.getElementById('viewForm<%=rnd%>').style.display = 'block';
            } else {
