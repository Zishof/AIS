...
<%@page import="ais.database.model.PrestasiMahasiswa"%>
...
// Jika login sebagai mahasiswa, hanya boleh lihat prestasi-nya sendiri
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-2">
                        <div>
                            <h5 class="fw-bold text-dark mb-1">
                                <i class="fas fa-trophy text-primary me-2"></i><%=Common.getBahasaConfig("Manajemen Prestasi Mahasiswa")%>
                            </h5>
                            <small class="text-muted"><%=Common.getBahasaConfig("Kelola data pencapaian dan prestasi mahasiswa.")%></small>
                        </div>
                        
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            <div class="input-group input-group-sm shadow-sm" style="min-width:180px;flex:1 1 200px;">
                                <span class="input-group-text bg-white border-end-0"><i class="fas fa-search text-muted"></i></span>
                                <input type="text" class="form-control border-start-0 ps-0 bg-white fw-medium" id="searchPrestasi<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Cari Prestasi, NIM / Nama...")%>">
                            </div>
                ...
                <div class="card-header bg-white border-0 pt-4 pb-0 px-4">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="fw-bold text-dark mb-0" id="formTitle<%=rnd%>">
                            <i class="fas fa-edit text-success me-2"></i><%=Common.getBahasaConfig("Formulir Prestasi Mahasiswa")%>
                        </h5>
                    </div>
                </div>
                    ...
                    <form id="formPrestasi<%=rnd%>" onsubmit="event.preventDefault(); simpanPrestasi<%=rnd%>();">
                        ...
                        <div class="row g-4 mt-1" id="formContainer<%=rnd%>">
                            ...
                            <div class="col-md-8">
                                ...
                                <input type="text" class="form-control fw-bold shadow-sm field-to-disable<%=rnd%>" id="inputNama<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Contoh: Juara 1 Lomba Web Design Nasional")%>" required>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-12">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Alamat / Lokasi Penyelenggaraan")%></label>
...
<script>
    ...
    // ==========================================
    const masterModelClass<%=rnd%> = "<%=PrestasiMahasiswa.class.getName()%>";
    let activeEditingPrestasiId<%=rnd%> = "";
    let isDocumentUploaded<%=rnd%> = false; // State pelacakan mandatory dokumen
    ...
    // Khusus multi-select format String Database 
    ...
    const disableFormIfApproved<%=rnd%> = (status) => {
        ...
        // Jika user adalah Mahasiswa, tombol Simpan juga didisable jika disetujui
        <% if (idMhsLogin != null && (add || edit)) { %>
            document.getElementById('btnSimpan<%=rnd%>').disabled = isApproved;
        <% } %>
        
        // Hide info upload jika disetujui (karena tidak bisa upload lagi)
        if (isApproved) {
    ...
    // Fungsi Extract Excel (Hanya mendownload tabel yang di-render saat ini)
    const downloadExcelPrestasi<%=rnd%> = () => {
        const table = document.getElementById('tabelHTMLDataPrestasi<%=rnd%>');
        if (!table) return;

        // Clone table agar elemen UI aslinya tidak rusak saat tag-tag tidak relevan dihapus
        ...
        const wb = XLSX.utils.table_to_book(cloneTable, {sheet: "<%=Common.getBahasaConfig("Data Prestasi")%>"});
        XLSX.writeFile(wb, "Data_Prestasi_Mahasiswa_<%=System.currentTimeMillis()%>.xlsx");
    };
    ...
    async function checkExistingFile<%=rnd%>(idPrestasi, isApproved) {
        var reqObj = { 
            ...
            "jenis": "<%=PrestasiMahasiswa.class.getName()%>",
            "kondisiTambahan": "",
            "refresh": "false",
            "usingId": false
        };
    ...
    function generatePreview<%=rnd%>(data, isApproved) {
        ...
        // Hanya tampilkan tombol Hapus dan Ganti jika belum disetujui (dan user punya akses add/edit/delete)
        if (!isApproved) {
            <% if (delete) { %>
    ...
    const hapusFile<%=rnd%> = async (idLampiran) => {
        ...
        try {
            ...
            if (result.status === '00' || result.status === 'success') {
                ...
                loadDataPrestasi<%=rnd%>(); // Update tabel belakang
            } else {
    ...
    const loadCabang<%=rnd%> = async (targetId, selectedId) => {
        var reqObj = { 
            "action": "daftar", 
            "class": "ais.database.model.CabangPrestasiMahasiswa", 
            "order1": "asc", 
            "sort1": "nama" 
        };
        try {
    ...
    const loadKategori<%=rnd%> = async (targetId, selectedId) => {
        var reqObj = { 
            "action": "daftar", 
            "class": "ais.database.model.KategoriPrestasiMahasiswa", 
            "order1": "asc", 
            "sort1": "nama" 
        };
        try {
    ...
    const loadJenisAktifitas<%=rnd%> = async (targetId, selectedId) => {
        var reqObj = { 
            "action": "daftar", 
            "class": "ais.database.model.JenisAktfitasMahasiswa", 
            "where1": "aktif = true or aktif is null",
            "order1": "asc", 
            "sort1": "nama" 
        };
    ...
    const buildFilterSQL<%=rnd%> = () => {
        ...
        // Filter text box NIM/Nama/Prestasi 
        if (keyword !== '') {
        ...
        if(cbg) filters.push("this_.cabangPrestasiMahasiswa IN (" + cbg + ")");
        
        const kat = getMultiSelectValues<%=rnd%>('filterKategori<%=rnd%>');
        if(kat) filters.push("this_.kategoriPrestasiMahasiswa IN (" + kat + ")");
    ...
    const loadDataPrestasi<%=rnd%> = async () => {
        ...
        var reqObj = {
            "action": "daftar", 
            "class": "<%=PrestasiMahasiswa.class.getName()%>",
            "deep": "1", 
        ...
        try {
            ...
            if (recordList.length === 0) {
                ...
                // Concurrent fetch data file via API untuk setiap baris
                ...
                recordList.forEach((row) => {
                    ...
                    const kategoriNama = row["kategoriPrestasiMahasiswa.nama"] || '-';
                    ...
                    // Logika rendering Status Persetujuan
                    ...
                    // Logika render berdasarkan data file hasil fetch API
                    ...
                    if (row.lampiranData) {
                        ...
                        if (isImage) {
                            ...
                            // Menghindari eksposur ID via URL public
                    ...
                    <% if (delete) { %>
                        <% if (idMhsLogin != null) { %>
                            // Jika mahasiswa dan sudah disetujui, tidak boleh hapus
                            if (statusVal !== 'Disetujui') {
    ...
    const bukaFormTambah<%=rnd%> = () => {
        ...
        isDocumentUploaded<%=rnd%> = false; // Reset state tracking
        disableFormIfApproved<%=rnd%>('Belum diproses'); // Aktifkan semua field
        ...
        document.getElementById('uploadPhotoArea<%=rnd%>').style.display = 'none'; // Hanya tampil saat edit
    ...
    const uploadFile<%=rnd%> = async () => {
        ...
        // Buat dummy object untuk preview lokal sementara
        const localData = {
        ...
        try {
            ...
            if(result.status === 'Sukses' || result.status === '00' || result.keterangan === 'Sukses'){
                isDocumentUploaded<%=rnd%> = true; // Tandai berhasil upload
                ...
                // Fetch file ID terbaru dari API
                ...
                $('#preview-area-<%=rnd%>').html('').hide(); // Reset preview jika gagal
                resetUploadButtons<%=rnd%>(false);
            }
        } catch (err) {
            console.error(err);
    ...
    const simpanPrestasi<%=rnd%> = async () => {
        ...
        // Cek wajib upload untuk validasi di mode Edit
        ...
        if(!mhsVal) {
            showToast<%=rnd%>('<%=Common.getBahasaConfigJS("Harap isi dan pilih data Mahasiswa dengan benar.")%>', 'bg-warning text-dark');
            return;
        }
        ...
        dataObj.mahasiswa = "null";
        dataObj.jenisAktfitasMahasiswa = "null";
        dataObj.fakultas = "null";
        dataObj.jurusan = "null";
        dataObj.cabangPrestasiMahasiswa = "null";
        dataObj.kategoriPrestasiMahasiswa = "null";
        dataObj.dosenPembina1 = "null";
        dataObj.dosenPembina2 = "null";
        ...
        dataObj.cabangPrestasiMahasiswa = cbgVal ? parseInt(cbgVal) : null;
        
        const katVal = document.getElementById('selectKategori<%=rnd%>').value;
        dataObj.kategoriPrestasiMahasiswa = katVal ? parseInt(katVal) : null;
        
        const jnsVal = document.getElementById('selectJenisAktifitas<%=rnd%>').value;
        dataObj.jenisAktfitasMahasiswa = jnsVal ? parseInt(jnsVal) : null;

        const dos1Val = document.getElementById('hiddenIdDosen1<%=rnd%>').value;
        dataObj.dosenPembina1 = dos1Val ? parseInt(dos1Val) : null;

        const dos2Val = document.getElementById('hiddenIdDosen2<%=rnd%>').value;
        dataObj.dosenPembina2 = dos2Val ? parseInt(dos2Val) : null;
        ...
        try {
            ...
            if (result.status === '00' || result.status === 'success' || result.id) {
                // Skenario pembuatan data baru, belum ada upload dokumen
                if (!isDocumentUploaded<%=rnd%>) {
                    ...
                    loadDataPrestasi<%=rnd%>(); // Memperbarui grid di background
                } else {
    ...
    const editPrestasi<%=rnd%> = async (idPrestasi) => {
        ...
        try {
            ...
            if (result.status === '00' || result.status === 'success' || result.id) {
                ...
                activeEditingPrestasiId<%=rnd%> = data.id; // Assign ke variabel global upload
                ...
                if(idFak) {
                    ...
                    loadJurusan<%=rnd%>(null, idJur); // Mendukung jurusan terisi meski fakultas tidak dipaksa
                }
                <% } %>
                
                loadCabang<%=rnd%>('selectCabang<%=rnd%>', data["cabangPrestasiMahasiswa.id"] ? data["cabangPrestasiMahasiswa.id"] : null);
                loadKategori<%=rnd%>('selectKategori<%=rnd%>', data["kategoriPrestasiMahasiswa.id"] ? data["kategoriPrestasiMahasiswa.id"] : null);
                loadJenisAktifitas<%=rnd%>('selectJenisAktifitas<%=rnd%>', data["jenisAktfitasMahasiswa.id"] ? data["jenisAktfitasMahasiswa.id"] : null);

                // Disable Form Jika Status = Disetujui
                disableFormIfApproved<%=rnd%>(data.status);

                // Fetch lampiran details via API Fetch file
                checkExistingFile<%=rnd%>(data.id, isApproved);

                document.getElementById('uploadPhotoArea<%=rnd%>').style.display = 'block'; // Tampilkan input upload bukti
