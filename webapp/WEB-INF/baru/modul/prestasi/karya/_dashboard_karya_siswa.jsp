...
Long idSiswaLogin = (currentSiswa != null) ? currentSiswa.getId() : null;

String baseWhere = " WHERE p.id IS NOT NULL ";
if (idSiswaLogin != null) {
    baseWhere += " AND p.siswa = " + idSiswaLogin;
}
%>

<div class="animate__animated animate__fadeInUp" id="dashboardKaryaSiswa<%=rnd%>">
    <!-- KPI Row -->
...
(function(){
    ...
    function renderTerbaru(rows) {
        ...
        rows.forEach(function(r) {
            html += '<li class="list-group-item px-0 py-2 border-0 border-bottom">' +
                ...
                '<div class="fw-semibold text-dark" style="font-size:.85rem;line-height:1.3;">' + (r.nama || '-') + '</div>' +
                fmtBadge(r.status) +
                '</div>' +
                '<small class="text-muted">' + (r.nama_siswa || '') + '</small>' +
                '</li>';
        });
        html += '</ul>';
        el.innerHTML = html;
    ...
    async function muatDashboard<%=rnd%>() {
        var sqlTotal = "SELECT COUNT(*) as total, SUM(CASE WHEN p.status='Disetujui' THEN 1 ELSE 0 END) as disetujui, SUM(CASE WHEN p.status='Sedang diproses' THEN 1 ELSE 0 END) as proses FROM sekolah.penghargaan_siswa p " + baseWhere;
        var sqlStatus = "SELECT p.status, COUNT(*) as jml FROM sekolah.penghargaan_siswa p " + baseWhere + " GROUP BY p.status ORDER BY jml DESC";
        var sqlTerbaru = "SELECT p.nama, p.status, sw.nama as nama_siswa FROM sekolah.penghargaan_siswa p LEFT JOIN sekolah.siswa sw ON p.siswa = sw.id " + baseWhere + " ORDER BY p.id DESC LIMIT 10";
