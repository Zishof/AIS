...
if (currentMahasiswa != null) {
    baseWhere += " AND p.mahasiswa = " + currentMahasiswa.getId();
...
<div class="animate__animated animate__fadeInUp" id="dashboardKaryaMhs<%=rnd%>">
    <!-- KPI Row -->
...
(function(){
    ...
    function renderTerbaru(rows) {
        ...
        rows.forEach(function(r) {
            html += '<li class="list-group-item px-0 py-2 border-0 border-bottom">' +
                ...
                fmtBadge(r.status) +
                '</div>' +
                '<small class="text-muted">' + (r.nama_mahasiswa || '') + '</small>' +
                '</li>';
        });
        html += '</ul>';
        el.innerHTML = html;
    ...
    async function muatDashboard<%=rnd%>() {
        var sqlTotal = "SELECT COUNT(*) as total, SUM(CASE WHEN p.status='Disetujui' THEN 1 ELSE 0 END) as disetujui, SUM(CASE WHEN p.status='Sedang diproses' THEN 1 ELSE 0 END) as proses FROM public.penghargaan_mahasiswa p " + baseWhere;
        var sqlStatus = "SELECT p.status, COUNT(*) as jml FROM public.penghargaan_mahasiswa p " + baseWhere + " GROUP BY p.status ORDER BY jml DESC";
        var sqlTerbaru = "SELECT p.nama, p.status, m.nama as nama_mahasiswa FROM public.penghargaan_mahasiswa p LEFT JOIN public.mahasiswa m ON p.mahasiswa = m.id " + baseWhere + " ORDER BY p.id DESC LIMIT 10";
