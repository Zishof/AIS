...
if (idDsnLogin != null) {
    baseWhere += " AND p.dosen = " + idDsnLogin;
...
<div id="wrapperDashboard<%=rndDash%>" class="animate__animated animate__fadeIn">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 pb-3 border-bottom border-light gap-3">
        <h5 class="fw-bold text-dark mb-0">
            <i class="fas fa-chart-pie text-primary me-2"></i><%=Common.getBahasaConfig("Statistik & Rekapitulasi Prestasi Dosen")%>
        </h5>
    ...
    <div class="row g-4 mb-4">
        ...
        <div class="col-md-4">
            <div class="card border-0 shadow-sm rounded-4 h-100">
                <div class="card-header bg-white border-0 pt-4 pb-2 px-4">
                    <h6 class="fw-bold text-dark mb-0"><i class="fas fa-chart-pie text-success me-2"></i><%=Common.getBahasaConfig("Kategori Prestasi")%></h6>
                </div>
    ...
    <div class="row g-4 mb-4">
        ...
        <div class="col-md-6">
            <div class="card border-0 shadow-sm rounded-4 h-100 border-top border-success border-3">
                <div class="card-header bg-white border-0 pt-4 pb-2 px-4 d-flex justify-content-between align-items-center">
                    <h6 class="fw-bold text-dark mb-0"><i class="fas fa-map-marker-alt text-success me-2"></i><%=Common.getBahasaConfig("Rekap Lokasi Prestasi")%></h6>
                    <button class="btn btn-sm btn-light fw-bold rounded-pill shadow-sm border" onclick="downloadExcel<%=rndDash%>('tblLokasi<%=rndDash%>', 'Rekap_Lokasi_Prestasi')">
                        <i class="fas fa-file-excel text-success me-1"></i> Excel
                    </button>
                </div>
                <div class="card-body p-4 pt-2">
                    <div class="table-responsive" style="max-height: 250px; overflow-y: auto;">
                        <table class="table table-hover table-striped align-middle border" id="tblLokasi<%=rndDash%>">
                            <thead class="table-dark text-center" style="position: sticky; top: 0; z-index: 1;">
                                <tr>
                                    <th><%=Common.getBahasaConfig("Lokasi Kegiatan")%></th>
                                    <th><%=Common.getBahasaConfig("Total Diajukan")%></th>
                                    <th><%=Common.getBahasaConfig("Disetujui")%></th>
                                </tr>
                            </thead>
                            <tbody id="bodyLokasi<%=rndDash%>">
    ...
    <div class="row g-4 mb-4">
        ...
        <div class="col-md-12">
            <div class="card border-0 shadow-sm rounded-4 h-100 border-top border-dark border-3">
                <div class="card-header bg-white border-0 pt-4 pb-2 px-4 d-flex justify-content-between align-items-center">
                    <h6 class="fw-bold text-dark mb-0"><i class="fas fa-user-tie text-dark me-2"></i><%=Common.getBahasaConfig("Rekap Dosen Berprestasi")%></h6>
                    <button class="btn btn-sm btn-light fw-bold rounded-pill shadow-sm border" onclick="downloadExcel<%=rndDash%>('tblDosen<%=rndDash%>', 'Rekap_Dosen_Berprestasi')">
                        <i class="fas fa-file-excel text-success me-1"></i> Excel
                    </button>
                </div>
                <div class="card-body p-4 pt-2">
    ...
    const loadDashboard<%=rndDash%> = async () => {
        ...
        try {
            ...
            const sqlSummary = "SELECT " +
                               ...
                               " FROM public.prestasi_dosen p " + baseWhere;
            ...
            // 5. Load Tabel Rekap Lokasi Prestasi (Luar Kampus vs Dalam Kampus)
            const sqlLokasi = "SELECT (CASE WHEN p.prestasiluarkampus = true THEN 'Luar Kampus' ELSE 'Dalam Kampus' END) as lokasi, count(p.id) as total, " +
                              " sum(case when p.status='Disetujui' then 1 else 0 end) as disetujui " +
                              " FROM public.prestasi_dosen p " +
                              baseWhere +
                              " GROUP BY p.prestasiluarkampus ORDER BY total DESC";
            const resLokasi = await fetchSqlDashboard<%=rndDash%>(sqlLokasi);
            renderTableRekapHtml<%=rndDash%>(resLokasi, 'lokasi', 'bodyLokasi<%=rndDash%>');

            // 6. Load Tabel Rekap Cabang
            const sqlCbg = "SELECT c.nama as cabang, count(p.id) as total, " +
                           " sum(case when p.status='Disetujui' then 1 else 0 end) as disetujui " +
                           " FROM public.prestasi_dosen p " +
                           " JOIN public.cabang_prestasi_dosen c ON p.cabang_prestasi_dosen = c.id " +
                           baseWhere +
            ...
            const sqlKategori = "SELECT k.nama as kategori, count(p.id) as total, " +
                                " sum(case when p.status='Disetujui' then 1 else 0 end) as disetujui " +
                                " FROM public.prestasi_dosen p " +
                                " JOIN public.kategori_prestasi_dosen k ON p.kategori_prestasi_dosen = k.id " +
                                baseWhere +
            ...
            // 8. Load Tabel Rekap Struktural (Fakultas, Prodi, Dosen) Jika bukan Dosen
            <% if (idDsnLogin == null) { %>
            ...
            const sqlDosen = "SELECT d.nama as nama_dosen, d.nidn as nidn, count(p.id) as total, " +
                             " sum(case when p.status='Disetujui' then 1 else 0 end) as disetujui " +
                             " FROM public.prestasi_dosen p " +
                             " JOIN public.dosen d ON p.dosen = d.id " +
                             baseWhere +
