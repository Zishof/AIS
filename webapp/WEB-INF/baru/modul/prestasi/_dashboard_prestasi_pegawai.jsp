...
// Mapping hak akses pada Tbmuser untuk environment Pegawai
...
// Jika sistem mendukung role admin Fakultas / Jurusan, ID dapat di-set di sini
Long idFakultasLogin = null;
Long idJurusanLogin = null;
...
if (idPegawaiLogin != null) {
    baseWhere += " AND p.pegawai = " + idPegawaiLogin;
} else if (idJurusanLogin != null) {
    baseWhere += " AND peg.tendik_jurusan = " + idJurusanLogin;
} else if (idFakultasLogin != null) {
    baseWhere += " AND peg.tendik_fakultas = " + idFakultasLogin;
}
%>
...
<div id="wrapperDashboard<%=rndDash%>" class="animate__animated animate__fadeIn">
	...
	<div class="d-flex flex-wrap justify-content-between align-items-center mb-4 pb-3 border-bottom border-light gap-3">
		<h5 class="fw-bold text-dark mb-0">
			<i class="fas fa-chart-pie text-primary me-2"></i><%=Common.getBahasaConfig("Statistik & Rekapitulasi Prestasi Pegawai")%>
		</h5>
			...
			<div class="card border-0 shadow-sm rounded-4 h-100">
				<div class="card-header bg-white border-0 pt-4 pb-2 px-4">
					<h6 class="fw-bold text-dark mb-0">
						<i class="fas fa-chart-line text-primary me-2"></i><%=Common.getBahasaConfig("Tren Prestasi (Disetujui) TA & Semester")%></h6>
				</div>
		...
		<div class="col-md-6">
			<div class="card border-0 shadow-sm rounded-4 h-100 border-top border-primary border-3">
				<div class="card-header bg-white border-0 pt-4 pb-2 px-4 d-flex justify-content-between align-items-center">
					<h6 class="fw-bold text-dark mb-0">
						<i class="fas fa-calendar-alt text-primary me-2"></i><%=Common.getBahasaConfig("Rekap Per TA & Semester")%></h6>
					...
					<div class="table-responsive" style="max-height: 250px; overflow-y: auto;">
						<table class="table table-hover table-striped align-middle border" id="tblTaSmt<%=rndDash%>">
							...
							<tbody id="bodyTaSmt<%=rndDash%>">
								<tr>
									<td colspan="3" class="text-center"><div class="spinner-border spinner-border-sm text-primary"></div></td>
								</tr>
							</tbody>
						</table>
					</div>
					...
					<div class="table-responsive" style="max-height: 250px; overflow-y: auto;">
						<table class="table table-hover table-striped align-middle border" id="tblCabang<%=rndDash%>">
							...
							<tbody id="bodyCabang<%=rndDash%>">
								<tr>
									<td colspan="3" class="text-center"><div class="spinner-border spinner-border-sm text-warning"></div></td>
								</tr>
							</tbody>
						</table>
					</div>
					...
					<div class="table-responsive" style="max-height: 250px; overflow-y: auto;">
						<table class="table table-hover table-striped align-middle border" id="tblKategoriRekap<%=rndDash%>">
							...
							<tbody id="bodyKategoriRekap<%=rndDash%>">
								<tr>
									<td colspan="3" class="text-center"><div class="spinner-border spinner-border-sm text-info"></div></td>
								</tr>
							</tbody>
						</table>
					</div>
	...
	<div class="row g-4 mb-4">
		...
		<div class="col-md-6">
			<div class="card border-0 shadow-sm rounded-4 h-100 border-top border-secondary border-3">
				<div class="card-header bg-white border-0 pt-4 pb-2 px-4 d-flex justify-content-between align-items-center">
					<h6 class="fw-bold text-dark mb-0">
						<i class="fas fa-building text-secondary me-2"></i><%=Common.getBahasaConfig("Rekap Per Fakultas")%></h6>
					...
					<div class="table-responsive" style="max-height: 300px; overflow-y: auto;">
						<table class="table table-hover table-striped align-middle border" id="tblFakultas<%=rndDash%>">
							...
							<tbody id="bodyFakultas<%=rndDash%>">
								<tr>
									<td colspan="3" class="text-center"><div class="spinner-border spinner-border-sm text-secondary"></div></td>
								</tr>
							</tbody>
						</table>
					</div>
		...
		<div class="col-md-6">
			<div class="card border-0 shadow-sm rounded-4 h-100 border-top border-danger border-3">
				<div class="card-header bg-white border-0 pt-4 pb-2 px-4 d-flex justify-content-between align-items-center">
					<h6 class="fw-bold text-dark mb-0">
						<i class="fas fa-briefcase text-danger me-2"></i><%=Common.getBahasaConfig("Rekap Per Jurusan")%></h6>
					<button class="btn btn-sm btn-light fw-bold rounded-pill shadow-sm border" onclick="downloadExcel<%=rndDash%>('tblJurusan<%=rndDash%>', 'Rekap_Jurusan')">
						<i class="fas fa-file-excel text-success me-1"></i> Excel
					</button>
				</div>
				<div class="card-body p-4 pt-2">
					<div class="table-responsive" style="max-height: 300px; overflow-y: auto;">
						<table class="table table-hover table-striped align-middle border" id="tblJurusan<%=rndDash%>">
							<thead class="table-dark text-center" style="position: sticky; top: 0; z-index: 1;">
								<tr>
									<th><%=Common.getBahasaConfig("Jurusan")%></th>
									<th><%=Common.getBahasaConfig("Total")%></th>
									<th><%=Common.getBahasaConfig("Disetujui")%></th>
								</tr>
							</thead>
							<tbody id="bodyJurusan<%=rndDash%>">
								<tr>
									<td colspan="3" class="text-center"><div class="spinner-border spinner-border-sm text-danger"></div></td>
								</tr>
							</tbody>
						</table>
					</div>
		...
		<div class="col-md-12">
			<div class="card border-0 shadow-sm rounded-4 h-100 border-top border-dark border-3">
				<div class="card-header bg-white border-0 pt-4 pb-2 px-4 d-flex justify-content-between align-items-center">
					<h6 class="fw-bold text-dark mb-0"><i class="fas fa-medal text-dark me-2"></i><%=Common.getBahasaConfig("Rekap Prestasi Per Pegawai Utama")%></h6>
					<button class="btn btn-sm btn-light fw-bold rounded-pill shadow-sm border" onclick="downloadExcel<%=rndDash%>('tblPegawaiUtama<%=rndDash%>', 'Rekap_Pegawai_Utama')">
						<i class="fas fa-file-excel text-success me-1"></i> Excel
					</button>
				</div>
				<div class="card-body p-4 pt-2">
					<div class="table-responsive" style="max-height: 350px; overflow-y: auto;">
						<table class="table table-hover table-striped align-middle border" id="tblPegawaiUtama<%=rndDash%>">
							<thead class="table-dark text-center" style="position: sticky; top: 0; z-index: 1;">
								<tr>
									<th class="text-start"><%=Common.getBahasaConfig("Nama Pegawai")%></th>
									<th><%=Common.getBahasaConfig("NIP / Code")%></th>
									<th><%=Common.getBahasaConfig("Total Prestasi")%></th>
									<th><%=Common.getBahasaConfig("Prestasi Disetujui")%></th>
								</tr>
							</thead>
							<tbody id="bodyPegawaiUtama<%=rndDash%>">
								<tr>
									<td colspan="4" class="text-center"><div class="spinner-border spinner-border-sm text-dark"></div></td>
								</tr>
							</tbody>
						</table>
					</div>
    ...
    const loadDashboard<%=rndDash%> = async () => {
        ...
        try {
            ...
            const sqlSummary = "SELECT " +
                               ...
                               " SUM(CASE WHEN p.status = 'Sedang diproses' THEN 1 ELSE 0 END) as proses, " +
                               " SUM(CASE WHEN p.status = 'Belum diproses' THEN 1 ELSE 0 END) as belum " +
                               " FROM public.prestasi_pegawai p JOIN public.pegawai peg ON p.pegawai = peg.id " + baseWhere;
            ...
            const sqlTrend = "SELECT CONCAT(p.tahunakademik, ' - ', p.jenissemester) as label, count(p.id) as value " +
                             " FROM public.prestasi_pegawai p JOIN public.pegawai peg ON p.pegawai = peg.id " + 
                             baseWhere + " AND p.tahunakademik IS NOT NULL AND p.status = 'Disetujui' " +
                             " GROUP BY p.tahunakademik, p.jenissemester ORDER BY p.tahunakademik ASC, p.jenissemester ASC";
            ...
            const sqlTaSmt = "SELECT CONCAT(p.tahunakademik, ' - ', p.jenissemester) as tasmt, count(p.id) as total, " +
                             " sum(case when p.status='Disetujui' then 1 else 0 end) as disetujui " +
                             " FROM public.prestasi_pegawai p JOIN public.pegawai peg ON p.pegawai = peg.id " +
                             baseWhere + " AND p.tahunakademik IS NOT NULL " +
                             " GROUP BY p.tahunakademik, p.jenissemester ORDER BY p.tahunakademik DESC, p.jenissemester DESC";
            ...
            const sqlCbg = "SELECT c.nama as cabang, count(p.id) as total, " +
                           " sum(case when p.status='Disetujui' then 1 else 0 end) as disetujui " +
                           " FROM public.prestasi_pegawai p JOIN public.pegawai peg ON p.pegawai = peg.id " +
                           " JOIN public.cabang_prestasi_pegawai c ON p.cabang_prestasi_pegawai = c.id " +
                           baseWhere +
            ...
            const sqlKategori = "SELECT k.nama as kategori, count(p.id) as total, " +
                                " sum(case when p.status='Disetujui' then 1 else 0 end) as disetujui " +
                                " FROM public.prestasi_pegawai p JOIN public.pegawai peg ON p.pegawai = peg.id " +
                                " JOIN public.kategori_prestasi_pegawai k ON p.kategori_prestasi_pegawai = k.id " +
                                baseWhere +
            ...
            // 7. Load Tabel Rekap Struktural (Fakultas & Jurusan) Jika bukan Pegawai Biasa
            <%if (idPegawaiLogin == null) {%>
            const sqlFakultas = "SELECT f.nama as fakultas, count(p.id) as total, " +
                               " sum(case when p.status='Disetujui' then 1 else 0 end) as disetujui " +
                               " FROM public.prestasi_pegawai p JOIN public.pegawai peg ON p.pegawai = peg.id " +
                               " JOIN public.fakultas f ON peg.tendik_fakultas = f.id " +
                               baseWhere +
                               " GROUP BY f.nama ORDER BY total DESC";
            
            const resFakultas = await fetchSqlDashboard<%=rndDash%>(sqlFakultas);
            renderTableRekapHtml<%=rndDash%>(resFakultas, 'fakultas', 'bodyFakultas<%=rndDash%>');

            const sqlJurusan = "SELECT j.nama as jurusan, count(p.id) as total, " +
                               " sum(case when p.status='Disetujui' then 1 else 0 end) as disetujui " +
                               " FROM public.prestasi_pegawai p JOIN public.pegawai peg ON p.pegawai = peg.id " +
                               " JOIN public.jurusan j ON peg.tendik_jurusan = j.id " +
                               baseWhere +
                               " GROUP BY j.nama ORDER BY total DESC";

            const resJurusan = await fetchSqlDashboard<%=rndDash%>(sqlJurusan);
            renderTableRekapHtml<%=rndDash%>(resJurusan, 'jurusan', 'bodyJurusan<%=rndDash%>');
            <%}%>

        	 // 8. Load Tabel Rekap Per Pegawai Utama
            const sqlPegawaiUtama = "SELECT peg.nama as nama_pegawai, peg.code as code, count(p.id) as total, " +
                                      " sum(case when p.status='Disetujui' then 1 else 0 end) as disetujui " +
                                      " FROM public.prestasi_pegawai p JOIN public.pegawai peg ON p.pegawai = peg.id " +
                                      baseWhere +
                                      " GROUP BY peg.id, peg.nama, peg.code ORDER BY total DESC, peg.nama ASC";
            
            const resPegawaiUtama = await fetchSqlDashboard<%=rndDash%>(sqlPegawaiUtama);
            let htmlPegawai = '';
            if(resPegawaiUtama.length === 0) {
                htmlPegawai = '<tr><td colspan="4" class="text-center text-muted fw-medium"><i class="fas fa-folder-open me-2 opacity-50"></i><%=Common.getBahasaConfig("Tidak ada data prestasi")%></td></tr>';
            } else {
                resPegawaiUtama.forEach(row => {
                    htmlPegawai += '<tr>' +
                            '  <td class="fw-bold text-start text-dark">' + (row.nama_pegawai || '-') + '</td>' +
            ...
            document.getElementById('bodyPegawaiUtama<%=rndDash%>').innerHTML = htmlPegawai;
