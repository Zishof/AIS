<%@page import="ais.database.model.KategoriPrestasiDosen"%>
<%@page import="ais.database.model.CabangPrestasiDosen"%>
<%@page import="ais.database.model.Perkuliahan"%>
<%@page import="java.net.URLEncoder"%>
<%@page import="ais.database.model.PrestasiDosen"%>
...
// Jika login sebagai admin Fakultas, hanya boleh lihat prestasi di Fakultas-nya sendiri
Fakultas loginSebagaiFakultas = tbmuser.getFakultas();
// Jika login sebagai admin Jurusan, hanya boleh lihat prestasi di Jurusan-nya sendiri
Jurusan loginSebagaiJurusan = tbmuser.getJurusan();
// Jika login sebagai dosen, hanya boleh lihat prestasinya sendiri
Dosen loginSebagaiDosen = tbmuser.getDosen();
Long idDsnLogin = (loginSebagaiDosen != null) ? loginSebagaiDosen.getId() : null;
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-2">
                        <div>
                            <h5 class="fw-bold text-dark mb-1">
                                <i class="fas fa-award text-primary me-2"></i><%=Common.getBahasaConfig("Manajemen Prestasi Dosen")%>
                            </h5>
                            <small class="text-muted"><%=Common.getBahasaConfig("Kelola data pencapaian dan prestasi dosen.")%></small>
                        </div>
                        
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            <div class="input-group input-group-sm shadow-sm" style="min-width:180px;flex:1 1 200px;">
                                <span class="input-group-text bg-white border-end-0"><i class="fas fa-search text-muted"></i></span>
                                <input type="text" class="form-control border-start-0 ps-0 bg-white fw-medium" id="searchPrestasi<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Cari Prestasi, NIDN / Nama...")%>">
                            </div>
                        ...
                        <table class="table table-hover table-striped align-middle mb-0" id="tabelHTMLDataPrestasi<%=rnd%>">
                            <thead class="table-dark text-center align-middle">
                                <tr>
                                    ...
                                    <th class="text-start fw-semibold text-uppercase small px-3"><i class="fas fa-chalkboard-teacher me-1"></i><%=Common.getBahasaConfig("Dosen")%></th>
                ...
                <div class="card-header bg-white border-0 pt-4 pb-0 px-4">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="fw-bold text-dark mb-0" id="formTitle<%=rnd%>">
                            <i class="fas fa-edit text-success me-2"></i><%=Common.getBahasaConfig("Formulir Prestasi Dosen")%>
                        </h5>
                    </div>
                </div>
                <div class="card-body p-4">
                    <form id="formPrestasi<%=rnd%>" onsubmit="event.preventDefault(); simpanPrestasi<%=rnd%>();">
                        ...
                        <div class="row g-4 mt-1" id="formContainer<%=rnd%>">
                            <div class="col-md-12">
                                <h6 class="fw-bold text-primary border-bottom pb-2 mb-0"><i class="fas fa-chalkboard-teacher me-2"></i>1. Data Dosen & Akademik</h6>
                            </div>
                            
                            <% if (idDsnLogin == null) { %>
                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Cari Dosen (NIDN / NUPTK / Nama)")%> <span class="text-danger">*</span></label>
                                <input type="text" class="form-control fw-bold shadow-sm field-to-disable<%=rnd%>" id="inputCariDosen<%=rnd%>" list="dlDosen<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Ketik NIDN/NUPTK atau Nama untuk mencari...")%>" oninput="onInputDosenCari<%=rnd%>(this.value)" onchange="checkSelectedDosenCari<%=rnd%>(this.value)" autocomplete="off" required>
                            ...
                            <div class="col-md-3">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Fakultas (Opsional)")%></label>
                            ...
                            <div class="col-md-3">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Jurusan (Opsional)")%></label>
                            ...
                            <% } else { %>
                                <input type="hidden" id="hiddenIdDosen<%=rnd%>" value="<%=idDsnLogin%>">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control fw-bold shadow-sm field-to-disable<%=rnd%>" id="inputNama<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Contoh: Dosen Berprestasi Tingkat Nasional")%>" required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Nama Prestasi (Bahasa Inggris)")%></label>
                                <input type="text" class="form-control shadow-sm field-to-disable<%=rnd%>" id="inputNamaEn<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Achievement Name in English")%>">
                            </div>
                            ...
                            <div class="col-md-4">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Tempat Pelaksanaan")%></label>
                                <input type="text" class="form-control shadow-sm field-to-disable<%=rnd%>" id="inputTempat<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Kota / Negara")%>">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Prestasi Luar Kampus?")%></label>
                                <select class="form-select shadow-sm field-to-disable<%=rnd%>" id="inputLuarKampus<%=rnd%>">
                                    <option value="true"><%=Common.getBahasaConfig("Ya (Luar Kampus)")%></option>
                                    <option value="false"><%=Common.getBahasaConfig("Tidak (Dalam Kampus)")%></option>
                                </select>
                            </div>
                            
                            <div class="col-md-3">
                                <label class="form-label small fw-bold text-secondary"><%=Common.getBahasaConfig("Juara (Cth: Harapan 1)")%></label>
                                <input type="text" class="form-control shadow-sm field-to-disable<%=rnd%>" id="inputJuara<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Contoh: Juara I")%>">
                            </div>
                            <div class="col-md-3">
...
<script>
    ...
    // ==========================================
    const masterModelClass<%=rnd%> = "<%=PrestasiDosen.class.getName()%>";
    ...
    // Fungsi untuk Disable/Enable Form berdasarkan status
    const disableFormIfApproved<%=rnd%> = (status) => {
        ...
        // Jika user adalah Dosen, tombol Simpan juga didisable
        <% if (idDsnLogin != null) { %>
    ...
    const downloadExcelPrestasi<%=rnd%> = () => {
        ...
        XLSX.writeFile(wb, "Data_Prestasi_Dosen_<%=System.currentTimeMillis()%>.xlsx");
    };

    // --- 2. Fungsi Cek File Existing ---
    async function checkExistingFile<%=rnd%>(idPrestasi, isApproved) {
        var reqObj = { 
            ...
            "jenis": "<%=PrestasiDosen.class.getName()%>",
            "kondisiTambahan": "",
            "refresh": "false",
            "usingId": false
        };
    ...
    const loadFakultas<%=rnd%> = async (targetId, selectedId) => {
        ...
        try {
            ...
            if (data.length === 1) {
                ...
                options = '<option value="">-- ' + (targetId.includes('filter') ? 'Semua Fakultas' : 'Pilih Fakultas') + ' --</option>';
                data.forEach((row) => {
    ...
    const loadFilterJurusan<%=rnd%> = async (idFakultas) => {
        ...
        try {
            ...
            if (data.length === 1) {
                ...
                options = '<option value="">-- Semua Jurusan --</option>';
                data.forEach((row) => {
    ...
    const loadCabang<%=rnd%> = async (targetId, selectedId) => {
        var reqObj = { 
            "action": "daftar", 
            "class": "<%=CabangPrestasiDosen.class.getName()%>", 
            "order1": "asc", 
            "sort1": "nama" 
        };
        try {
    ...
    const loadKategori<%=rnd%> = async (targetId, selectedId) => {
        var reqObj = { 
            "action": "daftar", 
            "class": "<%=KategoriPrestasiDosen.class.getName()%>", 
            "order1": "asc", 
            "sort1": "nama" 
        };
        try {
    ...
    <% if (idDsnLogin == null) { %>
    const onInputDosenCari<%=rnd%> = async (val) => {
        ...
        let extSql = "aktif = true AND (nidn ILIKE '%" + val + "%' OR nuptk ILIKE '%" + val + "%' OR nama ILIKE '%" + val + "%')";
        if (valJur) {
        ...
        try {
            ...
            let options = '';
            data.forEach((row) => {
                let idn = row.nidn ? row.nidn : (row.nuptk ? row.nuptk : '-');
                options += '<option data-id="' + row.id + '" value="' + idn + ' - ' + row.nama + '"></option>';
            });
            document.getElementById('dlDosen<%=rnd%>').innerHTML = options;
        } catch (e) { console.error("Error search Dosen", e); }
    };

    const checkSelectedDosenCari<%=rnd%> = (val) => {
    ...
    const buildFilterSQL<%=rnd%> = () => {
        ...
        if (keyword !== '') {
            filters.push("(this_.nama ILIKE '%" + keyword + "%' OR dosen1_.nidn ILIKE '%" + keyword + "%' OR dosen1_.nama ILIKE '%" + keyword + "%')");
        }
        ...
        if(cbg) filters.push("this_.cabangPrestasiDosen IN (" + cbg + ")");
        
        const kat = getMultiSelectValues<%=rnd%>('filterKategori<%=rnd%>');
        if(kat) filters.push("this_.kategoriPrestasiDosen IN (" + kat + ")");
    ...
    const loadDataPrestasi<%=rnd%> = async () => {
        ...
        var reqObj = {
            "action": "daftar", 
            "class": "<%=PrestasiDosen.class.getName()%>",
            "deep": "1", 
            ...
            if (recordList.length === 0) {
                ...
                const filePromises = recordList.map(async (row) => {
                    ...
                    try {
                        ...
                        console.error("Gagal menarik file row ID:", row.id, e);
                    }
                    return row;
                });
                ...
                recordList.forEach((row) => {
                    ...
                    const dsnNidn = row["dosen.nidn"] || '-';
                    const dsnNama = row["dosen.nama"] || '-';
                  
                    const kategoriNama = row["kategoriPrestasiDosen.nama"] || '-';
                    const tgl = row["tanggal.formated2"] || row["tanggal.datetime_format"] || row["tanggal"] || '-';
                    const statusVal = row.status || 'Belum diproses';
                    
                    const infoDsn = '<div class="fw-bold text-dark fs-6">' + dsnNama + '</div>' + 
                                    '<small class="text-muted"><i class="fas fa-id-card me-1"></i>NIDN/NUPTK: ' + dsnNidn + '</small>';
                    ...
                    htmlList += '<tr>' +
                            ...
                            '<td class="text-start">' + infoDsn + '</td>' +
    ...
    const bukaFormTambah<%=rnd%> = () => {
        ...
        <% if (idDsnLogin == null) { %>
            document.getElementById('inputCariDosen<%=rnd%>').value = '';
            document.getElementById('hiddenIdDosen<%=rnd%>').value = '';
    ...
    const simpanPrestasi<%=rnd%> = async () => {
        ...
        const dsnVal = document.getElementById('hiddenIdDosen<%=rnd%>').value;
        if(!dsnVal) {
            showToast<%=rnd%>('<%=Common.getBahasaConfigJS("Harap isi dan pilih data Dosen dengan benar.")%>', 'bg-warning text-dark');
            return;
        }
        ...
        const dataObj = {
            ...
            prestasiLuarKampus: document.getElementById('inputLuarKampus<%=rnd%>').value === 'true'
        };
        ...
        dataObj.dosen = "null";
        dataObj.fakultas = "null";
        dataObj.jurusan = "null";
        dataObj.cabangPrestasiDosen = "null";
        dataObj.kategoriPrestasiDosen = "null";
        
        dataObj.dosen = parseInt(dsnVal);
        
        <% if (idDsnLogin == null) { %>
            const fakVal = document.getElementById('selectFakultas<%=rnd%>').value;
            dataObj.fakultas = fakVal ? parseInt(fakVal) : null;
            
            const jurVal = document.getElementById('selectJurusan<%=rnd%>').value;
            dataObj.jurusan = jurVal ? parseInt(jurVal) : null;
        <% } %>
        
        const cbgVal = document.getElementById('selectCabang<%=rnd%>').value;
        dataObj.cabangPrestasiDosen = cbgVal ? parseInt(cbgVal) : null;
        
        const katVal = document.getElementById('selectKategori<%=rnd%>').value;
        dataObj.kategoriPrestasiDosen = katVal ? parseInt(katVal) : null;
    ...
    const editPrestasi<%=rnd%> = async (idPrestasi) => {
        ...
        try {
            ...
            if (result.status === '00' || result.status === 'success' || result.id) {
                ...
                let isLuarKampus = data.prestasiLuarKampus;
                if(isLuarKampus === null || isLuarKampus === undefined) isLuarKampus = true;
                document.getElementById('inputLuarKampus<%=rnd%>').value = isLuarKampus.toString();
                ...
                if(data["dosen.id"]) {
                    ...
                    <% if (idDsnLogin == null) { %>
                    let dsnIdn = data["dosen.nidn"] ? data["dosen.nidn"] : (data["dosen.nuptk"] ? data["dosen.nuptk"] : '-');
                    document.getElementById('inputCariDosen<%=rnd%>').value = dsnIdn + ' - ' + (data["dosen.nama"] || '');
                    <% } %>
                }

                // Load & Set Combobox Masters Form
                let idFak = data["fakultas.id"] ? data["fakultas.id"] : null;
                let idJur = data["jurusan.id"] ? data["jurusan.id"] : null;
                
                <% if (idDsnLogin == null) { %>
                loadFakultas<%=rnd%>('selectFakultas<%=rnd%>', idFak);
                if(idFak) {
                    loadJurusan<%=rnd%>(idFak, idJur);
                } else {
                    loadJurusan<%=rnd%>(null, idJur);
                }
                <% } %>
                
                loadCabang<%=rnd%>('selectCabang<%=rnd%>', data["cabangPrestasiDosen.id"] ? data["cabangPrestasiDosen.id"] : null);
                loadKategori<%=rnd%>('selectKategori<%=rnd%>', data["kategoriPrestasiDosen.id"] ? data["kategoriPrestasiDosen.id"] : null);
