...
<style>
    ...
    .nav-lazy-tab .nav-link.active { transform: translateY(-3px); box-shadow: 0 0.5rem 1rem rgba(40,167,69,.2); border-color: transparent; }
</style>
    ...
    <ul class="nav nav-pills nav-lazy-tab justify-content-center mb-4 border-bottom pb-4" role="tablist">
        <li class="nav-item mx-2">
            <a class="nav-link fw-bold px-5 py-2 rounded-pill shadow-sm <%= "bayar".equals(activeTab) ? "active bg-success text-white" : "bg-white text-secondary border" %>" href="<%=baseUrl%>&tab=bayar">
                <i class="fas fa-user-plus me-2"></i><%= Common.getBahasaConfig("Pendaftaran Calon Mahasiswa") %>
            </a>
        </li>
        <li class="nav-item mx-2">
            <a class="nav-link fw-bold px-5 py-2 rounded-pill shadow-sm <%= "riwayat".equals(activeTab) ? "active bg-success text-white" : "bg-white text-secondary border" %>" href="<%=baseUrl%>&tab=riwayat">
                <i class="fas fa-clipboard-check me-2"></i><%= Common.getBahasaConfig("Riwayat Pendaftaran") %>
            </a>
        </li>
    </ul>
