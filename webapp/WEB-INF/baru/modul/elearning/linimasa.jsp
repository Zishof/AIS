...
<%
    ...
    String faIcon = "fas fa-stream";
    String judulRaw = "Linimasa Pertemuan";
    ...
    try {
        ...
        e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit webapp/WEB-INF/baru/modul/elearning/linimasa.jsp:45");
    } finally {
        try { sessFilter.disconnect(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/linimasa.jsp:47");}
...
<style>
    ...
    /* Filter tab pills: scrollable on mobile */
    @media (max-width: 767.98px) {
        ...
        #pills-tab-<%=rnd%> .nav-link { white-space: nowrap; font-size: 0.78rem; padding: 0.4rem 0.65rem; margin-bottom: 0; }
        /* Action buttons row: stack on mobile */
        #wrapper-linimasa-<%=rnd%> .row.align-items-center.mb-4 .col-xl-6:last-child { margin-top: 0.5rem; }
        #wrapper-linimasa-<%=rnd%> .col-xl-6:last-child .btn-group { width: 100% !important; }
        #wrapper-linimasa-<%=rnd%> .col-xl-6:last-child .btn-group .btn { flex: 1 1 0; font-size: 0.78rem; padding: 0.4rem !important; }
        /* Source-filter checkboxes: 2-column grid on mobile */
        #pane-sumber-<%=rnd%> .col-md-3 { width: 50% !important; flex: 0 0 50%; max-width: 50%; }
    }
</style>

<div id="wrapper-linimasa-<%=rnd%>" class="container py-4">
        ...
        <div class="card border-0 shadow-sm rounded-lg" style="background-color: #ffffff;">
            <div class="card-body p-4">
                ...
                <ul class="nav nav-pills nav-pills-custom-<%=rnd%> mb-4 border-bottom pb-3" id="pills-tab-<%=rnd%>" role="tablist">
                    ...
                    <li class="nav-item"><a class="nav-link" onclick="gantiTab<%=rnd%>(event, 'pane-materi-<%=rnd%>')"><i class="fas fa-book-open mr-1"></i> <%=Common.getBahasaConfig("Substansi")%></a></li>
                ...
                <form id="form-filter-<%=rnd%>" onsubmit="event.preventDefault(); terapkanFilter<%=rnd%>();">
                    <div class="tab-content" id="pills-tabContent-<%=rnd%>">
                        <div class="tab-pane fade show active" id="pane-akademik-<%=rnd%>" role="tabpanel">
                            <div class="row align-items-end">
                                <div class="col-md-3 mb-3">
                                    ...
                                    <select class="form-control input-elegan-<%=rnd%>" id="filter-ta-<%=rnd%>">
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <select class="form-control input-elegan-<%=rnd%>" id="filter-smt-<%=rnd%>">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" id="filter-matakuliah-<%=rnd%>" class="form-control input-elegan-<%=rnd%>">
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Kelas Perkuliahan")%></label>
                                    <input type="text" id="filter-kelas-kuliah-<%=rnd%>" class="form-control input-elegan-<%=rnd%>">
                                </div>
                                <% if (isSekolahAtauYayasan) { %>
                                <div class="col-md-6 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Matapelajaran")%></label><input type="text" id="filter-matapelajaran-<%=rnd%>" class="form-control input-elegan-<%=rnd%>"></div>
                                <div class="col-md-6 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Kelas Siswa")%></label><input type="text" id="filter-kelas-siswa-<%=rnd%>" class="form-control input-elegan-<%=rnd%>"></div>
                                <% } %>
                            </div>
                        </div>
                        ...
                        <div class="tab-pane fade" id="pane-personal-<%=rnd%>" role="tabpanel">
                            <div class="row align-items-end">
                                <div class="col-md-6 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Dosen")%></label><input type="text" id="filter-dosen-<%=rnd%>" class="form-control input-elegan-<%=rnd%>"></div>
                                <div class="col-md-6 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Mahasiswa")%></label><input type="text" id="filter-mahasiswa-<%=rnd%>" class="form-control input-elegan-<%=rnd%>"></div>
                                <% if (isSekolahAtauYayasan) { %>
                                <div class="col-md-6 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Guru")%></label><input type="text" id="filter-guru-<%=rnd%>" class="form-control input-elegan-<%=rnd%>"></div>
                                <div class="col-md-6 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Siswa")%></label><input type="text" id="filter-siswa-<%=rnd%>" class="form-control input-elegan-<%=rnd%>"></div>
                                <% } %>
                            </div>
                        </div>
                        ...
                        <div class="tab-pane fade" id="pane-materi-<%=rnd%>" role="tabpanel">
                            <div class="row align-items-end">
                                <div class="col-md-6 col-lg-6 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Topik Pembahasan")%></label><input type="text" id="filter-topik-<%=rnd%>" class="form-control input-elegan-<%=rnd%>"></div>
                                <div class="col-md-4 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Indikator")%></label><input type="text" id="filter-indikator-<%=rnd%>" class="form-control input-elegan-<%=rnd%>"></div>
                                <div class="col-md-4 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Metode")%></label><input type="text" id="filter-metode-<%=rnd%>" class="form-control input-elegan-<%=rnd%>"></div>
                                <div class="col-md-4 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Catatan")%></label><input type="text" id="filter-catatan-<%=rnd%>" class="form-control input-elegan-<%=rnd%>"></div>
                            </div>
                        </div>

                        <div class="tab-pane fade" id="pane-fasilitas-<%=rnd%>" role="tabpanel">
                            <div class="row align-items-end">
                                <div class="col-md-4 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Ruangan")%></label><input type="text" id="filter-ruang-<%=rnd%>" class="form-control input-elegan-<%=rnd%>"></div>
                                <div class="col-md-4 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Rujukan Utama")%></label><input type="text" id="filter-buku1-<%=rnd%>" class="form-control input-elegan-<%=rnd%>"></div>
                                <div class="col-md-4 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Rujukan Tambahan")%></label><input type="text" id="filter-buku2-<%=rnd%>" class="form-control input-elegan-<%=rnd%>"></div>
                                ...
                                <div class="col-md-5 mb-3">
                                    <label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Rentang Tanggal")%></label>
                                    <div class="input-group">
                                        <input type="date" id="filter-tgl-mulai-<%=rnd%>" class="form-control input-elegan-<%=rnd%> text-center">
                                        <div class="input-group-prepend input-group-append"><span class="input-group-text bg-white border-top-0 border-bottom-0 text-muted"><i class="fas fa-minus"></i></span></div>
                                        <input type="date" id="filter-tgl-selesai-<%=rnd%>" class="form-control input-elegan-<%=rnd%> text-center">
                                    </div>
                                </div>
                                <div class="col-md-4 mb-3">
                                    <label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Pertemuan Ke-")%></label>
                                    <div class="input-group">
                                        <input type="number" id="filter-ke-mulai-<%=rnd%>" class="form-control input-elegan-<%=rnd%> text-center" placeholder="Min">
                                        <div class="input-group-prepend input-group-append"><span class="input-group-text bg-white border-top-0 border-bottom-0 text-muted"><i class="fas fa-arrow-right"></i></span></div>
                                        <input type="number" id="filter-ke-selesai-<%=rnd%>" class="form-control input-elegan-<%=rnd%> text-center" placeholder="Max">
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <div id="paging-top-<%=rnd%>" class="d-flex justify-content-center mb-4">
        <button id="btn-load-next-<%=rnd%>" class="btn btn-primary btn-sm rounded-pill px-4 py-2 shadow-sm font-weight-bold" style="display: none;">
            ...
            <span id="label-btn-next-<%=rnd%>"><%=Common.getBahasaConfig("Muat Pertemuan Selanjutnya")%></span> 
    ...
    <div id="timeline-container-<%=rnd%>" class="timeline-container position-relative">
        <div id="loading-init-<%=rnd%>" class="text-center text-secondary py-5 fade-in-<%=rnd%>">
            ...
            <p class="mb-0 font-weight-bold"><%=Common.getBahasaConfig("Sedang memuat data pertemuan...")%></p>
        </div>
    ...
    <div id="paging-bottom-<%=rnd%>" class="d-flex justify-content-center mt-4 mb-5">
        <button id="btn-load-back-<%=rnd%>" class="btn btn-outline-secondary btn-sm rounded-pill px-4 py-2 shadow-sm font-weight-bold bg-white" style="display: none;">
            ...
            <span id="label-btn-back-<%=rnd%>"><%=Common.getBahasaConfig("Muat Pertemuan Sebelumnya")%></span> 
...
(function() {
    ...
    function jalankanScript<%=rnd%>(container) {
        container.querySelectorAll('script').forEach(function(oldScript) {
            var newScript = document.createElement('script');
            Array.from(oldScript.attributes).forEach(function(a) { if (a.name.toLowerCase() !== 'type') newScript.setAttribute(a.name, a.value); });
    ...
    async function muatLinimasa<%=rnd%>(arah, isFilterBaru) {
        if (isFilterBaru) {
            ...
            if(divContainer) {
                divContainer.innerHTML = '<div id="loading-init-<%=rnd%>" class="text-center text-secondary py-5 fade-in-<%=rnd%>"><i class="fas fa-circle-notch fa-spin fa-3x text-info mb-3"></i><p class="mb-0 font-weight-bold"><%=Common.getBahasaConfig("Sedang memuat data pertemuan...")%></p></div>' +
                                         '<div id="timeline-content-next-<%=rnd%>"></div>' +
                                         '<div id="timeline-content-prev-<%=rnd%>"></div>';
            }
        }
        ...
        try {
            ...
            var servletUrlApi<%=rnd%> = rootUrl<%=rnd%> + "/baru?hanya_tampil_jsp=true&p=elearning%2Fservices&s=_linimasa_service&mulai=" + (arah === 'next' ? startNext<%=rnd%> : startBack<%=rnd%>) + "&banyak=" + limit<%=rnd%> + parameterArah<%=rnd%> + queryFilter<%=rnd%>;
            ...
            var fetchPromises<%=rnd%> = arrayIdPermintaan<%=rnd%>.map(function(itemReq) {
                var urlHtml = rootUrl<%=rnd%> + "/baru?hanya_tampil_jsp=true&p=elearning&s=load_timeline_perkuliahan&item=" + encodeURIComponent(itemReq);
                return fetch(urlHtml).then(function(res) {
                    if (!res.ok) throw new Error('<%=Common.getBahasaConfigJS("Sistem gagal merender antarmuka.")%>');
                    return res.text();
                });
            });
            ...
            if (arah === 'next') {
                ...
                if (divNext) {
                    ...
                    jalankanScript<%=rnd%>(divNext.firstElementChild);
                ...
                if (divPrev) {
                    ...
                    jalankanScript<%=rnd%>(divPrev.lastElementChild);
            ...
            if (typeof tampilkanToast === "function") {
                tampilkanToast(error.message || "<%=Common.getBahasaConfig("Terjadi gangguan saat mengambil data riwayat pertemuan.")%>", 'bg-danger');
    ...
    muatLinimasa<%=rnd%>('next', true);
        ...
        window.generatePertemuanHTML = function(idPertemuan) {
            const targetDiv = document.getElementById('tampilan_pertemuan_' + idPertemuan);
            
            if (targetDiv) {
                // Simpan state HTML sebelum diload untuk fallback bila terjadi error
                const htmlAsli = targetDiv.innerHTML;
                
                // Tampilkan efek animasi loading agar pengguna tahu proses sedang berjalan
                targetDiv.innerHTML = '<div class="text-center p-5"><div class="spinner-border text-primary" role="status"></div><div class="mt-2 text-muted fw-bold"><%=Common.getBahasaConfig("Memuat ulang data pertemuan...")%></div></div>';
                
                // Panggil ulang URL pertemuan ini tanpa mereload page penuh
                const url = '<%=Common.ROOT%>/baru?hanya_tampil_jsp=true&p=elearning&s=pertemuan&pertemuan=' + idPertemuan + '&refresh=true';
                ...
                fetch(url)
                    ...
                    .catch(error => {
                        ...
                        targetDiv.innerHTML = htmlAsli; // Kembalikan ke tampilan awal
                        
                        if (typeof tampilkanToast === 'function') {
                            tampilkanToast('<%=Common.getBahasaConfigJS("Gagal memuat ulang data pertemuan. Silakan segarkan halaman.")%>', 'bg-danger');
                        } else {
                            alert('<%=Common.getBahasaConfigJS("Gagal memuat ulang data pertemuan.")%>');
                        }
                    });
            } else {
                console.warn('Elemen penampung untuk pertemuan ID ' + idPertemuan + ' tidak ditemukan. Halaman penuh akan dimuat ulang.');
                location.reload(); // Fallback jika div tidak ada
            }
        };
