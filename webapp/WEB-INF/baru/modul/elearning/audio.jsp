...
<%
    ...
    // Variabel Header Modul Khusus Audio
    String faIcon = "fas fa-headphones";
    String judulRaw = "Linimasa Audio Pembelajaran";
    ...
    try {
        ...
        e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit webapp/WEB-INF/baru/modul/elearning/audio.jsp:55");
    } finally {
        try { sessFilter.disconnect(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/audio.jsp:57");}
...
<div id="wrapper-linimasa-<%=rnd%>" class="container-fluid px-2 px-md-4 py-4">
        ...
        <div class="card border-0 shadow-sm rounded-lg" style="background-color: #ffffff;">
            <div class="card-body p-4">
                ...
                <ul class="nav nav-pills nav-pills-custom-<%=rnd%> mb-4 border-bottom pb-3" id="pills-tab-<%=rnd%>" role="tablist">
                    ...
                    <li class="nav-item"><a class="nav-link" onclick="gantiTab<%=rnd%>(event, 'pane-materi-<%=rnd%>')"><i class="fas fa-volume-up mr-1"></i> <%=Common.getBahasaConfig("Substansi Audio")%></a></li>
                        ...
                        <div class="tab-pane fade" id="pane-materi-<%=rnd%>" role="tabpanel">
                            <div class="row align-items-end">
                                <div class="col-md-12 col-lg-6 mb-3">
                                    <label class="label-filter-<%=rnd%> text-primary"><%=Common.getBahasaConfig("Nama Berkas / Judul Audio")%></label>
                                    <input type="text" id="filter-nama-audio-<%=rnd%>" class="form-control input-elegan-<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Ketikkan judul audio...")%>">
                                </div>
                                ...
                                <div class="col-md-12 col-lg-12 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Keterangan Tambahan")%></label><input type="text" id="filter-catatan-<%=rnd%>" class="form-control input-elegan-<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Catatan audio...")%>"></div>
                            </div>
                        </div>
                        ...
                        <div class="tab-pane fade" id="pane-fasilitas-<%=rnd%>" role="tabpanel">
                            <div class="row align-items-end">
                                ...
                                <div class="col-md-5 mb-3">
                                    <label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Rentang Tanggal Upload Audio")%></label>
    ...
    <div id="paging-top-<%=rnd%>" class="d-flex justify-content-center mb-4">
        <button id="btn-load-next-<%=rnd%>" class="btn btn-primary btn-sm rounded-pill px-4 py-2 shadow-sm font-weight-bold" style="display: none;">
            ...
            <span id="label-btn-next-<%=rnd%>"><%=Common.getBahasaConfig("Muat Audio Terbaru")%></span> 
    ...
    <div id="timeline-container-<%=rnd%>" class="timeline-container position-relative">
        <div id="loading-init-<%=rnd%>" class="text-center text-secondary py-5 fade-in-<%=rnd%>">
            ...
            <p class="mb-0 font-weight-bold"><%=Common.getBahasaConfig("Sedang memuat data audio...")%></p>
        </div>
    ...
    <div id="paging-bottom-<%=rnd%>" class="d-flex justify-content-center mt-4 mb-5">
        <button id="btn-load-back-<%=rnd%>" class="btn btn-outline-secondary btn-sm rounded-pill px-4 py-2 shadow-sm font-weight-bold bg-white" style="display: none;">
            ...
            <span id="label-btn-back-<%=rnd%>"><%=Common.getBahasaConfig("Muat Audio Sebelumnya")%></span> 
...
(function() {
    ...
    var bangunQueryFilter<%=rnd%> = function() {
        var q = "";
        // Param khusus Audio
        q += "&q_nama_audio=" + getFilterVal<%=rnd%>("filter-nama-audio-<%=rnd%>");
    ...
    async function muatLinimasa<%=rnd%>(arah, isFilterBaru) {
        if (isFilterBaru) {
            ...
            if(divContainer) {
                divContainer.innerHTML = '<div id="loading-init-<%=rnd%>" class="text-center text-secondary py-5 fade-in-<%=rnd%>"><i class="fas fa-circle-notch fa-spin fa-3x text-info mb-3"></i><p class="mb-0 font-weight-bold"><%=Common.getBahasaConfig("Sedang memuat data audio...")%></p></div>' +
                                         '<div id="timeline-content-next-<%=rnd%>"></div>' +
                                         '<div id="timeline-content-prev-<%=rnd%>"></div>';
            }
        }
        ...
        try {
            ...
            // Target Endpoint: _linimasa_audio
            var servletUrlApi<%=rnd%> = rootUrl<%=rnd%> + "/baru?hanya_tampil_jsp=true&p=elearning%2Fservices&s=_linimasa_audio&mulai=" + (arah === 'next' ? startNext<%=rnd%> : startBack<%=rnd%>) + "&banyak=" + limit<%=rnd%> + parameterArah<%=rnd%> + queryFilter<%=rnd%>;
            ...
            // Target Loader: load_audio
            var fetchPromises<%=rnd%> = arrayIdPermintaan<%=rnd%>.map(function(itemReq) {
                // Catatan: Pastikan file load_audio.jsp (berfungsi seperti load_materi.jsp) tersedia di server Anda
                var urlHtml = rootUrl<%=rnd%> + "/baru?hanya_tampil_jsp=true&p=elearning&s=load_audio&item=" + encodeURIComponent(itemReq);
                return fetch(urlHtml).then(function(res) {
                    if (!res.ok) throw new Error('<%=Common.getBahasaConfigJS("Gagal merender kartu audio.")%>');
                    return res.text();
                });
            });
