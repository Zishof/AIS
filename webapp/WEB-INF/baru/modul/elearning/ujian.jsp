...
<%
    ...
    String faIcon = "fas fa-file-signature";
    String judulRaw = "Linimasa Ujian";
    ...
    try {
        ...
        e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit webapp/WEB-INF/baru/modul/elearning/ujian.jsp:46");
    } finally {
        try { sessFilter.disconnect(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/ujian.jsp:48");}
...
<div id="wrapper-linimasa-<%=rnd%>" class="container-fluid px-2 px-md-4 py-4">
        ...
        <div class="card border-0 shadow-sm rounded-lg" style="background-color: #ffffff;">
            <div class="card-body p-4">
                ...
                <ul class="nav nav-pills nav-pills-custom-<%=rnd%> mb-4 border-bottom pb-3" id="pills-tab-<%=rnd%>" role="tablist">
                    ...
                    <li class="nav-item"><a class="nav-link" onclick="gantiTab<%=rnd%>(event, 'pane-materi-<%=rnd%>')"><i class="fas fa-book-open mr-1"></i> <%=Common.getBahasaConfig("Substansi Ujian")%></a></li>
                        ...
                        <div class="tab-pane fade" id="pane-materi-<%=rnd%>" role="tabpanel">
                            <div class="row align-items-end">
                                <div class="col-md-12 col-lg-6 mb-3">
                                    <label class="label-filter-<%=rnd%> text-primary"><%=Common.getBahasaConfig("Nama Ujian")%></label>
                                    <input type="text" id="filter-nama-ujian-<%=rnd%>" class="form-control input-elegan-<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Ketikkan nama ujian...")%>">
                                </div>
                                <div class="col-md-6 col-lg-6 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Topik / Tema")%></label><input type="text" id="filter-topik-<%=rnd%>" class="form-control input-elegan-<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Kata kunci topik...")%>"></div>
                                ...
                                <div class="col-md-4 col-lg mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Metode")%></label><input type="text" id="filter-metode-<%=rnd%>" class="form-control input-elegan-<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Contoh: Daring, Luring")%>"></div>
                        ...
                        <div class="tab-pane fade" id="pane-fasilitas-<%=rnd%>" role="tabpanel">
                            <div class="row align-items-end">
                                ...
                                <div class="col-md-5 mb-3">
                                    <label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Rentang Tanggal Ujian")%></label>
    ...
    <div id="paging-top-<%=rnd%>" class="d-flex justify-content-center mb-4">
        <button id="btn-load-next-<%=rnd%>" class="btn btn-primary btn-sm rounded-pill px-4 py-2 shadow-sm font-weight-bold" style="display: none;">
            ...
            <span id="label-btn-next-<%=rnd%>"><%=Common.getBahasaConfig("Muat Ujian Selanjutnya")%></span> 
    ...
    <div id="timeline-container-<%=rnd%>" class="timeline-container position-relative">
        <div id="loading-init-<%=rnd%>" class="text-center text-secondary py-5 fade-in-<%=rnd%>">
            ...
            <p class="mb-0 font-weight-bold"><%=Common.getBahasaConfig("Sedang memuat data ujian...")%></p>
        </div>
    ...
    <div id="paging-bottom-<%=rnd%>" class="d-flex justify-content-center mt-4 mb-5">
        <button id="btn-load-back-<%=rnd%>" class="btn btn-outline-secondary btn-sm rounded-pill px-4 py-2 shadow-sm font-weight-bold bg-white" style="display: none;">
            ...
            <span id="label-btn-back-<%=rnd%>"><%=Common.getBahasaConfig("Muat Ujian Sebelumnya")%></span> 
...
(function() {
    ...
    var bangunQueryFilter<%=rnd%> = function() {
        var q = "";
        q += "&q_nama_ujian=" + getFilterVal<%=rnd%>("filter-nama-ujian-<%=rnd%>");
    ...
    async function muatLinimasa<%=rnd%>(arah, isFilterBaru) {
        if (isFilterBaru) {
            ...
            if(divContainer) {
                divContainer.innerHTML = '<div id="loading-init-<%=rnd%>" class="text-center text-secondary py-5 fade-in-<%=rnd%>"><i class="fas fa-circle-notch fa-spin fa-3x text-info mb-3"></i><p class="mb-0 font-weight-bold"><%=Common.getBahasaConfig("Sedang memuat data ujian...")%></p></div>' +
                                         '<div id="timeline-content-next-<%=rnd%>"></div>' +
                                         '<div id="timeline-content-prev-<%=rnd%>"></div>';
            }
        }
        ...
        try {
            ...
            var servletUrlApi<%=rnd%> = rootUrl<%=rnd%> + "/baru?hanya_tampil_jsp=true&p=elearning%2Fservices&s=_linimasa_ujian&mulai=" + (arah === 'next' ? startNext<%=rnd%> : startBack<%=rnd%>) + "&banyak=" + limit<%=rnd%> + parameterArah<%=rnd%> + queryFilter<%=rnd%>;
            ...
            var fetchPromises<%=rnd%> = arrayIdPermintaan<%=rnd%>.map(function(itemReq) {
                var urlHtml = rootUrl<%=rnd%> + "/baru?hanya_tampil_jsp=true&p=elearning&s=load_ujian&item=" + encodeURIComponent(itemReq);
            ...
            if (typeof tampilkanToast === "function") {
                tampilkanToast(error.message || "<%=Common.getBahasaConfig("Terjadi gangguan saat mengambil data riwayat ujian.")%>", 'bg-danger');
