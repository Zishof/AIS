...
<%
    ...
    // Variabel Header Modul Khusus Materi
    String faIcon = "fas fa-book-reader";
    String judulRaw = "Linimasa Materi & Berkas";
    ...
    try {
        ...
        e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit webapp/WEB-INF/baru/modul/elearning/materi.jsp:55");
    } finally {
        try { sessFilter.disconnect(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/materi.jsp:57");}
...
<div id="wrapper-linimasa-<%=rnd%>" class="container-fluid px-2 px-md-4 py-4">
        ...
        <div class="card border-0 shadow-sm rounded-lg" style="background-color: #ffffff;">
            <div class="card-body p-4">
                ...
                <ul class="nav nav-pills nav-pills-custom-<%=rnd%> mb-4 border-bottom pb-3" id="pills-tab-<%=rnd%>" role="tablist">
                    ...
                    <li class="nav-item"><a class="nav-link" onclick="gantiTab<%=rnd%>(event, 'pane-materi-<%=rnd%>')"><i class="fas fa-file-alt mr-1"></i> <%=Common.getBahasaConfig("Substansi Materi")%></a></li>
                ...
                <form id="form-filter-<%=rnd%>" onsubmit="event.preventDefault(); terapkanFilter<%=rnd%>();">
                    <div class="tab-content" id="pills-tabContent-<%=rnd%>">
                        ...
                        <div class="tab-pane fade" id="pane-materi-<%=rnd%>" role="tabpanel">
                            <div class="row align-items-end">
                                <div class="col-md-12 col-lg-6 mb-3">
                                    <label class="label-filter-<%=rnd%> text-primary"><%=Common.getBahasaConfig("Nama Berkas / Judul Materi")%></label>
                                    <input type="text" id="filter-nama-tugas-<%=rnd%>" class="form-control input-elegan-<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Ketikkan judul materi...")%>">
                                </div>
                                <div class="col-md-6 col-lg-6 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Topik Pembahasan (Pertemuan)")%></label><input type="text" id="filter-topik-<%=rnd%>" class="form-control input-elegan-<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Kata kunci topik...")%>"></div>
                                <div class="col-md-12 col-lg-12 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Keterangan Tambahan")%></label><input type="text" id="filter-catatan-<%=rnd%>" class="form-control input-elegan-<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Catatan materi...")%>"></div>
                            </div>
                        </div>
                        ...
                        <div class="tab-pane fade" id="pane-fasilitas-<%=rnd%>" role="tabpanel">
                            <div class="row align-items-end">
                                <div class="col-md-3 mb-3">
                                    <label class="label-filter-<%=rnd%> text-primary"><%=Common.getBahasaConfig("Pilihan Rentang Upload")%></label>
                                ...
                                <div class="col-md-5 mb-3">
                                    <label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Rentang Tanggal Upload Materi")%></label>
                                ...
                                <div class="col-md-4 mb-3">
                                    <label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Materi Pertemuan Ke-")%></label>
                                    <div class="rentang-group-<%=rnd%>">
    ...
    <!-- ── Filter Tipe Konten: Buku, Artikel, dst ─────────────────────────── -->
    <div class="d-flex align-items-center flex-wrap gap-2 mb-3" id="tipe-filter-bar-<%=rnd%>">
        <span class="small fw-bold text-muted me-1"><%=Common.getBahasaConfig("Tipe")%>:</span>
        <% String[][] tipeOpt = {
            {"semua",   "fas fa-th-large",        "Semua",       "btn-primary"},
            {"dokumen", "fas fa-file-alt",         "Dokumen",     "btn-outline-secondary"},
            {"buku",    "fas fa-book",             "Buku",        "btn-outline-secondary"},
            {"artikel", "fas fa-link",             "Artikel/URL", "btn-outline-secondary"},
            {"gambar",  "fas fa-image",            "Gambar",      "btn-outline-secondary"},
            {"video",   "fas fa-film",             "Video",       "btn-outline-secondary"},
            {"audio",   "fas fa-music",            "Audio",       "btn-outline-secondary"}
        };
        for (String[] tp : tipeOpt) { %>
        <button type="button"
            class="btn btn-sm <%=tp[3]%> rounded-pill fw-semibold tipe-btn-<%=rnd%>"
            data-tipe="<%=tp[0]%>"
            onclick="gantiTipeFilter<%=rnd%>(this)">
            <i class="<%=tp[1]%> me-1 mr-1"></i><%=Common.getBahasaConfig(tp[2])%>
        </button>
        <% } %>
    </div>
    ...
    <div id="paging-top-<%=rnd%>" class="d-flex justify-content-center mb-4">
        <button id="btn-load-next-<%=rnd%>" class="btn btn-primary btn-sm rounded-pill px-4 py-2 shadow-sm font-weight-bold" style="display: none;">
            ...
            <span id="label-btn-next-<%=rnd%>"><%=Common.getBahasaConfig("Muat Materi Terbaru")%></span> 
    ...
    <div id="timeline-container-<%=rnd%>" class="timeline-container position-relative">
        <div id="loading-init-<%=rnd%>" class="text-center text-secondary py-5 fade-in-<%=rnd%>">
            ...
            <p class="mb-0 font-weight-bold"><%=Common.getBahasaConfig("Sedang memuat data materi...")%></p>
        </div>
    ...
    <div id="paging-bottom-<%=rnd%>" class="d-flex justify-content-center mt-4 mb-5">
        <button id="btn-load-back-<%=rnd%>" class="btn btn-outline-secondary btn-sm rounded-pill px-4 py-2 shadow-sm font-weight-bold bg-white" style="display: none;">
            ...
            <span id="label-btn-back-<%=rnd%>"><%=Common.getBahasaConfig("Muat Materi Sebelumnya")%></span> 
...
(function() {
    ...
    window.ubahFakultas<%=rnd%> = function(isReset) {
        var elFakultas = document.getElementById("filter-fakultas-<%=rnd%>");
        if(elFakultas) {
            var fId = elFakultas.value;
            ...
            if(jSel) {
                ...
                dataJurusan<%=rnd%>.forEach(function(j) {
                    if (fId === "" || j.idFakultas === fId) {
                        var opt = document.createElement("option"); opt.value = j.id; opt.text = j.nama; jSel.add(opt);
                    }
                });
            }
        }
    ...
    window.ubahYayasan<%=rnd%> = function(isReset) {
        <% if (apakahSekolah && !isSivitas) { %>
        var elYayasan = document.getElementById("filter-yayasan-<%=rnd%>");
        if(elYayasan) {
            var yId = elYayasan.value;
            ...
            if(sSel) {
                ...
                dataSekolah<%=rnd%>.forEach(function(s) {
                    if (yId === "" || s.idYayasan === yId) {
                        var opt = document.createElement("option"); opt.value = s.id; opt.text = s.nama; sSel.add(opt);
                    }
                });
            }
        }
    ...
    var tipeAktif<%=rnd%> = "semua";

    window.gantiTipeFilter<%=rnd%> = function(btn) {
        tipeAktif<%=rnd%> = btn.getAttribute("data-tipe");
        document.querySelectorAll('.tipe-btn-<%=rnd%>').forEach(function(b) {
            b.className = b.className.replace(/btn-primary\b/, 'btn-outline-secondary');
        });
        btn.className = btn.className.replace(/btn-outline-secondary\b/, 'btn-primary');
        resetFilter<%=rnd%>();
    };
    ...
    var bangunQueryFilter<%=rnd%> = function() {
        var q = "";
        // Param tipe konten (Buku, Artikel, dst)
        q += "&q_tipe=" + encodeURIComponent(tipeAktif<%=rnd%>);
        // Param khusus Materi
    ...
    async function muatLinimasa<%=rnd%>(arah, isFilterBaru) {
        if (isFilterBaru) {
            ...
            if(divContainer) {
                divContainer.innerHTML = '<div id="loading-init-<%=rnd%>" class="text-center text-secondary py-5 fade-in-<%=rnd%>"><i class="fas fa-circle-notch fa-spin fa-3x text-info mb-3"></i><p class="mb-0 font-weight-bold"><%=Common.getBahasaConfig("Sedang memuat data materi...")%></p></div>' +
                                         '<div id="timeline-content-next-<%=rnd%>"></div>' +
                                         '<div id="timeline-content-prev-<%=rnd%>"></div>';
            }
        }
        ...
        try {
            ...
            // Target Endpoint: _linimasa_materi
            var servletUrlApi<%=rnd%> = rootUrl<%=rnd%> + "/baru?hanya_tampil_jsp=true&p=elearning%2Fservices&s=_linimasa_materi&mulai=" + (arah === 'next' ? startNext<%=rnd%> : startBack<%=rnd%>) + "&banyak=" + limit<%=rnd%> + parameterArah<%=rnd%> + queryFilter<%=rnd%>;
            
            var responApi<%=rnd%> = await fetch(servletUrlApi<%=rnd%>);
            if (!responApi<%=rnd%>.ok) throw new Error('<%=Common.getBahasaConfigJS("Terjadi kesalahan HTTP.")%>');
            ...
            // Target Loader: load_materi
            var fetchPromises<%=rnd%> = arrayIdPermintaan<%=rnd%>.map(function(itemReq) {
                var urlHtml = rootUrl<%=rnd%> + "/baru?hanya_tampil_jsp=true&p=elearning&s=load_materi&item=" + encodeURIComponent(itemReq);
                return fetch(urlHtml).then(function(res) {
                    if (!res.ok) throw new Error('<%=Common.getBahasaConfigJS("Gagal merender kartu materi.")%>');
                    return res.text();
                });
            });
            ...
            console.error("<%=Common.getBahasaConfig("Error di muatLinimasa:")%>", error);
            if (typeof tampilkanToast === "function") {
                tampilkanToast(error.message || "<%=Common.getBahasaConfig("Terjadi gangguan jaringan.")%>", 'bg-danger');
