...
try {
	...
	try {
		...
		for (Object o : list) {
			CapaianPembelajaranLulusan cp = (CapaianPembelajaranLulusan) o;
	...
	if (daftar.length() == 0) daftar.append("(Belum ada CPMK terdaftar)\n");

	// Prompt (mirror ZK tampilGenerateAiSeleksi CPMK) + konteks OBE
	...
	p.append("Daftar Capaian Pembelajaran Matakuliah (CPMK) yang tersedia di program studi ini:\n").append(daftar).append("\n");
	p.append("Tolong analisis: Capaian Pembelajaran Matakuliah (CPMK) mana yang paling cocok untuk matakuliah ini?\n");
	p.append("Juga usulkan Capaian Pembelajaran Matakuliah (CPMK) BARU jika relevan tapi belum ada di daftar.\n\n");
	...
	p.append("USUL_BARU:\n- [KODE_BARU]: [rumusan CPMK baru yang disarankan]\n(tulis TIDAK ADA jika tidak ada usulan baru)");
	...
	String csv = mkDb.getCapaianPembelajaranLulusan();
	...
	for (String[] ub : sel.baru) {
		...
		Long id = ObeAiJspHelper.buatCapaianPembelajaran(s, mkDb, pt, ub[0], ub[1]);
		csv = ObeAiJspHelper.appendId(csv, id); nBaru++;
	}
	mkDb.setCapaianPembelajaranLulusan(csv);
	s.update(mkDb);
	s.flush();
	tx.commit();
	...
	out.print("{\"status\":\"success\",\"cocok\":" + nCocok + ",\"baru\":" + nBaru
			+ ",\"message\":\"" + esc(nCocok + " CPMK cocok ditambahkan, " + nBaru + " CPMK baru dibuat via AI.") + "\"}");
} catch (Exception e) {
	if (tx != null && tx.isActive()) { try { tx.rollback(); } catch (Exception ex) {} }
	e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit webapp/WEB-INF/baru/modul/elearning/obe/_generate_cpmk_ai.jsp");
