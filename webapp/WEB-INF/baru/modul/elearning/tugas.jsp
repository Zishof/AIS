...
<%
    ...
    // Variabel Header Modul Khusus Tugas
    String faIcon = "fas fa-clipboard-list";
    String judulRaw = "Linimasa Tugas";
    ...
    try {
        ...
        e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit webapp/WEB-INF/baru/modul/elearning/tugas.jsp:47");
    } finally {
        try { sessFilter.disconnect(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/tugas.jsp:49");}
...
<div id="wrapper-linimasa-<%=rnd%>" class="container-fluid px-2 px-md-4 py-4">
                        ...
                        <div class="tab-pane fade" id="pane-materi-<%=rnd%>" role="tabpanel">
                            <div class="row align-items-end">
                                <div class="col-md-12 col-lg-6 mb-3">
                                    <label class="label-filter-<%=rnd%> text-primary"><%=Common.getBahasaConfig("Judul Tugas")%></label>
                        ...
                        <div class="tab-pane fade" id="pane-fasilitas-<%=rnd%>" role="tabpanel">
                            <div class="row align-items-end">
                                ...
                                <div class="col-md-5 mb-3">
                                    <label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Rentang Tanggal Tugas")%></label>
    ...
    <div id="paging-top-<%=rnd%>" class="d-flex justify-content-center mb-4">
        <button id="btn-load-next-<%=rnd%>" class="btn btn-primary btn-sm rounded-pill px-4 py-2 shadow-sm font-weight-bold" style="display: none;">
            ...
            <span id="label-btn-next-<%=rnd%>"><%=Common.getBahasaConfig("Muat Tugas Selanjutnya")%></span> 
    ...
    <div id="timeline-container-<%=rnd%>" class="timeline-container position-relative">
        <div id="loading-init-<%=rnd%>" class="text-center text-secondary py-5 fade-in-<%=rnd%>">
            ...
            <p class="mb-0 font-weight-bold"><%=Common.getBahasaConfig("Sedang memuat data tugas...")%></p>
        </div>
    ...
    <div id="paging-bottom-<%=rnd%>" class="d-flex justify-content-center mt-4 mb-5">
        <button id="btn-load-back-<%=rnd%>" class="btn btn-outline-secondary btn-sm rounded-pill px-4 py-2 shadow-sm font-weight-bold bg-white" style="display: none;">
            ...
            <span id="label-btn-back-<%=rnd%>"><%=Common.getBahasaConfig("Muat Tugas Sebelumnya")%></span> 
...
(function() {
    ...
    async function muatLinimasa<%=rnd%>(arah, isFilterBaru) {
        if (isFilterBaru) {
            ...
            if(divContainer) {
                divContainer.innerHTML = '<div id="loading-init-<%=rnd%>" class="text-center text-secondary py-5 fade-in-<%=rnd%>"><i class="fas fa-circle-notch fa-spin fa-3x text-info mb-3"></i><p class="mb-0 font-weight-bold"><%=Common.getBahasaConfig("Sedang memuat data tugas...")%></p></div>' +
                                         '<div id="timeline-content-next-<%=rnd%>"></div>' +
                                         '<div id="timeline-content-prev-<%=rnd%>"></div>';
            }
        }
        ...
        try {
            ...
            var servletUrlApi<%=rnd%> = rootUrl<%=rnd%> + "/baru?hanya_tampil_jsp=true&p=elearning%2Fservices&s=_linimasa_tugas&mulai=" + (arah === 'next' ? startNext<%=rnd%> : startBack<%=rnd%>) + "&banyak=" + limit<%=rnd%> + parameterArah<%=rnd%> + queryFilter<%=rnd%>;
            ...
            var fetchPromises<%=rnd%> = arrayIdPermintaan<%=rnd%>.map(function(itemReq) {
                var urlHtml = rootUrl<%=rnd%> + "/baru?hanya_tampil_jsp=true&p=elearning&s=load_tugas&item=" + encodeURIComponent(itemReq);
            ...
            if (typeof tampilkanToast === "function") {
                tampilkanToast(error.message || "<%=Common.getBahasaConfig("Terjadi gangguan saat mengambil data riwayat tugas.")%>", 'bg-danger');
