...
String mulai = request.getParameter("mulai") == null ? "0" : request.getParameter("mulai");
String banyak = request.getParameter("banyak") == null ? "10" : request.getParameter("banyak");

int startIdx = Integer.parseInt(mulai);
int limitRows = Integer.parseInt(banyak);

// Parameter Filter Pencarian
String q_nama_ujian = request.getParameter("q_nama_ujian");
...
if (refDateStr != null && !refDateStr.trim().isEmpty()) {
    try { refDate = new Date(Long.parseLong(refDateStr)); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_ujian.jsp:98");}
}
...
try {
    ...
    if (q_jurusan != null && !q_jurusan.trim().isEmpty()) {
        try { listJurusanIds = new ArrayList<Long>(); listJurusanIds.add(Long.parseLong(q_jurusan)); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_ujian.jsp:107");}
        ...
        } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_ujian.jsp:116");}
    }

    List<Long> listSekolahIds = null;
    if (q_sekolah != null && !q_sekolah.trim().isEmpty()) {
        try { listSekolahIds = new ArrayList<Long>(); listSekolahIds.add(Long.parseLong(q_sekolah)); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_ujian.jsp:121");}
        ...
        } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_ujian.jsp:130");}
    }
    ...
    if (q_dosen != null && !q_dosen.trim().isEmpty()) {
        ...
        if (ids != null && !ids.isEmpty()) { djDosen = Restrictions.disjunction(); for (Long id : ids) djDosen.add(Restrictions.like("pertemuan.dosens", "%," + id + ",%")); } 
    ...
    if (q_mahasiswa != null && !q_mahasiswa.trim().isEmpty()) {
        ...
        if (ids != null && !ids.isEmpty()) { djMhs = Restrictions.disjunction(); for (Long id : ids) djMhs.add(Restrictions.like("pertemuan.mahasiswas", "%," + id + ",%")); } 
        else kosongMhs = true;
    }
    ...
    if (q_guru != null && !q_guru.trim().isEmpty()) {
        ...
        if (ids != null && !ids.isEmpty()) { djGuru = Restrictions.disjunction(); for (Long id : ids) djGuru.add(Restrictions.like("pertemuan.gurus", "%," + id + ",%")); } 
    ...
    if (q_siswa != null && !q_siswa.trim().isEmpty()) {
        ...
        if (ids != null && !ids.isEmpty()) { djSiswa = Restrictions.disjunction(); for (Long id : ids) djSiswa.add(Restrictions.like("pertemuan.siswas", "%," + id + ",%")); } 
        else kosongSiswa = true;
    }

    Criteria criteriaList = sess.createCriteria(PertemuanPunyaUjian.class).createAlias("pertemuan", "pertemuan");
    Criteria criteriaCountNext = sess.createCriteria(PertemuanPunyaUjian.class).createAlias("pertemuan", "pertemuan");
    Criteria criteriaCountBack = sess.createCriteria(PertemuanPunyaUjian.class).createAlias("pertemuan", "pertemuan");
    ...
    for (Criteria c : allCriteria) {
        ...
        if (siswa != null || mahasiswa != null || dosen != null || guru != null) {
            ...
            if (siswa != null) djAkses.add(Restrictions.like("pertemuan.siswas", "%," + siswa.getId() + ",%"));
            if (mahasiswa != null) djAkses.add(Restrictions.like("pertemuan.mahasiswas", "%," + mahasiswa.getId() + ",%"));
            if (dosen != null) djAkses.add(Restrictions.like("pertemuan.dosens", "%," + dosen.getId() + ",%"));
            if (guru != null) djAkses.add(Restrictions.like("pertemuan.gurus", "%," + guru.getId() + ",%"));
            c.add(djAkses);
        }

        if (jurusan != null) {
            c.add(Restrictions.eq("pertemuan.jurusan", jurusan));
        } else if (fakultas != null) {
            c.createAlias("pertemuan.jurusan", "jStruk", CriteriaSpecification.LEFT_JOIN);
            c.add(Restrictions.eq("jStruk.fakultas", fakultas));
        }

        if (sekolah != null) {
            c.add(Restrictions.eq("pertemuan.sekolah", sekolah));
        } else if (yayasan != null) {
            c.createAlias("pertemuan.sekolah", "sStruk", CriteriaSpecification.LEFT_JOIN);
            c.add(Restrictions.eq("sStruk.yayasan", yayasan));
        }

        if (q_nama_ujian != null && !q_nama_ujian.trim().isEmpty()) {
            c.add(Restrictions.ilike("nama", q_nama_ujian, MatchMode.ANYWHERE));
        }

        // PENERAPAN FILTER SUMBER KEGIATAN (Dengan namespace "pertemuan." karena ini ada di tabel Pertemuan)
        ...
        if (q_ruang != null && !q_ruang.trim().isEmpty()){
            c.createAlias("pertemuan.ruang", "ruangAlias", CriteriaSpecification.LEFT_JOIN); 
            c.add(Restrictions.or(
        ...
        if (q_ke_mulai != null && !q_ke_mulai.trim().isEmpty()) {
            try { c.add(Restrictions.ge("pertemuan.pertemuanKe", Integer.parseInt(q_ke_mulai))); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_ujian.jsp:273");}
        }
        if (q_ke_selesai != null && !q_ke_selesai.trim().isEmpty()) {
            try { c.add(Restrictions.le("pertemuan.pertemuanKe", Integer.parseInt(q_ke_selesai))); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_ujian.jsp:276");}
        }

        if (q_tgl_mulai != null && !q_tgl_mulai.trim().isEmpty()) {
            try { c.add(Restrictions.ge("mulaiUjian", sdf.parse(q_tgl_mulai))); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_ujian.jsp:280");}
        }
        if (q_tgl_selesai != null && !q_tgl_selesai.trim().isEmpty()) {
            try { 
                ...
                c.add(Restrictions.le("mulaiUjian", tglSelesai)); 
            } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_ujian.jsp:287");}
        }
    }
    ...
    // PEMBELAHAN MUTLAK refDate BERDASARKAN mulaiUjian
    if ("prev".equalsIgnoreCase(dir)) {
        criteriaList.add(Restrictions.lt("mulaiUjian", refDate));
        criteriaList.addOrder(Order.desc("mulaiUjian"));
    } else {
        criteriaList.add(Restrictions.ge("mulaiUjian", refDate));
        criteriaList.addOrder(Order.asc("mulaiUjian"));
    }
    ...
    // 5. Eksekusi Perhitungan Paginasi (Selalu menggunakan batasan waktu 'sekarang')
    criteriaCountNext.setProjection(Projections.rowCount());
    criteriaCountNext.add(Restrictions.ge("mulaiUjian", sekarang));
    Long countNext = (Long) criteriaCountNext.uniqueResult();

    criteriaCountBack.setProjection(Projections.rowCount());
    criteriaCountBack.add(Restrictions.lt("mulaiUjian", sekarang));
    ...
    e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit webapp/WEB-INF/baru/modul/elearning/services/_linimasa_ujian.jsp:325");
    responseJson.put("data", new JSONArray());
    responseJson.put("countNext", 0);
    responseJson.put("countBack", 0);
} finally {
    try { sess.disconnect(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_ujian.jsp:330");}
