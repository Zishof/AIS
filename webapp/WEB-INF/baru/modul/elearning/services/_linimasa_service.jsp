...
if (refDateStr != null && !refDateStr.trim().isEmpty()) {
    try { refDate = new Date(Long.parseLong(refDateStr)); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_service.jsp:94");}
}
...
try {
    ...
    if (q_jurusan != null && !q_jurusan.trim().isEmpty()) {
        try { listJurusanIds = new ArrayList<Long>(); listJurusanIds.add(Long.parseLong(q_jurusan)); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_service.jsp:103");}
        ...
        } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_service.jsp:112");}
    }

    List<Long> listSekolahIds = null;
    if (q_sekolah != null && !q_sekolah.trim().isEmpty()) {
        try { listSekolahIds = new ArrayList<Long>(); listSekolahIds.add(Long.parseLong(q_sekolah)); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_service.jsp:117");}
        ...
        } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_service.jsp:126");}
    }
    ...
    if (q_dosen != null && !q_dosen.trim().isEmpty()) {
        ...
        if (ids != null && !ids.isEmpty()) { djDosen = Restrictions.disjunction(); for (Long id : ids) djDosen.add(Restrictions.like("dosens", "%," + id + ",%")); } 
    ...
    if (q_mahasiswa != null && !q_mahasiswa.trim().isEmpty()) {
        ...
        if (ids != null && !ids.isEmpty()) { djMhs = Restrictions.disjunction(); for (Long id : ids) djMhs.add(Restrictions.like("mahasiswas", "%," + id + ",%")); } 
        else kosongMhs = true;
    }
    ...
    if (q_guru != null && !q_guru.trim().isEmpty()) {
        ...
        if (ids != null && !ids.isEmpty()) { djGuru = Restrictions.disjunction(); for (Long id : ids) djGuru.add(Restrictions.like("gurus", "%," + id + ",%")); } 
    ...
    if (q_siswa != null && !q_siswa.trim().isEmpty()) {
        ...
        if (ids != null && !ids.isEmpty()) { djSiswa = Restrictions.disjunction(); for (Long id : ids) djSiswa.add(Restrictions.like("siswas", "%," + id + ",%")); } 
        else kosongSiswa = true;
    }

    Criteria criteriaList = sess.createCriteria(Pertemuan.class);
    Criteria criteriaCountNext = sess.createCriteria(Pertemuan.class);
    Criteria criteriaCountBack = sess.createCriteria(Pertemuan.class);
    ...
    for (Criteria c : allCriteria) {
        ...
        if (siswa != null || mahasiswa != null || dosen != null || guru != null) {
            ...
            if (siswa != null) djAkses.add(Restrictions.like("siswas", "%," + siswa.getId() + ",%"));
            if (mahasiswa != null) djAkses.add(Restrictions.like("mahasiswas", "%," + mahasiswa.getId() + ",%"));
            if (dosen != null) djAkses.add(Restrictions.like("dosens", "%," + dosen.getId() + ",%"));
            if (guru != null) djAkses.add(Restrictions.like("gurus", "%," + guru.getId() + ",%"));
            c.add(djAkses);
        }
        ...
        // PENERAPAN FILTER SUMBER KEGIATAN
        ...
        if (q_ke_mulai != null && !q_ke_mulai.trim().isEmpty()) { try { c.add(Restrictions.ge("pertemuanKe", Integer.parseInt(q_ke_mulai))); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_service.jsp:229");} }
        if (q_ke_selesai != null && !q_ke_selesai.trim().isEmpty()) { try { c.add(Restrictions.le("pertemuanKe", Integer.parseInt(q_ke_selesai))); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_service.jsp:230");} }

        if (q_tgl_mulai != null && !q_tgl_mulai.trim().isEmpty()) { try { c.add(Restrictions.ge("tanggal", sdf.parse(q_tgl_mulai))); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_service.jsp:232");} }
        if (q_tgl_selesai != null && !q_tgl_selesai.trim().isEmpty()) { try { Date tSelesai = sdf.parse(q_tgl_selesai); tSelesai.setHours(23); tSelesai.setMinutes(59); tSelesai.setSeconds(59); c.add(Restrictions.le("tanggal", tSelesai)); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_service.jsp:233");} }
    }
    ...
    if ("prev".equalsIgnoreCase(dir)) {
        criteriaList.add(Restrictions.lt("tanggal", refDate));
        criteriaList.addOrder(Order.desc("tanggal"));
    } else {
        criteriaList.add(Restrictions.ge("tanggal", refDate));
        criteriaList.addOrder(Order.asc("tanggal"));
    }
 ...
 List<Long> pertemuns = criteriaList.list();
    ...
    criteriaCountNext.add(Restrictions.ge("tanggal", sekarang));
    Long countNext = (Long) criteriaCountNext.uniqueResult();

    criteriaCountBack.setProjection(Projections.rowCount());
    criteriaCountBack.add(Restrictions.lt("tanggal", sekarang));
    ...
    e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit webapp/WEB-INF/baru/modul/elearning/services/_linimasa_service.jsp:271");
    responseJson.put("data", new JSONArray());
    responseJson.put("countNext", 0);
    responseJson.put("countBack", 0);
} finally {
    try { sess.disconnect(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_service.jsp:276");}
