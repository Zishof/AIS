...
<%
    ...
    // Parameter Filter Khusus Video
    String q_nama_video = request.getParameter("q_nama_video"); 
    ...
    try {
        ...
        if (needPertemuanQuery) {
            ...
            if (q_jurusan != null && !q_jurusan.trim().isEmpty()) {
                try { cPertemuan.add(Restrictions.eq("jurusan.id", Long.parseLong(q_jurusan))); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_video.jsp:113");}
            } else if (q_fakultas != null && !q_fakultas.trim().isEmpty()) {
                cPertemuan.createAlias("jurusan", "jStruk", CriteriaSpecification.LEFT_JOIN);
                try { cPertemuan.add(Restrictions.eq("jStruk.fakultas.id", Long.parseLong(q_fakultas))); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_video.jsp:116");}
            }

            if (q_sekolah != null && !q_sekolah.trim().isEmpty()) {
                try { cPertemuan.add(Restrictions.eq("sekolah.id", Long.parseLong(q_sekolah))); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_video.jsp:120");}
            } else if (q_yayasan != null && !q_yayasan.trim().isEmpty()) {
                cPertemuan.createAlias("sekolah", "sStruk", CriteriaSpecification.LEFT_JOIN);
                try { cPertemuan.add(Restrictions.eq("sStruk.yayasan.id", Long.parseLong(q_yayasan))); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_video.jsp:123");}
            }
        ...
        // =========================================================================
        // LANGKAH 2: TARIK DATA VIDEO DARI DATABASE STREAMING
        // =========================================================================
        ...
        if (needPertemuanQuery) {
            ...
            executeFileQuery = true; // Langsung eksekusi karena BYPASS
        }

        if (executeFileQuery) {
            Criteria cFile = sessStream.createCriteria(VideoPertemuan.class);
            Criteria cCount = sessStream.createCriteria(VideoPertemuan.class);
            ...
            // Filter Nama Video
            if (q_nama_video != null && !q_nama_video.trim().isEmpty()) {
                cFile.add(Restrictions.ilike("nama", q_nama_video, MatchMode.ANYWHERE));
                cCount.add(Restrictions.ilike("nama", q_nama_video, MatchMode.ANYWHERE));
            }
        ...
        e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit webapp/WEB-INF/baru/modul/elearning/services/_linimasa_video.jsp:192");
        responseJson.put("data", new JSONArray());
        responseJson.put("countNext", 0);
        responseJson.put("countBack", 0);
    } finally {
        try { sessMain.disconnect(); sessMain.close(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_video.jsp:197");}
        try { sessStream.disconnect(); sessStream.close(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_video.jsp:198");}
