...
<%
    ...
    // Parameter Filter
    String q_nama_materi = request.getParameter("q_nama_tugas"); // UI materi.jsp menggunakan id filter-nama-tugas
    String q_tipe = request.getParameter("q_tipe"); // semua|dokumen|buku|artikel|gambar|video|audio
    ...
    try {
        ...
        if (needPertemuanQuery) {
            ...
            if (isSivitas) {
                ...
                // Filter Institusi untuk Admin Level Menengah (Kajur, Dekan, Kepsek)
            ...
            // Filter Struktural dari UI (Fakultas, Jurusan, Sekolah, Yayasan)
            if (q_jurusan != null && !q_jurusan.trim().isEmpty()) {
                try { cPertemuan.add(Restrictions.eq("jurusan.id", Long.parseLong(q_jurusan))); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_materi.jsp:114");}
            } else if (q_fakultas != null && !q_fakultas.trim().isEmpty()) {
                cPertemuan.createAlias("jurusan", "jStruk", CriteriaSpecification.LEFT_JOIN);
                try { cPertemuan.add(Restrictions.eq("jStruk.fakultas.id", Long.parseLong(q_fakultas))); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_materi.jsp:117");}
            }

            if (q_sekolah != null && !q_sekolah.trim().isEmpty()) {
                try { cPertemuan.add(Restrictions.eq("sekolah.id", Long.parseLong(q_sekolah))); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_materi.jsp:121");}
            } else if (q_yayasan != null && !q_yayasan.trim().isEmpty()) {
                cPertemuan.createAlias("sekolah", "sStruk", CriteriaSpecification.LEFT_JOIN);
                try { cPertemuan.add(Restrictions.eq("sStruk.yayasan.id", Long.parseLong(q_yayasan))); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_materi.jsp:124");}
            }
        ...
        // =========================================================================
        // LANGKAH 2: TARIK DATA FILE DARI DATABASE STREAMING
        // =========================================================================
        ...
        if (executeFileQuery) {
            Criteria cFile = sessStream.createCriteria(PertemuanFileContent.class);
            Criteria cCount = sessStream.createCriteria(PertemuanFileContent.class);
            ...
            // Filter Nama Materi
            if (q_nama_materi != null && !q_nama_materi.trim().isEmpty()) {
                cFile.add(Restrictions.ilike("nama", q_nama_materi, MatchMode.ANYWHERE));
                cCount.add(Restrictions.ilike("nama", q_nama_materi, MatchMode.ANYWHERE));
            }

            // Filter Tipe Konten (Buku, Artikel, Gambar, Video, Audio, Dokumen)
            if (q_tipe != null && !q_tipe.trim().isEmpty() && !q_tipe.equals("semua")) {
                if ("buku".equals(q_tipe)) {
                    cFile.add(Restrictions.ilike("fileMimeType", "%book%", MatchMode.ANYWHERE));
                    cCount.add(Restrictions.ilike("fileMimeType", "%book%", MatchMode.ANYWHERE));
                } else if ("artikel".equals(q_tipe)) {
                    cFile.add(Restrictions.sqlRestriction("link IS NOT NULL AND link <> ''"));
                    cCount.add(Restrictions.sqlRestriction("link IS NOT NULL AND link <> ''"));
                } else if ("gambar".equals(q_tipe)) {
                    cFile.add(Restrictions.ilike("fileMimeType", "image/%", MatchMode.START));
                    cCount.add(Restrictions.ilike("fileMimeType", "image/%", MatchMode.START));
                } else if ("video".equals(q_tipe)) {
                    cFile.add(Restrictions.ilike("fileMimeType", "video/%", MatchMode.START));
                    cCount.add(Restrictions.ilike("fileMimeType", "video/%", MatchMode.START));
                } else if ("audio".equals(q_tipe)) {
                    cFile.add(Restrictions.ilike("fileMimeType", "audio/%", MatchMode.START));
                    cCount.add(Restrictions.ilike("fileMimeType", "audio/%", MatchMode.START));
                } else if ("dokumen".equals(q_tipe)) {
                    Disjunction djDok = Restrictions.disjunction();
                    djDok.add(Restrictions.ilike("fileMimeType", "application/pdf", MatchMode.EXACT));
                    djDok.add(Restrictions.ilike("fileMimeType", "%word%", MatchMode.ANYWHERE));
                    djDok.add(Restrictions.ilike("fileMimeType", "%excel%", MatchMode.ANYWHERE));
                    djDok.add(Restrictions.ilike("fileMimeType", "%sheet%", MatchMode.ANYWHERE));
                    djDok.add(Restrictions.ilike("fileMimeType", "%presentation%", MatchMode.ANYWHERE));
                    djDok.add(Restrictions.ilike("fileMimeType", "%powerpoint%", MatchMode.ANYWHERE));
                    cFile.add(djDok);
                    cCount.add(djDok);
                }
            }

            // Paginasi & Eksekusi ID File
        ...
        e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit webapp/WEB-INF/baru/modul/elearning/services/_linimasa_materi.jsp:226");
        responseJson.put("data", new JSONArray());
        responseJson.put("countNext", 0);
        responseJson.put("countBack", 0);
    } finally {
        try { sessMain.disconnect(); sessMain.close(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_materi.jsp:231");}
        try { sessStream.disconnect(); sessStream.close(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/services/_linimasa_materi.jsp:232");}
