...
<%
    ...
    // Variabel Header Modul Khusus Video
    String faIcon = "fas fa-video";
    String judulRaw = "Linimasa Video Pembelajaran";
    ...
    // PENGUNCIAN FILTER INSTITUSIONAL
    ...
    try {
        ...
        e.printStackTrace(); ais.common.ErrorAuditUtil.record(e, "auto-audit webapp/WEB-INF/baru/modul/elearning/video.jsp:75");
    } finally {
        try { sessFilter.disconnect(); } catch (Exception e) { ais.common.ErrorAuditUtil.record(e, "auto-audit(empty-catch) webapp/WEB-INF/baru/modul/elearning/video.jsp:77");}
...
<style>
    ...
    .nav-pills-custom-<%=rnd%> .nav-link { color: #6c757d; font-weight: 600; border-radius: 0.5rem; margin-right: 0.5rem; margin-bottom: 0.5rem; background-color: #f8f9fa; transition: all 0.2s; cursor: pointer; padding: 10px 15px; }
    .nav-pills-custom-<%=rnd%> .nav-link:hover { background-color: #e9ecef; }
...
<div id="wrapper-linimasa-<%=rnd%>" class="container-fluid px-2 px-md-4 py-4">
        ...
        <div class="card border-0 shadow-sm rounded-lg" style="background-color: #ffffff;">
            <div class="card-body p-4">
                ...
                <ul class="nav nav-pills nav-pills-custom-<%=rnd%> mb-4 border-bottom pb-3" id="pills-tab-<%=rnd%>" role="tablist">
                    ...
                    <li class="nav-item"><a class="nav-link" onclick="gantiTab<%=rnd%>(event, 'pane-video-<%=rnd%>')"><i class="fas fa-film mr-1"></i> <%=Common.getBahasaConfig("Substansi Video")%></a></li>
                ...
                <form id="form-filter-<%=rnd%>" onsubmit="event.preventDefault(); terapkanFilter<%=rnd%>();">
                    <div class="tab-content" id="pills-tabContent-<%=rnd%>">
                        <div class="tab-pane fade show active" id="pane-akademik-<%=rnd%>" role="tabpanel">
                            <div class="row align-items-end">
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <select class="form-control input-elegan-<%=rnd%>" id="filter-fakultas-<%=rnd%>" onchange="ubahFakultas<%=rnd%>()" <%= (myFakultas != null || myJurusan != null) ? "disabled" : "" %>>
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <select class="form-control input-elegan-<%=rnd%>" id="filter-jurusan-<%=rnd%>" <%= (myJurusan != null) ? "disabled" : "" %>>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-control input-elegan-<%=rnd%>" id="filter-yayasan-<%=rnd%>" onchange="ubahYayasan<%=rnd%>()" <%= (myYayasan != null || mySekolah != null) ? "disabled" : "" %>>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-control input-elegan-<%=rnd%>" id="filter-sekolah-<%=rnd%>" <%= (mySekolah != null) ? "disabled" : "" %>>
                        ...
                        <div class="tab-pane fade" id="pane-video-<%=rnd%>" role="tabpanel">
                            <div class="row align-items-end">
                                <div class="col-md-12 col-lg-6 mb-3">
                                    <label class="label-filter-<%=rnd%> text-primary"><%=Common.getBahasaConfig("Nama Berkas / Judul Video")%></label>
                                    <input type="text" id="filter-nama-video-<%=rnd%>" class="form-control input-elegan-<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Ketikkan judul video...")%>">
                                </div>
                                ...
                                <div class="col-md-12 col-lg-12 mb-3"><label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Keterangan Tambahan")%></label><input type="text" id="filter-catatan-<%=rnd%>" class="form-control input-elegan-<%=rnd%>" placeholder="<%=Common.getBahasaConfig("Catatan video...")%>"></div>
                            </div>
                        </div>
                        ...
                        <div class="tab-pane fade" id="pane-fasilitas-<%=rnd%>" role="tabpanel">
                            <div class="row align-items-end">
                                ...
                                <div class="col-md-5 mb-3">
                                    <label class="label-filter-<%=rnd%>"><%=Common.getBahasaConfig("Rentang Tanggal Upload Video")%></label>
    ...
    <div id="paging-top-<%=rnd%>" class="d-flex justify-content-center mb-4">
        <button id="btn-load-next-<%=rnd%>" class="btn btn-primary btn-sm rounded-pill px-4 py-2 shadow-sm font-weight-bold" style="display: none;">
            ...
            <span id="label-btn-next-<%=rnd%>"><%=Common.getBahasaConfig("Muat Video Terbaru")%></span> 
    ...
    <div id="timeline-container-<%=rnd%>" class="timeline-container position-relative">
        <div id="loading-init-<%=rnd%>" class="text-center text-secondary py-5 fade-in-<%=rnd%>">
            ...
            <p class="mb-0 font-weight-bold"><%=Common.getBahasaConfig("Sedang memuat data video...")%></p>
        </div>
    ...
    <div id="paging-bottom-<%=rnd%>" class="d-flex justify-content-center mt-4 mb-5">
        <button id="btn-load-back-<%=rnd%>" class="btn btn-outline-secondary btn-sm rounded-pill px-4 py-2 shadow-sm font-weight-bold bg-white" style="display: none;">
            ...
            <span id="label-btn-back-<%=rnd%>"><%=Common.getBahasaConfig("Muat Video Sebelumnya")%></span> 
...
(function() {
    ...
    var bangunQueryFilter<%=rnd%> = function() {
        var q = "";
        // Param khusus Video
        q += "&q_nama_video=" + getFilterVal<%=rnd%>("filter-nama-video-<%=rnd%>");
    ...
    async function muatLinimasa<%=rnd%>(arah, isFilterBaru) {
        if (isFilterBaru) {
            ...
            if(divContainer) {
                divContainer.innerHTML = '<div id="loading-init-<%=rnd%>" class="text-center text-secondary py-5 fade-in-<%=rnd%>"><i class="fas fa-circle-notch fa-spin fa-3x text-info mb-3"></i><p class="mb-0 font-weight-bold"><%=Common.getBahasaConfig("Sedang memuat data video...")%></p></div>' +
                                         '<div id="timeline-content-next-<%=rnd%>"></div>' +
                                         '<div id="timeline-content-prev-<%=rnd%>"></div>';
            }
        }
        ...
        try {
            ...
            // Target Endpoint: _linimasa_video
            var servletUrlApi<%=rnd%> = rootUrl<%=rnd%> + "/baru?hanya_tampil_jsp=true&p=elearning%2Fservices&s=_linimasa_video&mulai=" + (arah === 'next' ? startNext<%=rnd%> : startBack<%=rnd%>) + "&banyak=" + limit<%=rnd%> + parameterArah<%=rnd%> + queryFilter<%=rnd%>;
            ...
            // Target Loader: load_video
            var fetchPromises<%=rnd%> = arrayIdPermintaan<%=rnd%>.map(function(itemReq) {
                var urlHtml = rootUrl<%=rnd%> + "/baru?hanya_tampil_jsp=true&p=elearning&s=load_video&item=" + encodeURIComponent(itemReq);
                return fetch(urlHtml).then(function(res) {
                    if (!res.ok) throw new Error('<%=Common.getBahasaConfigJS("Gagal merender kartu video.")%>');
                    return res.text();
                });
            });
