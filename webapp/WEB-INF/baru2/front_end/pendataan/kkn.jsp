...
<div class="ais2-page-header">
  <div class="ais2-ph-icon"><i class="fa-solid fa-people-carry-box"></i></div>
  <div>
    <div class="ais2-breadcrumb"><a href="<%=rootCtx%>/baru2?modul=pendataan">Pendataan</a> <span class="sep"><i class="fa-solid fa-chevron-right" style="font-size:9px"></i></span> <span>KKN</span></div>
    <div class="ais2-ph-title">Kuliah Kerja Nyata (KKN)</div>
    <div class="ais2-ph-sub">Kelola program KKN, persyaratan, penilaian, dan pembagian kelompok</div>
  </div>
  <div class="ais2-ph-actions">
    <button class="ais2-btn ais2-btn-primary" onclick="bukaTambah()"><i class="fa-solid fa-plus"></i> Tambah Program KKN</button>
  </div>
</div>
...
<div class="ais2-filter-card">
  ...
  <div class="ais2-filter-row">
    <div class="ais2-filter-field"><label>Nama KKN</label><input type="text" id="f_nama" placeholder="Cari nama KKN..."></div>
    <div class="ais2-filter-field"><label>Fakultas</label>
      <select id="f_fak"><option value="">Semua</option></select>
    </div>
    <div class="ais2-filter-field"><label>Program Studi</label>
      <select id="f_prodi"><option value="">Semua</option></select>
    </div>
    <div class="ais2-filter-field"><label>Mahasiswa</label><input type="text" id="f_mhs" placeholder="Cari NIM/nama..."></div>
  </div>
</div>

<div class="ais2-tabs" id="tabKkn">
  <div class="ais2-tab active" onclick="pilihTab('program')"><i class="fa-solid fa-list"></i> Program KKN</div>
  ...
  <div class="ais2-tab" onclick="pilihTab('seleksi')"><i class="fa-solid fa-filter-circle-dollar"></i> Seleksi Penerima</div>
  <div class="ais2-tab" onclick="pilihTab('kelompok')"><i class="fa-solid fa-people-group"></i> Pembagian Kelompok</div>
</div>

<div class="ais2-tab-pane active" id="tab_program"><div id="tabelKKN"></div></div>
<div class="ais2-tab-pane" id="tab_persyaratan">
  <div class="ais2-empty"><i class="fa-solid fa-clipboard-list"></i><h4>Pilih Program KKN</h4><p>Klik tombol Persyaratan pada baris program KKN untuk mengelola persyaratan.</p></div>
</div>
<div class="ais2-tab-pane" id="tab_penilaian">
  <div class="ais2-empty"><i class="fa-solid fa-star"></i><h4>Pilih Program KKN</h4><p>Klik tombol Penilaian pada baris program KKN untuk mengelola komponen penilaian.</p></div>
</div>
<div class="ais2-tab-pane" id="tab_seleksi"><div id="tabelSeleksiKKN"></div></div>
<div class="ais2-tab-pane" id="tab_kelompok"><div id="tabelKelompokKKN"></div></div>
...
document.addEventListener('DOMContentLoaded',function(){
    muatFakultas();
    tbl=AIS2.Table.init({
        container:'#tabelKKN', modul:'pendataan', svc:'kkn_list',
        bisa:{tambah:false,ubah:true,hapus:true},
        kolom:[
            {field:'nama',     label:'Nama KKN', sort:true},
            ...
            {field:'tahunAkademik',label:'TA/Smt',lebar:'90px',sembunyiMobile:true},
            {field:'sks',       label:'SKS', lebar:'50px'}
        ],
        onUbah:function(d){bukaForm(d);},
        onHapus:function(id,cb){AIS2.Api.post('pendataan','kkn_hapus',{id:id}).then(function(r){cb(r&&r.ok,r&&r.msg);});},
...
function terapkan(){tbl&&tbl.setFilter({nama:document.getElementById('f_nama').value,fakultas:document.getElementById('f_fak').value,prodi:document.getElementById('f_prodi').value,mhs:document.getElementById('f_mhs').value});}
function reset(){['f_nama','f_mhs'].forEach(function(id){var el=document.getElementById(id);if(el)el.value='';});['f_fak','f_prodi'].forEach(function(id){var el=document.getElementById(id);if(el)el.selectedIndex=0;});terapkan();}
function pilihTab(k){
    document.querySelectorAll('#tabKkn .ais2-tab').forEach(function(t,i){t.classList.toggle('active',['program','persyaratan','penilaian','seleksi','kelompok'][i]===k);});
...
function bukaForm(d){
    ...
    var html='<input type="hidden" name="id" value="'+esc(d.id)+'">'
        +'<div class="ais2-form-section-title">Identitas Program KKN</div>'
        +'<div class="ais2-form-grid">'
        +'<div class="ais2-field ais2-form-full"><label>Nama KKN <span class="req">*</span></label><input class="ais2-input" name="nama" value="'+esc(d.nama)+'" required></div>'
        ...
        +'<div class="ais2-field"><label>Min. SKS Ditempuh</label><input class="ais2-input" type="number" name="minSks" value="'+esc(d.minSks)+'"></div>'
        ...
        +'</div>'
        +'<div class="ais2-form-section-title" style="margin-top:20px">Lingkup Program</div>'
        +'<div class="ais2-form-grid">'
        ...
        +'<div class="ais2-field"><label>Program</label><select name="program" class="ais2-input"><option value="">Semua</option><option>Reguler</option><option>Karyawan</option></select></div>'
        +'</div>'
    ...
    var modal=AIS2.Modal.buka({
        judul:d.id?'Ubah Program KKN':'Tambah Program KKN',ikon:'fa-people-carry-box',ukuran:'',
        konten:html,
        tombol:[
            {label:'Simpan',kelas:'ais2-btn-primary',onClick:function(){
                ...
                if(!data.nama){AIS2.Toast.peringatan('Nama KKN wajib diisi');return;}
                AIS2.Api.post('pendataan',d.id?'kkn_ubah':'kkn_tambah',data).then(function(r){
