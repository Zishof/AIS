<%-- @generated by generate_new_jsp_scaffold.py
Source: src/ais/action/master/helper/PenjaminanMutuAnalisisHelper.java
Generated: 2026-08-06T18:20:20Z
Safe service-adapter scaffold. Implement data access in a Java service layer, not directly in this JSP.
--%>
<%@ page language="java" contentType="application/json; charset=UTF-8" pageEncoding="UTF-8" %>
<%
request.setAttribute("nuiServiceModule", "helper");
request.setAttribute("nuiServicePage", "penjaminan_mutu_analisis_helper");
request.setAttribute("nuiServiceTitle", "Penjaminan Mutu Analisis Helper");
request.setAttribute("nuiServiceType", "dashboard");
request.setAttribute("nuiServiceSourceClass", "PenjaminanMutuAnalisisHelper");
request.setAttribute("nuiServiceSourcePackage", "ais.action.master.helper");
request.setAttribute("nuiServiceSourcePath", "src/ais/action/master/helper/PenjaminanMutuAnalisisHelper.java");
request.setAttribute("nuiServiceMethods", new String[]{"render", "onEvent", "getStatus"});
request.setAttribute("nuiServiceEntities", new String[]{"PertemuanPunyaUjian", "Perkuliahan"});
%>
<jsp:include page="/WEB-INF/new/_shared/services/dispatcher.jsp" />
