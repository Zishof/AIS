...
DELETE FROM rev11.customer_profile;
DELETE FROM rev11.produk;
DELETE FROM rev11.salesperson;
DELETE FROM rev11.customer;
DELETE FROM rev11.toko;

-- Urutan serial di-restart supaya id dokumennya tentu; berkas ini dijalankan berulang.
ALTER SEQUENCE rev11.sales_order_id_seq RESTART WITH 1;
ALTER SEQUENCE rev11.sales_order_detail_id_seq RESTART WITH 1;
ALTER SEQUENCE rev11.faktur_penjualan_id_seq RESTART WITH 1;
ALTER SEQUENCE rev11.piutang_customer_id_seq RESTART WITH 1;

INSERT INTO rev11.toko (id, nama, kode) VALUES (1, 'Toko Uji', 'T01');
...
INSERT INTO rev11.produk (id, kode, nama) VALUES (100, 'P100', 'Gula'), (101, 'P101', 'Kopi');
INSERT INTO rev11.customer_profile (customer_id, syarat_bayar_hari) VALUES (5, 30);
...
INSERT INTO rev11.sales_order (nomor_dokumen, tanggal, customer_id, salesperson_id, toko_id,
...
SELECT
  (SELECT COUNT(*) FROM rev11.sales_order o WHERE o.status = 'DRAFT')          AS "saringan mentah",
  (SELECT COUNT(*) FROM rev11.sales_order o
     WHERE CASE WHEN o.status = 'DRAF' THEN 'DRAFT'
                ELSE COALESCE(o.status,'') END = 'DRAFT')                      AS "ternormalkan",
  CASE WHEN (SELECT COUNT(*) FROM rev11.sales_order o WHERE o.status = 'DRAFT') = 0
        AND (SELECT COUNT(*) FROM rev11.sales_order o
               WHERE CASE WHEN o.status = 'DRAF' THEN 'DRAFT'
...
UPDATE rev11.sales_order SET nomor_dokumen = 'SO-1-000001', total = 250000 WHERE id = 1;

\echo ''
\echo '== BLOK 2: total header setara jumlah barisnya ====================================='

SELECT o.total AS "total header",
       (SELECT SUM(i.total) FROM rev11.sales_order_detail i WHERE i.sales_order_id = o.id)
         AS "jumlah baris",
       CASE WHEN o.total = (SELECT SUM(i.total) FROM rev11.sales_order_detail i
...
-- =====================================================================================
DELETE FROM rev11.sales_order_detail WHERE sales_order_id = 1;
INSERT INTO rev11.sales_order_detail
  (sales_order_id, baris_ke, produk_id, kuantitas, harga_satuan, total, dibuat_pada, oleh)
  VALUES (1, 1, 100, 4, 15000, 60000, now(), 'uji');
UPDATE rev11.sales_order SET tanggal = COALESCE(NULL, tanggal), keterangan = 'diubah',
...
SELECT
  (SELECT COUNT(*) FROM rev11.sales_order_detail WHERE sales_order_id = 1) AS "jumlah baris",
  o.total    AS "total",
  o.tanggal  AS "tanggal (harus tetap)",
  CASE WHEN (SELECT COUNT(*) FROM rev11.sales_order_detail WHERE sales_order_id = 1) = 1
...
-- =====================================================================================
UPDATE rev11.sales_order SET status = 'TERKIRIM' WHERE id = 1;

INSERT INTO rev11.faktur_penjualan (nomor_dokumen, nomor_faktur, tanggal, jatuh_tempo,
...
UPDATE rev11.faktur_penjualan SET nomor_dokumen = 'INV-1-000001', nomor_faktur = 'INV-1-000001'
  WHERE id = 1;
INSERT INTO rev11.piutang_customer (customer_id, salesperson_id, faktur_penjualan_id,
  ...
  VALUES (5, 7, 1, 'INV-1-000001', DATE '2026-03-05', DATE '2026-03-05' + 30, 60000, 0, 60000,
          'TERBUKA', now(), 'uji');
UPDATE rev11.sales_order SET status = 'SIAP_TAGIH', oleh = 'uji' WHERE id = 1;
...
SELECT
  ...
  CASE WHEN f.total = d.nilai AND d.nilai = 60000
        AND d.jatuh_tempo = DATE '2026-03-05' + (SELECT syarat_bayar_hari
                                                 FROM rev11.customer_profile WHERE customer_id = 5)
        AND o.status = 'SIAP_TAGIH'
        AND d.faktur_penjualan_id = f.id
       THEN 'LULUS' ELSE 'GAGAL' END AS hasil
FROM rev11.faktur_penjualan f, rev11.piutang_customer d, rev11.sales_order o
...
SELECT
  (SELECT d.id FROM rev11.piutang_customer d
  ...
  (SELECT COALESCE(d.nomor_faktur,'') FROM rev11.piutang_customer d
     JOIN rev11.faktur_penjualan f ON d.faktur_penjualan_id = f.id
    WHERE f.sales_order_id = 1 ORDER BY d.id LIMIT 1)                    AS "piutangDocNomor",
  CASE WHEN (SELECT COALESCE(d.nomor_faktur,'') FROM rev11.piutang_customer d
               JOIN rev11.faktur_penjualan f ON d.faktur_penjualan_id = f.id
...
INSERT INTO rev11.sales_order (nomor_dokumen, tanggal, customer_id, toko_id, total,
                               idempotency_key, status, dibuat_pada, oleh)
  VALUES ('', DATE '2026-03-09', 5, 1, 0, 'SO-UNIK-1', 'DRAF', now(), 'uji');
INSERT INTO rev11.faktur_penjualan (nomor_dokumen, nomor_faktur, tanggal, customer_id,
    total, idempotency_key, status, dibuat_pada, oleh)
  VALUES ('X', 'X', DATE '2026-03-09', 5, 1, 'SO-INV-1', 'AKTIF', now(), 'uji');
\set ON_ERROR_STOP on

SELECT
  (SELECT COUNT(*) FROM rev11.sales_order WHERE idempotency_key = 'SO-UNIK-1')      AS "order",
  (SELECT COUNT(*) FROM rev11.faktur_penjualan WHERE idempotency_key = 'SO-INV-1')  AS "faktur",
  CASE WHEN (SELECT COUNT(*) FROM rev11.sales_order WHERE idempotency_key = 'SO-UNIK-1') = 1
        AND (SELECT COUNT(*) FROM rev11.faktur_penjualan WHERE idempotency_key = 'SO-INV-1') = 1
