...
DELETE FROM rev11.sales_trip_setoran;
DELETE FROM rev11.sales_trip_hasil;
DELETE FROM rev11.sales_trip_nota;
DELETE FROM rev11.sales_trip_barang;
DELETE FROM rev11.sales_trip;
DELETE FROM rev11.surat_perintah_sales;
DELETE FROM rev11.produk;
DELETE FROM rev11.salesperson;
DELETE FROM rev11.gudang;
...
INSERT INTO rev11.gudang (id, kode, nama, toko_id) VALUES (1, 'G01', 'Gudang', 1);
...
INSERT INTO rev11.surat_perintah_sales (id, nomor_dokumen, tanggal, salesperson_id, gudang_id,
                                        status)
  VALUES (1, 'SPJ-001', DATE '2026-05-01', 7, 1, 'ACTIVE');
INSERT INTO rev11.sales_trip (id, nomor_dokumen, surat_perintah_sales_id, salesperson_id,
                              gudang_id, tanggal_berangkat, status, dibuat_pada, oleh)
  VALUES (1, 'TRIP-001', 1, 7, 1, DATE '2026-05-01', 'ACTIVE', now(), 'uji');
INSERT INTO rev11.sales_trip_barang (sales_trip_id, produk_id, kuantitas_bawa, dibuat_pada, oleh)
  VALUES (1, 100, 10, now(), 'uji'), (1, 101, 5, now(), 'uji');
INSERT INTO rev11.sales_trip_nota (sales_trip_id, nomor_nota, tanggal, total, tunai, kredit,
                                   dibuat_pada, oleh)
  VALUES (1, 'NOTA-1', DATE '2026-05-02', 1000000, 1000000, 0, now(), 'uji');
-- setoran: persis yang ditulis sisipSetoran()
INSERT INTO rev11.sales_trip_setoran (sales_trip_id, tanggal, cara_bayar, nomor_bukti, nilai,
...
FROM (SELECT COALESCE(SUM(k.nominal),0) AS saldo FROM koperasi.nota_sales_kas k
      WHERE k.sesi = 1) leg,
     (SELECT COALESCE((SELECT SUM(n.tunai) FROM rev11.sales_trip_nota n
                       WHERE n.sales_trip_id = 1), 0)
           - COALESCE((SELECT SUM(t.nilai) FROM rev11.sales_trip_setoran t
...
SELECT COALESCE((SELECT SUM(n.tunai) FROM rev11.sales_trip_nota n WHERE n.sales_trip_id = 1),0)
         AS "tanpa dikurangi setoran",
       CASE WHEN COALESCE((SELECT SUM(n.tunai) FROM rev11.sales_trip_nota n
...
FROM rev11.sales_trip_barang b
LEFT JOIN rev11.sales_trip_hasil h
...
INSERT INTO rev11.sales_trip_hasil (sales_trip_id, produk_id, kuantitas_terjual,
...
SELECT
  (SELECT COUNT(*) FROM koperasi.spj_sales_barang b WHERE b.spj = 1
    ...
    AS "sisa legacy",
  (SELECT COUNT(*) FROM rev11.sales_trip_barang b
     LEFT JOIN rev11.sales_trip_hasil h
            ON h.sales_trip_id = b.sales_trip_id AND h.produk_id = b.produk_id
    WHERE b.sales_trip_id = 1
      AND ABS(COALESCE(b.kuantitas_bawa,0) - COALESCE(h.kuantitas_terjual,0)
  ...
  CASE WHEN (SELECT COUNT(*) FROM koperasi.spj_sales_barang b WHERE b.spj = 1
        ...
        AND (SELECT COUNT(*) FROM rev11.sales_trip_barang b
               LEFT JOIN rev11.sales_trip_hasil h
                      ON h.sales_trip_id = b.sales_trip_id AND h.produk_id = b.produk_id
              WHERE b.sales_trip_id = 1
                AND ABS(COALESCE(b.kuantitas_bawa,0) - COALESCE(h.kuantitas_terjual,0)
...
-- =====================================================================================
UPDATE rev11.sales_trip SET status = 'RETURNED',
       tanggal_kembali = COALESCE(DATE '2026-05-03', tanggal_kembali),
       oleh = 'uji', tanggal_dirubah = now() WHERE id = 1;
UPDATE rev11.surat_perintah_sales SET status = 'RETURNED', tanggal_dirubah = now(),
       oleh = 'uji' WHERE id = 1;
UPDATE rev11.sales_trip SET status = 'RECONCILING',
       tanggal_kembali = COALESCE(NULL, tanggal_kembali),
       oleh = 'uji', tanggal_dirubah = now() WHERE id = 1;
UPDATE rev11.surat_perintah_sales SET status = 'RECONCILING', tanggal_dirubah = now(),
       oleh = 'uji' WHERE id = 1;
...
SELECT t.status AS "status trip", t.tanggal_kembali AS "tanggal kembali",
       ...
       CASE WHEN t.status = 'RECONCILING' AND t.tanggal_kembali = DATE '2026-05-03'
            ...
            THEN 'LULUS' ELSE 'GAGAL' END AS hasil
FROM rev11.sales_trip t JOIN rev11.surat_perintah_sales j
