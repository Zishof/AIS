...
`ais.database.model.config.SchemaConfig` (r85226) — dua entity yatim
total (nol Action/listener/pemanggil, hanya diri sendiri +
`hibernate.cfg.xml`, `svn log` cuma 1 revisi = impor awal repo tanpa
jejak pernah dipakai lalu dihapus). `SchemaConfig` khususnya:
satu-satunya entity di skema DB `config` (bukan `public`), kolom
`nama`/`schema_yang_digunakan` menyiratkan rancangan skema-per-tenant
yang ditinggalkan — arsitektur multi-tenant AIS yang benar-benar
dipakai berbasis KOLOM (`satuanKerja`/tenant key), bukan skema-per-
tenant; tidak ada `hibernate.multiTenancy` di mana pun. Kedua entity
diputuskan DIBIARKAN sebagai skema tidur (bukan dihapus) — risiko
migrasi destruktif tidak sepadan manfaatnya untuk entity yang tidak
punya beban performa/keamanan aktif.

**KOREKSI SCOPE**: grep pola persis `/** * Bank generated by hbm2java */`
(placeholder murni, beda dari sekadar menyebut frasa itu di prosa
javadoc yang sudah lengkap — banyak file "TUNTAS" tetap menyebut frasa
ini sebagai catatan historis) menemukan **250+ file** (batas hasil
grep tercapai, jumlah riil lebih tinggi) yang masih placeholder-only,
tersebar di: package ROOT `ais.database.model` (ratusan file lepas,
mis. `Agama`, `Jabatan`, `TemplateSurat`, dll — belum pernah disentuh
inisiatif ini sama sekali sebagai domain koheren) + package kecil yang
juga 100% belum disentuh: `config`, `sisdes`, `kedokteran`, `ojs`,
`sapto`, `obe`, `beasiswa`, `streaming`, `lkp`, `crm`(sebagian),
`kkn`(sebagian), `pkl`(sebagian), `radius`(sebagian), `spi`/`spmi`
(sebagian), `antarjemput`(sebagian), `sirkulasisurat`(sebagian).
Estimasi akumulasi "~37,5%" di milestone `repository` (batch 119)
kemungkinan HANYA mengukur file di 24 package domain yang sudah
di-"TUNTAS"-kan, BUKAN keseluruhan 7.401 file — perlu audit ulang
angka total di sesi mendatang. Package kecil `ticket` (3 file:
`Ticket`/`TicketKategori`/`TicketKomentar`, modul Ticketing Management
2025) ditemukan SUDAH punya javadoc substansial meski tak pernah
dicatat di file ini — kemungkinan ditulis manual di luar inisiatif,
bukan gap.
