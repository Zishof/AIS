...
Saat `PAYMENT` tiba, server mengambil PostgreSQL session advisory lock berdasarkan nomor transaksi BMT. Lock dibuat pada tingkat session, bukan transaction, karena proses harus melewati dua batas transaksi: menyimpan ledger lebih dahulu, lalu memanggil mesin pembayaran kanonik yang membuka transaksinya sendiri. Jika nomor transaksi sudah sukses untuk invoice, nominal, dan channel yang sama, retry mengembalikan hasil sukses tanpa posting kedua. Jika nomor transaksi yang sama dipakai untuk invoice, nominal, atau channel berbeda, request ditolak. `CHANNEL_BMT` disimpan di ledger, bukan sekadar divalidasi saat request masuk, agar audit dapat membedakan pembayaran teller, mobile nasabah, mobile petugas, mobile agen, dan virtual account secara deterministik.
