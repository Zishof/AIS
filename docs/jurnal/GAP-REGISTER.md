...
| JRN-B-005 | Sinkronisasi panel izin jurnal dan menu fisik role | `BLOCKED` | Deployment demo masih menolak generator walau checkbox panel jurnal terlihat aktif. Authorization mewajibkan `jurnal_akses_json` dan `job_has_menu`, sedangkan checkbox `Buka` panel jurnal belum otomatis menyinkronkan menu fisik. Reproduksi save-role, atomic sync, cache refresh, negative-role test, dan verifikasi DB deployment wajib sebelum ditutup |
