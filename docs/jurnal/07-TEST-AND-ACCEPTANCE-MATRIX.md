...
| Landing login/admin controls | anonymous, invalid CSRF/password, authenticated admin/non-admin rendering | PARTIAL_PASS: login AIS POST+CSRF dan generic failure diuji pada Tomcat/browser lokal; password tidak direfleksikan, mobile 390 px tanpa overflow. Tombol Import OJS serta create/delete sample dibatasi server-side `Common.getApakahAdmin()` dan service authorization. Deployment demo membuktikan render admin positif, tetapi aksi sample masih ditolak karena dual gate `job_has_menu` + `jurnal_akses_json` belum tersinkron |
| Sample dataset lifecycle | admin create 50×100, idempotency, scoped cleanup | PARTIAL_PASS: service/SIT namespace self-test membuat 50 jurnal/5.000 artikel/5.000 contributor lalu cleanup menghasilkan 50/5.000/5.000 removed dan 0 row tersisa; cleanup menolak dataset yang sudah memiliki dependency/activity. Journey positif melalui UI deployment demo masih blocked oleh provisioning/sinkronisasi izin role |
